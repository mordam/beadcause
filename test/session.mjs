...
//
// A session appears in four lists — /sessions, the advocate console's worker and
// "Claude sessions" rows, the Elsewhere card, and the mirror — and the detail behind it
// used to exist in exactly one of them, folded inline under the row on /sessions. So the
// same tap on the same session meant "show me what it is doing" in one place and nothing
// at all in the other three. `/session?pid=…` is that detail with an address, and this
// file pins the parts of the move that a reasonable refactor breaks silently:
//
...
const LISTS = ['work.js', 'monitor.js', 'mirror.js'];
