/*
  ...
  the Releases view bc-khoe.7 moved the deploy strip onto — and there will be roughly nine
  once bc-khoe.4 (Advocates, Mirror) has landed. A row that wraps to two lines on a
  360px phone is the exact thing this epic exists to stop — it spends a second row of a
  screen that is mostly chrome already. So the row takes the width it needs and the
  current pill is scrolled into view on load, which is the one moment the offset can be wrong without
