...
credentials for **[signing in with Google](#signing-in-with-google)**. Re-run them any
time with `npm run configure`; nothing is written until the last answer — not even the
client secret, which goes to a file of its own — so Ctrl+C is always safe.
