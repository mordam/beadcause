...
async function launch() {
  ...
  const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'beadcause-console-'));
  const proc = spawn(
    CHROME,
    [
      '--headless=new',
      ...
      // Offscreen, Chrome runs the renderer at about a frame a second, and a smooth
      // scroll that never gets frames never arrives anywhere.
