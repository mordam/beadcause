...
//
// What is *not* here any more is the deploy strip: what is being built and restarted
// right now, the four endings, and the daemon going away mid-restart. All of it moved to
// scripts/releases-check.mjs with the strip itself (bc-khoe.7), because the board no
// longer draws it. The release queue below — the Ship button and the count over it — is
// still this screen's and is still checked here.
//
...
function serve() {
  const server = http.createServer((req, res) => {
    ...
    /* Everything else, including `/api/deploys`: this page stopped asking for the deploy
       journal when the strip left it (bc-khoe.7), and a fixture that still answered it
       would be the one thing that could hide a poll nobody meant to leave behind. */
...
try {
  ...
  // Put the queue back *and reload*, because the screenshots below are of whatever is on
  // screen right now, and the Ship button with its count is the thing worth looking at.
  RELEASE = QUEUE;
