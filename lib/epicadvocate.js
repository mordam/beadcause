...
/**
 * The label that says this P0's advocate is paused — no more windows under it until
 ...
 * that occasionally erases the sentence that keeps the P0 enrolled would un-advocate the
 ...
 *
 * Deliberately **not** folded into `wantsAdvocate`. A paused P0 still *has* an advocate —
 * that is what makes it resumable, and it is why its section stays on the console with a
...
/**
 * Every paused P0 in a graph, as a Set of ids — what a queue filter needs.
 *
 ...
 * is not.
 *
 * Every P0 with the label, not only the ones `wantsAdvocate` accepts. A P0 that was
...
/**
 * Every P0 in a workspace that has an advocate assigned to it — the roster.
 *
 ...
 * a *window*, and a window lives minutes — so nineteen of the twenty P0s that had one
...
/**
 * How long a P0 card goes on saying an advocate is opening before it offers the button
 * back.
 *
...
/**
 * P0s an advocate was launched on here, by `workspace/id`, and when.
 *
...
/**
 * The session advocating this P0 — what the card needs to offer a way *in* to it.
 *
 ...
 * door already makes: one session per P0, whatever opened it. That is the same rule
 ...
 * closed and re-opened on the same P0 for weeks, and the only thing it was ever handed
 ...
 * window has to ask for, then read, to find the two lines that are about this P0.
 *
 ...
 * folding the children in is tempting — but the children's text is where a P0's own
...
export function epicAdvocatePrompt(workspace, p0, kids = [], plan = null, owner = 'the owner', extra = {}) {
  const { waiting = null, reason = '', notes = null, debriefs = [], corpus = null, gleaning = [] } = extra;
  ...
  if (!kids.length) {
    ...
    lines.push(
      ...
      `  all? One carrying \`unendorsed\` is waiting on ${ownerOf(p0) || owner} rather than on you.`,
      '- **A child stalled** — the tracker says `in_progress` while nothing on this Mac is in a window on it',
      '  and no other machine holds a lease. Look for a branch or an open pull request carrying its work',
      '  before you touch it: if there is one, the work exists and the bead is where it should be. If there',
  ...
  const learned = notesBrief(notes || {}, p0, { who: 'P0 advocate' });
  ...
  // parent, a root P0 has none, so what arrives is the reports of previous runs *at this
  // P0* rather than its children's. That is the right narrowing twice over: it is this
  ...
  const past = debriefBrief(debriefs || [], p0);
  if (past) lines.push(past);

  // bc-fvmx. After what earlier sessions worked out and before the closing instructions,
  // because it is a fact about the P0 rather than something to do first — and because an
  ...
  lines.push(
    ...
    'progressing look identical without it — and it is also what keeps you being re-opened: the sweep',
    'that brings you back on a child closing, being filed or stalling only does so for a P0 whose notes',
    'carry that block. Erase it and this P0 needs somebody to press a button again.',
    '',
    ...
    'That is what the next window on this P0 — yours or a planner’s — is handed before it plans',
    'anything, and you are the only run that will ever be able to write it.',
    '',
    `**And if what this P0 needs is a decision only ${owner} can make, that is not a note to yourself.** File`,
    `it under ${p0.id} as a bead labelled \`human\` carrying a \`decision\` block — the question, the options,`,
    ...
    `${owner} agrees to it. You may not change the priority or the owner of ${p0.id}: those are the two`,
    'facts the board is built out of, and they are theirs. And you may not close anything — a work bead',
    'closes when its merge lands and the merge queue is what closes it, and this P0 closes when its theme',
    'is done, which is a call only they can make. If you think it is finished, say so in the sentence above',
    'and leave the close to them.'
  );
  return lines.join('\n');
}
