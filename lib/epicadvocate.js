...
export function epicAdvocatePrompt(workspace, epic, kids = [], plan = null, owner = 'the owner', extra = {}) {
  ...
  lines.push(
    ...
    'that brings you back on a child closing, being filed or stalling only does so for an epic whose notes',
    'carry that block. Erase it and this epic needs somebody to press a button again.',
    '',
    ...
    'bc-htoy an epic needs no particular priority to have you, so there is nothing to reach for. And you may',
    'not close anything — a work bead closes when its merge lands and the merge queue is what closes it, and',
    'this epic closes when its theme is done, which is a call only they can make. If you think it is finished,',
    'say so in the sentence above and leave the close to them.'
  );
  return lines.join('\n');
}
