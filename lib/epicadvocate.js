...
export function epicAdvocatePrompt(workspace, epic, kids = [], plan = null, owner = 'the owner', extra = {}) {
  ...
  lines.push(
    ...
    `${owner} agrees to it. And you may not change the priority or the owner of ${epic.id}: the owner is`,
    'what the board is built out of and the priority is their statement of how urgent this is — both theirs,',
    'and since bc-htoy an epic needs no particular priority to have you, so there is nothing to reach for.'
  );
  return lines.join('\n');
}
