...
// bc-jvt0.4: the childless branch of the brief hands over the exact block and label that
// record "one job", so the two writers of that document cannot drift. lib/plan.js imports
// nothing of this file's, so there is no cycle — and the format living there rather than
// here is deliberate: bin/plan.js validates against it and this only quotes it.
import { MIN_WHY_CHARS, WHOLE_CLOSE, WHOLE_LABEL, WHOLE_OPEN } from './plan.js';
...
export function epicAdvocatePrompt(workspace, epic, kids = [], plan = null, owner = 'the owner', extra = {}) {
  ...
  if (!kids.length) {
    ...
    lines.push(
      ...
      'dispatched as itself, and the comment is why:',
      '',
      // Four backticks, because the block being quoted contains a ```json fence of its own
      // and a three-backtick wrapper would close on it — the agent would be handed a
      // half-block and would write one.
      '````',
      WHOLE_OPEN,
      '```json',
      '{',
      `  "epic": "${epic.id}",`,
      '  "whole": true,',
      '  "why": "what in the bead is enough to act on, and why it is one change rather than several"',
      '}',
      '```',
      WHOLE_CLOSE,
      '````',
      '',
      `Then \`bd label add ${epic.id} ${WHOLE_LABEL}\`. The \`why\` is not a formality and has a floor of`,
      `${MIN_WHY_CHARS} characters where nothing else here has one: it is the whole of what you decided, and`,
      'the worker opened on this epic reads it before the diff.',
      '',
  ...
  lines.push(
    ...
    '"leave this on the list" — every advocate gave every option a close as a workaround and reasoned it',
    'out again from scratch. An option marked this way comments the answer, leaves the bead open and',
    'unclaimed, and keeps the `human` label, so the card stays in their inbox and nothing opens a session',
    'on it. Use it for the option that defers rather than decides; the other two endings are unchanged.',
    '',
