 ...
 *   - **A home**, which is the epic — so bc-rfnr.7's "no P0 above it" gate is satisfied
 ...
 */
export async function runAgent({ dir, prompt, owner = 'the user', timeoutMs = TIMEOUT_MS }) {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn('/bin/zsh', ['-lc', command], { cwd: dir, env: agentEnv(f), stdio: ['ignore', 'pipe', 'pipe'] });
    let pending = '';
    let answer = '';
    let stderr = '';
...
export function createIngester({ bd = null, run = runAgent, fetchImpl = undefined, onSettled = null } = {}) {
  ...
  async function createChildren(workspace, { epic, ticket, beads, endorsed }) {
    ...
    // Edges last, once every id is known, exactly as the console does it. A dependency
    // that cannot be resolved is a warning and never a reason to lose a created bead.
    const warnings = [];
    for (const bead of beads) {
      const from = ids.get(bead.ref);
      for (const dep of bead.dependsOn || []) {
        // A ref in this proposal, or a real id the agent named that the tracker will own
        // to. `bd dep add` against neither is the error the console's `resolve` exists to
        // turn into a warning, and it costs one `bd show` per named id at most.
        let target = ids.get(dep) || null;
        if (!target) target = (await bd.exists(workspace, dep).catch(() => false)) ? dep : null;
        if (!from || !target || target === from) {
          warnings.push(`${from || bead.ref}: dependency on ${dep} skipped — no such bead`);
          continue;
        }
        try {
          await bd.addDep(workspace, from, target);
        } catch (err) {
          warnings.push(`${from}: dependency on ${target} refused — ${String(err?.message || err).split('\n')[0]}`);
        }
      }
    }
