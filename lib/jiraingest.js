 ...
 * being answered.
 *
 * **And a space can decline it** (bc-6s383): `jira.<space>.ingest: false` keeps the
 * epics arriving and reads none of them. The row then draws no reading line at all,
 * because no reading was ever going to happen — see `jiraIngests` in lib/jira.js.
 */
...
import { descriptionText, issue as fetchIssue, jiraIngests, settingsFor, threadOf } from './jira.js';
 ...
 * being quoted, and `agentEnv` so what the agent learned is filed as the agent it is. The
 * spawn-and-parse itself is lib/agentrun.js's `runHeadless` — lib/sessionaudit.js's runner
 * is the same shape, and bc-dgx7.8 is the two of them sharing it rather than each streaming
 * to `refs/beadcause/agentlogs` in its own slightly different way, or not at all.
 *
 ...
 */
export async function runAgent({ dir, prompt, owner = 'the user', timeoutMs = TIMEOUT_MS, cfg = null, key = null, meta = {} }) {
  return runHeadless({
    dir,
    prompt,
    systemText: `${memoryBrief(owner)}\n\n${PROTOCOL}`,
    timeoutMs,
    cfg,
    key,
    meta,
    tmpPrefix: 'beadcause-ingest',
  });
}
...
export function createIngester({ bd = null, run = runAgent, fetchImpl = undefined, onSettled = null } = {}) {
  ...
  async function ingestOne(cfg, workspace, ticket, epic) {
    ...
    try {
      ...
      // Computed before the run rather than after: `endorsed` also becomes the archived
      // run's own endorsement field below, and re-deriving it a second time is how the
      // two answers end up disagreeing the day `autoEndorseAllowed` gains a clock.
      ...
      const text = await run({
        ...
        // `bc-dgx7.8` — every ingestion is now a run on `refs/beadcause/agentlogs`. Keyed
        // on the epic rather than the ticket, because `agentarchive.runs({ bead })`
        // greps the *key* for `/<bead> ·` (see lib/dispatch.js's own `<workspace>/<id>`)
        // rather than reading the `bead` field back out — a key that did not end in a
        // real bead id would be a run "by bead" could never find. lib/jiraepic.js gives
        // every ticket exactly one epic, forever, so this is still unique per ticket.
        key: `${name}/${epic.id}`,
        meta: {
          workspace: name,
          bead: epic.id,
          agent: 'jira-ingest',
          endorsed,
          endorsementNote: endorsed
            ? 'auto-endorsement is on for this repo'
            : 'reading a ticket is beadcause doing its own job, not work opened on a bead',
        },
      });
  ...
  return {
    ...
    async sweep(cfg, pending = []) {
      ...
      for (const item of pending) {
        ...
        // `jira.<space>.ingest: false` — the epic without the reading (bc-6s383). Asked
        // per item rather than filtered by the caller, so no way into here can skip it.
        if (!jiraIngests(cfg, name)) continue;
