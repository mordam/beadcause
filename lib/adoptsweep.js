...
export async function sweepAdoptions(bd, workspaces = [], { onLog = () => {} } = {}) {
  ...
  for (const workspace of workspaces || []) {
    ...
    if (index?.error) {
      onLog(`[adopts] not planning against ${workspace?.name || 'the tracker'}: its shape has never been read — ${index.error}`);
      continue;
    }
    // And a *stale* one is worse than an empty one, which is bc-j52g. `Bd.graph` answers a
    // failed export with the last good index rather than with nothing, which is right for
    // everything drawing a screen and wrong here: this is the caller that turns the index
    // into `bd update --parent` writes, and an epic filed since that reading is simply
    // absent — so the sweep plans nothing for it, applies nothing, refuses nothing, and
    // says nothing, which is indistinguishable from a tick with no work to do. Skipping it
    // costs one cycle, and the next tick has a graph again. Loudly, because a sweep that
    // did nothing for a reason is the thing that was impossible to see from the outside.
    if (index?.stale) {
      onLog(`[adopts] not planning against ${workspace?.name || 'the tracker'}: its shape could not be re-read, and the last one predates what is being asked about — ${index.stale}`);
      continue;
    }
