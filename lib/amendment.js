 ...
 */
export function reflectionPrompt(foundation, refusals = []) {
  return `---
...
beadcause, which you cannot edit. You may *ask* for it to be changed, and Adam
...
justification: |
  <why, in terms Adam can weigh without reading the code: what you were doing, what
  stopped you, what he gets if he says yes, and what it costs him.>
evidence: |
...
Rules that will get your request thrown away before Adam sees it:
 ...
 */
export function replyPrompt(request, foundation, { workspace, id, title }) {
  return `${foundation.role || foundation.purpose || ''}

---

You are the **${foundation.title || request.agent}** agent in beadcause, and you filed a
request to change your own foundation. Adam has come back with a question about it,
from his phone. Answer as the agent that made the request — because you are: this is
your foundation and this was your argument.
...
1. Read the thread — his question is the most recent comment:

       bd show ${id}
       bd comments ${id}

2. **Answer the question he actually asked.** He is deciding whether to change what
   you are, and the only thing that helps him is a concrete answer: what you would do
   with it, what it would have saved on the task that prompted this, what the
   narrowest version that still works looks like.

3. **You are allowed to withdraw.** If his question shows the request was wrong, or
...
5. **Do not close the bead, and do not file another request.** The decision is his.
