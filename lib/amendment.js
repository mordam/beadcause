...
export function parseAmendment(text) {
  ...
  // **The card gate, refused here as well as at the commit.** `foundation.validate` is
  // what actually stops it — that is the funnel every write goes through, and it stays
  // the enforcement. This is the *earlier* half, and it earns its place by being the
  // point at which the agent is still the audience: refuse here and the agent is told,
  // in its own transcript, which field it moved and which card sentence it owes. Let it
  // through and the same refusal reaches Adam as a 422 while he is tapping approve,
  // which is a worse place for both of them to learn it.
  //
  // Errored and complete again, for the reason `beyond` is: the argument survives the
  // parse, so what the agent wanted is still legible to whatever handles the refusal.
  // The fields come off the request rather than a resolved patch, because a list touched
  // by `add`/`remove` is a change to that list whatever it currently holds.
  const touched = { ...set };
  for (const field of LIST_FIELDS) {
    if (add[field] || remove[field]) touched[field] = [];
  }
  const gap = cardGap(touched);
  if (gap) {
    return { ...request, cardGap: gap, error: `refused: ${gap.message}` };
  }
  return request;
}
...
export function changeLines(request, current) {
  ...
  return Object.keys(patch).map((field) => {
    ...
    // A card field is prose, sometimes three lines of it, and `show` would wrap it in a
    // code span — an unwrapped monospace paragraph running off the side of a phone. The
    // decision here is the same one the list branch makes above: render what the reader
    // is actually deciding about. For a capability that is what changed; for a sentence
    // it is the sentence, with the old one under it so it can be compared rather than
    // taken on trust.
    if (CARD_FIELDS.includes(field)) {
      return {
        ...
        summary: [
          `**now:** ${patch[field]}`,
          current?.[field] ? `**was:** ${current[field]}` : '**was:** _(unset)_',
        ].join('  \n'),
      };
    }
...
And your **system card** — what beadcause documents you as, which is what anyone
affected by you is told:

${CARD_FIELDS.map((field) => `- **${field}:** ${clip(foundation[field] || '(unset)', 400)}`).join('\n')}
...
- **Ask for a capability and your card moves with it.** ${Object.keys(CARD_IMPLICATIONS)
  .map((f) => `\`${f}\``)
  .join(', ')} are the fields that change what you can actually *do*, and each owes a
  card sentence in the same block — ${Object.entries(CARD_IMPLICATIONS)
    .map(([field, cards]) => `\`${field}\` needs ${cards.map((c) => `\`${c}\``).join(' or ')}`)
    .join('; ')} — written under \`set:\` beside your \`add:\`. This one is *refused*, not
  nagged about: a request that widens what you may do while leaving your own
  documentation describing the narrower agent is one nobody can audit afterwards, and
  it is thrown out before ${owner} ever sees it. Say what actually changes, in a
  sentence someone affected by you would recognise — not the same words one field over.
...
export async function fileRequest(bd, workspace, dir, request, { from = '' } = {}) {
  ...
  // A `human` card with nothing decided above it is not merely held — bc-rfnr.2's inbox does not
  // draw it at all, so an agent's request for a permission would sit unanswerable on a
  // screen that never showed it. Under the P0 of whatever the asking session was working,
  // or the unsorted backlog. See lib/homing.js.
  const { parent } = await homeIn(bd, workspace, { from });
