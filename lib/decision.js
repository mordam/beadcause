...
/**
 * `defers: true` — an option that means *not yet, leave this on the list*.
 *
 ...
 * deferral hands it back to you, with the card still on the screen. So an option says
 * which, and a deferral is `closes: false` **plus** this — `defers` forces `closes` false
 * in `normalizeOptions`, so every surface that already reads `closes === false` keeps
 * treating a deferral as non-closing, which is the half of it they all get right.
 *
 ...
 * ending, because a deferral that was meant to be a commission sits in the inbox where
 * somebody will look at it again, while the reverse loses the card.
 */
 ...
 *   shape of a question to Adam is "not yet, leave it on the list", and a session with no
 ...
 *   deferred question to an advocate to open a worker on, the opposite of leaving it for
 *   Adam (bc-7qo.10). This third option does not carry `recommended: true` of its own; the
...
const ASK_TAIL_LINES = [
  ...
  '  - id: <short-slug>',
  '    label: <not yet — leave this on the list>',
  '    hint: <what it costs or buys>',
  '    response: <the full sentence recorded as the answer>',
  '    defers: true   # answers without closing, and keeps this card in the inbox for Adam',
  '```',
];
