...
/**
 * A sentence is the other thing that is obvious and invalid, in two spellings.
 *
 *     response: Render fetches the assets from gdrive4t: at build or container start
 *     question: 'Trinan' names an Askra population and an Oobin population. Which one?
 *
 * Both are well-formed English and neither is YAML. The first reads `gdrive4t:` as a
 * nested mapping inside a compact one; the second parses `'Trinan'` as the value and
 * then finds a sentence sitting after it. Either way the **whole block** goes —
 * the question, every option, the recommendation — and what the card shows is a
 * question with nothing to tap, which on a phone is indistinguishable from a bead
 * whose author forgot to write options. dv-k4n.13 and dv-gr6.52 lost three-option
 * decisions to a colon; dv-5i2.111 lost one to a quoted first word; bc-xl7n.101's
 * first draft lost one to a `superseded-by:` inside a `hint:`.
 *
 * Every prose field in a block is a sentence written by an agent, so this is both the
 * likeliest mistake in the format and the most expensive one. On the known prose keys,
 * quote the value when it carries a `: `, trails a `:`, or is a complete quoted scalar
 * with more sentence after it.
 *
 * Left alone: a value that is *only* a quoted scalar (with a trailing comment or
 * nothing after it), a block scalar (`|`, `>`), a flow collection, a YAML tag or
 * anchor, and an opening quote with no partner on the line — that last one is the
 * first line of a multi-line quoted scalar, and wrapping it would break a value that
 * was never broken. URLs are untouched too: `https://x` has no colon-space.
 *
 * Only ever runs as a **retry**, after the ordinary parse has already failed, so a
 * block that parses today parses identically tomorrow. A multi-line plain scalar whose
 * colon lands on a continuation line is still a refusal — that line is not a
 * `key: value`, and quoting half a scalar makes the document worse, not better.
 */
const PROSE_KEY = 'question|context|label|title|response|answer|hint|detail|id';
const PROSE_LINE_RE = new RegExp(`^(\\s*(?:-\\s+)?(?:${PROSE_KEY}):[ \\t]+)(\\S[^\\n]*?)[ \\t]*$`, 'gm');
const QUOTED_HEAD_RE = /^'(?:[^']|'')*'|^"(?:[^"\\]|\\.)*"/;

function prosePastItsScalar(value) {
  const head = QUOTED_HEAD_RE.exec(value);
  if (head) {
...
function forgiveProseScalars(yamlText) {
  return yamlText.replace(PROSE_LINE_RE, (line, lead, value) =>
    prosePastItsScalar(value) ? `${lead}"${value.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"` : line,
  );
}
...
export function parseDecision(text) {
  ...
  try {
    ...
    try {
      spec = YAML.parse(forgiveProseScalars(forgiveMarkdownLinks(m[2])));
    } catch {
      // The *first* failure is the one reported, not the retry's. The retry quoted
      // lines the author never wrote, so its `at line N, column M` describes a document
      // that does not exist and `b7e-card`'s annotation would print the wrong source
      // line back at whoever has to fix it.
      //
      // And report it at all rather than silently dropping to free-text — a broken
