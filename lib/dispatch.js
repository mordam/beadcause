...
// Never `agentlog.reset` directly — see the note on it. The previous run is archived and
// chained first, or the record of a retried dispatch is the one that never survives.
import { archiveAndReset } from './agentarchive.js';
import { effective, promptArgs, agentEnv } from './foundation.js';
// The endorsement a run proceeded under is read off the row the dispatch already has, not
// re-derived — lib/endorse.js is the one place that decides what the marker means.
import { isHeld, UNENDORSED } from './endorse.js';
...
Question: ${title || '(untitled)'}${
    repo
      ? `\nRepo: **${repo.name}** — the ${workspace} workspace holds several checkouts behind one
tracker, and this bead names that one. You are standing in it; everything you look up
...
export async function dispatchReply(
  ...
  let repo = null;
  try {
    // The issue is already in hand for the amendment check below, and it is what says
    // which checkout this agent answers from. A reply agent in the wrong repo answers
    // the question with the wrong repo's files — quietly, unattended, onto the bead.
    ({ dir, repo } = resolveSessionRepo(cfg, workspace, issue));
  } catch (err) {
  ...
  const prompt = arguing
    ...
    : promptFor(workspace.name, id, title, agent, tools !== baseTools, reflection, owner, held, repo, confluenceBrief(cfg, owner));
  ...
  // derives BEADS_DIR and CLAUDE_CONFIG_DIR — which account this is billed to — from
  // $PWD at shell startup (it derives BEADS_ACTOR from $PWD too, and that one
  // `agentEnv` now overrides — see bc-y3qk.1), and the `cd`-triggered chpwd hook lives
  ...
  //
  // And the run being replaced is archived first — the whole of bc-eqn1.7. The provenance
  // is handed over rather than re-derived: `model` is what the process below is actually
  // launched with, and `held`/`issue` are the endorsement the tracker already answered for
  // this dispatch. Awaited, at a handful of git reads, because a record written after the
  // spawn is a record racing the first `append` for the file it is meant to be copying.
  const kept = await archiveAndReset(key, {
    ...
    persona: agent.name,
    model,
    endorsed: !held && !isHeld(issue),
    endorsementNote: held ? `dispatched on a bead still carrying ${UNENDORSED}` : null,
    repo: repo?.name || null,
  });
  if (kept.archived && !kept.chained) console.error(`[agentlog] ${key}: kept but unchained — ${kept.reason}`);
  agentlog.append(key, `● ${agent.name} dispatched on ${whereLanded(workspace.name, repo)} in ${dir}`);

  const child = spawn('/bin/zsh', ['-lc', command], {
    cwd: dir,
    // `agentEnv` is what puts `beadcause-memory` on PATH and stamps who this agent
    // is, so `recall` reaches the right memory without the agent naming itself. `cfg`
    // is there for the byline its own `bd` writes under — `agent (carol@example.com)`
    // rather than the engineer's address the login shell would have supplied.
    env: agentEnv(f, {}, cfg),
