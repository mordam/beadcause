...
import { effective, agentEnv } from './foundation.js';
...
function promptFor(workspace, id, title, agent, elevated, reflection = '', owner = ownerName(), held = false) {
...
export async function dispatchReply(
  ...
  const prompt = arguing
    ...
    : promptFor(workspace.name, id, title, agent, tools !== baseTools, reflection, owner, held);
