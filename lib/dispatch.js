 ...
 */
export function dispatchReply(cfg, workspace, id, title, { agentId = null, elevated = false } = {}) {
  ...
  // `elevated` is armed for ONE reply and consumed by the caller. The tools string
  // itself still comes from the config file: arming decides *whether* the override
  // applies, never *what it says*.
  const tools = elevated && agent.tools ? agent.tools : ALLOWED_TOOLS;
  ...
  fs.writeFileSync(promptFile, promptFor(workspace.name, id, title, agent, tools !== ALLOWED_TOOLS), { mode: 0o600 });
