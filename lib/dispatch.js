...
function promptFor(workspace, id, title, agent) {
 ...
 */
export function dispatchReply(cfg, workspace, id, title, { agentId = null } = {}) {
  ...
  fs.writeFileSync(promptFile, promptFor(workspace.name, id, title, agent), { mode: 0o600 });
  ...
  const command =
    ...
    `exec claude -p "$P" --allowedTools '${agent.tools || ALLOWED_TOOLS}'${
  ...
  console.log(`[beadcause] dispatched ${agent.name} for ${key} in ${dir}`);
  return { dispatched: true, dir, agent: { id: agent.id, name: agent.name } };
}
