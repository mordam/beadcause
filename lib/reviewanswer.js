...
import { REVIEW_ANSWERS, commentsForReviewer, commentsForWorker } from './mergebead.js';
...
/**
 * `(blocking)`, or nothing when the comment did not carry a recognised severity — the one
 * thing this brief did not used to say, and the one thing the worker's single judgement
 * (decline it free, or hold the branch another round) turns on. Absent rather than guessed,
 * because a comment from before this field existed is not thereby a suggestion.
 */
export const severityTag = (c) => (c?.severity ? ` (${c.severity})` : '');
...
export function reviewAnswerPrompt(workspace, issue, spec, state = {}, extra = {}) {
  ...
  /**
   * Comments this worker has answered and the reviewer has *not settled* — which is not the
   * same population as `owed`, and printing them matters in exactly the case a second round
   * arrives in: the reviewer kept a comment open over an answer it did not accept. Whether
   * folding the new verdict clears the old answer is the reviewer's business, so this brief
   * must read as sense either way rather than telling a worker that nothing is outstanding
   * while the reviewer is still holding two of its answers.
   */
  const standing = commentsForReviewer(state);
  ...
  if (owed.length) {
    for (const c of owed) {
      ...
      lines.push(`- **${c.id}**${severityTag(c)}${at ? ` \`${at}\`` : ''} — ${c.body}${c.why ? ` ${c.why}` : ''}`);
    }
  } else {
    lines.push(
  ...
  if (standing.length) {
    lines.push(
      '',
      `**${standing.length} answer${standing.length === 1 ? '' : 's'} of yours the reviewer has not settled**, which ` +
        'is not the same as being asked again. If what you said still holds, leave it; answer again only where its ' +
        'new sentence changes your answer, and the later answer replaces the earlier one.',
      '',
      ...standing.map((c) => {
        const at = commentAt(c);
        return `- **${c.id}**${severityTag(c)}${at ? ` \`${at}\`` : ''} — ${c.body}${c.why ? ` ${c.why}` : ''}\n  - you said: ${c.answer}${c.note ? ` — ${c.note}` : ''}`;
      })
    );
  }
  if (resolved.length) {
