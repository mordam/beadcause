...
`;
  ...
  async function tickOne(a, sessions) {
    ...
    const globalFree = globalLimit() - totalWorkers() - totalResolvers() - unattended;
    ...
    if (globalFree <= 0) {
      ...
      const unattendedNote = unattended
        ...
        : '';
      return note(a, `${a.queue.length} ready · held by globalMaxWorkers (${globalLimit()})${resolverNote}${unattendedNote}${gaveUpNote(a)}`);
    }
    if (!ready.length) {
