...
- **${
    holds.length
      ...
      : `Nothing was held back by contention.** I run only on a tick where all ${QUEUE_HOLDS.length} of my hold lists are empty, so take as given that no ready bead here is waiting on a child, an open pull request, a live session, another Mac's claim, a paused epic, a busy file, a missing checkout, a merge-bead still retrying its own close, or a decision nobody has made about how a childless epic splits. Nothing is hiding behind those.`
  }
