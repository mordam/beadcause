  ...
  async function tickOne(a, sessions) {
    ...
    if (!a.queue.length) {
      ...
      note(
        a,
        a.workers.length
          ? `${a.workers.length} session(s) working, nothing else ready${repoNoteText}${heldNote}${twinNoteText}${prNoteText}${liveNote}`
          : `${
              held || twins || inflightHeld || sitting || unplaceable ? 'nothing ready' : 'clear — no ready beads'
            }${repoNoteText}${heldNote}${twinNoteText}${prNoteText}${liveNote}`
      );
      if (!a.workers.length && !held && !twins && !inflightHeld && !sitting && !unplaceable && o.propose) await propose(a);
      return;
    }
