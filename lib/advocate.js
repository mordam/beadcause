...
export function createAdvocates(cfg, { bd, bus, say = messageSession, open = openWorkSession, prs = openWork }) {
  ...
  async function tickOne(a, sessions) {
    ...
    if (!a.queue.length) {
      ...
      const quiet = held || twins || inflightHeld || sitting || claimed;
      note(
        a,
        a.workers.length
          ? `${a.workers.length} session(s) working, nothing else ready${heldNote}${twinNoteText}${prNoteText}${liveNote}${leaseNote}`
          : `${quiet ? 'nothing ready' : 'clear — no ready beads'}${heldNote}${twinNoteText}${prNoteText}${liveNote}${leaseNote}`
      );
