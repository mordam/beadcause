...
// The recount behind `auditAttempts` — the two shapes that charged a bead for a window
// this daemon shut itself are fixed, and neither fix touches a counter already written.
import { auditLog, repairNote, DAEMON_LOG } from './attemptaudit.js';
...
import { ctxUpdate, sessionTokens, tokenLine, LONG_WINDOW } from './sessiontokens.js';
...
/**
 * Every id this tick's export has filed *something* under — a `Set`, built once per index.
 *
 * The cheap half of `heldByChildren`'s third check since bc-xl7n.137, and cheap is the
 * whole reason it exists. That check asks `bd children`, which is a subprocess, and the
 * rule it now enforces — *a parent with an open child is not work, whatever it is typed
 * as* — would otherwise ask it of **every** ready row on every tick rather than of the
 * handful of epics. So the export the tick has already paid for answers "could this bead
 * possibly have children" for free, and only a `true` costs a call. Measured in the
 * beadcause workspace on 2026-08-28: 331 open beads, of which 5 are non-epics with any
 * child at all.
 *
 * Keyed on the `parents` map itself rather than on the workspace, so it cannot outlive the
 * index it summarises: `bd.graph` builds a fresh `Map` on every export and the old one is
 * collected with its `Set` still attached to it. A cache keyed on a name would have to be
 * invalidated by somebody, and the somebody is exactly what a stale hold hides behind.
 *
 * `false` where there is no index — a tracker that would not answer holds nothing back,
 * which is the direction every other unanswerable question in this file takes.
 */
const CHILD_BEARING = new WeakMap();
const hasChildrenIn = (parents, id) => {
  if (!parents?.size || !id) return false;
  let set = CHILD_BEARING.get(parents);
  if (!set) {
    set = new Set(parents.values());
    CHILD_BEARING.set(parents, set);
  }
...
export function createAdvocates(cfg, {
  ...
  // The daemon's own log, which `auditAttempts` replays to recount the attempt charges —
  // injected for the same reason `psLines` above it is: the real one is this Mac's live
  // log, 39MB of it, and a suite that ticked would be replaying whatever happened on this
  // laptop last week against a fixture's counters. See lib/attemptaudit.js.
  ...
  function record(ws, prev = {}) {
    ...
    return {
      ...
      attemptAudited: prev.attemptAudited && typeof prev.attemptAudited === 'object' ? prev.attemptAudited : {},
  ...
  function persist() {
    ...
    for (const [name, a] of advocates) {
      out[name] = {
        ...
        attempts: a.attempts,
        // Which counters `auditAttempts` has already recounted, and at what value — so a
        // 39MB log is read once per bead that reaches the cap rather than once per tick.
        // Persisted for the same reason `lastWindowSweepAt` is: this daemon is restarted
        // by its own merges several times a day, and a forgetful one would re-read the
        // whole log on every boot to reach the same answer it had already written down.
        attemptAudited: a.attemptAudited,
...
`;
   ...
   *
   * ## A bead at the attempt cap reserves nothing, because it will never be opened
   *
   * bc-nc6o.15, and it is the sentence above — "the loser comes up on the next tick" — being
   * false for one class of winner. This filter is the last stage of the survey and the
   * attempt filter is the *first* stage of `candidates`, so a bead already at
   * `maxAttemptsPerBead` is still in the queue when this runs: it sorts in pick order, it
   * reserves every file it declares, and it is then dropped before any window opens. The
   * counter is a floor nothing decrements, so the bead behind it is not deferred a tick —
   * it is withheld for ever, waiting on work that can never come up. Measured at eighteen
   * hours and one file, bc-nc6o.11 behind bc-ka5y.43 over `public/graph.js`, the same
   * daemon line every thirty seconds. With 38 retired beads in one tick that is not a
   * corner: it is the cap having stopped being a per-bead retirement and become a repo-wide
   * lock on every surface any of them ever named.
   *
   * So a retired bead is skipped before it is compared, and it neither reserves nor is
   * reserved against. **It stays in the queue**, because `givenUp` is what reports a retired
   * bead and reads `a.queue`; and **it is kept out of `heldBySurface`**, because a bead
   * drawn as retired *and* as held behind a file is one bead as two pills, which is the
   * double count the ordering comment in `candidates` exists to prevent. Skipping the
   * comparison altogether is what gets both at once.
   *
   * `describeOverlap` needs no wording for this case, because after the skip the case cannot
   * arise: `clash` is only ever a bead that went into `taken`, so "waits for the next tick"
   * is true of every bead it is said about again. The skip also restores what `deferredNote`
   * asserts a few thousand lines down — that a tick with a deferral is, by construction, a
   * tick that is opening something.
   *
  ...
  function withoutCollidingSiblings(a, queue, kin = null) {
    ...
    for (const bead of queue) {
      // A bead at `maxAttemptsPerBead` is not one this tick is about to open, so it may not
      // reserve a file against one that is — bc-nc6o.15, and the section above for why the
      // skip is what gets both halves of that. Through untouched, so it is still in the
      // queue for `givenUp` to report and not in `heldBySurface` to be counted twice.
      if ((a.attempts[bead.id] || 0) >= maxAttempts) {
        workable.push(bead);
        continue;
      }
   ...
   * 3. **It is a parent with an open child**, wherever that child is — in_progress,
   *    blocked, deferred, parked behind a merge bead, or filtered out of the queue by the
   *    priority floor. This is the only one that costs a `bd` call, and only for beads
   *    that survived the first two and that the tick's export says have children at all.
   *    An epic *is* its children until they are done; since bc-xl7n.137 so is any other
   *    bead with children, because a **review follow-up parent** (lib/reviewfollowup.js)
   *    is filed as a `task` and is exactly that — its acceptance is "every child is
   *    closed" and it has no diff of its own to deliver.
   * 4. **It is an owned epic with no children at all, and nobody has decided what that
   *    means** — bc-jvt0.4, and the one hold here that is not about hierarchy. An epic's
   *    alone, where check 3 above is every parent's.
   *
   ...
   *
   * ## Check 3 on a bead that is not an epic, and what it does not become
   *
   * bc-xl7n.137. The widening above is a hold, and a hold is not a life: it stops a window
   * being opened on a follow-up parent while its children are in flight, and says nothing
   * about the parent whose children have all **closed**. That one is nobody's work either
   * — its acceptance is met and there is no branch to push — and it is lib/finishedepic.js
   * that ends it, with the same one-tap card an epic in that state gets. The two halves are
   * deliberately in different files and deliberately land together: with only this one, a
   * finished follow-up parent leaves the hold and goes straight back into `bd ready` for
   * ever, which is the worse half of the bug rather than a smaller version of it.
   *
   * **What this does not do is make the parent furniture.** `container` (lib/container.js)
   * is the machine-readable "not work, ever", and it was the cheaper candidate here: one
   * label in lib/reviewfollowup.js. It is wrong for this bead precisely because it works —
   * `Bd.ready` filters containers out unconditionally, so the parent could never reach the
   * close sweep either, and a bead that is *meant to end* would be held out of the one
   * queue that could end it.
   */
  /**
  ...
  async function heldByChildren(a, bead, queue) {
    ...
    // **An epic, or anything else the export says has children under it — bc-xl7n.137.**
    // The type test used to be the whole gate, and the bead it missed is the review
    // follow-up parent lib/reviewfollowup.js files: a `task` whose own acceptance is
    // *every child is closed*, whose body is a list of the reviewer's findings, and which
    // has no diff of its own for a worker to push. Check 1 held it, but only while a child
    // was still in `bd ready` — delivery parks a child behind its merge bead, which takes
    // it out of the queue and leaves it open — and check 2 could not, because `finish`
    // drops a delivered worker from `a.workers` up to half a minute before its window
    // actually exits. Measured twice, three minutes after the last child left each time:
    // a window opened on bc-xl7n.131 at 23:33:06Z and on bc-xl7n.132 at 00:53:54Z, both
    // with nothing in them to do.
    //
    // Widening the question rather than naming the follow-up label is the honest shape of
    // it, because check 1 has never cared what the parent is typed as: *a parent and its
    // child must not be launched in the same tick* already applies to a plain task. All
    // this adds is the child that is open somewhere the queue cannot see it — parked,
    // blocked, or below the floor — which is the exact difference between check 1 and this
    // one and was never a difference about types.
    //
    // `hasChildrenIn` is what keeps the cost where it was: the tick's own export answers
    // "could this bead have children at all" with no subprocess, and only a bead it says
    // yes about pays for the `bd children` call. An epic pays it as it always did, without
    // consulting the index — check 4 below needs the answer even for an epic the export
    // has never seen a child of, and a cold or failed export must not be able to turn a
    // childless owned epic into a workable one.
    if (bead.type !== 'epic' && !hasChildrenIn(parents, bead.id)) return null;
    let children;
    try {
    ...
    const open = (children || []).filter((c) => c && c.status !== 'closed');
    if (open.length) {
      // "an epic" only where it is one: the card says why a bead is held, and a task told
      // it is being held as an epic reads as the advocate having the wrong bead.
      const what = bead.type === 'epic' ? 'an epic' : 'a parent';
      return hierarchy(`${what} with ${open.length} open child ${open.length === 1 ? 'issue' : 'issues'}`);
    }

    // And check 4 is an epic's alone. A childless *task* is an ordinary bead and always
    // was; the question below — one job or several — is one only an epic's advocate is
    // ever asked, and `ownerOf` would otherwise hold every owned leaf in the workspace.
  ...
  async function reconcile(a, sessions) {
    ...
    for (const w of a.workers) {
      ...
      if (ended) {
        /**
         * It exited, closed nothing, delivered nothing and asked nothing. That is the one
         * ending here nobody chose, and it costs an attempt —
         * **unless this daemon is the one that chose it** (bc-xl7n.146).
         *
         * `parkIdle` closes a quiet window on purpose: it writes the conversation down,
         * drops it out of the open register and puts the pid on the closing list, and
         * lib/reap.js signals it. The shell writes `$?` on the way out, so the exit status
         * this arm is reading is *this daemon's own SIGTERM* coming back round — and the
         * sentence above is then false in the one way that matters, because the premise it
         * charges on is that nobody chose the ending.
         *
         * Measured on this Mac, `bc-xl7n.142` on 2026-08-27: briefed at 18:31, parked at
         * 18:51 "quiet for 20m — it is waiting on you", reaped at 18:55 as "exited without
         * closing it (exit 143)" and charged; resumed at 19:04, parked again at 19:26,
         * reaped at 19:29 and charged again. Two charges, `maxAttemptsPerBead` is 2, and the
         * bead has been sitting in `bd ready` looking like unstarted work ever since, on the
         * strength of one brief and one replay of that brief's silence. `bc-xl7n.144` the
         * same hour, and ~34 live beads were at the cap when this was written.
         *
         * **This is the guard `carryOver` already gives the `gone` ending, reaching the path
         * that actually fires.** The two are the same event seen from either side — a window
         * that stopped answering, with a transcript on disk worth resuming — and the only
         * difference was that one of them vanished on its own and the other was shut by this
         * process. `parkIdle` has already asked `carryOver` on this worker's behalf: it withholds
         * the record from a conversation that has had its trips, so a bead whose *resumed*
         * window also went quiet is charged here exactly as it always was and the loop stays
         * bounded. The flag is only ever set once the park reached the disk.
         *
         * Nothing else about the ending softens. `strand` still puts the claim back, the slot
         * is still freed, and `finish` is still given `unfinished` — which is what leaves the
         * park record alone, because `parkWorker` writes for three endings and this is not
         * one of them.
         */
        const ours = Boolean(w.idleParked);
        if (!ours) a.attempts[w.id] = (a.attempts[w.id] || 0) + 1;
        strand(w, anyone);
        finish(
          a,
          w,
          `the session exited without closing it${ended.code ? ` (exit ${ended.code})` : ''}` +
            (ours ? ' — this daemon parked it for being quiet, so no attempt is charged and the conversation comes back on the next dispatch' : ''),
          'unfinished'
        );
        continue;
      }
  ...
  function parkIdle(a, sessions, { idleMinutes: override = null } = {}) {
    ...
    for (const rec of openList(opened, a.name)) {
       ...
       */
      const owner = a.workers.find((w) => w.sessionId === rec.sessionId || Number(w.pid) === Number(live.pid)) || null;
      if (owner) {
      ...
      /**
       * May this conversation come back — bc-xl7n.146.
       *
       * The same question `parkWorker` asks of a `gone` ending, asked here for the same
       * reason and answered by the same function. An idle park is `gone`'s shape with the
       * window still on screen: **nobody is in the loop**. A `handback` waits on Adam and a
       * `delivered` waits on a merge, so neither can run away — the loop needs him in it,
       * which is why lib/parked.js says only `gone` parks are counted against `maxResumes`.
       * A window that simply stopped talking waits on nothing at all, so a park that
       * ignored the trip count would resume the same transcript into the same silence for
       * ever, which is the one shape `maxResumes` exists to stop.
       *
       * `resumes` has to be handed to `recordPark` for that to bind, and this is where the
       * count was being dropped: the row this sweep is holding is the *open-register* row,
       * which never carried one, and a resume drops the park record it could otherwise have
       * been read back off — so every idle park was written down as the first. The trip
       * count rides on the worker in between, exactly as it does for `parkWorker`.
       *
       * A record is only withheld from a worker this advocate owns. A resolver, a reviewer
       * or a hand-opened window has no worker and no attempt counter behind it, so there is
       * no runaway to bound and nothing to charge either way.
       */
      // `ending: 'idle'` — never left `null`, the way the open-register row it is spread
      // from arrives (`registerOpen` writes no such field). A record with no ending reads
      // as "not gone" in `resumeFor` below, which used to mean handback/delivered — the
      // only two kinds `parkWorker` ever wrote before this sweep existed. Now it means
      // "the idle sweep closed this and nobody was in the loop", which is `interruptedPrompt`'s
      // territory, not `resumePrompt`'s — see bc-xl7n.147 and the note on `resumeFor`.
      const bringBack = !owner || carryOver(owner);
      const next = bringBack
        ? recordPark(parked, key, {
            ...rec,
            waitingOn: 'it went quiet — nothing has come back to it',
            ending: 'idle',
            resumes: Number(owner?.resumes) || 0,
          })
        : parked;
      /**
      ...
      changed = false;
      /**
       * What `reconcile` needs to know a tick later, and it can only be written here —
       * bc-xl7n.146.
       *
       * This sweep is about to close a window this advocate holds a slot for. The shell in
       * it writes its exit status on the way out, so the next `reconcile` finds a done file
       * and takes the `ended` arm — whose whole sentence is "it exited, closed nothing,
       * delivered nothing and asked nothing. That is the one ending here nobody chose".
       * **Somebody did choose it: this line of code.** Without the flag the two ticks cannot
       * tell a window the daemon shut from a session that fell over, so a bead that got one
       * brief and one replay of its silence is charged twice and retired.
       *
       * Set after the write rather than beside `bringBack`, for the same reason `handBack`
       * sets `w.handedBack` after its own: a park that did not reach the disk will not be
       * resumed, and the attempt for that window is owed exactly as it always was.
       */
      if (owner) owner.idleParked = bringBack ? iso() : null;
      a.closing.push({
      ...
      const what = rec.bead || (rec.pr ? `#${rec.pr}` : rec.title);
      // Two sentences because there are two outcomes now, and the difference is exactly
      // what somebody reading the log a day later needs: a window that closed with its
      // conversation kept, or one that closed with nothing to bring back and an attempt
      // about to be charged for it. Saying "parked" about the second is how the trip count
      // becomes invisible again.
      const said = bringBack
        ? why
        : `${why}, and it has already been brought back ${Number(owner?.resumes) || 0} time(s); ` +
          `closed without parking it, so the next window gets a fresh brief`;
      console.log(`[advocate] ${a.name}: ${bringBack ? 'parked' : 'closed'} ${rec.kind} ${what} — ${said}`);
      emit(a, 'parked', { id: rec.bead || rec.title, title: rec.title, detail: said });
    }
  ...
  async function resumeFor(a, bead) {
    ...
    /**
     * A conversation nobody was in the loop for when it stopped wakes up to a different
     * turn, and the branch is on the recorded `ending` rather than on whether an answer
     * happens to be in hand — bc-y7l2m.
     *
     ...
     * that agent would then be told nobody decided anything, when in fact its question was
     * answered and the answer is sitting on a bead it is now being told not to go looking
     * for. The ending is a fact the park wrote down; the answer's absence is an inference
     * about a store, and only one of those is safe here.
     *
     * **Two endings land here, and `interruptedPrompt` tells them apart by `reason`.**
     * `gone` is `parkWorker`'s: the window disappeared and `handBack` forced the claim off.
     * `idle` is `parkIdle`'s: the sweep closed a quiet window on purpose — see the note on
     * that ending's `recordPark` call. Both share the one fact that makes this branch
     * correct at all (nobody answered, nobody decided), so both take it; before bc-xl7n.147
     * an idle park's `ending` was left `null` by the open-register row it is spread from,
     * which fell through to the *other* branch below and told the resumed agent Adam had
     * answered when nobody had.
     */
    if (rec.ending === 'gone' || rec.ending === 'idle') {
      return {
        key,
        rec,
        prompt: interruptedPrompt({ owner: ownerName(cfg), bead: bead.id, parkedAt: rec.at, reason: rec.ending }),
      };
    }
   ...
   *
   * `maxAttemptsPerBead` is a floor nothing decrements: it is cleared on the four endings
   * that are not failures (`closed`, `delivered`, `handback`, `stood down`) and on nothing
   * else, all four of them inside `reconcile`, all four of them about a window *this
   * advocate opened*. A hand-back that **you** asked for is none of those and reaches none
   * of them — lib/server.js's `handBackWorkBead` writes the tracker and this map is in the
   * daemon's memory — so `Request changes` on a delivery card put the bead back in `bd
   * ready` and left `candidates` refusing it for ever, while the card said "back in the
   * queue". That is bc-xl7n.117, and bc-xl7n.87 is the bead it happened to: both of its
   * advocate windows timed out at two hours and charged an attempt, the pull request was
   * made later by a session the daemon never opened, so nothing ever cleared them.
   *
   * **Which is a commission and not a retry**, and that is the whole argument for clearing
   * rather than counting: the reasoning `reconcile` already states out loud for
   ...
   * for a change; the two windows that died before it are not evidence about the work that
   * was asked for now.
   *
   * **Not a second `forget`.** That button clears every counter in the workspace, which is
   * the right shape for "I have read the list and none of these deserve it" and the wrong
   * one for a single card being answered — a bead that really does break every window it
   * gets must keep its charges when its neighbour is handed back. So this takes an id, and
   * it says how many charges it found: a caller that cleared nothing has nothing to tell
   * anybody, and a caller that cleared a *retired* bead has the one sentence worth putting
   * on the card.
   *
   * **Two callers now, and the second is why this is a function rather than a method on
   * the returned object** — bc-xl7n.149. The hand-back reaches it through `rearm`; the
   * console reaches it through `control`, which is what gives a bead that *never
   * delivered* — the whole population the cap actually retires — a lever that is not
   * `forget`.
   */
  function rearmOne(a, id) {
    const cap = attemptCap();
    ...
    // Cleared with the counter, so a bead charged again after this is a new question for
    // `auditAttempts` rather than one it has already answered.
    if (a.attemptAudited) delete a.attemptAudited[bead];
    persist();
  ...
  /**
   * Take back the charges written for windows this daemon shut itself — bc-xl7n.149.
   *
   * `givenUp` above is the diagnosis and this is the arithmetic behind it. bc-xl7n.146
   * stopped the `ended` arm charging for a window `parkIdle` closed, and bc-xl7n.147
   * stopped the resumed conversation waking as somebody's answer. **Neither fix touches a
   * counter already written**, and when this was added 64 beads in this workspace sat at
   * the cap, 36 of them live open work — about a third of the ready queue, every one of
   * them in `bd ready` looking exactly like work about to be picked up.
   *
   * The only lever that existed was `forget` on the console, which clears every counter
   * in the workspace at once. That is the right shape for "I have read the list and none
   * of these deserve it" and the wrong one for this: a bead that really did break two
   * windows has to still be retired when the repair is over, and the whole difficulty is
   * that nothing on any screen told the two populations apart. lib/attemptaudit.js is
   * what tells them apart, off the log the endings were already written to — see its
   * header for why the replay is a proof rather than a heuristic, and for the bead it
   * refuses to touch because the rule underneath it changed.
   *
   * **Once per bead, not once per tick.** The log is 39MB and grows for the life of the
   * daemon. `attemptAudited` remembers which counters have been recounted and at what
   * value, so the read happens only when a bead newly reaches the cap — a handful of times
   * a day at most, and never at all on the ordinary tick. In the steady state after
   * bc-xl7n.146 this finds nothing, which is the point: it is a repair for a backlog and a
   * backstop for a shape that is supposed to be fixed, not a second opinion about the cap.
   *
   * **It never raises a counter.** `owed` from the recount is only ever accepted when it
   * is *lower* than what is held, so the worst a wrong reading of the log can do is give a
   * bead one more window. The opposite direction — a repair that could retire something —
   * is not reachable from here at all.
   *
   * Failure is silent by design beyond a warning: a log that cannot be read is a source
   * with nothing to say, and the counters stay exactly as they were. Nothing downstream of
   * this needs it to have run.
   */
  async function auditAttempts(a) {
    const cap = attemptCap();
    if (!a.attemptAudited || typeof a.attemptAudited !== 'object') a.attemptAudited = {};
    // A counter outlives the bead it was charged to (see `givenUp`), and so would its
    // audit record. Dropped together, so this map cannot outgrow the one it shadows.
    for (const id of Object.keys(a.attemptAudited)) {
      if (!(id in a.attempts)) delete a.attemptAudited[id];
    }
    const due = Object.entries(a.attempts).filter(
      ([id, n]) => (Number(n) || 0) >= cap && a.attemptAudited[id] !== n
    );
    if (!due.length) return null;

    let report;
    try {
      report = await auditLog(daemonLog, { workspace: a.name, attempts: a.attempts });
    } catch (err) {
      console.warn(`[advocate] ${a.name}: could not recount the attempt charges — ${err.message}`);
      return null;
    }
    // **A log that is not there is not an answer, and must not be recorded as one.** Every
    // verdict comes back unexplained either way, so the repair is the same nothing — but
    // marking them would spend the one look each of these beads gets on a read that never
    // happened, and the next tick would skip them for good. `exists` is the only thing that
    // tells that apart from a log that was read and had nothing to say about them.
    if (report.exists === false) return null;
    const dueIds = new Set(due.map(([id]) => id));
    const repaired = [];
    for (const v of report.verdicts) {
      if (!dueIds.has(v.id)) continue;
      // Marked whether or not it was repaired, and marked *before* the write: an id the
      // log cannot explain must not be re-read on every tick for ever, and the value is
      // what makes a later charge on the same bead a new question rather than a settled
      // one.
      a.attemptAudited[v.id] = a.attempts[v.id];
      if (!v.repairable) continue;
      if (v.owed > 0) a.attempts[v.id] = v.owed;
      else delete a.attempts[v.id];
      a.attemptAudited[v.id] = v.owed;
      repaired.push(v);
      console.log(`[advocate] ${a.name}: ${repairNote(v, cap)}`);
      // The same event `forget` and `rearm` emit, because it is the same fact having
      // happened to this bead: the console draws `givenUp` off the snapshot, and a card
      // still showing it retired is the same lie one screen along.
      emit(a, 'forgot', { id: v.id, detail: repairNote(v, cap) });
    }
    if (repaired.length) {
      console.log(
        `[advocate] ${a.name}: recounted ${due.length} retired bead(s) against the daemon log — ` +
          `${repaired.length} had charges for a window this daemon parked and closed itself, ` +
          `${due.length - repaired.length} kept theirs`
      );
    }
    persist();
    return { due: due.length, repaired: repaired.length };
  }
   ...
   *
   * **`ran:opus` and `ran:opus[1m]` are different labels (bc-nc6o.14).** `tokens.limit`
   * already carries that distinction — it is `contextWindow(worker.model)`, graded off the
   * *selection*, which is the only place the `[1m]` marker survives; `models` never has it
   * (see `ranToken` in lib/ranmodel.js). Reusing `tokens` here rather than re-deriving the
   * window from `worker.model` keeps the two labels agreeing with each other by
   * construction: whatever window `ctx:` was graded against is the window `ran:` names.
   */
  async function recordRun(a, worker, models, tokens) {
    ...
    const longWindow = tokens?.limit === LONG_WINDOW;
    const addLabels = [...ranUpdate(row, models, { longWindow }).addLabels, ...ctxUpdate(row, pressure).addLabels];
  ...
  async function tickOne(a, sessions) {
    ...
    // Immediately after the one function that writes attempt charges, and before anything
    // reads them — `candidates` filters on the map and `givenUp` reports off it, so a
    // counter repaired here is repaired for this tick rather than the next. Above the
    // pause and drain returns for the reason the tracker sweeps are: recounting a number
    // this daemon wrote is housekeeping, and a paused advocate that has been asked to open
    // no more sessions has not been asked to keep a bead retired for a window it shut
    // itself. See `auditAttempts`; on an ordinary tick it reads nothing and returns null.
    await auditAttempts(a);
  ...
  async function control(name, action, value) {
    ...
    if (action === 'pause') {
      ...
      // Clears the attempt counters, so beads it gave up on are eligible again. The
      // whole-workspace half; `rearm` below it is the one-bead half, and which of the two
      // is right is a question about how many beads you have read rather than about how
      // many you want re-armed. Both clear `attemptAudited` for what they touch, so a
      // counter cleared by hand and charged again afterwards is recounted rather than
      ...
      /**
       * One bead, by id — the door bc-xl7n.149 was filed about.
       *
       * `rearm` on the returned object has always existed and has always done exactly
       * this; what it did not have was a caller anyone could reach. Its only one is
       * `handBackWorkBead`, which runs when **Request changes** or **Decline** is
       * answered on a delivery or merge card — so a bead that delivered had a lever and a
       * bead that never delivered had none, and "never delivered" is the whole population
       * the cap actually retires. The console's list of retired beads names them one at a
       * time; this is the verb that goes beside the name.
       *
       * The id rides in `value`, which is the slot `epicPause` already uses: `control` is
       * the one door onto /api/advocate and it takes one argument beyond the action, so a
       * second door for this would be a second contract to keep in step.
       */
      return rearmOne(a, value);
  ...
  return {
    ...
    /**
     * `rearmOne` by workspace name — the hand-back's door onto it, from lib/server.js.
     *
     ...
     */
    rearm: (workspace, id) => rearmOne(advocates.get(String(workspace || '')), id),
