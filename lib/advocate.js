...
`;
  ...
  async function survey(a) {
    ...
    // And outside even that one, because it is the only filter here whose input is the
    // *result* of the others: bc-42ow.4 compares the beads this tick is about to open
    // against each other, so it cannot run until they are known. Everything inside it
    // subtracts a bead for a reason that exists whether or not the rest of the queue does;
    // this one is a fact about the queue itself. `kin` travels for `withoutClaimedFiles`'s
    // reason — a declared surface is read from the bead's whole row.
