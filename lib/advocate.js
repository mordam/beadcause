...
import { advocateSession, openedRecently, rememberAdvocateOpened } from './epicadvocate.js';
...
export function createAdvocates(cfg, {
  ...
  async function rosterFor(a) {
    let beads = null;
    try {
      beads = (await bd.graph(a.workspace, { wait: false }))?.beads || null;
    } catch {
      // A tracker mid-write is a passing state, and the roster is a display. Keeping the
      // last one is better than blanking every section on the card for one tick.
      return a.epicAdvocates || [];
    }
...
`;
  ...
  async function survey(a) {
    ...
    return withoutClaimedFiles(
      ...
      withoutLiveSessions(
        a,
        withoutOpenPrs(a, await withoutTwins(a, await withoutLeases(a, workable, labels, kin, inflight), inflight))
      ),
      kin
    );
  }
      ...
      const why = opened
        ...
        : mins < cooldown
          ...
          : live
            ...
            // `advocateSession` cannot see until it has named itself, and a live lease on it
            // is another Mac's window, which nothing on this laptop can see at all.
            : busy(p0)
