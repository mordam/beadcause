...
import { advocatedRoots, reentryFor, waitingOnMerge, waitingOnMergeCard, REENTER_DEFAULTS } from './reenter.js';
import {
  ADVOCATE_LABEL,
  advocateSession,
  isAssigned,
  ...
  function givenUp(a) {
    ...
    return (a.queue || [])
      ...
      .map((b) => ({
        ...
        why: `${a.attempts[b.id] || 0} window(s) ended without delivering, and maxAttemptsPerBead is ${cap} — nothing here will open another. Forget attempts re-arms it.`,
      }));
  }
  ...
  async function tickOne(a, sessions) {
    ...
    if (globalFree <= 0) {
      ...
      const resolverNote = resolving
        ...
        : '';
      return note(a, `${a.queue.length} ready · held by globalMaxWorkers (${globalLimit()})${resolverNote}${gaveUpNote(a)}`);
    }
    if (!ready.length) {
      ...
      const gone = givenUp(a);
      const settling = a.queue.length - ready.length - gone.length;
      const settlingNote = settling ? ` · ${settling} settling or already under way` : '';
      return note(a, `${a.queue.length} ready${settlingNote}${gaveUpNote(a)}`);
    }
    ...
    if (await landed(a, { force: true })) {
      ...
      ready = candidates(a);
      if (!ready.length) return note(a, `${a.queue.length} ready · nothing to open a session on${gaveUpNote(a)}`);
    }
