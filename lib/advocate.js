...
import { dispatchable, formatPlan, isUnder as underEpic, PLANNED_LABEL, PROMOTED_LABEL, readPlan, unplanned } from './plan.js';
...
import { liveSessions, workspaceFor } from './claude.js';
  ...
  async function plansFor(a, queue, labels) {
    ...
    for (const epic of epics) {
      ...
      if (loose.length && canPlan) {
        ...
        // `underEpic` rather than `isDescendantOf`, and it has to be the same test
        // `unplanned` just used: an adopted child that triggered this rewrite would
        // otherwise not be held by it, and would take an ordinary window against the plan
        // being rewritten — the one thing this hold exists to prevent.
