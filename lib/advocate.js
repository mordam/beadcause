...
`;
  ...
  async function launch(a, bead) {
    ...
    const { dir, mode, term, model, tier } = planning
   ...
   * reads as contested by one machine. A window whose name **says `DONE-`**, because a
   * session finished by its own account is not working the bead and its window may sit
   * there for an hour afterwards. And a bead **another Mac already holds**, because a
