...
export function createAdvocates(cfg, { bd, bus, say = messageSession, open = openWorkSession, prs = openWork }) {
  ...
  async function survey(a) {
    ...
    return withoutLiveSessions(a, withoutOpenPrs(a, await withoutTwins(a, workable)));
  }
  ...
  function withoutOpenPrs(a, queue) {
    ...
    for (const bead of queue) {
      ...
      const why = inflightWhy(hit);
      a.heldByPr.push({ id: bead.id, why, number: hit.number, url: hit.url });
  ...
  async function inflight(a, { force = false } = {}) {
    ...
    try {
      ...
      // the ordinary state of a scratch tracker under ~/beads, and not an error.
      return false;
    }
    ...
    let result;
    try {
      result = await openWorkFor(a, dir);
    } catch (err) {
      a.inflight = { summary: `open-PR check failed: ${err.message.split('\n')[0]}`, held: 0, at: a.lastInflightAt };
      console.error(`[advocate] ${a.name}: open-PR check failed — ${err.message.split('\n')[0]}`);
      return false;
    }

    if (!result.ok) {
      a.inflight = { summary: describeInflight(result), held: a.openPrs?.size || 0, at: a.lastInflightAt, reason: result.reason };
      return false;
    }

    a.openPrs = result.beads;
    a.inflight = {
      summary: '',
      held: result.beads.size,
      checked: result.checked,
      at: a.lastInflightAt,
    };
    const after = new Set([...result.beads.keys()]);
  ...
  async function batchesFor(a, queue) {
    ...
    for (const epic of epics) {
      ...
      const kids = queue.filter((other) => isDescendantOf(other.id, epic.id) && !foldedInto.has(other.id));
  ...
  async function flagInMain(a) {
    ...
    try {
      ...
      // No checkout maps to this workspace, so there are no branches to ask git about.
      // Ordinary for a scratch tracker under ~/beads, and not an error.
      return;
    }
    ...
    let result;
    try {
      result = await sweepInMain(bd, a.workspace, dir, { base: cfg.pr?.base || 'main' });
    } catch (err) {
      // A sweep is a courtesy on top of the tick and may not take it down.
      a.inMain = { summary: `in-main sweep failed: ${err.message.split('\n')[0]}`, flagged: 0, at: a.lastInMainAt };
      console.error(`[advocate] ${a.name}: in-main sweep failed — ${err.message.split('\n')[0]}`);
      return;
    }
  ...
  async function tickOne(a, sessions) {
    ...
    if (!a.queue.length) {
      ...
      note(
        a,
        a.workers.length
          ? `${a.workers.length} session(s) working, nothing else ready${heldNote}${twinNoteText}${prNoteText}${liveNote}`
          : `${held || twins || inflightHeld || sitting ? 'nothing ready' : 'clear — no ready beads'}${heldNote}${twinNoteText}${prNoteText}${liveNote}`
      );
      if (!a.workers.length && !held && !twins && !inflightHeld && !sitting && o.propose) await propose(a);
      return;
    }
  ...
  const snapshot = () =>
    [...advocates.values()].map((a) => ({
      ...
      heldByPr: (a.heldByPr || []).map((h) => ({ id: h.id, why: h.why, number: h.number, url: h.url })),
