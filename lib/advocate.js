...
const SURVEY_TOOLS = 'Bash(bd *) Bash(git log:*) Bash(git status:*) Bash(git diff:*) Read Grep Glob';
...
export function createAdvocates(cfg, { bd, bus }) {
  ...
  function surveyAgent(a) {
    ...
    fs.writeFileSync(promptFile, surveyPrompt(a.name, o), { mode: 0o600 });
    ...
    const command =
      ...
      `exec claude -p "$P" --allowedTools '${SURVEY_TOOLS}' --output-format stream-json --verbose`;

    return new Promise((resolve, reject) => {
      const child = spawn('/bin/zsh', ['-lc', command], { cwd: dir, stdio: ['ignore', 'pipe', 'pipe'] });
      let pending = '';
      let answer = '';
      let stderr = '';
  ...
  async function notePending(a, dir) {
    if (!a.pendingNotes.length || !o.sessionLog) return;
    let main;
    try {
  ...
  async function tidy(a, sessions) {
    ...
    try {
      const result = await sweepWorktrees(dir, { sessions });
...
function surveyPrompt(workspace, o) {
