...
export function createAdvocates(cfg, { bd, bus, say = messageSession, open = openWorkSession, prs = openWork }) {
  ...
  async function survey(a) {
    ...
    return withoutOpenPrs(a, await withoutTwins(a, workable));
  }
  ...
  async function tickOne(a, sessions) {
    ...
    if (!a.queue.length) {
      ...
      note(
        a,
        a.workers.length
          ? `${a.workers.length} session(s) working, nothing else ready${heldNote}${twinNoteText}${prNoteText}`
          : `${held || twins || inflightHeld ? 'nothing ready' : 'clear — no ready beads'}${heldNote}${twinNoteText}${prNoteText}`
      );
      if (!a.workers.length && !held && !twins && !inflightHeld && o.propose) await propose(a);
      return;
    }
