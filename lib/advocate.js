  ...
  async function tickOne(a, sessions) {
    ...
    if (!a.queue.length) {
      ...
      const claimNote = onBusyFiles
        ...
        : '';
      const quiet = held || twins || inflightHeld || sitting || claimed || unplaceable || onBusyFiles;
      note(
        a,
        a.workers.length
          ? `${a.workers.length} session(s) working, nothing else ready${repoNoteText}${heldNote}${twinNoteText}${prNoteText}${liveNote}${leaseNote}${claimNote}`
          : `${quiet ? 'nothing ready' : 'clear — no ready beads'}${repoNoteText}${heldNote}${twinNoteText}${prNoteText}${liveNote}${leaseNote}${claimNote}`
      );
