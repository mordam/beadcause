...
export function createAdvocates(cfg, { bd, bus, say = messageSession, open = openWorkSession }) {
  ...
  async function survey(a) {
    ...
    return withoutTwins(a, workable);
  }
  ...
  async function tickOne(a, sessions) {
    ...
    if (!a.queue.length) {
      ...
      note(
        a,
        a.workers.length
          ? `${a.workers.length} session(s) working, nothing else ready${heldNote}${twinNoteText}`
          : `${held || twins ? 'nothing ready' : 'clear — no ready beads'}${heldNote}${twinNoteText}`
      );
      if (!a.workers.length && !held && !twins && o.propose) await propose(a);
      return;
    }
