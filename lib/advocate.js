...
export function createAdvocates(cfg, { bd, bus, say = messageSession, open = openWorkSession, openPlan = openPlanSession, prs = openWork }) {
  ...
  async function surveyAgent(a) {
    ...
    fs.writeFileSync(
      promptFile,
      surveyPrompt(a.name, o, reflection, ownerName(cfg), repo?.brief || '', confluenceBrief(cfg, ownerName(cfg))),
      { mode: 0o600 }
    );
    const command =
 ...
 */
function surveyPrompt(workspace, o, reflection = '', owner = ownerName(), repoBrief = '', wiki = '') {
