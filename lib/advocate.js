...
import { dispatchable, formatPlan, PLANNED_LABEL, PROMOTED_LABEL, readPlan, unplanned } from './plan.js';
import { advocatedP0s, reentryFor, REENTER_DEFAULTS } from './reenter.js';
import { advocateSession, isPaused, openedRecently, PAUSED_LABEL, pausedEpics, rememberAdvocateOpened } from './epicadvocate.js';
...
import { archiveSession, mergeCommitFor, noteMerge, mainCheckout, ranModelsOf } from './sessionlog.js';
...
import { prepareResume, resumePrompt } from './resume.js';
...
const DEFAULTS = {
  ...
  // number of EpicAdvocates" would mean read literally: twenty open P0s on this repo
  // alone on the day this was written, and twenty iTerm windows is not a plan. Every
  // assigned epic still gets its section on the console; three of them have a window at
  ...
  // P0 advocate and the worker read the same key through `agentrepo.armSetting` — the
  // value lives in lib/agentrepo.js now, because lib/session.js cannot import this module.
  agentRepo: agentrepo.ARM_SETTING,
  // Re-opening the P0 advocate when something moves under a P0 it has already been on —
...
export function createAdvocates(cfg, {
  ...
  prs = openWork,
  // The P0 advocate, re-opened on child events by `reenter` below. The fourth door into
  // an unattended window and the third injectable of this shape — the default is THE REAL
  // ONE, which drives iTerm, so a suite whose fixture has an enrolled P0 in it must stub
  ...
  function record(ws, prev = {}) {
    ...
    return {
      ...
      // The roster: one entry per open, owned P0 in this workspace, rebuilt from the
      ...
      // When the P0 advocate re-entry sweep last looked, and what it saw under each
      // enrolled P0 — `{ [p0]: { kids, stalls, stalled, at } }`. **Both persisted, and the
      ...
      // child of every enrolled P0 as newly filed on the first sweep after every boot, and
      // beadcause is restarted by its own merges several times a day. `at` is the per-P0
      ...
      heldByLive: [],
      // And the ones held because the P0 above them is paused. Not contention at all,
      // like `heldByNoP0` below and unlike the rest: every other hold here is two things
  ...
  /**
   * The cached export, read **once per tick** and shared by the two things that want it.
   *
  ...
  async function tickBeads(a) {
    if (a.tickBeads !== undefined) return a.tickBeads;
    try {
      a.tickBeads = (await bd.graph(a.workspace, { wait: false }))?.beads || null;
    } catch {
  ...
  async function rosterFor(a) {
    ...
    return assignedAdvocates(beads).map((epic) => {
      ...
      return {
        ...
        // keeps its section, its window row and its button — that is what makes it
...
`;
  ...
  async function withoutLeases(a, queue, labels, kin = null, inflight = null) {
    ...
    for (const bead of queue) {
      ...
      if (!holder) {
        ...
        const down = leaseHolderBelow(bead.id, await leasedElsewhere());
        if (down) {
          holder = down.holder;
   ...
   * any of it under this bead" — and `isDescendantOf` answers the second for free, at any
   * depth, in the ids already in hand.
   *
   ...
   */
  function leaseHolderBelow(id, claims) {
    ...
    // ordering argument in the other direction. Depth and not string length: `x-1.10` is no
    // further down than `x-1.2`, and sorting by characters would say it was.
    const depth = (i) => String(i).split('.').length;
    const under = claims
  ...
  /**
   * Beads under a paused P0, taken out of the queue and said out loud.
   *
   ...
   *
   * **The bead itself counts, not only its descendants.** A leaf P0 is workable in its
   * own right, and pausing its advocate and then dispatching a window onto the P0 is the
  ...
  function withoutLiveSessions(a, queue) {
    ...
    for (const bead of queue) {
      ...
      // A P0 advocate this daemon launched in the last ten minutes and whose window has not
      ...
      const why = hit
        ? sittingWhy(hit)
        : 'a P0 advocate was opened on it a moment ago and its window has not named itself yet';
   ...
   *
   * **And a P0 advocate this tick opened, which is the case with no session record at all.**
   * `reenter` runs after the survey and before the launch, so a P0 that was in the queue
  ...
  function resight(a, ready) {
    ...
    return ready.some(
      (b) => a.liveSessions.some((s) => namesBead(s.name, b.id)) || openedRecently(`${a.name}/${b.id}`)
    );
  }
  ...
  async function reconcile(a, sessions) {
    ...
    for (const w of a.workers) {
      ...
      // The fallback, since bc-2uj4.5: `launch` mints the id and puts it on the record
      // before the window exists, so this is now only how a worker *adopted* from a
      // previous daemon — or launched before ids were minted — gets one. Still never
      // overwritten with null, and for a harder reason than before: once the process is
      // gone this id is the only route back to what it did, and it is what decides
      // whether the window may be closed at all. See lib/parked.js.
   ...
   *
   * A tracker that refuses the write is not an error worth stopping a tick for: the bead
   * stays claimed, which is where it already was, and the next window that reaches one of
   * these endings tries again.
   */
  async function handBack(a, w) {
    if (OBSERVING) return;
    try {
      await bd.reopen(a.workspace, w.id);
    } catch (err) {
  ...
  /**
   * The two endings where the next move is Adam's, written down so his answer can bring
   * the same agent back.
   *
   * **`handback` and `delivered`, and deliberately not the other five.** The test is not
   * "did this window stop" — every ending stops a window — it is *does a conversation with
   * this agent still have somewhere to go*:
   *
   ...
   *   attempt deserves the fresh brief the attempt counter is already arranging.
   *
  ...
  function parkWorker(a, w, kind) {
    ...
    if (kind !== 'handback' && kind !== 'delivered') return;
    ...
    try {
      ...
      saveAppState({
        ...
        parked: prunePark(
          recordPark(state.parked, beadKey(a.workspace, w.id), {
   ...
   * do, a MergeAdvocate waiting on a review, a P0 advocate that wrote its waiting-on
  ...
  function parkIdle(a, sessions, { idleMinutes: override = null } = {}) {
    ...
    a.parkedRows = parkedList(parked, a.workspace);
    if (o.parkIdleWindows === false || o.closeFinishedSessions === false || OBSERVING) return;

    for (const rec of openList(opened, a.workspace)) {
      ...
      const { act, why } = parkDecision(rec, live, { idleMinutes });
      ...
      if (a.workers.some((w) => w.sessionId === rec.sessionId || Number(w.pid) === Number(live.pid))) continue;
      if (a.closing.some((c) => Number(c.pid) === Number(live.pid))) continue;

      const key = rec.bead
        ? beadKey(a.workspace, rec.bead)
        : rec.pr
          ? prKey(a.workspace, rec.pr)
  ...
  async function resumeFor(a, bead) {
    ...
    const key = beadKey(a.workspace, bead.id);
  ...
  async function launch(a, bead) {
    ...
    const { dir, mode, term, model, tier, sessionId } = planning
  ...
  async function leaseHandOpened(a, sessions) {
    ...
    const open = new Map();
    const held = (id) =>
      a.workers.some((w) => w.id === id || isDescendantOf(id, w.id)) || a.closing.some((c) => c.id === id);
    for (const s of sessions) {
  ...
  /**
   * What a finished session actually ran on, written onto its bead (bc-nc6o.3).
   *
   ...
   * both — see lib/ranmodel.js.
   *
   * Silent about the ordinary cases, and there are two of them. A session with no
   * transcript left says nothing rather than guessing the routed model, and a run that
   * went where it was sent says nothing either. **A divergence is said out loud, once**,
   * because that one is news: it is the case both cards exist to show and the case a
   * plan-only record would have hidden.
   */
  async function recordRan(a, worker, models) {
    ...
    const { addLabels } = ranUpdate(row, models);
  ...
  async function archiveFinished(a) {
     ...
     */
    if (!o.sessionLog) {
      for (const { worker } of finished) await recordRan(a, worker, ranModelsOf(worker.sessionId || null));
      return;
    }
    ...
    for (const { worker, outcome } of finished) {
      ...
      await recordRan(a, worker, ran || ranModelsOf(worker.sessionId || null));
    }
  }
  ...
  /**
   * What a landing has to say about requirements — and what it owes when it says nothing.
   *
   * Wrapped here rather than inlined at the call site because the sweep must not be able
   * to fail on this: a landing whose requirement could not be recorded is still a landing,
   * the note is still written, and the index is rebuildable from the note. Everything
   * below is best-effort by construction and returns `{ extra: '', ids: [] }` when it
   * cannot do its job.
   */
  async function requirementsAtLanding(a, main, sha, beadId) {
    const none = { extra: '', ids: [] };
    try {
      const corpus = corpusFor(a);
      if (!corpus?.ids?.size) return none;
      ...
      return { extra: res.extra, ids: res.ids };
    } catch (err) {
      console.error(`[advocate] ${a.name}: ${beadId} requirements at landing — ${err.message.split('\n')[0]}`);
      return none;
    }
  }
  ...
  async function notePending(a, dir) {
    ...
    for (const p of a.pendingNotes) {
      try {
        ...
        // bc-fvmx: the one moment a merge commit and a bead are both in hand. What the
        // bead said it fulfils becomes evidence here — on the note, which is immutable,
        // and in the index, which is rebuilt from the note when it is wrong. A repo with
        // no requirements corpus takes none of this and gets the note it always got.
        const req = await requirementsAtLanding(a, main, merged, p.bead);
        await noteMerge(main, { sha: merged, bead: p.bead, workspace: a.name, ref: p.ref, extra: req.extra });
        console.log(
          `[advocate] ${a.name}: ${p.bead} landed in ${merged.slice(0, 8)} — noted` +
            (req.ids.length ? ` (${req.ids.join(', ')})` : '')
        );
      } catch {
  ...
  /**
   * Re-open the P0 advocate on a P0 something has moved under — the fourth door into an
   ...
   * because it is not queue work — a P0 advocate takes no worker slot, competes for none,
   ...
   * workspace, one per P0 per `reenterCooldownMinutes`, and never one where a session
  ...
  async function reenter(a) {
     ...
     * is a timestamp: a worker this advocate is holding (its batch included, because a
     * batch head's window is briefed on every id in it), a live Claude Code session naming
     * the bead — which covers a window Adam opened by hand — and a live lease from another
     * Mac, which is the only one of the three that can see off this laptop.
     */
    const busy = (row) =>
      a.workers.some((w) => w.id === row.id || (w.batch || []).includes(row.id)) ||
    ...
    const keep = {};
    let opened = 0;
    for (const { p0, kids, tree } of advocatedP0s(index)) {
      const prev = a.advocated?.[p0.id] || null;
      const { reason, record } = reentryFor(prev, tree, { busy, stallMinutes });
      if (!reason) {
        keep[p0.id] = record;
        held.delete(p0.id);
        continue;
      ...
      const live = advocateSession(sessions, p0.id, { openedAt: openedRecently(`${a.name}/${p0.id}`) });
      const why = isPaused(p0)
        ? // First of the five, because it is the only one that is somebody's decision
          // rather than a state of the machine — and because `keep[p0.id] = prev` below
          ...
          // design, and a P0 that came back saying nothing had moved would be the worst
          // possible reading of a fortnight's work.
          'it is paused'
        : opened
          ? "another P0 already took this tick's window"
          : mins < cooldown
            ...
            : live
              ...
              // And the P0 itself put through the same three questions its children are: a
              // worker or planner window this advocate is holding *on the P0* is one
              ...
              : busy(p0)
                ...
                : null;
      if (why) {
        keep[p0.id] = prev || record;
        // Once per P0 per spell of being held, like every other filter here: this runs
        // every ten minutes and a cooldown lasts three hours, so a line per sweep would be
        // eighteen of them saying the same thing about a P0 nothing is wrong with.
        if (!held.has(p0.id)) {
          held.add(p0.id);
          console.log(`[advocate] ${a.name}: holding a P0 advocate on ${p0.id} — ${why} (${reason})`);
        }
        continue;
      }
      held.delete(p0.id);

      // Read for the one P0 about to get a window rather than for every enrolled one: this
      // is a `bd comments` spawn, where everything above it came out of one cached export.
      const plan = await readPlan(bd, a.workspace, p0.id).catch(() => null);
      try {
        await openAdvocate(cfg, a.workspace, p0, {
          kids,
        ...
        // event intact.
        keep[p0.id] = { ...(prev || record), at: iso() };
        console.warn(`[advocate] ${a.name}: could not re-open the P0 advocate on ${p0.id} — ${detail}`);
        emit(a, 'failed', { id: p0.id, detail });
        continue;
      }
      record.at = iso();
      keep[p0.id] = record;
      ...
      rememberAdvocateOpened(`${a.name}/${p0.id}`);
      opened += 1;
      console.log(`[advocate] ${a.name}: re-opened the P0 advocate on ${p0.id} — ${reason}`);
      emit(a, 'advocated', { id: p0.id, why: reason });
    }
    // Assigned rather than merged, which prunes the record of a P0 that is no longer
    // enrolled — its advocate took its waiting-on sentence off, or somebody closed it. A
    // P0 that comes back is a first sight again, which is silent, and that is the right
  ...
  async function tickOne(a, sessions) {
    ...
    // `tickBeads`. `undefined` rather than `null`, because those mean different things.
    a.tickBeads = undefined;
    ...
    if (a.draining) {
      a.queue = [];
      const open = a.workers.length + a.closing.length;
      if (!open) {
    ...
    // The fourth door into an unattended window, and it is not queue work: a P0 advocate
    ...
    if (!a.queue.length) {
      ...
      const quiet =
        held || twins || inflightHeld || sitting || claimed || unplaceable || onBusyFiles || orphaned || held0;
      note(
        a,
        a.workers.length
          ? `${a.workers.length} session(s) working, nothing else ready${repoNoteText}${heldNote}${twinNoteText}${prNoteText}${liveNote}${leaseNote}${claimNote}${orphanNote}${pauseNote}`
          : `${quiet ? 'nothing ready' : 'clear — no ready beads'}${repoNoteText}${heldNote}${twinNoteText}${prNoteText}${liveNote}${leaseNote}${claimNote}${orphanNote}${pauseNote}`
      );
  ...
  const snapshot = () =>
    [...advocates.values()].map((a) => ({
      ...
      // One entry per epic with an advocate assigned — the console draws a section from
      // each. Sent whole rather than as a count for the reason every other list here is:
      // a number you cannot open is a number you have to go and check.
      //
  ...
  async function reclaim(a) {
    ...
    parkIdle(a, a.liveSessions || [], { idleMinutes: 0 });
  ...
  /**
   * Pause or resume one epic's advocate — the button on its section of the console.
   *
   * **Three things stop and one thing does not**, and the one that does not is the whole
   * shape of the feature. What stops: `reenter` no longer opens a window on this P0,
  ...
  async function epicPause(a, id, paused) {
    ...
    if (paused) {
      ...
      // as much "a session working under this epic" as the head is. The same `isDescendantOf`
      // the rest of the file uses, so this and the queue filter can never disagree about
      // which windows a pause is about.
      const under = (w) =>
        [w.id, ...(w.batch || [])].some((x) => x === bead || isDescendantOf(x, bead));
