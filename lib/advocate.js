  ...
  async function tickOne(a, sessions) {
    ...
    if (!a.queue.length) {
      ...
      note(
        a,
        a.workers.length
          ? `${a.workers.length} session(s) working, nothing else ready${heldNote}`
          : `${held ? 'nothing ready' : 'clear — no ready beads'}${heldNote}`
      );
      if (!a.workers.length && !held && o.propose) await propose(a);
      return;
    }
