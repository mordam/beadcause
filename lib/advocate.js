...
import { advocatedRoots, reentryFor, waitingOnMerge, REENTER_DEFAULTS } from './reenter.js';
...
import { list as liveResolvers } from './resolvers.js';
...
import { sweepFinishedEpics, describeFinishedEpics } from './finishedepic.js';
...
import { closeEmptyWindows } from './iterm.js';
...
const DEFAULTS = {
  ...
  // Ask about an open epic whose children have all closed — see lib/finishedepic.js. On by
  // default for bc-xl7n.74's reason: the failure it fixes is a worker window opened on an
  // epic with no diff left to deliver, whose only honest ending is a hand-written card —
  // this offers the same card for free, before the window ever opens.
  flagFinishedEpics: true,
  // One `bd ready --json` per repo plus one `bd list --parent` per epic it finds, this far
  // apart. An epic finishing is not an event anything else here watches for, so the same
  // ten minutes as the sweep above is cheap relative to how rarely the answer changes.
...
`;
    ...
    for (const repo of dirs) {
      ...
      try {
        ...
        one = await reconcileLanded(bd, a.workspace, repo.dir, {
          base: configuredBase(cfg, a.workspace),
  ...
  async function flagFinishedEpics(a) {
    ...
    const summary = describeFinishedEpics(result);
    a.finishedEpic = { summary, flagged: result.flagged.length, at: a.lastFinishedEpicAt };
    // Quiet skips — no children yet, or still has an open one — are the ordinary state of
    // almost every epic on almost every sweep, and logging them every ten minutes would
    // bury the rare one that could not be read at all.
    for (const s of result.skipped) {
  ...
  async function flagInMain(a) {
    ...
    for (const repo of dirs) {
      let one;
      try {
        one = await sweepInMain(bd, a.workspace, repo.dir, { base: configuredBase(cfg, a.workspace) });
      } catch (err) {
        // A sweep is a courtesy on top of the tick and may not take it down, and one
  ...
  async function flagNotInMain(a) {
    ...
    if (cards) {
      for (const repo of dirs) {
        let f;
        try {
          f = await followNotInMain(bd, a.workspace, repo.dir, { base: configuredBase(cfg, a.workspace), cards });
        } catch (err) {
    ...
    for (const repo of dirs) {
      let one;
      try {
        one = await sweepNotInMain(bd, a.workspace, repo.dir, { base: configuredBase(cfg, a.workspace) });
      } catch (err) {
        threw.push(`${inWhich(repo)}${err.message.split('\n')[0]}`);
        continue;
      }
  ...
  async function reenter(a) {
    ...
    for (const { epic, kids, tree } of advocatedRoots(index)) {
      ...
      try {
        ...
        // event intact.
        keep[epic.id] = { ...(prev || record), at: iso() };
  ...
  async function tickOne(a, sessions) {
    ...
    const globalFree = globalLimit() - totalWorkers() - totalResolvers();
    ...
    if (!a.queue.length) {
      ...
      const holdNotes = `${repoNoteText}${heldNote}${twinNoteText}${prNoteText}${liveNote}${leaseNote}${claimNote}${orphanNote}${pauseNote}`;
      note(
        a,
        a.workers.length
    ...
    if (globalFree <= 0) {
      ...
      const resolverNote = resolving
        ...
        : '';
      return note(a, `${a.queue.length} ready · held by globalMaxWorkers (${globalLimit()})${resolverNote}`);
    }
    if (!ready.length) {
  ...
  async function sweepEmptyWindows() {
    ...
    // tidy, and a line per tick saying "closed 0" is how a log stops being read.
    if (res?.closed) {
...
- **Four labels never reach my queue** (\`QUEUE_EXCLUDED\`, lib/endorse.js). \`human\` — a
  ...
  \`superseded-by:<id>\` are out as well.${labelCounts.length ? ` ${labelCounts.join(', and ')}.` : ''}
- **Priority.** ${
    deferred
...
- **${
    holds.length
      ...
      : `Nothing was held back by contention.** I run only on a tick where all ${QUEUE_HOLDS.length} of my hold lists are empty, so take as given that no ready bead here is waiting on a child, an open pull request, a live session, another Mac's claim, a paused epic, a busy file or a missing checkout. Nothing is hiding behind those.`
  }
