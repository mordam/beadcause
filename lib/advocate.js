...
import { beadToken, multiRepo, repoList, resolveRepo } from './repos.js';
 ...
 */
export function createAdvocates(cfg, { bd, bus, say = messageSession, open = openWorkSession, prs = openWork }) {
  const o = options(cfg);
  ...
  function repoDirs(a) {
    if (!multiRepo(cfg, a.name)) {
      try {
        return [{ name: null, dir: resolveSessionDir(cfg, a.workspace) }];
      } catch {
    ...
    return repoList(cfg, a.name).repos.map((r) => ({ name: r.name, dir: r.dir }));
  }
  ...
  async function survey(a) {
    ...
    const { batchOf, foldedInto } = await batchesFor(a, queue);
    ...
    return withoutLiveSessions(a, withoutOpenPrs(a, await withoutTwins(a, await withoutLeases(a, workable, labels))));
  }
   ...
   * the bead itself is the straightforward one. A claim on an *ancestor* of it is the same
   * fact one level up: the other machine's window is responsible for a subtree, and this
   * bead is inside it. It is the hole a batch head opens — one window, several beads, and
   * only the epic leased — but it is not about batching, and that is why it is stated over
   * ids rather than over anything a worker record carries: from here all we have of the
   * other machine is one label, and the id it hangs on is the name of the subtree.
   *
   ...
   */
  async function withoutLeases(a, queue, labels) {
    ...
    for (const bead of queue) {
      ...
      if (!holder) {
        ...
        const up = await leaseHolderAbove(a, bead.id, { labels, now, cache: above });
        if (up) {
          holder = up.holder;
  ...
  /**
   * Another Mac's live claim on something *above* this bead, or null — the subtree
   * question, asked of the shared tracker rather than of `a.workers`.
   *
   * bc-etbq, and the failure is bc-thid's with the guard removed. Mac A opens one window
   * responsible for an epic and its ready children and leases the epic; Mac B syncs, holds
   * the epic correctly because it can see that claim, and then opens a window on a child —
   * because nothing leases the child, and the worker that would have held it belongs to a
   * `a.workers` on the other machine. Two windows in one subtree, two branches carrying
   * the same work, which is the exact thing lib/lease.js exists to prevent; it just
   * prevented it a bead at a time, and a batch is the first thing here that makes one
   * window responsible for several.
   *
   * **A lease on an ancestor holds the whole subtree, whatever the other window is doing
   * in it.** The label says a machine and a moment and nothing else — not whether that
   * window took a batch, not what it was briefed on — so there is no narrower rule
   * available from here, and the choice is between holding the subtree and the duplicate
   * window. Holding is the cheaper mistake and it is a self-resolving one: the claim
   * expires on lib/lease.js's own clock, comes off when that worker ends, and the hold is
   * on the card with the handle of the person to go and ask while it lasts. The local rule
   * in `heldByChildren` is deliberately *narrower* than this, and the asymmetry is honest
   * rather than an oversight — about this laptop we can see the worker itself.
   *
   ...
   * caller has already returned on `leasing()`. For a child, one `bd show` per ancestor per
   * survey, cached across the queue — and usually zero even then, because a leased ancestor
   * that is still open comes back in the same `bd ready` rows the labels map was built
   * from. The read is only for the ancestor that is *not* ready: claimed, blocked, or under
   * the priority floor.
   *
   * A `bd` that will not answer holds nothing back, exactly as in `heldByChildren`: a
   * tracker mid-write must not be able to empty an advocate's queue. What that costs is
   * one tick's worth of exposure to the race this closes, and `reconcile` is the backstop
   * — it asks the same question of a live worker and stands the window down.
   */
  async function leaseHolderAbove(a, id, { labels = null, now = new Date(), cache = new Map() } = {}) {
    for (const ancestor of ancestorsOf(id)) {
      let owned = labels?.get(ancestor);
      if (owned === undefined) {
        if (!cache.has(ancestor)) {
          try {
            cache.set(ancestor, (await bd.show(a.workspace, ancestor))?.labels || []);
          } catch {
            // Cannot tell, which is not "nobody holds it" — but it is not grounds to hold
            // a bead either. Null rather than `[]` so a retry on a later tick is not
            // answered out of this cache.
            cache.set(ancestor, null);
          }
        }
        owned = cache.get(ancestor);
      }
      if (!owned) continue;
      const v = leaseVerdict(owned, me, { now, ...leaseOpts() });
   ...
   * 2. **A session is already working a child.** The same fact one tick later. Without
   *    it the parent would simply be picked up on the tick after the child was, which
   *    is the incident with a pause in the middle.
   ...
   * reason it should need a human to retype it before anything will pick it up.
   *
   ...
   */
  async function batchesFor(a, queue) {
    const batchOf = new Map();
    const foldedInto = new Map();
    if (o.batchEpicChildren === false) return { batchOf, foldedInto };
    ...
    for (const epic of epics) {
      ...
      // windows would hold overlapping subtrees. Batch heads only, for the reason
      // `heldByChildren` gives on the same test: a session on a plain parent was handed one
      // bead, and an epic's children are still the work.
      if (a.workers.some((w) => isDescendantOf(w.id, epic.id))) continue;
      if (a.workers.some((w) => w.batch?.length && isDescendantOf(epic.id, w.id))) continue;
  ...
  async function heldByChildren(a, bead, queue) {
    ...
    // And the same question the other way up — but only against a **batch head**, and the
    // qualifier is the whole of it.
    //
    ...
    //
    // Asking it unqualified is wrong, and test/twinqueue.mjs holds the line: a session on
    // a plain parent does **not** hold that parent's children, because an epic is not the
    // work — its children are — and holding a child because something sits on its parent
    // leaves nobody doing either. `w.batch.length` is exactly the difference between the
    // two: a worker that was handed the subtree owns it, and a worker that was handed one
    // bead owns one bead. With batching off no worker has a batch, so this never fires and
    // the old behaviour is bit-for-bit intact.
    const above = a.workers.find((w) => w.batch?.length && isDescendantOf(bead.id, w.id));
  ...
  async function reconcile(a, sessions) {
    ...
    for (const w of a.workers) {
      ...
      if (leasing()) {
         ...
         * another machine holds something above it, so its window is responsible for the
  ...
  async function launch(a, bead) {
    ...
    console.log(
      `[advocate] ${a.name}: opened a session on ${bead.id} in ${dir}${inRepo ? ` (${inRepo})` : ''} (${mode}, attempt ${attempt})`
    );
    emit(a, 'launched', {
      id: bead.id,
      title: bead.title,
      detail: `session in ${inRepo || path.basename(dir)}`,
      repo: inRepo,
    });
  }
  ...
  async function surveyAgent(a) {
    ...
    fs.writeFileSync(promptFile, surveyPrompt(a.name, o, reflection, ownerName(cfg), repo?.brief || ''), {
      mode: 0o600,
    });
    const command =
      `P="$(cat '${promptFile}')"; rm -f '${promptFile}'; ` +
      `exec claude -p ${claudeArgs(f).join(' ')} --output-format stream-json --verbose ` +
  ...
  async function tidy(a, sessions) {
    ...
    try {
      ...
      const result = await sweepWorktrees(dir, { sessions, prMerges });
      ...
      const attic = await expireRetired(dir, { sessions, prMerges, days: atticDays(o) });
      ...
      const slim = await slimAttic(dir, { sessions });
      const summary = [describeSweep(result), describeExpiry(attic), describeSlim(slim)].filter(Boolean).join(' · ');
      a.tidy = {
        summary,
      ...
      if (result.retired.length || attic.removed.length || slim.slimmed.length) {
  ...
  async function landed(a, { force = false } = {}) {
    ...
    for (const repo of dirs) {
      let one;
      try {
        one = await reconcileLanded(bd, a.workspace, repo.dir, { base: cfg.pr?.base || 'main' });
      } catch (err) {
  ...
  async function tickOne(a, sessions) {
    ...
    if (!a.queue.length) {
      ...
      const quiet = held || twins || inflightHeld || sitting || claimed || unplaceable;
      note(
        a,
        a.workers.length
          ? `${a.workers.length} session(s) working, nothing else ready${repoNoteText}${heldNote}${twinNoteText}${prNoteText}${liveNote}${leaseNote}`
          : `${quiet ? 'nothing ready' : 'clear — no ready beads'}${repoNoteText}${heldNote}${twinNoteText}${prNoteText}${liveNote}${leaseNote}`
      );
 ...
 */
function surveyPrompt(workspace, o, reflection = '', owner = ownerName(), repoBrief = '') {
