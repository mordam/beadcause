...
`;
  ...
  async function tickOne(a, sessions) {
    ...
    const globalFree = globalLimit() - totalWorkers() - totalResolvers() - reservedForResolvers();
    ...
    if (globalFree <= 0) {
      ...
      const queueNote = inLine
        ...
        : '';
      return note(a, `${a.queue.length} ready · held by globalMaxWorkers (${globalLimit()})${resolverNote}${queueNote}${gaveUpNote(a)}`);
    }
    if (!ready.length) {
