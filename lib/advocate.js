...
export function createAdvocates(cfg, { bd, bus, say = messageSession, open = openWorkSession, prs = openWork }) {
  ...
  async function survey(a) {
    ...
    return withoutLiveSessions(a, withoutOpenPrs(a, await withoutTwins(a, withoutLeases(a, workable, labels))));
  }
   ...
   * the cheapest
   * — the evidence arrived with the `bd ready` rows, so there is no read to throttle and
   * nothing to force before a launch, which is why this filter has no counterpart in
   * `tickOne`'s pre-launch re-reads. What moves inside a tick is a `gh` call or a process
   * appearing; a label cannot change without a sync, and a sync is minutes wide.
   *
   ...
   * older than `leaseMinutes` is not a holder, and the bead comes back on its own.
   */
  function withoutLeases(a, queue, labels) {
    ...
    for (const bead of queue) {
      ...
      const why = describeLease(v.holder, { now });
      a.heldByLease.push({ id: bead.id, why, handle: v.holder.handle, at: v.holder.at });
   ...
   */
  async function standDown(a, w, holder) {
    const why = standDownWhy(holder, w.lease);
    if (w.term && !OBSERVING) {
      try {
        await say(
          w.term,
          `Stand down: ${holder.handle}'s Mac claimed ${w.id} first, so the work on it is theirs. ` +
