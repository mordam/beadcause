...
export function createAdvocates(cfg, { bd, bus, say = messageSession, open = openWorkSession, prs = openWork }) {
   ...
   * only the epic leased. `heldByChildren` closes it for this laptop off `w.batch.length`;
   ...
   *
   * **Epics only, which is the same qualifier `heldByChildren` puts on `above` and for the
   * same reason.** A batch head is always an epic — `batchesFor` picks from `b.type ===
   * 'epic'` — so the case this exists for is covered. A session on a *plain* parent was
   * handed one bead and speaks only for that bead, and holding its children behind it would
   * leave nobody doing either; test/twinqueue.mjs holds that line for the local rule and
   * this must not contradict it, because the same two beads must not resolve one way when
   * the window is on this Mac and another way when it is on the next desk.
   *
  ...
  async function leaseHolderAbove(a, id, { kin = null, now = new Date(), cache = new Map() } = {}) {
    for (const ancestor of ancestorsOf(id)) {
      ...
      if (!row || (row.issue_type || 'task') !== 'epic') continue;
