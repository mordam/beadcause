...
import { sweepWorktrees, describeSweep, expireRetired, describeExpiry } from './tidy.js';
...
export function createAdvocates(cfg, { bd, bus, say = messageSession, open = openWorkSession }) {
  ...
  async function tidy(a, sessions) {
    ...
    try {
      ...
      const summary = [describeSweep(result), describeExpiry(attic)].filter(Boolean).join(' · ');
      a.tidy = { summary, retired: result.retired.length, expired: attic.removed.length, at: a.lastTidyAt };
      ...
      if (result.retired.length || attic.removed.length) {
