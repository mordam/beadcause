...
import { archiveSession, mergeCommitFor, noteMerge, mainCheckout, ranModelsOf, salvageNote } from './sessionlog.js';
...
`;
  ...
  async function archiveFinished(a) {
    ...
    for (const { worker, outcome } of finished) {
      ...
      try {
         ...
         *
         * Three conditions and each is doing work. `handedBack` is what makes this one of
         ...
         */
        if (worker.handedBack && res.head && !res.merged && res.commits.length) {
          try {
  ...
  async function tickOne(a, sessions) {
    ...
    if (!a.queue.length) {
      ...
      const holdNotes = `${repoNoteText}${heldNote}${undecidedNote}${twinNoteText}${prNoteText}${liveNote}${leaseNote}${claimNote}${orphanNote}${pauseNote}${gaveUpNote(a)}`;
      note(
        a,
        a.workers.length
...
- **${
    holds.length
      ...
      : `Nothing was held back by contention.** I run only on a tick where all ${QUEUE_HOLDS.length} of my hold lists are empty, so take as given that no ready bead here is waiting on a child, an open pull request, a live session, another Mac's claim, a paused epic, a busy file, a missing checkout or a decision nobody has made about how a childless epic splits. Nothing is hiding behind those.`
  }
