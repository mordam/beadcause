...
import { openWorkSession, resolveSessionDir } from './session.js';
...
import { MEMORY_BRIEF } from './memory.js';
...
export function createAdvocates(cfg, { bd, bus }) {
  const o = options(cfg);
  ...
  async function reconcile(a, sessions) {
    ...
    for (const w of a.workers) {
      ...
      if (issue.status === 'closed') {
        ...
        finish(a, w, ended ? 'closed by the session, which then exited' : 'closed by the session', 'done');
        continue;
      }
      if (ended) {
        ...
        // And so is delivering. In PR mode the bead is *supposed* to still be open
        // when the session exits: merging is what closes it, and merging is Adam's.
        // Without this the best possible ending would read as "exited unfinished",
        // cost an attempt, and after two deliveries the advocate would give up on a
        // bead whose work was sitting in a pull request waiting to be approved.
  ...
  async function launch(a, bead) {
    ...
    const { dir, mode } = await openWorkSession(cfg, a.workspace, bead, {
      attempt,
  ...
  async function surveyAgent(a) {
    ...
    let reflection = '';
    try {
      reflection = amendment.reflectionPrompt(f, await amendment.refusalsFor(dir, 'advocate'));
    } catch (err) {
    ...
    fs.writeFileSync(promptFile, surveyPrompt(a.name, o, reflection), { mode: 0o600 });
    const command =
  ...
  async function notePending(a, dir) {
    ...
    for (const p of a.pendingNotes) {
      try {
        const merged = await mergeCommitFor(main, p.head);
        if (!merged) {
          still.push(p);
          continue;
        }
  ...
  function control(name, action) {
    ...
    if (action === 'pause') {
      ...
      // "I closed those windows myself." Frees the slots without touching the beads.
      const n = a.workers.length;
      for (const w of a.workers) clearActivity(`${name}/${w.id}`);
      a.workers = [];
      console.log(`[advocate] ${name}: released ${n} slot(s) by hand`);
      emit(a, 'released', { detail: `${n} slot(s) released` });
...
function surveyPrompt(workspace, o, reflection = '') {
...
You are running read-only. You can read the repo, run \`bd\`, and read git history.
You cannot edit anything, and **you must not create any beads** — Adam approves
every bead before it exists. Your entire output is a proposal for him to accept or
reject.
...
Adam within one sentence of reading it. A bug you can point at. A half-finished
...
finished should stay empty; a proposal filed to look busy costs Adam a decision and
...
rediscover from scratch, and Adam is approving it on the strength of what you wrote.

${MEMORY_BRIEF}

You are the agent this matters most to: you survey the same repo again and again, and
"I proposed this and he declined it" is not something the tracker will tell you —
a declined proposal leaves no bead behind. \`recall\` before you propose, and
\`remember\` what he turned down and why. \`post\` on a topic when it is the *other*
