 ...
 *
 * ## And a fourth that is not said from a row at all
 *
 * **Forget** (bc-0i27.19) drops a cancel record whose ticket the poller can no longer
 * find, and it is here rather than beside the record in lib/jiracancel.js for the reason
 * cancel's half is: that file owns what is written, this one owns what a tap means. It
 * is not on a row because there is no row — that is the whole of its case — it is on the
 * cancelled fold, beside a record listed by `strandedCancels`. See the function.
 *
...
import { cancelTicket, cancelledRecord, inSweep, uncancelTicket } from './jiracancel.js';
 ...
 *
 * Four things, in order, and the second is what makes "one epic, not two" true.
 *
 ...
 *    this happened.
 * 4. **And the ingester's memory of this one ticket**, for the same reason one step
 *    down: it skips anything it has already answered about, so a ticket cancelled with
 *    a *failed* reading behind it would come back unreadable forever.
 *
 ...
 */
export async function beadifyTicket(bd, workspace, key, { filer = null, ingester = null } = {}) {
  const name = workspace?.name || '';
  ...
  filer?.forget?.(name);
  // And the *reading* of the ticket is forgotten too, which is the fourth thing and the
  // one that makes "back through ingestion" true rather than nearly true. The ingester
  // skips any ticket it already has an answer about (lib/jiraingest.js), so a ticket
  // cancelled while its ingestion had failed would come back with the failure still in
  // memory and nothing would ever read it again — the row saying *could not be read*
  // with no button left anywhere that would retry. One ticket, not the workspace.
  ingester?.forgetTicket?.(workspace, key);
...
/**
 * Forget: drop a cancel record for a ticket that is not coming back (bc-0i27.19).
 *
 * The fourth act, and the only one of the four that touches no bead at all. A cancel
 * record never expires, and `cancelledTickets` — the fold that draws them — is a filter
 * over what the poller answered. So a ticket cancelled and then resolved, reassigned, or
 * moved out of the configured projects leaves a record with nothing on any screen that
 * can reach it (`strandedCancels` is the list that now can). This is the drop.
 *
 * **Not beadify, and the difference is the epic.** Beadify says *put this ticket back*:
 * it lifts the earmark, reopens the closed epic and expects the row to return on the next
 * sweep. That is exactly wrong here — the ticket is not coming back, so reopening its
 * epic would leave a held bead behind for a ticket nobody is assigned, with no row
 * anywhere to approve or cancel it from. Forget lifts the earmark and *leaves the epic
 * closed*, which is what the history of a cancelled ticket should look like.
 *
 * **It takes a workspace name rather than a workspace**, alone among the acts in this
 * file, and that is the point rather than an inconsistency: the record's whole reason for
 * being stranded may be that its workspace is gone from the config, and a route that
 * resolved it through `requireWorkspace` would 400 on precisely the records this exists
 * to clear. Nothing here calls `bd`, so there is nothing a workspace object would be for.
 *
 * **And it re-asks whether the ticket is live**, against the poller's answer as it is at
 * the moment of the write rather than as it was when the payload was drawn — that is the
 * rule the bead asks for, that no drop may un-cancel a live ticket. A ticket reassigned
 * back to you in the minute since is a 409 naming beadify, not a silent un-cancel.
 */
export function forgetCancel(workspace, key, { tickets = [] } = {}) {
  const name = clean(workspace?.name ?? workspace);
  const k = clean(key);
  if (inSweep(tickets, name, k)) {
    throw Object.assign(
      new Error(`${k} is assigned to you again — beadify it to put it back, rather than dropping its record`),
      { status: 409 }
    );
  }
  const record = cancelledRecord(name, k);
  // Not an error, the same way a second tap on beadify is not: the record is gone, which
  // is what was asked for, and two phones tapping one row must not make the second wrong.
  if (!record) return { forgotten: false, workspace: name, key: k, bead: null };
  uncancelTicket(name, k);
  return { forgotten: true, workspace: name, key: k, bead: record.bead };
}
