...
// The one spelling of "is this session's name the name of a session on *this* bead".
// `name.includes(id)` is the obvious test and is wrong — see the comment on `namesBead`.
...
export function createApp(cfg) {
  ...
  async function p0Board(rows) {
    ...
    for (const [name, ws] of workspaces) {
      ...
      for (const b of beads.values()) {
        ...
        roots.add(b.id);
        p0s.push(p0Card(name, b, beads, children));
      }
   ...
   */
  function p0Card(workspace, bead, beads, children) {
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/bead/advocate' && req.method === 'POST') {
        ...
        const id = String(body.id || '').trim();
        if (!BEAD_ID_RE.test(id)) return json(res, 400, { error: 'not a bead id' });
        // `namesBead`, not `includes`. A bead's children are `bc-xl7n.8`, `bc-xl7n.8.1`,
        // so **every parent id is a prefix of its children's** — and this route was the
        // one call site that never adopted the shared matcher. What that cost, measured
        // 2026-08-13: pressing this on bc-xl7n was refused because a session named
        // `human.bc-xl7n.8.1.…` was open on a *descendant*, and bc-1kwl was refused
        // because of a window on bc-1kwl.2. The refusal named neither, and the console
        // showed no advocate because it draws the windows the daemon opened and this
        // reads every `claude` process on the Mac — so "there already is one" and "I
        // cannot see one" were both true at once.
        const already = liveSessions(cfg).find((sn) => namesBead(sn.name, id));
        if (already) {
          return json(res, 409, {
            ...
            // phone that is reading it, and one that does not is a dead end.
            error: `${id} already has a session open on it — “${already.name}”. One advocate per P0, never two.`,
