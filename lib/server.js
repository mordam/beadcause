      ...
      if (p === '/api/respond' && req.method === 'POST') {
         ...
         */
        {
          ...
          settleSlack(cfg, `${ws.name}/${body.id}`, {
            ...
            verb: ambiguous ? 'Said, not yet decided' : deferring ? 'Set aside' : commission ? 'Handed back' : 'Answered',
        ...
        return json(res, 200, {
          ...
          // is `deferred` and `until` below saying so — a client told `handedBack` would
          // announce work starting over a question that has just been put off.
