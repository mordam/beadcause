...
import { landedEvent, deployEvent, deployClearEvent, syncStuckEvent, syncClearEvent, epicDoneEvent, mutedNews } from './news.js';
...
export function startPoller(cfg, app) {
  ...
  const sweepEpicsDone = async () => {
    ...
    for (const epic of out.done) {
      ...
      bus.emit(epicDoneEvent(epic, { quiet: mutedNews(cfg, epic.workspace) }));
    }
  };
