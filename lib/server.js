...
import { loadState, saveState, saveConfig, OBSERVING, OBSERVING_NOTE } from './config.js';
import { publicRoster, addAgent, removeAgent } from './agents.js';
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/respond' && req.method === 'POST') {
        ...
        return json(res, 200, { ok: true, closed: true, created, declined });
      }
      ...
      if (p === '/api/comment' && req.method === 'POST') {
        ...
        const dispatch = dispatchReply(cfg, ws, body.id, q?.question || q?.title || '', {
          agentId: body.agent ? String(body.agent) : null,
        });
      ...
      if (p === '/api/console/message' && req.method === 'POST') {
        ...
        sendTurn(cfg, c, String(body.text));
      ...
      if (p === '/api/agents' && req.method === 'GET') {
        return json(res, 200, { agents: publicRoster(cfg), default: cfg.defaultAgent || 'answerer' });
      }
