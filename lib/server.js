...
import { ancestorsOf, childrenFrom, treeUnder } from './ancestry.js';
...
import { advocateSession, isCrash, openedRecently, rememberAdvocateOpened, waitingOn } from './epicadvocate.js';
...
import { awaitingEndorsement, endorsementQueue, forget as forgetQueue } from './endorsequeue.js';
...
import { collectBoard, forgetBoard, landLocally, landParent, narrowBoard, pickCard } from './prboard.js';
...
import { deployFor, deployHint, deployable, startDeploy, listDeploys, showDeploy, briefDeploy, deployLog, keyOf, runningFor, whereOf, reportingQuiet, sweepDeploys, unannounced, markAnnounced, ownWorkspace } from './deploy.js';
...
import { KEPT_HEADER, combine as combineKept, describe as describeKept } from './cache.js';
 ...
 * answering one takes back off. Read here by the P0 board, which has the whole graph in
...
export function createApp(cfg) {
   ...
   *
   * **The board is the epics you have *started*, not the ones you have raised** (bc-6s96).
   * A card is drawn for a P0 you own whose status is `in_progress`. Closed has never been
   * on it — a P0 that landed is not something to lead the screen with, and its descendants
   ...
   * You put a P0 on the board by starting it — from the phone, through the picker at the
   * foot of the board (`startable` below, and `POST /api/bead/start`), or on the Mac with
   ...
   * effect.** The list is narrowed by `under`, which is keyed on `roots` — so a question
   * belonging to a P0 you have not started leaves the inbox with its card. It is not moved
   * anywhere and it comes back the moment the epic is started; nothing is dispatched under
   * a not-started epic anyway. What it must *not* do is come back as `unhomed`, because
   * that map means "nobody's P0 has this" and would be a lie here — so `anyP0` below stays
   * every *open* P0 in the workspace, deliberately wider than `roots`, and a row under a
   * P0 of yours that has not started is in neither map. Two maps disagreeing on purpose.
   *
   * **And with nothing started the board is empty, which switches the whole section off.**
   * The client draws the flat inbox when `p0s` is empty (`isBoarded` in public/app.js) —
   ...
   * empty one.
   *
   * **lib/underp0.js is untouched, and the two are supposed to differ now.** Workability
   * still means an *open* P0 above — the gate is asking "did anybody decide this work
   * should happen", and raising a P0 is that decision whether or not you have got to it.
   * Narrowing the gate to started epics would stop the advocate on five sixths of the
   * tracker overnight.
   *
   ...
   * the tree on each card** (bc-rfnr.9.1): every descendant of that P0 at any depth, not
   * only the ones with a question pending. It comes off the same cached shape in the same
   * pass, so a board of a dozen P0s over 800 beads is one inversion of the parent map and
   ...
   * carries a count and no tree — see `p0Offer` — because forty candidates each carrying
   ...
   * open P0 above it *at all* — not yours, not anybody's — and the client draws those
   * whatever the board says. Without it, `under` alone decides, and a question filed with
   * no parent is in no P0's descendants, so it is drawn on no screen: `/api/ask` (the
   ...
   * inbox with nothing saying so. lib/underp0.js's pill is the *held* half and is loud;
   * this is the invisible half, and a pill on a screen that does not draw the card cannot
   * reach it. It is not the same question as `under`: "which P0 do I hang off" and "does
   * anybody's P0 have me" have different answers on a shared graph, and only the second
  ...
  async function p0Board(rows) {
    if (!meHandles(cfg).length) return { p0s: [], startable: [], under: {}, unhomed: {}, owned: false };
    const p0s = [];
    const startable = [];
    const under = {};
    const unhomed = {};
    ...
    // Started. Normalised the way lib/underp0.js normalises its own status read, because
    ...
    for (const [name, ws] of workspaces) {
      ...
      // dozen P0s each inverting it themselves is a dozen passes over 800 beads for an
      // index that is the same every time.
      const children = childrenFrom(parents);
      const roots = new Set();
      for (const b of beads.values()) {
        if (!isP0(b) || !ownedByMe(cfg, b)) continue;
        if (!started(b)) {
          if (offerable(b)) startable.push(p0Offer(name, b, beads, children));
          continue;
        }
        roots.add(b.id);
        p0s.push(p0Card(name, b, beads, children, sessions));
      }
      // Every open P0 in this workspace, whoever owns it — lib/underp0.js's root set, and
      // deliberately a wider one than `roots` above, now in two directions rather than one.
      // The board is `ownedByMe` and `in_progress` because it answers "what am I working
      // on"; `unhomed` is answering "has anybody at all decided where this belongs", and a
      // colleague's P0 — or your own, raised and not yet started — is that decision. That
      // is the gate's own distinction, kept here so the two cannot drift apart. The gap
      // between them is where a row under a not-started P0 of yours sits: in `under` no
      // longer, and never in `unhomed`, so it leaves the list without being relabelled as
      // a bead nobody has homed (bc-6s96).
      const anyP0 = p0RootsOf(beads);
      for (const r of rows) {
        ...
        // Not under one of yours. Is it under anybody's? No — and no P0 in the workspace
        // at all counts as no, which is where a tracker nobody has raised a P0 in lands:
        // the flat list, not an empty screen. (`hasP0Above` answers *true* on that shape,
        ...
        if (isBead(name, r) && !line.some((id) => anyP0.has(id))) unhomed[r.key] = true;
      }
    }
    ...
    // P0 with the most still open under it is the one the week is most likely to be
    // about, and a picker that ordered by id would put the epic filed first at the top
    // for ever. Ties by id and not by date, because `bd export` carries `created_at` as
    // a string this has no reason to parse and a stable order is all a tie needs.
    startable.sort((a, b) => b.open - a.open || String(a.id).localeCompare(String(b.id), 'en', { numeric: true }));
    return { p0s, startable, under, unhomed, owned: true };
  }
   ...
   * `under` is a fact about the *inbox rows*: which P0 each one hangs off, so the list can
   ...
   */
  function p0Card(workspace, bead, beads, children, sessions = []) {
    ...
    return {
      ...
      // between the launch and the window naming itself, or null, which is the card's
      // offer to put somebody there. See `advocateSession`.
      advocate: advocateSession(sessions, bead.id, { openedAt: openedRecently(`${workspace}/${bead.id}`) }),
      // Everything under this P0 at any depth, closed included, parents before children.
  ...
  /**
   * One P0 as the picker draws it — a card that could be, without the cost of one.
   *
   ...
   *
   * What it deliberately does not carry is `tree`. A started P0 sends every descendant so
   ...
   */
  function p0Offer(workspace, bead, beads, children) {
   ...
   * else (`p0Board`). Not a phone-local pin: the same fact then reads the same from `bd
  ...
  async function boardMove(body, { on }) {
    ...
    if (!isP0(bead)) refuse(`${id} is not a P0 — the board is the P0s you own`);
    if (!ownedByMe(cfg, bead)) refuse(`${id} carries nobody's owner: label for you — it is not yours to put on the board`);
    if (status === 'closed') refuse(`${id} is closed — a P0 that landed does not lead the screen`);
    if (on) {
    ...
    console.log(`[beadcause] ${on ? 'started' : 'un-started'} ${ws.name}/${id} from the P0 board`);
  ...
  async function answerShape(ws, id, optionId) {
    ...
    return {
      ...
      anyCommission: (question?.decision?.options || []).some((o) => o.closes === false),
    };
  }
      ...
      for (const bead of toCreate) {
        const newId = await bd.create(
          ws,
          {
            ...
            // (bc-nc6o).
            labels: ['advocate', ...bead.labels, ...complexityLabels(bead.complexity)],
          },
  ...
  async function finishWorkBead(ws, workId, questionId, reason, { actor = null } = {}) {
    ...
    // The reason rides along because one gate is about the sentence rather than the
    // bead: an epic does not close because a pull request merged (lib/bd.js). A work
    // bead — which is what this nearly always is — is unaffected by it.
    ...
    if (gate.kind === 'merge-reason') {
      console.log(`[pr] ${ws.name}: ${workId} stays open — ${gate.reason}`);
      await bd
        .comment(ws, workId, `${reason}. This epic stays open: ${gate.reason}. Close it when the theme is done, not on this merge.`, { actor })
  ...
  async function runMergeQueue({ log = (line) => console.log(line) } = {}) {
    ...
    for (const ws of cfg.workspaces || []) {
       ...
       * cached for a minute and already kept warm by the P0 board, and it carries the two
      ...
      try {
        out = await sweepMergeQueue(bd, ws, {
          ...
          openResolver: async (entry, dir) => {
            ...
            return Boolean(outcome?.opened || outcome?.queued || outcome?.reused);
          },
          ...
          afterMerge: async (entry, landed, where) => {
            ...
            /**
             * And the notification with nothing to answer — `pushLanded`, unchanged, and
             * moved here for the same reason the merge was.
             *
             * It used to be `bin/deliver.js`'s last act. Losing it would have been the
             * quietest regression in this whole change: work would land correctly, close
             * correctly, and simply stop being mentioned — and the one push in beadcause
             * that reports a decision *already taken on your behalf* is exactly the one
             * whose absence nobody notices until they go looking for something that
             * shipped a week ago.
             */
            await pushLanded(cfg, {
              ...
              owed: '',
            }).catch((err) => log(`[merge-queue] ${ws.name}: merged #${entry.spec.number}, but the notification did not send — ${String(err.message || err).split('\n')[0]}`));
          },
  ...
  async function resolveSweepFor(ws, id, response, { option = null } = {}) {
    ...
    const brief = {
      number: row.number,
      branch: row.branch,
      base: rec.base || cfg.pr?.base || 'main',
      title: row.title || '',
      url: row.url || '',
      repo: rec.repo || '',
  ...
  async function serveStatic(req, res, url, urlPath) {
    ...
    // and not three, unlike its neighbours above: it has always been reached from the
    // app's own navigation, so that is the only thing that has ever pointed at it and there is no
    ...
    if (urlPath === '/closed' || urlPath === '/done') {
      const narrowed = new URLSearchParams(url.search);
      narrowed.set('status', 'closed');
      return redirect(res, `/history?${narrowed}`);
    }
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/respond' && req.method === 'POST') {
        ...
        else if (commission) await bd.commission(ws, body.id, answer, { actor: who, stayInInbox: resolving.bound });
        else {
         ...
         */
        {
          ...
          settleSlack(cfg, `${ws.name}/${body.id}`, {
            response,
            by: by || null,
            verb: ambiguous ? 'Said, not yet decided' : commission ? 'Handed back' : 'Answered',
        ...
        return json(res, 200, {
          ...
          handedBack: commission && !ambiguous,
       ...
       *     been**. `wantsAdvocate` needs a P0 carrying an `owner:` label; the skill files
       *     handoffs as `--type=task --priority=1` and never claims them, deliberately,
       *     because a handoff is the top of the next session's queue, not a P0 on the board.
       ...
       *   - **Never two on one handoff**, through the same `advocateSession` the P0 door
      ...
      if (p === '/api/bead' && req.method === 'GET') {
        ...
        // And which model a session on this bead runs, on the same terms as `noP0`: a
       ...
       * workspaces configured here. The inbox's own P0 board warms all of them with the
       ...
       * already gives: the P0 board keys every row by the P0 it descends from, and a P0
       ...
       * what it enrols a P0 on is the waiting-on sentence *this* window writes before it
       * exits. So the tap is not a weaker version of the loop — it is what starts one, and
       * it stays the fallback for the P0 whose advocate died before writing anything down.
       *
          ...
          return json(res, 409, {
            error: already.opening
              ...
              : // The window's own name, in the sentence rather than only in the payload:
                ...
                `${id} already has a session open on it — “${already.name}”. One advocate per P0, never two.`,
            live: already.name || null,
            pid: already.pid,
          });
        }
  ...
  return { handler, allQuestions, foundationRequests, splitChannels, bd, hooks, bus, advocates, syncer, jira, jiraEpics, jiraResolved, jiraIngest, runMergeQueue };
}
...
export function startPoller(cfg, app) {
  ...
  const sweepRelease = async () => {
    ...
    // `under <p0>` where the pull request named a bead with a P0 over it, and nothing
