...
import { multiRepo, reposBlock, splitRepoKey, unitFor, whereLanded } from './repos.js';
...
// `advocacy` field on `rootCard`. lib/finishedepic.js imports nothing, so this costs the
// server no new dependency edge.
...
import { accountAgainst, resolveFor, setMaxLive } from './resolvers.js';
...
// by `/api/workspaces` for the two things only discovery knows: where a tracker being
// brought back actually lives now, and whether one is findable at all.
import { discoverWorkspaces } from './workspaceroots.js';
...
export function createApp(cfg) {
      ...
      try {
        out = await sweepMergeQueue(bd, ws, {
          ...
          openReview: async (entry, dir, outcome) => {
            ...
            return Boolean(opened?.opened || opened?.queued || opened?.reused);
          },
          ...
          afterMerge: async (entry, landed, where) => {
            ...
            /**
             * And the notification with nothing to answer, which used to be
             * `bin/deliver.js`'s last act and was moved here for the same reason the
             * merge was.
             *
             * Losing it would have been the quietest regression in that change: work
             * would land correctly, close correctly, and simply stop being mentioned —
             * and the one notification in beadcause that reports a decision *already
             * taken on your behalf* is exactly the one whose absence nobody notices
             * until they go looking for something that shipped a week ago.
             *
             * An event now rather than an ntfy push (bc-ka5y.15.1), which is why there
             * is nothing to `await` and nothing to fail: the phone is already parked on
             * `/api/poll` and this wakes it. The four merge doors are enumerated in
             * lib/mergesweep.js's header, and this is deliberately not on all four —
             * the two that *are* a tap of yours, a delivery card and the PR board's
             * Merge button, must not chime for it. See lib/news.js.
             */
            const landing = landedEvent(
              {
                workspace: ws.name,
                bead: entry.spec.bead,
                repo: entry.spec.repo,
                number: entry.spec.number,
                url: entry.spec.url,
  ...
  const viewHop = (view, url, narrow) => {
    ...
    return `/${search ? `?${search}` : ''}#${view}${query ? `?${query}` : ''}`;
  };
  ...
  async function serveStatic(req, res, url, urlPath) {
    ...
    //
    // public/history.js still holds the document half and public/history.html is still
    // on disk; both are unreachable from here, and taking them out is bc-khoe.30.15
    // rather than this bead, which is about where the addresses land.
    ...
    // in public/viewbar.js, and `/releases.html` is the file itself.
    //
    // A hop since bc-khoe.30.14 filled the Releases container. `/releases.html` joins the
    // other two here for the reason `/history.html` did: it was the service worker's own
    // name for the page, and the shell is what the worker precaches now.
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/prs' && req.method === 'GET') {
        ...
        const { kept, ...swept } = await collectBoard(bd, cfg, { force });
      ...
      if (p === '/api/queues' && req.method === 'GET') {
        const force = url.searchParams.get('refresh') === '1';
        const { kept: boardKept, ...swept } = await collectBoard(bd, cfg, { force });
        const board = narrowBoard(swept, (card) =>
          repoInAccount(activeAccount(cfg, loadState()), card.workspace, splitRepoKey(card.key).wanted)
        );
        const { merges, errors, kept: mergeKept } = await gatherMerges(bd, cfg, { refresh: force });
      ...
      if (p === '/api/respond' && req.method === 'POST') {
         ...
         * without handing anything over: the words go on the thread and the card stays in
         * the inbox with its options, because what you said was "leave this on the list".
         * bc-7qo.10 offered precisely that, hinted "keeps this card on the list", and
         * tapping it took the `human` label off and dropped the bead into `bd ready`,
         * where the next advocate tick opened a worker window on the question that had
         * just been deferred.
         *
         ...
         * of "answered, still yours". `resolving.bound` wins where both somehow hold,
         * because an instruction aimed at a branch is a fact about the sentence rather
         * than about the button, and it is decided above with a write behind it.
         */
        const deferring = !resolving.bound && shape.picked?.defers === true;
        const commission = resolving.bound || shape.picked?.closes === false || ambiguous;
              ...
              : resolving.bound
                ...
                : deferring
                  ? // Said in as many words because the reader is an agent deciding whether
                    ...
                    // question that had just been put off.
                    'Left open and still in the inbox — this answer defers the question rather than settling it or commissioning anything. Nothing has been handed to an agent.'
        ...
        // exactly where it is, because the card staying on your list *is* the answer.
        if (ambiguous) await bd.noteOnly(ws, body.id, answer, { actor: who });
        else if (commission)
        ...
        if (commission && !ambiguous && !deferring) {
          try {
        ...
        console.log(
          `[beadcause] answered ${ws.name}/${body.id}${who ? ` as ${who}` : ''}${
            ambiguous
              ...
              : deferring
                ? ' — deferred: left open in the inbox, and nothing handed to an agent'
                : commission
         ...
         */
        {
          ...
          settleSlack(cfg, `${ws.name}/${body.id}`, {
            response,
            by: by || null,
            verb: ambiguous ? 'Said, not yet decided' : deferring ? 'Deferred' : commission ? 'Handed back' : 'Answered',
        ...
        return json(res, 200, {
          ...
          // was handed to anybody. It is the one of the three non-closing endings where
          // the card is *still yours*, and a client told otherwise would announce a
          // hand-back over a question it is still showing you.
          handedBack: commission && !ambiguous && !deferring,
      ...
      if (p === '/api/unendorsed' && req.method === 'GET') {
        ...
        const { kept, ...body } = await endorsementQueue(bd, scopedWorkspaces(), { refresh });
