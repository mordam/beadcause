...
/**
 * Is any of these keys worth filling right now? bc-1kwl.4's whole gate, in one place.
 *
 * `keys` is `{ ws, key }` per workspace, because both halves of the question are
 * per-workspace: whether anything is kept for that key, and whether *that* tracker has
 * been written to. A fan-out answers yes if any one of its keys does — the producers
 * these gate are whole-fleet functions, and lib/cache.js then does the per-key work for
 * nothing, skipping the ones that are still fresh.
 *
 ...
 * the clock says. A key that *has* something kept is warmed only when its tracker has
 * moved since — because `bd` is the only source these have, so a manifest that has not
 * moved (lib/detect.js) means a fresh sweep would return the same bytes. Re-asking
 ...
 */
export function warmDue(keys, { changed = new Set(), now = Date.now(), floorMs = 30_000, peek = cachePeek } = {}) {
  return (keys || []).some(({ ws, key }) => {
    const kept = peek(key);
    if (!kept) return true;
...
export function createApp(cfg) {
  ...
  /**
   * What the warmer fills on a clock, cheapest first, each with the keys it stands for.
   *
   * Built per pass rather than once, so a workspace added to `cfg` is warmed without a
   * restart. `keys` is what `warmDue` gates on and `fill` is the ordinary route-side
   * function — deliberately, because a warmer holding its own copy of a producer is two
   * call sites that drift, and one of them is the one nobody tests. See `warmKeys`.
   */
  const warmables = () => [
    {
      what: 'the foundation channel',
      keys: (cfg.workspaces || []).map((w) => ({ ws: w.name, key: `foundation:${w.name}` })),
      fill: () => foundationRequests(),
    },
    {
      what: 'the agent beads',
      keys: (cfg.workspaces || []).map((w) => ({ ws: w.name, key: `agentbeads:${w.name}` })),
      fill: () => agentBeads(),
    },
    {
      what: 'the console rows',
      keys: (cfg.workspaces || []).map((w) => ({ ws: w.name, key: workKey(w.name) })),
      ...
      fill: () => collectWork(bd, cfg.workspaces || []),
    },
  ];
   ...
   * show`s, then the board — and it is awaited step by step rather than fanned out, so
   * a warm pass is one thing queueing on Dolt's single writer rather than four.
   *
   * The last two are cold-only and own that rule themselves: `warmQueue` because it is
   * the most expensive sweep in the app, `warmBoard` because it is the only one that
   * reaches the network. Both are argued where they live.
   *
  ...
  async function warmKeys({ moved = [], floorMs = WARM_FLOOR_MS, now = Date.now() } = {}) {
    ...
    for (const entry of warmables()) {
      if (!warmDue(entry.keys, { changed, now, floorMs })) continue;
      try {
        await entry.fill();
        filled.push(entry.what);
      } catch (err) {
...
export function startPoller(cfg, app) {
  ...
  const cycle = async () => {
    ...
    // And the same beat fills the keys behind the screens the notification just sent
