...
export function createApp(cfg) {
   ...
   *
   * **Closed roots are not on the board.** A root that landed is not something to lead the
  ...
  async function rootBoard(rows) {
    if (!meHandles(cfg).length) return { roots: [], under: {}, unhomed: {}, owned: false };
    const cards = [];
    const under = {};
    const unhomed = {};
    ...
    for (const [name, ws] of workspaces) {
      ...
      for (const b of beads.values()) {
        if (String(b.status) === 'closed' || !isRoot(b) || !ownedByMe(cfg, b)) continue;
        mine.add(b.id);
      ...
      // and deliberately a wider one than `mine` above. The board is `ownedByMe` because it
      // answers "what am I answerable for"; `unhomed` is answering "has anybody at all
      // decided where this belongs", and a colleague's epic on a shared graph is a decision.
    ...
    return { roots: cards, under, unhomed, owned: true };
  }
