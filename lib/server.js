...
export function createApp(cfg) {
   ...
   */
  const WARMABLES = [
    { what: 'the foundation channel', key: (name) => `foundation:${name}`, fill: (only) => foundationRequests({ only }) },
  ...
  async function warmKeys({ moved = [], floorMs = WARM_FLOOR_MS, now = Date.now() } = {}) {
    ...
    for (const entry of WARMABLES) {
      ...
      const only = (cfg.workspaces || []).filter((w) => warmDue(entry.key(w.name), w.name, { changed, now, floorMs }));
      if (!only.length) continue;
      try {
        await entry.fill(only);
        filled.push(only.length === (cfg.workspaces || []).length ? entry.what : `${entry.what} (${only.map((w) => w.name).join(', ')})`);
      } catch (err) {
