  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/bead/create' && req.method === 'POST') {
        ...
        // (bc-xl7n.115); `fileBeads` still carries the old sentence and the same false
        // claim for a root-shaped bead, tracked separately as bc-xl7n.128, because it
        // judges a whole filed *batch* at once rather than one bead. And `!isRoot(...)`:
        // an epic, or a P0 of any type, needs nothing above it to be workable
        // (lib/underroot.js says so in as many words), so the same warning about *this*
        // bead would be exactly as false — the bug bc-xl7n.115 fixed here.
