...
export function createApp(cfg) {
  ...
  async function p0Board(rows) {
    ...
    for (const [name, ws] of workspaces) {
      ...
      let beads;
      try {
        ({ parents, beads } = await bd.graph(ws));
      } catch {
   ...
   *
   * `waitingOn` is null and stays null until bc-rfnr.3 puts an EpicAdvocate behind each
   * P0 — that sentence is the advocate's to write, and inventing one here from the
   * dependency graph would be a different claim in the same field. The client draws
   * nothing where it is null rather than a placeholder, so the card gains a line the day
   * the advocate lands and loses nothing today.
   */
  ...
  async function inboxPayload(rows, requests, channels) {
