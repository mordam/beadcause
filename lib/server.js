...
import { CHILD_MODES, moveSpace, publicPlan } from './movespace.js';
...
// The PR board's Merge button and `beadcause-merge` are one act now (bc-02ldo) — the
// decision, and the writes that put a pull request where the queue will find it.
import { admitPlan, admitToQueue } from './mergeadmit.js';
...
export function createApp(cfg) {
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      /**
       * Move a bead into another bead-space — the sheet's ⋮ *Move*, and the one act on a
       * bead that two trackers have to agree about.
       *
       * **A route of its own rather than a field on `/api/bead/edit`, and not because of
       * where the code lives.** Every field that route writes is a fact about one bead in
       * one graph, and `changed: false` is its ordinary answer. This one writes into a
       * second Dolt database, closes the bead it was given, may create an epic beside it
       * and may reparent its children — there is no version of it that is a save. Folding
       * it in would have given a form that posts on every keystroke a code path that
       * closes beads.
       *
       * **`dryRun` is not a courtesy, it is how this is meant to be used.** The plan names
       * the new id of every bead that would cross, every child that would stay and where
       * it would land, the epic it would mint and the edges the copy would not carry — and
       * the ids in it are the ids the real run uses, because `mintId` is a deterministic
       * hash rather than something `bd create` hands back. So the phone can show what is
       * about to happen and the confirmation is over the actual plan, not over a promise.
       *
       * **Two refusals, both 409, both naming what is in the way**: a live window on any
       * bead being moved (the process table, lib/onewindow.js) and an open pull request
       * against one (lib/inflight.js). A `gh` that cannot answer holds nothing back and
       * says so in `prCheck.reason` — an unreachable GitHub must not be the reason a
       * tracker cannot be tidied, which is lib/inflight.js's own doctrine.
       *
       * `rows` — the whole source export, which `planMove` and `applyMove` share so the
       * plan shown is the plan run — is stripped by `publicPlan` before anything reaches
       * the wire.
       */
      if (p === '/api/bead/move' && req.method === 'POST') {
        const body = await readBody(req);
        const ws = requireWorkspace(body.workspace);
        const target = requireWorkspace(body.to);
        const id = String(body.id || '').trim();
        if (!BEAD_ID_RE.test(id)) return json(res, 400, { error: 'not a bead id' });
        const children = String(body.children || 'leave').trim();
        if (!CHILD_MODES.includes(children)) {
          return json(res, 400, { error: `children may be ${CHILD_MODES.join(' or ')}` });
        }
        // A workspace this Mac cannot place a checkout for has no pull requests to ask
        // about, which is a missing check rather than a refusal — `planMove` says so in
        // `prCheck.reason` and carries on.
        let dir = '';
        try {
          dir = resolveSessionDir(cfg, ws);
        } catch {
        ...
        const out = await moveSpace(bd, {
          from: ws,
          to: target,
          id,
          children,
          epicTitle: String(body.epicTitle || '').trim(),
          dryRun: Boolean(body.dryRun),
          actor: actorFor(req),
          dir,
        });
        // The queue draws titles, priorities and labels off beads that have just been
        // closed, and the board's shape has changed on both sides — the same drop
        // `/api/bead/edit` makes, for the same reason, and unconditional here because a
        // move that got as far as writing anything has changed what both lists say.
        if (out.applied) forgetQueue();
        return json(res, out.refused ? 409 : 200, publicPlan(out));
      }
