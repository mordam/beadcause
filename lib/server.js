...
import { spaceFor, isQuiet, summarise, reconcileFilter } from './spaces.js';
    ...
    try {
      if (p === '/api/questions' && req.method === 'GET') {
        ...
        return json(res, 200, {
          ...
          summary: summaryNow(),
          // The chips the phone should be wearing, carried by the request that brings
          // the rows they filter. On the same response on purpose: a filter fetched
          // separately arrives after the first paint, and the whole complaint is
          // about seeing the unfiltered list before the filter catches up.
          //
          ...
          // while nobody is writing anything. See reconcileFilter.
          filter: reconcileFilter(spaces, [...workspaces.keys()], loadState().filter),
          scope,
        });
      }

      /**
       * The inbox's space and workspace chips, held for every device rather than per
       * browser.
       *
       * Server-side and not localStorage for two reasons. The notification path runs
       * in the poll and has to be able to read the same value — a filter that only
       * exists in one browser's storage cannot silence a push. And one install with
       * one human should not have a phone and a laptop disagreeing about what is
       * filtered. The accepted consequence is the obvious one: changing it in one
       * place changes it everywhere, on the other device's next poll.
       *
       * Stored as sent, not validated against the current config. The client can only
       * send a chip that was on its screen, checking would cost the `bd` sweep that
       * builds the space counts, and a chip tap has to feel instant — staleness is
       * handled where it actually shows up, on the way back out.
       *
       * Not the scope setting (`human` / `both` / `agent`). That stays in
       * localStorage: it is which slice of the tracker you are looking at, which is
       * a property of the screen in front of you, and it is deliberately no part of
       * what decides whether a question pushes.
       */
      if (p === '/api/filter' && req.method === 'POST') {
        const body = await readBody(req);
        const pick = (v) => {
          // Strings only — `String(v)` would turn a stray object into the filter name
          // "[object Object]", which is a real name as far as everything downstream is
          // concerned and matches nothing forever.
          const s = typeof v === 'string' ? v.trim() : '';
          // Long enough for any real space or workspace name, short enough that a junk
          // body cannot grow the state file the poller rewrites every tick.
          return s && s.length <= 120 ? s : 'all';
        };
