...
import { advocateSession, OPENING_TTL_MS, waitingOn } from './epicadvocate.js';
...
import { writtenByDaemon } from './byline.js';
...
export function createApp(cfg) {
  ...
  /**
   * P0s an advocate was launched on here, by key, and when.
   *
   * In memory and not on disk on purpose: it answers one question — "is the window we
   * just opened still on its way up?" — and a daemon restart is a fact that makes the
   * answer no. What survives a restart is the session itself, which the name match finds
   * without this. Pruned as it is read, so it holds a handful of entries on the busiest
   * day and nothing on an ordinary one.
   */
  const advocateOpened = new Map();

  function openedRecently(key, now = Date.now()) {
    const at = advocateOpened.get(key);
    if (!at) return null;
    if (now - at >= OPENING_TTL_MS) {
      advocateOpened.delete(key);
      return null;
    }
    return at;
  }
  ...
  async function inboxPayload(rows, requests, channels = ['questions', 'beads', 'foundation']) {
    ...
    return {
      ...
      tickets: liveTickets(held).map(stampTicket),
      ...
      cancelledTickets: cancelledTickets(held).map(stampTicket),
    ...
    try {
      ...
      if (p === '/api/spaces' && req.method === 'GET') {
        ...
        return json(res, 200, {
          spaces,
          workspaces: names,
          // Per workspace and *only* per workspace: the picker sums its repos for a
          // space's total and ignores `spaces[].count` entirely, because two numbers for
          // the same thing computed two ways is how they start disagreeing — which they
          // did, the moment a page published counts of its own over a scope this cache
          // knows nothing about. See "the numbers" in public/spacebar.js.
      ...
      if (p === '/api/notifications/dismiss' && req.method === 'POST') {
        ...
        for (const key of asked) {
          ...
          bus.emit({
            ...
            // Why, for the log and for a client that wants to say so. The other one is
            // `addressed`, from `/api/bead/addressee` — a question handed to somebody
            // else has stopped being this phone's, which is the same act aimed at a
            // stronger fact. Naming it beats a client inferring it from the route.
            reason: 'filtered',
          });
        }
      ...
      if (p === '/api/bead/advocate' && req.method === 'POST') {
        ...
        if (already) {
          return json(res, 409, {
            error: already.opening
              ...
              : `${id} already has a session open on it — one advocate per P0, never two`,
        ...
        advocateOpened.set(`${ws.name}/${id}`, Date.now());
