...
import { parseProposal, isApproval, parseApproval } from './proposal.js';
...
export function createApp(cfg) {
   ...
   *
   * `terminals.open` is handed in from here rather than imported there so a
   * reopened terminal is seeded with the same brief `POST /api/terminal` writes,
   * from the one place that knows how to write it.
   */
   ...
   */
  async function createProposed(ws, id, response, picked) {
    ...
    const chosen = wanted.all ? proposal.beads : proposal.beads.filter((_, i) => wanted.indices.includes(i + 1));
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/respond' && req.method === 'POST') {
        ...
        return json(res, 200, { ok: true, closed: true, created, declined, amendment: amended.result });
      }
