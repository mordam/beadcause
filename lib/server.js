...
export function createApp(cfg) {
  ...
  async function p0Board(rows) {
    ...
    for (const [name, ws] of workspaces) {
      ...
      for (const b of beads.values()) {
        ...
        roots.add(b.id);
        p0s.push(p0Card(name, b, beads, parents, sessions));
      }
   ...
   */
  function p0Card(workspace, bead, beads, parents, sessions = []) {
...
export function startPoller(cfg, app) {
  ...
  const sweepMerges = async () => {
    for (const outcome of await sweepMerged(app.bd, cfg)) {
      ...
      if (!line) continue;
      if (outcome.status === 'swept' && outcome.result?.error) console.error(`[prsweep] ${outcome.workspace}: ${line}`);
