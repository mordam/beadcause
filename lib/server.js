  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/bead/advocate' && req.method === 'POST') {
        ...
        //
        // Measured from the phone on 2026-08-13, before this: pressing it on bc-xl7n was
        // refused because a session named `human.bc-xl7n.8.1.…` was open on a *descendant*,
        // and bc-1kwl because of a window on bc-1kwl.2. Worth keeping the other half of
        // that report beside it, because no fix here addresses it — the console draws the
        // windows the *daemon* opened and this reads every `claude` process on the Mac, so
        // "there already is one" and "I cannot see one" can both be true at once.
