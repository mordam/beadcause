...
// The other side of the same tracker: what is finished, rather than what is waiting to be
// looked at. A leaf on lib/spaces.js alone, so it adds no edge to an import graph an
// ordering mistake has already broken once (bc-u4na).
import { closedHistory, PAGE_DEFAULT } from './history.js';
...
import { intake as intakeError, ERROR_LABEL } from './errors.js';
import {
  messageSession,
  openConflictSession,
  openSession,
...
import { deliveryAction, parseDelivery, cardsForDelivery, DELIVERY_LABEL } from './delivery.js';
...
export function createApp(cfg) {
   ...
   */
  function inboxPayload(rows, requests, channels) {
    const trouble = sweepTrouble(channels);
  ...
  async function serveStatic(req, res, url, urlPath) {
    ...
    // What has been finished. Three paths for the same reason the endorsement queue has
    // three: the screen has more than one honest name, and which one you reach for depends
    // on the question you arrived with. "What did I get done" is `/history`; "did that ever
    // close" is `/closed`; `/done` is what a thumb types.
    if (urlPath === '/history' || urlPath === '/closed' || urlPath === '/done') {
      urlPath = '/history.html';
    }
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/pr/detail' && req.method === 'GET') {
        const ws = requireWorkspace(url.searchParams.get('workspace'));
        const row = await prBoardRow(ws, Number(url.searchParams.get('number')), {
      ...
      if (p === '/api/pr/conflicts' && req.method === 'POST') {
        ...
        const opened = await openConflictSession(cfg, ws, row);
      ...
      if (p === '/api/release/ship' && req.method === 'POST') {
        ...
        if (board.unavailable) throw Object.assign(new Error(board.unavailable), { status: 409 });
        const card = board.repos.find((r) => r.workspace === ws.name);
        if (!card) throw Object.assign(new Error(`no ${ws.name} on the board`), { status: 404 });
        if (card.error) throw Object.assign(new Error(card.error), { status: 409 });

        const queue = releaseFor(card, listDeploys({ limit: 200 }).filter((d) => d.workspace === ws.name), loadLedger() || {});
        if (!queue.count) {
          return json(res, 409, { error: `nothing is waiting to ship in ${ws.name} — everything merged is already live` });
        }
        ...
        if (!plan) {
          return json(res, 409, {
            error: `${ws.name} declares no deploy beadcause can run, so its queue cannot be shipped in one press — open a pull request's own Ship instead`,
          });
        }
        ...
        console.log(
          `[release] ${ws.name}: shipping ${queue.count} merged pull request${queue.count === 1 ? '' : 's'} — ${rec.id} (pid ${rec.pid})`
        );
      ...
      if (p === '/api/space' && req.method === 'POST') {
        ...
        let changed;
        try {
          changed = applySettings(space, body.settings);
        } catch (err) {
        ...
        if (changed.length) {
          saveConfig(cfg);
          console.log(`[beadcause] space ${name}: ${changed.join(', ')} changed from the app`);
      ...
      if (p === '/api/poll' && req.method === 'GET') {
        ...
        if (since > bus.seq) {
          ...
          return json(res, 200, {
            ...
            workspaces: [...workspaces.keys()],
            ...(fresh
              ? inboxPayload(fresh.questions, fresh.requests, ['questions'])
        ...
        return json(res, 200, {
          ...
          ...(polled
            ? inboxPayload(polled.questions, polled.requests, ['questions'])
            : { questions: null, requests: null, spaces: null }),
      ...
      if (p === '/api/error' && req.method === 'POST') {
        ...
        try {
          ...
          // wake every parked poller several times a second.
          if (out.action !== 'commented') {
      ...
      /**
       * What has been finished: every closed bead, newest first, one page at a time.
       *
       * The only list in this app that is about work which is *over*, and the only one
       * that pages. Both of those are lib/history.js's to argue and it does; what is
       * worth knowing at the door is why this one route takes the space filter as a
       * parameter when `/api/unendorsed` directly below it does not. Whoever slices has
       * to be whoever pages — narrowing a 30-row page on the client after the slice
       * yields three rows and a "show more" that walks through repos you asked not to
       * see. So `space` and `workspace` come in on the query, each defaulting to `all`,
       * and the client sends what the picker has selected.
       *
       * `offset` and `limit` are clamped rather than validated: `limit` lands inside
       * `[1, PAGE_MAX]` and a negative `offset` reads as 0. There is no request here
       * that is *wrong* — a stale client asking for page nine of a history that has
       * since narrowed wants an empty page, not a 400 that breaks the screen on an
       * ordinary tap.
       *
       * `?refresh=1` skips the minute-long cache, which is what ⟳ on the page sends.
       */
      if (p === '/api/closed' && req.method === 'GET') {
        return json(
          res,
          200,
          await closedHistory(bd, cfg, {
            filter: { space: url.searchParams.get('space'), workspace: url.searchParams.get('workspace') },
            offset: url.searchParams.get('offset'),
            limit: url.searchParams.get('limit') || PAGE_DEFAULT,
...
export function startPoller(cfg, app) {
   ...
   */
  const autoShip = async (ws, queue) => {
    if (OBSERVING) throw new Error(OBSERVING_NOTE);
    const already = runningFor(ws.name);
    if (already) throw new Error(`a deploy of ${ws.name} is already running (${already.id})`);
    return startDeploy(cfg, ws.name, {
      bead: queue.prs[queue.prs.length - 1]?.bead || null,
      reason: shipReason(queue),
    });
  };
  ...
  const sweepRelease = async () => {
    ...
    for (const w of out.watermarked) {
      console.log(
        `[release] ${w.workspace}: watching from now — ${w.merged} pull request${w.merged === 1 ? '' : 's'} already merged, none filed`
      );
    }
    for (const f of out.filed) console.log(`[release] ${f.workspace}: filed ${f.bead} for #${f.number} — merged, not live`);
    for (const c of out.closed) console.log(`[release] ${c.workspace}: closed ${c.bead} — #${c.number} is live`);
    for (const a of out.armed) {
      console.log(
        `[release] ${a.workspace}: auto-ship armed for #${a.numbers.join(', #')} — one deploy when the settle window closes`
      );
    }
    for (const s of out.shipped) {
      console.log(
        `[release] ${s.workspace}: auto-shipping ${s.count} merged pull request${s.count === 1 ? '' : 's'} — ${
          s.deploy || 'no record'
        } (${s.why})`
      );
    }
...
function ownTls() {
  ...
  const material = boundServers.filter(isSecure).find((s) => s.tlsMaterial)?.tlsMaterial;
  if (!material) return { tls: false, name: null };
  const left = daysLeftOf(material.cert);
  return { tls: true, name: material.name, daysLeft: left === null ? null : Math.round(left * 10) / 10 };
}
