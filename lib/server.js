...
import { advocateSession, openedRecently, rememberAdvocateOpened, waitingOn } from './epicadvocate.js';
...
export function createApp(cfg) {
   ...
   * You put a P0 on the board by starting it, which today means `bd update <id> --claim`;
   * a picker for starting one from the board itself is bc-s8mc and is not built yet.
   *
  ...
  async function p0Board(rows) {
    if (!meHandles(cfg).length) return { p0s: [], under: {}, unhomed: {}, owned: false };
    const p0s = [];
    const under = {};
    const unhomed = {};
    ...
    for (const [name, ws] of workspaces) {
      ...
      for (const b of beads.values()) {
        if (!started(b) || !isP0(b) || !ownedByMe(cfg, b)) continue;
        roots.add(b.id);
    ...
    return { p0s, under, unhomed, owned: true };
  }
  ...
  async function serveStatic(req, res, url, urlPath) {
    ...
    // The map (bc-34i0). Not a tab — public/tabbar.js's own rule is that a tab is a
    // claim the page is somewhere you go repeatedly, and this is a page you read when
    // you are new to the system or arguing about it, not one you check. `/map` as well
    // as `/flow` because both are what somebody types.
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/queues' && req.method === 'GET') {
        ...
        if (kept) res.setHeader(KEPT_HEADER, describeKept(kept));
        return json(res, 200, {
          ...queues(board, { merges, deploys: listDeploys({ limit: 200 }), ledger: loadLedger() || {} }),
