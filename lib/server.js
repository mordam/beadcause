...
export function createApp(cfg) {
  ...
  async function serveStatic(res, urlPath) {
    ...
    // `/api/work` payload, and the console answers "what is running" per repo, which
    // is the way you arrive at the question. All three old paths are kept and pointed
    // at it: they are on the phone's home screen and in the Android shell's history,
    // and a bookmark that 404s is a worse outcome than five paths for one page.
    // `/work.html` is in the list because the file behind it is deleted — it used to
    // resolve as a file on disk, and without this line it is the one path that breaks.
