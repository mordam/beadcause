...
import { landedEvent, deployEvent, deployClearEvent, syncStuckEvent, syncFlappingEvent, syncClearEvent, mutedNews } from './news.js';
...
import { collectBoard, forgetBoard, landLocally, landParent, narrowBoard, pickCard, warmBoard } from './prboard.js';
...
import { relayMark, relayTrail } from './relayjournal.js';
...
import { createSyncer, syncEnabled, syncEveryMs, syncCeilingMs, describeSync } from './sync.js';
...
export function createApp(cfg) {
   ...
   * You put a root on the board by starting it — from the phone, through the picker at the
  ...
  async function rootBoard(rows) {
    if (!meHandles(cfg).length) return { roots: [], startable: [], under: {}, unhomed: {}, owned: false };
    const cards = [];
    const startable = [];
    const under = {};
    const unhomed = {};
    ...
    return { roots: cards, startable, under, unhomed, owned: true };
  }
  ...
  function rootCard(workspace, bead, beads, children, sessions = []) {
    ...
    const tree = treeUnder(children, beads, bead.id).map((row) => ({
      ...
      // Where a department relay on this bead has got to — the last step alone, off the
      // same export row `pending` is read from. bc-bmry.4, and the reason it is here rather
      // than behind the tap: `dv-vzg` chose to let a relay run through four roles
      // unattended *on condition* that the steps show up on the epic card, and a trail you
      // have to open a bead to see is one you only look at once you already suspect
      // something. `null` on every bead nothing has relayed, which is every bead in every
      // workspace but deluvia's studio.
      //
      ...
      relay: relayMark(beads.get(row.id)),
    }));
  ...
  async function runMergeQueue({ log = (line) => console.log(line) } = {}) {
    ...
    for (const ws of cfg.workspaces || []) {
       ...
       * waiting to land.
       *
       * A workspace of forty repos is deliberately not: forty `gh` calls every five
       * minutes to answer a question that only matters where something is about to merge
       * is the wrong trade, and there the queue's own reading is the whole of it.
       */
  ...
  async function handBackWorkBead(ws, bead) {
    if (!bead) return { ok: true, why: null };
    try {
  ...
  function handBackSaid(bead, back) {
    ...
    return back.ok
      ? `${bead} is back in the queue`
      : `${bead} could NOT be handed back and is still claimed — ${back.why}`;
  }
  ...
  async function resolveDeliveryFor(ws, id, response, { actor = null } = {}) {
    ...
    if (act.action === 'changes') {
      ...
      return {
        ...
        note: back.ok
          ? `Changes requested on #${d.number} — ${bead || 'the work'} is back in the queue.`
        ...
        result: { action: 'changes', handedBack: back.ok },
        deploy: null,
      };
    }
    ...
    return {
      ...
      note: backFromDecline.ok
        ? `Declined #${d.number}${why ? ' with direction' : ''} — ${declined || 'the work'} is back in the queue.`
      ...
      result: { action: 'decline', directed: Boolean(why), handedBack: backFromDecline.ok },
      deploy: null,
    };
  }
      ...
      if (p === '/api/console/create' && req.method === 'POST') {
        ...
        try {
          for (const ref of sequence) {
            ...
            // What the card may actually say about a bead that does not exist yet. The
            // six the daemon owns are dropped here with a warning each rather than
            // refused — lib/proposedlabels.js has the whole argument, including why this
            // is deliberately *not* the same list `isProtectedLabel` refuses on an
            // adjust. bc-xl7n.44.
            const proposed = filterProposedLabels(b.labels, { ref: b.ref });
            warnings.push(...proposed.warnings);
            const id = await bd.create(
              ws,
              {
                title: b.title,
                // The declared surface rides *inside* the description, because bd has no
                // field for it and a `bd list --json` row carries the description for
                // free (lib/beadfiles.js, bc-42ow). `withSurface` is the only thing that
                // spells the block, here and in lib/filing.js both, so the console's
                // beads and an advocate's cannot come to declare a file differently. A
                // card that named no file hands its description back untouched — and
                // untouched literally, not `withSurface(desc, [])`, whose empty case
                // withdraws a block a person may have typed into the description
                // themselves. See the same guard and the whole argument in
                // `beadToIssue`, lib/filing.js.
...
export function startPoller(cfg, app) {
  ...
  const settleDeploys = async () => {
    ...
    for (const rec of unannounced()) {
       ...
       */
      bus.emit(deployEvent(rec, { quiet: mutedNews(cfg, rec.workspace) }));
      ...
      if (rec.status === 'ok') bus.emit(deployClearEvent(rec));
    }
  };
...
<<<<<<< HEAD
   ...
   * of them was a push. So a fourth transition inside an hour swaps the two-sentence
...
>>>>>>> origin/main
  ...
  const sweepSync = async () => {
    ...
    const trouble = broke.length ? app.syncer.trouble() : [];
    if (trouble.length) bus.emit(syncStuckEvent(trouble));
    if (recovered.length) bus.emit(syncClearEvent(recovered));
    if (settled.length) bus.emit(syncClearEvent(settled));
