...
import { editBead } from './beadedit.js';
import { isEpic, isRoot, ownedByMe, ownerUpdate, ownersOf, ownersOn } from './ownership.js';
...
import { chronicle as beadChronicle, docketFrom, epicsFrom, familyOf, rootOf } from './docket.js';
...
import { deliveryAction, parseDelivery, slugOf } from './delivery.js';
...
import { MAX_REVIEW_ROUNDS, nextReviewRound, reviewState, mergeBeadBody, mergeBeadTitle, withQueueBlock, MERGE_ASSIGNEE } from './mergebead.js';
// The delivery card's own Merge tap (bc-xl7n.135), the PR board's Queue to merge
// (bc-02ldo) and `beadcause-merge` are one decision now: the decision itself, the
// sentence recording it on the bead, and the writes that put a pull request where the
// queue will find it.
import { admitPlan, admitComment, admitToQueue } from './mergeadmit.js';
...
import { addresseeUpdate, addressedElsewhere, addresseesOf, describeAddressees, meHandles, ownAddresseeLabels } from './addressee.js';
...
export function createApp(cfg) {
  ...
  async function rootBoard(rows) {
    if (!meHandles(cfg).length)
      return { roots: [], startable: [], under: {}, unhomed: {}, assigned: {}, unowned: [], owned: false };
    const cards = [];
    const startable = [];
    const under = {};
    const unhomed = {};
    const assigned = {};
    // Workspaces with roots, none of which carries an `owner:` label — see `unowned` at the
    // return. Counted per workspace as the loop below already reads every bead and already
    // asks this exact question of the roots; a second pass to ask it again would be a pass
    // for nothing, which is the argument `owners` makes two lines further down.
    const rootsSeen = new Map();
    const rootsOwned = new Map();
    ...
    for (const [name, ws] of workspaces) {
      ...
      for (const b of beads.values()) {
        ...
        if (isRoot(b)) {
          rootsSeen.set(name, (rootsSeen.get(name) || 0) + 1);
          // Any owner at all, not `ownedByMe` — a workspace whose roots are all a
          // colleague's is a workspace that has been triaged and simply is not yours, and
          // saying "nobody has said who owns these" about it would be false. What this
          // counts is whether the *fact* has ever been written here.
          if (ownersOf(b.labels).length) rootsOwned.set(name, (rootsOwned.get(name) || 0) + 1);
        }
    ...
    /**
     * Workspaces that have roots and where **not one of them** carries an `owner:` label.
     *
     * This is bc-njfui, and it is the only part of that bug that was ever a bug. Ownership
     * is the `owner:<handle>` label and deliberately not bd's `owner` cell (lib/ownership.js
     * :20-45, and lib/ancestry.js says the sharper half of it: `owner` is the git identity
     * that *created* the bead — 633 of 822 records here carry the same one). So a workspace
     * nobody has ever labelled is a workspace where `ownedByMe` is false for every root,
     * every root is skipped by the gate below, and My Beadepics draws an empty board **and**
     * an empty ＋. Adam hit exactly that with eight deluvia epics he had filed himself.
     *
     * **The fix is not a fallback to `owner`.** Three reasons, and the first is the one that
     * matters: `bd update --claim` overwrites that cell, so a root would drop off the board
     * at the moment a worker picked it up — the same symptom returning wearing a different
     * face. Second, it is a creator field, so falling back would call every bead this Mac
     * ever filed "yours" and bury the picker. Third, the label is what survives a sync
     * between six Macs where a cell is a write conflict.
     *
     * So this reports instead of resolving, which is the same shape `unhomed` is: a state,
     * named, rather than a fault the tracker throws on. The screen can then say the one
     * thing the empty board could not — *nobody has said who owns anything here* — and the
     * remedy is a label, which is a thing a person can type.
     *
     * A workspace whose roots are all a colleague's is **not** on this list: it has owners,
     * they are simply not you, and that is the feature working.
     */
    const unowned = [...rootsSeen.keys()].filter((name) => !rootsOwned.get(name)).sort();
    return { roots: cards, startable, under, unhomed, assigned, unowned, owned: true };
  }
  ...
  /**
   * Put an Epic Advocate on one epic and record the assignment — the body of
   * `POST /api/bead/advocate`, as a function because a second door needs exactly it.
   *
   * The second door is bc-9i62w: ＋ Add a bead-space ends by asking which of the bead-space's
   * epics are yours and which get an advocate, and "get an advocate" has to mean what the
   * card's button means — a window, opened now, with `advocate-assigned` stamped the moment
   * it is up. A label alone would not: lib/reenter.js treats an enrolled epic it has never
   * seen as a *first sight*, which is silent, so an epic assigned and not opened would wait
   * for a child to move before anything looked at it.
   *
   * "There already is one" comes back as `{ refused }` rather than thrown, because the
   * button's 409 carries `live` and `pid` beside the sentence and the top-level catch keeps
   * only the sentence. The refusals inside `openEpicAdvocateSession` still throw, exactly as
   * they did from the route.
   */
  async function openAdvocateOn(ws, id, reason) {
    ...
    return { opened: true, assigned, repo: opened.repo || null };
  }
    ...
    try {
      ...
      if (created.length) {
        await bd
          ...
          .catch(() => {});
        // And on the card, not only on the thread. This throws before `/api/respond` has
        // reached `performed`, so without this the request fails carrying nothing and the
        // phone says *nothing was written and nothing was lost* over beads that exist —
        // the same lie, from the one act that can half-happen. Same words as the comment
        // above and as `record`, because they are one account of one thing.
        err.landed = [`Created: ${created.join(', ')}`, ...(err.landed || [])];
      }
      throw err;
    }
  ...
  /**
   * Put a delivery card's pull request on the merge queue instead of merging it here —
   * the same decision `admitPlan` makes for `beadcause-merge` and, since bc-02ldo, for
   * the PR board's own Merge button (lib/mergeadmit.js). This was the odd one out: `pr.
   * merge` straight through resolveDeliveryFor, no downmerge, no baseline comparison, no
   * one-at-a-time, and no record anywhere that Adam had decided anything. bc-xl7n.135
   * measured what that cost: this exact tap merged bc-xl7n.87's four-day-red branch past
   * a check its own branch had broken, turned `main` red for the whole repo for four
   * hours, and held eight other pull requests behind it.
   *
   * **`id` — the card being answered — is deliberately left out of what `admitPlan` is
   * asked about.** That card is about to be closed by the ordinary answer machinery in
   * `/api/respond` (`bd.respond`), a few lines after this returns, and closing it does not
   * touch any other bead. Admitting *this exact bead* into the queue instead would put it
   * in the path of `bd.commission`'s `reopenAbandoned` on a different answer shape, and
   * that clears the assignee `queueFor` selects on — undoing the very write this function
   * just made, in the same request. So a separate bead carries the approval, exactly the
   * one `admitPlan` already files when nothing in the tracker is about a pull request yet,
   * and the card's own close comment says which one it is.
   */
  async function admitDeliveryToQueue(ws, id, issue, d) {
    const at = new Date().toISOString();
    const by = ownerName(cfg);
    let rows;
    try {
      rows = (await bd.listLive(ws)).filter((r) => r.id !== id);
    } catch (err) {
      throw Object.assign(new Error(`could not read the ${ws.name} tracker — ${err.message.split('\n')[0]}`), { status: 409 });
    }
    const plan = admitPlan(rows, { repo: d.repo || '', number: d.number, bead: d.bead, by, at });
    if (plan.others.length) {
      console.error(
        `[pr] ${ws.name}: ${plan.others.join(', ')} ${plan.others.length === 1 ? 'is' : 'are'} also open about #${d.number} — ` +
          `this acted on ${plan.id}, and the rest should be closed or superseded`
      );
    }
    const comment = admitComment(plan, { by });

    if (plan.action === 'file') {
      // The delivery card's own title is `Merge #N? <what>` (lib/delivery.js
      // `deliveryTitle`) — stripping the question back off is the only way to recover
      // `<what>`, since `parseDelivery` keeps no title field of its own.
      const what = String(issue.title || '').replace(new RegExp(`^Merge #${d.number}\\?\\s*`), '').trim();
      const delivery = { ...d, title: what || d.bead || '' };
      const body = mergeBeadBody(delivery, { tests: d.tests || '' });
      let filedId;
      try {
        filedId = await bd.create(ws, {
        ...
        throw Object.assign(new Error(`could not file a merge-bead in ${ws.name} — ${err.message.split('\n')[0]}`), { status: 500 });
      }
      if (!filedId) throw Object.assign(new Error(`filed a merge-bead in ${ws.name} but bd said nothing about which`), { status: 500 });
      try {
        await bd.assign(ws, filedId, MERGE_ASSIGNEE);
      } catch (err) {
        throw Object.assign(
          new Error(`filed ${filedId}, but could not assign it to ${MERGE_ASSIGNEE} — the queue will not see it (${err.message.split('\n')[0]})`),
          { status: 500 }
        );
      }
      if (d.bead) {
        await bd
          .addDep(ws, d.bead, filedId)
          .catch((err) => console.error(`[pr] ${ws.name}: ${d.bead} is NOT parked behind ${filedId} — ${err.message.split('\n')[0]}`));
      }
      await bd
        .comment(ws, filedId, comment)
        .catch((err) => console.error(`[pr] ${ws.name}: ${filedId} took no comment — ${err.message.split('\n')[0]}`));
      return { ...plan, id: filedId };
    }

    /* An existing bead about the same pull request — a raised merge-bead, or a sibling
     * delivery card bc-8fyu made rare rather than impossible — relabelled and re-armed. */
    const row = rows.find((r) => r.id === plan.id);
    if (!row) {
      throw Object.assign(
        new Error(`${plan.id} is not in the rows this plan was made from, so its notes cannot be rewritten without losing the rest of them`),
        { status: 500 }
      );
    }
    const description = plan.action === 'admit' ? mergeBeadBody(plan.spec, { tests: plan.spec.tests || '' }) : undefined;
    try {
      ...
      throw Object.assign(new Error(`could not put ${plan.id} back on the queue — ${err.message.split('\n')[0]}`), { status: 500 });
    }
    if (plan.assignee) {
      try {
        ...
        throw Object.assign(
          new Error(
            `${plan.id} carries the queue label now, but could not be assigned to ${plan.assignee} — the queue will not pick it up (${err.message.split('\n')[0]})`
          ),
          { status: 500 }
        );
      }
    }
    if (d.bead && plan.action === 'admit') {
      await bd
        .addDep(ws, d.bead, plan.id)
        .catch((err) => console.error(`[pr] ${ws.name}: ${d.bead} is NOT parked behind ${plan.id} — ${err.message.split('\n')[0]}`));
    }
    await bd
      .comment(ws, plan.id, comment)
      .catch((err) => console.error(`[pr] ${ws.name}: ${plan.id} took no comment — ${err.message.split('\n')[0]}`));
    return plan;
  }
  ...
  async function resolveDeliveryFor(ws, id, response, { actor = null } = {}) {
    ...
    if (act.action === 'merge' || act.action === 'ship') {
      // bc-xl7n.135. This no longer merges — it admits the pull request to the merge
      // queue, exactly as the PR board's own button does (`admitDeliveryToQueue` above).
      // Everything the direct merge used to do here afterwards — the conflict sweep, the
      // window rename, closing the work bead — belongs to the *merge*, and the merge has
      // not happened yet: it belongs to `lib/mergequeue.js`'s own `finish`, which already
      // does all three the moment it lands one.
      const plan = await admitDeliveryToQueue(ws, id, issue, d);
      await pr
        .comment(dir, d.number, `${ownerName(cfg)} approved this. It is on the beadcause merge queue as ${plan.id}.`)
        .catch((err) => console.error(`[pr] ${ws.name}: could not comment on #${d.number} — ${err.message.split('\n')[0]}`));
      // A card has just left the inbox, or a queue entry has just appeared above the
      // work — a genuinely different list either way. Not fired for a bare `approve`:
      // the bead was already queued and moving, and nothing was relabelled.
      if (plan.action !== 'approve') {
        bus.emit({ type: 'queued', key: `${ws.name}/${plan.id}`, workspace: ws.name, id: plan.id, bead: d.bead, number: d.number });
      }
      console.log(
        `[pr] ${ws.name}: #${d.number} ${plan.action === 'file' ? 'queued as' : plan.action === 'admit' ? 're-armed as' : 'approved on'} ${plan.id}`
      );

      // Ship used to merge and then deploy in the same request; the merge is now
      // unattended, so there is nothing left here to deploy. What happens once it lands
      // depends entirely on whether this workspace ships itself — see lib/release.js —
      // and the note says which, rather than promising a deploy this request cannot start.
      const shipNote =
        act.action !== 'ship'
          ? ''
          : autoShipAllowed(cfg, ws.name)
            ? ` ${ws.name} deploys itself once a merge lands, so nothing further is owed here.`
            : ` This workspace has no automatic ship declared — once it merges, deploy it from the PR board.`;
      const note =
        (plan.action === 'approve'
          ? `Approved — ${plan.id} was already on the merge queue and moving, so nothing was relabelled.`
          : `Approved and put on the merge queue as ${plan.id}.`) +
        ` On its next tick the queue brings \`${d.base}\` into \`${d.branch}\`, judges the checks against whatever \`${d.base}\` is ` +
        `already failing, and merges.` +
        (d.bead ? ` **${d.bead} closes when that happens.**` : '') +
        ` **The gates still apply** — a check this branch broke, or a conflict nothing can resolve, comes back here as a card ` +
        `rather than merging on the strength of this approval.` +
        shipNote;
      return { note, result: { action: 'queue', plan }, deploy: null };
    }
  ...
  /**
   * The archived session runs for a docket's family — `Map(beadId → [{ at, subject }])`.
   *
   * **One ref listing for the workspace, then a `git log` only where there is something
   * to log.** `archivedBeads` is one `for-each-ref` whatever the size of the tracker and
   * exists for exactly this — its own comment refuses the per-row shape — so the family
   * is intersected with it *before* anything is read. On this repo that is 174 refs in
   * about ten milliseconds and then typically one or two `git log`s, rather than one per
   * bead in the family to discover that most have no archive at all.
   *
   * **An empty map is what every failure looks like, and that is deliberate.** A
   * workspace with no checkout behind it throws synchronously out of `resolveSessionDir`
   * with a 409; a retired directory, a repo that is not a git repo and one where no
   * session has ever been archived are the ordinary cases. None of them is something an
   * epic's page should refuse to draw over — the sessions are a decoration on a stream
   * whose spine is the tracker, and lib/history.js's `fetchRows` makes the same trade for
   * the same marker on the ledger.
   *
   * Capped at `RUNS_PER_BEAD` runs per bead: a bead worked twenty times contributes
   * twenty folded lines to a stream where it is one row of the map, and the newest are
   * the ones that describe the state the work is actually in.
   */
  const RUNS_PER_BEAD = 8;
  async function docketSessions(ws, chron, id) {
    const runs = new Map();
    try {
      const dir = resolveSessionDir(cfg, ws);
      const have = await archivedBeads(dir);
      if (!have?.size) return runs;
      const family = familyOf(chron, rootOf(chron.parents, id));
      for (const bead of family) {
        if (!have.has(bead.id)) continue;
        const { sessions } = await readArchive(dir, bead.id, { limit: RUNS_PER_BEAD });
        if (sessions?.length) runs.set(bead.id, sessions);
      }
    } catch {
      return runs;
    }
  ...
  async function serveStatic(req, res, url, urlPath) {
    ...
    // The docket — an epic's whole arc in the order it happened (bc-it26z), and the index
    // of every epic that has one when no `id` is in the query. Both on the same page,
    // because "which epic" is the only difference between them and a second document
    // would be a second copy of the same fetch, the same styles and the same empty state.
    //
    // A page of its own rather than a pane of the shell, unlike History: this is addressed
    // by *bead*, the way `/graph`, `/session` and `/bead-session` are, and it is opened
    // from a card in a new tab. A standing view is somewhere you go; this is somewhere a
    // row sends you, and it has to survive being pasted into a message. `/epic` too,
    // because "the epic map" is the other name Adam gave it, and which of the two comes to
    // mind depends on whether you arrived from a card or went looking.
    if (urlPath === '/docket' || urlPath === '/dockets' || urlPath === '/epic') urlPath = '/docket.html';
    ...
    // what its agents may do unasked, and what each of its repos resolves to. Three
    // paths: `/config` is the word the chip it used to be was labelled with, `/settings`
    // is what somebody types, and `.html` is what the service worker precached by name.
    // Not `/space`, which reads as the picker rather than as the thing it selects — and
    // *not* the machine's settings, which are /admin.
    //
    // A hop since bc-khoe.60 filled the Config container. public/config.js still has a
    // document half — unlike History and Releases, nothing has deleted public/config.html
    // yet — because that is a cleanup for once this hop has taken traffic for a while, not
    // a condition of landing it; see `viewHop` above for why a rewrite cannot do this and
    // where the query string goes.
    if (urlPath === '/config' || urlPath === '/settings' || urlPath === '/config.html') {
      const signIn = hopGate(req, url);
      if (signIn) return redirect(res, signIn);
      return redirect(res, viewHop('config', url));
    }
  ...
  const handler = async (req, res) => {
    ...
    try {
       ...
       * GitHub unreachable, and — since bc-4jkjv — a tracker too slow to read the bead
       * out of. A card that cannot reach GitHub should say so and still offer the link,
       * because the link is what you wanted anyway.
       */
      if (p === '/api/pr' && req.method === 'GET') {
        ...
        try {
          ...
          // A tracker that did not answer is `unavailable` here, exactly as an unreachable
          // GitHub is — the promise the docstring above makes, kept on the read it did not
          // used to cover. `bd.run` kills its own child at `BD_TIMEOUT` and marks that one
          // error `timedOut` (lib/bd.js), which is the whole discriminator: a workspace
          // whose Dolt cannot be read makes *every* `bd` call on this Mac spend the full
          // two minutes, and on 2026-08-25 five of these cards fell out of the route's
          // catch-all as 500s inside one second. The page files a sev2 P0 for any status
          // >= 500 (public/report.js), so a daemon that was answering every other route
          // raised an incident about itself — bc-4jkjv, and this is the fix for it.
          //
          // The sentence is written here rather than passed through, because `err.message`
          // on this path is bd's whole command line with the actor's name and email
          // address in it, and `ensurePr` paints `unavailable` straight onto the card.
          //
          // **Only a timeout.** A bd that answered and said no is a different fact and
          // still reaches the route error handler as itself: turning *that* into the 404
          // it should be changes this door's status codes, which is bc-fggc's bead and
          // its review, not this one's.
          if (!err?.timedOut) throw err;
          return json(res, 200, {
            delivery: null,
            pr: null,
            unavailable: `the ${ws.name} tracker did not answer in time — this card cannot be read until it does`,
          });
        }
        if (!issue) return json(res, 404, { error: 'not found' });
        const d = parseDelivery([issue.description, issue.design, issue.notes].filter(Boolean).join('\n\n'));
      ...
      /**
       * Put it on the merge queue — bc-02ldo, and the end of the last door into `main`
       * that went round the queue.
       *
       * **This used to merge.** `pr.merge` → `gh pr merge`, immediately, then
       * `landLocally`, then `retireDeliveryCards`. It left no tracker record of the
       * decision at all, which is how deluvia #55 came to be merged from a phone with its
       * approval existing nowhere while #53 and #54, merged the same afternoon through
       * `/merge`, each left a merge-bead closed with the commit it landed as. The two
       * doors read as accidentally inconsistent rather than deliberately different — and
       * the merge skill names *this endpoint*, by path, as the thing not to do.
       *
       * So it does what `bin/merge.js` does, through the one function both call:
       * `admitPlan` decides which bead this pull request already is — a raised merge-bead,
       * a delivery card, one already queued, or nothing at all — and `admitToQueue` writes
       * the approval into the queue's own block in `notes`. `gateVerdict` reads it there,
       * because the review GitHub would want is one it will not accept from the author of
       * the branch.
       *
       * **The cost is that the button stops being instant, and Adam took it knowingly.**
       * The queue's "is anything waiting" read is cached, so the merge lands a minute or
       * two later. Which is why the reply says `queued` and never `merged`, and why the
       * button on both screens says *Queue* — a control that looks like it did nothing is
       * a control you press again, and pressing this one twice is what started the bead.
       *
       * **The three things this used to do afterwards are not dropped, they moved.** The
       * conflict sweep and the local fast-forward are `afterMerge` in this same file,
       * which the queue calls the moment it lands one; the delivery card is not retired
       * but *converted* — `admitPlan` picks the very card that was asking and relabels it
       * into the queue entry, so it leaves the inbox now and closes when the merge lands,
       * with the work bead behind it. Doing any of the three *here* would be doing them
       * before the merge they describe.
       *
       * No `OBSERVING` guard, deliberately, and it is the same call `boardMove` makes:
       * the merge was the daemon acting and is exactly what has just moved to the queue;
       * what is left on this path is Adam saying the change is wanted, on whichever
       * machine he happens to be holding.
       */
      if (p === '/api/pr/merge' && req.method === 'POST') {
        ...
        // Already in, and answered the way `beadcause-merge` answers it: "it is already
        // merged" is the outcome the tap wanted, arrived at without it. A 409 over work
        // that is in `main` would send somebody to GitHub to find out nothing is wrong.
        if (row.merged || String(row.state || '').toUpperCase() === 'MERGED') {
          return json(res, 200, { ok: true, alreadyMerged: true, queued: false, action: 'merged', id: null, others: [] });
        }
        if (String(row.state || '').toUpperCase() !== 'OPEN') {
          throw Object.assign(new Error(`${where} is closed and was never merged — reopen it before it can go on the queue`), {
            status: 409,
          });
        }
        if (row.draft) {
          throw Object.assign(new Error(`${where} is a draft, and GitHub will not merge one — mark it ready first`), { status: 409 });
        }
        ...
        const at = new Date().toISOString();
        /**
         * Matched on the **pull request only**, never on a bead — which is the one place
         * this deliberately does less than `beadcause-merge -b`.
         *
         * `beadsAbout` treats a bead match as authoritative, and it is right to: a session
         * that abandoned a branch and re-delivered the same bead on a new one is exactly
         * the case a number misses. But a session *says* which bead; the board *infers*
         * one from the branch name, and an inference that picks the card for a second,
         * still-open pull request against the same bead would queue #8 for a tap on #7.
         * So the plan is decided by the number, and the inferred bead is used only where
         * it cannot choose the wrong thing — naming the work on a bead being filed fresh,
         * below, where by construction nothing open is about this pull request at all.
         */
        const plan = admitPlan(rows, { repo: row.repo || '', number: row.number, by, at });
        const inferred = (row.beads || []).length === 1 ? String(row.beads[0]?.id || '') || null : null;
        const bead = plan.action === 'file' ? inferred : null;
        // What the merge will close, which is not always what was just named: an existing
        // queue entry or delivery card already carries the work bead in its own `beadpr`
        // block, and that one is the session's word rather than this daemon's inference.
        const closes = bead || plan.spec?.bead || null;

        const warnings = [];
        const done = await admitToQueue(bd, ws, plan, {
          cfg,
          view: row,
          repo: row.repo || '',
          bead,
          method: body.method || cfg.pr?.mergeMethod || 'merge',
          by,
          rows,
          // Through the checkout this unit *is*, for `raise`'s reason two screens up: a
          // workspace of forty repos has no single directory, and `gh pr comment 42` in
          // the wrong one comments on a different repo's #42 without failing (bc-l853.6).
          prComment: (n, text) => pr.comment(dir, n, text),
          onWarn: (msg) => {
            warnings.push(msg);
            console.error(`[pr] ${ws.name}: ${msg}`);
          },
        });

        console.log(`[beadcause] ${ws.name} ${where} ${done.action === 'approve' ? 'approved' : 'queued'} as ${done.id} — ${plan.why}`);

        /**
         * One event, and only when something actually moved in the tracker — the same
         * discipline the merge that used to be here ended on, for the same cost.
         * `changed` in the poll handler is `events.some((e) => e.type !== 'presence')`, so
         * any event but presence makes the daemon sweep `bd` across every workspace and
         * hand the result to every parked client.
         *
         * `admit` and `file` are both genuinely different lists — a card has just left the
         * inbox, or a queue entry has just appeared above the work. `approve` is not: the
         * bead was already queued and moving, nothing was relabelled, and the only write
         * was a line in `notes` that no list draws.
         */
        if (done.queued) {
          bus.emit({ type: 'queued', key: `${ws.name}/${done.id}`, workspace: ws.name, id: done.id, bead: closes, number: row.number });
          // The card that was in the inbox is not in it any more, and the board draws the
          // queue beside each row. Both sweeps are cached; neither is wrong about GitHub,
          // which is why this is a drop and not a refresh.
          forgetInbox(ws.name);
          forgetBoard(dir);
        }
        ...
        return json(res, 200, {
          ok: true,
          alreadyMerged: false,
          // `queued` is false on `approve` — it was already on the queue, and a phone that
          // said "queued" over that would be reporting a move nothing made.
          queued: done.queued,
          action: done.action,
          id: done.id,
          // The work bead the merge will close, so the phone can say what is waiting on
          // this rather than only which queue entry now holds it.
          bead: closes,
          why: plan.why,
          /**
           * Never swallowed. Two open beads about one pull request is the pile that blocks
           * the work bead's close, and the one this did not act on stays open with nobody
           * looking at it — `beadcause-merge` prints them to stderr and the phone has no
           * stderr, so they ride back in the reply.
           */
          others: plan.others,
          warnings,
        });
      ...
      if (p === '/api/respond' && req.method === 'POST') {
        ...
        /**
         * Everything above this line has already happened, whatever happens below it.
         *
         * The five acts an answer can carry all run *before* the answer is written, and
         * each one is deliberately there: a merge GitHub refuses, an endorsement bd
         * declines, a create that fails must leave the question open to be answered
         * again, and a question closed on a promise nobody kept is the ending this app
         * exists to prevent. The cost of that ordering is the window this collects — a bd
         * that goes away between the merge and the comment fails the *answer*, and the
         * card would say `nothing was written and nothing was lost` over a pull request
         * that is merged and a branch that is gone (bc-s2d8 was one path to it; this is
         * the class).
         *
         * So the sentences the thread would have carried are kept, and a failure below
         * takes them with it. The notes rather than a flag: `Merged #7 as c5004cce —
         * closed zz-work` is the same sentence whether it lands on the thread or on a red
         * card, and inventing a second vocabulary for the failure case would mean two
         * accounts of one act that can disagree.
         *
         * Only the acts. `record` below adds sentences about what the answer *means* —
         * left open, handed back, deferred — and none of those is a thing that happened.
         */
        const performed = [
        ...
        try {
          ...
          // The write the whole ordering above exists to protect, and the one place a
          // failure can be standing over acts that already happened. Everything after
          // this point is caught and logged rather than thrown — a card left visible, a
          // marker left on — because what has been written is written. This is the last
          // throw in the handler, and it is the only one that can be a lie: see
          // `performed`.
          if (performed.length) err.landed = performed;
          throw err;
        }
       ...
       * reporting a clean success over a bead nothing will draw. Except when the bead
       * itself is a root: an epic, or a P0 of any type, is workable with nothing above
       * it (`isRoot`, lib/ownership.js) — bc-xl7n.115, where filing one this way read
       * as a refusal, because the warning judged the *workspace* and never the bead.
       * A workspace *with* an `unsorted` root still adopts a filed epic under it, same
       * as any other bead — it stays a root either way, since `isRoot` is type and
       * priority, not parentage, and "which epic does this belong to" is as fair a
       * question for a new epic as for anything else that arrives with no more specific
       * answer.
       *
      ...
      if (p === '/api/bead/create' && req.method === 'POST') {
        ...
        // Computed here, ahead of the warning below, rather than inline in the
        // `createBead` call further down — the warning has to judge the bead this
        // filing is actually about to make, not the raw, un-normalised body.
        const type = BEAD_TYPES.includes(String(body.type || '')) ? String(body.type) : 'task';
        ...
        // hold there would be a false claim printed at every filing. Same distinction as
        // `fileBeads` in lib/filing.js — the wording below has since drifted from it
        // (bc-xl7n.115), but the judgement has not: bc-xl7n.128 has since landed the same
        // `isRoot` gate there, batch-shaped (`anyNonRoot` across the whole filing) because
        // `fileBeads` warns once for a batch rather than once per bead. So it is only the
        // sentence that differs now, and the two decisions are meant to stay in step.
        // And `!isRoot(...)`: an epic, or a P0 of any type, needs nothing above it to be
        // workable (lib/underroot.js says so in as many words), so the same warning about
        // *this* bead would be exactly as false — the bug bc-xl7n.115 fixed here.
        if (!home.parent && home.gated && !isRoot({ type, priority })) {
          warnings.push(
            'filed with no parent, and there is nothing to hang this under — nothing will work it until ' +
       ...
       */
      /**
       * The epics a bead-space just added or linked could be yours — what ＋ Add a bead-space
       * asks about in its last round (bc-9i62w). `ownershipOffer` in lib/newspace.js decides
       * which rows and what each already is; this only reads the graph for it.
       *
       * `wait: true`, for `/api/roots`'s reason and more so: a bead-space added a second ago
       * has never been read, and a cold cache answering `[]` would tell the dialog there is
       * nothing to own in a tracker full of epics — which it would then say, and close on.
       * `me` is on the payload so the dialog can say why it is not asking, rather than
       * drawing checkboxes the write would refuse.
       */
      if (p === '/api/workspaces/roots' && req.method === 'GET') {
        const ws = requireWorkspace(url.searchParams.get('workspace'));
        const { beads, error } = await bd.graph(ws);
        return json(res, 200, {
          workspace: ws.name,
          me: meHandles(cfg)[0] || null,
          roots: ownershipOffer(beads, cfg),
          error: error || null,
        });
      }
      ...
      if (p === '/api/workspaces' && req.method === 'POST') {
        ...
        /**
         * `own` — the answer to the question ＋ Add a bead-space ends on (bc-9i62w): which of
         * this bead-space's epics are yours, and which get an Epic Advocate. Zero is an answer.
         *
         * The picks are checked whole against a **fresh** read before anything is written
         * (`ownershipPicks`), so a list drawn a minute ago cannot label a root that closed in
         * between, and a refusal means no label went on at all. Then, one epic at a time:
         * the `owner:` label through `ownerUpdate` — the same pair `/api/bead/owner` writes,
         * so taking an epic somebody else owned moves it rather than giving it two owners —
         * and, where asked, the advocate through `openAdvocateOn`, the card button's door.
         * Owner first, because `openEpicAdvocateSession` refuses a root nobody owns and it
         * re-reads the bead to decide.
         *
         * Each epic reports on its own row. A window refused for one epic is not a reason to
         * leave the other four unlabelled, and a 500 over five writes that mostly landed
         * would say none of them had.
         */
        if (action === 'own') {
          const ws = requireWorkspace(name);
          const { beads } = await bd.graph(ws, { refresh: true });
          const picked = ownershipPicks(ownershipOffer(beads, cfg), { own: body.own, advocate: body.advocate }, cfg);
          if (picked.problem) return json(res, 400, { error: picked.problem });
          const handle = meHandles(cfg)[0];
          const results = [];
          for (const step of picked.steps) {
            const row = { id: step.id, owned: false, advocate: null, error: null };
            try {
              if (step.own) {
                const issue = await loadBead(bd, ws, step.id);
                const { addLabels, removeLabels } = ownerUpdate(issue, handle);
                if (addLabels.length || removeLabels.length) {
                  await bd.update(ws, step.id, { addLabels, removeLabels }, { actor: actorFor(req) });
                }
              }
              row.owned = true;
              if (step.advocate) {
                const out = await openAdvocateOn(ws, step.id, 'you picked this epic for an advocate when you added its bead-space');
                if (out.refused) row.error = out.refused.error;
                else row.advocate = 'opened';
              }
            } catch (err) {
          ...
          if (picked.steps.length) {
            console.log(
              `[beadcause] ${ws.name}: took ${results.filter((r) => r.owned).length} epic(s), opened ${
                results.filter((r) => r.advocate).length
              } advocate(s) — from the add dialog`
            );
          }
          return json(res, 200, { ok: results.every((r) => !r.error), action, workspace: ws.name, results });
        }
      ...
      /**
       * The docket — one epic's whole arc, in the order it happened (bc-it26z).
       *
       * Adam: "the cards coming to me are sometimes hard to understand quickly because a
       * lot of context is missing." lib/docket.js is the read model and carries the
       * argument for every rule in it; this route is the joining, and the joining is
       * where the two costs live.
       *
       * **Neither of the two joins may be paid for here.** The page is opened from a card
       * with a card's budget, so:
       *
       *   - **Pull requests come off a board somebody else already swept** (`peekBoard`,
       *     never `collectBoard`). `null` travels all the way to the payload as
       *     `prsKnown: false`, because "no delivery for this epic" and "nobody has looked"
       *     are different sentences and the page must only say the first one when it is
       *     true.
       *   - **Sessions are one `for-each-ref` for the whole workspace, then a `git log`
       *     only for the beads in *this family* that the ref listing says have one.**
       *     `archivedBeads` exists precisely so a list does not ask per row — see its own
       *     comment — and a family of twenty on a tracker of eight hundred would otherwise
       *     be twenty `git log`s to discover that two of them have an archive.
       *
       * A workspace with no checkout behind it throws out of `resolveSessionDir`
       * synchronously with a 409, so the whole archive read is wrapped rather than
       * awaited bare: the archive is a decoration, and an epic that would not draw
       * because its sessions could not be counted is the trade lib/history.js's
       * `fetchRows` already refused for the ledger.
       *
       * Nothing here writes, and there is no `OBSERVING` guard, for `/api/graph`'s reason:
       * reading the record is not acting on it.
       */
      if (p === '/api/docket' && req.method === 'GET') {
        const asked = String(url.searchParams.get('workspace') || url.searchParams.get('ws') || '').trim();
        const ws = requireWorkspace(asked);
        const id = String(url.searchParams.get('id') || '').trim();
        if (!BEAD_ID_RE.test(id)) return json(res, 400, { error: 'not a bead id' });
        const got = await beadChronicle(bd, ws, { refresh: url.searchParams.get('refresh') === '1' });
        const chron = got.value;
        if (!chron?.beads?.has(id)) {
          return json(res, 404, { error: `${ws.name} has no bead called ${id}` });
        }

        const payload = docketFrom(chron, id, {
          sessions: await docketSessions(ws, chron, id),
          // Every repo's rows flattened, with the card's own `repoName` folded onto each
          // so an event can say which checkout delivered it. Not narrowed by account,
          // unlike `/api/prs`: this is a lookup by bead id inside one workspace's family,
          // and a pull request naming a bead in it is that bead's delivery whichever
          // account chip happens to be up.
          prs: (() => {
            const board = peekBoard();
            if (!board) return null;
            return (board.repos || []).flatMap((card) =>
              (card.prs || []).map((row) => ({ ...row, repoName: row.repoName || card.repoName }))
            );
          })(),
          cfg,
        });
        if (got) res.setHeader(KEPT_HEADER, describeKept(got));
        return json(res, 200, { workspace: ws.name, ...payload });
      }

      /**
       * Every epic with a docket, newest activity first — the index you browse cold.
       *
       * The third of the three shapes bc-it26z settled: a docket is reached from a card,
       * and it is also somewhere you go when you want to catch up rather than answer
       * something. Same scoping as the ledger (`ledgerWorkspaces`) because it is the same
       * question at a different grain — one workspace, a space, or all of this account's
       * — and rows carry their own `workspace` so a merged list can label them.
       *
       ...
       * and the one that did not answer is named: an index that silently dropped a repo
       * would be telling you that repo has no epics.
       */
      if (p === '/api/dockets' && req.method === 'GET') {
        const { picked, workspace, space } = ledgerWorkspaces(url.searchParams);
        ...
        const perWorkspace = await Promise.all(
          (picked || []).map(async (ws) => {
            try {
              const got = await beadChronicle(bd, ws, { refresh });
              kept.push(got);
              if (got.error) errors.push({ workspace: ws.name, error: got.error, stale: true });
              return epicsFrom(got.value).map((row) => ({ ...row, workspace: ws.name }));
            } catch (err) {
        ...
        const rows = perWorkspace
          .flat()
          .sort((a, b) => String(b.lastAt || '').localeCompare(String(a.lastAt || '')) || a.id.localeCompare(b.id));
        const combined = combineKept(kept);
        if (combined) res.setHeader(KEPT_HEADER, describeKept(combined));
        return json(res, 200, { workspace, space, rows, errors });
      }
      ...
      if (p === '/api/bead/advocate' && req.method === 'POST') {
        ...
        const id = String(body.id || '').trim();
        if (!BEAD_ID_RE.test(id)) return json(res, 400, { error: 'not a bead id' });
        // The whole launch lives in `openAdvocateOn`, because ＋ Add a bead-space opens
        // advocates through the same door (bc-9i62w).
        const out = await openAdvocateOn(ws, id, 'somebody asked for you from the epic card on the phone');
        if (out.refused) return json(res, 409, out.refused);
        return json(res, 200, { workspace: ws.name, id, ...out });
      }
      ...
      /**
       * Edit a bead that is already real — the bead detail sheet's ✎, not a fifth verdict.
       *
       * **A route of its own rather than a field of `/api/bead/adjust`, for the reason
       * `/api/bead/owner` is one.** Adjust refuses any bead that is no longer
       * `unendorsed`, and that refusal is the whole of what lib/verdict.js is for:
       * rewriting a bead over a queue row drawn a minute ago is how work somebody has
       * just agreed to gets silently changed. The sheet is not that screen — you are
       * looking at the bead you are editing — so it works on any open bead, endorsed or
       * not, and folded into adjust it would have inherited a guard that means nothing
       * here. The verdict path is untouched: same clamps, same protected labels, a
       * different door and a different refusal. lib/beadedit.js carries the argument.
       *
       * **The protections that were never about endorsement all still hold**, because
       * they come from the same `isProtectedLabel` — `unendorsed` and `agent-filed` are
       * the daemon's, and `owner:`, `for:`, `ran:` and `filed-while:` are prefixes no
       * card offers you to type, so a save that posted the label set it was showing would
       * destroy them by omission. `owner:` moves through `/api/bead/owner` and nowhere
       * else, which is what keeps fixing a P0's title from handing it back to nobody.
       *
       * **One bead, for `/api/bead/discuss`'s reason.** The four verdicts take a list
       * because "endorse these five" is a thing a busy week produces; a sheet is open on
       * one bead, and a title given to six of them is the client bug adjust already
       * refuses. So this answers `{ id, changed, fields, title, status, inProgress, held }`
       * rather than the verdict shape — an id posted as `ids` simply has no `id` and is a
       * 400, which is the honest thing to tell a client with the wrong form.
       *
       * **`changed: false` is the ordinary answer**, not a failure: a form that posts on
       * every save mostly posts what the bead already says, and `updateFor` turns that
       * into no `bd` write at all. A closed bead is the one refusal — 409, because its
       * description is the record of what was done rather than an instruction — and a
       * bead a session is working is allowed through with a sentence on the thread saying
       * so, which is decided and argued in lib/beadedit.js.
       */
      if (p === '/api/bead/edit' && req.method === 'POST') {
        const body = await readBody(req);
        const ws = requireWorkspace(body.workspace);
        const id = String(body.id || '').trim();
        if (!BEAD_ID_RE.test(id)) return json(res, 400, { error: 'not a bead id' });
        ...
        if (!Object.keys(edits).length) {
          return json(res, 400, { error: `nothing to edit — edits may name ${EDITABLE.join(', ')}` });
        }
        // No try/catch: `loadBead`'s 404 and the closed bead's 409 both carry a `status`,
        // and the chain's own handler is what turns one of those into a response — the
        // same path `/api/bead/owner` relies on for an id that is gone.
        const out = await editBead(bd, ws, id, { edits, actor: actorFor(req) });
        // A held bead's title, priority and labels are exactly what the endorsement queue
        // draws, and that list is cached for a few seconds (lib/endorsequeue.js). Dropped
        // for `announceVerdict`'s reason rather than left to a `?refresh=1`: the phone
        // that edited is not the only thing looking, and a laptop on its own poll would
        // otherwise redraw the row as it read before the save. No bus event, because this
        // is not a verdict and nothing decided anything about the bead.
        if (out.changed && out.held) forgetQueue();
        return json(res, 200, { workspace: ws.name, edits, ...out });
      }
      ...
      // `landed` is the one thing a failure can carry besides its reason, and it exists
      // because *nothing was written* is a claim a card cannot always make. An answer is
      // several acts in a row — a merge, an endorsement, beads a "yes" files — and the
      // answer itself is written last, deliberately, so that a merge GitHub refuses
      // leaves the question answerable. Which means the ordering that protects the
      // answer is exactly the one that lets an act land under a failure: bd unreachable
      // between the merge and the comment puts *nothing was written and nothing was
      // lost* on a card whose pull request is merged and whose branch is gone. Whatever
      // is on this array already happened, in the words the thread would have used, and
      // `failedNoteHtml` in public/app.js draws it in place of the reassurance.
      return json(res, status, { error: err.message, ...(err.landed?.length ? { landed: err.landed } : {}) });
    }
  };
...
export function startPoller(cfg, app) {
  ...
  const sweepSync = async () => {
    ...
    //
    // And the same argument once more, for the card `trouble()` cannot see — bc-y3qk.15.
    // A flapping workspace alternates through `ok`, so it is absent from `trouble()` on
    // every good tick it has, including — because the marks cross `FLAP_AFTER` whichever
    // way the transition went — the very tick the FLAPPING card is raised on. So the
    // syncer is asked a second question, off `all()` rather than `trouble()`, and read
    // after the sweep for the same reason: is that card still true of anybody? While it
    // is, an unrelated recovery does not get to take it away.
    if (flapping.length) {
      app.bus.emit(
        syncFlappingEvent(
    ...
    // Every workspace the flapping card is still true of, not just the ones that crossed
    // on this tick: a workspace stays flapping until its marks age out, and the one that
    // settles is out of this list by the time it reaches `clearing` below.
    const stillFlapping = app.syncer.all().filter((o) => o.flapping);
    const clearing = [...recovered, ...settled];
    const verdict = syncCardVerdict({ broke, clearing, trouble, flapping: stillFlapping });
