...
import { arrivedQuiet, quietArrival, retainQuiet } from './quiet.js';
      ...
      cfg.workspaces.map(async (ws) => {
        try {
          ...
          return rows.map((r) => {
            ...
            // so a live answer would always be "it would ring now". See lib/quiet.js.
...
export function startPoller(cfg, app) {
  ...
  const tick = async () => {
    ...
    // was. The card is the only surface that could ever say so — see lib/quiet.js.
    const hushed = {};
    ...
    // The quiet map is merged over disk on the same terms, and then the keys that rang
    // are taken back out of it. A bead is one or the other, never both, and the drop is
    // what keeps that true across a bead that left the inbox and came back louder than
    // it went: the record from the first arrival must not outlive it.
