...
import { openSession, resolveSessionDir } from './session.js';
...
import { agentList, agentDetail, agentLog, logKeyFor, AGENTS } from './agents.js';
...
export function createApp(cfg) {
  ...
  async function serveStatic(res, urlPath) {
    ...
    if (urlPath === '/agents') urlPath = '/agents.html';
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/agent' && req.method === 'GET') {
        ...
        if (!AGENTS.includes(id)) return json(res, 404, { error: `no such agent: ${id}` });
        const { ws, dir } = agentTarget(url.searchParams.get('workspace'));
       ...
       */
      if (p === '/api/agent/foundation' && req.method === 'POST') {
        const body = await readBody(req);
        const id = String(body.id || '');
        if (!AGENTS.includes(id)) return json(res, 404, { error: `no such agent: ${id}` });
      ...
      if (p === '/api/agent/decline' && req.method === 'POST') {
        const body = await readBody(req);
        const id = String(body.id || '');
        if (!AGENTS.includes(id)) return json(res, 404, { error: `no such agent: ${id}` });
        const { ws, dir } = agentTarget(body.workspace);
      ...
      if (p === '/api/console' && req.method === 'POST') {
        ...
        if (!AGENTS.includes(agent)) return json(res, 400, { error: `no such agent: ${agent}` });
