...
export function createApp(cfg) {
   ...
   */
  function inboxPayload(rows, requests) {
  ...
  const handler = async (req, res) => {
    ...
    try {
      if (p === '/api/questions' && req.method === 'GET') {
        ...
        return json(res, 200, {
          // `questions`, `requests`, `workspaces`, `spaces`, `filter`, `dismissAsk`
          // and `summary`, all of them built by `inboxPayload` above — which is also
          // what `/api/poll` answers with, so a client can refresh itself from either
          // and get the same screen. Its own field, never folded into `questions`: a
          // client that does not know about the foundation channel shows the inbox
          // exactly as it did before and simply does not draw the requests, which is
          // the right failure.
          ...inboxPayload(rows, scope === 'agent' ? agentScopeRequests : requests),
          scope,
      ...
      if (p === '/api/poll' && req.method === 'GET') {
        ...
        if (since > bus.seq) {
          ...
          return json(res, 200, {
            ...
            workspaces: [...workspaces.keys()],
            ...(fresh
              ? inboxPayload(fresh.questions, fresh.requests)
        ...
        return json(res, 200, {
          ...
          ...(polled
            ? inboxPayload(polled.questions, polled.requests)
            : { questions: null, requests: null, spaces: null }),
...
/**
 * What this process's own `listen()` put on the tailnet address, for `/api/tls`.
 *
 * `null` until something has been bound, and `{tls: false}` for the loopback-only
 * case, which is every backend behind the router and every test. It exists for
 * `npm run start:bare`, where this process *is* what owns the port and so is the only
 * thing that can say whether the socket is a TLS one — behind the router that answer
 * comes from the router's own state instead. Two sources, one question, and the screen
 * takes whichever can answer it.
 */
let boundTls = null;
...
export function listen(cfg, handler) {
  ...
  const servers = hosts.map((host) => {
    ...
    listener.listen(cfg.port, host, () => {
      ...
      if (secure) console.log(`[beadcause] listening on https://${material.name}:${port} (${host}, ${MIN_VERSION} floor)`);
      else console.log(`[beadcause] listening on http://${host}:${port}`);
    });
    return server;
  });
  boundTls = material ? { tls: true, name: material.name, daysLeft: material.daysLeft ?? null } : { tls: false, name: null };
  return servers;
}
