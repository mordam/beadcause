...
export function createApp(cfg) {
   ...
   */
  async function retireDeliveryCards(ws, { number, repo = '', base = 'main', sha = '' }) {
    const out = { cards: [], why: '' };
    let rows;
    try {
    ...
    for (const card of cardsForDelivery(rows || [], { repo, number })) {
      ...
      try {
        await bd.close(ws, card.id, reason);
      } catch (err) {
        const why = String(err.message || err).split('\n')[0];
      ...
      const work =
        card.bead && card.bead !== card.id
          ? await finishWorkBead(ws, card.bead, card.id, `Merged #${number}${landed} into ${base}`)
          : null;
