    ...
    try {
      if (p === '/api/questions' && req.method === 'GET') {
        ...
        return json(res, 200, {
          ...
          // same reason `requests` is: a chat session is not a bead, it has no id in
          // any tracker and nothing about it can be answered, so a client that has
          // never heard of the field draws the inbox exactly as it did before.
