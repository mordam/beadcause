...
import { createEpicFiler } from './jiraepic.js';
...
export function createApp(cfg) {
  ...
  async function inboxPayload(rows, requests, channels = ['questions', 'beads', 'foundation']) {
    ...
    return {
      ...
      //
      // Two more fields, and both are what makes the row's three buttons possible
      // (bc-0i27.7). `bead` is the id of the epic this ticket got, read straight out of
      // the filer's in-memory map — no `bd` call, on a payload every parked phone
      // rebuilds — and `held` is whether the hold is still on it, which is what lets a
      // row say *approved* rather than offering approve a second time. Both are `null`
      // until the epic has been filed, and the row draws that as a bead on its way
  ...
  return { handler, allQuestions, foundationRequests, splitChannels, bd, hooks, bus, advocates, syncer, jira, jiraEpics };
}
...
export function startPoller(cfg, app) {
  ...
  const sweepJira = async () => {
    ...
    const epics = (await app.jiraEpics?.sweep(cfg, cfg.workspaces, liveResults(out.results))) || {
      filed: [],
      failed: [],
    };
