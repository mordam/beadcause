...
export function createApp(cfg) {
  ...
  async function serveStatic(req, res, url, urlPath) {
     ...
     * written in a shape that would let a repo name one of ours, and it is why
     * lib/pagealias.js's regex — which reads `urlPath === '…'` literals — cannot see this
     * block and does not need to.
     *
     * Two shapes:
     *
