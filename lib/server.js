...
import { deployFor, deployHint, deployable, startDeploy, listDeploys, showDeploy, briefDeploy, deployLog, runningFor, reportingQuiet, sweepDeploys, unannounced, markAnnounced } from './deploy.js';
 ...
 */
function beginDeploy(bus, cfg, workspaceName, opts) {
  const rec = startDeploy(cfg, workspaceName, opts);
  bus.emit({ type: 'deploy', workspace: rec.workspace, id: rec.id, status: rec.status, bead: rec.bead || null });
  return rec;
}
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/pr/conflicts' && req.method === 'POST') {
         ...
         */
        const outcome = await resolveFor(ws.name, row.number, () => openConflictSession(cfg, ws, row), {
          branch: row.branch,
          owner: ownerName(cfg),
        });
        if (outcome.error) {
      ...
      if (p === '/api/release/ship' && req.method === 'POST') {
        ...
        const rec = beginDeploy(bus, cfg, ws.name, {
      ...
      if (p === '/api/respond' && req.method === 'POST') {
        ...
        if (delivered.deploy) {
          try {
            deploy = beginDeploy(bus, cfg, delivered.deploy.workspace, {
      ...
      if (p === '/api/deploy' && req.method === 'POST') {
        ...
        const already = runningFor(ws.name);
        if (already) return json(res, 409, { error: `a deploy of ${ws.name} is already running`, deploy: already });
        const rec = beginDeploy(bus, cfg, ws.name, { bead: body.bead || null, reason: String(body.reason || '') });
...
export function startPoller(cfg, app) {
  ...
  const autoShip = async (ws, queue) => {
    ...
    return beginDeploy(app.bus, cfg, ws.name, {
      bead: queue.prs[queue.prs.length - 1]?.bead || null,
      reason: shipReason(queue),
    });
  };
