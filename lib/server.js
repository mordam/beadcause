  ...
  const handler = async (req, res) => {
    ...
    try {
      if (p === '/api/questions' && req.method === 'GET') {
        return json(res, 200, { questions: await allQuestions(), workspaces: [...workspaces.keys()] });
      }
      ...
      if (p === '/api/poll' && req.method === 'GET') {
        ...
        return json(res, 200, {
          ...
          events: events || [],
          workspaces: [...workspaces.keys()],
          questions: changed ? await allQuestions() : null,
        });
      }
      ...
      if (p === '/api/session' && req.method === 'POST') {
        ...
        if (!/^[a-z][a-z0-9]*-[a-z0-9.]+$/i.test(id)) return json(res, 400, { error: 'not a bead id' });
...
export function startPoller(cfg, app) {
  ...
  const tick = async () => {
    ...
    for (const q of fresh) {
      app.bus.emit({ type: 'question', key: q.key, workspace: q.workspace, id: q.id, title: q.question || q.title });
      try {
