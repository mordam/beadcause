...
import { createOrphanWatch, describeOrphan, describeCensus } from './orphancensus.js';
...
export function startPoller(cfg, app) {
  ...
  const sweepOrphanCensus = async () => {
    ...
    // The standing number, and the only line that can say the count went *down* —
    // bc-xl7n.132.2. `newOrphans` above names a bead, so it only ever fires on a rise;
    // without this an adoption is silent and the last `[census]` line in the log is a
    // high-water mark. Suppressed for a workspace a line above already spoke for, since
    // `describeOrphan` carries the same numbers — a rise stays one line, not two.
    const named = new Set(out.newOrphans.map((row) => row.workspace));
    for (const row of out.counts) {
      if (!row.changed || named.has(row.workspace)) continue;
      console.log(`[census] ${row.workspace}: ${describeCensus(row)}`);
    }
  };
