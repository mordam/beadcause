...
import { landedEvent, deployEvent, deployClearEvent, syncStuckEvent, syncClearEvent, mutedNews } from './news.js';
...
export function startPoller(cfg, app) {
  ...
  const sweepSync = async () => {
    ...
    const broke = out.changed.filter(
      (o) => o.transition === 'broke' || (o.transition === null && (o.state === 'conflict' || o.state === 'stuck'))
    );
