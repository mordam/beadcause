...
import { anyArmed, decorateBoard, loadLedger, releaseFor, shipReason, sweepReleases } from './release.js';
import {
  closeTerminal,
  getTerminal,
  listTerminals,
...
import { cancelledTickets, liveResults, liveTickets } from './jiracancel.js';
import { approveTicket, beadifyTicket, cancelTicketAndEpic } from './jiragate.js';
...
export function createApp(cfg) {
  ...
  async function p0Board(rows) {
    ...
    for (const [name, ws] of workspaces) {
      ...
      // deliberately a wider one than `roots` above. The board is `ownedByMe` because it
      // answers "what am I answerable for"; `unhomed` is answering "has anybody at all
      // decided where this belongs", and a colleague's P0 on a shared graph is a decision.
      // That is the gate's own distinction, kept here so the two cannot drift apart.
    ...
    for (const ws of cfg.workspaces || []) {
      ...
      try {
        out = await sweepMergeQueue(bd, ws, {
           ...
           *
           * Keyed per repo like every other caller of `resolveFor`, because two Climative
           * services each have a #1 and a lock keyed by workspace would refuse the second
           * as a duplicate of a session in a different checkout entirely.
           */
          openResolver: async (entry, dir) => {
            ...
            const row = { number: entry.spec.number, branch: entry.spec.branch, base: entry.spec.base, url: entry.spec.url };
            const outcome = await resolveFor(unit.key, entry.spec.number, () => openConflictSession(cfg, ws, row, { dir }), {
              branch: entry.spec.branch,
              owner: ownerName(cfg),
              instruction:
                `This branch is on the merge queue as ${entry.issue.id}. Resolve the conflict with ` +
                `\`${entry.spec.base}\` and push; the queue merges it on its next tick. Do not merge it yourself, and ` +
                `do not close ${entry.spec.bead} — the queue closes both beads when the merge lands.`,
            });
            return outcome?.status === 200 || outcome?.status === 202;
          },
          /** Out of attempts, or nothing here can fix it: it becomes Adam's card. */
          raise: (entry, why, opts = {}) =>
            raiseMergeCard(bd, ws, entry, why, {
              ...opts,
              owner: ownerName(cfg),
              prComment: (spec, text) => pr.comment(unitDirFor(ws, spec), spec.number, text),
            }),
          /**
  ...
  /** The checkout for a pull request, or this workspace's, for the comment paths. */
  function unitDirFor(ws, spec) {
    try {
  ...
  async function resolveSweepFor(ws, id, response, { option = null } = {}) {
     ...
     *
     * The card's row is the last thing the follow-up saw, and for a row that has stopped
     * that is the last thing it will *ever* see: `chaseRow` only chases the two live
     * states. So a branch somebody rebased by hand yesterday still reads `handed-back`
     * here, and a window opened for it is one to go and close — the exact refusal
     * `/api/pr/conflicts` makes before a tap opens anything.
     *
    ...
    // only chases the two live states, so a row left at `handed-back` is one nothing would
    // ever ask about again — and the card would go on saying a session stopped on it while
    // a session was working on it.
   ...
   * had, `TECH-1204`.
   *
   * The same job `BEAD_ID_RE` does above, for the three routes that address a *ticket*
  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/prs' && req.method === 'GET') {
        ...
        // account that names three of forty checkouts draws three cards.
        const board = narrowBoard(await collectBoard(bd, cfg, { force }), (card) =>
      ...
      if (p === '/api/respond' && req.method === 'POST') {
        ...
        const commission = resolving.bound || (await chosenOption(ws, body.id, body.option))?.closes === false;
        ...
        // `stayInInbox` for a sweep card, and only for one: the card is still the only
        // thing reporting on the window it just started, and a card without `human` is
        // both out of the inbox and *in* an advocate's ready queue. See `commission`.
        if (commission) await bd.commission(ws, body.id, answer, { actor: who, stayInInbox: resolving.bound });
        else {
      ...
      if (p === '/api/unendorsed' && req.method === 'GET') {
        ...
        return json(res, 200, await endorsementQueue(bd, scopedWorkspaces(), { refresh }));
      }
      ...
      /**
       * The three decisions on a JIRA ticket's row — approve, cancel, and the beadify
       * that undoes a cancel. lib/jiragate.js is the whole argument; what is here is
       * the door. **Discuss is deliberately not among them**: the row hands you to
  ...
  return { handler, allQuestions, foundationRequests, splitChannels, bd, hooks, bus, advocates, syncer, jira, jiraEpics, jiraIngest, runMergeQueue };
}
...
export function startPoller(cfg, app) {
   ...
   */
  const sweepEvery = () => {
    const slow = Math.max(60, Number(cfg.release?.seconds) || 300) * 1000;
