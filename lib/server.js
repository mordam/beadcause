...
import { openSession, resolveSessionDir, terminalPrompt } from './session.js';
import { closeTerminal, getTerminal, listTerminals, openTerminal, summary as terminalSummary, terminalsEnabled } from './terminal.js';
...
import { amend, decline } from './foundation.js';
 ...
 * is the decision. "Should the console be allowed to run git log" is not a question
...
export function createApp(cfg) {
  ...
  /**
   * The two numbers the inbox's top bar draws, and neither of them costs a `bd` call.
   *
   * `liveSessions` is a readdir plus a JSON parse per session file — cheap enough for
   * the poll every client already makes, which is the whole reason these two live on
   ...
   * - **Close it unmerged.** The PR closes with your reason on it. The work bead is
   *   left open on purpose: deciding not to ship *this* attempt is not the same as
   *   deciding not to do the work, and quietly closing the bead would make it so.
   */
  async function resolveDeliveryFor(ws, id, response) {
    ...
    if (act.action === 'changes') {
      ...
      const note = act.note || 'Adam asked for changes — see the bead.';
    ...
    // close
    const why = act.note || 'Closed unmerged from beadcause.';
    await pr.close(dir, d.number, { comment: why, deleteBranch: false });
    if (d.bead && d.bead !== id) {
      await bd.comment(ws, d.bead, `#${d.number} was closed without merging: ${why}\n\nThe bead is still open — the work was not withdrawn, this attempt was.`);
    }
    console.log(`[pr] ${ws.name}: closed #${d.number} unmerged`);
    bus.emit({ type: 'pr-closed', key: `${ws.name}/${id}`, workspace: ws.name, id, bead: d.bead, number: d.number });
    return { note: `Closed #${d.number} without merging. ${d.bead || 'The bead'} stays open.`, result: { action: 'close' } };
  }
  ...
  async function resolveAmendmentFor(ws, id, response) {
    ...
    if (outcome.declined) {
      ...
      return { note: `Declined: ${agent} keeps its ${fields.join(', ')}.`, result: outcome };
    }
    ...
    // A console holds a session, so it has to be told. The others do not, and saying
    // so on the card is worth a line: "approved" and "in effect" are the same moment
    // here, and that is not obvious.
    ...
    if (agent === 'console') {
      ...
      for (const c of open) {
        reseedConsole(
          c,
          `Adam approved a change to what this console is (${fields.join(', ')}). ` +
      ...
      where = open.length
        ? `${open.length} open console${open.length === 1 ? '' : 's'} re-seeded`
        : 'takes effect on the next console you open';
    }
    bus.emit({ type: 'amended', key: `${ws.name}/${id}`, workspace: ws.name, id, agent, fields });
    return { note: `Amended: ${agent} ${fields.join(', ')} — ${where}.`, result: outcome };
  }
  ...
  async function serveStatic(res, urlPath) {
    ...
    // The bead console, with or without a console id in the query.
    ...
    try {
      if (p === '/api/questions' && req.method === 'GET') {
        ...
        return json(res, 200, {
          ...
          // did. Anything that needs more than these two counts wants /api/work.
          summary: summaryNow(),
          scope,
        });
      }
      ...
      /**
       * The bead console: a conversation about what to file, before anything is.
       *
      ...
      if (p === '/api/console' && req.method === 'POST') {
        ...
        // bead console. Same conversation machinery, different foundation, and no
      ...
      if (p === '/api/console' && req.method === 'GET') {
        ...
        if (!c) return json(res, 404, { error: 'no such console' });
      ...
      if (p === '/api/graph' && req.method === 'GET') {
        ...
        return json(res, 200, parseGraph(await bd.graphHtml(ws, id || null)));
      }
