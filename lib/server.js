...
export function startPoller(cfg, app) {
  ...
  /**
   * A sweep in this cycle failed, has been logged, and the cycle carries on — so this is
   * fire-and-forget on purpose, in two senses.
   *
   * Not awaited, because a `bd create` in the middle of a poll tick would delay the next
   * sweep by seconds for a failure that has already been handled; and the promise is
   * dropped rather than caught, because `reportSweepFailure` is documented never to
   * reject. That matters more than it looks: a dropped promise that *could* reject would
   * be an `unhandledRejection`, which lands in the crash handler, about the crash handler.
   */
