...
export function startPoller(cfg, app) {
  ...
  const sweepSync = async () => {
    ...
    // A failure that has *become* a conflict — or become stuck — is worth a second push
    // even though it was already failing, because it has stopped being the kind of
    // problem that fixes itself and the first notification said it would retry.
    //
