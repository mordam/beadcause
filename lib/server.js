...
// The delivery card's own Merge tap and `beadcause-merge` are one decision now
// (bc-xl7n.135, the same move bc-02ldo is making for the PR board's button): the
// decision itself, and the sentence recording it on the bead.
import { admitPlan, admitComment } from './mergeadmit.js';
  ...
  async function serveStatic(req, res, url, urlPath) {
    ...
    if (urlPath === '/v' || urlPath.startsWith('/v/')) {
      ...
      if (seg.length === 2) {
        ...
        return redirect(res, viewHop(repoviews.viewId(seg[0], seg[1]), url));
      }
    ...
    try {
      ...
      if (p.startsWith('/api/views/') && p.endsWith('/data') && req.method === 'GET') {
        ...
        if (out.problem && out.data === undefined) {
          return json(res, 502, { error: out.problem, view: full });
        }
