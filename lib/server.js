  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/dismiss' && req.method === 'POST') {
        ...
        // The same gate `/api/respond` asks about, and for the same reason — a
        // dismissal is a close too, and `bd.dismiss` comments before it closes.
        //
        // This endpoint arrived while the answer path was being fixed, so it shipped
        // with the exact bug the fix was for: dv-gr6 collected three
        // "Dismissed via Beadcause" comments in a row, one per attempt, because the
        // comment landed and the close threw and the card came back looking untouched.
        // Ahead of the refusal record below as well as the write, or a dismissal that
        // never happened still tells an agent its request was refused.
        const gate = await bd.closeGate(ws, body.id);
        if (gate) {
          console.log(`[beadcause] ${ws.name}/${body.id} cannot be dismissed — ${gate.reason}; nothing written`);
          return json(res, 409, {
            error: `bd will not close ${body.id}: ${gate.reason}`,
            gate,
            // A dismissal usually has nothing to save — the note is optional here —
            // so the phone offers the comment only when something was typed.
            canComment: Boolean(note),
          });
        }
      ...
      if (p === '/api/advocate' && req.method === 'POST') {
        ...
        await advocates.control(name, String(body.action || ''));
