  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/prs' && req.method === 'GET') {
        ...
        const { kept, ...board } = await collectBoard(bd, cfg, { force });
        // How old the sweep is goes on a header, never into the body — the convention
        // /api/history set for the routes converting after it. See `KEPT_HEADER`.
      ...
      if (p === '/api/unendorsed' && req.method === 'GET') {
        ...
        const { kept, ...body } = await endorsementQueue(bd, cfg.workspaces, { refresh });
