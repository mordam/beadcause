  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (p === '/api/respond' && req.method === 'POST') {
        ...
        if (commission) await bd.commission(ws, body.id, answer);
        else await bd.respond(ws, body.id, answer);
        console.log(
          `[beadcause] answered ${ws.name}/${body.id}${commission ? ' — left open and handed back as ready work' : ''}`
        );
        /**
       ...
       *
       * `/api/foundation/agent`, not `/api/foundation` — the channel took that name
       * first, up at the top of this handler, and for a while both were registered on
       * it. Two `if`s in one straight-line chain do not collide loudly: the first one
       * wins and the second is dead code, so every open of the agent detail got the
       * channel's `{requests, workspaces}` and threw on `data.agent.title`. The
       * narrower route is the one that moved, because the channel's name is the older
       * one and is in the README, on watch faces and in `curl` lines. `assertRoutes`
       * below is what makes the next one of these a boot failure instead of a screen
       * that silently goes blank.
       */
 ...
 * exact string equality and nothing looser.
 */
export function routeTable(handler) {
  const src = String(handler);
  const routes = [];
  const forms = [
    /p === '([^']+)'\s*&&\s*req\.method === '([A-Z]+)'/g,
    /req\.method === '([A-Z]+)'\s*&&\s*p === '([^']+)'/g,
  ];
