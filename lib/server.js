...
export function createApp(cfg) {
  ...
  async function inboxPayload(allRows, requests, channels = ['questions', 'beads', 'foundation']) {
    ...
    return {
      ...
      // spaceless row does. `mine` applies for the reason it applies to every list here;
      // a record from a workspace that is in no account's list is reachable only from an
      // install with no accounts configured, which is the account model working rather
      // than a hole this field can close.
      strandedCancels: strandedCancels(held)
        .filter(mine)
        .map((r) => ({ ...r, space: spaceFor(cfg, r.workspace)?.name || null })),
