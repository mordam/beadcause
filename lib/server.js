...
export function startPoller(cfg, app) {
  ...
  const sweepMerges = async () => {
    ...
    for (const o of await followSweepCards(app.bd, cfg)) {
      // Amended, closed or given up on — three different inboxes, and a phone parked on
      // `/api/poll` is drawing the one from before all three.
      if (o.amended || o.closed || o.gone) {
        app.bus.emit({
          type: 'sweep-card',
