...
export function mergeReport(
  spec,
  { landed = null, state = {}, review = null, baseline = [], queuedAt = null, now = Date.now(), how = '', owner = 'a person' } = {}
) {
 ...
 */
async function finish(bd, ws, issue, spec, { landed, baseline, how, markMerged = null, prApi = null, dir = '', state = {}, review = null, owner = 'a person' }) {
  const sha = String(landed?.mergeCommit || '').slice(0, 8);
   ...
   */
  const report = mergeReport(spec, { landed, state, review, baseline, how, owner, queuedAt: issue?.created || issue?.createdAt || null });
