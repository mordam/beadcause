...
import { MAX_ATTEMPTS, mergeSpec, queueState, reviewState, withQueueBlock } from './mergebead.js';
...
export async function sweepMergeQueue(
  ...
  for (const entry of queued) {
     ...
     */
    if (state.held) {
      Object.assign(state, { held: false, refused: null });
 ...
 * approved it and when, and how long the whole thing took.
 *
 ...
 */
export function mergeReport(spec, { landed = null, state = {}, baseline = [], queuedAt = null, now = Date.now(), how = '' } = {}) {
 ...
 */
async function finish(bd, ws, issue, spec, { landed, baseline, how, markMerged = null, prApi = null, dir = '', state = {} }) {
  const sha = String(landed?.mergeCommit || '').slice(0, 8);
   ...
   */
  const report = mergeReport(spec, { landed, state, baseline, how, queuedAt: issue?.created || issue?.createdAt || null });
