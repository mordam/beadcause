  ...
  for (const entry of queued) {
    ...
    if (!verdict.merge) {
      ...
      if (verdict.stale) {
         ...
         *
         * **Bounded by the same counter, and spent on the ask rather than on the
         * success.** The `BEHIND` arm above deliberately does not count a refused update,
         * because there a refusal is usually a race with somebody else's merge landing a
         * second earlier and the next tick asks again. Here it must: this path is reached
         * only when git has already said there is something to bring in, so an update
         * GitHub will not perform is a standing condition rather than a race, and not
         * counting it would spend a GitHub write every thirty seconds for ever.
         *
         ...
         */
        const spent = state.downmerges || 0;
        ...
        if (typeof drift === 'number' && drift > 0) {
          const { updated, reason } = await prApi.updateBranch(where.dir, spec.number);
          await note(bd, ws, issue, { ...state, downmerges: spent + 1, refused: verdict.refused, at: iso() });
          if (updated) {
            out.updated.push(issue.id);
            log(
        ...
        log(
          `${issue.id}: #${spec.number} — ${verdict.refused}` +
            (spent >= MAX_DOWNMERGES
              ? ` Its base has been brought in ${MAX_DOWNMERGES} times already.`
              : drift === null
