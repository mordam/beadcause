 ...
 *
 *    **Unless the repo said not to wait** — `trustChecksAcrossDownmerge`, bc-kbvhg. Where
 *    it is on, a downmerge GitHub put in with no conflict, on a branch already passing,
 *    carries straight on to the verdict below and merges on this tick. The cost it
 *    removes is real and measured: `MAX_DOWNMERGES` is 3 and `main` here takes about
 *    forty merges a day, so a branch can pay for three whole gate runs over a diff that
 *    never changed. The risk it takes is bc-rcrt's, and lib/spaces.js argues it where the
 *    switch lives.
...
import { git, gitCode, ok } from './gitref.js';
...
/**
 * How far back the strand report looks, and why it is a number at all — bc-uxrix.
 *
 * It used to take `lib/pr.js`'s default of 40 by saying nothing, and `gh pr list` sorts
 * newest first. Measured on mordam/beadcause on 2026-08-24: 75 open pull requests, of which
 * the query returned #656..#699 — so 35 of them could not be reported as stranded whatever
 * state they were in, and the 35 were the *oldest*, which is precisely where a strand
 * accumulates. The tick's own line ("1 open with no merge-bead") read as a complete count
 * and was not one, which is the failure `strandedPrs` exists to end wearing a different hat.
 *
 * Two hundred because the cost is one `gh pr list` per repo per sweep either way — the
 * limit is on the query, not on a second round trip — and because no repository here has
 * been within a factor of two of it. It is a ceiling rather than a window, and unlike the
 * old one the sweep **says when it hits it**: a report that reached its own bound and
 * presented the result as the whole answer is the exact bug being fixed, so reproducing it
 * one order of magnitude higher would be no fix at all.
 */
export const STRAND_SCAN_MAX = 200;
 ...
 *
 * **What it bounds is the asks, and the stale arm keeps successes and refusals apart**
 * (bc-johbj): `state.downmerges + state.downmergeRefusals` is what is compared against
 * this, so a standing refusal cannot spend a GitHub write every tick for ever, while
 * `downmerges` still means only the times the base actually went in. Three of *those* is
 * a branch that was unlucky; three refusals is usually the environment, and the two must
 * not be reported with the same sentence.
 */
export const MAX_DOWNMERGES = 3;

/**
 * How many commits the base has that this branch does not — measured in the checkout
 * rather than asked of GitHub. bc-xl7n.121.
 *
 * `mergeStateStatus` is GitHub's **mergeability** field, and `BEHIND` there does not mean
 * "the base has moved". It means "the base has moved *and this repository refuses a merge
 * until you update*", which is only ever true under a branch-protection rule requiring
 * branches to be up to date. `mordam/beadcause` has no protection at all — `gh api
 * repos/mordam/beadcause/branches/main/protection` answers *404 Branch not protected* — so
 * GitHub reports `CLEAN` however far a branch drifts, and the `BEHIND` arm below has
 * therefore never executed once here: `grep -c "brought .* into "` over 209,308 lines of
 * the daemon log is **0**. That is why the stale branch's promised remedy never ran, and
 * why twelve pull requests were held on it for five days.
 *
 * `rev-list --count <head>..<base>` carries no such condition. It is the git relationship
 * the stale docblock actually reasons about — *is there anything to bring in* — and both
 * readings are kept because they answer different questions. Nothing here replaces the
 * `BEHIND` arm: a repo that does require branches to be current still says so through
 * GitHub, and turning every queued branch on an unprotected repo into a downmerge is the
 * failure `MAX_DOWNMERGES` was written about, on a `main` that moves twenty times an hour.
 *
 * **`null` is "could not tell", and it is not zero.** A head this checkout has never
 * fetched, a base ref that is not here, a directory that is not a repository — the caller
 * waits rather than acts, for the same reason an unreadable stamp leaves the stale
 * question unasked. Acting on a guess is the direction that spends GitHub writes on a
 * branch there is nothing to do for.
 */
export async function behindBase(dir, head, base) {
  const tip = String(head || '').trim();
  const name = String(base || '').trim();
  if (!dir || !tip || !name) return null;
  // The commit has to be here before it can be measured against anything, and `cat-file
  // -e` answers that with an exit code rather than a throw — `gitCode`, lib/gitref.js.
  const present = await gitCode(dir, ['cat-file', '-e', `${tip}^{commit}`]);
  if (present.code !== 0) return null;
  // origin's copy of the base first, for `pickBase`'s reason in lib/notinmain.js: local
  // `main` on a Mac running twenty sessions is routinely behind the thing it is being
  // compared against, and answering from it would under-report every time.
  for (const ref of [`refs/remotes/origin/${name}`, `refs/heads/${name}`]) {
    const out = await ok(git(dir, ['rev-list', '--count', `${tip}..${ref}`]));
 ...
 * - `announceLanding(spec, issue, { landed, findings })` — the phone's landed card
 *   (`landedEvent`, lib/news.js), told once this merge's review follow-up — if it filed
 *   one — has a bead id to name. **Not `afterMerge`, on purpose, and that is bc-9ntye.5**:
 *   `afterMerge` fires above, before `finish` runs, and `finish` is where a follow-up is
 *   filed and `findings` is minted — so a card built in `afterMerge` cannot say where an
 *   open finding went. Called from inside `finish`, once, only on the path that merges
 *   through the queue: a pull request Adam merged himself from GitHub or the phone does
 *   not get one either way, which is unchanged — that door was never wired to `afterMerge`
 *   and stays that way, for the reason in lib/server.js's own comment on it (a tap of
 *   yours must not chime for it).
...
export async function sweepMergeQueue(
  ...
  ws,
  {
     ...
     */
    tellAdvocate = null,
    announceLanding = null,
    holdFor = null,
    /**
     * Is this branch behind its base *in git* — `behindBase` above, and bc-xl7n.121.
     *
     * A parameter rather than a bare call for this module's standing reason: what is under
     * test here is a decision procedure that merges to `main`, and a suite must be able to
     * reach both arms of it — something to bring in, and nothing to — without a checkout.
     * The default is the real reading, so no caller has to know it exists.
     */
    behind = behindBase,
    owner = 'a person',
  ...
  const out = {
    ...
    reviewing: [],
    /**
     * Held on a verdict about a base that has moved — bc-xl7n.121.
     *
     * Counted apart from `waiting` because it is the one wait on this path that nothing
     * else would ever move: a stale refusal deliberately costs no attempt, so it never
     * ejects to a card, never appears in `givenUp`, and until this list existed the only
     * trace of it was one log line per tick per branch. Twelve pull requests were in it
     * for five days and no surface anywhere said so. Named rather than counted in the
     * card, for `inLine`'s reason: "which ones" was the unanswerable question.
     */
    stale: [],
    // Asked for and not opened — bc-xl7n.129. `openAnswer` and `openReview` both go
    // through `resolveFor`, which queues rather than refusing when the Mac is full, and
    // both used to report that as success: a pull request whose window never opened was
    // counted in `answering` beside one whose did. The two are the same *decision* and a
    // very different *state*, and telling them apart here is the only place the wait is
    // sayable at all — see `describeMergeQueue`.
    inLine: [],
  };

  let beads = rows;
  /**
   * **The second read, and without it half this function is unreachable** — bc-uxrix.
   *
   * `bd.listAgent` runs `--exclude-label human`, and `raiseMergeCard` hands a pull request
   * over by putting `human` on. So the moment a merge-bead becomes a card it leaves the
   * only list this sweep ever read, and two things below it went quiet rather than wrong:
   * `cardedFor` could only ever return `[]`, so the take-a-card-back loop of bc-91srt had
   * never run once in the daemon; and `strandedPrs` built its cover set from the same rows,
   * so every card was reported as a pull request nothing was about — 22 of them on
   * 2026-08-26, once per tick each.
   *
   * `bd.listCards` narrows to this queue's *own* cards by assignee, which is what keeps
   * this from being the inbox read twice: a question Adam filed is not assigned to
   * `merge-advocate` and has no business in a queue's row set.
   *
   * **A caller that supplies `rows` supplies all of them.** No second read then — the
   * option means "here is the tracker", and reading half of it from the real one behind
   * the caller's back is worse than either.
   *
   * **And an unreadable card list suppresses the two things that depend on it**, rather
   * than letting them run on a set known to be short. A reclaim skipped costs a tick; a
   * strand report over a half-read tracker names pull requests that are perfectly well
   * looked after, which is precisely the noise this bead is about. `queueFor` is unharmed
   * either way — a card carries no `merge-queue` label, so it was never in that half.
   */
  let cards = [];
  let cardsRead = true;
  if (!beads) {
    ...
    if (typeof bd.listCards === 'function') {
      try {
        cards = (await bd.listCards(ws, MERGE_ASSIGNEE)) || [];
      } catch (err) {
        cardsRead = false;
        log(`could not read this queue's own cards — ${first(err)}`);
      }
    } else {
      cardsRead = false;
    }
  }
  /** The whole tracker as this sweep sees it: what it is holding, plus what it handed over. */
  const withCards = cards.length ? [...beads, ...cards] : beads;
   ...
   */
  for (const entry of cardsRead ? cardedFor(withCards) : []) {
    const { issue, spec, state } = entry;
    /**
   ...
   *
   * **A carded merge-bead names a directory as well as any other** — bc-uxrix. It is still
   * a merge-bead, still carries the block that says which repo it is about, and the whole
   * reason the queue is not acting on it is that it handed it to Adam. Reading the
   * directory set off the queued half alone meant a workspace whose merge-beads had all
   * been carded reported nothing at all, which is the "unsayable" failure one step further
   * on: not a strand that goes unmentioned, but every strand in a repo going unmentioned
   * together.
   *
   * **And the report says when it hit its own ceiling.** It used to take `prApi.list`'s
   * default of 40, silently, against 75 open pull requests on this repo — so 35 of them
   * could not be reported as stranded whatever state they were in, and `gh` sorts newest
   * first, which puts exactly the oldest and most stuck ones outside the window. That is
   * the failure this function exists to end, reproduced one layer up and harder to see: a
   * partial answer presented as a whole one. `STRAND_SCAN_MAX` is generous enough that no
   * repo here reaches it, and reaching it is said out loud rather than absorbed.
   */
  if (typeof prApi?.list === 'function') {
    const dirs = new Map();
    // Every card, not `cardedFor`'s selection: that one answers "which of these may the
    // queue take back", and a card with no recorded refusal — or one a reviewer vetoed —
    // names its repository just as well as one that is reclaimable.
    const carded = cards.map((issue) => ({ spec: mergeSpec(issue) }));
    for (const entry of [...queued, ...stuck, ...broken, ...resolving, ...carded]) {
      const spec = entry?.spec;
      if (!spec?.repo || spec.error || dirs.has(spec.repo)) continue;
      try {
        const where = await resolve(spec);
    ...
    for (const [repo, dir] of dirs) {
      let open;
      try {
        open = await prApi.list(dir, { state: 'open', limit: STRAND_SCAN_MAX });
      } catch (err) {
        log(`could not list the open pull requests in ${repo} — ${first(err)}`);
        continue;
      }
      if ((open?.length || 0) >= STRAND_SCAN_MAX) {
        log(
          `${repo} has at least ${STRAND_SCAN_MAX} open pull requests — that is as many as this looks at, ` +
            `so anything older than the ${STRAND_SCAN_MAX} newest is not in the count below`
        );
      }
      /**
       * Cards count as cover, which is `strandedPrs`'s own docblock and was not true of
       * its input until bc-uxrix: a card is a handover, not a strand, and reporting one
       * buries the real ones. Suppressed entirely when the card read failed — a cover set
       * known to be short cannot tell the two apart, and guessing names pull requests that
       * are perfectly well looked after.
       */
      if (!cardsRead) {
        log(`not saying what is stranded in ${repo} — this queue's own cards could not be read, and every one would read as a strand`);
        continue;
      }
      for (const pr of strandedPrs(open, withCards)) {
  ...
  for (const entry of queued) {
    ...
    if (outcome && outcome.act !== 'merge') {
      ...
      if (outcome.act === 'answer' && openAnswer) {
        ...
        // `'queued'` is the door saying it asked and the Mac was full — bc-xl7n.129. Still
        // truthy, so nothing that only tests this for a yes has changed its mind; counted
        // apart, because "a window is on it" and "it is in line behind four others" are
        // different answers to the only question this line is ever read for.
        if (opened === 'queued') out.inLine.push(spec.number);
        else if (opened) out.answering.push(issue.id);
      }
      /**
      ...
      if (outcome.act === 'review' && openReview) {
        ...
        // Split exactly as the answer above is, and for its reason.
        if (opened === 'queued') out.inLine.push(spec.number);
        else if (opened) out.reviewing.push(issue.id);
      }
     ...
     */
    /**
     * Set when the downmerge below went in cleanly and the policy lets this tick carry
     * on to the verdict rather than end here — see `trusted` inside it.
     *
     * Declared out here for one reason, and it is the line below the branch: a fall-
     * through leaves `mergeState` reading `BEHIND`, because it is the reading taken
     * before the downmerge and nothing re-reads it. Without this the "for the third
     * time" line would fire on a branch that was downmerged once, which is a sentence
     * that is not true and would be believed.
     */
    let trustedDownmerge = false;
    ...
    if (mergeState === 'BEHIND' && (state.downmerges || 0) < MAX_DOWNMERGES) {
      ...
      /**
       * Whether this tick may go on and judge the branch on the checks it already had —
       * bc-kbvhg, and `trustChecksAcrossDownmerge` in lib/spaces.js for the argument.
       *
       * Two conditions, and both are load-bearing.
       *
       * **The repo asked for it.** Off everywhere by default, because the reason
       * `.github/workflows/test.yml` exists is bc-rcrt's: two branches that each pass
       * alone can still break the base together, and the downmerge is where that gets
       * caught. Skipping the re-run moves that catch from before the merge to after it —
       * the `test` job runs on `push` to the base as well, and lib/redbase.js holds the
       * whole queue on a red base and files the fix. That is a good trade in a repo whose
       * CI runs on its base and a bad one in a repo whose does not, which is exactly the
       * kind of answer a policy is for.
       *
       * **`updated` means there was no conflict.** `pr.updateBranch` refuses on conflict
       * and says so — that refusal is the `continue` above — so reaching here at all is
       * GitHub reporting that the base went in cleanly. Nothing here trusts a resolution:
       * a conflicted branch never gets this far, having been handed to a resolver by the
       * branch above, whose brief still runs the repo's own gate.
       *
       * **And `passing` rather than "not failing".** `rollup` in lib/pr.js reports four
       * states and only one of them is evidence: `pending` and `none` are both *nothing
       * ran*, and `none` is the shape that condemned #480 (bc-ysqd.1). A branch that is
       * anything but green goes down the old path and waits for the re-run, which is the
       * behaviour it would have had anyway — so this can only ever shorten the life of a
       * pull request that was already passing.
       */
      const trusted = !!policy.trustChecksAcrossDownmerge && String(view.checks?.state || '') === 'passing';
      trustedDownmerge = trusted;
      log(
        trusted
          ? `${issue.id}: brought ${spec.base} into ${spec.branch} with no conflict — judging it on the ` +
              `${view.checks?.total || 0} checks it already passed rather than waiting for them to run again`
          : `${issue.id}: brought ${spec.base} into ${spec.branch} — its checks are re-running`
      );
      if (!trusted) continue;
    }
    if (mergeState === 'BEHIND' && !trustedDownmerge) {
    ...
    if (!verdict.merge) {
      ...
      if (verdict.stale) {
        /**
         * And the wait given the producer its own sentence promises — bc-xl7n.121.
         *
         * "Nothing is counted against it until they have run again" is only a wait if
         * something runs them again. The docblock in `gateVerdict` names the downmerge as
         * that something and stakes the safety claim on it — *"which is what makes this a
         * wait rather than a deadlock: a branch whose checks predate the repair is behind
         * the repaired base by construction"* — and the git relationship is exactly as it
         * says. The **field** the `BEHIND` arm above tests is not: `mergeStateStatus` only
         * reaches `BEHIND` under a branch-protection rule this repo does not have, so on
         * `mordam/beadcause` that arm has never fired and the promised remedy was
         * unreachable. Twelve pull requests sat here for five days, four of them green.
         *
         * So the reading is taken in the checkout instead (`behindBase`), and only here —
         * a branch whose checks are current is judged exactly as it was, because making
         * every queued branch downmerge on an unprotected repo is the failure
         * `MAX_DOWNMERGES` was written about rather than a fix for this one.
         *
         * **Bounded by the ask, counted by what the ask did** — bc-johbj. The `BEHIND` arm
         * above deliberately does not count a refused update, because there a refusal is
         * usually a race with somebody else's merge landing a second earlier and the next
         * tick asks again. Here the *bound* must count it: this path is reached only when
         * git has already said there is something to bring in, so an update GitHub will
         * not perform is a standing condition rather than a race, and not bounding it
         * would spend a GitHub write every thirty seconds for ever.
         *
         * But it must not be counted as a **downmerge**, and the first tick on the shipped
         * fix is the proof. `pr.updateBranch` was running as the ambient `gh` account,
         * which has no `push` here, and GitHub masks that as a 404 — so all twelve held
         * pull requests were refused and all twelve had `downmerges: 1` written against
         * them for a merge that never happened. Two more ticks and the sentence below
         * would have told Adam *"Its base has been brought in 3 times already"* about a
         * base that was never once brought in: an invisible indefinite wait replaced by a
         * visible permanent dead end carrying a false explanation. So the refusals are
         * their own counter and the last reason is kept, which is what lets the exhausted
         * sentence say what actually happened — and, when it is a standing condition of
         * the *environment* rather than of the branch, say that out loud instead of
         * blaming the branch. `downmerges` now means the same thing in both arms: the
         * number of times the base actually went in, which is what `mergeReport` has always
         * claimed of it in prose.
         *
         * Still no attempt, and that is unchanged: nothing has been asked of this branch's
         * *diff*, so nothing has refused it, and this must not reach `record`.
         */
        const broughtIn = state.downmerges || 0;
        const refusals = state.downmergeRefusals || 0;
        const spent = broughtIn + refusals;
        const drift = spent < MAX_DOWNMERGES ? await behind(where.dir, view.headSha, spec.base).catch(() => null) : null;
        if (typeof drift === 'number' && drift > 0) {
          ...
          await note(
            ...
            issue,
            updated
              ? { ...state, downmerges: broughtIn + 1, refused: verdict.refused, at: iso() }
              : {
                  ...state,
                  downmergeRefusals: refusals + 1,
                  downmergeRefusal: String(reason || '').slice(0, 200),
          ...
          if (updated) {
            out.updated.push(issue.id);
            log(
              `${issue.id}: brought ${spec.base} into ${spec.branch} — it was ${drift} commit${drift === 1 ? '' : 's'} behind ` +
                `and its checks predated the base coming back, so they are re-running`
            );
          } else {
            out.waiting.push(issue.id);
            log(`${issue.id}: could not bring ${spec.base} in — ${reason}`);
        ...
        // Counted here rather than in `waiting`, so the tick can say this out loud — see
        // `out.stale`. A branch that has had its base brought in `MAX_DOWNMERGES` times and
        // is *still* stale is not waiting on the queue any more, and neither is one whose
        // head this checkout cannot see; either way the list is the only thing that says so.
        out.stale.push(spec.number);
        log(
          `${issue.id}: #${spec.number} — ${verdict.refused}` +
            (spent >= MAX_DOWNMERGES
              ? broughtIn > 0
                ? ` Its base has been brought in ${broughtIn} time${broughtIn === 1 ? '' : 's'} already.`
                : // Never once brought in, and saying otherwise is the sentence bc-johbj
                  // exists to stop. GitHub refused every ask, so the reason it gave is the
                  // only thing here that points at the actual problem.
                  ` Its base could not be brought in at all — GitHub refused ${refusals} time${refusals === 1 ? '' : 's'}` +
                  `${state.downmergeRefusal ? `, last saying: ${state.downmergeRefusal}` : ''}.`
              : drift === null
                ? ' This checkout cannot say whether it is behind its base, so nothing was brought in.'
                : '')
        );
        continue;
      }
...
export function mergeReport(
  ...
  const downmergeRefusals = Number(state.downmergeRefusals) || 0;
  /**
  ...
  /**
   * And the asks that were refused — bc-johbj. Without this a branch GitHub would not
   * update three times reads as "straight through", which is the most misleading line in
   * the whole report: nothing moved *because* the queue could not move it. Named
   * separately from `downmerges` above because the two are different facts, and the
   * useful half is that this one is almost never about the branch.
   */
  if (downmergeRefusals > 0) {
    passage.push(
      `- **The base could not be brought in.** GitHub refused to update the branch ` +
        `${downmergeRefusals === 1 ? 'once' : `${downmergeRefusals} times`}` +
        `${state.downmergeRefusal ? `, last saying: \`${state.downmergeRefusal}\`` : ''}.`
    );
  }
...
async function finish(
  ...
  /**
   * The phone's landed card, now that `findings` can name the follow-up — bc-9ntye.5.
   *
   * `afterMerge` in lib/server.js already built and emitted this card once, before this
   * function ran, and at that point the follow-up above did not exist yet: `afterMerge`
   * fires straight after the merge and `finish` — where `followUpFor` mints the bead — is
   * still ahead of it. Moving the card here rather than reordering the two calls keeps
   * `afterMerge`'s own ordering, whose comment argues for it on grounds unrelated to this
   * (the sweep and the local `main` are meant to happen while the merge is still fresh).
   *
   * Best-effort like everything below it: the merge and both closes must never wait on a
   * push notification, and a card that failed to send is not a reason to leave two beads
   * open.
   */
  if (announceLanding) {
    try {
      await announceLanding(spec, issue, { landed, findings });
    } catch (err) {
      console.error(`[merge-queue] ${issue.id}: merged ${where} but the landed card was not sent — ${first(err)}`);
    }
  }
...
/**
 * The pull requests in line for a window, as a card can print them — bc-xl7n.129.
 *
 * Three and a tally rather than all of them: this goes inside one bullet of a one-line
 * card, and the day the bead was filed the line was twenty-five long. Three is enough to
 * recognise which branches are stuck, and the number behind them is what says how bad it
 * is; a card that wrapped to four lines would be read by nobody, which is the state the
 * count was already in.
 */
function namesInLine(numbers, show = 3) {
  const named = numbers.slice(0, show).map((n) => `#${n}`).join(', ');
...
export function describeMergeQueue(out) {
  ...
  // Beside the hold and the review for their reason, and it is the one of the three that
  // had no sentence at all — bc-xl7n.121. A stale refusal costs no attempt by design, so
  // it never ejects to a card and never reaches `givenUp`: for five days the only trace of
  // twelve held pull requests was a log line per tick, and the queue's own card said
  // nothing. Named rather than counted, for `inLine`'s reason — "which ones" was exactly
  // what nobody could answer. Optional for `held`'s reason: a caller predating the field
  // hands this a plain object.
  if (out.stale?.length) bits.push(`${out.stale.length} held on checks older than the base (${namesInLine(out.stale)})`);
  // The three windows this queue opens for itself, and the one it could not — bc-xl7n.129.
  // A reviewer and a worker answering one are progress and were sayable nowhere: `awaiting`
  // is true of a pull request whose reviewer is reading it right now and of one nothing has
  // ever looked at, so the line could not tell a review that is happening from a review that
  // is owed. And the wait is the reason this bead exists — 25 pull requests were in line for
  // a window for three hours and the only thing that said so was a count in the resolver's
  // own log naming nothing. Named rather than counted, because the answer to "which ones"
  // was otherwise a read of an in-memory registry in a running daemon.
  if (out.reviewing?.length) bits.push(`${out.reviewing.length} being reviewed`);
  if (out.answering?.length) bits.push(`${out.answering.length} being answered`);
  if (out.inLine?.length) bits.push(`${out.inLine.length} in line for a window (${namesInLine(out.inLine)})`);
