...
import { freshVerdict, inlineComments, reviewFromVerdict, threadIdsFor } from './reviewsync.js';
...
async function syncReviewVerdict(bd, ws, prApi, issue, spec, { dir, headSha = '', owner = 'a person', policy = {} } = {}) {
  ...
  // bc-36xx.31: anchor each fresh inline comment to the GitHub thread `submitReview` just
  // created for it, so a later round can settle it there rather than only in this bead's
  // own notes. Read straight back rather than trusted from the submission response,
  // because the reviews endpoint hands back the review it made, not the per-comment ids
  // inside it — `reviewThreads` is the one place those exist. `prApi.reviewThreads` is
  // optional the same way `reviewerFor` is above: a fake that predates this carries none
  // and every comment simply goes unanchored, exactly as it did before this existed.
  let threadIds = null;
  if (inline.length && submission?.submitted && typeof prApi.reviewThreads === 'function') {
    const threads = await prApi.reviewThreads(dir, spec.number).catch(() => null);
    threadIds = threadIdsFor(verdict, threads);
  }

  // Comments the PRIOR round anchored and the worker answered, that this fresh verdict
  // does not carry forward — the reviewer read the answer and dropped the objection,
  // which is "answered and accepted" (bc-36xx.31: nothing called `resolveThread` at all
  // before this). Resolved on GitHub under the reviewer identity before the block below
  // replaces `state.comments` wholesale, since after that fold they are gone from the
  // bead for good and there is no second tick to catch it on.
  if (typeof prApi.resolveThread === 'function') {
  ...
  const folded = { ...state, ...reviewFromVerdict(verdict, { headSha, reviewer, threadIds }) };
