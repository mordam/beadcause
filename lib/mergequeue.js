...
export async function sweepMergeQueue(
  ...
  const out = { ok: false, reason: '', queued: 0, merged: [], updated: [], waiting: [], refused: [], raised: [], stuck: [], reclaimed: [], held: [] };
  ...
  for (const entry of queued) {
    ...
    if (hold && !exemptFrom(hold, spec)) {
      ...
      if (state.refused !== why) await note(bd, ws, issue, { ...state, refused: why, at: iso() });
