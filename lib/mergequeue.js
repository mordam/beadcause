...
export async function sweepMergeQueue(
  ...
  for (const entry of queued) {
    ...
    if (String(view.state || '').toUpperCase() !== 'OPEN') {
      ...
      if (view.mergedAt) {
        await finish(bd, ws, issue, spec, { landed: view, baseline: [], how: `#${spec.number} was ${how} outside the queue` });
    ...
    if (mergeState === 'BEHIND') {
      ...
      if (updated) {
        ...
        // Not an attempt. GitHub refusing an update is very often a race with somebody
        // else's merge landing a second earlier, and the next tick asks again.
        out.waiting.push(issue.id);
    ...
    await finish(bd, ws, issue, spec, { landed, baseline: verdict.baseline, how: '' });
    out.merged.push(issue.id);
  }
 ...
 */
async function finish(bd, ws, issue, spec, { landed, baseline, how }) {
  ...
  await bd
    ...
    .catch(() => {});
  await bd
    .close(ws, work, `Merged ${where}.${over ? ` ${over}` : ''}`, { overClaim: true })
    .catch((err) => console.error(`[merge-queue] ${work}: merged ${where} but the work bead did not close — ${first(err)}`));
}
