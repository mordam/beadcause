 ...
 */
/**
 * The `b7e-*` classifications, read from the tools' own headers rather than written out
 * here — bc-wbrhi, and `lib/tooldecl.js` for the whole argument.
 *
 * The one import this file has, and it cannot start a cycle: `tooldecl.js` imports
 * `node:` builtins and `lib/toolbelt.js`, which is a leaf and asserted to stay one by
 * `test/loadorder.mjs`. Neither of them imports this.
 */
import { B7E_GRANTS } from './tooldecl.js';
...
export const GRANTS = Object.freeze({
  ...
  'Bash(b7e-import:*)': { kind: 'read' },
  'Bash(b7e-owes:*)': { kind: 'read' },
  'Bash(b7e-affected:*)': { kind: 'read' },
  'Bash(b7e-notes:*)': { kind: 'read' },
  'Bash(b7e-readme:*)': { kind: 'read' },
  'Bash(b7e-ws:*)': { kind: 'read' },
  'Bash(b7e-siblings:*)': { kind: 'read' },
  'Bash(b7e-base:*)': { kind: 'read' },
  'Bash(b7e-deliverbase:*)': { kind: 'read' },
  'Bash(b7e-census:*)': { kind: 'read' },
  'Bash(b7e-count:*)': { kind: 'read' },
  'Bash(b7e-cites:*)': { kind: 'read' },
  'Bash(b7e-claims:*)': { kind: 'read' },
  'Bash(b7e-which:*)': { kind: 'read' },
  'Bash(b7e-moment:*)': { kind: 'read' },
  'Bash(b7e-orient:*)': { kind: 'read' },
  'Bash(b7e-prior:*)': { kind: 'read' },
  'Bash(b7e-precedent:*)': { kind: 'read' },
  'Bash(b7e-harness:*)': { kind: 'read' },
  'Bash(b7e-pinned:*)': { kind: 'read' },
  'Bash(b7e-brief:*)': { kind: 'read' },
  'Bash(b7e-known:*)': { kind: 'read' },
  'Bash(b7e-premerge:*)': { kind: 'read' },
  'Bash(b7e-enroll:*)': { kind: 'read' },
  'Bash(b7e-gated:*)': { kind: 'read' },
  'Bash(b7e-entry:*)': { kind: 'read' },
  'Bash(b7e-propagated:*)': { kind: 'read' },
  'Bash(b7e-stillred:*)': { kind: 'read' },
  'Bash(b7e-show:*)': { kind: 'read' },
  'Bash(b7e-already:*)': { kind: 'read' },
  'Bash(b7e-answered:*)': { kind: 'read' },
  'Bash(b7e-window:*)': { kind: 'read' },
  'Bash(b7e-where:*)': { kind: 'read' },
  'Bash(b7e-ran:*)': { kind: 'read' },
  'Bash(b7e-callers:*)': { kind: 'read' },

  /* ------------------------------------------------ the b7e family, from their own files

     Thirty-four entries used to be written out here, one line each, and every branch that
     added a tool added its line at the same place — ten of the fourteen pull requests open
     on 2026-08-26 were inserting at line 333 of this file, so one of them landing conflicted
     the other nine. They are now read from the tools themselves: a `bin/b7e-*` file says
     `@grant read` in its own header and `lib/tooldecl.js` turns that into the entry below.

     It sits where the block sat, so this file still reads in the order it always did. What
     that means for precedence is worth being exact about rather than leaving to be
     discovered: a spread overwrites what came before it and loses to what comes after, so
     a name written by hand *above* this line would be replaced by a declaration and one
     written *below* would replace it. Neither is a rule worth relying on, so there is no
     rule — `test/tooldecl.mjs` fails the repo when a name is both declared in `bin/` and
     written out here, and a collision is treated as the mistake it is rather than silently
     resolved in whichever direction the line happens to sit. bc-wbrhi. */
  ...B7E_GRANTS,
