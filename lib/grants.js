...
export const GRANTS = Object.freeze({
  ...
  'Bash(beadcause-epicplan:*)': {
    kind: 'write',
    scope: 'tracker',
    why: 'Spawns `node bin/plan.js`, which writes a group plan or the whole-job decision onto an epic — ' +
      'the same `bd comment` / `bd label add` / `bd update` writes already granted above, bundled behind ' +
      'one validated door instead of typed by hand to match its markers (bc-jvt0.6).',
    granted: ['epic-advocate'],
  },
