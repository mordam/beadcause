 ...
 *
 * - **The script tag.** A file *added whole* on this branch, whose only way onto the
 *   page is a `<script src>` tag a cached page only just gained, and which another
 *   cached file now calls into off the window. That is bc-nib3.3, and it is why the
 *   member rule could not see it: `filtermenu.js` was new, so it is neither half of a
 *   member pair; the tag that loads it is markup, not a call; and the pair that actually
 *   breaks is the phone's *old* `index.html` beside the branch's new `inboxfilter.js`.
 *   Unguarded that is the same `TypeError` as the member rule and fails the same way.
 *   Guarded — `if (window.beadcause && window.beadcause.filterMenu)`, which is how
 *   `history.js` mounts the identical control — it is a control that never appears, and
 *   it is named inside the advisory instead. Measured the same way the stylesheet half
 *   was: replayed over all 240 merges `main` has ever had it fires on no unbumped branch
 *   at all, and independently explains 4 of the 31 that did bump.
 *
...
/**
 * Every script a page loads, as repo-relative paths.
 *
 * Only same-origin absolute `src`s, which is every script tag this app writes. The
 * crudeness is the same as everywhere else in here: a tag inside a multi-line HTML
 * comment is skipped only on the line that opens the comment, because nothing under
 * `public/` comments a tag out and leaves it lying there.
 */
export function scriptSrcs(lines) {
  const srcs = new Set();
  for (const raw of lines) {
    const line = stripMarkupComments(raw);
    for (const m of line.matchAll(/<script\b[^>]*\bsrc\s*=\s*(["'])(\/[^"'?#]+)\1/g)) srcs.add(`public${m[2]}`);
  }
  return srcs;
}

/**
 * The global surface a script installs — `beadcause.filterMenu`, as a dotted path with
 * the receiver taken off.
 *
 * Every shared browser module under `public/` ends the same way, and it is the only way
 * one of them reaches another: there are no modules and no imports on these pages, so a
 * file writes `window.beadcause.<name> = …` at the bottom and everybody else reads it
 * back off the window. The namespace line itself — `window.beadcause = window.beadcause
 * || {}` — is in every one of those files and is not a surface anybody needs *this* file
 * for, so an assignment whose right-hand side names the same path again is skipped.
 */
export function globalInstalls(source) {
  const paths = new Set();
  for (const raw of String(source).split('\n')) {
    const line = stripComments(raw);
    for (const m of line.matchAll(/\b(?:window|globalThis|self)\.((?:[A-Za-z_$][\w$]*)(?:\.[A-Za-z_$][\w$]*)*)\s*=(?!=)/g)) {
      const path = m[1];
      const rest = line.slice(m.index + m[0].length);
      if (new RegExp(`\\b(?:window|globalThis|self)\\.${path.replace(/\./g, '\\.')}(?![\\w$])`).test(rest)) continue;
      paths.add(path);
    }
  }
  return paths;
...
/**
 * Every call these lines make *through* the window — `window.a.b.c(…)` — as its parts.
 *
 * `optional` records whether the chain used `?.` anywhere, because that is the whole
 * difference between a `TypeError` and a control that quietly does not appear.
 */
export function globalCalls(lines) {
  const calls = [];
  for (const raw of lines) {
    const line = stripComments(raw);
    for (const m of line.matchAll(/\b(?:window|globalThis|self)((?:\??\.[A-Za-z_$][\w$]*)+)\s*\(/g)) {
      const parts = m[1].split(/\??\./).filter(Boolean);
      calls.push({ parts, chain: parts.join('.'), member: parts[parts.length - 1], optional: m[1].includes('?.') });
    }
  }
  return calls;
}

/** Is `path` the whole of, or the front of, the chain this call walks down? */
const reaches = (path, parts) => {
  const want = path.split('.');
  return want.length <= parts.length && want.every((seg, i) => seg === parts[i]);
};

/**
 * Does this file only ever touch that global behind a check?
 *
 * `public/history.js` mounts the very same control as `public/inboxfilter.js` and reads
 * `if (host && window.beadcause && window.beadcause.filterMenu)` first, so on a phone
 * without the file it draws a ledger with no filter bar instead of throwing. That is a
 * different cost from a `TypeError`, and it is reported differently.
 */
function isGuarded(source, path) {
  const p = path.replace(/\./g, '\\.');
  const re = new RegExp(`(?:&&|\\|\\||[!(?]|typeof\\s+)\\s*(?:window|globalThis|self)\\.${p}(?![\\w$.])`);
  return String(source).split('\n').some((raw) => re.test(stripComments(raw)));
}

/**
 * The script-tag half of a mixed pair: a file added whole on this branch, whose only way
 * onto the page is a `<script src>` tag that a cached file only just gained.
 *
 * This is the hole bc-nib3.3 went through, and the reason the member rule above could
 * never see it. `public/filtermenu.js` was added whole — so it is not a *modified* file
 * and cannot be either half of a member pair — and the thing that puts
 * `window.beadcause.filterMenu` on the page is a line of *markup* in
 * `public/index.html`. The pair that actually breaks is therefore the old cached
 * `index.html`, which has no such tag, beside the new `inboxfilter.js`, which calls
 * `.mount(...)` on the `undefined` that leaves behind. swbump printed an advisory.
 *
 * "A file added whole, with its callers, is never a mixed pair" still stands, and every
 * fixture asserting it still passes: a cache from before the branch has neither half of
 * *that* pair. What this adds is the case where one half is not the new file at all —
 * it is the page that loads it, which the cache has had all along and which this branch
 * edited. `bc-p38c.2` stays silent under it for the reason it always did: `report.js`
 * went onto twelve pages and nothing gained a call into it, so old cached HTML without
 * the tag is the app exactly as it was.
 *
 * The two strengths split the way they do everywhere else here. An unguarded call is a
 * `TypeError` and fails the run; a call behind `&&` or `?.` is a control that silently
 * does not appear, which is named inside the advisory and left to the reader to price.
 */
export function scriptCouplings({ added, modified }) {
  const found = [];
  const pages = modified.filter((f) => f.path.endsWith('.html'));
  const callers = modified.filter((f) => f.path.endsWith('.js'));
  for (const file of added) {
    if (!file.path.endsWith('.js')) continue;
    const src = file.path.replace(/^public/, '');
    const tags = pages.filter((p) => scriptSrcs(p.added).has(file.path) && !scriptSrcs(String(p.base).split('\n')).has(file.path));
    if (!tags.length) continue;
    const installs = [...globalInstalls(file.head)];
    if (!installs.length) continue;
    for (const caller of callers) {
      // A chain the caller already walked is one the cached older page already had to
      // load this file for; only what the branch newly reaches for is evidence.
      const before = new Set(globalCalls(String(caller.base).split('\n')).map((c) => c.chain));
      const seen = new Set();
      for (const call of globalCalls(caller.added)) {
        if (before.has(call.chain) || seen.has(call.chain)) continue;
        const path = installs.find((p) => reaches(p, call.parts));
        if (!path) continue;
        seen.add(call.chain);
        found.push({
          path,
          ...
          guarded: call.optional || isGuarded(caller.head, path),
        });
      }
    }
  }
...
/** The script pairs that throw, as against the ones that merely disappear. */
export const breaking = (result) => (result.scripts || []).filter((s) => !s.guarded);
...
export function analyse({ swBase, swHead, files }) {
  ...
  const arrived = changed.filter((f) => f.status === 'A');
  ...
  return {
    ...
    scripts: bumped ? [] : scriptCouplings({ added: arrived, modified }),
  };
}
...
export function report(result) {
  const lines = [];
  const throws = breaking(result);
  if (result.couplings.length || throws.length) {
    ...
    for (const s of throws) {
      lines.push(`  ${s.calls} calls window.${s.chain}(), which ${s.provides} installs and`);
      lines.push(`  ${s.tags.join(', ')} only gained the <script src="${s.src}"> that loads it on this branch`);
    }
  ...
  if (result.advisory) {
    ...
    // The same claim from the other direction: not a name one half draws and the other
    // styles, but a whole file one half loads and the other has never heard of. The
    // caller checked for it before reaching in, so nothing throws and nothing is red —
    // and a phone on the older page has no such control at all.
    const quiet = (result.scripts || []).filter((s) => s.guarded);
    if (quiet.length) {
      lines.push('and a file added on this branch is reached only through a <script src> a cached half only just gained:');
      for (const s of quiet) {
        lines.push(`  ${s.calls} reaches window.${s.chain} behind a check, and`);
        lines.push(`  ${s.tags.join(', ')} only gained the <script src="${s.src}"> that loads ${s.provides} on this branch`);
      }
      lines.push('A phone on the older page never loads it, so the control simply is not there —');
      lines.push('which is exactly the "looks like a working page" failure the version exists to prevent.');
      named = true;
    }
