 ...
 *
 * ## And no setting at all is enough, when bd already points somewhere (bc-6s383)
 *
 * The block used to be the only way on, and on this machine it sat empty for weeks over
 * a workspace whose `bd` had the site, the address and the project all set — so the
 * tickets assigned to Adam never arrived and nothing said why, because *off* is silent by
 * design. A workspace whose tracker has been pointed at a JIRA site has already answered
 * the question the switch asks.
 *
 * So the switch has three positions, and `jiraSwitch` is the one place that reads it:
 *
 * - `enabled: true` — **on**, whatever `bd` says. The block supplies what `bd` lacks.
 * - `enabled: false` — **off**, and it costs nothing: no `bd` spawn, no network call.
 *   This is the way to keep a JIRA-wired tracker out of beadcause.
 * - anything else, including no block — **auto**: on exactly when a site URL is known,
 *   from the block's `url` or from a `bd config get jira.url` that answers an http(s)
 *   URL — anything else bd prints is no answer (see `settingsFor`). That is one memoised `bd`
 *   spawn per workspace (lib/jirapoll.js holds it for ten minutes), and it is the whole
 *   price a bd-only workspace pays for this.
 *
 * Auto keys on the URL alone. A workspace with a site and no credential is still *on*,
 * and says so as trouble naming the file to write — a tracker wired to JIRA with nothing
 * arriving is precisely the silent state this section exists to end. The sentence also
 * names `enabled: false`, because the person reading it never asked for JIRA here.
 *
...
/** A site, as far as auto is concerned: an http(s) URL with a host. See `settingsFor`. */
const SITE = /^https?:\/\/[^\s/]+/i;
...
/**
 * The block for one workspace, or null. A block that is not an object is no block.
 *
 * What `enabled` means is `jiraSwitch`'s business, not this one's: only a literal
 * boolean decides, and everything else — no block included — is auto, which asks the
 * tracker whether it already points at a site rather than guessing generously from here.
 */
...
/** Is JIRA *explicitly* switched on for this workspace? `jiraSwitch` is the whole answer. */
export function jiraEnabled(cfg = {}, workspaceName = '') {
  return jiraBlock(cfg, workspaceName)?.enabled === true;
}

/**
 * Does an arriving ticket get *read* — an agent run that files its children (lib/jiraingest.js)?
 *
 * On unless the block says `ingest: false` (bc-6s383). The switch exists because auto
 * made JIRA arrive for a space whose owner asked for *cards*: one epic per ticket is a
 * `bd create`, but an ingestion is a `claude -p` that reads the ticket and greps a repo —
 * minutes and money each — and a first sweep of 32 tickets queued 32 of them. So the
 * epic and the reading are two decisions, and only a literal `false` turns the second
 * one off, for the same reason only a literal boolean moves `jiraSwitch`: the default
 * stays what it was for every space that already had JIRA on.
 */
export function jiraIngests(cfg = {}, workspaceName = '') {
  return jiraBlock(cfg, workspaceName)?.ingest !== false;
}

/**
 * `'on'`, `'off'` or `'auto'` — see "no setting at all is enough" in the header.
 *
 * Only a literal boolean is a decision. `enabled: "true"` is not one and reads as auto,
 * the same position as no block: on if the tracker already points at a site, and off —
 * for free — if it does not.
 */
export function jiraSwitch(cfg = {}, workspaceName = '') {
  const enabled = jiraBlock(cfg, workspaceName)?.enabled;
 ...
 *
 * `auto` says the workspace is on because its bd points at a site rather than because
 * somebody switched it on (`jiraSwitch`). An auto workspace with no site is simply off.
 */
export async function settingsFor(bd, workspace, cfg = {}) {
  const name = workspace?.name || '';
  const block = jiraBlock(cfg, name);
  const off = { workspace: name, enabled: false, auto: false, url: null, email: null, projects: [], token: null, tokenFile: null, problem: null };
  const position = jiraSwitch(cfg, name);
  if (position === 'off') return off;

  // The URL first, because in auto it is also the switch — and asking for it once is
  // what keeps a bd-only workspace at a single spawn rather than three.
  const named = String(block?.url || '').trim();
  const url = (named || String((await bdConfig(bd, workspace, 'jira.url')) || '').trim()).replace(/\/+$/, '');
  // Auto switches on for a site a person named in the block, or for a bd answer that is
  // *plainly* a site. Anything else bd prints — a `[]`, a warning on stdout with exit 0 —
  // is no answer, and must not turn JIRA on for every workspace on the machine only to
  // report each of them as "not a URL". An explicit `enabled: true` still gets that
  // sentence, because there somebody asked.
  if (position === 'auto' && !named && !SITE.test(url)) return off;
    ...
    : !/^https?:\/\//i.test(url)
      ...
      : !email
        ...
        : !token
          ...
          : null;
  // Nobody switched this one on: its bd points at a site. So the way *out* is named too.
  const problem =
    wrong && auto ? `${wrong}; JIRA is on because ${name}'s bd points at ${url} — set jira.${name}.enabled to false to keep it off` : wrong;

  return { workspace: name, enabled: true, auto, url, email, projects, token, tokenFile, problem };
}
 ...
 * workflow may be a word nobody uses. It is also the second half of the answer — see
 * `DONE_CATEGORY` — and it carries its `statusCategory` without being asked.
 */
export const RESOLUTION_FIELDS = ['summary', 'status', 'resolution'];

/**
 * The status category JIRA gives every *finished* status, whatever the site calls it.
 *
 * `resolution` is not enough on its own, and bc-6s383 measured by how much: on
 * climative.atlassian.net the workflow's `5: Done` never sets a resolution, so of the 281
 * TECH tickets `assignee = <Adam> AND resolution = EMPTY` answered on 2026-09-11, **169
 * were finished** — `5: Done`, `5D: UAT QA Passed`, `5X: Duplicate Issue - Removed`. The
 * poll would have filed a P1 epic for work that shipped months ago.
 *
 * The category is the field nobody renames — every status belongs to one of three, keyed
 * `new`, `indeterminate` and `done`, and a site can rename a status or add a hundred of
 * them but cannot add a fourth category. So the poll asks by category too — the *In
 * Progress* one only, `IN_PROGRESS` in lib/jirapoll.js, so it never answers a finished
 * ticket in the first place — and `resolutionOf` treats a status in this category as
 * resolved, so the ticket that moves to Done *after* arriving, without a resolution,
 * closes its held epic exactly as a resolved one does.
 */
...
export function resolutionOf(issue) {
  ...
  return {
    // Or a status in the Done *category* with no resolution at all — see `DONE_CATEGORY`.
    resolved: Boolean(res) || fields.status?.statusCategory?.key === DONE_CATEGORY,
