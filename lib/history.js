 ...
 *
 * **And past the window nothing waits for it any more.** The sweeps sit on the shared
 * layer (lib/cache.js) as of bc-1kwl.2: the eleventh second after one used to be a cliff
 * — whoever asked next paid for `bd list --all`, the most expensive call in the app and
 * measured at 28.6 seconds here under a load average of 33 — and is now a request that
 * returns the ten-second-old rows immediately with the sweep running behind it. What
 * still waits is a workspace nothing has ever been kept for, which is the first read
 * after a restart and is bc-1kwl.4's to remove. How old the rows are travels back with
 * them, so the page can say so: `kept` on the return, `x-beadcause-kept` on the wire.
 *
...
/**
 * The sweeps, on the shared layer — one key per workspace. See lib/cache.js.
 *
 * This file used to keep its own `Map` with a TTL beside an `inflight` `Map` for the
 * single-flight, which is four of the five things lib/cache.js does; what it did not do
 * was the fifth, and the fifth is the expensive one. **Past ten seconds a request used to
 * wait for the sweep**, and that sweep is `bd list --all` — the most expensive call in
 * the app, ~1s over the largest workspace on an idle Mac and measured at 28.6 seconds
 * here under a load average of 33. So every eleventh second, whoever asked first paid a
 * second or half a minute for a list that had barely changed. Now they are handed the
 * ten-second-old rows immediately and the sweep runs behind the response; only a key
 * nothing has ever been kept for can still wait.
 *
 * The `<what>:<scope>` key convention is lib/cache.js's and this is its first caller:
 * `ledger:<workspace>` holds the *unfiltered* slim rows, so every filter chip and every
 * page of one scroll is still served from one sweep.
 */
const PREFIX = 'ledger:';
...
/**
 * One workspace's rows, and how old they are — the envelope, not the array.
 *
 * `refresh` reaches the layer unchanged and keeps meaning exactly what it meant here
 * before: skip what is kept, pay the cost, the user asked. It still *joins* a sweep
 * already in flight rather than starting a second one, which is the right reading — a
 * sweep that began a moment ago and has not returned is reading the tracker now, so it
 * is exactly as fresh as one started here would be.
 */
const sweep = (bd, ws, { archivedFor, refresh, now }) =>
  cache.read(keyFor(ws.name), () => fetchRows(bd, ws, { archivedFor }), { freshMs: CACHE_MS, now, refresh });

async function fetchRows(bd, ws, { archivedFor }) {
  ...
  const archived = archivedFor
    ? await Promise.resolve()
        ...
        .catch(() => new Set())
    : new Set();
  return (issues || []).filter((i) => i && i.id).map((i) => toRow(ws.name, i, archived));
}
...
export async function ledger(
  ...
  const perWorkspace = await Promise.all(
    (workspaces || []).map(async (ws) => {
      try {
        const got = await sweep(bd, ws, { archivedFor, refresh, now });
        kept.push(got);
        // **A workspace being served over a failed refresh is named too.** Its rows are
        // here and they are drawn — that is what the layer keeping the last good answer
        // is for — but "these are ten seconds old" and "these are old and `bd` has been
        // refusing ever since" are different facts, and only the second one is worth
        // interrupting somebody about. It rides in the same array as an outright failure
        // because the page already draws that array; `stale: true` is what tells them
        // apart, and it is why this is not simply swallowed.
  ...
  return {
    ...
    errors,
    // How old the oldest part of this answer is, for the caller to put on the wire. Not
    // a field in the HTTP body: the convention is a header (lib/cache.js `KEPT_HEADER`),
    // because the routes converting after this one do not all answer with an object.
    kept: cache.combine(kept),
  };
}
