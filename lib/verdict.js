...
/**
 * The same guard, asked one label at a time — because the third protected thing is a
 ...
 *
 * Setting an owner is a route of its own — `POST /api/bead/owner` — for the same reason
 * endorsing is: it is a decision about the bead rather than a field of it, and it should
 * be one deliberate act rather than a side effect of saving a form.
 */
export const isProtectedLabel = (label) => {
  const l = clean(label);
  return PROTECTED_LABELS.includes(l) || isOwnerLabel(l);
};
...
export function normalizeEdits(patch) {
  ...
  for (const key of asked) {
    ...
    if (key === 'labels') {
      out.labels = (norm.labels || []).filter((l) => !isProtectedLabel(l));
      continue;
    }
    out[key] = norm[key];
  }
