 ...
 * an empty map and nothing to compare against — and, left there, its epics would stay
 * open forever. It is the same shape as the rename's "a summary rewritten while the
 * daemon was down", and this file takes the same trade the rename does *not*: a full `bd
 * list --all` per workspace per tick would answer a question that is nearly always no, so
 * instead `sweepOne` keeps its own snapshot of the filer's map in `state.json` (`SEEN_KEY`,
 * `seenFor`/`rememberSeen`) and falls back to it exactly when the filer's own map is empty.
 * The snapshot costs nothing on an ordinary tick — one local write, skipped entirely when
 * nothing has moved — and it is what lets an empty restart still know which epic a ticket
 * that resolved overnight belonged to. bc-0i27.23.
 *
...
/**
 * The field in `state.json` for the restart-survives snapshot — see `seenFor`/`rememberSeen`
 * and the header's "one case that seeding does not cover".
 */
export const SEEN_KEY = 'jiraResolvedSeen';
...
/** One workspace's persisted `ref → id` object, dropped to nothing if it is the wrong shape. */
function normalizeSeen(raw) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return {};
  const out = {};
  for (const [ref, id] of Object.entries(raw)) {
    const r = clean(ref);
    const i = clean(id);
    if (r && i) out[r] = i;
  }
...
/** Every workspace's snapshot, normalised — the same permissive-on-read shape as `readResolved`. */
function readSeen() {
  const raw = loadState()[SEEN_KEY];
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return {};
  const out = {};
  for (const [name, refs] of Object.entries(raw)) {
    const n = clean(name);
    if (!n) continue;
    const norm = normalizeSeen(refs);
    if (Object.keys(norm).length) out[n] = norm;
  }
  return out;
}

/**
 * One workspace's persisted map, as a `Map` — what `sweepOne` falls back to when the
 * filer's own map is empty. `{}` when nothing has ever been recorded for it, which reads
 * as "nothing known" rather than as an error: a workspace with no JIRA on, or one whose
 * first tick has not run yet, must not be treated as a mass resolution.
 */
function seenFor(name) {
  return new Map(Object.entries(readSeen()[clean(name)] || {}));
}

/**
 * Write down what the filer knows for one workspace, so it survives a restart.
 *
 * Skipped entirely when nothing has changed since the last write, so a workspace whose
 * epics are unmoved costs no write at all — the same free-quiet-tick shape as the rest of
 * this file. Overwrites rather than merges: the filer's map is authoritative whenever it
 * is non-empty (`fileFor` seeds it from *every* ref in the tracker on the read it makes,
 * not merely the ones it went looking for), so a ref this snapshot used to have and the
 * filer no longer does is one the tracker no longer has either.
 */
function rememberSeen(name, knownMap) {
  const n = clean(name);
  if (!n) return;
  const next = {};
  for (const [ref, id] of knownMap) {
    const r = clean(ref);
    const i = clean(id);
    if (r && i) next[r] = i;
  }
  ...
  if (prevKeys.length === nextKeys.length && nextKeys.every((ref) => prev[ref] === next[ref])) return;
  all[n] = next;
  saveState({ [SEEN_KEY]: all });
}
...
export function createResolvedSweep({ bd = null, fetchImpl = undefined } = {}) {
  ...
  async function sweepOne(cfg, workspace, result, { filer, settings, records, gone, now }) {
    ...
    const fromFiler = filer?.knownFor?.(name) || new Map();
    // A restart leaves the filer's map empty for a workspace `fileFor` has not re-read yet
    // — which, for one whose live list is also empty, is forever. `seenFor` is the fallback:
    // whatever this sweep last saw the filer know, kept on disk across the restart. And
    // whenever the filer's map *is* non-empty it is written back, so the snapshot never
    // falls behind while the daemon is actually running. See `rememberSeen` and bc-0i27.23.
    const known = fromFiler.size ? fromFiler : seenFor(name);
    if (fromFiler.size) rememberSeen(name, fromFiler);
