...
function manifests(dir) {
  ...
  return names.map((name) => path.join(dir, DOLT, name, ...MANIFEST)).sort();
}
...
export function trackerMark(ws) {
  ...
  if (!dir) return null;
  const marks = [];
  for (const file of manifests(dir)) {
    try {
      marks.push(fs.readFileSync(file, 'utf8'));
    } catch {
  ...
  // A separator rather than a bare concatenation: two databases whose manifests both
  // moved could otherwise join to the same string as before, and a printable one keeps
  // the mark readable in the log line that names it.
