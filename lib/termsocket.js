...
import { attach, closeTerminal, getTerminal, resize, scrollback, summary, terminalsEnabled, writeTo } from './terminal.js';
...
export async function attachTerminalSocket(cfg, servers) {
  ...
  for (const server of servers) {
    server.on('upgrade', (req, socket, head) => {
      ...
      if (!offered.includes(PROTOCOL) || !timingSafeEqual(supplied, cfg.token)) {
  ...
  console.log(`[beadcause] terminal    ws://${cfg.host}:${cfg.port}${PATH}`);
  return wss;
}
...
function onConnection(cfg, ws, req, t) {
  ...
  say({ type: 'hello', terminal: summary(t), replay: t.bytes > 0, truncated: t.truncated });
