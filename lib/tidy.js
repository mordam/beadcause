 ...
 *
 * `slimAttic`, last in the file, is a third and much smaller job: an attic bounded by
 * age is still not bounded by size, because four entries out of a hundred carried a
 * real 160 MB `node_modules` and were half of it. See its header for where those come
 * from and why dropping one is not the same kind of act as removing an entry.
 */
 ...
 *
 * Exported for lib/attic.js, which sweeps the other end of the same convention and
 * must read the same listing the same way. One parser is the point: the attic sweep
 * used to have its own, in shell, and the way it read this listing is what produced
 * bc-bcdp's false bug report.
 */
 ...
 *
 * Exported because lib/sessionlog.js compares a worktree path *recorded in a meta.json
 * months ago* against what `git worktree list` says today, and gets the same answer wrong
 * in the same direction: a live worktree reads as `gone`, silently, on any path that came
 * from somewhere other than git.
 */
...
/* The short name every occupancy check in this file was written against. */
const real = realPath;
 ...
 *
 * Exported for lib/attic.js, which needs the same answer to count the entries that are
 * simply not old enough yet — the bulk of a healthy attic, and the one thing the report
 * a human reads has to say that `expireRetired` has no reason to return.
 */
export function retiredAt(note) {
  let first;
  try {
...
export async function expireRetired(dir, { sessions = [], days = ATTIC_DAYS, dryRun = false, prMerges = false } = {}) {
  ...
  for (const wt of list) {
    ...
    //
    // And GitHub really is the only thing that knows. Two local tests get suggested in
    // its place every time this comes up, and neither works — measured against
    // `worktree-config-secret-guard-m6m`, squash-merged as #20, and asserted in
    // test/attic.mjs so they cannot quietly come back:
    //
    // - `git diff main...<branch>` is **not** empty. A three-dot diff is taken from the
    //   merge base, so the branch's own changes are still sitting there after the
    //   squash landed them — nine files, in that case.
    // - `git cherry -v main <branch>` reports its commits as `+`, not upstream: a
    //   squash is one new commit with a patch-id matching none of the originals. (A
    //   one-commit branch is the exception that makes this worse, not better — it
    //   happens to match, so the test looks like it works until the first branch with
    //   two commits on it.)
    //
    // So the question stays where it can be answered, and `--no-pr` is the honest
    // degradation: a repo with no `gh` keeps the old behaviour and keeps its attic.
    if (!merged && prMerges) {
...
/* ------------------------------------------------- the weight of the attic */

/**
 * What a retired worktree may carry that is not worth carrying: build output, rebuilt
 * from committed source by one command, and ignored by git either way.
 *
 * `node_modules` is the one that filed bc-mf9s — 160 MB in four entries, half the attic
 * — and the gradle output is the same shape one order of magnitude down: 83 MB across
 * the handful of entries that had built an APK, and the only reason any of them was
 * still over ~10 MB once the dependency trees had gone.
 *
 * What is deliberately *not* here is `public/vendor/`: it is 4 MB, it is what every
 * browser check in this repo refuses to start without, and it is the difference between
 * a resumed worktree that runs and one that has to be set up again. The rule for this
 * list is heavy *and* rebuilt by a command the resumer was going to run anyway.
 */
const REGENERABLE = ['node_modules', path.join('android', 'app', 'build'), path.join('android', 'build'), path.join('android', '.gradle')];

/**
 * How the attic gets heavy, given that `expireRetired` above bounds how *full* it gets.
 *
 * A worktree of this repo is meant to borrow the main checkout's dependency tree —
 * `ln -s ../../../node_modules node_modules`, which is what the `ship` skill says and
 * what 37 of the 41 attic entries carrying anything at all had. The other four had a
 * real installed tree of 160–167 MB each, against ~6 MB for a normal entry: 650 MB, and
 * half the attic, in four directories out of 122. Each one has `.package-lock.json`
 * inside it, so each is a plain `npm install` run in the worktree rather than the
 * symlink — most likely following `scripts/vendor.js`'s own advice, which used to say
 * "run npm install" to a fresh worktree with nothing to copy from. (That file now says
 * something else; this is the other end of the same fix.)
 *
 * Dropping build output is deliberately *not* the same kind of act as removing an attic
 * entry, which is why it does not wait for the two-day line and does not ask GitHub
 * anything:
 *
 * - it destroys no work and no history — `npm install`, or the symlink, puts the tree
 *   back byte for byte from `package-lock.json`, which is committed, and `npm run
 *   android` rebuilds the rest;
 * - it is invisible to git either way, since every path in `REGENERABLE` is ignored;
 * - and a resumed worktree that needs it will say so the first time it runs anything.
 *
 * So the gates are only the ones about *interrupting somebody*: a lock, a live session
 * sitting in it, or a directory touched in the last ten minutes. It walks registrations
 * like `expireRetired` and for the same reason — an unregistered directory in the attic
 * is somebody having moved things by hand, and nothing here should be the first to
 * touch it.
 *
 * A symlinked `node_modules` is left exactly alone: it weighs nothing, and removing it
 * would take a resumable worktree's borrowed tree away for no gain at all.
 */
export async function slimAttic(dir, { sessions = [], dryRun = false } = {}) {
  const main = await mainCheckout(dir);
  const retiredRoot = path.join(main, RETIRED);
  const slimmed = [];
  const kept = [];
  if (!fs.existsSync(retiredRoot)) return { slimmed, kept, retiredRoot, bytes: 0 };
  ...
  for (const wt of list) {
    ...
    if (!under(here, path.resolve(retiredRoot))) continue;
    const name = path.basename(here);
    // `lstat`, so the borrowed `node_modules` symlink answers false: it is a directory
    // only through the link, and following it would delete the main checkout's tree.
    const found = REGENERABLE.map((rel) => path.join(here, rel)).filter((p) => {
      try {
    ...
    if (!found.length) continue; // nothing built here — the normal case, not worth a row

    if (wt.locked) {
      kept.push({ name, why: 'locked — a live session is in it' });
      continue;
    }
    const inIt = sessions.find((s) => s.cwd && under(real(s.cwd), real(here)));
    if (inIt) {
      kept.push({ name, why: `a session is working in it (pid ${inIt.pid})` });
      continue;
    }
    // The same ten minutes retirement uses, and here it is guarding a running process
    // rather than an edit: a resumed worktree mid-`npm test` is one whose files moved
    // recently, and pulling its dependency tree out from under it is a strange failure
    // to debug. `lastTouched` skips `node_modules` itself, so an install that finished
    // an hour ago does not keep the entry alive on its own timestamps.
    const touchedMinsAgo = (Date.now() - lastTouched(here)) / 60000;
    if (touchedMinsAgo < QUIET_MINUTES) {
      kept.push({ name, why: `touched ${Math.max(1, Math.round(touchedMinsAgo))} minute(s) ago` });
      continue;
    }

    const dropped = found.map((p) => path.relative(here, p));
    const bytes = found.reduce((n, p) => n + dirBytes(p), 0);
    if (dryRun) {
      slimmed.push({ name, bytes, dropped, dryRun: true });
      continue;
    }
    try {
      for (const p of found) fs.rmSync(p, { recursive: true, force: true });
      // A second line on the `.note`, never a replacement for the first: `retiredAt`
      // reads line one and the stamp there is what decides the entry's age. This says
      // what a resumer needs to know before wondering why nothing runs.
      const note = path.join(retiredRoot, `${name}.note`);
      if (fs.existsSync(note)) {
        fs.appendFileSync(
          note,
          `${new Date().toISOString()}  ${dropped.join(', ')} dropped by beadcause (${megabytes(bytes)}) — relink node_modules or npm install, npm run android, to resume\n`
        );
      }
      slimmed.push({ name, bytes, dropped });
    } catch (err) {
      kept.push({ name, why: `could not drop ${dropped.join(', ')} — ${err.message.split('\n')[0]}` });
    }
  }

  return { slimmed, kept, retiredRoot, bytes: slimmed.reduce((n, e) => n + e.bytes, 0) };
}

/**
 * What a directory occupies, near enough for a log line.
 *
 * `lstat`, so a symlink counts as the link and never as what it points at — a
 * dependency tree is full of `.bin/` links back into itself, and following them would
 * count the same megabytes several times over. Apparent size rather than blocks on
 * disk, which reads a few percent low for a tree of small files and is not a number
 * anything decides anything on.
 */
...
/** One line for a log or a card: what the attic put down, and what would not let go. */
export function describeSlim({ slimmed = [], kept = [] } = {}) {
  const parts = [];
  if (slimmed.length) {
    const freed = megabytes(slimmed.reduce((n, e) => n + e.bytes, 0));
    parts.push(`slimmed ${slimmed.map((s) => s.name).join(', ')} (freed ${freed})`);
  }
  // A locked entry is somebody sitting in a resumed worktree, which is the normal and
  // correct reason to leave a tree alone; the rest are worth a card's width.
  const notable = kept.filter((k) => !/^locked/.test(k.why));
  if (notable.length) parts.push(`kept ${notable.map((k) => `${k.name} (${k.why})`).join(', ')}`);
  return parts.join(' · ');
}
