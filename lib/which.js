 ...
 * question it exists to answer, `lib/toolbelt.js` carries a prose comment beside its
 * `DEFAULT_TOOL_LIST` entry (or says why there isn't one), and `README.md` has a `### `
 * section naming it. Three registries, and nothing before this read any of them.
 *
...
/**
 * `{ granted: true }` when `Bash(<name>:*)` is on `DEFAULT_TOOL_LIST`; `{ granted:
 * false, decided: true }` when it is not, but the array's own comments name it (a
 * recorded "deliberately not" decision, the same reading bin/b7e-enroll's
 * allowlistProblem uses); `{ granted: false, decided: false }` when dispatch's reach
 * into it has never been decided at all.
 */
export function allowlistStatus(name, toolbeltSrc) {
  ...
  if (grantedRe.test(listText)) return { granted: true, decided: true };
  if (listText.includes(name)) return { granted: false, decided: true };
...
export function loadContext(root) {
  ...
  return { root, pkg, toolbeltSrc, readmeAnalysis };
}

export function entryFor(root, name, ctx) {
  const { pkg, toolbeltSrc, readmeAnalysis } = ctx;
  ...
  return {
    ...
    raw,
    malformed: !question,
    allowlist: allowlistStatus(name, toolbeltSrc),
