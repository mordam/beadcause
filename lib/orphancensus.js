 ...
 *
 * **And the second population it must not call ordinary: a bead already decided against.**
 * A bead carrying `superseded-by:<id>` (lib/superseded.js) has been looked at and ruled a
 * duplicate of work living somewhere else, so it is exactly *not* a bead "nothing has
 * decided above" — counting it there inflates the number this file exists to keep honest
 * and spends a `[census]` line naming resolved work. Both siblings in this family already
 * drop it: `strandingsIn` (lib/rootclose.js) and `createEpicWatch`'s `worthSaying`
 * (lib/epicdone.js). Like the merge genre it is still counted into `unrooted`, because
 * having no root above it is a true thing to say about it.
 *
 * **Which is why `mergeGenre` is counted rather than inferred.** It used to be the
 * residual `unrooted - ordinary.length`, which is only right while there is exactly one
 * exclusion; a second one would have quietly reported every superseded bead as a merge
 * card. Each exclusion now counts itself, and `unrooted` is the sum of the three.
 */
...
export function orphanCensus(index) {
  ...
  for (const bead of beads?.values?.() || beads || []) {
    ...
    unrooted += 1;
    // Both exclusions are unrooted on purpose and neither is ordinary. Merge genre is
    // asked first so a card that is somehow both is reported as what filed it, and so
    // that adding an exclusion can never change what an existing one counts.
    if (isMergeGenre(bead)) mergeGenre += 1;
    else if (isSuperseded(bead)) superseded += 1;
    else ordinary.push(String(bead.id));
  }

  return { nonClosed, unrooted, mergeGenre, superseded, ordinary };
}
