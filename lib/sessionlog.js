...
import { stagedDebrief, stagedBeads, clearDebrief, renderDebrief } from './memory.js';
...
// And what it cost, which is the same read again — bc-nc6o.8. The vocabulary, the window
// and the verdict are in lib/sessiontokens.js; the only thing owed here is that the
// megabytes are opened once and that the answer reaches `meta.json`.
import { EMPTY_USAGE, mergeUsage, sessionTokens, usageInTranscript } from './sessiontokens.js';
...
/**
 * One pass over the transcript files: `{ lines, models, usage }`.
 *
 ...
 *
 * `models` is every model an assistant turn actually ran on (bc-nc6o.3), and `usage` is
 * what those turns cost and how close they came to filling the window (bc-nc6o.8). Both
 * are collected here rather than by a second reader for the obvious reason: this is the
 * one place in the archive where those megabytes are already open.
 */
function readTranscripts(files) {
  ...
  for (const file of files) {
    ...
    // The models and the cost are collected off the same read rather than two more: a
    // transcript runs to megabytes and this is the only place it is already open.
    for (const id of modelsInTranscript(text)) if (!models.includes(id)) models.push(id);
    // Merged rather than summed by hand, because a session that entered a worktree has two
    // files and a peak is a maximum over both of them, not a total.
    usage = mergeUsage(usage, usageInTranscript(text));
  ...
  // Truncating the *rendering* must not truncate the models or the cost: a session that
  // changed model in its last ten minutes is exactly the session whose log ran long, and
  // the one the whole of `ran:` exists for — and a session long enough to blow a 4MB
  // rendering cap is the one most likely to have run out of window. So both scans above
  // happen per file before the cap, and `truncated` only stops the part that has a size
  // budget.
  return { lines: out, models, usage };
}

const NOTES_REF = 'refs/notes/beadcause';
export const SESSIONS_PREFIX = 'refs/beadcause/sessions';

/** How many commit subjects a salvage comment carries before it says "and N more". */
const SALVAGE_SUBJECTS = 12;

/** Behind main by more than this and a salvage comment recommends a cherry-pick. */
const SALVAGE_STALE = 50;
...
function findTranscripts(sessionId) {
  ...
  // mtimeMs can tie — a session moving into a worktree can write two transcript
  // files within the same millisecond — and an untied tie falls back to whatever
  // order readdirSync happened to return, which can misorder which file is "the
  // last place it wrote" (see worktreeOfSession below). path is a stable tie-break.
 ...
 * A thin reading of `ranFactsOf`, which answers the cost alongside it.
 *
 ...
 */
export const ranModelsOf = (sessionId) => ranFactsOf(sessionId).models;

/**
 * Both facts the transcript carries about a finished run, off one read:
 * `{ models, usage }` — see `ranModelsOf` above for when this path is taken at all.
 *
 * One function and not two because the alternative is two full reads of the same
 * megabytes, seconds apart, for two halves of the same sentence. `usage` is always a
 * record and never null — `measured()` in lib/sessiontokens.js is what distinguishes a
 * session that spent nothing from one there is no file for, and it can only do that if
 * the shape is the same either way.
 */
export function ranFactsOf(sessionId) {
  if (!sessionId) return { models: [], usage: { ...EMPTY_USAGE } };
  const files = findTranscripts(sessionId);
  if (!files.length) return { models: [], usage: { ...EMPTY_USAGE } };
  const models = [];
  let usage = { ...EMPTY_USAGE };
  for (const file of files) {
    let text;
...
/**
 * What a dead window left behind, as a sentence for the bead it was working — or null
 * when there is nothing to say.
 *
 * bc-xl7n.102. A window that commits and then dies is the one ending where the archive's
 * facts are load-bearing and the archive is the only thing holding them. `archiveSession`
 * records `branch`, `head` and `commits` into the ref and hands them back, and the
 * advocate puts them into `pendingNotes` — whose only consumer writes its git note **when
 * the branch reaches main**. For a bead being handed back to the queue that never happens:
 * nothing will push the branch, because the bead has gone back to `bd ready` and the next
 * window starts a fresh worktree. So the facts were counted, logged and then dropped, and
 * the next window rebuilt work that already existed and already passed.
 *
 * The carrier is a comment, and it is chosen rather than invented: an ordinary worker's
 * brief tells it to run `bd comments <id>` before it claims, so the thread is read by every
 * window at attempt 1 — no new label, no new sweep, and in particular **not** by teaching
 * the stall sweep to look at handed-back beads, which would put the bead back into the
 * state bc-xl7n.85 exists to end.
 *
 * Four facts, because for an unpushed branch nothing else records them: the branch, the
 * head sha, the subjects and the count. Two more that change what the next window should
 * *do* with them:
 *
 * - **How far behind main it is.** A branch a hundred commits back wants a cherry-pick onto
 *   a fresh worktree rather than a checkout, and that is not something a reader can guess
 *   from a sha.
 * - **Whether it was ever pushed.** Unpushed, that worktree is the only copy of the work in
 *   the world and the sentence has to say so. Pushed, there is very likely a pull request
 *   carrying it — bc-xl7n.83 is that shape — and telling a worker "never pushed" there
 *   would be false. One `rev-parse` decides which sentence gets written.
 *
 * Read-only, and every git call is `ok`-wrapped: this runs on the way out of a session that
 * has already failed, and a repo that cannot answer should cost the bead its comment rather
 * than cost the tick its archive.
 */
export async function salvageNote(dir, { bead, ref = null, branch, head, worktree = null, commits = [] }) {
  if (!branch || !head || !commits.length) return null;
  const main = await mainCheckout(dir);

  // Capped: a window that ran for two hours can leave more subjects than belong in a
  // comment, and the first few are what identify the work.
  const shown = commits.slice(0, SALVAGE_SUBJECTS);
  const log = await ok(git(main, ['log', '--no-walk', '--format=%h %s', ...shown]));
  const subjects = (log || '').trim().split('\n').filter(Boolean);
  const counted = (await ok(git(main, ['rev-list', '--count', head + '..main'])))?.trim();
  const behind = /^\d+$/.test(counted || '') ? Number(counted) : null;
  const pushed = Boolean(await ok(git(main, ['rev-parse', '--verify', '--quiet', 'refs/remotes/origin/' + branch])));

  const n = commits.length;
  const rest = commits.length - shown.length;
  return [
    '**A window that was opened on this bead committed ' + n + ' time' + (n === 1 ? '' : 's') + ' and then died.**',
    '',
    'It never delivered and never closed the bead, so the claim was handed back and ' + bead + ' went',
    'back into the queue — which is what should happen, and is also why nothing else records',
    'that anything was built. It was:',
    '',
    '    branch   ' + branch,
    ...(worktree ? ['    worktree ' + worktree] : []),
    '    head     ' + head,
    '    commits  ' + n + (behind === null ? '' : ', ' + behind + ' behind main'),
    ...(ref ? ['    log      ' + ref] : []),
    '',
    ...subjects.map((line) => '    ' + line),
    ...(rest > 0 ? ['    … and ' + rest + ' more'] : []),
    '',
    ...(pushed
      ? [
          '`origin/' + branch + '` has it, so it is not lost — look for the pull request that already',
          'carries this work before starting anything.',
        ]
      : [
          'That branch was **never pushed**: the worktree it is in is the only copy of this work.',
          'Look at it before building any of this again.',
        ]),
    ...(behind !== null && behind > SALVAGE_STALE
      ? [
          '',
          'It is ' + behind + ' commits behind main, which is far enough that cherry-picking onto a fresh',
          'worktree is likely to be less work than a checkout and a downmerge.',
        ]
      : []),
    '',
    '— beadcause, when the claim was handed back',
  ].join('\n');
}
...
export async function archiveSession(dir, session) {
  ...
  const read = transcripts.length
    ? readTranscripts(transcripts)
    : { lines: [], models: [], usage: { ...EMPTY_USAGE } };
  const ranModels = read.models;
  // What the run cost, graded against the window the *selection* bought — `model`, not
  // anything in the file. See `contextWindow` in lib/sessiontokens.js: the transcript
  // writes `claude-opus-5` for both the 200k model and the 1M one, so the window is only
  // knowable from what the launcher asked for, and this is where that is known.
  const tokens = sessionTokens(read.usage, model);
  ...
  const meta = {
    ...
    // And what that hour cost (bc-nc6o.8): the turns, the tokens, the peak occupancy, the
    // window it was routed into and whether the harness had to compact to stay inside it.
    // `null` — not a record of zeros — for a session that left nothing measurable and for
    // every entry archived before this field existed, because a zeroed record would read
  ...
  return {
    ...
    commits,
    branch,
    // Where the session actually was. Already resolved above for the archive's own sake,
    // and returned because a bead being handed back needs an address rather than a branch
    // name: the branch is how to get the work, the directory is where it already is
    // checked out (bc-xl7n.102, `salvageNote`).
    worktree,
    head,
    merged,
    transcriptBytes,
    ...
    ran: ranModels,
    // Same division for the cost: the numbers stay in `meta.json`, and what reaches the
    // bead is the one-word verdict on `tokens.pressure` (bc-nc6o.8).
    tokens,
  };
}
 ...
 * other end, and it is here because the archive is where they end up: a debrief lives as
 * `memory.md` in a session's tree, so reading one is walking a bead's archive newest-first
 * until a session that left one turns up.
 *
 * **But the archive is not the only place a report can be**, and reading it as if it were
 * is what made a debrief invisible for as long as it was pending — for ever, for a window
 * nobody ever archives. So this reads the staging ref too, newest-first, which is what
 * `staged: true` on a row means. Both come back in the same shape and render identically,
 * because "this report is pending" is a fact about the plumbing and not about the report.
 *
...
export async function readDebriefs(dir, beads, { perBead = 1, scan = 10 } = {}) {
  ...
  for (const bead of beads || []) {
    let found = 0;
    // Staged first, because staged *is* the newest run at this bead: the report was
    // written by a window the archive has not caught up with, and `perBead` is 1 for the
    // brief precisely so the most recent one is what gets quoted. Reading the archive
    // first would spend that budget on an older run and drop the pending one.
    const pending = await stagedDebrief(main, bead);
    if (pending.entries.length) {
      const text = renderDebrief(pending.entries).trim();
      if (text) {
        out.push({
          bead,
          // No archive commit exists yet — this is what `staged` means, and a reader that
          // needs to tell the two apart keys off this rather than off the text.
          commit: null,
          staged: true,
          at: pending.entries[pending.entries.length - 1]?.at || null,
          text,
        });
        found += 1;
      }
    }
    if (found >= perBead) continue;
    ...
    for (const line of history.split('\n').filter(Boolean)) {
      ...
      out.push({ bead, commit, staged: false, at, text: text.trim() });
      found += 1;
    }
  }
  return out;
...
/**
 * Every bead whose debriefs are worth asking about — archived or still staged.
 *
 * The candidate set for a tier-4 read, and the one place the union is decided, because
 * there are two callers of it (`debriefs` in bin/beadcause-memory and `debriefsFor` in
 * lib/session.js) and they had drifted into the same bug once already: both took
 * `archivedBeads` alone, so a bead whose only report was pending could not even be
 * *named*, let alone read. See `stagedBeads` in lib/memory.js for what that cost.
 *
 * Tolerant like both halves it unions — the honest answer to a directory that is not a
 * checkout is an empty set, which is also the truth about a repo where nobody has ever
 * left a report.
 */
export async function debriefBeads(dir) {
  const [archived, staged] = await Promise.all([
    archivedBeads(dir).catch(() => new Set()),
    stagedBeads(dir).catch(() => new Set()),
  ]);
  return new Set([...archived, ...staged]);
}
 ...
 *
 * `filed-while:<filerBead>` (`lib/filing.js`) already names the bead the filer was
 * working; this reads *that* bead's own archive and matches an entry in it to the
 * moment of filing. The chain — label, then archive — already exists everywhere an
 * agent has filed a bead. What was missing was answering it in one call instead of
 * three, and saying plainly when it does not close rather than showing nothing
 * (the acceptance criterion this function exists to meet).
 *
 * **Matching, not just reading the newest entry.** A bead a filer worked more than
 * once — a stall, then a resumed session — archives more than one entry under the
 * same ref, and the one that filed a *particular* bead is whichever session was
 * actually running at the moment it was created, not whichever happened to be
 * archived last. `createdAt` (the filed bead's own `created_at`) is matched against
 * each candidate's `meta.json` window — `startedAt <= createdAt`, and the archive
 * itself did not happen before the bead existed (`at >= createdAt`) — read only for
 * the candidates archived at-or-after `createdAt`, cheapest-first, stopping at the
 * first containing window. **`exact: true`** means a session's own recorded start
 * and archive time actually bracket the moment of filing; with no `createdAt` to
 * check against, or when nothing's window contains it (a session archived without
 * ever recording `startedAt`, say), the nearest archive by wall-clock distance is
 * still returned, flagged `exact: false` — a guess worth having, not a hit.
 *
 * `sessions: 0` is the chain not closing: the filer bead was never archived at all,
 * most often because that session is still running, or ended without a commit to
 * archive (`archiveSession`'s own `{skipped}` case). Say so; do not show nothing.
 */
export async function filerSession(dir, filerBead, createdAt = null) {
  const { sessions } = await readArchive(dir, filerBead, { limit: 50 });
  if (!sessions.length) return { filerBead, sessions: 0, session: null, exact: false };

  const createdMs = createdAt ? Date.parse(createdAt) : NaN;
  if (!Number.isFinite(createdMs)) {
    const detail = await readSessionDetail(dir, filerBead, { commit: sessions[0].commit });
    return { filerBead, sessions: sessions.length, session: detail.session, exact: false };
  }

  // Cheapest first: only the entries that could possibly contain the moment of
  // filing (archived at-or-after it), nearest one first, and stop at the first
  // whose own recorded start actually reaches back before it.
  const afterOrAt = [...sessions]
    .filter((s) => Date.parse(s.at) >= createdMs)
    .sort((a, b) => Date.parse(a.at) - Date.parse(b.at));
  for (const s of afterOrAt) {
    const detail = await readSessionDetail(dir, filerBead, { commit: s.commit });
    const startedAt = detail.session?.meta?.startedAt;
    if (startedAt && Date.parse(startedAt) <= createdMs) {
      return { filerBead, sessions: sessions.length, session: detail.session, exact: true };
    }
  }

  // Nothing's own window contains it — the best this repo can offer is whichever
  // archive sits closest to the moment of filing, said as a guess and not a hit.
  let nearest = sessions[0];
  let nearestDiff = Math.abs(Date.parse(nearest.at) - createdMs);
  for (const s of sessions) {
    const diff = Math.abs(Date.parse(s.at) - createdMs);
    if (diff < nearestDiff) {
      nearest = s;
      nearestDiff = diff;
    }
  }
  const detail = await readSessionDetail(dir, filerBead, { commit: nearest.commit });
  return { filerBead, sessions: sessions.length, session: detail.session, exact: false };
}
