 ...
 *
 * Without this every archived session is authored by whoever's git identity the
 * repo carries, which would put Adam's name on objects he did not write — the same
...
async function git(cwd, args, opts = {}) {
  try {
    const { stdout } = await run(
      'git',
      ['-c', 'user.name=beadcause', '-c', 'user.email=beadcause@localhost', ...args],
      { cwd, timeout: 30000, maxBuffer: 96 * 1024 * 1024, ...opts }
    );
    return stdout;
  } catch (err) {
 ...
 * hangs on an empty stdin and fails with a message that names no cause. Every
 * object written here (`hash-object`, `mktree`) arrives that way, so this is the
 * path that matters.
 */
function gitInput(cwd, args, input) {
  return new Promise((resolve, reject) => {
    const child = spawn('git', ['-c', 'user.name=beadcause', '-c', 'user.email=beadcause@localhost', ...args], {
      cwd,
...
/** Write stdin to a blob and return its sha. */
async function hashObject(cwd, buf) {
