 ...
 *   steps   an ordered array of `{ type: 'file', path, content }`, `{ type: 'delete',
 *           path }` or `{ type: 'commit', message }`, in the order the caller wants
 *           them applied — see bin/b7e-fixture for how `--file`/`--commit` become the
 *           first and last of these. Anything written after the last commit step is
 *           left uncommitted on purpose: that is what reproduces bc-68ou.14's cp-smoke
 *           (one commit, an uncommitted fix). `delete` (bc-xl7n.154) exists so a rename
 *           can be built: a `file` step at the new path plus a `delete` at the old one,
 *           staged into the same `commit` step — `git add -A` picks up both, and it is
 *           git's own rename detection that turns the pair into a recorded rename, not
