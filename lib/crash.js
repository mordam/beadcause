 ...
 *
 * The poll cycle in lib/server.js catches and logs five background sweeps — the poll
 * itself, the deploy sweep, the owed-close sweep, the advocate tick, the release sweep —
 * and carries on. That is right: none of them should be able to stop the others, and most
 * of what they hit is the tracker being locked or `gh` being logged out. But it also
 * means a `TypeError` in the advocate has been logged every thirty seconds for a week
 * with nobody reading it, and *that* is a bug that should be a bead.
 *
...
import { fingerprint, intake } from './errors.js';
 ...
 * (lib/errors.js), and there is no coalescing window on those yet — bc-5f9b. A sweep
 * broken for a day is 2,880 ticks, and a bead carrying 2,880 identical comments is a bead
 * nobody can read.
 */
...
export async function reportCrash(value, { kind = 'error', from = '' } = {}) {
  ...
  total += 1;
  try {
    const out = await withTimeout(intake(armed.bd, armed.workspace, report, { actor: armed.actor || null, from }));
 ...
 */
export function installCrashHandlers(cfg, { bd, workspace, where = '', bus = null, actor = null, exit = null } = {}) {
  armed = { cfg, bd, workspace, where, bus, actor };
