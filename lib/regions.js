 ...
 *
 * Exported for lib/siblings.js, the second legitimate reader of "what has this worktree
 * done to this file" — asked before any claim exists, rather than after one is refused.
 * Same derivation, same fail-open contract; nothing here changed for it.
 */
export async function changedSince(dir, file, base) {
...
/**
 * The same reading between two refs, needing no working tree at all.
 *
 * `changedSince` above runs `git diff` against a directory's actual working tree, which is
 * exactly right for a live worktree and cannot answer at all for one whose directory is
 * gone — `execFile` has nowhere to run. But a worktree's *branch* survives its checkout
 * being deleted: every worktree of a repo shares one object store, so a branch tip is
 * still readable from any other worktree, main checkout included. This is that reading —
 * `git diff base..ref`, two commits, no `cwd` requirement beyond "some checkout of this
 * repo" — for lib/siblings.js's pruned-worktree case, where `changedSince` would return
 * `null` for a directory that no longer exists to be diffed against.
 *
 * No on-disk fallback for an empty diff, unlike `changedSince`: there is no working tree
 * here to read an untracked file out of, so an empty diff between two refs means exactly
 * what it says — unchanged.
 */
export async function changedBetween(dir, base, ref, file) {
  const diff = await read(dir, ['diff', '--unified=0', `${base}..${ref}`, '--', file]);
  if (diff === null) return null;
  const out = parse(diff);
  out.newFile = /^--- \/dev\/null$/m.test(diff);
  return out;
