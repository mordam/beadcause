...
export const beadKey = (workspace, id) => `${workspace}/${id}`;

/** The key for a resolver window, which has a pull request and no bead. */
export const prKey = (workspace, number) => `${workspace}/pr-${number}`;
...
export function parkable(rec) {
  ...
  if (!String(rec.workspace || '').trim()) return false;
  return true;
}
...
export function recordPark(parked, key, rec, now = new Date()) {
  ...
  return {
    ...
    [key]: {
      ...
      // Carried across the overwrite for the reason in the header: this counts trips,
      // not parks-since-resume.
      resumes: Number(prior?.resumes) || 0,
    },
  };
}
...
export function parkedList(parked, workspace = null) {
  return Object.entries(parked || {})
    ...
    .filter(([, rec]) => rec && (!workspace || rec.workspace === workspace))
...
export function openList(opened, workspace = null) {
  return Object.entries(opened || {})
    .filter(([, rec]) => parkable(rec) && (!workspace || rec.workspace === workspace))
    .sort((a, b) => Date.parse(b[1].at || 0) - Date.parse(a[1].at || 0))
    .map(([, rec]) => ({ ...rec }));
}
 ...
 *
 * `anythingElse` — a status this does not recognise — is `wait`, which is the safe
 * direction: an unrecognised status leaves a window open, and the state before this
 * feature existed was every window open.
 */
export function parkDecision(rec, live, { idleMinutes = 10, now = new Date() } = {}) {
  ...
  if (live.status !== 'idle') return { act: 'wait', why: `its status is "${live.status || 'unknown'}"` };
  ...
  if (idleFor < idleMinutes) return { act: 'wait', why: `quiet for ${Math.round(idleFor)}m of ${idleMinutes}m` };
