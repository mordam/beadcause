...
export function prunePark(parked, now = new Date()) {
  ...
  const rows = Object.entries(parked || {})
    ...
    // Same shape as pruneAnswered's eviction: `at` is millisecond-resolution, so key
    // breaks a tie instead of letting accidental object order pick who gets evicted.
