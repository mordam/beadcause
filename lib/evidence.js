...
export const NOT_EVIDENCE = Object.freeze([
  ...
  {
    file: 'lib/scaffold.js',
    why: "b7e-scaffold (bc-dgx7.32) prints the skeleton of a NEW bin/ command as text, and one of its templates — the printed lib/<name>.js stub — carries a paragraph warning its future author that if THAT module ends up touching CONFIG_DIR, IT will owe a register entry. The word only exists here inside a template literal being assembled as text to hand to stdout; this module never imports lib/config.js, never reads or writes CONFIG_DIR itself, and the scan cannot tell 'a string being printed happens to contain this word' from 'this identifier is used in real code', because blankComments (correctly) leaves string and template contents alone. See `persistsState`'s own 'a module that only mentions CONFIG_DIR in prose does not count' case, which this is the same shape as, one level removed — prose here is a docblock TEXT VALUE rather than a docblock COMMENT.",
  },
]);
