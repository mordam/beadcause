...
export const NOT_EVIDENCE = Object.freeze([
  ...
  {
    file: 'lib/access.js',
    why: "the access register. Like this one it names paths — every `where` on a credential row is a `${CONFIG_DIR}/…` string — and it opens none of them: the module imports no filesystem at all. What it does record, the periodic review, is the append-only `REVIEWS` array in its own source, so the evidence that a review happened is the commit that appended the entry, in this repo's history.",
  },
  {
