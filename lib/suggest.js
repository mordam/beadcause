 ...
 * options, wrote the exact sentence each one records, and the card draws buttons
 * that answer and close on two taps. Most questions do not arrive that way. An
 * ordinary session files a `human` bead with the choices sitting in the prose —
 *
 ...
 * field (`q.suggested`) because it has its own trust level and its own gesture:
 * a real option's `response` was written by the agent as the answer, so tapping
 * it twice sends it; a suggestion is a line this file lifted out of a paragraph,
 * so tapping it fills the answer box and you send it yourself. The words that go
 * on the thread under Adam's name are always words he has actually read.
 *
...
function byBoldList(units) {
  ...
  const bold = [...lists.values()].filter(
    (items) => items.length >= MIN_OPTIONS && items.every((u) => /^(?:\*\*|__)\s*\S/.test(u.text))
  );
...
export function suggestOptions(markdown) {
  ...
  for (const strategy of [byOptionLabel, byLeadIn, byBoldList]) {
