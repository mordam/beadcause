...
export async function sweepConflicts(
  ...
  for (const c of conflicting) {
    ...
    try {
      ...
      out.failed.push({ ...brief(row), why: oneLine(err) });
      continue;
    }
    if (outcome.error) out.failed.push({ ...brief(row), why: outcome.error });
    else if (outcome.reused) out.reused.push({ ...brief(row), note: outcome.note || '' });
    else if (outcome.queued) out.queued.push({ ...brief(row), place: outcome.queued.place, note: outcome.note || '' });
    else out.handed.push({ ...brief(row), opened: outcome.opened || null });
  }
