...
/**
 * Is this the brief of a *coding* window — the one thing `globalMaxWorkers` counts
 * (bc-2uj4.13)?
 *
 * `workPromptFor` in lib/session.js opens every worker's first line the same way,
 * whatever shape the bead is — `You are working bead **<workspace>/<id>**` for an ordinary one,
 * `You are working bead **<workspace>/<id>** and the N … grouped with it` for a plan
 * group, `You are working epic **<workspace>/<id>** and the N … under it` for a batch
 * head. Nothing else in this file opens with that phrase: a planner says "You are the
 * **epic worker** for …", a resolver says "You are the MergeAdvocate/ReviewAdvocate
 * for …" or "You are the worker for … pull request", an Epic Advocate says "You are the
 * Epic Advocate for …". So this line alone is enough to tell a coding window from every
 * other kind of session `sessionCommand` ever launches, with no id to compare it to —
 * which is what a *census* needs, as opposed to `linesNameBead` above, which already
 * knows the id it is looking for.
 */
const WORKER_BRIEF_RE = /You are working (?:epic|bead) \*\*/;
export const isWorkerLine = (args) => WORKER_BRIEF_RE.test(String(args || ''));
