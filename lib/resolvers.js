 ...
 *
 * So there is a cap of `maxLive()`, and the requests that do not fit **queue**
...
/**
 * How many resolvers this Mac runs at once, when nothing has said otherwise.
 *
 ...
 * loud interpolates it from `maxLive()`.
 *
 * It is the *default* rather than the number, since bc-29b3. It used to be a constant,
 * which meant the one cap on this Mac's unattended windows that nobody could turn down
 * — `advocates.globalMaxWorkers` has a config key, a console stepper and a migration,
 * and this had none of the three. `advocates.maxResolvers` is the key; the export stays
 * because it is what a suite asserting the shipped default should read.
 */
export const MAX_LIVE = 2;

/**
 * The most resolvers that could ever be asked for, whatever a config file says.
 *
 * The same reasoning as `GLOBAL_WORKERS_CEILING` in lib/advocate.js: a key you can turn
 * down is a key somebody can turn up by a digit too many, and a clamp that lives in code
 * cannot drift. Six is already past the point where the gate runs start eating each
 * other; it is here as a backstop against a typo, not as a recommendation.
 */
export const MAX_LIVE_CEILING = 6;

/** What `maxLive()` answers. Set once at boot from the config — see `setMaxLive`. */
let capacity = MAX_LIVE;

/** How many resolvers this Mac runs at once. */
export function maxLive() {
  return capacity;
}

/**
 * Take the cap from the config, clamped, and answer with what was actually taken.
 *
 * Anything that is not a finite number leaves the default alone rather than becoming
 * `Number(null)`, which is 0 — a cap of zero would queue every conflict on the Mac
 * forever and read, in the log, exactly like a sweep that found nothing to do.
 */
export function setMaxLive(value) {
  const asked = Number(value);
  if (!Number.isFinite(asked)) return capacity;
  capacity = Math.max(1, Math.min(MAX_LIVE_CEILING, Math.round(asked)));
  return capacity;
}

/**
 * Everything else on this Mac that comes out of the same budget — see bc-29b3.
 *
 * A resolver is the same kind of thing as an advocate's worker: an unattended Claude
 * Code session in an iTerm window running this repo's own gate. Until this hook existed
 * the two were counted by two caps that could not see each other, so a Mac already at
 * `globalMaxWorkers` sessions would still take two more for a sweep, and the number that
 * actually matters — the sum — was written down nowhere and enforced by nobody.
 *
 * A hook rather than an import because the direction matters: lib/advocate.js imports
 * *this* file to subtract resolvers from its own free slots, and this file importing it
 * back would be a cycle. The daemon owns both and wires them together in one line.
 *
 * Answering `null` is the honest default and the state every test and every consumer
 * that is not the daemon is in: nothing has told this file what else is running, so it
 * caps only what it can count, exactly as it did before.
 */
let elsewhere = () => null;

/**
 * Tell this file what else is open, and against what total.
 *
 * The function is asked at the moment a decision is made rather than read once, because
 * both halves move: windows open and close, and `globalMaxWorkers` is a stepper on the
 * console. `{ live, cap }` — how many non-resolver windows are open, and the cap the sum
 * of both populations is measured against.
 */
export function accountAgainst(fn) {
  elsewhere = typeof fn === 'function' ? fn : () => null;
}

/**
 * Whether one more window would be one too many, and — the point of the string — *why*.
 *
 * Two caps, asked in the order they bind, and the answer is the sentence a queued pull
 * request reads back on the phone. "Two resolvers are already running" and "this Mac is
 * full" are different facts about a wait, and a queue that says the first when the
 * second is true sends whoever reads it looking at the wrong number.
 *
 * Empty string means there is room. Never throws for the budget's sake: a hook that
 * fails is the daemon's problem, and it must not turn a sweep into a refusal.
 */
function noRoom(now = Date.now()) {
  const mine = list(now).length;
  if (mine >= maxLive()) {
    return `${maxLive()} resolver${maxLive() === 1 ? ' is' : 's are'} already running on this Mac`;
  }
  let other = null;
  try {
    other = elsewhere();
  } catch {
    /* the daemon's hook, not this file's business — cap on what can be counted */
  }
  const cap = Number(other?.cap);
  const live = Number(other?.live);
  if (!Number.isFinite(cap) || !Number.isFinite(live)) return '';
  if (live + mine < cap) return '';
  return (
    `this Mac is at its ${cap}-window limit — ${live} session${live === 1 ? '' : 's'} ` +
    `and ${mine} resolver${mine === 1 ? '' : 's'} are already open`
  );
}
...
export function reset() {
  ...
  stopDrain();
  draining = null;
  // The two things a daemon told this file about the world outside it. A suite that
  // staged a full Mac and did not put these back would cap the *next* case against a
  // budget it never set, and the failure would read as the cap being wrong rather than
  // as the fixture being dirty.
  capacity = MAX_LIVE;
  elsewhere = () => null;
  try {
 ...
 *
 * `why` is `noRoom`'s answer and not a fixed clause, because since bc-29b3 there are two
 * different reasons to be in line — the resolver cap, and the whole Mac's window budget
 * — and they are relieved by different things closing.
 */
function queuedNote(number, place, why = '') {
  const because = why || `${maxLive()} resolver${maxLive() === 1 ? ' is' : 's are'} already running on this Mac`;
  return `#${number} is ${nth(place)} in line — ${because}, and a window opens for it as soon as one of them is done`;
}
 ...
 *
 * The probe is only ever asked when there is **no room** — either cap, since bc-29b3 —
  ...
  draining = (async () => {
    ...
    try {
      ...
      for (const rec of list(now)) {
        if (!waiting.length || !noRoom(now)) break;
      ...
      while (waiting.length && !noRoom(now)) {
    ...
    // Left on stdout, deliberately, unlike the boot line below — bc-zjab.12. This runs
    // from `pump()`, which nothing reaches at module load — only `scheduleDrain`'s
    // interval calls it — and `--url` exits (bin/beadcause.js reads the flag and calls
    // `process.exit`) long before any interval could ever fire. So it never lands in
    // that command-substituted stdout. Moving it anyway would be a second, unrelated
    // change riding on this bug's fix.
...
export async function resolveFor(
  ...
  return underLock(keyFor(workspace, number), async () => {
    ...
    // `noRoom` again rather than the reason it queued with: minutes may have passed, and
    // which cap is holding it now is the question a second ask is asking. An empty answer
    // means a slot freed between the drain's last tick and this press, and the fallback
    // clause is the honest thing to say about that — it is in line, and about to move.
    if (already >= 0) {
      return { queued: inLine(waiting[already], already + 1), note: queuedNote(number, already + 1, noRoom(now)) };
    }
    ...
    //
    // Two caps now, both inside `noRoom` — this file's own, and the Mac's whole budget
    // for unattended windows, which advocate workers come out of too (bc-29b3). The
    // reason travels with the entry because it is the sentence the phone is shown.
    const cramped = noRoom(now);
    if (cramped) {
      const entry = {
        workspace,
        number: Number(number),
      ...
      return { queued: inLine(entry, place), note: queuedNote(number, place, cramped) };
    }
 ...
 *
 * **On stderr, not stdout — bc-zjab.12.** This runs at module load, before any flag in
 * bin/beadcause.js has been read, so it is unavoidable from `--url`'s side: every import
 * has already executed by the time that flag is checked. `--url`'s stdout is a value, not
 * a display (scripts/build-android.sh command-substitutes it), and this is a notice about
 * a daemon that is starting, exactly like `sayIfCleartext` there — so it goes where every
 * other notice of that kind in this tree already goes. beadcause.log still gets it: the
 * router spawns backends with `stdio: 'inherit'`, which passes both streams through.
 */
