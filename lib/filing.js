 ...
 */
export function provenanceNotes(bead, { from = '', clamped = false, asked = null } = {}) {
  const lines = [
 ...
 */
export function beadToIssue(bead, { from = '', floor = PRIORITY_FLOOR, labels = [] } = {}) {
  ...
  return {
    ...
    design: bead.design || '',
    notes: provenanceNotes(bead, { from, clamped, asked }),
    ...
    labels: [UNENDORSED, FILED_LABEL, ...labels, ...(bead.labels || [])].filter(
 ...
 */
export async function fileBeads(bd, workspace, beads, { from = '', floor = PRIORITY_FLOOR, onWarn = () => {} } = {}) {
  ...
  for (const bead of beads || []) {
    const issue = beadToIssue(bead, { from: source, floor });
    try {
      const id = await bd.create(workspace, issue);
      if (!id) throw new Error('bd create returned no id');
      filed.push({ id, title: issue.title, priority: issue.priority, clamped: issue.clamped, duplicate: bead.duplicate || null });
    } catch (err) {
      failed.push({ title: issue.title, error: String(err?.message || err).split('\n')[0] });
    }
  }
  return { filed, failed, from: source };
}
