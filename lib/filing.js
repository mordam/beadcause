 ...
 * auto-endorse sentence names the space setting, because for `autoEndorse` that setting
 * *is* the reason and the reader's next question is where to turn it off. A reported
...
export function provenanceNotes(
  bead,
  { from = '', clamped = false, asked = null, endorsed = false, endorsedNote = '' } = {}
) {
  const lines = [
...
export function beadToIssue(
  bead,
  { from = '', floor = PRIORITY_FLOOR, labels = [], endorsed = false, endorsedNote = '' } = {}
) {
  ...
  return {
    ...
    design: bead.design || '',
    notes: provenanceNotes(bead, { from, clamped, asked, endorsed, endorsedNote }),
