...
export async function fileBeads(
  ...
  // (bc-xl7n.115), made here about a whole batch instead. Checked against the
  // *normalised* type and priority — the values about to reach `bd create`, via the
  // same `clampPriority` `beadToIssue` uses below — so a bead asking for P0 through a
  // caller whose `floor` still allows it (lib/errors.js, priority 0 for a crash report)
  // is judged on the P0 it will actually get, not clamped to P2 first and missed.
  //
