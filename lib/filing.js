...
export function provenanceNotes(
  ...
  // who made it — and the answer, "it had to be under a P0 to be workable at all", is
  ...
  if (homed) {
    lines.push(
      '',
      `**Filed under ${homed}.** A bead with no P0 above it is not workable (bc-rfnr.7), and nothing ` +
...
export async function fileBeads(
  ...
  // `gated` and not merely `!parent`: on a workspace with no open P0 at all the gate
  // fails open and the bead is perfectly workable, so warning about a hold would be a
  // false claim printed at every filing. See lib/homing.js.
  const home = await homeIn(bd, workspace, { from: source });
  if (!home.parent && home.gated) {
    onWarn(
      'nothing to hang this under — filing with no parent, which means nothing will work it until you ' +
        'adopt it under a P0 (bc-rfnr.7). Label an open P0 `unsorted` and the next one lands there.'
    );
  }
