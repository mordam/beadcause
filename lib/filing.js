...
export async function fileBeads(
  ...
  workspace,
  beads,
  { from = '', floor = PRIORITY_FLOOR, onWarn = () => {}, endorsed = false, homeLabel = '' } = {}
) {
  ...
  // `homeLabel` is bc-khoe.48, and it is handed straight through rather than decided
  // here: a seam that files one *kind* of bead every time (lib/sessionaudit.js and its
  // skill candidates) knows which epic owns that kind, and `from` — the session that
  // happened to evidence it — does not. See lib/homing.js. Every other caller passes
  // nothing and gets exactly what it got before.
  const home = await homeIn(bd, workspace, { from: source, homeLabel, onWarn });
  ...
  // See lib/homing.js.
  //
  // **Judged per bead, not just per workspace — bc-xl7n.128.** An epic, or a P0 of any
  // type, is workable with nothing above it (`isRoot`, lib/ownership.js), so a batch
  // that is entirely root-shaped beads gets no warning at all — the same false claim
  // an earlier pass at this fixed for `/api/bead/create` filing one bead at a time
  // (bc-xl7n.115), made here about a whole batch instead.
  //
  // **`type: epic` is the arm a caller can reach; the P0 arm is defensive.** Neither
  // caller of this function passes a `floor` — bin/file.js and lib/sessionaudit.js both
  // omit it — so `clampPriority` cannot return 0 here, and the P0 crash report that
  // *does* lower the floor (lib/errors.js, `ERROR_PRIORITY`) never comes through
  // `fileBeads` at all: it calls `beadToIssue` directly. So what a real agent actually
  // gets suppressed by is epic-ness at the default floor of 2, and that is the arm
  // test/filing.mjs pins first. The priority is still read *normalised* — through the
  // same `clampPriority` `beadToIssue` uses below, `floor` and all — so that a caller
  // that ever does lower the floor is judged on the P0 the bead will actually get
  // rather than on the P2 it would have been clamped to. That threading has a check of
  // its own, because a defensive branch nothing exercises is one a later edit drops
  // silently.
  //
  // **"At least one" non-root, not "every"** — the choice `/api/bead/create` never had
  // to make, since it only ever judges one bead. A batch of three where two are epics
  // and one is an ordinary orphan still has a bead nothing will work; suppressing the
  // warning because *some* of the batch is self-sufficient would hide that. The cost is
  // that the one sentence still reads as if it were about the whole batch rather than
  // naming which beads it does and doesn't apply to — the wording stays batch-shaped,
  // only whether it fires at all is now per-bead.
  const anyNonRoot = (beads || []).some(
    (bead) => !isRoot({ type: bead?.type || 'task', priority: clampPriority(bead?.priority, floor).priority })
  );
  if (!home.parent && home.gated && anyNonRoot) {
    onWarn(
