...
export async function fileBeads(
  ...
  // See lib/homing.js.
  //
  // NOT gated on `isRoot`, unlike the identical warning in lib/server.js's
  // `/api/bead/create` (bc-xl7n.115) — this fires once for a whole filed *batch*, so a
  // per-bead check needs a different shape, and a root-shaped bead filed alongside
  // others here can still get this same false claim. Tracked as bc-xl7n.128.
