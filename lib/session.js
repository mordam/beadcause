...
import { effective, claudeArgs, promptArgs, systemPrompt, agentExports } from './foundation.js';
import { memoryBrief, notesBrief, notesIn } from './memory.js';
...
export function workPromptFor(
  ...
  pr = null,
  owner = ownerName(),
  { autoEndorse = false, notes = null, repo = null, edit = '', requirements = '' } = {}
) {
  const id = bead.id;
 ...
 */
export function conflictPromptFor(workspace, row, owner = ownerName(), { sweptAfter = null } = {}) {
  const beads = (row.beads || []).map((b) => b.id).join(', ');
 ...
 *
 * `sweptAfter` is passed straight through to the brief and is the one thing a sweep has to
 * say that a press does not: the number of the pull request whose merge caused this. A
 * caller that omits it gets the brief that names the press, so the tap path is unchanged by
 * having a second caller — see `conflictPromptFor`.
 */
export function openConflictSession(cfg, workspace, row, { dir = null, sweptAfter = null } = {}) {
  const where = dir || resolveSessionDir(cfg, workspace);
  const name = row?.repoName ? `${workspace.name} · ${row.repoName}` : workspace.name;
  return launch(
    cfg,
    where,
    conflictPromptFor(name, row, ownerName(cfg), { sweptAfter }),
...
export async function openWorkSession(cfg, workspace, bead, { attempt = 1, doneFile = null, bd = null } = {}) {
  const row = assertStillOpen(assertNotSuperseded(await assertEndorsed(bd, workspace, bead)));
...
export function planPromptFor(
  ...
  kids = [],
  owner = ownerName(),
  { notes = null, repo = null, revising = false, autoEndorse = false } = {}
) {
  const id = epic.id;
  const n = kids.length;
 ...
 */
export function sessionCommand(f, { dir, promptFile, systemFile = null, mode = null, doneFile = null, commandFile = null }) {
 ...
 * about.
 */
async function launch(cfg, dir, prompt, tabTitle, { doneFile = null, agent = 'worker' } = {}) {
  ...
  fs.writeFileSync(commandFile, sessionCommand(f, { dir, promptFile, systemFile, mode, doneFile, commandFile }) + '\n', {
    mode: 0o600,
  });
