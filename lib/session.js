...
import { multiRepo, resolveRepo, beadToken } from './repos.js';
 ...
 */
export function workPromptFor(workspace, bead, attempt, pr = null, owner = ownerName(), { autoEndorse = false, notes = null } = {}) {
  const id = bead.id;
  const retry =
    attempt > 1
 ...
 *
 * Three things the brief pins down, because an unattended session with a vague scope
 * invents one:
 *
 ...
 */
export function conflictPromptFor(workspace, row, owner = ownerName()) {
  const beads = (row.beads || []).map((b) => b.id).join(', ');
...
\`${row.base}\`, so the pull request cannot merge as it stands${beads ? ` — it carries ${beads}` : ''}.
...
2. **If that tree is already mid-merge, stop.** \`git status\` says so, and so does a
   ...
   log.
3. \`git fetch origin ${row.base}\` and merge \`origin/${row.base}\` into \`${row.branch}\`. Resolve
...
4. Run this repo's own gate afterwards. What that is, is in its CLAUDE.md — use it, not a
   guess. A clean merge of two working branches is not a working tree, and this is exactly
   the case where that bites.
5. Push the branch. Then stop.
...
branch is a much better outcome than a resolution nobody can check.
`;
}
 ...
 */
export function openConflictSession(cfg, workspace, row) {
  const dir = resolveSessionDir(cfg, workspace);
  return launch(
    cfg,
    dir,
    conflictPromptFor(workspace.name, row, ownerName(cfg)),
    safeTitle(`⚠ rebase #${row.number} · ${workspace.name}`)
  );
}
...
export function sessionCommand(f, { dir, promptFile, systemFile = null, mode = null, doneFile = null }) {
  ...
  return [
    ...
    `rm -f ${shq(promptFile)}`,
    `claude${flags} ${promptArgs().join(' ')}${sessionEnding(doneFile, systemFile ? [systemFile] : [])}`,
  ].join(' && ');
}
