...
`
  ...
  //
  // Placed below the group and above the retry line, deliberately. A relay changes *who
  // the agent is*, which has to be settled before the session reads a single instruction
  // underneath it — but it comes after `groupBrief`, because a bead that is both grouped
  // and relayed is one whose *scope* an epic worker decided and whose *authorship* this
  // decided, and scope is the thing you read first.
