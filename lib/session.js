...
2. \`git fetch origin ${row.base}\` and merge \`origin/${row.base}\` into \`${row.branch}\`. Resolve
...
3. Run this repo's own gate afterwards. What that is, is in its CLAUDE.md — use it, not a
   guess. A clean merge of two working branches is not a working tree, and this is exactly
   the case where that bites.
4. Push the branch. Then stop.
