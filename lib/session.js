...
3. **It is the same job as a bead that already exists.** Closing it is not yours to do —
   ...
   marker instead — **with the command, not by hand** — then stop:
   ...
   whose one tap is the close, which sends them to your comment. Do the three writes by
   hand and there are three ways to get them wrong that all look like success: \`bd dep
   add\` is *refused* when the original is an epic, a bead left \`in_progress\` is
...
export function sessionCommand(f, {
  ...
  return [
    `cd ${shq(dir)}`,
    agentExports(f, bead ? { BEADCAUSE_BEAD: String(bead) } : {}),
 ...
 * about.
 */
async function launch(cfg, dir, prompt, tabTitle, { doneFile = null, agent = 'worker', bead = null } = {}) {
  ...
  const script = sessionCommand(f, { dir, promptFile, systemFile, mode, doneFile, commandFile, bead });
