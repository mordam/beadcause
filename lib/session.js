...
next step — not at the end, when a window that ran out of room has already stopped:
    ...
    ${RELAY_CMD()} -w ${workspace} -b ${id} --role <the role that just ran> --step <draft|check|revise|file> \\
      --next <the role you are handing to> --flag '<what you flagged, if anything>' \\
      -m '<what you did, and what you are handing on>'
...
and its age on the row, the whole trail when the bead is opened. \`--flag\` is what the next
role has to answer rather than everything you noticed; a flag on every step is a trail with
no flags in it.
...
**So expect an ending you do not usually get.** beadcause refuses this close in code rather
than trusting the paragraph above, on every path a merge can arrive by, so what happens is:
the pull request merges, **this bead stays open**, still claimed by you, with a comment on it
saying why. That is the work finishing correctly and there is nothing to retry — not the
delivery, not a \`bd close\` at a terminal, which is the one route that refusal cannot cover.
Say in your final message that it is merged and deliberately still open.

**Deliver with \`--review\` if you want ${owner} to decide the merge too.** That is the habit the
studio charter asks for on ${isGate ? 'a gate' : 'work waiting on an approval'}: the code guarantees only that nothing agent-shaped
closes this bead, and \`--review\` is what additionally puts the merge itself in front of them.
${
  isGate
    ? `
 ...
 *
 * **And a fourth: it refuses a bead with nothing decided above it** — the one refusal here that is
 ...
 * bead, so refusing on it would stop every session on the Mac. See lib/underroot.js.
 *
 ...
 *
 * **And a sixth: it refuses a container.** A standing root — bc-w156, bc-xl7n — is the
 ...
 * closed or unendorsed while the conversation was parked must not get a window, and
 * "there is a parked session for it" is not an exemption from any of them.
 *
...
export async function resumeWorkSession(cfg, workspace, bead, resume, { bd = null, doneFile = null } = {}) {
  const row = assertNotContainer(
    assertNotShipBead(assertStillOpen(assertNotSuperseded(await assertEndorsed(bd, workspace, bead))))
  );
...
- **Two groups may not expect to touch the same file.** Say what each group will touch, in
  ...
  document where it can simply not be created. Put them in one group, or decide which group
  owns the file.
 ...
 *
 * **And the fourth refusal with them**: an epic with nothing decided above it may not be planned
 ...
 *
 * **And a fifth, which lands hardest here of the three doors**: a container may not be
 ...
 * because this agent's job includes filing children and a window opened on an epic that
...
export async function openEpicAdvocateSession(cfg, workspace, bead, { kids = [], plan = null, reason = '', doneFile = null, bd = null } = {}) {
  ...
  if (isPaused(row)) {
    throw Object.assign(
      new Error(
        `${row.id}'s advocate is paused — nothing is being dispatched under it. Resume it from its section on the console first.`
      ),
