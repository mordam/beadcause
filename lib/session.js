 ...
 */
function workPromptFor(workspace, bead, attempt, pr = null, owner = ownerName()) {
  const id = bead.id;
  const retry =
    attempt > 1
  ...
  const delivery = pr ? deliverSection(workspace, id, pr, owner) : closeSection(id);
...
/**
 * Say one line to a session this daemon opened, and find out if it is still there.
 *
 * Resolves `'sent'` or `'missing'`. Missing means no iTerm session carries that id
 ...
 */
export function messageSession(termId, text) {
  const line = String(text).replace(/\s*\n+\s*/g, ' ').trim();
  return new Promise((resolve, reject) => {
    execFile(
      '/usr/bin/osascript',
      [MESSAGE_SCRIPT, String(termId), line],
      { timeout: 20000 },
      (err, stdout, stderr) => {
        if (err) {
          const detail = `${stderr || ''}${err.message}`;
