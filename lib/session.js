...
export function workPromptFor(workspace, bead, attempt, pr = null, owner = ownerName()) {
  ...
  const discovery = proposeSection(workspace, id, owner);
...
/**
 * The one rule, with three channels behind it.
 *
 * "Do not create beads" is unchanged and absolute. What changed is that a session no
 * longer has to *drop* what it found. The ending this replaced said "write it in a
 * comment under `## Discovered`", and that comment was invisible until the repo's
 * advocate ran out of ready work and surveyed the comments, which on a repo with a
 * queue is never — so a discovery arrived a fortnight after the context that made it
 * obvious had gone, or it arrived not at all.
 *
 ...
 *
 * The three are the three things a worker can owe a human, and they are deliberately
 * different acts:
 *
 * - **Discovery** — "there is more work here". `beadcause-propose --kind discovery`
 *   files it as a question the moment it is found, carrying the full text of every
 *   bead, approve/adjust/decline per row. Nothing is created until he presses a
 *   button; only the waiting is gone. The work carries on meanwhile.
 ...
 * - **A contradiction** — "two things disagree and it is not mine to settle".
 *   `--kind conflict` parks the bead behind the question. A session that picks one is
 *   how an unattended queue quietly does the wrong thing for a week.
 */
function proposeSection(workspace, id, owner) {
  return `
...
as a question they can answer from their phone:
    ...
    ${PROPOSE_CMD} -w ${workspace} --from ${id} --kind discovery <<'EOF'
    - title: <one line>
      type: task
      priority: 2
      description: |
...
export async function openWorkSession(cfg, workspace, bead, { attempt = 1, doneFile = null, bd = null } = {}) {
  await assertEndorsed(bd, workspace, bead);
  const dir = resolveSessionDir(cfg, workspace);
  return launch(
    cfg,
    dir,
