 ...
 * - **Say so, in one fixed phrase, when the work is done.** A `DONE-` prefix on the
 *   session name and a literal `** BEAD WORK DONE ** CAN BE CLOSED **` as the last
 *   line. Both are for a human scrolling a `/resume` list or a wall of windows hours
 *   later, and neither follows from the closed bead: a session that closed its bead
 *   and one that quietly stalled look identical there, and a session can close a bead
 *   and still have more to say. The wording is fixed rather than paraphrased so it can
 *   be searched for.
 */
...
2. **Make the last line of your final message this, on its own, with nothing after
   it:**
...
line is what says this window has nothing left to do, which a closed bead does not —
a session can close a bead and still be mid-thought. ${doneWhen} Neither of the two
endings below earns them.
