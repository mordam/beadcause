 ...
 */
export function openConflictSession(cfg, workspace, row, { dir = null } = {}) {
  const where = dir || resolveSessionDir(cfg, workspace);
  const name = row?.repoName ? `${workspace.name} · ${row.repoName}` : workspace.name;
  return launch(cfg, where, conflictPromptFor(name, row, ownerName(cfg)), safeTitle(`⚠ rebase #${row.number} · ${name}`));
}
...
export async function openWorkSession(cfg, workspace, bead, { attempt = 1, doneFile = null, bd = null } = {}) {
  const row = assertNotSuperseded(await assertEndorsed(bd, workspace, bead));
 ...
 *
 * Same door as `openWorkSession` and deliberately the same two refusals in front of it: an
 * unendorsed epic may not be planned any more than it may be worked, and a superseded one
 * is somebody else's work either way. A planner is cheaper than a worker but it still files
 * beads, still claims the epic, and still costs an unattended window — there is no version
 * of "it is only planning" that earns a weaker gate.
 *
