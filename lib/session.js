...
/**
 * The two commands a worker is told to run, as absolute paths.
 *
 ...
 *   same four acts.
 * - **The session names them; it does not do them.** Merging, pushing and deploying
 *   stay Adam's, which is the rule `deliverSection` already states for PR mode and for
 *   the same reason: several sessions run at once, and when each merged its own work
 *   the merges raced and every conflict landed on him anyway, hours later and in a repo
 *   he had not been reading.
 *
 * In PR mode none of those verbs can apply, because none of those acts are the
 * session's: it delivers a branch and a question, and the merge *is* Adam answering.
 * The line there is `CAN BE REVIEWED` — the same sentence about a different
 * outstanding step, and the only one a delivered session can honestly write.
 */
function workPromptFor(workspace, bead, attempt, pr = null) {
  const id = bead.id;
  const retry =
    attempt > 1
  ...
  const delivery = pr ? deliverSection(workspace, id, pr) : closeSection(id);
  const discovery = pr ? proposeSection(workspace, id, pr) : DISCOVERED_SECTION;
  // In PR mode the session does not close the bead — merging does — so the marker
  // paragraph cannot go on saying "only when the bead really is closed". The two
  // strings say the same thing about a different fact.
  const doneWhen = pr
  ...
  // depends entirely on which of the two endings it was given. In PR mode it can owe
  // nothing but a review, because merging, pushing and deploying were never its.
  const marker = pr ? PR_MARKER_STEP : OWED_MARKER_STEP;
...
/**
 * The same line in PR mode, where there is exactly one thing a session can owe.
 *
...
/**
 * How a session finishes when there *is* somewhere to open a pull request — which
 * is now the ordinary case, and the reason this file changed.
 *
 ...
 */
function deliverSection(workspace, id, pr) {
  return `
**When it is done, you do not merge it — you deliver it.**

This repo lands work through pull requests that Adam approves from his phone. You
...
already open for this branch), and files the question that carries its link to Adam.
...
he asks for changes is something the author already knew they had skipped.

**Do not close ${id}.** Merging closes it, and merging is his. Three things can come
back instead, and they mean different things:

- **Changes requested.** The approach is right, something on it is wrong. His note is
...
- **Declined.** The approach is wrong. The pull request is closed and that branch is
  ...
  he left direction, it is on the bead under *This approach was declined* — read it
  before you write anything, because it is the only thing separating the next attempt
  from the one he just turned down.
- **Nothing yet.** He has not answered. That is not a state you can do anything about,
  and this session is finished either way.
`;
}

/** The old discovery ending: a comment nobody reads until the queue runs dry. */
const DISCOVERED_SECTION = `
 ...
 */
function proposeSection(workspace, id, pr) {
  return `
**Do not create beads — propose them, and do it when you find them.** Adam approves
...
as a question he can answer from his phone:
...
He gets a row per bead with approve, adjust and decline on each. Nothing is created
until he taps. Propose *while the reason is still on your screen* — a discovery
...
That one parks ${id} behind the question, so nothing reopens this work until he has
answered. Then stop.
`;
}
...
export async function openWorkSession(cfg, workspace, bead, { attempt = 1, doneFile = null } = {}) {
  ...
  return launch(
    cfg,
    dir,
    workPromptFor(workspace.name, bead, attempt, await prMode(cfg, dir)),
...
async function launch(cfg, dir, prompt, tabTitle, { doneFile = null } = {}) {
  ...
  // `withConfig` applies for the console.
  ...
  return new Promise((resolve, reject) => {
    execFile('/usr/bin/osascript', [SCRIPT, command, tabTitle], { timeout: 20000 }, (err, stdout, stderr) => {
      ...
      if (!err) return resolve({ dir, mode: mode || 'default' });
