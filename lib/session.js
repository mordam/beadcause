...
const SUPERSEDE_CMD = `node ${path.join(ROOT, 'bin', 'supersede.js')}`;
...
export function workPromptFor(
  ...
  pr = null,
  owner = ownerName(),
  { autoEndorse = false, notes = null, repo = null, edit = '', debriefs = [], requirements = '' } = {}
) {
  const id = bead.id;
...
${busyBrief}${learned}${past}${requirements}${edit}
 ...
 *     P0 epic and its children, and its door refuses a handoff outright anyway: `/handoff`
 *     files them `--type=task --priority=1` and never claims them, and `wantsAdvocate`
 *     needs a P0 carrying an `owner:` label. Every handoff there has ever been would 409.
 *
 ...
 * re-entrant in exactly the way the P0 advocate is, so its own store is the only thing
 ...
 * marker is a label the queue filter and this door both read; the P0 advocate door is
...
export async function openEpicAdvocateSession(cfg, workspace, bead, { kids = [], plan = null, reason = '', doneFile = null, bd = null } = {}) {
  ...
  // a P0, a crash, nobody owns it — and this one says it has one and somebody switched it
  // off. Refused here rather than only in the daemon so the two doors agree:
  // lib/advocate.js's `reenter` already skips a paused P0, and a button that opened one
  ...
  const opened = await launch(
    ...
    // open, and a P0 advocate handed those notes would be reading somebody else's
    ...
    epicAdvocatePrompt(workspace.name, row, kids, plan, ownerName(cfg), {
      ...
      // Tier 4's read half, and for a root P0 `debriefFamily` narrows to the P0 itself:
    ...
    // agent writes therefore waits for the next archived session on the same P0, which is a
...
/**
 * What the P0 advocate is handed about requirements — the corpus, and its glean backlog.
 *
 ...
 * opening.
 *
 * The backlog is narrowed to this P0's own subtree by the dotted id convention beads
 * already uses (`bc-fvmx.4` is under `bc-fvmx`) plus an explicit parent match, because a
 * P0 advocate is answerable for its own tree and being handed another P0's landed work
 ...
 * *next* window — no more sessions open anywhere under this P0 until somebody unpauses
 ...
 * every worker, planner and P0-advocate brief already reads). Named as a command rather
