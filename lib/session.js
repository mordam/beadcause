...
// The fourth refusal, and the only one that is about this Mac rather than the tracker:
// a bead a live window already names may not get a second one. See lib/onewindow.js.
import { assertNoOtherWindow } from './onewindow.js';
...
import { chainIn, packetFor, profilePath } from './relay.js';
import { relayDefFor } from './relaydefs.js';
...
/**
 * `beadcause` -> `beadcaus[e]`: a pattern that cannot match the command line that carries
 * it, which is the whole of why the check in the brief can be trusted.
 *
 * Without it the answer is always at least two — `grep` finds its own argv in the process
 * table, and so does the shell wrapper around the pipeline — and a window told to stop
 * when it sees two would stop every time. `grep -v grep` is the other idiom and it is
 * wrong *here* specifically: a worker's argv carries its whole memory store, and this
 * repo's store discusses `grep` at length, so filtering on the word drops the real
 * window and keeps nothing.
 *
 * The bracket goes on the *workspace*, never on the bead id: an id can be one character
 * long, and `[b]` alone is a pattern matching `b`.
 */
export const selfExcluding = (word) => {
  const w = String(word || '');
  return w.length > 1 ? `${w.slice(0, -1)}[${w.slice(-1)}]` : w;
};
...
`
  ...
  // Named by the plan but not in this tick's queue at all — bc-ogicx.12. `dispatchable`
  // used to leave these off the brief with no trace, so a P4 member (invisible to the
  // queue on *every* tick, not one — lib/plan.js's `dispatchable` doc) never got told
  // about. `'priority'` is the one kind that is actionable — nothing is in its way, it is
  // only below the advocate's priority floor — so it earns the same "do it" treatment as a
  // sibling; everything else is a bead nobody has cleared, and is named so the window does
  // not go looking for why it is missing, without being told to touch it.
  const absent = group ? (group.absent || []).filter((k) => k && k.id) : [];
  const alsoDo = absent.filter((k) => k.reason === 'priority');
  const waiting = absent.filter((k) => k.reason !== 'priority');
  const ABSENT_WHY = {
    unendorsed: 'not yet endorsed',
    human: 'waiting on a question only Adam can answer',
    closed: 'already closed',
    blocked: 'blocked behind a dependency nobody has cleared',
    ship: 'a ship bead',
    container: 'a container bead',
    unknown: 'the tracker has no row for it',
  };
...
`
  ...
  alsoDo.length
    ? `
**The plan also names ${alsoDo.length} more ${alsoDo.length === 1 ? 'bead' : 'beads'} in this group that this tick's queue skipped over — not because anything
is blocking ${alsoDo.length === 1 ? 'it' : 'them'}, only because ${alsoDo.length === 1 ? 'it is' : 'they are'} below the priority floor the advocate dispatches from. Nothing is in
${alsoDo.length === 1 ? 'its' : 'their'} way, so do ${alsoDo.length === 1 ? 'it' : 'them'} too:**

${alsoDo.map((k) => `    bd show ${k.id}    # ${k.title || '(untitled)'}`).join('\n')}
`
    : ''
}${
  waiting.length
    ? `
**The plan names ${waiting.length} more ${waiting.length === 1 ? 'bead' : 'beads'} in this group that ${waiting.length === 1 ? 'is' : 'are'} not currently ready. Nobody has
cleared ${waiting.length === 1 ? 'it' : 'them'} for you — leave ${waiting.length === 1 ? 'it' : 'them'} alone rather than working ${waiting.length === 1 ? 'it' : 'them'} anyway:**

${waiting.map((k) => `    ${k.id}    # ${k.title || '(untitled)'} — ${ABSENT_WHY[k.reason] || k.reason}`).join('\n')}
`
    : ''
}
...
advocate that opened this window — that the bead is being worked, and it is what the
advocate reads before it opens anything else.

**The claim is not, on its own, what stops a second window — check the process table
before your first edit.** Every window on this Mac claims as the same \`bd\` actor, so a
bead claimed by three windows looks exactly like a bead claimed by one, and on
2026-08-21 three windows ran this very brief for one bead in the same minute, two of
them editing the same file. What can tell them apart is that the first line of this
brief is on each window's own command line from the moment it starts:

    ps -Ao pid=,args= | grep '${selfExcluding(workspace)}/${id}'

Every line that comes back is a live window on this bead and one of them is yours, so two
lines is the bug: another window is running this same brief right now. (The bracket is not
a typo. It is what stops the pattern matching the command you just typed, which would
otherwise answer "2" on a Mac with one window on it.)

**A second window is not yours to kill and its worktree is not yours to write in.** Say
what you found in a comment on the bead, leave your claim on — unclaiming only invites a
fourth window — and stop. If the worktree you were given already existed, \`git status
--short\` twice a minute apart is the confirmation: a file list that grows has a live
writer in it, and a dead attempt's tree is frozen.
...
to happen here.
${noDiffBrief(id, owner, pr)}${delivery}
...
/**
 * The gap bc-khoe.30.8 is about: a bead whose whole deliverable already happened as a
 * tracker write, in a repo where the ordinary ending goes through a pull request.
 *
 * bc-khoe.30.1 is the worked example. Its entire fix was one \`bd update --description\`,
 * finished in about ten minutes — and the rest of that session's window went on working
 * out how it was allowed to *end*, because none of the wording below fit: the work was
 * not blocked, not bigger than it looked, and not a duplicate, and \`bin/deliver.js\`
 * refuses a branch with no commits (its own doc comment says so), so there was no pull
 * request and nothing to park the bead behind. \`lib/owed.js\` is explicit that an
 * unblocked, open work bead is one the advocate opens a second session onto — so
 * commenting "done" and stopping would have bought a second window on work already
 * finished.
 *
 * The session found its own way there: ending 1 below already covers this, because "does
 * this bead need a decision from Adam" is true here too — the decision is just "close
 * it" rather than "which way should I go". What was missing was not a mechanism, it was
 * this paragraph, naming the case *before* a session spends its own time re-deriving that
 * ending 1 applies. Closing the bead outright was considered and rejected: this repo's
 * whole ending ladder rests on "closing a bead is not a worker's call" (see
 * \`deliverSection\`, \`landSection\`, and every comment in lib/superseded.js), and a
 * no-diff session is not a principled exception to that — \`closeSection\` right above
 * already *is* the exception, for the one case where it is warranted: a repo with no
 * GitHub remote, where there was never a pull request for anything to wait on.
 *
 * Gated on \`pr\` — only rendered in the two modes where \`bin/deliver.js\` is the ordinary
 * ending and its no-commits refusal is a real thing a session could run into. A session in
 * \`closeSection\`'s mode already has a same-length ending for a no-diff finish (the exact
 * command above, whatever it did), so adding this there would be a paragraph solving a
 * problem that mode does not have.
 *
 * The decision block is written out in full, not just referenced, because a template
 * localized to this exact question is the whole point: bc-khoe.30.1 also came within one
 * write of shipping an option that *closed the bead while asking for another pass* on it —
 * \`lib/decision.js\` defaults an option with no \`closes:\` to \`closes: true\`, and the
 * general \`askTemplate\` a session would otherwise reach for does not show the field at
 * all. So the non-closing option below sets \`closes: false\` explicitly, in place, rather
 * than leaving a session to discover that rule the same way bc-khoe.30.1 nearly didn't.
 */
function noDiffBrief(id, owner, pr) {
  if (!pr) return '';
  return `
**If the whole of what you did here already happened as a tracker write — a \`bd update\`,
a comment, a label — and there is no code to commit, stop here instead of reading the
section below.** \`bin/deliver.js\` refuses a branch with no commits on purpose: there is
no diff, so there is nothing to open a pull request over, and inventing one to satisfy it
would be work done for the tool rather than for the bead. This is not a fifth kind of
trouble — it is ending 1 further down, because the one thing still undecided is whether
${owner} agrees the bead is finished, and that is exactly the kind of decision ending 1 is
for. A ready-made version of it:

    bd update ${id} --append-notes "$(cat <<'DECISION'
    The change this bead asked for is already made — see the comment above — and there is
    no code left to commit, so \`bin/deliver.js\` has nothing to open a pull request over.

    \`\`\`decision
    question: Close ${id} — is the tracker write above the whole of the fix?
    options:
      - id: close
        label: Close it
        hint: Nothing else is coming from this bead
        response: "Confirmed — closing ${id}."
        recommended: true
      - id: reopen
        label: Not finished — send it back
        hint: Leaves the bead open for another session, with what is still missing
        response: "Not finished — <say what is still missing>."
        closes: false
    \`\`\`
    DECISION
    )"
    bd comment ${id} "Whole deliverable was a tracker write; nothing for deliver.js to push. Decision card asks to close."
    bd label add ${id} human

**Do this instead of guessing at how long to wait, or closing it yourself.** \`bd label add
... human\` is what takes ${id} out of every future advocate tick while the card waits for
a tap — the same guarantee ending 1 already gives, so there is nothing further to protect
here. And closing it yourself is not a shortcut worth taking even though the work is
genuinely done: every other ending in this brief holds that the close is ${owner}'s, and a
session that skips the card because it is confident is the one case that guarantee exists
to catch.
`;
}
...
**The tracker now says how the last rating turned out, and it is the one feedback this
decision has.** A finished session leaves \`ctx:fit\`, \`ctx:tight\` or \`ctx:over\` on its
bead: \`over\` means it ran out of context on the model its tier picked and spent part of
the hour compacting, \`tight\` means it fitted with little to spare. \`bd list --label
ctx:over\` is the list of beads whose tier was wrong, and if the bead you are filing looks
like one of those — same files, same breadth — rate it above what you were about to.
...
5. **Push the branch, and let its own checks be the gate.** A clean merge of two working
   branches is not a working tree — the merge is a third thing neither side tested — so the
   suite has to run over it. That is not the same as *you* having to be the one sitting on
   it. Where the pull request has checks, the push starts them on the merge commit itself,
   and the merge advocate refuses to merge over a red rollup, a pending one, or a head
   commit nothing ran on at all (bc-ysqd.1) — so an unverified push cannot land by being
   ignored, and the verdict reaches ${owner} whether this window is still open or not. Push,
   read \`gh pr checks ${row.number}\` once, and say what it said.

   **Do not queue a local gate behind a verdict something else is already going to catch.**
   Beadcause's own gate serialises across every worktree on this Mac, so a resolver that
   starts one can watch \`3 gates ahead of you\` for an hour while GitHub answers the same
   question in a minute — and running both is the doubled load that message exists to warn
   about. Run the local gate where nothing else will: a workspace with **no** CI wired up is
   a real configuration, and there what the repo's CLAUDE.md names is the only gate there
   is, so run it before you push. Same when a check comes back red on something the diff
   does not explain — the local run is how you find out which suite, and it is worth the
   wait then.
6. **Release the lock** you took in step 2 — \`git worktree unlock .\`, standing in the tree
   — and stop.
...
Then tell me, in one short message: what conflicted, how you resolved it, and what the
checks said — and if you ran the local gate instead of them, which and why. If you could
not resolve it — a conflict where both sides are load-bearing and only ${owner} can say
which wins — say that instead and leave the branch alone. An unmergeable
 ...
 */
export async function resumeWorkSession(cfg, workspace, bead, resume, { bd = null, doneFile = null, ps = null } = {}) {
  ...
  // Last of the refusals and after the tracker's four on purpose: it costs a `ps` and the
  // four above it cost a `bd show` that has already been paid for. A parked conversation
  // whose window somebody re-opened by hand is exactly the case here — the park record
  // says nothing about what is on the Mac now. See lib/onewindow.js.
  await assertNoOtherWindow(workspace.name, row.id, { ps });
...
export async function openWorkSession(cfg, workspace, bead, { attempt = 1, doneFile = null, bd = null, resume = null, ps = null } = {}) {
  ...
  if (resume) return await resumeWorkSession(cfg, workspace, bead, resume, { bd, doneFile, ps });
  const row = assertNotContainer(
    assertNotShipBead(assertStillOpen(assertNotBlockedElsewhere(assertNotSuperseded(await assertEndorsed(bd, workspace, bead)))))
  );
  await assertUnderRoot(bd, workspace, row);
  ...
  //
  // **Off `dir` and not off the workspace**, which is the whole of bc-ogicx.5: the
  // definition is read from the checkout this bead is about (`resolveSessionRepo`, forty
  // lines up), so a multi-repo workspace gets several relays out of it for free — each
  // checkout answers for itself, and neither this function nor lib/relaydefs.js has to
  // know that a workspace can hold more than one. `cfg.relays[<ws>]` is still the answer
  // where a checkout defines nothing, wrapped as the one unnamed relay it has always been,
  // so every workspace on this Mac dispatches to the character it did before.
  //
  // A definition that would not parse or would not validate falls back to exactly that and
  // says so in `problem`. It is never a hold and never a throw: a bead nothing will work is
  // a worse outcome than a bead worked without a relay, and the sentence is the whole
  // repair. Where it is *said* is lib/advocate.js (bc-ogicx.6); all this owes is that the
  // fall-through happens and that the sentence survives on what the launch returns.
  //
  // `packetFor` stays off `cfg` deliberately — lib/relay.js says why at PACKET: a repo file
  // naming `packet:` is refused rather than honoured, so the pair that makes a review
  // answerable from a lock screen cannot be dropped by a branch.
  const { def: relayDef, problem: relayProblem } = relayDefFor(cfg, workspace.name, dir, row);
  const relay = chainIn(relayDef, workspace.name, row, { packet: packetFor(cfg, workspace.name) });
  const opened = await launch(
    cfg,
    dir,
  ...
  //
  // `relayProblem` travels the same way and for the same reason the tier does: it is a fact
  // about *this launch* that nothing downstream can recompute. The definition is read off a
  // checkout that a branch may rewrite a minute later, and re-reading it from the card
  // would be showing what the file says now rather than what this window was opened under.
  // `null` on every launch — which is every launch today, since no checkout ships a
  // `.beadcause/relays.yaml`. A sentence here is never a hold: the window is already open by
  // the time it is returned, and saying it is lib/advocate.js's (bc-ogicx.6).
  //
  // And `dept` beside it, for the sharpest version of the same argument (bc-ogicx.6). The
  // department is derived from the **assignee**, and this window's first act is
  // `bd update --claim`, which overwrites it — so a caller that re-derived the department a
  // minute from now would get `null` for every window that had started work, which is every
  // window a capacity would have to count. It is written down here or it cannot be known at
  // all. `relay.dept` and not `departmentOf` directly: a bead whose only claim on a
  // department is a label, with a person for an assignee, gets no chain and therefore no
  // department, and must not be counted against a ceiling it is not under.
  return {
    ...opened,
    tier,
    repo: repoSummary(repo),
    relayProblem: relayProblem || null,
    dept: relay?.dept || null,
  };
}
...
**Some of those may sit a level or more below ${id}** — a sub-epic's children, or a bead a
child-worker filed under its own bead rather than under the epic. Group them exactly as you
would a direct child: a group may name anything under ${id} **at any depth**, and the plan
tool checks membership against the tracker's parent edges rather than against the shape of
an id. It is worth knowing why that matters, because it used to be the other way round: a
bead ready under this epic that no group names is what re-opens a planner here, so a
grandchild you cannot group is a bead the plan is required to cover and forbidden to
name — which is two wasted planner windows and then one window per bead.
${learned}${past}
## What you produce
 ...
 */
export async function openPlanSession(cfg, workspace, bead, { kids = [], revising = false, doneFile = null, bd = null, ps = null } = {}) {
  const row = assertNotContainer(
    assertNotShipBead(assertStillOpen(assertNotBlockedElsewhere(assertNotSuperseded(await assertEndorsed(bd, workspace, bead)))))
  );
  await assertUnderRoot(bd, workspace, row);
 ...
 */
export async function openEpicAdvocateSession(
  cfg,
  workspace,
  bead,
  { kids = [], tree = [], plan = null, reason = '', doneFile = null, bd = null, deliveryCard = () => null } = {}
) {
  ...
  const opened = await launch(
    ...
    epicAdvocatePrompt(workspace.name, row, kids, plan, ownerName(cfg), {
      reason,
      deliveryCard,
      // bc-khoe.33: what `kids` cannot show. A plan may name a bead under this epic at any
      // depth, so the brief has to be able to show one; `advocatedRoots` already built this
      // beside `kids` off one index, and the door that has no index passes nothing.
      tree,
    ...
    execFile('/usr/bin/osascript', args, { timeout: 20000 }, (err, stdout, stderr) => {
      ...
      if (!err) {
        ...
        return resolve({
          ...
          // The three paths this function has always minted and, until bc-xl7n.113.2,
          // always forgot the moment `osascript` returned. They are what `launchProgress`
          // reads a minute later to answer the one question nothing else here can — did
          // the shell in that window ever get as far as line 3 — and the caller that
          // wants the answer has to write them down, because nothing on this side of the
          // Apple event is still running by then.
          launchFiles: { prompt: promptFile, system: systemFile, command: commandFile },
        });
      }
...
/* ------------------------------------------------ did the window ever start? */

/**
 * The three files a launch minted, read a moment later as the one exact fact about
 * whether the window ever ran a line of its command.
 *
 * There is no process to watch — the window belongs to iTerm — so until this existed the
 * daemon's first evidence that a dispatch had gone nowhere was `workerTimeoutMinutes`,
 * two hours later. Which is a long time to wait for something already written on the
 * disk. `sessionCommand` emits, in order, `P="$(cat <prompt>)" && rm -f <prompt> &&
 * claude … && rm -f <system> <command>` — so the prompt file is gone *before* `claude`
 * starts and the command file is gone *after* it exits. Two files, four states, and the
 * one that matters is exact rather than inferred:
 *
 * | prompt  | command | what happened                                          |
 * |---------|---------|--------------------------------------------------------|
 ...
 * | present | gone    | something else took a file; say nothing                 |
 *
 * The last row is why this is two `existsSync` calls rather than the obvious one. A
 * prompt file with no command file beside it is not a state this daemon can produce, so
 * it is somebody else's — a TMPDIR sweeper, a suite tidying up after a fixture — and the
 * safe answer to a state you cannot explain is `unknown`, which every caller reads as
 * *leave it alone*. Asking only about the prompt file would call that never-started and
 * take a live window's slot away from it.
 *
 * **What it cannot mistake is the two endings a session reaches without exiting.**
 * `delivered` and `handback` both leave the bead open and the TUI on screen, which from
 * every other angle is exactly what a stalled window looks like — and both consumed
 * their prompt file at line 2, an hour before they got there. Nothing here has to know
 * either of them exists, which is the property that makes this safe to run ahead of the
 * inference branches in lib/advocate.js rather than behind them.
 *
 * `null`, `undefined`, and a worker record written before any of this existed all come
 * back `unknown` for one reason: an absent measurement must not read as a failed one. A
 * worker adopted across a daemon restart has no paths on it and is left to the timeout,
 * exactly as it was.
 */
export function launchProgress(launchFiles) {
  const prompt = launchFiles?.prompt;
  const command = launchFiles?.command;
  if (typeof prompt !== 'string' || !prompt || typeof command !== 'string' || !command) return 'unknown';
  const hasPrompt = fs.existsSync(prompt);
  const hasCommand = fs.existsSync(command);
  if (hasPrompt) return hasCommand ? 'never-started' : 'unknown';
  return hasCommand ? 'running' : 'finished';
}

/**
 * Take the three files back, for a launch that will never clean up after itself.
 *
 * Every file here is normally removed by the shell that read it: the prompt at line 2,
 * the other two by the command file's own last line. A window that never ran a line has
 * no such shell, and the three then sit in TMPDIR for as long as the operating system
 * leaves them — 82 of them had accumulated in eighteen hours when this was measured, and
 * counting the ones with their prompt still beside them is how the defect was found at
 * all (bc-xl7n.113.2).
 *
 * The count it returns is for the log line and nothing else. `force` means a file already
 * gone is not an error, and there is nothing for a caller to decide either way.
 */
export function discardLaunchFiles(launchFiles) {
  let taken = 0;
  for (const f of [launchFiles?.prompt, launchFiles?.system, launchFiles?.command]) {
    if (typeof f !== 'string' || !f) continue;
    try {
      if (fs.existsSync(f)) taken += 1;
      fs.rmSync(f, { force: true });
    } catch {
      // A temp file that will not delete is no reason to fail anything: what this sweep is
      // for is the slot and the attempt, and this is the tidying up after them.
    }
  }
  return taken;
}
