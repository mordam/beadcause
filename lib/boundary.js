...
export function gaps(record) {
  ...
  if (named.length && !cuecs(record).length) {
    found.push({
      kind: 'cuec',
      held: '',
      recorded: 0,
      why:
        `${named.map((e) => e.label).join(' and ')} ${named.length === 1 ? 'is' : 'are'} named as user ` +
        'entities and no complementary control has been written for them. A report with user entities ' +
        'and no CUEC list has told them nothing they have to do.',
    });
  }
  return found;
}
