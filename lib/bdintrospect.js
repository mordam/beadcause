...
function callSitesIn(source, fnNode, constMap) {
  ...
  walkPath(fnNode.body, [], (node) => {
    ...
    } else if (argsNode.type === 'Identifier') {
      const decl = findDeclarator(fnNode.body, argsNode.name, argsNode.start);
      if (decl && decl.init && decl.init.type === 'ArrayExpression') {
        ({ argv } = resolveArrayLiteral(source, decl.init));
        pushes = pushesFor(source, fnNode.body, argsNode.name);
        dynamic = true;
      } else {
        argv = [`<${argsNode.name}>`];
        dynamic = true;
        note = `argv is the variable \`${argsNode.name}\` — its declaration could not be resolved statically`;
      }
    } else {
