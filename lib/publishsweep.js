 ...
 *
 * **A transition with no bead is published as `'none'`, not skipped.** The `transition`
 * kind in lib/publishable.js requires all three of its fields, `bead` is one of them, and
 * `setManagement` takes a bead and defaults it to null — a manual `on`/`off` typed at a
 * terminal is not always done for a piece of work, and refusing to publish it would leave
 * the compliance window's own boundary the one thing this chain cannot carry. `'none'` is
 * `TYPES.bead`'s sentinel for exactly that (bc-3muu.21): a fact an auditor can act on — "no
 * bead was named for this transition" — rather than an absent field standing in for it.
 * Every transition with a commit on the ref is owed, whatever its bead; `skipped` is left
 * only for the one thing that is not this file's to fix, a payload longer than its history.
 */
export function transitionsOwed(transitions, commits, published) {
  ...
  for (let i = 0; i < transitions.length; i += 1) {
    ...
    const rawBead = transitions[i]?.bead;
    const bead = typeof rawBead === 'string' && rawBead ? rawBead : 'none';
