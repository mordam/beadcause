 ...
 *
 * **A red-base hold is skipped, and this is bc-beleq.2.** A hold bead (lib/redbase.js) is
 * a P0 with no parent, so it *is* its own root — and before this, a worker filing
 * `--from <the hold>` (following the ordinary "found more work, file it" instruction
 * while working the hold itself) landed the new bead as the hold's direct child. bd
 * refuses to close any bead with open children, and `sweepBase` closes the hold with
 * `overClaim` (which lifts the claim guard, not the children guard) the moment the base
 * is green — so that close is refused forever, over a base that is already fixed, and the
 * merge queue holds every branch in the repo behind a log line nobody is watching. A hold
 * bead's own close is not a person finishing work; it is a sweep lifting a standing
 * condition, and nothing should ever be parented under it. So a step that is a hold is
 * skipped exactly like one that fails `roots.has` — the search keeps climbing past it —
 * and falls out to `null` when (as today) it has no ancestor of its own, which sends
 * `homeFor`'s caller on to the unsorted-backlog root instead.
 */
export function rootOver(index, id) {
  ...
  for (const step of [key, ...ancestorsOf(index?.parents, key)]) {
