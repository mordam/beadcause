 ...
 * decision — no `from`, a `from` under nothing, no unsorted P0 — and one is an accident:
 ...
 * when it is empty, and being in it is a question — *which P0 does this belong to?* — so
 ...
 * the beads that are. Such a caller says so here and gets `''` where the P0 is not
...
export function homeFor(index, { parent = '', from = '', unsorted = true } = {}) {
  ...
  if (overFrom) return { parent: overFrom, why: `${overFrom}, the P0 ${String(from).trim()} belongs to`, gated, error };

  if (!unsorted) return { parent: '', why: '', gated, error };
  const pile = unsortedP0(index);
 ...
 * same: *there are no P0s in this workspace*, where a parentless bead is perfectly
 ...
 * reachable. An empty index is a graph with no P0s in it, which is precisely the
  ...
  if (home.error && !home.parent) {
    onWarn(
      `could not read ${workspace?.name || 'the tracker'} to find a home — ${home.error}. ` +
        'Filing with no parent: if this workspace has an open P0 the bead is held by bc-rfnr.7 ' +
