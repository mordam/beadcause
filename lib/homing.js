...
export function homeFor(index, { parent = '', from = '', unsorted = true } = {}) {
  ...
  if (!unsorted) return { parent: '', why: '', gated };
  const pile = unsortedP0(index);
  if (pile) return { parent: pile, why: `${pile}, the unsorted backlog`, gated };
 ...
 */
export async function homeIn(bd, workspace, { parent = '', from = '', unsorted = true } = {}) {
  const named = String(parent || '').trim();
  if (named) return { parent: named, why: '', gated: false };
  if (typeof bd?.graph !== 'function') return { parent: '', why: '', gated: false };
  try {
    return homeFor(await bd.graph(workspace), { from, unsorted });
  } catch {
