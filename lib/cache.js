 ...
 *
 * **The cold miss is the only wait, and it is bounded.** A key with nothing kept awaits
 * the producer, capped at `CEILING_MS`. The cap is not about the caller — `bd` has its
 * own 120-second ceiling and `gh` has whatever the network gives it — it is about the
 * *slot*: a refresh that never settles must stop holding the single-flight entry, or
 * that key is never refreshed again for the life of the process and the cache quietly
 * becomes a permanent snapshot. An abandoned refresh that lands afterwards is still
 * allowed to write, unless something newer arrived while it was gone.
 *
 ...
 * - **Nothing kept** — awaits the producer, bounded by `ceilingMs`. The only wait there
 *   is, and the whole of what bc-1kwl.4 exists to make rare.
 ...
 */
export async function read(key, producer, { freshMs, now = clock, refresh = false, ceilingMs = CEILING_MS } = {}) {
  ...
  // Cold, or asked for outright. Bounded by the same ceiling the slot has: a producer
  // with none of its own must not be able to park a request forever.
  let timer = null;
  let produced;
  try {
        ...
        timer = setTimeout(
          () =>
            reject(
              ...
              // running and may still write, so a caller is entitled to say "not yet".
              Object.assign(new Error(`${key} did not answer within ${secs(ceilingMs)}`), { cacheTimeout: true, key, ceilingMs })
            ),
          ceilingMs
        );
      }),
