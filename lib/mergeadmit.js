...
export function admittedState(state, { by = '', at = '' } = {}) {
  return {
    attempts: 0,
    downmerges: 0,
    // Beside `downmerges` for the same reason, and it has to be named: the two together
    // are what `MAX_DOWNMERGES` bounds (bc-johbj), so a reset that cleared only one would
    // readmit a pull request whose downmerge budget is still spent on refusals.
    downmergeRefusals: 0,
    downmergeRefusal: '',
    refused: null,
    at: at || null,
