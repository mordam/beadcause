  ...
  return (rows || [])
    ...
    .map(({ row, spec }) => {
      ...
      return {
         ...
         * exclusions plus the read's own, asked of a single row — a bead the queue skips
         * is a bead this has to re-arm, and the two must agree about which those are.
         */
        queued: labelled && mine && !shunned && !state.resolving && state.attempts < MAX_ATTEMPTS,
        labelled,
        assigned: mine,
        shunned,
        /**
