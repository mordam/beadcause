...
export function queueState(issue) {
  ...
  const empty = { attempts: 0, refused: null, at: null, baseline: [], resolving: false };
...
export function mergeBeadBody(d, { tests = '' } = {}) {
  const parts = [
    ...
    '',
    `The MergeAdvocate takes it from here: it brings \`${d.base}\` into \`${d.branch}\`, verifies the ` +
      `checks that ${d.base} is not already failing, merges, and closes **${d.bead}** and this bead ` +
      `together. Nothing is owed by the session that filed it.`,
  ].join('\n');
  ...
  const ran = tests ? `\n\n**What the worker ran:** ${String(tests).trim()}` : '';
