...
import { SEVERITIES } from './reviewadvocate.js';
...
/**
 * How much of a failing check's log the block will hold — bc-xl7n.116.
 *
 * The same two numbers as `FAILURE_LINES` and `FAILURE_LINE_CHARS` in lib/pr.js, declared
 * here rather than imported for `MERGE_ASSIGNEE`'s reason one paragraph up: this module is
 * the leaf that the filer and the advocate both import, and lib/pr.js is the module that
 * spawns `gh`. That the two agree is pinned in test/mergered.mjs, because a block that
 * quietly held less than the excerpt it was written from is a card that names half a
...
/**
 * The refusal sentence as the block will hold it — and therefore as it must be compared.
 *
 * `record` in lib/mergequeue.js decides whether to spend an attempt by comparing this
 * tick's refusal against the one already on the bead, and the one already on the bead has
 * been through *this*: whitespace collapsed, and cut at 400 characters. Comparing a fresh
 * sentence against a stored one that was truncated is a comparison that can never match,
 * and a merge-bead whose refusal is long enough to be cut is one that would sit at
 * `attempts: 1` for ever and never eject to a card. One function so the two cannot drift.
 *
 * 400 because this is prose in a state block a phone renders, and because everything
 * beyond it is a second sentence rather than a different refusal. What that costs is real
 * and worth naming: two refusals that differ only past the cut count as the same one.
 */
export const refusalKey = (s) => String(s || '').replace(/\s+/g, ' ').trim().slice(0, 400);
...
export function queueState(issue) {
  ...
  const downmergeRefusals = Number(raw.downmergeRefusals);
  return {
    ...
    /**
     * How many times GitHub was *asked* to bring the base in and would not — bc-johbj.
     *
     * Apart from `downmerges` above for the same reason `downmerges` is apart from
     * `attempts`: they are different facts and folding them makes a false one. All twelve
     * pull requests held on 2026-08-26 had a refusal counted as a downmerge, so the queue
     * was two ticks away from telling Adam their base had been brought in three times
     * when it had never once gone in. Both are still bounded by `MAX_DOWNMERGES` together
     * — a standing refusal that cost nothing would spend a GitHub write every tick for
     * ever — but only the successes are ever described as downmerges.
     */
    downmergeRefusals: Number.isInteger(downmergeRefusals) && downmergeRefusals > 0 ? downmergeRefusals : 0,
    /** GitHub's own sentence for the last such refusal, so the exhausted line can name it. */
    downmergeRefusal: raw.downmergeRefusal ? String(raw.downmergeRefusal).trim() : '',
    refused: raw.refused ? String(raw.refused).trim() : null,
    /**
     * What the red check actually said — bc-xl7n.116, and `checkFailure` in lib/pr.js.
     *
     * Beside `refused` rather than inside it, and the separation is load-bearing in two
     * directions. `record` in lib/mergequeue.js counts an attempt by comparing the
     * refusal sentence against the one already here, so a branch stuck at one place
     * ejects after three — and an excerpt that a network hiccup dropped for one tick, or
     * that `queueBlock` truncated, would make every tick look like a new refusal and the
     * bead would never eject at all. And `queueBlock` collapses `refused` to a single
     * line, which is exactly what a log tail must not be.
     *
     * A list of lines, for `failureExcerpt`'s reason: it is YAML in a bead's notes, where
     * a list of strings round-trips and a multi-line scalar is something to be wrong
     * about.
     */
    failure: Array.isArray(raw.failure) ? raw.failure.map((s) => String(s)) : [],
    ...
    /**
     * A resolver was opened on this one and it is **gone**, with the branch still
     * conflicting — bc-5mdsw.
     *
     * The other half of `resolving`, and the field exists because those two states are
     * indistinguishable without it. `resolving` is written when a window goes up and the
     * only thing that takes it off is the branch ceasing to conflict, so a conflict a
     * resolver *declined* — the honest ending its brief offers it, where both sides are
     * load-bearing and only Adam can say which wins — reads from in here exactly like one
     * still being worked on. `MAX_ATTEMPTS` counts refusals and a resolver spends none, so
     * the eject to a card was unreachable down that path and the queue simply opened
     * another window, on byte-identical state, for as long as the branch existed. Nine of
     * them on #488.
     *
     * So this is the queue having been told *that conflict is not going to resolve
     * itself*: it stops the door opening again, and every tick that finds the branch still
     * conflicting spends an attempt instead, which is what carries it to the card the
     * three attempts were always for. It is taken back by the first read that says the
     * branch is no longer conflicting, on `held`'s reasoning — a sentence the queue wrote
     * about somebody else's window has to be one the queue can withdraw.
     */
...
export function queueBlock(state) {
  ...
  if (state?.downmergeRefusals) out.downmergeRefusals = Number(state.downmergeRefusals);
  if (state?.downmergeRefusal) out.downmergeRefusal = String(state.downmergeRefusal).replace(/\s+/g, ' ').slice(0, 200);
  if (state?.refused) out.refused = refusalKey(state.refused);
  // Lines, not a paragraph: whatever survives here has to survive a YAML round trip, and
  // the caps are `failureExcerpt`'s own so a block written from an excerpt reads back as
  // the same excerpt.
  if (state?.failure?.length) {
    out.failure = state.failure
      .slice(0, FAILURE_LINES)
      .map((s) => String(s).replace(/\s+$/, '').slice(0, FAILURE_LINE_CHARS))
      .filter(Boolean);
  }
  ...
  if (state?.declined) out.declined = true;
 ...
 *
 * `severity` and `why` exist because a verdict comment (lib/reviewadvocate.js's
 * `checkVerdict`) carries both and this block used to drop them on the floor: a comment
 * folded in here read as `- **id** \`path:line\` — body` with nothing saying whether
 * declining it costs a round or costs nothing. Validated against the same `SEVERITIES`
 * `checkVerdict` uses, and the same defensive direction as everywhere else in this
 * function — an unrecognised or absent severity lands as `''`, a *defined* "unknown",
 * never the raw value trusted through and never grounds to drop the comment.
 *
 * `threadId` is GitHub's own review-thread id (`reviewThreads`/`resolveThread` in
 * lib/pr.js) — never this block's own `id`, which is beadcause's bookkeeping and means
 * nothing to GitHub. It has no caller yet; it exists so that whatever folds a verdict
 * into this block can anchor a comment to the thread `resolveThread` would close, rather
 * than inventing a second field later for the same purpose.
 */
function reviewComment(raw, i = 0) {
  ...
  const severity = SEVERITIES.includes(String(src.severity || '').trim().toLowerCase())
    ? String(src.severity).trim().toLowerCase()
    : '';
  return {
    ...
    body,
    /** blocking / suggestion / question, or '' when the comment did not carry a recognised one. */
    severity,
    /** One sentence, when the reviewer gave one — why the comment matters, not what it says. */
    why: oneLine(src.why, 400),
    answer,
    ...
    /** GitHub's own review-thread id, when this comment has been anchored to one. */
    threadId: oneLine(src.threadId, 200),
  };
}
...
export function reviewBlock(state) {
  ...
  if (comments.length) {
    ...
    out.comments = comments.map((c) => {
      ...
      if (c.threadId) row.threadId = c.threadId;
      return row;
    });
  }
  if (verdict === 'approved') {
