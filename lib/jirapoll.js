 ...
 *
 * `assignee = "<email>" AND resolution = EMPTY AND statusCategory = indeterminate`,
 * scoped to the workspace's projects when it has them, newest first. That is the whole
 * of what this epic asks JIRA, and the JQL
 ...
 *
 * **And only what is in progress (bc-6s383).** A site need not set a resolution at all,
 * and climative's `5: Done` does not: of the 281 TECH tickets the resolution clause alone
 * returned on 2026-09-11, 169 were finished. Of the 112 left, 81 sat in the *To Do*
 * bucket — New, backlog, Blocked, Shelved — and every ticket that arrives becomes a P1
 * epic, so Adam's answer was to take only the 31 being worked: `statusCategory =
 * indeterminate`. The category is not a status name — it is one of three fixed buckets
 * every status belongs to, which no site can rename or add to — so it keeps the property
 * the paragraph above is about. `resolution = EMPTY` stays beside it as a belt: a status
 * in progress with a resolution set is a workflow somebody has bent, not work.
 *
 * What that means for a ticket that *leaves* the bucket is the resolved sweep's
 * business, and it splits the right way on its own: to Done, and `resolutionOf` (see
 * `DONE_CATEGORY` in lib/jira.js) calls it resolved and its held epic closes; back to To
 * Do or Blocked, and it is not resolved, so its epic is left exactly as it is — bc-uz6e's
 * answer for a ticket that stops arriving — and found again by ref if it comes back.
 *
 ...
 *
 * **A workspace switched off costs exactly nothing** — not a network call, not a `bd`
 * spawn, not a cache entry. **A workspace with no `jira` block costs one `bd config get
 * jira.url` per ten minutes and no network call**, because no block is *auto* (bc-6s383,
 * lib/jira.js): on exactly when its bd already points at a JIRA site. That used to be
 * nothing as well, and the price of nothing was a JIRA-wired tracker whose assigned
 * tickets never arrived because a switch nobody knew about was never flipped. Both are
 * checked in test/jira-poll.mjs rather than left as an intention, because most workspaces
 * on any machine are bd-only, and a poller whose per-workspace cost drifts upwards is one
 * whose cost is the size of somebody's `~/beads`.
 *
 ...
 * for. That includes a workspace on *auto* whose bd names a site but which has no token
 * file yet. A workspace that is switched **off** is silent, because that is what off means.
 *
 * Deliberately not persisted, for lib/sweep.js's reason: a restart has no last-good
 * answer for anybody and should ask for one.
 */
import { check, jiraSwitch, search, settingsFor } from './jira.js';
...
/**
 * The one status category a ticket has to be in to arrive — JIRA's own key for *In
 * Progress*. See "and only what is in progress" in the header.
 *
 * The key rather than the display name: `indeterminate`, `new` and `done` are fixed,
 * where "In Progress" is the English label of the same bucket and a site in another
 * language answers to another word. Checked against climative.atlassian.net on
 * 2026-09-11 — the key, the name and the id (4) all answer the same 32 tickets.
 */
...
export function assignedJql(settings, { currentUser = false } = {}) {
  ...
  return `assignee = ${who} AND resolution = EMPTY AND statusCategory = ${IN_PROGRESS}${scope} ORDER BY updated DESC`;
}
...
export function createJiraPoller({ bd = null, fetchImpl = undefined } = {}) {
  ...
  /** Stop holding anything for a workspace that is off — its rows and its trouble. */
  const goOff = (name) => {
    latest.delete(name);
    record.ok(name, []);
    return null;
  ...
  async function pollOne(cfg, workspace) {
    ...
    const off = { workspace: name, state: 'off', tickets: [], error: null };
    if (jiraSwitch(cfg, name) === 'off') return off;
    ...
    // Auto, and its bd points at no site: off, exactly as if it had been switched off.
  ...
  return {
    ...
    /**
     * One workspace's resolved settings, off the same ten-minute memo the sweep uses.
     *
     * For the caller that has to make a *second* kind of read against the same site:
     * lib/jiraresolved.js asks a vanished ticket by key whether it was resolved, and
     * `settingsFor` is three `bd config get` spawns (see the header). Calling it for
     * itself would pay all three again, minutes after this poller paid them, to learn
     * the same URL — so the memo is shared rather than the resolution duplicated, and
     * `invalidate` goes on being the one thing that drops it.
     */
    settings(cfg, workspace) {
      return settingsFor(memoBd(Date.now()), workspace, cfg);
    },
     ...
     *
     * The filter is what makes a workspace switched **off** free, and it is deliberately
     * the *first* thing: `jiraSwitch` reads the config in memory. A workspace left on
     * auto (bc-6s383 — see lib/jira.js) costs one `bd config get jira.url` per ten
     * minutes, through `memoBd`, and stops there when its bd points at no site.
     */
    async sweep(cfg, workspaces = cfg?.workspaces || []) {
      const on = (workspaces || []).filter((w) => jiraSwitch(cfg, w?.name || '') !== 'off');
      ...
      // could make, and there is no read to stand in for any more. Nor its trouble: off
      // is not a failure, and a warning about a switched-off JIRA would never clear.
      const live = new Set(on.map((w) => w.name));
      for (const name of [...latest.keys()]) if (!live.has(name)) goOff(name);
      ...
      const results = await Promise.all(
        due.map(async (w) => {
          try {
            ...
            // Auto with no site: not swept, and whatever it held goes, as above.
            if (out.state === 'off') return goOff(w.name);
      ...
      const swept = results.filter(Boolean);
      return { results: swept, changed: swept.filter((r) => r.changed), skipped, tickets: this.tickets() };
    },
  };
}
