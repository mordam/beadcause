 ...
 * (The fourth shape, a page that only exists once its JavaScript has run, is
 * lib/browse.js — it borrows `vetUrl` from here so both reach the same conclusions
 * about what an agent may point at.)
 *
 ...
 */
export const lookupBrief = (owner = ownerName()) => `**You can look things up outside this checkout.** Four shapes, smallest first:
    ...
    beadcause-browse <url>          a page that only renders under JavaScript
...
\`beadcause-browse\` is the last resort and costs a browser start-up, so use it only when
the others came back with a **shell**: an empty container, a "loading…", a table that is
not in the HTML. It renders the page in a throwaway headless Chrome with no cookies, no
history, no extensions and no identity, deleted when the run ends — so it is not the
browser on this machine and it acts as nobody. There is no way to click, type, submit,
log in or run JavaScript through it; it reads a page. If what you want is behind a
login, that is the answer.
