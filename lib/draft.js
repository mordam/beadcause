...
/**
 * The `beads` block — what the console agent proposes, before anything is created.
 *
 ...
 * one-shot question you approve or don't, while a console draft is edited on screen
 ...
 *
 * beads has no "draft issue" concept, and the whole point of the console is that
...
function normalizeBead(raw, index, taken) {
  ...
  return {
    ...
    design: str(o.design),
    notes: str(o.notes),
    labels: asList(o.labels ?? o.label).map((l) => slug(l)).filter(Boolean),
