...
import { normalizeSurface } from './beadfiles.js';
import { daemonOnly } from './proposedlabels.js';
 ...
 *       labels: [api]
 *       files: [lib/dms.js, lib/retry.js]   # what it expects to touch
 *     - ref: retry-tests
 *       title: Cover the retry policy
 *       dependsOn: [retry-client]    # a ref above, or a real id like cl-1jw
 *       parent: retry-client
 *       files: [test/retry.mjs]
 *   ```
 *
 * `files` is the one field here that says something about the *other* cards. Two beads in
 * one proposal that name one file are two windows that must never open at once, and this
 * is the earliest moment anybody can see that and split them — bc-42ow. It is not stored
 * as a field on the created bead, because bd has none: it is written into the bead's
 * description as the fenced block lib/beadfiles.js reads back, by
 * `POST /api/console/create` in lib/server.js. Nothing here refuses a card over it, and a
 * card that names nothing files a bead that declares nothing, which is every bead filed
 * before this existed.
 *
...
/**
 * Which of a bead's labels `bd create` would drop, and why — the same question
 * `filterProposedLabels` (lib/proposedlabels.js) answers at create time, asked here
 * instead so the card can say so *before* the button is pressed. Keyed by the label
 * string itself, so the editor can look one up without re-deriving it, and only
 * present for a label that has an answer — an unflagged label costs the renderer
 * nothing to check. bc-xl7n.87: this is a second look at exactly what
 * `filterProposedLabels` will do, not a rule of its own, so the two can never
 * disagree about which labels are the daemon's alone.
 */
const labelIssues = (labels) => {
  const out = {};
...
function normalizeBead(raw, index, taken) {
  ...
  const labels = labelList(o.labels ?? o.label);
  ...
  return {
    ...
    notes: str(o.notes),
    labels,
    // See `labelIssues` above — a label the create would drop, with the reason, so
    // the editor can mark it before "Create N beads" rather than only warning after.
    labelIssues: labelIssues(labels),
    // What this bead expects to touch, as lib/beadfiles.js will spell it — normalised
    // here rather than at filing time so the card shows the path that will actually be
    // written, and so two cards that spelled one file `./lib/a.js` and `lib/a.js` read as
    // the collision they are. Four spellings of the key because the text is generated and
    // the cost of an unrecognised one is a surface that silently is not there.
    files: normalizeSurface(o.files ?? o.touches ?? o.paths ?? o.surface),
...
export function draftToYaml(draft) {
  const beads = (draft?.beads || []).map((b) => {
    ...
    // Carried back to the agent for the same reason every other field is: it argues with
    // what is on your screen, and a surface you corrected on the card that the agent
    // never sees is one it will confidently propose again next turn.
    if (b.files?.length) o.files = b.files;
