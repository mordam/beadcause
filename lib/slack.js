...
/**
 * A token file anybody on this Mac can read — said out loud, once, at startup.
 *
 * The construction protects these files from the *history*: `.key` is ignored by the
 * config repo and refused by its commit guard, so a token in the default place cannot be
 * committed. It says nothing at all about the filesystem, and "the token is in a 0600
 * file" is only true if whoever created it made it one — which is a sentence in a README
 * and therefore a promise nobody has checked.
 *
 * So it is checked. A warning rather than a refusal, because turning Slack off over a
 * permission bit would be a worse outcome than the bit; and a warning rather than a
 * `chmod`, because silently changing the mode of somebody's file is more surprising than
 * telling them about it. Group and other bits only — an owner-readable file is the point.
 */
export function slackTokenWarnings(cfg = {}) {
  if (!cfg.slack?.enabled) return [];
  const out = [];
  for (const [what, file] of [
    ['bot token', botTokenFile(cfg)],
    ['app token', appTokenFile(cfg)],
  ]) {
    let mode;
    try {
      mode = fs.statSync(file).mode & 0o777;
    } catch {
      // Absent is not a permissions problem, and `slackStatusLine` already says which
      // of the two is missing and what it costs.
      continue;
    }
    if (mode & 0o077) {
      out.push(`${file} is mode ${mode.toString(8).padStart(3, '0')} — the ${what} is readable by other accounts on this Mac. \`chmod 600\` it.`);
    }
  }
  return out;
}
...
export function optionsFor(q, max) {
  return (q.decision?.options || []).slice(0, Math.max(1, max || 5)).map((o) => ({
    ...
    // Recorded for the same reason `closes` is, and read by nothing here either: the
    // registry is what the button *offered*, and the bead's block may have changed since
    // the message went up. What an answer does is decided in `/api/respond` off the id.
    defers: o.defers === true,
  }));
}
