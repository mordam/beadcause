 ...
 * the truth.
 *
 * **But only when the exit is actually ours to take — bc-fh0sz.** The first version of
 * that re-raise did `process.removeAllListeners(sig)` first, which is a much larger claim
 * than it looks: every *other* handler the process registered goes with it, and the
 * re-raise then meets node's default disposition, which is death on the spot. In a check
 * script that is exactly right, because there is nobody else. In the daemon it is a bug
 * with a name. `bin/beadcause.js`, `bin/router.js` and `bin/monitor.js` all register a
 * `shutdown` at load; the router's ends with `setTimeout(() => process.exit(0), 300)`, on
 * purpose, so `SIGTERM` lands on the backends before the router's own exit orphans them.
 * These handlers wire *lazily*, on the first `onExit` — which in the daemon is the first
 * browse — so they are registered second and run second, and the old line then killed the
 * process out from under a shutdown that had already started. Measured: shutdown started,
 * profile removed, exit 143, grace never elapsed. The daemon is signalled on every
 * restart, deploy and `launchctl kickstart`, so it was not a rare shape — and `wired`
 * latches, so the listeners outlive the browse that attached them and every later restart
 * lost the grace too, with nothing browsing at all.
 *
 * So the handler removes **only itself**, and re-raises only if
 * `process.listenerCount(sig) === 0` — if somebody else is still listening for that
 * signal, the exit is theirs to decide and this file has no business taking it. Nothing
 * changes for the process this was written for: a check registers no handler of its own,
 * so the count is zero, the re-raise happens, and it still dies of the signal it was sent.
 * The one thing given up is the process whose *other* handler ignores the signal entirely
 * — that one now goes on running, which is a bug in that handler and is what `SIGKILL`
 * and lib/strays.js are behind it for.
 *
...
function wire() {
  ...
  for (const sig of SIGNALS) {
    const handler = () => {
      drain();
      // Only ours. `removeAllListeners` here is what defeated the daemon's own graceful
      // shutdown — see the header, bc-fh0sz.
      process.removeListener(sig, handler);
      // And re-raise only if that leaves nobody. Somebody else listening for this signal
      // is somebody else's exit to take, and taking it for them is what the bug was.
