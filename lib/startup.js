...
/**
 * Which kind of failure a child that *exited before it was healthy* is.
 *
 ...
 */
export function exitKind(code) {
  return code === PORT_TAKEN_EXIT ? PORTLOST : EXITED;
}
 ...
 * and a lost port is about everything else running on it, and condemning a build for
 * either is how a good build stops being served for as long as it takes to notice.
 */
...
export function explain(snap, now = Date.now()) {
  ...
  if (deferred) {
    return {
      ok: false,
      code: 'retrying',
      summary: deferred.why
        ? `serving build ${active.build}; build ${disk} could not get a port and is being retried`
        : `serving build ${active.build}; build ${disk} was too slow to start and is being retried`,
      lines: [
