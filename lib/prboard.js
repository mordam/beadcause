 ...
 * act is gated and logged, and everything after that is a session on the Mac with a
 * human's name on it — see `openShipSession` in lib/session.js.
 */
...
async function prefixFor(bd, ws) {
 ...
 *
 * The tiers are the whole point, and the reason for them is a real row: a delivery
 * whose body ended "nothing was done about bc-2tr / bc-es8 / bc-dmt, which this
 * unblocks" came back linked to four beads, three of which it explicitly had not
 * touched. All four exist, so verifying against the tracker cannot separate them —
 * only *where they were written* can.
 *
 * So: a bead named in the `beadpr` block, in the title, or by the branch's own tag is
 * a bead this PR is **for**. Anything else in the body is a bead it *mentions*, and
 * mentions are used only when nothing stronger is there — which is the hand-opened
 * PR whose body says "fixes bc-x" and nothing else does.
 */
function candidateTiers(row, prefix) {
  if (!prefix) return [];
  const re = new RegExp(`\\b${prefix}-[a-z0-9]{2,10}\\b`, 'gi');
 ...
 *
 * `seen` is the sweep's own memo. Half a dozen PRs mentioning the same bead is normal
 ...
 */
async function beadsFor(bd, ws, prefix, row, seen) {
...
/**
 * Where a pull request has actually got to, as one word.
 *
 * Deliberately a ladder rather than three independent flags on the wire as well as in
 * the data: the flags are all still there, but the *stage* is what a phone sorts and
 * colours by, and computing it in two places is how the list and the card start
 * disagreeing about the same PR.
 */
function stageOf(row) {
  if (row.state === 'MERGED') {
    if (row.deployed === true) return 'deployed';
    if (row.pushed === true) return 'pushed';
    return 'merged';
  }
...
function noteFor(row) {
  ...
  if (!row.deployTracked) return 'Merged and pushed. This repo has no deploy this daemon can see.';
 ...
 */
async function forWorkspace(bd, cfg, ws, { boot, seen, force }) {
  const base = cfg.pr?.base || 'main';
  const card = { workspace: ws.name, repo: null, dir: null, base, prs: [], error: null, deployTracked: false };
  ...
  card.prs = await Promise.all(
    rows.map(async (row) => {
      ...
      out.stage = stageOf(out);
      out.note = noteFor(out);
      return out;
    })
  );

  // What you have to act on, first: open PRs, then merged work that has not landed
  // where it needs to, then everything already done.
  const rank = { open: 0, merged: 1, pushed: 2, deployed: 3, closed: 4 };
  card.prs.sort(
 ...
 */
export async function collectBoard(bd, cfg, { force = false, boot: override } = {}) {
  ...
  const seen = new Map();
  const repos = await Promise.all((cfg.workspaces || []).map((ws) => forWorkspace(bd, cfg, ws, { boot, seen, force })));
  ...
  const weight = (c) => c.prs.filter((p) => p.stage === 'open' || p.stage === 'merged' || p.stage === 'pushed').length;
...
const empty = () => ({ open: 0, merged: 0, pushed: 0, deployed: 0, closed: 0, owed: 0 });

/**
 * The numbers on the tab.
 *
 * `owed` is the one that matters and the one this whole view was built to make
 * visible: work that is merged and is not running anywhere. Open PRs are counted
 * beside it rather than folded into it — an open PR is a decision, a merged one that
 * has not shipped is a chore, and a single badge over both would say neither.
 */
