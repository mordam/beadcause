...
/**
 * One checkout's `gh` answers, reused for `ROWS_MS`.
 *
 ...
 * behind that has to be swept.
 *
 * A failure is **not** cached. A `gh` that could not reach the network, a checkout mid-move,
 * a token that expired: caching any of those would put an error on a card and hold it there
 * for two minutes after the thing had healed, and the sentence a card shows for a failure is
 * one nobody can act on except by retrying.
 */
const ghRows = new Map();

async function rowsFor(dir, force) {
  const hit = ghRows.get(dir);
  if (!force && hit && Date.now() - hit.at < ROWS_MS) return hit.value;
  ...
  const value = { slug, rows: slug ? recent(await pr.list(dir, { limit: QUERY_LIMIT })) : [] };
  ghRows.set(dir, { at: Date.now(), value });
  return value;
}

/** Drop the per-repo `gh` cache. For a test, and for anything that knows `gh` just changed. */
export const forgetRows = () => ghRows.clear();
...
/**
 * What is dirty, short enough to sit in a bead comment.
 *
 * `git status --porcelain` lines are `XY path`, and a rename is `XY old -> new`, where
 * the new name is the one worth printing.
 *
 * Whether *all* of it is untracked is the fact a reader acts on, so it is said outright.
 * Untracked means nobody's unsaved edits are at stake, which is the whole difference
 * between "leave it, that is Adam mid-edit" and "that is residue, and clearing it
 * unblocks every delivery on this laptop".
 */
function describeDirt(porcelain) {
  const lines = porcelain.split('\n').filter((l) => l.trim());
  const paths = lines.map((l) => l.slice(2).trim().split(' -> ').pop());
  const more = paths.length - DIRT_SHOWN;
  const list = paths.slice(0, DIRT_SHOWN).join(', ') + (more > 0 ? ` and ${more} more` : '');
  return lines.every((l) => l.startsWith('??')) ? `${list} (all untracked)` : list;
}
...
export async function landLocally(dir, base = 'main') {
  ...
  // is silent about *what*, one stray path holds up **every** delivery on this Mac until
  // a person happens to look. That is bc-s7fs: a `.beads/` left behind by a reverted
  // `bd init` held the fast-forward for a day and 114 commits, and each session that hit
  // it paid to rediscover why. The refusal itself is unchanged and must stay that way;
  // what changes is that it is now one line to act on.
