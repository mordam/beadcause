...
/**
 * The board exactly as it is kept right now — never a sweep, `null` when there is none.
 *
 * For a reader that wants the pull requests **if somebody has already paid for them** and
 * must not be the one who does. `collectBoard` is the wrong door for that: a cold key
 * turns a page load into `gh` over every configured repo, which is the most expensive
 * thing in the app, and the caller here (lib/docket.js, opened from a card) has a budget
 * of a page load.
 *
 * **`null` is a load-bearing answer, not an empty board.** It says nobody has swept, and
 * the difference matters wherever the page would otherwise write "no pull request for
 * this work": over a `null` that sentence is a claim about a question never asked.
 * public/app.js's `p0PrsFor` keeps the same distinction against the client's own copy,
 * and for the same reason.
 */
export const peekBoard = () => cache.peek(BOARD_KEY)?.value || null;
...
export async function collectBoard(
  bd,
  cfg,
  { force = false, boot: override, deploys: journal, ceilingMs = cache.CEILING_MS, waitMs = ceilingMs } = {}
) {
  ...
  let got;
  try {
    got = await cache.read(BOARD_KEY, () => sweepBoard(bd, cfg, { force, override, journal }), {
      freshMs: CACHE_MS,
      refresh: force,
      // The slot's own ceiling — how long the sweep may hold the single-flight entry.
      // The suite shrinks it because the real ceiling is 150 seconds and a check that
      // waited it out would be the slowest file in the repo; the daemon never passes it,
      // so a genuinely cold key still gives the sweep the full 150 to land in.
      ceilingMs,
      // How long THIS call is willing to wait on it — bc-19vt.1. `/api/prs` and
      // `/api/queues` (lib/server.js) pass `cache.WAIT_MS` here: a phone gains nothing
      // from inheriting the slot's whole 150 seconds, because the sweep it started
      // keeps running either way and this function already has somewhere honest to
      // land a "not yet" — see the catch below. An acting call (Ship) does not pass
      // it, because it needs the real answer rather than a fast one, and defaulting to
      // `ceilingMs` is what keeps it waiting the full ceiling exactly as before.
      waitMs,
    });
  } catch (err) {
    // **A cold sweep that ran out of ceiling is `unavailable`, not a throw — bc-19vt.**
    //
    // The only reader that ever waits is one arriving on a key with nothing kept (see
    // lib/cache.js), and on a busy Mac that wait can reach the 150-second ceiling: 82
    // child processes and 2,748,679ms of `bd` work behind one board sweep, measured on
    // 2026-08-23. The throw reached the route's catch-all as a bare error, so `/api/prs`
    // and `/api/queues` answered **HTTP 500** — and public/report.js is explicit that 5xx
    // means *the daemon is failing*, so the phone filed a P0 incident bead about a daemon
    // that was working perfectly well and merely busy.
    //
    // `unavailable` is the shape this already has for exactly this question. A missing
    // `gh` takes it, every caller already handles it — the routes draw the sentence, the
    // acting calls refuse with a 409 — and it is the honest answer here too: the board
    // could not be read, and it says why rather than pretending it is empty. The sweep
    // itself was not cancelled and is still out there, so the wording promises the next
    // read rather than an error, and that is usually what happens: the abandoned producer
    // lands into the keep and the next poll is a warm hit.
    //
    // Only the *ceiling* converts. A producer that threw for its own reasons is a real
    // failure and keeps its 500 — which is why lib/cache.js flags this one rather than
    // leaving callers to match on its message.
    if (!cache.timedOut(err)) throw err;
    console.error(`[board] ${err.message} — answering as unavailable; the sweep is still running`);
    // `err.waitMs` — how long THIS call waited, not `err.ceilingMs` — the slot's own
    // 150 seconds, which this call may not have waited out at all. See bc-19vt.1.
    const why =
      `the pull request sweep did not finish within ${Math.round(err.waitMs / 1000)}s — ` +
    ...
    // for this key, so there is no age to put on the wire. The routes skip the header.
    return { ...(await blankBoard(why, override)), kept: null };
  }
  return { ...got.value, kept: cache.combine([got]) };
}

/** The board nothing could be read into, and the one sentence saying why. */
async function blankBoard(reason, override) {
  ...
  return { unavailable: reason, repos: [], build: boot, counts: empty(), at: new Date().toISOString() };
}
...
async function sweepBoard(bd, cfg, { force, override, journal }) {
  ...
  if (!gh.ok) {
    ...
    return blankBoard(gh.reason, override);
  }

  // The daemon's own commit — see `blankBoard`, which resolves it the same way.
  const boot = override === undefined ? await BOOT : override;
  const seen = new Map();
