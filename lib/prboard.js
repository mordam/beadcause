 ...
 */
export async function collectBoard(bd, cfg, { force = false, boot: override, deploys: journal, ceilingMs = cache.CEILING_MS } = {}) {
  ...
  try {
    got = await cache.read(BOARD_KEY, () => sweepBoard(bd, cfg, { force, override, journal }), {
      freshMs: CACHE_MS,
      refresh: force,
      // The daemon never passes this; the suite does, because the real ceiling is 150
    ...
    const why =
      `the pull request sweep did not finish within ${Math.round(err.ceilingMs / 1000)}s — ` +
