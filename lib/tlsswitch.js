...
export function pairing(cfg, url = cfg.baseUrl) {
  ...
  // `appRefuses` is decided here rather than by the screen: which http addresses the
  // APK still accepts is lib/tls.js's list to keep in step with `Address.LOOPBACK`, and
  // a second copy of that rule in a browser is a copy that goes stale unwatched.
  return { url: link, origin: originOf(url), qr: qrSvg(link), appRefuses: Boolean(cleartextWarning(url)) };
}
...
export function tlsView(cfg, { live = null, withPairing = false } = {}) {
  ...
  return {
    ...
    //
    // Named fields rather than a spread of `live`, and `checkedAt` had to be added by
    // hand for that reason: whatever answered — the router's snapshot or this process's
    // own listener — is free to carry more than a screen should be shown, and a spread
    // would put every future field of it on the wire the day it was added. The cost is
    // this line, and the field it is worth paying for is `checkedAt`: when the renewal
    // loop last looked. Without it a card can say "89 days left" about a loop that died
    // six weeks ago, because the calendar and the loop are two different facts and
    // neither one alone says the other is fine.
    serving:
      live === null
        ? null
        : { tls: Boolean(live.tls), name: live.name || null, daysLeft: live.daysLeft ?? null, checkedAt: live.checkedAt ?? null },
