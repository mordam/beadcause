 ...
 *
 * **Nothing here rebinds a socket.** TLS is decided when the listener is created, by
 * whichever process owns the port — bin/router.js in the installed configuration. So
 * this writes the setting, fetches the certificate, moves the URL, and then says
 * plainly that a restart is what makes it true. Claiming otherwise would be the one
 * lie the screen could tell that you would not find out about until the phone would
 * not load.
 *
