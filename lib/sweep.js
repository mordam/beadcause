...
const oneLine = (err) => {
  ...
  // the reason starts. `timed out in <ws>:` is the same prefix wearing the other verb
  // (see `BD_TIMEOUT` in lib/bd.js): a sweep that was merely slow must reach this screen
  // saying so rather than as a wall of argv, since "still running when we killed it" and
  // "this repo is broken" are the two things a person most needs told apart here.
  const colon = first.indexOf(': ');
  const trimmed = colon > 0 && /(failed|timed out) in /.test(first.slice(0, colon)) ? first.slice(colon + 2) : first;
  if (trimmed.length <= 200) return trimmed || 'unknown error';
  // Two hundred is a phone's line, and that budget is unchanged — what moved is *where*
  // the characters come from. Cutting the tail loses the end of the message, and the end
  // is where a path is: `no JIRA credential for climative — put an API token in
  // <path>/config/jira-climative.key (it is 0600 …)` is 213 characters before anybody's
  // directory is unusually deep, and every character over the cap comes off the file name
  // it exists to say. It survived on this Mac only because the overflow happened to land
  // in the parenthetical after the path; a suite `$TMPDIR` (bc-5isv), a checkout on an
  // external disk, or one more directory of nesting all move the cut into the name itself
  // and the message keeps its width while losing its point. Elided in the middle instead:
  // the head still says which failure this is, and the tail still names the file.
  return `${trimmed.slice(0, 120)}…${trimmed.slice(-77)}`;
};
