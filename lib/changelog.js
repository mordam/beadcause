 ...
 */
function decisionFingerprint(body) {
  const m = /\*\*Decision:\*\*\s*([\s\S]*?)(?:\n\*\*|$)/.exec(body);
  return (m ? m[1] : body).replace(/\s+/g, ' ').trim();
}

/**
 * One `**Field:**` line's own text, e.g. `fieldOf(body, 'Status')` for `**Status:**
 * [PROPAGATED]`. Single-line only, like every field this template writes except
 * `Decision:` (which gets its own multi-line-aware `decisionFingerprint` above).
 */
 ...
 * row's prose routinely wraps and a nested sub-bullet of the same row is part of what
 * that row claims, not a claim of its own — only a line starting `- [ ]`/`- [x]` opens
 * a new row. `bodyLine` is the row's own 0-indexed line number *within `body`* — the
 * caller adds a heading's own file line number to turn it into a real one; see
 * `entryDetail` below, the only place that needs to.
 */
export function checklistRows(body) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (m) {
      current = { checked: m[1].toLowerCase() === 'x', text: m[2], bodyLine: i };
      rows.push(current);
      continue;
    }
 ...
 * Two shapes cover every checkable row `bin/b7e-propagated`'s own examples show: a
 * backtick-quoted arrow (`` `OLD` → `NEW` ``, ASCII `->` too) or a parenthetical "was
 * OLD" paired with "now NEW" elsewhere in the row. Anything else — prose describing the
 * change without quoting either side — returns null.
 */
...
/** The 1-indexed line number of `charIndex` within `text`. */
...
/**
 * One entry's own line span, fields and checklist, with **real file line numbers** —
 * the thing `bc-dgx7.100`'s seven sessions each re-derived by hand with `grep`/`sed`/
 * `awk` guesses at a line range, sometimes landing on the wrong entry (see the module
 * doc). Returns `null` for a `wanted` that names no entry — `bin/b7e-changelog` turns
 * that into exit 5, the same code `bin/b7e-propagated` already uses for the same case.
 *
 * `startLine`/`endLine` bound the entry's own body: the heading line through its last
 * non-blank line, before whatever gap of blank lines separates it from the next
 * heading (or EOF) — that gap belongs to neither entry, so it is never counted as an
 * overlap. Because entries are read in **file order**, not by doing arithmetic on
 * their numbers, a missing number (`entryHeadings` never invents one) costs nothing:
 * entry `117` ends wherever the next heading actually is, `115` here, not at a `116`
 * that was guessed into existence.
 */
export function entryDetail(text, wanted) {
  const headings = entryHeadings(text);
  const heading = findEntry(headings, wanted);
  if (!heading) return null;
  const startLine = lineOf(text, heading.index);
  ...
  return {
    ...
    startLine,
    endLine,
    type: fieldOf(heading.body, 'Type'),
    status: fieldOf(heading.body, 'Status'),
    checklist: checklistRows(heading.body).map(({ checked, text: rowText, bodyLine }) => ({
      checked,
      text: rowText,
      line: startLine + bodyLine,
    })),
    body: bodyLines.map((lineText, i) => ({ line: startLine + i, text: lineText })),
  };
}

/**
 * The index `--list` prints: every entry in file order with its own number, heading
 * date (whatever follows the `—` on the heading line, verbatim — this template never
 * constrains its shape beyond "some date") and `startLine`, so a caller can jump
 * straight to one without re-deriving `entryDetail` for every entry just to skim them.
 */
export function entryIndex(text) {
  const headings = entryHeadings(text);
  return headings.map((h) => {
    const m = /—\s*(.+)$/.exec(h.heading);
    return { entry: `${h.digits}${h.suffix}`, date: m ? m[1].trim() : null, startLine: lineOf(text, h.index) };
  });
}

/**
 * The whole answer for one `bin/b7e-changelog` run: one entry's detail (`entryDetail`)
 * or (`list: true`) the whole index (`entryIndex`), read from `file` at `ref` — `ref`
 * resolved the same way `bin/b7e-count`/`bin/b7e-propagated` already do (`resolveRef`:
 * the ref given, else `origin/main` falling back to `main`), so a bare invocation never
 * reads a stray `.claude/worktrees/*` copy sitting on disk. Never writes anywhere.
 */
export async function changelogLookup(dir, { ref: refGiven, file = 'CHANGE_LOG.md', entry, list = false } = {}) {
  ...
  if (list) return { ref, file, entries: entryIndex(text) };

  const detail = entryDetail(text, entry);
  if (!detail) throw new Error(`no Entry ${entry} in ${file} at ${ref}`);
  return { ref, file, ...detail };
}
