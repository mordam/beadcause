...
export function readOwed() {
  ...
  for (const [key, rec] of Object.entries(raw)) {
    ...
    out[key] = {
      ...
      why: String(rec.why || ''),
      at: String(rec.at || ''),
      // Who the close belongs to, carried across the wait. Null — the ordinary case,
      // and every record written before this field existed — means `beadcause`.
      actor: rec.actor ? String(rec.actor) : null,
    };
  }
  return out;
}
 ...
 *
 * `actor` is stored for the same reason as `reason`: the tap that merged is attributed
 * (bc-5l8s), and the retry runs minutes later on a poll with no request behind it. Not
 * carrying it would mean a close that went through immediately said your name and the
 * identical close that had to wait for a gate said `beadcause` — the difference being
 * nothing you did.
 */
export function oweClose({ workspace, id, reason, why = '', actor = null, at = new Date().toISOString() }) {
 ...
 *
 * Each record ends in one of six ways, and the caller logs the first three:
 *
 ...
 *   - `refused` — an epic, owed a close whose reason is a merge. Dropped, because no
 *                 amount of retrying turns a merged pull request into evidence that a
 *                 theme is finished. The only terminal refusal here; see lib/bd.js.
...
export async function sweepOwed(bd, workspaces) {
  ...
  for (const [key, rec] of Object.entries(records)) {
    ...
    // The reason goes in because one gate is about the reason rather than the bead: an
    // epic does not close on a merge, and this retry is the one place that rule could be
    // walked around — it runs minutes later with nothing but the stored sentence, and
    // `owed-closes.json` may already hold records written by a beadcause that had no such
    // rule. See `isMergeReason` in lib/bd.js.
    const gate = await bd.gateFor(ws, issue, { reason: rec.reason });
    if (gate) {
      // That one is terminal, and terminal is a different answer from blocked: nothing
      // about waiting makes a merge better evidence about a theme, so retrying it every
      // thirty seconds for the life of the machine would be a promise this file cannot
      // keep. Dropped, and reported — the record of what landed is the comment on the
      // bead, written by whichever path merged it.
      // `approval` is terminal for the same reason and one step harder: a gate closes when
      // the gate is met and a merge is not evidence about that, so no amount of waiting
      // turns this record into a close anybody wants. Dropping it is also what keeps the
      // rule from being walked around by a record written before the rule existed —
      // `owed-closes.json` may already hold one for a gate bead. See lib/approval.js.
      if (gate.kind === 'merge-reason' || gate.kind === 'approval') {
        results.push({ ...rec, key, status: 'refused', detail: gate.reason });
        done.push(key);
        continue;
      }
    ...
    try {
      // `overClaim` because every record in this file is a close that a delivery
      // already earned: the pull request is merged and this is the promise to finish
      // the bookkeeping. Without it, bd 1.2.1's claim guard makes each retry fail
      // identically forever — which is the one thing this file was written not to do,
      // and it had three records stuck in exactly that loop when bc-9d37.13 was filed.
      await bd.close(ws, rec.id, rec.reason, { actor: rec.actor, overClaim: true });
