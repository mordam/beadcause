 ...
 * draws an inbox containing only what descends from a P0 you own; on its own that is a
 ...
 * rather than a wait, the bead sheet offers it: pick a P0 to adopt it (public/graph.js).
 *
 * ## Three decisions, none of them inherited
 *
 * **A root is any *open* P0, not only one you own.** The board is `ownedByMe` because a
 * screen is answering "what am I answerable for"; this gate is answering "did anybody
 * decide this work should happen", and a colleague's P0 on a shared graph is a decision.
 * Scoping the gate to your own P0s would mean a six-person squad's advocate refused five
 * sixths of the tracker — the federation bc-y3qk is heading for, broken on arrival.
 *
 * **Open, though — a closed P0 is not a root.** This is the case bc-rfnr.7's own comment
 ...
 * disagree. lib/server.js's `p0Board` leaves closed P0s off the screen and stops pulling
 * their descendants in with them; if the gate counted them, a child left open under a
 * finished epic would be dispatched all night while being invisible on the phone — the
 * one combination this epic is built to make impossible. The cost is honest and is the
 * one the bead names: the first time an epic closes over an open child, that child
...
export const NO_P0_ABOVE = 'no P0 above this';

/**
 * Every open P0 in a workspace — the roots this gate measures against.
 *
 ...
 */
export function p0RootsOf(beads) {
  const roots = new Set();
  for (const b of beads?.values?.() || beads || []) {
    if (!b || !isP0(b)) continue;
...
/**
 * Is there a P0 above this bead? The whole rule, over an index already in hand.
 *
 ...
 * raised a P0 in yet. Neither is a reason to stop work, and `underAnyOf` answers `false`
 * for an empty root set — so the check has to be here rather than left to it.
 *
 * **A P0 is above itself.** `underAnyOf` puts a root in its own set, which is what makes
 ...
 * without a special case and stays a leaf you can work directly.
 */
export function hasP0Above(index, id) {
 ...
 *
 * `status: 409` and a named boolean — `noP0` — matching lib/endorse.js, lib/superseded.js
 ...
 * refusals this one is not a wait. Nothing clears it on its own.
 */
export const refusal = (id) =>
  Object.assign(
    new Error(
      `${id || 'that bead'} may not be worked — ${NO_P0_ABOVE}, and a bead nothing has decided is not work. ` +
        'Adopt it under a P0 (or raise one) and it becomes workable with no other change.'
    ),
    { status: 409, noP0: true }
  );
