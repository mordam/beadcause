 ...
 *
 * **Three shapes admit a named word beside their values — `never`, `unknown`,
 * `permanent` — and that is deliberately not the same thing as an optional field.** An
 * optional field means two things at once, not sent or sent as nothing, and the note on
 * `FIELDS` refuses them for exactly that reason. These are *values*: "it has never
 * happened", "this deployment could not establish it", "it is kept forever" are each a
 * single fact an auditor can act on, and each is the fact that would otherwise be
 * smuggled in as an absent key. Writing `unknown` down is the whole of bc-3muu.12 —
 * a deployment that cannot see something says so, and the interval it covers stops
 * being claimable rather than quietly reading as fine.
 */
export const TYPES = Object.freeze({
  ...
  bead: { why: 'a bead id such as bc-3muu.1', ok: (v) => typeof v === 'string' && BEAD_RE.test(v) },
