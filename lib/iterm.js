...
export function parseClosedWindows(stdout) {
  ...
  if (!Number.isInteger(closed) || closed < 0) return { closed: 0, ids: [] };
  return { closed, ids: parts.slice(1, closed + 1) };
}
...
export function closeEmptyWindows({ timeout = 5000 } = {}) {
  ...
  if (!mayLaunch()) return Promise.resolve({ closed: 0, ids: [], error: null, refused: true });
  return new Promise((resolve) => {
    execFile('/usr/bin/osascript', [EMPTY_SCRIPT], { timeout }, (err, stdout) => {
      if (err) return resolve({ closed: 0, ids: [], error: String(err.message || err).split('\n')[0] });
