...
import { recordRead } from './memoryuse.js';
import {
  git,
  ok,
  refTip,
...
/**
 * Who is doing the reading, when the caller did not say.
 *
 * `agent` in every read below is the *subject* — whose memory is being opened — and
 * with `--of` that is somebody else. The reader is the process, so it comes from the
 * environment the spawner stamped, exactly as the write half attributes. A caller that
 * knows better (the CLI, which has `--agent` for a human at a terminal) passes `by`; a
 * process with neither logs nothing, which is right: a read nobody made is not a read.
 */
const reader = (by) => by || process.env.BEADCAUSE_AGENT || '';
 ...
 *
 * The read is recorded (lib/memoryuse.js) rather than merely served, and that is the
 * whole of tier 2's read-back tracking. It goes here rather than in the CLI so that a
 * second caller cannot arrive uninstrumented — and `notesIn`, the one read the daemon
 * makes on an agent's behalf, deliberately does not come through here.
 */
export async function recall(agent, key = null, { by } = {}) {
  const a = name('agent', agent);
  const cwd = await ensureRepo();
  const mine = await readMemory(cwd, a);
  if (key === null) {
    recordRead({ by: reader(by), tier: 'memory', subject: a, hit: Object.keys(mine).length > 0 });
    return Object.fromEntries(Object.entries(mine).map(([k, v]) => [k, v?.value]));
  }
  const k = name('key', key);
  recordRead({ by: reader(by), tier: 'memory', subject: a, key: k, hit: k in mine });
  return k in mine ? mine[k].value : null;
}

/** When each key was last written — the half `recall` drops. For the CLI's listing. */
export async function recallDetail(agent, { by } = {}) {
  const cwd = await ensureRepo();
  const a = name('agent', agent);
  const mine = await readMemory(cwd, a);
  recordRead({ by: reader(by), tier: 'memory', subject: a, hit: Object.keys(mine).length > 0 });
 ...
 */
export async function notes(agent, key = null, { by } = {}) {
  const a = name('agent', agent);
  const cwd = await workingRepo();
  const mine = await readNotes(cwd, a);
  if (key === null) {
    recordRead({ by: reader(by), tier: 'notes', subject: a, repo: cwd, hit: Object.keys(mine).length > 0 });
    return Object.fromEntries(Object.entries(mine).map(([k, v]) => [k, v?.value]));
  }
  const k = name('key', key);
  recordRead({ by: reader(by), tier: 'notes', subject: a, key: k, repo: cwd, hit: k in mine });
  return k in mine ? mine[k].value : null;
}

/** When each note was last written — the half `notes` drops. For the CLI's listing. */
export async function notesDetail(agent, { by } = {}) {
  const cwd = await workingRepo();
  const a = name('agent', agent);
  const mine = await readNotes(cwd, a);
  // `repo` is stamped on a tier-1 read and on nothing else: a note is about one
  // codebase, so "has anyone ever read these" has a different answer per checkout, and
  // a pooled count would put another repo's reading on this repo's screen.
  recordRead({ by: reader(by), tier: 'notes', subject: a, repo: cwd, hit: Object.keys(mine).length > 0 });
  return mine;
}
...
// Exported, rather than kept module-private like `family` and `beadText` below: lib/b7enotes.js
// (bc-khoe.43) needs the same path-shaped tokeniser and the same cosine to score a note
// against a *file path* rather than a bead, and duplicating either would let the two drift —
// a note that scores 1.61 here and 1.59 there because somebody tuned one copy is a bug nobody
...
export function relevantNotes(all, bead, { keep = 4, chars = 4500, floor = RELEVANT } = {}) {
  ...
  const ranked = Object.entries(all || {})
    ...
    // order git happened to hand the keys back. `at` is still only millisecond
    // resolution — two notes written by the same automated pass can tie on it too —
    // so `key` breaks that remaining tie rather than falling back to object order.
    .sort(
      (a, b) =>
...
/**
 * The nearest things already on file to some new prose — `relevantNotes`' question,
 * generalised from "which of these notes fits this *bead*" to "which of these
 * already-filed entries reads like this *text*", and from tier 1 alone to every store an
 * agent can write into. Built for `bin/b7e-known` (bc-xl7n.112): the prose an agent is
 * about to file as a note, a remember or a debrief, checked against what is already
 * there before it becomes a fourth near-duplicate key.
 *
 * `candidates` is store-agnostic on purpose — `{ store, key, value, at }`, `key` null for
 * a debrief entry, which has none to update in place. The caller assembles it from
 * whichever of `recallDetail`/`notesIn`/`readDebriefs` apply; this function never touches
 * a ref itself, and it is pure, exactly as `relevantNotes` is and for the same reason —
 * the scoring rule is the part worth a test that goes nowhere near git.
 *
 * Same tokenizer, same `similarity`, same validated floor (`RELEVANT`) `relevantNotes`
 * uses — a key's own words count towards its text, the way a note's key does there, so a
 * key like `sw-cache-version-conflicts` still counts `cache`/`version`/`conflicts` even
 * when the prose being checked never quotes the value verbatim.
 */
export function nearestEntries(candidates, text, { keep = 5, floor = RELEVANT } = {}) {
  const want = tokens(text);
  return (candidates || [])
    .map((c) => ({ ...c, score: similarity(want, tokens(`${String(c.key || '').replace(/-/g, ' ')} ${c.value}`)) }))
    .filter((c) => c.value && c.score >= floor)
 ...
 *
 * `who` is the one word that has to move when a second kind of agent is handed this, and
 * it is not decoration. The section tells the reader these notes are "what another
 * worker wrote down", which is the sentence that makes them evidence rather than
 * instructions — and it is *false* for a P0 advocate reading its own store, where every
 * line was written by a supervisor taking stock rather than by somebody with the file
 * open. An agent that misreads the author misreads how much weight to give the note. The
 * default is `worker` because that is who this served first, so no existing caller says
 * anything different.
 */
...
/**
 * Every bead with a report still staged — the other half of `archivedBeads`.
 *
 * The candidate list for a read has to come from both places, and for most of tier 4's
 * life it came from the archive alone. That made a report unreadable for exactly as long
 * as it was pending: the write said `debriefed <bead>` and the read a second later said no
 * run had left one, because `archivedBeads` cannot name a bead whose only debrief is a
 * staging ref. A reader that says "the store is empty" over a report that exists is worse
 * than one that says "this is pending", and a session that never gets archived — a
 * resolver, a question window, a crash — turned "for as long as it is pending" into
 * "for ever". See `debriefBeads` in lib/sessionlog.js, which unions the two.
 *
 * Tolerant in the same way `archivedBeads` is, and for the same reason: a directory that
 * is not a checkout, a repo where nobody has ever written one, a git that would not answer
 * all mean "nothing is staged here", and not one of them is worth failing a read over.
 */
export async function stagedBeads(dir) {
  const cwd = await ok(mainCheckout(dir));
  if (!cwd) return new Set();
  // Trailing slash for the reason `topics` gives: without it this matches a ref *named*
  // refs/beadcause/debrief and nothing underneath it.
  const out = await ok(git(cwd, ['for-each-ref', '--format=%(refname)', `${DEBRIEF_PREFIX}/`]));
  ...
  return new Set(
    out
      ...
      .map((l) => l.trim())
      .filter(Boolean)
      .map((ref) => ref.slice(DEBRIEF_PREFIX.length + 1))
      .filter(Boolean)
  );
}
 ...
 */
export async function read(topic, since = 0, { limit = 50, by } = {}) {
  const t = name('topic', topic);
  const cwd = await ensureRepo();
  // The blackboard's read is recorded too. It is the one tier where a read is the
  // *point* — a message nobody collected is a message nobody was told — so a topic
  // with four messages and no reads is a finding rather than a gap in the log.
  recordRead({ by: reader(by), tier: 'bus', key: t });
...
/** The newest `at` across a store's entries — when this agent last wrote one. */
const lastWrite = (stored) =>
  Object.values(stored).reduce((newest, v) => (v?.at && (!newest || v.at > newest) ? v.at : newest), null);

/**
 * How much each agent has written, in both ref-backed stores. The numbers the agents
 * screen draws, and the numbers a suite can check against `git log`.
 *
 * **Keys and writes are both reported because they are different facts.** A store holds
 * one value per key and a write overwrites — so 244 commits over 31 keys is an agent
 * that keeps revising what it knows, and 31 commits over 31 keys is one that writes
 * once and never returns. Reporting either alone loses that, and the epic's own
 * hand-measurement counted commits, so a surface that only counted keys would not be
 * comparable with the table it exists to replace.
 *
 * `dir` decides which repo's tier-1 refs are read, and is the only directory this
 * module takes — for the same reason `notesIn` takes one. The daemon is not standing in
 * the repo it is drawing, and never will be, because it draws four of them.
 *
 * Tolerant throughout: a workspace that is not a repo, a ref that was never written and
 * an agent that has never existed are all "nothing yet", which is a result rather than
 * an error. An agent that has written nothing has to be *visible as such* on that
 * screen, and it cannot be if the read throws.
 */
export async function census(dir, agentIds) {
  const common = await ensureRepo();
  const memLog = await refHistory(common, MEMORY_REF, { limit: 5000 });
  let repo = null;
  try {
    repo = await mainCheckout(dir);
  } catch {
    /* not a repo: tier 1 has nowhere to be, and reports as empty rather than failing */
  }

  // Three git calls per agent, and every one of them is a read of a different ref or a
  // different file — so they go out together. Sequentially this is the slowest thing on
  // the agents list by a factor of three, for a screen whose whole job is to be looked at.
  const rows = await Promise.all(
    agentIds.map(async (id) => {
      ...
      const [mem, stored, noteWrites] = await Promise.all([
        readMemory(common, a),
        repo ? readNotes(repo, a) : Promise.resolve({}),
        repo ? refCount(repo, notesRef(a)) : Promise.resolve(0),
      ]);
      // Tier 2 is one ref for every agent, so the per-agent write count is the subjects
      // `remember` writes — `remember <agent>.<key>: …`. Tier 1 is a ref per agent, so
      // the same number is the ref's own length and needs no parsing.
      const memWrites = memLog.filter((c) => String(c.subject || '').startsWith(`remember ${a}.`)).length;
      return [
        a,
        {
          memory: { keys: Object.keys(mem), writes: memWrites, lastWriteAt: lastWrite(mem) },
          notes: {
            keys: Object.keys(stored),
            writes: noteWrites,
            lastWriteAt: lastWrite(stored),
            ref: notesRef(a),
          },
        },
      ];
  ...
  return { repo, agents: Object.fromEntries(rows.filter(Boolean)) };
}
