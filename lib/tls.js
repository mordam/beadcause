...
export function acquireOnce(cfg, servers, { log = console.log, warn = console.error } = {}) {
  ...
  log(`adopted a certificate for ${next.name} without a restart — ${left} days left; https from the next connection`);
  return { state: 'acquired', name: next.name, daysLeft: left, detail: `plain http until now — ${left} days left` };
}
...
export function startRenewal(
  ...
  if (acquiring) {
    ...
    say(`certificate for ${current.name} — ${roundDays(atStart)} days left, checked every ${cadence(everyMs)}`);
