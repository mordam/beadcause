...
export class Bd {
   ...
   *
   * `retries: 0` for the same reason as the two above: the interval is the retry, and
   * this only runs on a tick that has already established the sync is not moving.
   */
  doltCommit(workspace) {
    return this.run(workspace, ['dolt', 'commit'], { retries: 0 });
  }
