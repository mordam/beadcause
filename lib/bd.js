...
export class Bd {
   ...
   *
   * **The timeout is its own decision, and 30 seconds was not enough.** `run`'s default
   * is 30s, which is generous for every other call here and marginal for this one: 503
   * beads answer in about a second on an idle Mac and took **28.6 seconds** measured on
   * this one under a load average of 33 — twenty agent sessions and a full test suite,
   * which is an ordinary afternoon here rather than a pathological case. A timeout is
   * not a slow list, either: it throws, `ledger` turns the workspace into a row in
   * `errors`, and the History tab draws an empty ledger for a repo with five hundred
   * beads in it. The cost of the higher ceiling is one slow request at most once per
   * cache window, on a read that cannot block anything else.
   */
  async listAll(workspace) {
    return (
      (await this.json(workspace, ['list', '--all', '--limit', '0'], { retries: SWEEP_RETRIES, timeout: 120000 })) || []
    );
  }
