...
export class Bd {
  ...
  async closeGate(workspace, id) {
    ...
    if (issue.issue_type === 'epic') {
      let children = [];
      try {
        children = (await this.json(workspace, ['list', '--parent', id])) || [];
      } catch {
        return null;
      }
  ...
  /**
   * Close a question you are never going to answer.
   *
   * `bd human dismiss` is the tracker's own word for this, and it is broken in bd
   * 1.1.2 exactly the way `bd human respond` is ("resolving issue ID: storage is
   * nil") — so this does what its help text documents: close with a `Dismissed`
   * reason and an optional note. Do NOT "simplify" it back to `human dismiss`; it
   * fails at runtime, on the one path where the failure is silent from a phone.
   *
   * The note goes on twice, deliberately. In the close reason because that is the
   * line `bd show` prints months later, and as a comment because an agent watching
   * the thread reads comments, not close reasons — and a question that vanished
   * with no word in the thread is indistinguishable from one nobody ever saw. The
   * comment goes first for the same reason it does in `respond`: if the close then
   * loses to the Dolt lock, the reason survives instead of being lost with it.
   */
  async dismiss(workspace, id, note) {
    const line = note ? `Dismissed via Beadcause — ${note}` : 'Dismissed via Beadcause';
    await this.run(workspace, ['comment', id, line], { retries: 4 });
    await this.run(workspace, ['close', id, '--reason', line], { retries: 4 });
  }
