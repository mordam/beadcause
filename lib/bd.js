    ...
    if (!job) {
      job = this.run(workspace, ['export'], { retries: SWEEP_RETRIES })
        ...
        .catch((err) => {
          ...
          // quietly answering from before it. The Maps are shared with the entry it came
          // from rather than copied — this is one field on a wrapper, not a second graph.
          if (hit?.index) return { index: { ...hit.index, stale: why }, at: hit.at };
