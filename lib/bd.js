...
export class Bd {
  ...
  async showWithComments(workspace, id) {
    if (this.noIncludeComments !== true) {
      try {
        ...
        if (!/unknown flag|unknown shorthand|flag provided but not defined/i.test(String(err?.message || ''))) throw err;
  ...
  async closeAnswered(workspace, id, reason, { actor = null } = {}) {
    ...
    try {
      const out = await this.run(workspace, args, { retries: 4, actor });
      // The same `assertClosed` the delivery close takes, and it belongs here at least as
      // much: a question whose close silently did not happen is a card that stays in the
      // inbox having already been answered, which is the shape bc-jrvh's three identical
      // answers came out of. A throw here reaches lib/server.js, which reports it rather
      // than telling the phone the card is gone.
