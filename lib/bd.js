...
// The ledger only, not the watcher: lib/epicdone.js takes `bd` as an argument and imports
// nothing that reaches back here, so this is a leaf import rather than a cycle.
import { rememberHandClose } from './epicdone.js';
...
import { approvalStop } from './approval.js';
 ...
 *    BEADS_DOLT_SHARED_SERVER to dodge that, and note that it would not help if you
 *    did: that variable selects bd's *shared* (global) server, which is a different
 *    thing from the per-project `dolt sql-server` a workspace gets by setting
 *    dolt_mode="server" in its own .beads/metadata.json. The distinction matters here
 *    because `sharedServer` is one flag for the whole fleet — beadcause serves eleven
 *    workspaces and turning it on would force all of them into a mode none of them are
 *    in, so every command would fail against a server that isn't listening.
 *
 *    **Per-project server mode needs nothing from this file.** It is configured
 *    workspace-side, `bd` starts the server itself on first use, and the spawn below is
 *    already correct for it — the beadcause workspace has been on one since 2026-08-17
 *    and the retry backoff simply stops firing there, because there is no longer a
 *    single-writer lock to lose. The retries stay for the workspaces still embedded.
 *
...
/**
 * The one failure worth asking about again: embedded Dolt's single writer was held.
 *
 * **Every alternative that spells a lock is a whole word, and `\b` is doing the work.**
 * It used to read `/(lock|locked|…)/i` — a substring match — and *blocks* and *blocked*
 * both contain *lock*, which bd says in sentences that are refusals rather than
 * contention:
 *
 *     Error: dependency bc-a -> bc-b already exists with type "relates-to" (requested "blocks")
 *     cannot close bc-x: blocked by bc-y
 *
 * So a write with `retries: 4` spent five spawns and 400+800+1200+1600ms of backoff
 * proving something that was decided in the first millisecond, and a sweep read spent
 * three. It was invisible because the answer is identical either way — you only notice
 * by counting spawns, which is how it turned up (bc-arj0.20, where a fixture expecting
 * two `dep add`s watched ten go past).
 *
 * `\block\w*` is what tells the two families apart, and the leading boundary is the
 * whole trick: *lock*, *locked*, *locking* and *lockfile* all still match, while
 * *blocks* and *blocked* fail the boundary after a `b`. `deadlock` is its own
 * alternative for the same reason in reverse — the *lock* in it is preceded by a word
 * character, so `\block` would never reach it. Do **not** put bare `locked` or
 * `locking` back alongside as substrings: *blocked* contains *locked*, and the bug
 * comes straight back through the alternative that looks harmless.
 *
 * Narrowed under bc-arj0.22 and pinned by `test/lockword.mjs`, which drives `run`
 * against a fake bd emitting each sentence and counts the spawns. bc-arj0.20 had
 * already fixed exactly one sentence of this from the other end (`TERMINAL_RE` below),
 * because that one had just become routine on a request path; this is the general case
 * it deliberately left, and it is one line with a wide blast radius — every `bd` call
 * in the daemon goes through `run`.
 */
 ...
 *
 * bd's sentence for a pair that already holds an edge ends `(requested "blocks")`, and
 * when `LOCK_RE` above was a substring match on `lock` that made **every refused
 * `bd dep add` look like Dolt lock contention** — five spawns and four seconds of
 * backoff before reporting something decided in the first millisecond, on a request
 * path where `/api/console/create` is a tap on a phone. This was the answer to that,
 * one sentence at a time, because narrowing `LOCK_RE` touched every `bd` call in the
 * daemon and was not a change to make as a side effect of a bug fix about edges.
 *
 * **`LOCK_RE` has since been narrowed (bc-arj0.22), so this no longer has anything to
 * catch that the retry test would not now decline on its own.** It stays anyway, and
 * cheaply: it says of one sentence the thing that is true of it independently of how
 * the lock test happens to be spelled — asking again cannot change the answer — so a
 * future widening of `LOCK_RE` cannot bring the four seconds back to the one path
 * where they were actually measured. `test/lockword.mjs` holds the general half.
 */
...
/**
 * ## A third wording, next to the two above — and the one lib/take.js exists to answer
 *
 * bd 1.2.1 refusing a plain `--claim` because the bead already carries an assignee,
 * whoever it is:
 *
 ...
 *
 * This is the refusal seven sessions (dv-b5d.24, dv-b5d.26, dv-b5d.22, dv-gr6.40,
 * dv-3rn.4, dv-3rn.2, dv-gr6.36 — bc-dgx7.97) each hit on the very first `bd update
 * <id> --claim` they ran, every time because the "holder" it named was never a live
 * session at all: it was a studio role a bead had been routed to, a plain assignee
 * with nobody's claim behind it. `--claim` refuses on that exactly as it would on a
 * real holder, because from bd's side of the wording the two look identical — an
 * assignee is set and it is not you.
 *
 * Narrower than its neighbours in the same direction they are: this regex only
 * extracts *who* bd says the holder is. It does not decide whether taking the bead
 * over is safe — that answer needs the workspace's own role roster, which is not
 * bd's to know and not this file's business either. See lib/take.js.
 *
 * Anchored on "already assigned to" alone, deliberately not on the leading "issue
 * already claimed:" — `run` above prefixes every message with `bd <args> failed in
 * <ws>: `, so the wrapped sentence is what actually survives to here, and the part
 * this needs is the part after the colon.
 */
export const CLAIM_ROLE_GUARD_RE = /already assigned to\s*"([^"]*)"/i;

/** The holder bd 1.2.1 named on a `--claim` refusal, or null if it did not say this. */
export function claimedBy(err) {
  if (!err) return null;
  const said = `${err.message || ''}\n${err.stderr || ''}\n${err.stdout || ''}`;
  const m = CLAIM_ROLE_GUARD_RE.exec(said);
  return m ? m[1] : null;
}
...
/**
 * How many times the three `bd dolt` verbs retry a *lock* before giving up.
 *
 * These three used to be the only writes in the file that retried nothing, and the
 * argument for that was about the network rather than the lock — see `doltPull` for
 * the whole of it, and for the four-second hand-run that showed the two apart.
 *
 * Two rather than a write's four, and for `SWEEP_RETRIES`' reason rather than a read's:
 * these run on a timer with an interval that is itself the retry, so anything 400ms and
 * 800ms of backoff do not clear is the next tick's problem by definition. What that
 * buys is the same thing two buys a sweep read — the ordinary collision, where another
 * session held the writer for a moment — without spending 4s of a tick's budget on the
 * unusual one.
 *
 * As everywhere else, a *timeout* is not covered by this at all: `run` refuses to retry
 * one, so a call killed at its ceiling costs one ceiling and not three.
 */
const DOLT_LOCK_RETRIES = 2;
...
/**
 * How long a workspace that has just proved it cannot answer is refused without being
 * asked again, and the ceiling that backoff is not allowed to climb past. bc-xl7n.140.
 *
 * A timeout is not one slow answer, it is a broken workspace — and until this, nothing
 * remembered that between calls. On 2026-08-25 deluvia's Dolt went unreadable and every
 * caller (the sweep, `/api/questions`, `/api/history`, an on-demand route) spent the
 * full `BD_TIMEOUT` finding that out again, one at a time, for as long as the outage
 * lasted: 80 timeouts logged in one forty-second window, each one a child process held
 * for two minutes and, because the loop has to drain it, two minutes the daemon itself
 * could not spend on any *other* workspace either.
 *
 * The shape is `pr.available()`'s: a **no** is cached and expires, backing off on a
 * repeat no so a workspace that stays down is asked less and less often rather than
 * every `QUARANTINE_MS` forever. A **yes** — any call that gets a real answer, failure
 * included — clears it immediately, because a real answer is proof the tracker came
 * back and holding the refusal past that would quarantine a workspace that has already
 * recovered. The cap is well under `BD_TIMEOUT` on purpose: the whole point is to save
 * the two minutes, so a backoff allowed to reach it would spend on the *second* call
 * what this exists to avoid spending on the third.
 */
export const QUARANTINE_MS = 20_000;
export const QUARANTINE_MAX_MS = 60_000;
...
/**
 * **Every `bd` in this program takes the workspace *object*, and this is where a
 * caller who reached for the name finds out.** bc-ygwa.
 *
 * An advocate record carries the workspace twice — `a.workspace` is `{name, dir}` and
 * `a.name` is the string — and the two are interchangeable to the type system and to
 * every test that builds its own fixture. bc-2uj4.6 was that swap in the other
 * direction, in lib/parked.js, and it lived a fortnight because nothing threw: the
 * whole class is silent by construction, which is why it is worth one line of check at
 * the one place it cannot be routed around.
 *
 * Silent *here* too, without this. `workspace.dir` on a string is `undefined`; a spawn
 * env with an `undefined` value simply drops the key, so `BEADS_DIR` goes unset and
 * `cwd` falls back to the daemon's own — and `bd` then resolves whichever workspace the
 * daemon happens to be rooted in and answers, successfully, about the wrong tracker.
 * A read comes back with somebody else's beads and a write lands in somebody else's
 * graph. There is no exception to route around: `run` is the single funnel — `json`
 * calls it, `graph` calls it, every one of the ~40 public methods ends here — so the
 * check is paid once per `bd` spawn, beside an `execFile`, and costs nothing that can
 * be measured against it.
 *
 * The reverse direction — the object handed to something that wanted the name — has no
 * single funnel to guard, and lib/parked.js already normalises with `nameOf`. What
 * covers the rest is test/wsshape.mjs, which reads the advocates' call sites rather
 * than running them.
 */
function assertWorkspaceObject(workspace, rawArgs) {
  if (workspace && typeof workspace === 'object' && workspace.dir) return;
  const cmd = `bd ${(rawArgs || []).slice(0, 2).join(' ')}`.trim();
  const got =
    typeof workspace === 'string'
      ? `the name "${workspace}"`
      : workspace && typeof workspace === 'object'
        ? `an object with no \`dir\` (keys: ${Object.keys(workspace).join(', ') || 'none'})`
        : String(workspace);
  throw new Error(
    `${cmd}: needs the workspace object \`{name, dir}\`, got ${got}. ` +
      'Every `bd.*` call takes the object; it is `a.workspace`, never `a.name`. See bc-ygwa.'
  );
}
...
export class Bd {
  ...
  constructor({ bin, actor, sharedServer = false, me = null }) {
    ...
    this.me = me;
    // One `Bd` per daemon, so this is the one place a repeat timeout for a given
    // workspace is visible to every route and every sweep that shares it. Keyed by
    // `workspace.name`, not `.dir`: two workspaces never share a name, and the name is
    // what every message and log line already identifies the tracker by. See
    // `QUARANTINE_MS` above.
  ...
  run(workspace, rawArgs, { retries = 0, timeout = BD_TIMEOUT, actor = null } = {}) {
    assertWorkspaceObject(workspace, rawArgs);
    // Refused before anything is spawned — the whole saving is not paying the ceiling a
    // second time. See `QUARANTINE_MS`. `timedOut` is set on this error too, so nothing
    // downstream (`trouble`, `errors[]`, the phone) has to learn a second flag: a
    // quarantined workspace still reads exactly like a timed-out one, it is just cheap.
    const held = this.quarantines.get(workspace.name);
    const now = Date.now();
    if (held && now < held.until) {
      const e = new Error(
        `bd ${rawArgs.join(' ')} refused in ${workspace.name}: it timed out ${fmtMs(now - held.since)} ago ` +
          `and is being given room before being asked again — due back in ${fmtMs(held.until - now)}`
      );
      e.timedOut = true;
      e.quarantined = true;
    ...
    /**
     * A close somebody typed here, remembered so the phone does not chime at their own tap.
     *
     * Here rather than in `close` and `closeAnswered`, because this is the funnel both of
     * those already go through and a fifth close path added next month goes through it
     * too — the failure mode of forgetting a registration is a notification for an epic
     * you closed while looking at it, which is the whole of what bc-ka5y.15.2 exists to
     * stop. `actor` is exactly the signal: only a request carrying a signed-in session
     * passes one (see the comment above), so every agent, token caller and sweep in the
     * daemon is null here and stays audible.
     *
     * Recorded before the spawn and not after a zero exit, deliberately. A close that
     * then failed leaves an entry for a bead that never moved, which nothing will ever
     * look up; waiting for success would leave a window in which the bead is closed and
     * the ledger does not know it yet.
     */
    if (actor && rawArgs?.[0] === 'close' && rawArgs[1]) rememberHandClose(workspace, rawArgs[1]);
    const env = {
      ...process.env,
    ...
    // BEADS_ACTOR alone was not enough once: a workspace config.yaml with
    // `actor: "…"` in it took precedence — observed with a workspace pinning a
    // personal address — so comments written from the phone came back attributed
    // to the human rather than to beadcause, and reply-detection then notified you
    // about your own comments. The --actor flag wins over both, which is why it is
    // here. Re-measured against bd 1.2.1 under bc-y3qk.1 and the precedence has
    // since flipped — a pinned `actor:` is now a fallback the environment beats —
    // but the flag is what this does not have to care which way round it is, and
    // three of the four personal workspaces on this Mac still carry a pin.
      ...
      new Promise((resolve, reject) => {
        ...
        execFile(
          ...
          (err, stdout, stderr) => {
            spend('bd', spawned);
            if (!err) {
              // A real answer, from the workspace this quarantine was about — proof it
              // came back. Cleared rather than left to expire on its own, so a tracker
              // that recovers early is not held quarantined until a timer says otherwise.
              this.quarantines.delete(workspace.name);
            ...
            if (timedOut) {
              // Quarantined from here, not from the refusal above — this is the one place
              // that knows the workspace just proved it cannot answer. Backs off on a
              // repeat timeout so a workspace that stays down is asked less and less
              // often, capped at `QUARANTINE_MAX_MS` (well under `BD_TIMEOUT`, on purpose:
              // see the comment on it).
              const prior = this.quarantines.get(workspace.name);
              const wait = prior ? Math.min(prior.wait * 2, QUARANTINE_MAX_MS) : QUARANTINE_MS;
              this.quarantines.set(workspace.name, { since: Date.now(), until: Date.now() + wait, wait });
            } else {
              // A real failure is still a real answer — a bead that does not exist, a
              // permission refusal — so it clears a quarantine exactly like a success does.
              this.quarantines.delete(workspace.name);
            }
   ...
   *
   * ## Three gates that are beadcause's own, and bd does not share them
   *
   * Everything above models what the binary already refuses. The three below are rules
   * bd has no way to hold, and they are marked as such in test/closegatereal.mjs, where
   * every other case asserts that the gate and the binary agree and these assert
   ...
   *     a theme.
   *   - **a `gate` or `needs-approval` bead closing on a merge reason** — the one law, and
   *     bd is told a sentence rather than an approval. lib/approval.js.
   *
  ...
  async gateFor(workspace, issue, { reason = '' } = {}) {
    ...
    // **The one law, before anything about types or children: no agent closes a gate, and no
    // agent closes a bead waiting to be approved.** Checked here rather than in the epic
    // branch below because it is about neither the type nor the shape of the bead — a gate is
    // usually an ordinary task, and the four gates that were closed under an epic on
    // 2026-08-10 were tasks. See lib/approval.js for why this keys on the close *reason*
    // rather than holding the bead outright: the phone asks this gate with no reason before
    // it draws the close button, and *the tap is the close*.
    const approval = approvalStop(issue, isMergeReason(reason));
    if (approval) {
      return { kind: 'approval', blockers: [], reason: approval };
    }
   ...
   * on its way to closing the bead, so the status is not its business.
   *
   * **The clear itself can be refused, and this comment used to say otherwise.** "The
   * clear is permitted from an actor that is not the assignee" was true only of an
   * *open* bead — bd 1.2.1 refuses `--assignee ''` on an `in_progress` one with the same
   * sentence `REASSIGN_GUARD_RE` reads (`cannot reassign … held by "…" (in_progress)`),
   * measured directly against the binary rather than assumed (bc-xl7n.88). Every
   * question this method is ever called on is `in_progress` — a worker claims it as its
   * first act — so the refusal was not an edge case, it was the ordinary case: bd's own
   * log carried it as an HTTP 500 on `/api/respond` with the answer already written to
   * the thread, which is bc-ko7n's failure reopened by a second door.
   *
   * So the clear is lazy in exactly the shape `reopenAbandoned` already uses for the
   * same sentence: try it plain, and reach for `--force` only when *that* write is what
   * bd refused. It is idempotent on a bead that has none either way. This does not
   * reopen the argument `reopenAbandoned`'s docblock makes about `commission` and
   * `lib/jiragate.js` — those reassign a *live* status transition and keep the plain
   * write for that reason — because this write never touches status at all, so there is
   * no "moving into a done status" for `--force` to lift, only the one guard it names.
   * And it is not the `--force` bc-9d37.13 and this method's own header rule out: that
   * argument is about forcing the *close*, which skips the clear and leaves the
   * assignee standing as a stale artifact of the window that touched the card. Forcing
   * the clear still ends with an empty assignee — the same outcome Adam chose on
   * 2026-08-14, reached the one time the narrow instrument alone cannot get there.
   *
  ...
  async closeAnswered(workspace, id, reason, { actor = null } = {}) {
    ...
    try {
      ...
      try {
        ...
        if (!isReassignGuard(err2)) throw err2;
        // The ordinary case, not a second refusal: a question is `in_progress` from the
        // moment a worker claims it, so the plain clear above is refused every time and
        // this is the write that actually lands. See the docblock above bc-xl7n.88.
        console.error(`[bd] ${id}: forcing the claim off to close it — its holder already answered and moved on`);
        await this.run(workspace, ['update', id, '--assignee', '', '--force'], { retries: 4, actor });
      }
      return await this.run(workspace, args, { retries: 4, actor });
    }
  }
   ...
   *
   * **`reopenAbandoned`, not `reopen` — since bc-xl7n.88, and it used to be the other
   * way round.** `reopenAbandoned`'s own docblock named this method, alongside
   * `lib/jiragate.js`, as a case where the plain write is right because "the holder may
   * still be typing". That reasoning does not survive the same correction bc-36xx.17
   * already made for the review path: a question's assignee is whichever worker window
   * claimed it as its first act, and by the time an answer reaches this method that
   * window has always already delivered, handed back, or filed the question and
   * stopped — never "still typing". So the plain `reopen` this used to call was refused
   * on the ordinary case, not the edge case, the same shape `closeAnswered`'s clear was
   * refused in, and for the same reason: bd 1.2.1's reassign guard reads the bead's
   * *current* status, and a question is `in_progress` from the moment it is claimed.
   * `lib/jiragate.js` keeps the plain `reopen` — a ticket coming back off Jira really can
   * still be under a live claim, and that is the one caller this method's own history
   * warns against widening carelessly.
   *
   ...
   *
   * `stayInInbox` has **two** callers, and they want it for reasons that meet in the
   * same place — the card must not leave the inbox — while disagreeing about everything
   * either side of that.
   *
   *   - **A conflict sweep's hand-back** (bc-9d37.8), which is the one commission whose
   *     work is not a bead: answering it opens a resolver window on a branch, and the
   *     card the answer was given on stays a card — it amends itself as that window gets
   *     on with it and closes itself when the branch comes back mergeable. Taking `human`
   *     off it would do both halves of this wrong at once: the card would vanish from the
   *     inbox while it was still the only thing reporting what it had started, and it
   *     would land in an advocate's ready queue, where the next tick would open a
   *     *worker* on a summary of a sweep.
   *   - **A deferral** — an option marked `defers: true`, which is *not yet, ask me again
   *     later* (bc-7qo.11, and `defersFlag` in lib/decision.js for the bead it happened
   *     to). Here nothing has been started at all, so taking `human` off is unambiguously
   *     wrong rather than merely premature — bc-7qo.10 was deferred and the label came
   *     off, so the next advocate tick opened a worker window on the question that had
   *     just been put off. The card itself does leave the inbox now (bc-y9cof), but a
   *     dismissal record is what hides it, and `withoutDismissed` only keeps that record
   *     while the sweep still returns the bead. So the label is what makes the deferral
   *     *reversible*: drop it and the card is not set aside, it is gone.
   *
   * Either way the label stays and the bead is still open, which is all this flag does.
   * What the two endings are *called* is decided by the caller, on the thread — see the
   * record sentences in `/api/respond`, where "handed back" and "deferred" are the same
   * three writes described honestly rather than one write described twice.
   */
  async commission(workspace, id, response, { actor = null, stayInInbox = false } = {}) {
    ...
    await this.reopenAbandoned(workspace, id);
    await this.relateMentions(workspace, id, response);
  }
   ...
   * `bd update --claim`, which sets the assignee to the worker's own `BEADS_ACTOR`
   * (measured against bd 1.2.1 under bc-y3qk.1 — this used to say "git identity", which
   * is where `owner` comes from and not this), while everything beadcause runs carries
   * the byline `beadcause (…)` so `created_by` says which machine filed it. The two
   * strings differ — and go on differing now a worker signs `agent (carol@example.com)`
   * rather than an address — so from 2026-08-14 07:21 no delivery could close
   ...
   * claim is abandoned (crashed agent, expired lease)"*. Every caller has already
   * established exactly that before it gets here — `handBack` is called *because* the
   * window is gone, `bin/plan.js` is the holder releasing its own claim through an actor
   * string that does not match it, and lib/server.js's `handBackWorkBead` is answering a
   * delivery whose worker exited when it delivered. That is the whole reason this is a
   * second method and not a flag on `reopen`: lib/jiragate.js reopens a ticket coming
   * back off Jira, whose claim may still be **live**, where the guard is doing the job it
   * was added for, and a shared parameter is one careless `true` away from lifting it
   * there too.
   *
   * **The review path used to be on that second list and should never have been** — this
   * comment said so, in as many words, and it was wrong (bc-36xx.17). Since bc-r941 a
   * worker delivers and *stops*, so a `CHANGES:` or a decline answered hours later is
   * always releasing a claim held by a window that no longer exists: the guard fired on
   * exactly the case this method exists for, and the bead went out of `bd ready` for good
   * while the log announced it back in the queue. An assumption that is written down is
   * still an assumption — check it against the paths as they are now before adding to
   * either list.
   *
   * **`commission` was on the live-claim list too, and it was the same mistake in the
   * same shape (bc-xl7n.88).** A question's assignee is whichever worker window claimed
   * it as its first act, and every ending that path has — deliver, hand back, or file a
   * question and stop — leaves that window gone by the time an answer reopens the bead.
   * `commission` now calls this method rather than the plain one; see its own docblock.
   * The live-claim list is down to lib/jiragate.js — check it against the paths as they
   * are now before treating it as settled, the way this comment used to.
   *
  ...
  /**
   * Refresh bd's own per-issue claim lease — `bd heartbeat`, not `held:` (lib/lease.js).
   *
   * bd 1.2.1 ships a second, independent notion of a claim going stale: every `--claim`
   * takes a lease with its own TTL, kept alive by `bd heartbeat` and, once it lapses,
   * eligible for `bd reclaim` — "the reaper" for a dead worker's claim, in bd's own words
   * (`bd reclaim --help`). Nothing in this file ever drove either half of that: a worker
   * claims once, at the start, and nothing here ever calls `bd heartbeat` again — so the
   * lease on a bead being worked runs out on its own clock regardless of whether the work
   * is still going, exactly as bc-xl7n.114 describes ("a lease only lapses because nothing
   * renews it"). Whatever ends up running `bd reclaim` — a future supervisor here, one
   * outside this repo, or a later bd release doing it on its own — a fresh heartbeat is
   * what keeps this worker's claim off its list.
   *
   * Called from `reconcile` in lib/advocate.js once per tick for every worker it can still
   * see is live — "have the daemon renew it while it can see the window's pid alive" is
   * the bead's own direction (1), and the daemon is a better fit for it than the worker's
   * own brief: one line here covers every worker, forever, with nothing for a brief to
   * remember to do mid-task.
   *
   * Failure is swallowed rather than thrown: bd's own doc says heartbeat itself fails once
   * the lease is already gone or the issue closed ("so the worker learns to stop"), and
   * neither of those is a reason to break a reconcile pass over a claim this daemon is
   * about to re-examine anyway on its own terms. No retries — a heartbeat that missed this
   * tick gets another at the next one, which is the whole point of a periodic renew.
   */
  async heartbeat(workspace, id) {
    try {
      await this.run(workspace, ['heartbeat', id], { retries: 0 });
      return true;
    } catch {
      return false;
    }
   ...
   *
   * **And `blocked-by:<workspace>/<id>` is excluded too**, by the identical weaker
   * mechanism as `superseded-by:` — the label carries the far bead's id in it, so there
   * is no fixed string for `--exclude-label` and this row check is the only thing
   * keeping it out. See lib/farblock.js — bc-bmry.7. A block a real `bd dep add` edge
   * can express never reaches this label at all; this is only for the cross-tracker case
   * an edge cannot cover.
   *
  ...
  async ready(workspace, { excludeLabels = null, excludeLabel = null } = {}) {
    ...
    return rows.filter(
      (r) => !isSuperseded(r) && !isBlockedElsewhere(r) && !(r?.labels || []).some((l) => labels.includes(String(l).trim()))
    );
  }
  ...
  /**
   * The other side of `ready`'s `blocked-by:` filter: ready in bd's own eyes, and
   * marked as blocked on a bead in another tracker. What lib/farblock.js sweeps.
   *
   * No `--exclude-label human` here, unlike `readySuperseded` — that sweep skips a bead
   * already carrying the card it is about to raise, and this one raises no card at all.
   * Clearing the marker is idempotent (a bead with none is left alone) and cheap, so
   * there is nothing to avoid re-doing on the next pass.
   */
  async readyFarBlocked(workspace) {
    const args = ['ready', '--limit', '0'];
    const rows = (await this.json(workspace, args)) || [];
    return rows.filter((r) => isBlockedElsewhere(r));
  }
  ...
  /**
   * The half `listAgent` deliberately drops: live cards, narrowed to one agent's own.
   *
   * `listAgent`'s `--exclude-label human` is right for what it was written for — the
   * questions are already in hand from `listHuman`, and a workspace like climative is 88KB
   * of `bd list --json` because every row carries its whole description. What it also
   * does, silently, is put an agent's *own* handovers out of that agent's reach: the merge
   * queue hands a pull request over by taking `merge-queue` off and putting `human` on, so
   * the moment a merge-bead becomes a card it leaves the only list the queue ever reads.
   *
   * That was bc-uxrix, and it cost two things. `cardedFor` — the whole take-a-card-back
   * mechanism of bc-91srt — selects rows carrying `human` and *not* `merge-queue`, so fed
   * a list `human` had been filtered out of it could only ever return `[]`: the loop had
   * never run once in the daemon. And `strandedPrs` builds its cover set from the same
   * rows, so every carded pull request was reported as having no merge-bead about it —
   * measured at 22 live cards on 2026-08-26, once per tick each, which made it the
   * highest-volume false statement in the log.
   *
   * `assignee` is what keeps this from being "read the inbox twice". A card the merge
   * queue raised is assigned to `merge-advocate`; a question Adam filed, a decision bead,
   * an epic advocate's ask are not, and none of them has any business in a queue's row
   * set. Empty means every card, which no caller wants and which is why nothing defaults
   * it here — the one caller passes `MERGE_ASSIGNEE` and owns that choice.
   *
   * `--limit 0` for `listAgent`'s reason: bd's own default is 50.
   */
  async listCards(workspace, assignee = '') {
    const args = ['list', '--status=open,in_progress,blocked', '--label', 'human', '--limit', '0'];
   ...
   * are being filed. So the stand-in carries the reason it is a stand-in.
   *
   * **The last good answer carries `stale` for the same reason, and keeps its own age —
   * bc-j52g.** A stale reading is still a reading and its P0s are real, so it is still
   * what everything drawing a screen should be handed; what it is *not* is a reading of
   * now, and a caller that turns the index into writes has to be able to tell. It also
   * used to be re-stamped `at: Date.now()` on the way into the cache, which quietly
   * extended a failed read into a full minute of answering from before the failure, with
   * no retry in it — so a single lost `bd export` hid every write of the minute around it
   * from `refresh: true` itself. Keeping the previous `at` is what makes the next call
   * try again rather than be served the same old graph.
   *
   * **`refresh: true` chains behind an export already in flight, and does not join it —
   * bc-4m2j.7.** Every caller of `refresh: true` is asking because it just wrote
   * something (`adopt`, `setStatus`, `sweepAdoptions`'s own tail), and on a 30-second poll
   * across nine workspaces there is often already an export running for this key. Joining
   * that job — the original behaviour — hands the caller an answer that may have been
   * read *before* its write, which is the same failure shape bc-j52g fixed one line above
   * for a different cause. So a `refresh: true` that finds a job already in `PARENT_INFLIGHT`
   * does not take its promise; it starts a fresh export chained to run once that one
   * settles, and that chained job is what both `PARENT_INFLIGHT` and the caller get. The
   * cost is one extra `bd export` — about a second on a loaded Dolt — paid only by the
   * refreshing caller, and only when it actually overlapped one already running.
   */
  graph(workspace, { refresh = false, wait = true } = {}) {
    ...
    const runExport = () =>
      this.run(workspace, ['export'], { retries: SWEEP_RETRIES })
        .then((out) => ({ index: indexFrom(out), at: Date.now() }))
        .catch((err) => {
          ...
          //
          // **But it says that it is the old answer, and it does not pretend to be new**
          // — bc-j52g, and both halves of that cost a suite its determinism. The copy
          // carries `stale`, so a caller that must not decide on a reading it could not
          // take (lib/adoptsweep.js plans writes off this index) can tell "here is the
          // shape" from "here is the shape as of some earlier minute"; `error` stays what
          // it always was, the never-read case, so nothing already branching on it changes
          // behaviour. And `at` is the *previous* answer's, not now: stamping a failed read
          // fresh held the pre-failure graph in front of every caller for a further sixty
          // seconds with no retry, which is how a read that failed once became a minute of
          // quietly answering from before it. The previous entry is read fresh from
          // `PARENT_CACHE` here rather than closed over, so a fallback after a *chained*
          // export falls back to the export before it, not to whatever was cached when
          // this whole call started.
          const prior = PARENT_CACHE.get(key);
          if (prior?.index) return { index: { ...prior.index, stale: why }, at: prior.at };
          return { index: { parents: new Map(), beads: new Map(), adopts: new Map(), edges: new Map(), error: why }, at: Date.now() };
        })
        .then(({ index, at }) => {
          PARENT_CACHE.set(key, { at, index });
          return index;
        });

    // Track `job` as the in-flight one for `key`, and stop tracking it once it settles —
    // without clobbering a newer job that has already replaced it in the map (the chained
    // case below sets a new one before this one's own `finally` runs).
    const track = (job) => {
      PARENT_INFLIGHT.set(key, job);
      job.finally(() => {
        if (PARENT_INFLIGHT.get(key) === job) PARENT_INFLIGHT.delete(key);
      });
      return job;
    };

    let job = PARENT_INFLIGHT.get(key);
    // A request that finds an export already running and joins it — every case below
    // except a fresh export of its own — spawns nothing, so without this it comes back
    // `no subprocess, all ours` the way the cold path in lib/cache.js used to (bc-1kwl.33).
    // Not `refresh`, which chains a fresh export rather than joining (bc-4m2j.7) and so
    // pays for its own child; and not `wait: false`, which never awaits `job` at all.
    const waited = wait && job && !refresh ? joining() : null;
    if (!job) {
      job = track(runExport());
    } else if (refresh) {
      // A refresh that finds an export already running cannot be handed that export's
      // answer — see the note above. Chain a fresh one behind it instead of joining it.
      job = track(job.catch(() => {}).then(runExport));
    }
    ...
    if (!wait) return Promise.resolve(hit?.index || EMPTY_GRAPH);
    return waited ? job.finally(waited) : job;
  }
  ...
  /**
   * Collect this workspace's Dolt store — the nightly maintenance window's whole point.
   *
   * Two flags, and both are load-bearing:
   *
   * **`--skip-decay`, always, and it is not a parameter.** `bd gc` runs three phases and
   * the first one *deletes closed beads older than ninety days*. This repo's CLAUDE.md
   * makes the Dolt commit log the recovery path for beads that go missing, so a job that
   * silently deletes beads — and squashes the history you would look for them in — is
   * not maintenance. Phase 2 is advisory anyway: measured against bd 1.2.1 it reported
   * `9469 commits in history` and changed nothing, suggesting `bd flatten`, which is the
   * recovery net itself and is therefore also never run from here. The whole win is
   * phase 3. See lib/maintenance.js.
   *
   * **`--force`** answers the confirmation prompt an unattended daemon has nobody to
   * answer. Without it the child waits on stdin until `timeout` kills it.
   *
   * `retries: 0`. Every other write here retries through Dolt's lock because waiting is
   * the fix; a collection does not need that, because bd's own gate lock already makes
   * concurrent `bd` processes queue rather than fail — measured with six readers against
   * a store mid-collection, all six correct. And a *timed out* collection must not be
   * retried: it holds the gate for its whole run, so a second attempt would spend the
   * ceiling again on a machine that has just shown it is too busy.
   *
   * The default ceiling is fifteen minutes rather than `BD_TIMEOUT`'s two. The
   * measurement is 2.9 seconds on an 825MB store with readers on it, so two minutes is
   * ample — but this is the one call in the file whose cost scales with how long nobody
   * has run it, and the failure mode of being late (a store uncollected for another day)
   * is worse than the failure mode of waiting (a background call nothing is blocked on).
   */
  async gc(workspace, { timeout = 15 * 60 * 1000 } = {}) {
    return await this.run(workspace, ['gc', '--skip-decay', '--force'], { retries: 0, timeout });
  }
   ...
   *
   * ## Two costs, and this used to charge both of them to the network (bc-y3qk.2)
   *
   * These two were `retries: 0`, argued from the one thing that makes them unlike every
   * other write in this file: they are the only `bd` calls in the app that go to the
   * *network*, and a retry of a two-minute network timeout is four minutes of a poll
   * cycle spent on a workspace that will be tried again in two minutes anyway. **That
   * argument is still sound and it is still in force** — it is just not an argument
   * about the lock, and the lock is the other half of what these calls queue behind.
   *
   * `bd dolt pull` hand-run against the `architecture` workspace completed in **four
   * seconds** on the day the daemon had logged four *"still running after 120s"* lines
   * against the same remote, and 56 by the time anybody counted. The remote is not what
   * that time was spent on. Embedded Dolt is single-writer, this laptop runs twenty-plus
   * agent sessions that hold that lock in bursts, and a lock collision is the exact case
   * every other write here retries four times for — so a sync was losing a whole tick to
   * a lock another session held for three seconds, and reporting a working tracker as
   * broken.
   *
   * So they retry now, and **nothing about the network argument had to be given up to do
   * it**, because `run` refuses to retry a timeout at all (see the `timedOut` branch
   * there). A retry here fires only on `LOCK_RE`, which is a refusal that came back in
   * milliseconds; a call killed at its ceiling still costs exactly one ceiling and still
   * leaves the interval to be the retry.
   *
   * `DOLT_LOCK_RETRIES` rather than the four a write gets, for `SWEEP_RETRIES`' reason:
   * this is on a timer, so anything the two backoffs do not clear is the next tick's
   * problem by definition, and 1.2s of waiting inside a tick is a budget worth keeping
   * small — see `timeout` below for what that budget is.
   *
   * ## The ceiling is the caller's, because only the caller knows the interval
   *
   * `BD_TIMEOUT` is 120s and the default `sync.seconds` is 120 as well, so a tick that
   * burned its ceiling was still running when the next one came due — which is what
   * produced the *"still syncing from the last tick — skipped"* lines, and what made one
   * collision cost two intervals rather than one. Two numbers that have to stay apart
   * cannot both be defaults picked in different files, and the one that knows the
   * interval is lib/sync.js. So the ceiling arrives from there (`syncCeilingMs`), and
   * `BD_TIMEOUT` is what a caller that says nothing still gets.
   *
   * Deliberately not a shorter default here. A push is a git push under the hood, and
   * killing one mid-flight loses the push and nothing else — the remote either took the
   * whole thing or none of it — so the cost of waiting is only ever a slow tick, while
   * the cost of a ceiling set too low is a large first sync that can never complete and
   * reports as broken every time. lib/sync.js is where that case is distinguished,
   * because it is the only thing that knows whether this workspace has ever synced.
   */
  doltPull(workspace, { timeout = BD_TIMEOUT } = {}) {
    return this.run(workspace, ['dolt', 'pull'], { retries: DOLT_LOCK_RETRIES, timeout });
  }

  doltPush(workspace, { timeout = BD_TIMEOUT } = {}) {
    return this.run(workspace, ['dolt', 'push'], { retries: DOLT_LOCK_RETRIES, timeout });
  }

  /**
   * Commit whatever is sitting uncommitted in the workspace's Dolt working set.
   *
   * Only ever called by lib/sync.js's stuck-pull recovery, and it exists because Dolt's
   * own refusal names the remedy: *"local changes would be stomped by merge … Please
   * commit your changes before you merge."* A merge is refused while the working set is
   * dirty, so the way out is to stop it being dirty — and of the two ways to do that,
   * this is the one that keeps the changes. Discarding them (`dolt checkout <table>`)
   * would also clear the refusal, and would throw away a write one of the twenty agent
   * sessions on this laptop had in flight, which is not a thing a daemon may decide.
   *
   * `DOLT_LOCK_RETRIES` and a caller's ceiling, for the same reasons as the two above —
   * and this is the one of the three where the lock argument is not even about the
   * network, because a `dolt commit` never leaves the machine. It takes the single
   * writer, which is precisely what it can lose. Losing it here is worse than losing it
   * on a pull: the recovery runs once, on the tick the stuck sync is discovered, and a
   * lock collision inside it does not merely delay the fix, it files the workspace under
   * `commit-failed` and puts a sentence on the phone about a remedy that was never
   * actually tried.
   */
  doltCommit(workspace, { timeout = BD_TIMEOUT } = {}) {
    return this.run(workspace, ['dolt', 'commit'], { retries: DOLT_LOCK_RETRIES, timeout });
  }
  ...
  /**
   * Move a bead from one holder to another, but only if it was still held by `from`
   * the instant this ran — `bd update <id> --if-assignee <from> --assignee <to>` in
   * one call, optionally with a status and a label change riding the same write.
   *
   * This is the CAS the seven sessions behind bc-dgx7.97 each re-typed by hand once
   * they had read the memory explaining why their plain `--claim` was refused: `bd
   * update <id> --if-assignee=<role> --assignee=neadamthal@gmail.com
   * --status=in_progress`. See lib/take.js for the caller and `CLAIM_ROLE_GUARD_RE`
   * above for the refusal this exists to route around.
   *
   * No retry, deliberately, and unlike every other write in this file: a stale
   * `--if-assignee` guard (bd's own exit 13, "precondition guard did not match")
   * means somebody else already won the race between the read this call followed
   * and the write it just tried. Retrying would recompare the same expected value
   * against the same now-wrong assignee and lose the same way every time — the fix
   * is to re-read and re-decide, which is the caller's job, not a backoff loop's.
   */
  casAssign(workspace, id, { from, to, status = null, addLabel = null, removeLabel = null } = {}) {
    const args = ['update', id, `--if-assignee=${String(from ?? '')}`, `--assignee=${String(to ?? '')}`];
    if (status) args.push('--status', String(status));
    if (addLabel) args.push('--add-label', String(addLabel));
    if (removeLabel) args.push('--remove-label', String(removeLabel));
    return this.run(workspace, args);
  }
}
