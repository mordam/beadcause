...
export class Bd {
   ...
   *   - **A deferral** — an option marked `defers: true`, which is *not yet, leave this
   *     on the list* (bc-7qo.11, and `defersFlag` in lib/decision.js for the bead it
   *     happened to). Here nothing has been started at all, and the card staying put is
   *     not a side effect of the work reporting itself: it **is** the answer. That is
   *     what makes taking `human` off unambiguously wrong rather than merely premature —
   *     bc-7qo.10 was deferred and the label came off, so the next advocate tick opened a
   *     worker window on the question that had just been put off.
   *
