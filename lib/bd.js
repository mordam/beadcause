...
import { isP0, ownOwnerLabels, ownersOf } from './ownership.js';
// `byDoneThenId` lives there rather than here because the P0 board's tree sorts its
...
import { mentionsIn, planFor, prefixOf, WRITE_CAP } from './mentions.js';
...
/**
 * The one refusal from `bd close` that is not an objection.
 *
 ...
 *
 * bd will not let one actor close a bead assigned to another. That is a sound rule
 * between two people and a dead end here: the two strings can never be made equal. The
 * actor is a *byline* — `beadcause (<address>)`, see lib/byline.js — deliberately shaped
 * so a comment reads as the daemon acting for a person; the assignee is the bare git
 * address `bd update --claim` writes. Every worker window claims its bead, so every
 * question a session had touched became unanswerable — and because `respond` writes the
 * comment before the close (on purpose, see there), each attempt left the answer on the
 * thread and the card in the inbox to be answered again. bc-jrvh carried the same answer
 * three times over four comments before this was found.
 *
 * bd names two ways out of it, and **Adam chose the narrow one on 2026-08-14: reclaim,
 * then close. Not `--force`.** The flag would have worked and it overrides every gate bd
 * has — a blocked bead, a pinned one — so buying the claim fix with it would have spent
 * the backstop under all of them. Clearing the assignee overrides exactly the one rule
 * that is in the way. See `runClose`, and bc-ko7n's `design` for the fork itself.
 *
 * Matched on its own words rather than on the `--force` hint in its tail, because the
 * other refusals carry that hint too and none of them is this one.
 *
 * Exported because the daemon is not the only process that closes a bead: bin/deliver.js
 * closes the work bead from inside the worker session that has it claimed, which is the
 * shape that trips this hardest, and it spawns its own `bd`. One regex for both, so the
 * day bd rewords the refusal there is one place to fix rather than two to notice.
 */
export const CLAIM_RE = /cannot close \S+: assignee is ".*", actor is ".*"/i;
 ...
 *
 * Without it, `p0Board` asking for nine workspaces in a loop while a poll is already
...
export class Bd {
  ...
  /**
   * The close every close in this file goes through, and the one refusal it answers.
   *
   * A plain `bd close` first, so every gate bd has still applies: a blocked bead, a
   * pinned one, an unsatisfied molecule step are refused here exactly as they were.
   * Only when the refusal is the assignee/actor mismatch (`CLAIM_RE`) does this act —
   * and what it does is **reclaim**: drop the assignee, then close again. Adam's
   * decision, and the reason it is not `--force` is the sentence above `CLAIM_RE`.
   *
   * **The reclaim is lazy, and that is the only place this departs from bc-ko7n's
   * wording.** The bead describes a third write on every answer; here the clear happens
   * only on the beads that actually carry a claim, because a close that was never going
   * to be refused buys nothing with it. It matters most away from the answer path: one
   * lib/landed.js sweep closes every bead of every merged pull request, and a write per
   * bead against a single-writer Dolt is a cost nobody asked for. Order is unchanged for
   * the close that lands — the assignee is gone before it — and the first attempt costs
   * nothing, since `run` retries only on LOCK_RE and a refusal comes straight back.
   *
   * `--assignee ''` alone, not the reopen path's `--status open --assignee ''`: this is
   * on its way to closing the bead, so the status is not its business. Verified against
   ...
   * this now recovers from would make every claimed bead read as unclosable — the
   * "inventing a gate" failure test/closegatereal.mjs exists to catch. That file pins
   * the agreement instead: gate silent, close succeeds.
   */
  async runClose(workspace, id, reason, { actor = null } = {}) {
    const args = ['close', id, '--reason', reason];
    try {
      return await this.run(workspace, args, { retries: 4, actor });
    } catch (err) {
      if (!CLAIM_RE.test(String(err?.message || ''))) throw err;
   ...
   * the assignee/actor mismatch, which `runClose` reclaims past; that one is not a gate
   * at all, it is beadcause's own byline being read as somebody else. See `CLAIM_RE`.
   *
  ...
  async respond(workspace, id, response, { actor = null } = {}) {
    ...
    await this.runClose(workspace, id, 'Answered via Beadcause', { actor });
  ...
  async close(workspace, id, reason, { actor = null } = {}) {
    await this.runClose(workspace, id, reason, { actor });
   ...
   *
   * One `bd export`, shared with `graph` — which the P0 board, lib/homing.js and the
  ...
  async create(
    ...
    /**
     * A P0 filed by this daemon is this Mac's person's P0.
     *
     ...
     * of bead a phone rings about, and an owner is stamped on a **P0**, because that is
     * the kind of bead somebody has to be answerable for. The two are independent and a
     * bead can carry both — a P0 question is a decision you own.
     *
     ...
     * see lib/ownership.js for why an unowned P0 is the state bc-rfnr.5 exists to clear.
     *
     * **An owner the caller named wins.** A P0 filed *for* somebody else — the triage
     ...
     */
    const mine = isP0({ priority }) && !ownersOf(labels).length ? ownOwnerLabels({ me: this.me }) : [];
     ...
     * in the index finds no ancestors — so for that minute `hasP0Above` answers **false**
     * about a bead that was deliberately given a P0 above it, and lib/underp0.js says so
     * out loud: a skipped tick, a bus event and a `no P0 above this` pill making a claim
     ...
     * second or two before the rebuild arrives, `p0Board` has no shape and answers an
     * empty `p0s`, which the client already treats as "do not narrow anything" — the
   ...
   *
   * The one write bc-rfnr.7's refusal can be answered with. A bead with no P0 above it is
   * not workable, the sheet offers a P0 to adopt it, and this is what that tap runs; an
  ...
  async ready(workspace, { excludeLabels = null, excludeLabel = null } = {}) {
    ...
    const labels = [...new Set([...asked, UNENDORSED, SHIP_LABEL])];
   ...
   * P0 section, and then it does. `owned: true` with an empty `p0s` is a state the client
   * already treats as "do not narrow anything" (`underOwnedP0s`), so a warming daemon
   ...
   * every caller — which is fine for the P0 board, where the honest reading of both is
