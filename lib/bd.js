...
export class Bd {
   ...
   *
   * **Cached across repaints, per workspace, for a minute.** The inbox repaints every 25
   * seconds and this must not cost a `bd` spawn each time; sixty seconds is two of those
   * plus room, and the thing being cached moves only when somebody deliberately reparents
   * a bead. A stale map for under a minute draws one card in the wrong section — a map
   * rebuilt per repaint would put a second-and-a-third of `bd` on the request path.
   *
   ...
   */
  async graph(workspace, { refresh = false } = {}) {
    const key = workspace?.name || workspace?.dir || '';
    const hit = PARENT_CACHE.get(key);
    if (!refresh && hit && Date.now() - hit.at < PARENT_TTL_MS) return hit.index;
    let index = { parents: new Map(), beads: new Map() };
    try {
      index = indexFrom(await this.run(workspace, ['export'], { retries: SWEEP_RETRIES }));
    } catch (err) {
