...
import { mentionsIn, planFor, prefixOf } from './mentions.js';
...
const EMPTY_GRAPH = { parents: new Map(), beads: new Map() };
...
export class Bd {
  ...
  async create(
     ...
     */
    if (id) await this.relateMentions(workspace, id, [title, body, acceptance, design, notes].filter(Boolean).join('\n\n'));
    return id;
  }
   ...
   */
  async adopt(workspace, id, parent, { actor = null } = {}) {
    await this.run(workspace, ['update', id, '--parent', String(parent || '')], { retries: 4, actor });
    await this.graph(workspace, { refresh: true }).catch(() => {});
  }
  ...
  graph(workspace, { refresh = false, wait = true } = {}) {
    ...
    if (!job) {
      job = this.run(workspace, ['export'], { retries: SWEEP_RETRIES })
        .then(indexFrom)
        .catch((err) => {
          console.error(`[beadcause] could not read the shape of ${key} — ${err.message}`);
          ...
          // next success. Empty only when there has never been a good answer.
          return hit?.index || EMPTY_GRAPH;
        })
        .then((index) => {
  ...
  async relateMentions(workspace, id, text) {
    try {
      ...
      const made = [];
      for (const to of planFor({ id: self, prose: text, linked })) {
        try {
