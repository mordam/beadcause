...
export class Bd {
  ...
  /**
   * The hierarchical children of an issue — every status, closed ones included.
   *
   * Two callers want the same list for opposite reasons: `gateFor` above, to decide
   * whether an epic is a question or a question's future, and lib/advocate.js, to
   * decide whether an epic is work or the *sum* of work already in its queue. Both
   * need "are any of these still open", so neither may filter here.
   *
   * `--limit 0` for the reason `ready` above gives: bd's default of 100 turns a
   * thirty-child epic into a right answer and a hundred-child one into a wrong one,
   * and the wrong one is silent.
   */
  async children(workspace, id) {
    return (await this.json(workspace, ['list', '--parent', id, '--limit', '0'])) || [];
  }
