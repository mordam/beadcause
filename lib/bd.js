...
export class Bd {
  ...
  /**
   * File a new `human` question and return its id.
   *
   * Same shape as `bin/ask.js`, but the body arrives over HTTP rather than a pipe,
   * so there's no shell to quote a fenced decision block past.
   */
  async create(
    workspace,
    { title, body = '', priority = 1, type = 'task', labels = ['human'], acceptance = '', design = '', notes = '', parent = '' }
  ) {
    const args = ['create', '--title', title, '--type', type, '--priority', String(priority)];
    for (const label of labels) args.push('--label', label);
    if (body) args.push('--description', body);
    // The rest are what the console's proposal can carry beyond a question's
    // title-and-body: `bd lint` checks for acceptance, and a bead created without
    // its parent has to be re-parented by hand afterwards.
   ...
   *
   * The console's proposals may name real beads in `dependsOn` — that is how "this
   * new work waits on the one we started from" is written — and a ref and an id are
   * not reliably distinguishable by shape. So the tracker is asked rather than the
   * regex trusted, and an id that isn't there is reported instead of failing the
   * whole create at `bd dep add`.
   */
