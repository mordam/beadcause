...
/* ================================== the space is the unit, and stays it per repo
   ...
   (`autoEndorse` also takes a per-*workspace* override, `autoEndorsePerWorkspace` at the
   top of this file — which is a different question from the one settled here and settled
   the same way. It says one workspace in a space may answer for itself; it says nothing
   about a checkout inside a workspace, which is what the four reasons below refuse.)
 ...
 *
 * **And a single workspace outranks the space.** This is the one setting here that does
 * not group, and the shape of the config is what proved it: the two spaces on this Mac
 * are Personal — beadcause, deluvia, ehatt, sophab and two more — and Climative. Turning
 * the hold off for beadcause, whose tracker nobody else reads, meant turning it off for
 * five repos nobody had asked about, because the space was the finest thing that could
 * answer. `autoEndorsePerWorkspace.<repo>` is that answer, and it is a boolean rather
 * than a list because both directions have to be sayable: a repo held inside a space
 * that endorses is the same kind of exception as the other way round, and a list can
 * only ever say one of them.
 *
 * Three levels, most specific first, and each is skipped by anything that is not a real
 * boolean — a hand-edited `"true"` asked for nothing legible, and the hold is the safe
 * reading of an unreadable one. This is the only resolution path; every caller and
 * every screen goes through here, so what the space details card draws per repo and what
 * `bin/file.js` does are the same answer by construction.
 *
...
export function autoEndorseAllowed(cfg, workspaceName) {
  const own = cfg?.autoEndorsePerWorkspace?.[workspaceName];
  if (typeof own === 'boolean') return own;
  const space = spaceFor(cfg, workspaceName);
  if (typeof space?.autoEndorse === 'boolean') return space.autoEndorse;
  return cfg?.autoEndorse === true;
}
 ...
 *
 * Not `autoEndorseAllowed` on a doctored config: the row is drawn from a live `cfg` the
 * running daemon reads on every push, and cloning it per repo per repaint to ask a
 * hypothetical would be a copy of the whole config in the render path. The two are one
 * function apart and the tail of the resolution above is the whole of it.
 */
...
export const WORKSPACE_SETTINGS = ['autoEndorse'];
...
export function readWorkspaceSettings(cfg, workspaceName) {
  const own = cfg?.autoEndorsePerWorkspace?.[workspaceName];
  return { autoEndorse: typeof own === 'boolean' ? own : null };
}
 ...
 * Inherit button.
 *
 * The map is created on demand and left in place once empty, rather than deleted: it is
 * a config default (`autoEndorsePerWorkspace: {}` in lib/config.js), so an absent one and
 * an empty one already mean the same thing to every reader here.
 *
...
export function applyWorkspaceSettings(cfg, workspaceName, patch) {
  ...
  const map =
    cfg.autoEndorsePerWorkspace && typeof cfg.autoEndorsePerWorkspace === 'object' && !Array.isArray(cfg.autoEndorsePerWorkspace)
      ? cfg.autoEndorsePerWorkspace
      : (cfg.autoEndorsePerWorkspace = {});

  // One key today, and the loop stays a loop anyway: the next per-repo override is a
  // line in `WORKSPACE_SETTINGS` and a map of its own, not a rewrite of this.
 ...
 *   you will get.
 *
 *   `autoEndorse` is the one that is also *settable* from here — see
 *   `autoEndorsePerWorkspace` at the top of this file — so its row carries three fields
 *   rather than one: the resolved answer, what this repo itself says, and what Inherit
 *   would mean. That is the same trio the space rows are drawn from, one level down.
 *
 ...
 * the one settable row above sound: it writes a workspace's answer, which is the grain the
 * override is keyed at. The space is deliberately still the unit these answers are given
 * at — `autoEndorse` excepted, and only ever between workspaces rather than inside one
 * (see the block above `autoDispatchAllowed`); what a screen may not do is let one row
...
export function spaceDetail(cfg, name, now = new Date()) {
  ...
  return {
    ...
    repos: inside.map((w) => ({
      ...
      // The only one of these the row can *set*, so it is the only one that needs the
      // other two claims a three-state control is made of: what this repo itself says
      // (`null` for nothing, which is what puts Inherit on), and what Inherit would
      // resolve to today. Everything else here is a resolved answer and nothing else.
      autoEndorseOwn: readWorkspaceSettings(cfg, w).autoEndorse,
      autoEndorseInherited: autoEndorseInherited(cfg, w),
