 ...
 *
 * The last three are not about interruption, and they are here anyway: whether a bead an
 * agent filed may be worked without you having looked at it, whether a worker merges its
 * own pull request, and whether an approving review is required before it may, are the
 * same kind of answer as "may an agent reply to this unasked" — one you give once for a
 * group of repos rather than per repo. See `autoEndorseAllowed` and `prPolicyFor` below.
 */
...
export function applySettings(space, patch) {
  ...
  for (const [key, raw] of Object.entries(patch)) {
    ...
    if (key === 'quietHours') {
      ...
      // The five booleans — muted, autoDispatch, autoEndorse, autoMerge, requireApproval.
      if (clear) {
        delete space[key];
        continue;
      }
 ...
 * configure it has to say so rather than draw seven controls that write nowhere.
 *
 ...
 *   the same four resolvers every other caller uses. It exists because the space is
