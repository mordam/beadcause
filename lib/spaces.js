...
// The level above a space: which of your lives the whole app is currently about. Imports
// only lib/addressee.js, which imports nothing, so this cannot start a cycle either.
import { inAccount } from './accounts.js';
...
/**
 * The first segment that marks an address as scoped — `/bdcoz/personal/deluvia`.
 *
 * Said twice, here and as `SCOPE_ROOT` in public/hashroute.js, because one of them runs
 * in a browser off `window` and the other in the daemon off an import, and there is no
 * module both can read. test/spacepaths.mjs holds the two against each other, which is
 * the same arrangement `VIEW_HOPS` in public/sw.js has with `serveStatic`'s run of `if`s
 * and for the same reason: two copies that cannot drift are cheaper than a shared module
 * that has to be loadable from both sides.
 */
export const SCOPE_ROOT = 'bdcoz';

/**
 * A space's name as it appears in an address — `Personal` becomes `personal` (bc-xnj67).
 *
 * A space is named by a person, in a config file, for a row of chrome: `Personal`,
 * `Climative`, and one day something with a space in it. A URL segment is typed, pasted
 * into messages and held on a phone's home screen, so it wants the flattened form — and
 * the two must not become independent names for one thing, which is why this is a
 * function of the name rather than a second field somebody has to keep in step.
 *
 * Deliberately lossy and deliberately not inverted: `spaceBySlug` matches by running
 * every configured name through here rather than trying to reverse it. Two spaces whose
 * names flatten to the same slug are a collision the first one wins, which is the answer
 * `route.add` gives a duplicate view id and for the same reason — a later entry that
 * could take an earlier one's address is worse than a name that has to change.
 *
 * Workspace names need none of this. They come from directory names under `~/beads/` and
 * are already the flattened form: `beadcause` and `deluvia` are what they are called
 * everywhere, this included.
 */
export const spaceSlug = (name) =>
...
/** The configured space one address segment names, or `null` for a segment naming none. */
export function spaceBySlug(cfg, slug) {
  const want = spaceSlug(slug);
  if (!want) return null;
  return (cfg?.spaces || []).find((s) => spaceSlug(s.name) === want) || null;
}
...
export const PER_WORKSPACE = {
  ...
  reviewRequired: 'reviewRequiredPerWorkspace',
  autoShip: 'autoShipPerWorkspace',
  trustChecksAcrossDownmerge: 'trustChecksAcrossDownmergePerWorkspace',
};
...
const GLOBAL_DEFAULT = {
  ...
  /**
   * Off, everywhere, until a workspace says otherwise — and it is the one default here
   * that would be dangerous the other way round.
   *
   * The review gate *holds* a pull request until a verdict appears (lib/reviewgate.js),
   * so on in a workspace where no ReviewAdvocate window ever opens is not a slower queue
   * but a stopped one, on every branch at once, silently. Turn it on per repo once
   * something is actually reviewing there.
   */
  reviewRequired: (cfg) => cfg?.pr?.reviewRequired === true,
  autoShip: (cfg) => cfg?.release?.autoShip === true,
  /**
   * Off everywhere until a repo asks for it, and the argument for that default is
   * bc-rcrt's own.
   *
   * `.github/workflows/test.yml` exists because two branches that each pass alone can
   * still break `main` together — the merge is a third thing neither of them tested. The
   * queue's downmerge is where that gets tested, and `trustChecksAcrossDownmerge` is a
   * repo saying *not here*: judge this branch on the checks it already had rather than
   * waiting for them to run again against the base that just arrived.
   *
   * Which is a real risk taken deliberately, so it is a repo's to take rather than an
   * install's. Two things make it worth taking in beadcause and neither is guaranteed
   * elsewhere, which is exactly why this is a setting and not a change of behaviour:
   *
   * - **The combination is still tested, afterwards.** The `test` job runs on `push` to
   *   `main` as well, so a bad pairing reddens the base within minutes — and lib/redbase.js
   *   already holds the whole queue on a red base and files the fix (bc-6xr47). The check
   *   moves from before the merge to after it; it does not go away. A repo whose CI does
   *   *not* run on the base is one where this genuinely removes the check, and that repo
   *   should leave this off.
   * - **The downmerge is usually trivial where bc-8479e's union merge applies.** 30 of 34
   *   open branches merged onto `main` with no conflict at all when that was measured, and
   *   such a downmerge is git keeping both sides of an appended line. Paying a full gate
   *   run to re-prove a one-line list append is the cost this removes.
   *
   * **It never applies to a conflicted downmerge.** A conflict still goes to a resolver,
   * whose own brief still tells it to run the repo's gate — see `conflictPromptFor` in
   * lib/session.js, step 5. This is only about the *clean* case.
   *
   * ## Where it can fire at all, which is narrower than it sounds — bc-xl7n.121
   *
   * The arm this answers for tests `mergeStateStatus === 'BEHIND'`, and GitHub only ever
   * reports that value **under a branch-protection rule requiring branches to be up to
   * date**. `mordam/beadcause` has no protection rule at all, so on this repo that arm has
   * never fired: a green pull request here is judged and merged without any downmerge
   * being asked for, and there is no wait for this to shorten. bc-xl7n.121 found that the
   * hard way — it is why `verdict.stale` takes its own reading from the checkout with
   * `behindBase` rather than trusting the field.
   *
   * So this is **off everywhere, including here**, and the honest description of it is a
   * setting for a repo whose GitHub enforces up-to-date branches. Do not read it as
   * something beadcause is using.
   *
   * **And deliberately not wired into the `verdict.stale` path**, which is the one place a
   * downmerge really is asked for on an unprotected repo. The checks there are stale in
   * the precise sense that they are a verdict on a base that no longer exists — bc-91srt,
   * #475 and #488 — so they are exactly the checks that must not be trusted across a
   * downmerge. The one path where this would fire is the one path where it would be wrong.
   */
  trustChecksAcrossDownmerge: (cfg) => cfg?.pr?.trustChecksAcrossDownmerge === true,
};
...
export function prPolicyFor(cfg, workspaceName) {
  return {
    ...
    /**
     * Must an *agent* have reviewed it — bc-36xx.4, lib/reviewgate.js.
     *
     * A third answer beside the two rather than a mode of `requireApproval`, because the
     * two gates are in series and neither substitutes for the other: Adam's answer to
     * bc-0cop is that where a space asks for his approval the ReviewAdvocate's is
     * necessary and *not* sufficient, and everywhere else the agent's approval alone
     * releases the pull request. Written as one flat boolean for the reason the other two
     * are: it is read on the queue's own tick, and a policy that had to be derived from
     * two others is one that disagrees with itself at exactly the edges it was added for.
     */
    reviewRequired: workspaceAnswer(cfg, workspaceName, 'reviewRequired'),
    /**
     * May the queue judge a clean downmerge on the checks it already had — bc-kbvhg.
     *
     * Read on the queue's own tick like the three above it, and a flat boolean for the
     * same reason: lib/mergequeue.js asks this at the one moment the answer matters, in
     * the middle of the Behind branch, and a policy derived from two others is one that
     * disagrees with itself at exactly the edge it was added for.
     *
     * `GLOBAL_DEFAULT` carries the whole argument for it, including what makes the trade
     * safe here and what would make it unsafe elsewhere.
     */
    trustChecksAcrossDownmerge: workspaceAnswer(cfg, workspaceName, 'trustChecksAcrossDownmerge'),
  };
}
...
/**
 * Why this bead is not allowed to make a noise — `'addressed'`, `'account'`,
 * `'filtered'`, `'muted'`, or null when it may.
 *
 ...
 *
 * **`'account'` is next, and it sits above the filter for the same reason `'addressed'`
 * sits above both** (bc-exqi). An account is the level above a space — which of your
 * lives the app is currently about, see lib/accounts.js — and a bead in the other one
 * will not ring however far you widen the space picker, because that picker does not
 * offer the other account's repos at all. Reporting it as `'filtered'` would send you to
 * press **All** for a bead that pressing All cannot reach. Nothing is lost: the bead is
 * still swept, still filed, still counted, and it is on the screen the moment you switch
 * account — which is the same contract a mute and a filter already have.
 *
 * **A foundation request is exempt from it, exactly as it is exempt from the filter**,
 * and the exemption is load-bearing rather than symmetric: the inbox draws that channel
 * above the list and outside every scope on the page, so an account that silenced one
 * would buy the single state this contract exists to prevent — a request sitting visible
 * on the screen and silent on the phone. The two halves have to agree, so if the payload
 * ever starts scoping requests by account, this line changes with it.
 *
 ...
 * above the list and outside every filter on it (`requestsHtml` in public/app.js).
 * The other surface already said the filter does not reach this channel; the push was
 * the outlier, and honouring the filter here bought the
 ...
 */
export function quietReasonFor(cfg, filter, q, now = new Date(), account = null) {
  if (addressedElsewhere(cfg, q)) return 'addressed';
  if (!q?.foundation && !inAccount(account, q?.workspace)) return 'account';
...
/* ======================================================================= settings
   ...
   Every field above is something you set by opening `~/.config/beadcause/config.json` in an
...
export function readSettings(space) {
  ...
  return {
    ...
    reviewRequired: bool(space?.reviewRequired),
    autoShip: bool(space?.autoShip),
    trustChecksAcrossDownmerge: bool(space?.trustChecksAcrossDownmerge),
  };
}
...
export function applySettings(space, patch) {
  ...
  for (const [key, raw] of Object.entries(patch)) {
    ...
    if (key === 'quietHours') {
      ...
      // The eight booleans — muted, autoDispatch, autoEndorse, autoMerge,
      // requireApproval, reviewRequired, autoShip, trustChecksAcrossDownmerge.
      if (clear) {
        delete space[key];
        continue;
      }
...
export function spaceDetail(cfg, name, now = new Date()) {
  ...
  /* Retired on purpose — `workspaceDirs.<name>` is null, which lib/workspaceroots.js
     reads as "never serve this one". Told apart from drift because the two want opposite
     things said about them: drift is a checkout that went somewhere without telling
     anybody and is worth a warning, and this is a decision somebody made from the admin
     screen, which a warning would nag about forever.

     The name stays in the space deliberately. Retiring writes one key and nothing else,
     so bringing the repo back is deleting that key — and it lands back in the space it
     was always in, with its settings intact. Dropping it from `workspaces` here would
     mean a bring-back had to remember which space it came from, and getting that wrong
     puts the repo in "Other" without saying so. */
  const retiredNames = new Set(
    Object.entries(cfg?.workspaceDirs || {})
      .filter(([, dir]) => dir === null)
      .map(([name]) => name)
  );
  const absent = (space.workspaces || []).filter((w) => !configured.has(w));
  const retired = absent.filter((w) => retiredNames.has(w));
  ...
  return {
    ...
    missing: absent.filter((w) => !retiredNames.has(w)),
    /** Named by this space, deliberately not served. Absent on an install that has
     *  retired nothing, so an older client draws exactly what it always did. */
    ...
    repos: inside.map((w) => ({
      ...
      // The five a row can *set* need the other two claims a three-state control is made
    ...
    defaults: {
      ...
      // Off unless asked for, and an Inherit button here has to say so loudly: with
      // nothing reviewing in a workspace, on is a queue that never merges again. See
      // `GLOBAL_DEFAULT` above and lib/reviewgate.js.
      reviewRequired: cfg?.pr?.reviewRequired === true,
      ...
      // Off unless asked for, and an Inherit button here reading "Inherit (off)" is the
      // honest answer on a fresh install: waiting for the re-run is what the queue has
      // always done. See `GLOBAL_DEFAULT` above for what taking the other side costs.
      trustChecksAcrossDownmerge: cfg?.pr?.trustChecksAcrossDownmerge === true,
    },
  };
}
