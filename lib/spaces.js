 ...
 *
 * **`autoEndorse` is the exception to that last sentence, and it is deliberate.** The
 * others group cleanly: "don't buzz me about work at the weekend" and "don't land a diff
 * other people read without eyes on it" are properties of a *set* of repos, and a space
 * is that set. The hold is not. The reason to drop it — I am the only reader of this
 * tracker and the tap is not a review — is a fact about one repo, and it stops being
 * true of the repo beside it in the same space without either of them moving. So it has
 * a per-workspace override that outranks the space, the way `ntfy.minimalWorkspaces` and
