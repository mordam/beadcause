 ...
 *
 * The last two are not about interruption, and they are here anyway: whether a worker
 * merges its own pull request, and whether an approving review is required before it
 * may, are the same kind of answer as "may an agent reply to this unasked" — one you
 * give once for a group of repos rather than per repo. See `prPolicyFor` below.
 */
...
export function applySettings(space, patch) {
  ...
  for (const [key, raw] of Object.entries(patch)) {
    ...
    if (key === 'quietHours') {
      ...
      // The four booleans — muted, autoDispatch, autoMerge, requireApproval.
      if (clear) {
        delete space[key];
        continue;
      }
