 ...
 *
 * The last three are not about interruption, and they are here anyway: whether a worker
 * merges its own pull request, whether an approving review is required before it may,
 * and whether the merge then deploys itself, are the same kind of answer as "may an
 * agent reply to this unasked" — one you give once for a group of repos rather than per
 * repo. See `prPolicyFor` and `autoShipAllowed` below.
 */
...
export function applySettings(space, patch) {
  ...
  for (const [key, raw] of Object.entries(patch)) {
    ...
    if (key === 'quietHours') {
      ...
      // The five booleans — muted, autoDispatch, autoMerge, requireApproval, autoShip.
      if (clear) {
        delete space[key];
        continue;
      }
 ...
 * configure it has to say so rather than draw eight controls that write nowhere.
 *
 ...
 *   the same five resolvers every other caller uses. It exists because the space is
