 ...
 * panel that used to count one repo per row counts those instead. The space is
 * deliberately still the unit these answers are given at (see the block above
 * `autoDispatchAllowed`); what a screen may not do is let one row labelled `climative`
 * read as one checkout when the answer beside it governs forty.
 */
