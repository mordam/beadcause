...
/* ===================================================================================
  ...
  // b7e-signoff (bc-dgx7.44) is deliberately NOT on this list, for the b7e-apply reason
  // rather than a read-only one: its whole job is `node bin/deliver.js` — pushing a
  // branch, opening a pull request, filing a merge-bead — plus a debrief write and a
  // rename of the calling session, none of which is a read by any stretch. Its whole
  // occasion is a worker ending a session it has been writing code in all along;
  // `dispatch`, the one agent this list governs, has no branch of its own, delivers
  // nothing, and would only ever reach this command to end a session it never opened.
  // See bin/b7e-signoff and lib/signoff.js.
