/**
 * The `b7e-*` half of the read-only surface, read from the tools themselves.
 *
 * `lib/toolbelt.js` holds what a person wrote down by hand — the `bd` verbs, the four
 * shapes of looking something up, the one write. This file reads `bin/` and appends the
 * `b7e-*` tools that declare themselves granted, and `DEFAULT_TOOL_LIST` is the two
 * together. It is the one home of that list: `lib/foundation.js` records it as the
 * dispatch agent's baseline and `lib/agents.js` re-exports it, exactly as they did when
 * it was a literal.
 *
 * ## Why it is derived — bc-wbrhi
 *
 * Adding one `b7e-*` tool used to mean a line in `lib/grants.js`, seven in
 * `lib/toolbelt.js`, one in `package.json` and one more in `package-lock.json`, on top of
 * the three files that were actually new. Every one of those lines went in at the same
 * place in its file, so every branch in flight was appending at the same anchor: on
 * 2026-08-26, **ten of the fourteen open pull requests touching `lib/grants.js` were
 * inserting at line 333**, which means one of them landing conflicted the other nine, by
 * construction rather than by luck. bc-8479e made those conflicts merge themselves with
 * `merge=union`; this makes them not happen.
 *
 * A tool now says what it is in its own header, once:
 *
 *     * @grant read        it is on the list, and lib/grants.js classifies it a read
 *     * @grant write       it is on the list, and lib/grants.js classifies it a write
 *     * @grant excluded    it is deliberately NOT on the list
 *
 * and nothing else has to be edited. The pattern is built from the filename rather than
 * written out, because `Bash(b7e-window:*)` typed by hand is one more thing that can be
 * typed wrong, and a grant that does not match the command it is for is a grant for
 * nothing.
 *
 * ## `excluded` is a decision, not an absence
 *
 * Twenty-nine of these tools are deliberately off the list, each with a paragraph saying
 * what it writes or spawns, and `bin/b7e-enroll` has always treated that decision as
 * exactly as real as being on the list — a tool is unpaid only when **neither** is true.
 * It used to establish that by looking for the tool's name anywhere in the array's
 * comments, which is why naming a tool in passing could read as a decision about it (the
 * `b7e-hunks` paragraph below says so in as many words). `@grant excluded` is that same
 * decision written down where it cannot be made by accident.
 *
 * Two tools have no declaration at all — `b7e-packet` and `b7e-say` — because nobody has
 * decided about them. `b7e-enroll` reported both as unpaid before this change and reports
 * both as unpaid after it. Nothing here decides on their behalf.
 *
 * ## Why this is a module of its own and not part of `lib/toolbelt.js`
 *
 * Because `lib/toolbelt.js` is tagged `@manifest`, and `manifestProblems` in
 * `lib/affected.js` refuses that tag to any file that imports something or declares a
 * function. Reading `bin/` needs both. The tag is not decoration: bc-xlz32.7 measured
 * that adding a b7e tool used to select **205 suites**, 196 of them only because
 * `toolbelt.js` sits underneath nearly everything, and the tag is what took a narrowed
 * gate from a median 30% of the suite to 2%. Doing the scan there would have bought
 * bc-wbrhi by handing back bc-xlz32.7.
 *
 * ## The blind spot this has, said out loud
 *
 * `lib/affected.js` reads imports. This reads the filesystem, and no static analysis can
 * see that edge — so editing a `@grant` line inside an existing `bin/b7e-*` file changes
 * what this module produces and narrows to that tool's own suite rather than to
 * `test/tooldecl.mjs`. Adding or removing a tool is fine, because the file itself is new
 * or gone; it is the *edit in place* that is invisible. The full gate covers it, and
 * `test/tooldecl.mjs` names this file so anything touching **this** file still selects it.
 *
 * ## Failing soft, in the safe direction
 *
 * A `bin/` that cannot be read, or a declaration that makes no sense, produces **fewer**
 * grants rather than more — the list shrinks toward the hand-written base, so an agent
 * loses a capability instead of gaining one. That is the right way round, and it is still
 * not good enough on its own, because a silently shorter allowlist is a mystery at the far
 * end. So every problem is collected in `DECLARATION_PROBLEMS` and `test/tooldecl.mjs`
 * fails the repo when it is not empty.
 */
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

import { BASE_TOOL_LIST } from './toolbelt.js';

const HERE = path.dirname(fileURLToPath(import.meta.url));

/** The three things a declaration can say. Anything else is a problem, not a default. */
export const KINDS = Object.freeze(['read', 'write', 'excluded']);

/**
 * The declaration line, anywhere in a file: ` * @grant read`.
 *
 * Anchored to the start of a line and allowed only comment furniture before it, so a
 * `@grant` written mid-sentence in prose is not mistaken for one. Deliberately not
 * restricted to the header docblock — `declarationsIn` counts every match and refuses a
 * file with two, which is the failure that restriction would have been guarding against
 * and it catches the loose one as well.
 */
const GRANT_RE = /^[ \t]*(?:\*|\/\/)?[ \t]*@grant[ \t]+(\S+)[ \t]*$/gm;

/** The command name a file in `bin/` is invoked as — `b7e-owes.js` is run as `b7e-owes`. */
export const commandName = (file) => file.replace(/\.js$/, '');

/**
 * Every `@grant` in one file's text, as written.
 *
 * Returns the raw strings rather than validating, so the caller can say *which* file
 * carries a declaration it does not understand instead of quietly dropping it.
 */
export function declarationsIn(text) {
  return [...String(text ?? '').matchAll(GRANT_RE)].map((m) => m[1]);
}

/**
 * Read `bin/`, and answer with both halves: what was declared, and what could not be read.
 *
 * `dir` is a parameter so the suite can drive it over a made-up directory — the same
 * arrangement `grantProblems` has in lib/grants.js, and for the same reason: a check that
 * can only be run against the real repo can only ever be shown passing.
 */
export function readDeclarations(dir) {
  const declared = [];
  const problems = [];
  let files;
  try {
    files = fs.readdirSync(dir).filter((f) => /^b7e-/.test(f)).sort();
  } catch (err) {
    return { declared, problems: [`bin/ could not be read (${err.code || err.message}), so no tool is granted`] };
  }
  for (const file of files) {
    ...
    try {
      ...
      problems.push(`bin/${file} could not be read (${err.code || err.message})`);
      continue;
    }
    const found = declarationsIn(text);
    if (!found.length) continue; // undeclared is a state b7e-enroll reports, not a problem here
    if (found.length > 1) {
      problems.push(`bin/${file} declares @grant ${found.length} times (${found.join(', ')}) — a tool has one answer`);
      continue;
    }
    const [kind] = found;
    if (!KINDS.includes(kind)) {
      problems.push(`bin/${file} declares @grant ${kind}, which is not one of ${KINDS.join(', ')}`);
      continue;
    }
    declared.push({ name: commandName(file), file, kind });
  }
  const seen = new Set();
  for (const d of declared) {
    if (seen.has(d.name)) problems.push(`${d.name} is declared by more than one file in bin/`);
    seen.add(d.name);
  }
  return { declared, problems };
}

const read = readDeclarations(path.join(HERE, '..', 'bin'));

/** Every `b7e-*` tool that declared itself, whichever way it declared. */
export const DECLARATIONS = Object.freeze(read.declared.map((d) => Object.freeze(d)));

/** Everything wrong with the declarations. `test/tooldecl.mjs` asserts this is empty. */
export const DECLARATION_PROBLEMS = Object.freeze(read.problems);

/** The tools that are on the list, as `lib/grants.js` classifies them. */
export const B7E_GRANTS = Object.freeze(
  Object.fromEntries(
    DECLARATIONS.filter((d) => d.kind !== 'excluded').map((d) => [`Bash(${d.name}:*)`, Object.freeze({ kind: d.kind })])
  )
);

/** The tools a declaration deliberately keeps off the list, by name. */
export const EXCLUDED = Object.freeze(DECLARATIONS.filter((d) => d.kind === 'excluded').map((d) => d.name));

/**
 * The read-only surface every reply agent gets: what was written by hand, then what the
 * tools declared.
 *
 * The hand-written half keeps the order its arguments were made in; the derived half is
 * in filename order, because that is the only order `bin/` has and inventing another
 * would be one more thing to keep in step. Nothing depends on either — the list is a set
 * that happens to be written as an array, and `DEFAULT_TOOLS` joins it for a command line.
 */
export const DEFAULT_TOOL_LIST = Object.freeze([...BASE_TOOL_LIST, ...Object.keys(B7E_GRANTS)]);
...
/* ===================================================================================
   The argument — why each of these is on the list, and why twenty-nine are not.

   Moved here whole from lib/toolbelt.js by bc-wbrhi, in the order it was written and
   with not one word of it rewritten. That mattered more than tidiness: twenty-six of
   the thirty-four blocks name a sibling tool and seventeen of them do it positionally
   ("just above", "the four above", "beside them"), so the run survives being moved
   together and would not have survived being cut into sixty-three pieces and filed one
   per tool. The two-space indent is what it had inside the array; it is left alone for
   the same reason.

   Each block is followed by the line it used to sit above, now saying where the entry
   actually comes from. Those arrow lines are the only text here that is not original.
   =================================================================================== */
  ...
  //   → Bash(b7e-import:*) — declared `@grant read` in bin/b7e-import
  ...
  // See bin/b7e-owes.js.
  //   → Bash(b7e-owes:*) — declared `@grant read` in bin/b7e-owes
  ...
  //   → Bash(b7e-affected:*) — declared `@grant read` in bin/b7e-affected
  ...
  //   → Bash(b7e-notes:*) — declared `@grant read` in bin/b7e-notes
  ...
  //   → Bash(b7e-readme:*) — declared `@grant read` in bin/b7e-readme
  ...
  //   → Bash(b7e-ws:*) — declared `@grant read` in bin/b7e-ws
  ...
  // and lib/siblings.js.
  //   → Bash(b7e-siblings:*) — declared `@grant read` in bin/b7e-siblings
  ...
  //   → Bash(b7e-base:*) — declared `@grant read` in bin/b7e-base

  // Read-only in the same sense as b7e-base just above, and built on the same
  // lib/gitref.js primitives plus lib/prbase.js's baseFor (already used read-only by
  // bin/deliver.js itself before it ever writes anything) — a fetch, then rev-parse/
  // rev-list/ls-tree/show, never a write. bc-dgx7.58 is the argument: six sessions in
  // the deluvia workspace each worked out by hand, six different ways, that the
  // checkout's own branch is not what beadcause actually delivers into. See
  // bin/b7e-deliverbase and lib/prbase.js.
  //   → Bash(b7e-deliverbase:*) — declared `@grant read` in bin/b7e-deliverbase
  ...
  // lib/census.js.
  //   → Bash(b7e-census:*) — declared `@grant read` in bin/b7e-census
  ...
  //   → Bash(b7e-count:*) — declared `@grant read` in bin/b7e-count
  ...
  //   → Bash(b7e-cites:*) — declared `@grant read` in bin/b7e-cites
  ...
  //   → Bash(b7e-moment:*) — declared `@grant read` in bin/b7e-moment
  ...
  // See bin/b7e-orient.
  //   → Bash(b7e-orient:*) — declared `@grant read` in bin/b7e-orient
  ...
  //   → Bash(b7e-prior:*) — declared `@grant read` in bin/b7e-prior
  ...
  // lib/precedent.js.
  //   → Bash(b7e-precedent:*) — declared `@grant read` in bin/b7e-precedent
  ...
  //   → Bash(b7e-harness:*) — declared `@grant read` in bin/b7e-harness
  ...
  //   → Bash(b7e-pinned:*) — declared `@grant read` in bin/b7e-pinned
  ...
  //   → Bash(b7e-brief:*) — declared `@grant read` in bin/b7e-brief
  ...
  //   → Bash(b7e-known:*) — declared `@grant read` in bin/b7e-known
  ...
  //   → Bash(b7e-premerge:*) — declared `@grant read` in bin/b7e-premerge
  ...
  // bin/b7e-enroll.
  //   → Bash(b7e-enroll:*) — declared `@grant read` in bin/b7e-enroll
  ...
  //   → Bash(b7e-gated:*) — declared `@grant read` in bin/b7e-gated
  ...
  //   → Bash(b7e-entry:*) — declared `@grant read` in bin/b7e-entry
  ...
  //   → Bash(b7e-propagated:*) — declared `@grant read` in bin/b7e-propagated
  ...
  //   → Bash(b7e-stillred:*) — declared `@grant read` in bin/b7e-stillred
  ...
  // b7e-detach (bc-dgx7.25) is deliberately NOT on this list either, for the b7e-gate
  // reason exactly rather than a new one: what it starts *is* a b7e-gate run, and starting
  // one in a session of its own rather than in the foreground does not make it a lighter
  // capability than the `Bash(npm test:*)` lib/grants.js already holds to merge-advocate
  // alone — if anything it is heavier, since the run outlives the turn that asked for it.
  // Its occasion is a worker session with a branch to gate; `dispatch`, the one agent this
  // list governs, has no branch and no sweep of its own to start. See bin/b7e-detach.
  ...
  //   → Bash(b7e-claims:*) — declared `@grant read` in bin/b7e-claims
  ...
  // lib/which.js.
  //   → Bash(b7e-which:*) — declared `@grant read` in bin/b7e-which
  ...
  //   → Bash(b7e-show:*) — declared `@grant read` in bin/b7e-show
  ...
  //   → Bash(b7e-already:*) — declared `@grant read` in bin/b7e-already
  ...
  //   → Bash(b7e-answered:*) — declared `@grant read` in bin/b7e-answered

  // Read-only (bc-dgx7.88): one `ps -Ao pid=,etime=,args=` read plus (for `--family`) one
  // `bd export` — never a write, never touches a worktree. Its whole occasion is a
  // session self-checking whether another window is already on its own bead — seven
  // sessions in the deluvia tracker each wrote a different `ps` pipeline for exactly
  // this by hand, and one of the seven got a false negative on the one question this
  // exists to get right. See bin/b7e-window and lib/window.js.
  //   → Bash(b7e-window:*) — declared `@grant read` in bin/b7e-window

  // Read-only in the same construction sense as b7e-def/b7e-census above (bc-dgx7.16):
  // it enumerates the calling repo's own tracked-plus-untracked files with `git
  // ls-files`, reads each with `fs`, and matches a `RegExp` — never shells a `grep`
  // with a glob for a shell to reparse, so it never hits the `--include=*.py` unquoted-
  // glob trap that broke four sophab sessions' first call each. See bin/b7e-where and
  // lib/where.js.
  //   → Bash(b7e-where:*) — declared `@grant read` in bin/b7e-where

  // Read-only in the same construction sense as b7e-gated/b7e-stillred above (bc-dgx7.92):
  // it only parses `lib/gaterun.js`'s own JSONL run record and, to refuse a typo'd suite
  // name, shells to `scripts/test.mjs --list` — never spawns a suite, never touches `bd`.
  // Its whole occasion is a session asking "did the gate actually run this suite" and
  // answering it by grepping the persisted log for a basename, no two sessions the same
  // way; one of them (bc-dgx7.80) only found its own two new suites had never run at all
  // by building the real check — `--list` to a file, the log's suite names to another,
  // `comm -23` between them — by hand. See bin/b7e-ran and lib/gaterun.js's
  // resultsFor/missingFrom.
  //   → Bash(b7e-ran:*) — declared `@grant read` in bin/b7e-ran
  ...
  // b7e-hunks (bc-dgx7.78) is deliberately NOT on this list, for the b7e-apply/b7e-swbump
  // reason: its `--take ours|theirs` flag resolves conflict markers in place, writing
  // directly to whatever tracked files it is pointed at, and `--stage` follows that with
  // a `git add`. Both are exactly "writes to whatever checkout it runs in" — b7e-apply's
  // own argument above — and `dispatch`, the one agent this list governs, has no branch
  // of its own to commit that write to and no oversight loop past its own single
  // `bd comment`. This is also the decision the b7e-run paragraph just above deliberately
  // avoided recording early, per bin/b7e-enroll's allowlistProblem: naming "b7e-hunks"
  // anywhere in this array's text — comment or code — reads as a recorded decision about
  // it, so this paragraph is what actually makes that decision real rather than an
  // accidental name-collision. See bin/b7e-hunks and lib/hunks.js.
  // b7e-plate (bc-dgx7.91) is deliberately NOT on this list, on the same reading as
  // b7e-eyeball above: it shells out to `sips` and `python3`/Pillow to composite and
  // label a montage PNG, which is the shape lib/grants.js already calls a write on the
  // strength of "nothing about writing an image file is a read" — it never touches `bd`
  // or this repo's own tracked tree, but a grant here is not about that distinction, it
  // is about whether the command does something a read-only eval would have to special-
  // case. It is also pointless here on the b7e-eyeball/b7e-worktree reasoning: `dispatch`,
  // the one agent this list actually governs, answers one phone comment with one `bd
  // comment` and has no directory of stills in front of it to curate or check against a
  // style guide — that occasion belongs to a worker session with a branch of its own,
  // and `worker`'s tool list is the unrestricted CLI default, so a grant here would widen
  // nothing that agent cannot already reach. See bin/b7e-plate and lib/plate.js.
  // b7e-sheet (bc-dgx7.13) is deliberately NOT on this list, on the same reading as
  // b7e-plate just above: it writes PNG files (a `tools/sheet_probe.py` subprocess in
  // the sophab checkout does the actual rendering), which is a write regardless of
  // whether it touches `bd`, and it is pointless here besides — `dispatch` answers one
  // phone comment and has no drawing sheet in front of it to render and look at; that
  // occasion belongs to a worker session, which already has the unrestricted CLI
  // default. See bin/b7e-sheet and lib/sheet.js.
  // b7e-take (bc-dgx7.97) is deliberately NOT on this list, even though the bead's own
  // wiring checklist named it — that checklist is right for a read-only lookup and wrong
  // here, for the same reason `b7e-handback` (its closest sibling in shape) is not on it
  // either: `bd update --if-assignee ... --assignee ... --status in_progress` is a
  // tracker *write*, and this array is "the read-only surface every reply agent gets"
  // (this file's own opening docstring) — `dispatch` never claims a bead in the first
  // place, so there is nothing here for it to recover from. The worker session this
  // actually serves already runs `bd update --claim` itself as the very first line of
  // its brief, under the unrestricted CLI default, so a grant here would widen nothing
  // that session cannot already reach — same shape as b7e-eyeball/b7e-plate just above,
  // for a genuinely different reason (a write, not merely "not `bd`"). See bin/b7e-take
  // and lib/take.js.
  // b7e-write (bc-dgx7.86) is deliberately NOT on this list, and for the plainest reason
  // in the whole family: it IS the write door — `comment`, `update`, `close`, `claim`,
  // `unclaim`, `label`, `dep`, run through `Bd` with a timeout-then-verify contract, the
  // exact mutating set `b7e-ws` beside it refuses to forward for being "a lookup, not a
  // second door into bd". Granting this to `dispatch`, the one agent this list governs,
  // would be a general-purpose write into any configured tracker with no branch of its
  // own and no oversight loop past its own single `bd comment` — strictly more capability
  // than `Bash(bd label add:*)` or any other single mutating command lib/grants.js
  // classifies as a write. A worker session, which is what dv-k4n.8/dv-b5d.43/dv-gr6.65
  // actually hit this gap in, already carries an unrestricted allowlist and needs no
  // grant to run it. Checked against lib/grants.js first, per
  // [[b7e-command-write-shaped-check-grants-before-default-tool-list]] — since it is
  // never granted here, it needs no entry there either. See bin/b7e-write and lib/bd.js.

  // Read-only (bc-dgx7.18): every `bd` verb it spawns — `show`, `comments`, `children`,
  // `export` (`Bd.graph`), `ready`, and a batched `show` for surface notes — is a read,
  // and `--check` validates a candidate plan/whole-job document in memory without ever
  // writing the comment, the label or the handback `bin/plan.js`'s own write path does.
  // bc-ka5y.15, bc-1kwl and bc-khoe.33 each needed this and each built a different
  // one-off: ten `python3` heredocs re-measuring a prompt's length after every trim, five
  // scratchpad programs to reconstruct a plan through `planFrom` before revising it, a
  // `dump.mjs` plus a `python3 -` pass to print `prompt len` per group. Named
  // `b7e-plancheck` rather than the `bin/b7e-plan` its own bead asked for, because
  // `bc-jvt0.6` landed a write-shaped proxy at that exact path first — see the header of
  // bin/b7e-plancheck. See bin/b7e-plancheck and lib/plan.js.
  //   → Bash(b7e-plancheck:*) — declared `@grant read` in bin/b7e-plancheck

  // b7e-recard (bc-dgx7.27) is deliberately NOT on this list, for the b7e-write reason
  // narrowed to one shape: it is a tracker write — a `bd comment` plus up to three `bd
  // update --<field>` calls — and what it rewrites is the *question the phone is asking*.
  // `dispatch`, the one agent this list governs, exists to answer one comment on one
  // question bead and exit; handing it the ability to replace that bead's question with
  // another one is the plainest possible case of a grant nobody asked for. The sessions
  // it was built for (bc-khoe.33, bc-xl7n.77.1, bc-1kwl.29) were workers, which carry an
  // unrestricted allowlist already and need no grant to run it — the same argument
  // b7e-say and b7e-write are withheld under. Checked against lib/grants.js first, per
  // [[b7e-command-write-shaped-check-grants-before-default-tool-list]]; since it is never
  // granted here it needs no entry there either. See bin/b7e-recard and lib/decision.js.

  // b7e-register is deliberately NOT on this list. It edits four tracked files in whatever
  // checkout it runs in — the two bin maps, a tool's own header and this file — with no branch of
  // its own and no path to commit or review the result, which is the argument the apply-command
  // and the sw-cache-bump command above are excluded under. Its whole occasion is a worker
  // session that has just written a new command and is about to deliver it; dispatch, the one
  // agent that list governs, answers one comment and exits and has no bin/ file of its own to
  // register. See bin/b7e-register.

  // b7e-head (bc-dgx7.37) is deliberately NOT on this list, even though it reads only —
  // no create, no close, no bead write of any kind — and looks at a glance like
  // b7e-prior/b7e-siblings just above. The difference is what it does to get there:
  // `--file`, `--grep` and `--tree` each call `git fetch --no-write-fetch-head origin
  // <sha>` to pull the pull request's head objects in before reading them, and
  // `lib/grants.js` already classifies `Bash(git fetch:*)` as a write, scope `repo`,
  // granted to `merge-advocate` alone — "the only entry here whose effect is entirely
  // inside .git, and it is classified honestly rather than waved through." A narrower,
  // sha-only, no-ref-write fetch is still a fetch; calling it read here would be the
  // same wave-through that comment was written to refuse. Its occasion is also not
  // `dispatch`'s: this exists because three *worker* sessions (bc-36xx.9, bc-zjab.12,
  // bc-36xx.22) each hand-rolled the same fetch-and-read while testing the reviewer,
  // and `worker`'s `allowedTools` is already `null` — unrestricted — so a grant here
  // would widen nothing a worker cannot already reach. `review-advocate`'s own allowlist
  // (lib/foundation.js) is deliberately narrower still — no `git fetch` at all, on "it
  // reads; it does not run anything" — and whether that agent should ever be handed
  // `b7e-head` is a separate decision this bead does not make. See lib/head.js and
  // bin/b7e-head.
