...
/* ===================================================================================
  ...
  // Read-only in the same construction sense as b7e-siblings above, which this extends
  // rather than replaces: it runs `git worktree list`, `git diff`/`git log` against refs
  // and merge-bases, an optional `gh pr list --head`, and — for `--bead` — a read of
  // `~/.claude/sessions/*.json`. No `bd` call at all; `--bead` resolves a worktree from a
  // live session's own name (`beadInName`, lib/reap.js), never from the tracker. bc-dgx7.43
  // is the argument — six sessions, each handed a worktree name by a claim refusal, each
  // worked out what that worktree was actually doing by hand, no two the same way: one
  // took eight calls and was refused outright by `git -C <sibling>`; another's `git diff
  // main...<branch>` answered empty against a sibling whose whole change was still
  // uncommitted, which is silently wrong rather than merely slow. See bin/b7e-peek and
  // lib/peek.js.
  //   → Bash(b7e-peek:*) — declared `@grant read` in bin/b7e-peek
