...
export async function intake(bd, workspace, report, { actor = null, from = '', windowMs = WINDOW_MS } = {}) {
  const fp = fingerprint(report);
  ...
  const run = after.then(
    () => fileOne(bd, workspace, report, fp, { actor, from, windowMs }),
    () => fileOne(bd, workspace, report, fp, { actor, from, windowMs })
  );
...
async function fileOne(bd, workspace, report, fp, { actor, from, windowMs }) {
  const key = windowKey(workspace, fp);
