 ...
 * already covers it.
 *
 * Today an error in the app is a red toast and nothing else: `public/app.js`,
 * `public/console.js`, `public/foundations.js` and `public/term.js` each have their own
 * local `toast(msg, bad)`, and nothing on either side catches an uncaught exception at
 * all. The error is seen, shown, and lost. This is the other end of that — the daemon
 * side of bc-p38c, and the piece the browser reporter (bc-p38c.2) and the daemon's own
 * uncaught handler (bc-p38c.4) both post into.
 *
 ...
 * That split is what lets bc-p38c.4 file the daemon's own uncaught errors through the
 * same path without a loopback request to itself.
 */
...
export async function intake(bd, workspace, report, { actor = null, from = '' } = {}) {
  ...
  // rejection sitting in a Map is an `unhandledRejection` on the daemon, which under
  // bc-p38c.4 files a bead about the bead-filer.
  const tail = run.then(
    () => {},
    () => {}
  );
