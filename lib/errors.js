 ...
 *
 * Keyed by `fingerprintKeys` below, so **one window is in here under two keys** — which
 * is what makes it findable by a report that shares only one of them.
 */
const windows = new Map();

/**
 * The keys one report is serialised and coalesced under — **one per fingerprint, never
 * one for the pair**.
 *
 * This used to be a single `workspace::atLabel::msgLabel`, and that was the bug
 * (bc-jjdar.1): the *match* is an OR — `findByFingerprint` asks `--label-any` and
 * `pickMatch` accepts a row carrying either label — while the key was an AND. They
 * disagree for exactly the reports that share one fingerprint and not the other, so two
 * reports of one bug from two different messages (or one message from two different
 * lines) got different keys, never queued behind each other, and both ran the lookup
 * before either `bd create` had landed. Two P0 beads, 27ms apart, for one root cause —
 * bc-jjdar and bc-mwhkg are the pair that proved it.
 *
 * A key each, and the chain waits on *both*, is what makes the serialisation as coarse
 * as the match. Note it has to be both rather than a primary-with-fallback: a
 * cross-origin `Script error.` carries no `at` at all, so `atLabel || msgLabel` would
 * key it on its message while the report it matches keyed on its source — a pair that
 * match each other landing on different keys, which is the same bug wearing a hat. It
 * also gets transitivity for free: A(at1,msg1), B(at1,msg2), C(at2,msg2) all queue in
 * order, because B holds both of the keys the other two wait on.
 *
 * Serialising two reports that turn out *not* to match each other costs one of them a
 * wait; failing to serialise two that do costs a duplicate P0 and an unattended window
 * opened on it. The trade is not close.
 */
const fingerprintKeys = (workspace, fp) => {
  const labels = [fp.atLabel, fp.msgLabel].filter(Boolean);
  // A report with neither label matches nothing (`findByFingerprint` returns `[]`), so it
  // always files. The empty key keeps those serialised with each other, exactly as the
  // old conjunction did.
  return (labels.length ? labels : ['']).map((label) => `${workspace?.name || '?'}::${label}`);
};

/** The open window on any of these keys — a report needs only one of them to hit it. */
function findWindow(keys) {
...
function openWindow(keys, { bd, workspace, actor, id, at, ms }) {
  const w = { keys, bd, workspace, actor, id, at, ms, count: 0, first: '', last: '', url: '' };
  w.timer = null;
  w.closed = false;
  for (const key of keys) windows.set(key, w);
  armWindow(w);
  return w;
}

/** Take a window out from under every key it is registered on, and only its own. */
function dropWindow(w) {
  for (const key of w.keys) if (windows.get(key) === w) windows.delete(key);
}
 ...
 */
async function closeWindow(w, { reopen = true } = {}) {
  // The guard the `windows.get(key)` lookup used to be: a window closed by
  // `flushErrorWindows` must not be closed a second time when its own timer fires.
  if (!w || w.closed) return null;
  w.closed = true;
  clearTimeout(w.timer);
  dropWindow(w);
  if (!w.count) return null;
  if (reopen) openWindow(w.keys, { ...w, ms: Math.min(w.ms * 2, WINDOW_MAX_MS) });
  try {
...
export async function flushErrorWindows() {
  const out = [];
  // A `Set` because one window is registered under both of its keys, and closing it
  // twice would report it twice.
  for (const w of new Set(windows.values())) {
    const closed = await closeWindow(w, { reopen: false });
 ...
 *
 * **Coalesced** is that fourth: a window is already open on either of this report's
 * fingerprints, so the report is added to a count and no `bd` call is made, in either
 * direction. It is checked inside `fileOne` rather than out here, so it is asked *after*
 * the chain has given us our turn: three reports that arrive together must not each
 * decide there is no window, when the first of them is about to open one.
 *
 ...
 *
 * The chain is per-process and per-fingerprint — `fingerprintKeys`, a key each for the
 * source and the message, waited on together — so two errors sharing neither still file
...
export async function intake(bd, workspace, report, { actor = null, from = '', windowMs = WINDOW_MS, config = null } = {}) {
  const fp = fingerprint(report);
  const keys = fingerprintKeys(workspace, fp);
  // Behind *every* chain this report could match — not one. A report sharing only the
  // source with what is in flight, and only the message with what is behind that, has to
  // wait for both or it files a second bead for the one bug (bc-jjdar.1).
  const after = Promise.all(keys.map((key) => inflight.get(key) || Promise.resolve()));
  ...
  for (const key of keys) inflight.set(key, tail);
  // Dropped once nothing is behind it, so the map does not grow one entry per distinct
  // error for the life of the process. Per key, and only where this tail is still the
  // last one: a later report may already have claimed one of the two and not the other.
  tail.then(() => {
    for (const key of keys) if (inflight.get(key) === tail) inflight.delete(key);
  });
  return run;
}
...
async function fileOne(bd, workspace, report, fp, { actor, from, windowMs, config = null }) {
  ...
  if (match && !match.closed) {
    ...
    openWindow(keys, { bd, workspace, actor, id: match.bead.id, at: fp.at, ms: windowMs });
