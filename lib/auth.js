 ...
 * cannot be revoked for one device, because there is only one of it (a session can —
 * see lib/devices.js); and a QR code
 ...
 * `{sub, email, sid, exp}` with a key in `~/.config/beadcause/session.key`, which means
 * no session store to keep, nothing to migrate, and nothing left behind by a restart —
 * the daemon is replaced by bin/router.js several times an hour, and a store in memory
 * would sign everybody out on every swap.
 *
 * **What that used to cost was per-device revocation, and `sid` is what bought it
 * back.** The id names a row in `state.json` — a file every backend already reads, so
 * it survives both a swap and a restart — and the gate refuses a cookie whose row has
 * been deleted. So there are three acts now rather than two: revoke one device (delete
 * its row, which is also what signing out does), delete the key file (every session
 * everywhere, still the only thing that needs no list), and rotate the token, which is
 * a separate credential entirely. lib/devices.js holds the list and the reasoning.
 *
...
export function secretFileWarning(cfg = {}) {
  ...
  // The rule itself lives in lib/commonrepo.js, which owns the denylist it is asking
  // about; the two Atlassian integrations ask the same question through
  // `tokenFileWarning` in lib/atlassian.js. Silence for an install that never asked for
  // sign-in is *this* function's half, and stays here (bc-jv4p).
  return leakWarning(clientSecretFile(google));
}
 ...
 *
 * By the same argument it does not move when one *expires*: `certificateName` asks
 * whether there is a pair for this name, not whether the calendar still likes it
 * (bc-jv86), so the origin sign-in was registered against survives an expiry the same
 * way the socket does. Sign-in is unusable in that state either way — the browser is
 * refusing the origin — but it comes back with the certificate rather than needing the
 * URI re-registered.
 */
 ...
 * every browser out at once, which is still the only revocation that needs nothing but
 * this file: one device at a time is a row in the device list (lib/devices.js).
 */
 ...
 *
 * `claims.sid` is the session's own id, and it is what turned this from one session
 * into a list of them (lib/devices.js). The payload is otherwise unchanged, and the
 * signature is still the only thing that makes it trustworthy — the id names a row in
 * `state.json` and authorises nothing by itself.
 *
 * The expiry is returned alongside the cookie because the row and the cookie have to
 * agree about when this session dies, and computing it twice is how they stop
 * agreeing.
 */
export function sessionCookie(auth, claims, key = sessionKey(), now = Date.now()) {
  ...
  const value = sign({ sub: claims.sub, email: claims.email, sid: claims.sid, iat: Math.floor(now / 1000), exp }, key);
  return { cookie: cookie(SESSION_COOKIE, value, { maxAge: seconds, secure: secureCookies(auth) }), exp };
}
