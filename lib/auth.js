 ...
 *
 * The key file is `session.key` on purpose. `~/.config/beadcause` is a git repo
 * (lib/commonrepo.js), and `*.key` is both ignored there *and* in that file's
 * `FORBIDDEN` list — so the signing key cannot reach that history even if somebody
 * edits the ignore file. The Google client secret has no such protection yet, which is
 * why it can come from an env var or a file of your choosing; bc-m6m is the bead that
 * finishes moving it.
 */
...
/**
 * The client secret, from whichever of the three places has it.
 *
 * The env var first, because that is the one that leaves no copy on disk at all, and
 * a file second, because `~/.config/beadcause` is a git repo and `config.json` is
 * committed to it on every write. The plain config field is last and is documented as
 * the convenient-but-worst option; bc-m6m is the bead that takes it away.
 */
export function clientSecret(google = {}) {
  return (
    process.env.BEADCAUSE_GOOGLE_CLIENT_SECRET?.trim() ||
    (google.clientSecretFile ? secretFromFile(google.clientSecretFile) : null) ||
    google.clientSecret ||
    null
  );
}
...
export function googleProblem(cfg = {}) {
  ...
  if (!clientSecret(google)) return 'no client secret (config, clientSecretFile or BEADCAUSE_GOOGLE_CLIENT_SECRET)';
