...
function passBriefFor(row, { owner = 'Adam' } = {}) {
  return [
    ...
    `**None of this is yours to merge.** An in-app edit is merged by the person who asked for it,` +
      ` so pass \`--review\` when you deliver each one, and do not deploy anything. The delivery` +
      ` command holds these whether it is asked to or not.`,
    '',
  ].join('\n');
}
...
export function editBriefFor(row, { owner = 'Adam' } = {}) {
  ...
  return [
    ...
    `**And this one stops at the pull request.** ${owner}'s standing line is that the reversible` +
      ...
      ` what came back. So when you deliver, pass \`--review\` — and **you do not merge this and` +
      ` you do not deploy it**, whatever the rest of this page says about landing your own work.` +
      ` The delivery command holds it either way, so the flag is you agreeing with it rather than` +
