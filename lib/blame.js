...
/**
 * `git worktree add --detach` at `origin/main`, under a fresh scratch directory, with
 * `node_modules` symlinked from `root`'s own (never copied — the same rule
 * `b7e-worktree` and `scripts/vendor.js` hold elsewhere). Never `public/vendor`: the
 * suites this command targets are `scripts/test.mjs`'s discovery, i.e. `npm test`'s own,
 * and browser checks — the only suites that want a built `vendor/` — are not among them
 * (`[[browser-checks-not-in-npm-test]]`).
 *
...
export function makeMainWorktree(root) {
  const scratchRoot = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), 'beadcause-blame-'));
  const dir = path.join(scratchRoot, 'main');
  execFileSync('git', ['worktree', 'add', '--detach', '-q', dir, 'origin/main'], { cwd: root, stdio: 'pipe' });
  ...
  // this is pointed at), so `origin/main`'s checkout never has one — but never clobber
  // the rare case where it does: leaving a real directory alone rather than replacing it
  // is the same rule `b7e-worktree` holds for a worktree that already installed its own.
...
/**
 * Removes a worktree made by `makeMainWorktree`, best-effort and always — `git worktree
 * remove --force` (a detached worktree that only ever ran a read-only suite has nothing
 * of its own to lose), then `prune` to drop the registration if `remove` could not, then
 * the scratch directory itself so a failed `remove` cannot leave it behind forever.
 */
export function removeMainWorktree(root, dir, scratchRoot) {
  try {
