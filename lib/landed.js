 ...
 *
 * Nothing here merges, pushes or deploys anything. It reads GitHub and writes two kinds
 * of line to the tracker, and — since bc-6sqs — moves one ref on this laptop:
 *
 * 4. **A sweep that closed something fast-forwards the local base.** Not a fourth kind
 *    of write so much as the other half of the first three. `bc-0ntp` made a worker's
 *    own delivery bring the main checkout up (`landParent`, bin/deliver.js) and the tap
 *    on a delivery card has always done it (lib/server.js), which left exactly one door
 *    into `main` that did not: a pull request merged **on github.com itself**, from a
 *    phone browser or by somebody else. That is the door this file exists for. So the
 *    bead closed, the board drew merged, and local `main` stayed behind until something
 *    else happened to fetch — every worktree cut afterwards branching from before the
 *    merge, which is the staleness bc-0ntp was filed about arriving by the one route it
 *    did not close.
 *
 *    It is `landLocally` unchanged, including its refusal to touch a checkout with
 *    *edited* work in it — see lib/prboard.js, which calls fast-forwarding over
 *    somebody's open files the most destructive thing in this codebase, and means it.
 *    Untracked residue is the one thing it steps past (bc-45g8), here as everywhere.
 *
 *    **Only when this sweep closed something**, and that gate is the whole of the cost
 *    control. A fetch and a fast-forward per repo per interval, over a workspace of
 *    forty checkouts, is forty fetches every ten minutes for a question that is nearly
 *    always "no" — where "this sweep just closed a bead over a merge nothing here
 *    performed" is precisely the moment the answer is known to be "yes". What it does
 *    not cover is a hand-merged pull request with no bead behind it at all; nothing
 *    here noticed that merge either, and the next one that does will bring the checkout
 *    up past both.
 *
 * 5. **And a sweep that closed something asks for the *other* sweep.** The same gate
 *    again, and for a reason one sentence along from rule 4's: a merge does not only
 *    leave this laptop behind, it leaves every branch still open behind it measured
 *    against a base it has never seen, and some of them stop fitting. Three of the four
 *    doors into `main` know the moment they merge and record it on the spot
 *    (lib/mergesweep.js). This is the fourth, the one where nothing here performed the
 *    merge at all, and so the trigger arrives late — minutes, or the length of an
 *    advocate interval — and has to fire exactly once anyway. Recording it is all this
 *    does; the sweep itself belongs to the daemon, which is the only process holding the
 *    registry that stops two resolver windows opening on one branch.
 *
 *    It is deliberately behind the *close*, not behind the merge, so a repo where every
 *    merged bead is already closed asks for nothing: this sweep runs against every
 *    checkout every ten minutes, and a `gh pr list` per repo per tick for a question
 *    whose answer is nearly always "nothing" is the cost rule 4 refused to pay either.
 *    The blind spot is the same one and it is the same size — a hand-merged pull request
 *    with no bead behind it is not noticed here, and the next merge that is sweeps past
 *    both.
 *
 * Every failure is a returned sentence rather than a throw — a sweep is a courtesy on
 * top of the advocate's tick, and it may not be able to take the tick down with it.
 */
import { beadsFor, candidateTiers, prefixFor } from './beadref.js';
import { parseDelivery, DELIVERY_LABEL } from './delivery.js';
import { requestSweep } from './mergesweep.js';
import * as pr from './pr.js';
import { landLocally } from './prboard.js';
 ...
 *
 * `key` is the repo unit this checkout is — `climative/athena-service`, or the workspace
 * name where the workspace is the repo — and it is only ever used to ask for the conflict
 * sweep in rule 5, which is keyed the way lib/resolvers.js serialises. A caller that does
 * not know it gets the workspace's name, which is right for every single-repo workspace
 * and refused by `unitFor` for the others rather than resolved to the wrong checkout.
 */
export async function reconcileLanded(
  ...
  dir,
  {
    ...
    now = Date.now(),
    land = landLocally,
    request = requestSweep,
    /**
     * Rename the window that delivered this from `QUEUED-` to `DONE-` — lib/retitle.js,
     * and the same call lib/mergequeue.js makes in `finish`. This is the door for a pull
     * request merged on github.com, which the queue never sees and so never renames, and
     * a window left saying `QUEUED-` is one still claiming to be waiting on a queue that
     * is not going to touch it. Injected, like everything else here, so a test of the
     * sweep does not need a `~/.claude` to run.
     */
    markMerged = null,
  } = {}
) {
  const out = { ok: false, reason: '', checked: 0, closed: [], cards: [], skipped: [], truncated: null, landed: null, swept: null };
  ...
  for (const row of merged || []) {
    ...
    for (const bead of await beadsFor(bd, ws, prefix, row, seen)) {
      ...
      //
      // The reason goes in with it, because **this sweep is what closed bc-ka5y**: an
      // epic naming twenty-three adoptees, closed as "Merged #212 as 72789c0b into main
      // on GitHub" with twenty-one of them still open. A merge is no evidence about an
      // epic's theme, so the gate refuses that close outright now (lib/bd.js) and the
      // epic comes back as a skip — which is a sentence in the advocate's report rather
      // than a classification quietly deleted.
      let gate = null;
      try {
        gate = await bd.closeGate(ws, bead.id, { reason: landedReason(row, base) });
      } catch (err) {
      ...
      // Before the close, for lib/mergequeue.js's reason: closing the bead is what makes
      // the window reapable, and a rename after it races the signal that closes it.
      if (markMerged) {
        try {
          await markMerged(bead.id);
        } catch {
          /* A window wearing a stale name is cosmetic; the close below is not. */
        }
      }
      try {
        await bd.close(ws, bead.id, landedReason(row, base), { overClaim: true });
  ...
  // And bring this Mac up to what GitHub has — rule 4 in the header. Last, after every
  // write to the tracker, because the tracker is the part that matters and a git that
  // will not answer must not cost a bead its close. Wrapped for the same reason: a
  // fetch fails on a laptop with no network, and a sweep that closed six beads has
  // succeeded whatever the checkout then did.
  if (out.closed.length || out.cards.length) {
    try {
      out.landed = await land(dir, base);
    } catch (err) {
      out.landed = { fetched: false, advanced: false, note: `could not bring local ${base} up — ${String(err.message || err).split('\n')[0]}` };
    }

    // And rule 5. The number is the highest pull request this sweep closed something
    // over, which in a repo is the merge that moved the base last — the one the
    // resolver's brief should name, and the one candidate a sweep can skip outright.
    // Everything else about which branches conflict is asked of GitHub at sweep time,
    // so an approximate `after` costs nothing and a wrong one would only cost a sweep
    // re-reading a pull request that is already merged.
    const numbers = [...out.closed, ...out.cards].map((r) => Number(r.number)).filter((n) => Number.isInteger(n) && n > 0);
    out.swept = request({
      workspace: ws.name,
      key: key || ws.name,
      number: numbers.length ? Math.max(...numbers) : null,
      base,
      why: `a merge on github.com in ${key || ws.name}`,
    });
  }
