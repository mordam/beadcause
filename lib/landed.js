 ...
 *
 * These two are a window and a cap on reaching it, and on a busy repo the cap is what
 * decides: forty merged pull requests is under a day here, so the fourteen days is a
 * ceiling this has never once touched. When the cap bites first, everything older is
 * simply not asked about — and the *silence* is the defect, because a bead stranded past
 * it is stranded for good rather than for a fortnight. bc-8ug and bc-jin are what that
 * costs: both merged inside the fourteen days, both around a hundred pull requests back
 * by the time anything swept, both left `in_progress` over shipped work — bc-jin holding
 * three beads out of `bd ready` behind it for two days — until bc-4qmp closed them by
 * hand. Nothing in the log said the window had closed under them, because nothing looked.
 *
 * So the cap stays (widening it multiplies the per-row `bd` reads on every tick, which
 * is a cost question and its own bead), and `truncated` below says when it bit.
 */
...
export async function reconcileLanded(bd, ws, dir, { base = 'main', limit = QUERY_LIMIT, rows = null, now = Date.now() } = {}) {
  ...
  // the inside. `gh pr list` answers newest-first, so a query that came back full and
  // never reached the cutoff means the *cap* decided how far back this looked, not the
  // fortnight it claims to. Everything older than the oldest row is then not merely
  // unswept this tick — it is unswept for good, because the next tick asks the same
  // question and gets a window that has moved further forward. bc-8ug and bc-jin are
  // what falls through it. Nothing is guessed and nothing extra is asked for: this is a
  // report, and what to do about it belongs to whoever reads it.
  if ((merged || []).length >= limit && aged === 0 && oldest) {
    out.truncated = { limit, oldest, days: Math.max(0, Math.round((now - new Date(oldest).getTime()) / 86400000)) };
  }
