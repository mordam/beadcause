 ...
 *
 * A merge that was refused, a space that asks Adam to approve before anything merges, and
 * — since bc-36xx.7 — a **review** that ended without one: the reviewer refused outright,
 * or it and the worker did not agree inside `MAX_REVIEW_ROUNDS`. All three come through
 * this one function rather than through a raiser apiece, for the reason above: the second
 * bead is the bug, whatever raised it. The three differ only in the sentence the card
 * opens with, which is `deliveryBody`'s job.
 *
...
import { baselineNote, failureNote } from './mergeadvocate.js';
 ...
 *
 * The attempt tally is about *merging*, so it is dropped for both of the openings that
 * are not a merge that failed. An approval wait has no sentence at all — nothing refused
 * it. A review escalation has the reviewer's sentence and only that: it is stopped before
 * the queue has tried anything, and "Tried 0 times" beside a deadlocked review would be
 * counting the wrong thing at a person who is trying to work out what to do about it.
 */
export function raiseOpening(state, why, { approval = false, review = false } = {}) {
  if (approval) return '';
  if (review) return String(why || '').trim();
...
export async function raiseMergeCard(
  ...
  const refused = raiseOpening(state, why, { approval, review });
  const over = baselineNote(baseline);
  /**
   * And what the check actually said, when something red is why this is a card at all —
   * bc-xl7n.116.
   *
   * Off the queue's own block rather than fetched here: the tick that refused already
   * read it and wrote it down (`record` in lib/mergequeue.js), and a card raised three
   * ticks later has no business going back to GitHub for a log about a run nothing has
   * touched since. Empty on an approval wait and on a review escalation, because neither
   * has a red check to explain — and empty whenever the read failed, which is the card
   * exactly as it was before.
   *
   * In `context` rather than in `refused`: `deliveryBody` puts the refusal sentence in the
   * middle of a paragraph about what to press, and a log tail belongs under it rather than
   * inside it.
   */
  const said = approval || review ? '' : failureNote(state.failure);

  const body = deliveryBody(
    { ...spec, title: issue.title || spec.title || '' },
    {
      context: [[context, over].filter(Boolean).join(' '), said].filter(Boolean).join('\n\n'),
      refused,
      approval,
      review,
      ship: shipHint,
  ...
  if (prComment) {
    await prComment(
      ...
      approval
        ...
        : review
          ? `The beadcause review loop stopped here: ${refused} It is ${owner}'s call now — see ${issue.id}.`
