...
export function createBranchBeads({ cfg, bd, attribute, log = console.log }) {
  ...
  const asked = new Map();
  // The branches this has already complained about. A failure drops its memo, so a
  // tracker that stays unreadable is re-asked by every edit — and a line each time would
  // be a log nobody can read during exactly the incident you would be reading it for.
