/**
 * What re-opens a P0 advocate — the half of bc-rfnr.3 that was never delivered.
 *
 ...
 * `openEpicAdvocateSession` had exactly one caller, the button on the P0 card, so a P0
 ...
 * new registration file, no `advocated` label and no list in the config — a P0 is enrolled
 ...
 *   - **Erasing the block un-enrols the P0**, which is the off switch that costs no new
 *     control: an advocate that concludes a P0 needs no more supervision can take its own
 ...
 * enrols its P0, and a tap is needed again. That is the failure the button stays for, and
 ...
 * P0 whose children are epics has its real movement one level further down and the card
...
export const REENTER_DEFAULTS = {
  /**
   * Re-open the P0 advocate when something moves under an enrolled P0. The whole
  ...
  /**
   * The floor between two automatic windows on the same P0. **This is the rate limit the
...
/**
 * Every P0 in this workspace an advocate has already been on, with the two lists a
 * re-entry needs.
 *
 ...
 * P0 the launch would then reject: not a P0, closed, unowned, or a crash bead. Injectable
 * only so a test can drive enrolment without building four labels; the daemon never
 * overrides either.
 */
export function advocatedP0s(index, { wants = wantsAdvocate, enrolled = waitingOn } = {}) {
  ...
  for (const bead of beads.values()) {
    ...
    const kids = (kidsOf.get(bead.id) || [])
      ...
      .map((id) => beads.get(id))
      .sort(byDoneThenId);
    out.push({ p0: bead, kids, tree: treeUnder(kidsOf, beads, bead.id) });
  }
  ...
  return out.sort((a, b) => String(a.p0.id).localeCompare(String(b.p0.id), 'en', { numeric: true }));
}

/**
 * What has moved under this P0 since the last look, as the sentence the window is opened
 ...
 * would open a window on every enrolled P0 the first time it ever ran). The record it
