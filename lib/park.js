 ...
 *
 * bd will not let an epic be blocked by anything that is not itself an epic (and,
 * symmetrically, will not let a task be blocked by an epic). Every other pair is
 * fine — a bug blocked by a task, a chore blocked by a decision — so the rule is
 * narrower than it looks and it is exactly one line: **epic-ness has to match.**
 *
 ...
 *
 * Epic-ness has to match and nothing else does, so this is a two-valued answer:
 ...
 *
 * Returns `{ parked, labelled, note }`. `note` is empty when it worked and is one
 * plain sentence when it did not — no stack trace, and it always names the question
 * that does exist before it says what did not happen, because the caller's next
 * decision is whether to ask again.
 *
...
export function park(bd, target, question, { label = true } = {}) {
  try {
    bd(['dep', 'add', String(target), String(question)]);
    return { parked: true, labelled: false, note: '' };
  } catch (err) {
    const why = bdSaid(err);
    if (!label) {
      return { parked: false, labelled: false, note: `filed ${question}, but could not park ${target} behind it — ${why}` };
    }
    try {
