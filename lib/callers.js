...
/**
 * `text`, same length, with every regex literal replaced by spaces and *nothing else*
 * touched — the input the brace/paren walk in `definitionsFor` needs, and the only pass
 * that wants it. Strings and comments stay, because that walk already handles those and
 * blanking them would cost it the `//`/`/*` and quote characters it steers by.
 *
 ...
 */
export function blankRegexLiterals(text) {
  const ranges = blankRanges(text);
  if (ranges === null) return text;
  return spaceOut(text, ranges.regex);
}
 ...
 * `blankRegexLiterals(text)` — same length, same line numbers, regex literals spaced out
 * — because `findBody`/`bodyImmediatelyAfterParens`/`matchBrace` are hand-rolled scanners
 * that read a quote inside `/^["']|["']$/` as a string opener and a `{2,3}` quantifier as
 * a block. Left raw, one such regex runs a function's `endLine` to the end of the file,
 * `findCallers` skips that whole span as "the definition's own body", and a symbol with a
 * real caller reports `no reference found anywhere searched` — the exact silent miss this
 * file exists to end. See `blankRanges`.
 */
export function definitionsFor(name, file, text) {
  const scan = blankRegexLiterals(text);
