 ...
 * (`skipString`/`findBody`/`matchBrace`/`definitionsFor`), but its own
...
/**
 * Two lists of `[start, end)` char ranges, off one `acorn` parse — or `null` when `acorn`
 * cannot parse `text` at all.
 *
 * `blank` is what `blankNonCode` replaces with spaces: every comment `acorn` reports, and
 * every plain string literal and template-literal *chunk* (never the `${ … }` expression
 * between two chunks, which is real code and may itself hold a call site this repo cares
 * about — see the file header). A regex literal is *not* in it: `node.regex` marks it, and
 * blanking it would hide real code from the occurrence scan for no gain there.
 *
 * `regex` is every regex literal, and it exists for the one pass that does need them gone
 * — the brace/paren walk in `definitionsFor`. That walk is a hand-rolled scanner, not
 * `acorn`, and a regex literal is exactly the thing it cannot read: `/^["']|["']$/` (real,
 * `lib/beadfiles.js:130`) hands `skipString` a `"` that opens a string which never closes
 * where it should, so `matchBrace` runs a function's `endLine` to the end of the file and
 * `findCallers` then skips every real call site in that span. Its `(`, `[` and `{…}`
 * quantifiers desync the depth counters the same way. See `blankRegexLiterals`.
 *
 * Acorn gets both right where the hand-rolled scanner in `blankNonCodeNaive` cannot: it is
 * the same parser this repo already trusts for scope analysis in `lib/noundef.js`, not a
...
function blankRanges(text) {
  ...
  const walk = (node) => {
    ...
    if (node.type === 'TemplateLiteral') {
      // the two backticks themselves — the chunks between them are TemplateElements, and
  ...
  walk(ast);
  return { blank: ranges, regex, ticks };
}
...
export function blankNonCode(text) {
  ...
  return spaceOut(text, ranges.blank);
}

/**
 * `text`, same length, with every comment, string literal, template literal and regex
 * literal replaced by spaces — the input the brace/paren walk in `definitionsFor` needs,
 * and the only pass that wants it.
 *
 * The point is that the walk should never meet a quote, a backtick, a `//` or a `/` at
 * all: `skipString`, its comment skipping and its depth counters are then dead code that
 * cannot desync, and what is left for it to read is exactly the `(`/`)`/`[`/`]`/`{`/`}`
 * structure it is actually there to measure. Every way that walk was seen to go wrong on
 * this tree is one of those characters not meaning what it looked like — the `"` inside
 * `/^["']|["']$/` (`lib/beadfiles.js:130`), the `{2,3}` quantifier that unbalanced the
 * brace count, the escaped backtick of a nested template (`` `\`${x}\`` ``, real, 27 of
 * them in `lib/amendment.js` alone). Blanking the delimiters costs the walk nothing,
 * because it only ever needed them to get *past* the content they wrapped.
 *
 * `${ … }` expressions inside a template are deliberately left as code: their braces are
 * balanced, so they cannot unbalance anything, and a real definition can live inside one.
 *
 * When `acorn` cannot parse `text`, this hands back `text` unchanged: the walk is then no
 * worse off than it was before this existed, which is the same bargain `blankNonCode`
 * strikes with `blankNonCodeNaive`.
 */
export function blankForBraceWalk(text) {
  const ranges = blankRanges(text);
  if (ranges === null) return text;
  return spaceOut(text, [...ranges.blank, ...ranges.regex, ...ranges.ticks]);
}
 ...
 *
 * The head regexes match against the raw lines, but every *body* walk below runs against
 * `blankForBraceWalk(text)` — same length, same line numbers, every comment, string,
 * template and regex spaced out — because `findBody`/`bodyImmediatelyAfterParens`/
 * `matchBrace` are hand-rolled scanners and each of those is a way for a `"`, a backtick
 * or a `{` to not mean what it looks like. Left raw, one `/^["']|["']$/` runs a function's
 * `endLine` to the end of the file, `findCallers` skips that whole span as "the
 * definition's own body", and a symbol with a real caller reports `no reference found
 * anywhere searched` — the exact silent miss this file exists to end. Measured on this
 * tree: raw, 57 definition spans are wrong and 21 verdicts with them; blanked, every one
 * of 3191 top-level function declarations agrees with `acorn`'s own `loc`. See
 * `blankRanges`.
 */
export function definitionsFor(name, file, text) {
  ...
  for (let li = 0; li < lines.length; li += 1) {
    ...
    for (const head of heads) {
      ...
      const body = head.verifyBody ? bodyImmediatelyAfterParens(scan, afterHeadChar) : findBody(scan, afterHeadChar);
      if (!body) continue;
...
/**
 * Is the newline at `nl` *inside* a member chain rather than the end of one?
 *
 * A newline was originally a plain boundary here, which made every wrapped chain invisible:
 * `window.beadcause\n  ?.views\n  ?.mark?.(x)` cut at the last `\n`, left `?.` as the whole
 * window, and reported `no reference found anywhere searched` for a call that is right
 * there. 164 files in this tree wrap a chain. Simply dropping `\n` from `BOUNDARY_RE` is
 * the wrong fix in the other direction, because JavaScript has no required semicolon: it
 * would let `const a = foo` / `views.mark(x)` — two statements — read as one chain.
 *
 * So a newline is crossed only where a chain demonstrably continues across it: the first
 * non-space after it opens with `.` or `?.`, or the last non-space before it closes with
 * one. Both are unambiguous — neither can begin or end a statement — while a bare
 * identifier on the next line (the ASI case) still cuts.
 */
function chainCrossesNewline(text, nl) {
  let a = nl + 1;
  while (a < text.length && (text[a] === ' ' || text[a] === '\t')) a += 1;
  if (text[a] === '.') return true;
  if (text[a] === '?' && text[a + 1] === '.') return true;
  let b = nl - 1;
  while (b >= 0 && /\s/.test(text[b])) b -= 1;
  return text[b] === '.';
}
...
function chainEndsWith(windowText, parent) {
  ...
  for (let i = windowText.length - 1; i >= 0; i -= 1) {
    ...
    if (c === '\n') {
      if (chainCrossesNewline(windowText, i)) continue;
      cut = i;
      break;
    }
    if (BOUNDARY_RE.test(c)) {
      cut = i;
      break;
    }
  }
