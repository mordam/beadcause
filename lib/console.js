...
/** Read-only: research the repo and the tracker, change neither. */
const ALLOWED_TOOLS = [
  'Bash(bd show:*)',
  'Bash(bd list:*)',
  'Bash(bd ready:*)',
...
/**
 * The rules that have to hold on every turn, so they live in the system prompt
 * rather than in the first message where they would drift out of context.
 *
 * The "no banners, no renaming" clause is not decoration. A user-level CLAUDE.md
 * can carry output conventions meant for a terminal — a diff-fenced status banner,
 * a session rename on every turn — and both are noise in a chat bubble; the rename
 * additionally burns a denied tool call, since this allowlist has no shell.
 */
const SYSTEM = `You are the **bead console** in beadcause: a chat on the user's phone where they work
...
When you have enough to be useful, end the message with a proposal — a fenced
 ...
 */
export function createConsole(cfg, workspace, seed = null) {
  ...
  const c = {
    ...
    started: false,
    title: seed ? `From ${seed.id}` : 'New beads',
    status: 'idle',
    error: null,
    seq: 1,
    createdAt: now(),
  ...
  live.set(c.id, c);
  persist(c);
  console.log(`[beadcause] console ${c.id} opened on ${workspace.name}${seed ? ` from ${seed.id}` : ''} in ${dir}`);
  return c;
}
...
function openingContext(c) {
  const lines = [
    `This console is for the **${c.workspace}** workspace. You are running in \`${c.dir}\`, so a plain`,
    '`bd` command already points at the right tracker.',
  ];
  if (c.seed) {
    lines.push(
      '',
 ...
 */
export function sendTurn(cfg, c, text) {
  ...
  const args = [
    ...
    shq(systemFile),
    '--tools',
    shq('Bash,Read,Grep,Glob'),
    '--allowedTools',
    ALLOWED_TOOLS.map(shq).join(' '),
  ];
  if (cfg.consoleModel) args.push('--model', shq(cfg.consoleModel));
  ...
  const timeoutMs = cfg.consoleTimeoutMs ?? 900000;
...
function finish(c, reply, errorDetail) {
  ...
  const parsed = extractProposal(reply.text);
