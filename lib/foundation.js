...
- **The user is on a phone.** Short paragraphs. No walls of text, no ASCII tables, no
  ...
  advocate: {
    ...
    allowedTools: [
      ...
      // tracker" and grants `bd create`, `bd close`, `bd delete` and `bd label`, which
      // on *this* agent is the whole review step: the advocate's entire output is a
      // proposal for Adam to approve, and an agent that can file what it is proposing
      // has been asked not to rather than prevented from it. `writes: false` above is
      // the claim; this list is what makes it true.
      //
      // The eight below are what a survey actually runs (see `surveyPrompt`): what has
      // just closed, what is stuck, what is already open, and the comments under it.
