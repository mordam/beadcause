 ...
 * make the review step — the entire promise of the console — a formality. Those
 * change by editing this file in a release, which is a human writing code, not an
 * agent filing a request.
 */
 ...
 * wrongness is invisible at approval time. "Let the console call bd create" reads as
 * a one-line convenience and silently deletes the review step; a changed output
...
const BASELINES = {
  console: {
    ...
    role: `You are the **bead console** in beadcause: a chat on the user's phone where they work
...
- **The user is on a phone.** Short paragraphs. No walls of text, no ASCII tables, no
  ...
  dispatch: {
    ...
    // Named subcommand by subcommand, not `Bash(bd *)`. That one pattern was four
    // verbs too many — it allowed `bd create`, `bd close`, `bd delete` and
    // `bd label`, so the agent you chat with could file a bead the proposal flow
    // exists to make you approve, and could close the very question it was
    // answering. Adding a verb here should feel like a decision, which is why they
    // are listed. lib/agents.js reads this list rather than keeping its own.
    allowedTools: [
      'Bash(bd show:*)',
      'Bash(bd comments:*)',
      'Bash(bd comment:*)',
  ...
  advocate: {
    ...
    env: {},
    tools: null,
    allowedTools: ['Bash(bd *)', 'Bash(git log:*)', 'Bash(git status:*)', 'Bash(git diff:*)', 'Read', 'Grep', 'Glob'],
    role: null,
  },
 ...
 */
export async function amend(dir, agent, patch, { bead = null, justification = '', by = 'adam' } = {}) {
 ...
 */
export async function decline(dir, agent, { bead = null, request = '', reason = '', by = 'adam' } = {}) {
  baseline(agent);
  const main = await mainCheckout(dir);
