...
- **You close a work bead only when its merge has landed.** The merge is what makes the
  ...
  'review-advocate': {
    ...
    affects: 'The worker whose branch it reviews, which is reopened to answer each round it raises. Then ' +
      'everyone downstream of \`main\`, because its verdict is what is meant to decide whether a branch ' +
      'reaches the merge queue at all, and this repo deploys itself on merge. That last part is intent ' +
      'rather than current fact: the gate that makes a verdict binding is not yet wired (bc-36xx.4), so ' +
      'today the review informs a merge it cannot actually hold.',
    oversight: 'It cannot submit its own review. \`gh pr review\` is deliberately not in its allowlist: the ' +
      ...
      'The two-round cap before a disagreement becomes a card for a person is at present stated in its ' +
      'brief and not enforced in code (bc-36xx.7), so it is a thing the agent is told rather than a control ' +
      'holding it.',
    ...
    // again and learns the shape of *this* repo's merges. All of that is true of this agent
    // too — and as of bc-36xx.5 a window *is* opened on it, so the reason this said no
    // ("nothing runs, and an arm with no runs in it is an empty bucket in `summary`'s
    // comparison") has stopped being a reason. It is still `false` because a tier-3 arm is
    // a grant rather than a consequence, and granting one as a side effect of wiring a door
    // is how an experiment acquires a condition nobody chose: bc-36xx.23 is the bead for it.
    // `beadcause-memory note` carries what it works out about a repo in the meantime.
    ownsRepo: false,
    model: null,
    timeoutMs: null,
    permissionMode: null,
