...
export function routerStamp() {
  // Every leaf the router imports, not just its own file: a fix to lib/service.js is
  // as un-swappable as a fix to bin/router.js, and a stamp that missed it would leave
  // the router silently running the old copy while claiming to be current.
  return hash(['bin/router.js', 'lib/build.js', 'lib/config.js', 'lib/service.js']);
}
