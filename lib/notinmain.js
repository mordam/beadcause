 ...
 *
 *      The count is by sha, though, and a cherry-pick reproduces the same patches under
 *      new ones — the ordinary shape of a stranded-branch recovery on this laptop, not
 *      an edge case — so a nonzero count is gated by `commitsUpstream`'s patch-id check
 *      (`git cherry`) before it is trusted to mean "stranded". bc-1kwl.22 delivered as
 *      #471 this way and the sweep still called it stranded five days later, on a count
 *      that could never fall to zero.
 ...
 *
 * A third shape this function alone cannot see: a child and a grandchild whose ids
 * differ only by where the dots fall. `bc-1kwl.22` and `bc-1kwl.2.2` both tag to
 * `1kwl22` — `tagOf` throws the dots away with everything else that is not a letter or a
 * digit — so this answers `true` for both of them about the one branch. This function
 * only ever sees one id at a time, so it cannot refuse that collision; the caller does,
 * by checking every candidate id against a branch before trusting any single one of them
 * — see `sweepNotInMain`'s `claimants`. Changing what `tagOf` produces was the other way
 * to close this and was rejected: every worktree branch on this laptop was already named
 * with today's `tagOf`, so a scheme that changes the tag orphans every one of them.
 */
 ...
 *
 * Exported for lib/prior.js, which asks the same question of every branch owning a
 * bead's tag and has no reason to keep a second copy of it.
 */
export async function tipOf(dir, branch) {
 ...
 *
 * Exported for the same reason `tipOf` above is: lib/prior.js needs the identical
 * fallback, and a second implementation of "which main" is how the two disagree.
 */
export async function pickBase(dir, base) {
...
/**
 * Is every commit `commitsAhead` counted already in the base under some other sha — a
 * cherry-pick, or a redo on another branch?
 *
 * `commitsAhead` counts by sha and a cherry-pick reproduces the same patches under new
 * ones, so that count never falls to zero and a card filed against a cherry-picked
 * recovery stands for ever: bc-1kwl.22 delivered as #471 this way and the sweep still
 * called it stranded five days later. `git cherry <base> <tip>` answers the question the
 * count cannot — a leading `-` on a commit means its patch is already upstream under some
 * other sha, a leading `+` means it genuinely is not — so this gates what the count is
 * allowed to mean rather than replacing it: `commitsAhead` still does the counting and
 * supplies the subject line the card's prose needs, and stays honest for the branch that
 * really is not upstream.
 *
 * `null` when git could not answer, the same convention `commitsAhead` uses — the caller
 * skips rather than deciding. `false` on no output at all: the caller only asks this once
 * it already knows `ahead.ahead` is nonzero, so empty output here is a `git cherry` this
 * file cannot explain, and false is the side of that which does not drop a real finding.
 */
export async function commitsUpstream(dir, tip, baseRef) {
  const out = await ok(git(dir, ['cherry', baseRef, tip]));
...
If every commit on \`${branch}\` turns out to already be in \`${baseName}\` under a different
sha — a whole-branch cherry-pick or redo — this closes itself on the next sweep with a
note, and there is nothing to answer. One thing it still cannot tell apart from lost
work: a branch only *part* of which landed that way elsewhere. If that is what happened,
let it go.
...
export async function sweepNotInMain(
  ...
  // Every candidate id that owns a given branch, over the same beads this loop is about
  // to walk. `tagOf` collapses a child and a grandchild whose ids differ only by where
  // the dots fall onto one tag — `bc-1kwl.22` and `bc-1kwl.2.2` both become `1kwl22` —
  // so `ownsBranch` alone answers `true` for both about the one branch. Built once, so
  // the collision is known before either id is asked about rather than discovered by
  // filing the same finding twice, which is what actually happened: bc-1kwl.28 and
  // bc-1kwl.29 were one finding, filed twice, one of them against a bead that had never
  // touched the branch.
  const candidates = (beads || []).filter((b) => isCandidate(b, { now, days }));
  const claimants = new Map();
  for (const bead of candidates) {
    for (const branch of branches) {
      if (!ownsBranch(bead.id, branch)) continue;
      if (!claimants.has(branch)) claimants.set(branch, []);
      claimants.get(branch).push(bead.id);
    }
  }
  ...
  for (const bead of candidates) {
    ...
    for (const branch of owned) {
      // A tag collision, caught before anything else is asked. There is no fourth fact
      // to go and get here — the branch name is all `ownsBranch` ever had, and once two
      // ids collapse onto it the branch itself cannot say which one is the real owner —
      // so this is left to neither rather than guessed at. Guessing wrong would only
      // trade the bug for a different wrong answer instead of none.
      const claimedBy = claimants.get(branch) || [];
      if (claimedBy.length > 1) {
        const others = claimedBy.filter((id) => id !== bead.id).join(', ');
        out.skipped.push({
          id: bead.id,
          branch,
          why: `its tag also matches ${others} (see tagOf) — ownership is ambiguous, so neither is asked about it`,
        });
        continue;
      }
      ...
      // GitHub has nothing recorded, and fact 2 is a count by sha — a cherry-pick
      // reproduces the same patches under new ones, the ordinary shape of a
      // stranded-branch recovery on this laptop, not an edge case (see the debriefs'
      // recovery advice). Asked here rather than before the GitHub call, on purpose: a
      // single-commit branch GitHub squash-merged can carry the identical patch-id to
      // its squash commit, and GitHub is the authority for *that* case — see fact 3
      // above — so this only gets the last word once GitHub has already said it knows
      // nothing. See `commitsUpstream`.
      const upstream = await commitsUpstream(main, tip.sha, baseRef.ref);
      if (upstream === null) {
        out.skipped.push({ id: bead.id, branch, why: 'git could not tell whether it is already upstream by patch-id' });
        continue;
      }
      if (upstream) {
        out.skipped.push({
          id: bead.id,
          branch,
          why: `every commit is already in ${baseRef.name} by patch-id — cherry-picked or redone elsewhere, not stranded work`,
          quiet: true,
        });
        continue;
      }
 ...
 *
 * `cherryPicked` is the fourth way the underlying claim stops being true, alongside
 * `state.merged` and `state.open`: `commitsAhead` counts by sha, so a branch cherried
 * onto the base under new shas still reads "ahead" by that count for ever, and only
 * `commitsUpstream`'s patch-id check can tell the follow-up the card is wrong now. It is
 * a distinct case from the plain "the base holds it all" reading below, which is the
 * rev-list count itself having fallen to zero — an ordinary merge or fast-forward, not a
 * cherry-pick, since a cherry-pick's shas never let that count reach zero.
 */
export const correctedReason = (branch, state, baseName, cherryPicked = false) =>
  state.merged
    ...
    : state.open
      ...
      : cherryPicked
        ? `Wrong when it was asked: every commit on \`${branch}\` is already in \`${baseName}\` by patch-id — cherry-picked or redone under different shas, most likely. \`commitsAhead\` counts by sha and could never see past that on its own — see \`commitsUpstream\` in lib/notinmain.js — so this card would have stood for ever without it. There is nothing left to land.`
...
export const correctedComment = (branch, cardId, state, baseName, cherryPicked = false) =>
  `Taking that back: ${cardId} said \`${branch}\` had never reached \`${baseName}\`, and it has — ` +
  `${
    state.merged
      ? `merged as #${state.merged.number}`
      : state.open
        ? `#${state.open.number} is open for it`
        : cherryPicked
          ? `every commit is in ${baseName} by patch-id, cherry-picked or redone elsewhere`
          : `\`${baseName}\` holds all of it now`
  }. ` +
...
export async function followNotInMain(
  ...
  for (const card of open || []) {
    ...
    if (!state.merged && !state.open) {
      ...
      // not hold the commits, and that can stop being true on its own — a re-do on
      // another branch, somebody merging it by hand, or the base taking it in whole.
      ...
      if (ahead.ahead) {
        // The count staying nonzero is not the last word — a cherry-pick reproduces the
        // same patches under new shas, which is exactly the shape `commitsAhead` cannot
        // see past. `commitsUpstream` asks by patch-id instead; see bc-1kwl.22, #471.
        const upstream = await commitsUpstream(main, tip.sha, baseRef.ref);
        if (upstream === null) {
          out.skipped.push({ card: cardId, branch: subject.branch, why: 'git could not tell whether it is already upstream by patch-id' });
          continue;
        }
        if (!upstream) {
    ...
    try {
      await bd.close(ws, cardId, correctedReason(subject.branch, state, baseRef.name, cherryPicked));
    } catch (err) {
    ...
    if (state.open) {
      try {
        await bd.appendNotes(ws, subject.id, `${clearMark(subject.branch)}\n${correctedComment(subject.branch, cardId, state, baseRef.name, cherryPicked)}\n`);
        cleared = true;
      } catch {
    ...
    try {
      await bd.comment(ws, subject.id, correctedComment(subject.branch, cardId, state, baseRef.name, cherryPicked));
    } catch {
    ...
    out.corrected.push({
      ...
      why: state.merged
        ...
        : state.open
          ? `#${state.open.number} is open for it`
          : cherryPicked
            ? `already in ${baseRef.name} by patch-id`
            : `${baseRef.name} holds all of it`,
      cleared,
    });
  }
