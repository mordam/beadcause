...
/**
 * The `--json` field list for `headOf` below, named once so a caller never has to spell
 * it out itself. Kept off `PR_FIELDS`: `files` is a full changed-file list, which every
 * card this repo draws deliberately never asks for (a dozen rows on a board times a
 * hundred lines each is the whole payload, for prose nobody reads while scanning — the
 * same reasoning `detailed()`'s own comment gives for leaving `body` out of `normalize`).
 */
const HEAD_FIELDS = 'number,headRefOid,headRefName,baseRefName,baseRefOid,files';

/**
 * Which commit a pull request's head actually is, and what changed to get there — the
 * one `gh` call bc-dgx7.37's `b7e-head` needs before it ever reaches for `git`.
 *
 * `headRefOid` is the whole point: three sessions before this one each resolved a pull
 * request by fetching its *branch* (`git fetch origin <branch>`) and reading that, which
 * is wrong the moment a resolver pushes to it mid-review — the branch moved, the review
 * did not. Asking GitHub for the oid directly and never for the ref name is what makes
 * the answer immune to that race.
 *
 * `files` rides along for free on the same round trip and is `--list`'s entire answer:
 * GitHub already computed the diff to render the pull request's own "Files changed" tab,
 * so there is no `git diff` to run and no base commit to have fetched first.
 */
export async function headOf(dir, ref) {
  const out = await ghIn(dir, ['pr', 'view', String(ref), '--json', HEAD_FIELDS]);
  const row = JSON.parse(out);
  return {
    number: row.number,
    headOid: row.headRefOid,
    headRefName: row.headRefName,
    baseOid: row.baseRefOid,
