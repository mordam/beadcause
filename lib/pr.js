 ...
 *    closes pull requests. The commits are the session's business; landing them is
 *    Adam's, through the inbox. There is deliberately no `push` in this file — a
 *    daemon that can push is a daemon that can land work nobody approved.
 ...
 * would close on work that had not landed.
 */
export async function merge(dir, number, { method = 'squash', deleteBranch = true } = {}) {
  ...
  if (pr.mergeable === 'CONFLICTING') {
    throw Object.assign(
      new Error(`#${number} conflicts with ${pr.baseRefName} — the branch needs a rebase before it can merge`),
      { status: 409 }
    );
  }
