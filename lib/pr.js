...
export const PR_FIELDS = [
  ...
  'mergeStateStatus',
  'headRefName',
  // The commit at the tip of the branch, which is the only way the review gate can ask
  // whether an approval is still about what is on it — bc-36xx.4, and `reviewedSha` in
  // lib/mergebead.js is the other half. Free beside `statusCheckRollup`: it is one field
  // on the pull request GitHub already loaded, not a walk over anything.
  'headRefOid',
  'baseRefName',
  'additions',
  'deletions',
...
/**
 * `gh` with a working directory, resolved to stdout or rejected with its first stderr
 * line. `input`, when given, is written to stdin and the stream closed — the one thing
 * `gh api` needs to send a JSON body carrying a nested array (a review's inline
 * comments), since its own `-f`/`-F` flags each set one scalar field and have no syntax
 * for a list. Every other caller in this file leaves it out and nothing changes for them.
 */
function gh(args, { cwd, timeout = TIMEOUT_MS, env, input } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = execFile('gh', args, { cwd, timeout, env, encoding: 'utf8', maxBuffer: 16 * 1024 * 1024 }, (err, stdout, stderr) => {
      spend('gh', spawned);
    ...
    if (input != null && child.stdin) {
      // A failed spawn is already reported through the callback above; a write against a
      // dead stream must not throw a second, unhandled error on top of that one.
      try {
        child.stdin.end(input);
      } catch {
        /* the callback above already reports the failure */
      }
    }
  });
}
...
/**
 * A 5xx, a timeout, or the process never reaching the network at all — GitHub, or the
 * path to it, answering with a shrug. None of those says anything about whether the
 * repo exists, which is the one thing `seenBy` below is actually trying to find out, so
 * it is worth a retry rather than a verdict. A clean 404, "could not resolve to a
 * Repository", or no remote configured at all are real answers, and none of them look
 * like this.
 *
 * Measured 2026-08-17 ~13:45 and ~15:20 ADT (bc-36xx.14): `gh repo view` — GraphQL —
 * returned `HTTP 503: No server is currently available to service your request` on
 * roughly four calls in five while `gh api /rate_limit` — REST — answered fine
 * throughout, and running the exact same command five minutes later worked. That is
 * the shape this exists to catch: an outage in one API, not an absent repo.
 */
function isTransientErr(err) {
  const msg = String(err?.message || err?.detail || '');
  if (/\bHTTP 5\d\d\b/.test(msg)) return true;
  if (err?.killed || err?.signal) return true; // gh's own subprocess timed out
  return /^E(NOTFOUND|CONNRESET|CONNREFUSED|TIMEDOUT|AI_AGAIN|PIPE)$/.test(String(err?.code || ''));
}

/**
 * How many times a transient failure is retried, and how long between tries — long
 * enough that a blip clears, short enough that a delivery is not left deciding whether
 * a repo exists for minutes.
 */
const PROBE_RETRIES = 2;
const PROBE_BACKOFF_MS = 250;

/**
 * Which checkouts' most recent probe came back empty because nothing could get an
 * answer out of GitHub, rather than because no account here can see a repo there.
 * `bin/deliver.js` never reads this directly — `probeTransient` and `noRepoMessage`
 * below are the seam — but it is what tells the two apart, so it lives beside the one
 * function that fills it in.
 */
const transientDirs = new Set();

/** Whether `dir`'s most recent probe came up empty because GitHub itself never answered. */
export function probeTransient(dir) {
  return transientDirs.has(dir);
}

/**
 * What one account can do with `dir` — its `owner/repo`, and whether that account may
 * push there. Null when the account cannot see the repo at all, whether that is because
 * it genuinely is not there or because GitHub would not say.
 *
...
async function seenBy(dir, user) {
  for (let attempt = 0; ; attempt += 1) {
    try {
      ...
      if (isTransientErr(err) && attempt < PROBE_RETRIES) {
        await wait(PROBE_BACKOFF_MS * (attempt + 1));
        continue;
      }
      if (isTransientErr(err)) transientDirs.add(dir);
      return null;
    }
  }
}
...
async function resolve(dir) {
  ...
  // Cleared before every fresh sweep, so a `probeTransient` a caller reads right after
  // this call reflects THIS sweep — never a leftover from an outage that has since
  // cleared, or from a different dir whose probe happened to land on the same key.
  transientDirs.delete(dir);
...
/**
 * GitHub's own wording for "the account `resolve()` remembered for this checkout can no
 * longer write here" — not a status code, because `gh` exits 1 either way and carries
 * nothing more specific.
 *
 * This is the one shape a stale `accountFor` entry actually produces (bc-khoe.11):
 * `accountFor`/`accountsCache` are cached for the life of the process, and a `gh auth
 * switch` run by some *other* session — sharing this Mac's one `~/.config/gh/hosts.yml`
 * — repoints the ambient account out from under a probe this process already ran and
 * cached. Two sentences, both measured against this repo's own outage on 2026-08-17:
 * `pr create` says "must be a collaborator"; `pr merge` says "does not have the correct
 * permissions".
 */
function isPermissionErr(err) {
  return /must be a collaborator|does not have the correct permissions/i.test(String(err?.message || ''));
}

/**
 * Forget which account `dir` resolves to, and which account `gh auth status` calls
 * active — the two things a `gh auth switch` elsewhere on the Mac can invalidate for a
 * process that already cached an answer. Narrower than `forgetAvailability()`: a
 * write's own permission refusal justifies distrusting *this*, not the token and
 * reviewer caches too, which nothing here has given a reason to doubt.
 */
function forgetAccount(dir) {
  accountFor.delete(dir);
  accountsCache = null;
}
 ...
 *
 * Retries exactly once, against a freshly re-resolved account, when the account this
 * call ran as turns out not to be able to write here — see `isPermissionErr` above. A
 * single unconditional retry costs one extra sweep on the rare occasions the *first*
 * resolve of a checkout already lands on a write-refusing account (nothing here can
 * write to it at all, or a genuine race between the probe and the write), and buys
 * back every case this exists for: a long-lived process whose cached answer was right
 * when it was made and has since gone stale under it.
 */
async function ghIn(dir, args, opts = {}) {
  try {
    return await gh(args, { ...opts, cwd: dir, env: await envOf(await accountIn(dir)) });
  } catch (err) {
...
/**
 * The sentence a delivery says when `slugFor` came back null — which of two very
 * different things, decided by `probeTransient` above rather than guessed at here.
 *
 * bc-36xx.14: `bin/deliver.js` used to have one message for this, worded as if a null
 * could only mean the checkout has no GitHub remote. During a real GraphQL outage
 * (measured 2026-08-17) that told a session the repo was not there, to leave its
 * finished branch unpushed and say so on the bead — the one ending this whole channel
 * exists to avoid, over a repo that opened its pull request five minutes later. The
 * two endings are opposite advice — give up here, or try again shortly — so which one
 * prints has to come from whether GitHub ever actually answered, not from the bare
 * fact that the answer was null.
 */
export function noRepoMessage(dir, branch, beadId) {
  if (probeTransient(dir)) {
    return (
      `GitHub did not answer whether a repo is visible from ${dir}, after ${PROBE_RETRIES + 1} tries — ` +
      'that looks like an outage, not an absence. ' +
      `The work is committed on ${branch}; retry this delivery once GitHub is back, rather than ` +
      'treating the branch as done.'
    );
  }
  return (
 ...
 *
 * `search` is GitHub's own issue-search syntax (`listMergedSince` below already uses it
 * for a `merged:` range), passed through unchanged as `--search`. lib/prior.js is the
 * caller: a bead can be named in a pull request's title or body without ever owning the
 * branch it is on — bc-5e85's own bead was raised as the thing PR #488 does *not* fix,
 * on a branch tagged for a different bead entirely — and `--head` cannot find that,
 * because it never looks at the text. `head` and `search` are mutually exclusive in
 * `gh`'s own eyes; nothing here enforces that, since no caller has ever wanted both.
 */
export async function list(dir, { limit = 40, state = 'all', head = '', search = '' } = {}) {
 ...
 *
 * **`ghIn`, not `gh` — this is a write, and the ambient account cannot do it** (bc-johbj).
 * `update-branch` needs `push`, and on this Mac `gh auth status` has two logins with the
 * *read-only* one active: `viewerPermission` for the ambient account on `mordam/beadcause`
 * reads `READ` (measured 2026-08-26). GitHub masks a missing `push` on this endpoint as a
 * flat **404**, not a 403, so a bare `gh` here answered `Not Found (HTTP 404)` on every
 * pull request regardless of its state — twelve for twelve on the first tick after
 * bc-xl7n.121 shipped, and the queue read each one as a fact about that branch. `pr.merge`
 * a hundred lines down was already going through `ghIn`, which is why #777 merged as
 * `mordam` in the same tick that could not update a single branch: the daemon's
 * environment was never the difference, the call was. Every other write in this file
 * either goes through `ghIn` or passes an explicit `env` because it means a *particular*
 * account (the reviewer); this was the one running as whoever happened to be active.
 *
...
export async function updateBranch(dir, number, { timeoutMs = TIMEOUT_MS } = {}) {
  try {
    await ghIn(dir, ['api', '--method', 'PUT', `repos/{owner}/{repo}/pulls/${Number(number)}/update-branch`], {
      timeout: timeoutMs,
    });
...
/**
 * What lies between two commits — how the branch got from one to the other.
 *
 * bc-36xx.4's whole question, asked of the one endpoint that answers it in a single call:
 * `compare` returns the commits *between* rather than every commit on the branch, and it
 * returns each one's parents, which is what tells a downmerge from a worker's own push
 * (see `classifyCommits` in lib/reviewgate.js for why that is the distinction).
 *
 * `{ status, commits: [{ sha, parents, message, login }] }`, or **null**, and null is an
 * ordinary answer the caller must handle rather than an error — `baseChecks` above makes
 * the same shape and the same argument. `status` is GitHub's own word: `identical`,
 * `ahead`, `behind` or `diverged`, and only the first two mean the history the caller
 * remembers is still the history that is there.
 *
 * Abbreviated shas are fine at both ends: GitHub resolves them, which matters because a
 * review records whatever `gh` handed it and that is seven characters as often as forty.
 */
export async function commitsBetween(dir, from, to, { timeoutMs = TIMEOUT_MS } = {}) {
  ...
  try {
    raw = await gh(
      [
        'api',
        `repos/{owner}/{repo}/compare/${encodeURIComponent(a)}...${encodeURIComponent(b)}`,
        '--jq',
        '{status: .status, commits: [.commits[] | {sha: .sha, parents: (.parents | length), ' +
 ...
 *
 * Goes out through `ghIn`, same as every other write in this file — this is the queue
 * talking, not the reviewer, so it runs as the account `resolve` picked for `dir` rather
 * than whatever `gh` is ambiently on. It used to call bare `gh` with no env at all, which
 * meant it posted as the active login regardless of which account actually owns the pull
 * request; harmless while the two happened to read the same on this Mac, and exactly the
 * identity `approve` below deliberately avoids for its own comment.
 */
export async function comment(dir, number, text) {
  await ghIn(dir, ['pr', 'comment', String(number), '--body', text]);
}
 ...
 */
/**
 * The reviewer's identity, resolved once, for any write that must go out as it rather
 * than as the account everything else in this file runs as. Null when there is no
 * second account — see `reviewerFor`, which this wraps, for what that means and why it
 * is an ordinary answer rather than a failure.
 *
 * Shared by `approve`, `submitReview`, `reviewThreads` and `resolveThread` below: all
 * four are either outward-facing writes that must not go out as the pull request's own
 * author, or a read paired tightly enough with one of those writes that reading under a
 * third identity would be one more account for a caller to keep straight for no gain.
 */
async function reviewerContext(dir) {
  const reviewer = await reviewerFor(dir);
  if (!reviewer) return null;
  return { reviewer, env: await envOf(reviewer.user) };
}
...
export async function approve(dir, number, { body = '', note = '' } = {}) {
  ...
  const ctx = await reviewerContext(dir);
  if (!ctx) {
    return {
      ...blank,
...
/**
 * Every valid `event` a review submission may carry. `PENDING` is left out on purpose: it
 * leaves a draft review sitting on the pull request with nothing submitted, which is not
 * a state any caller here wants — every review this daemon starts, it finishes.
 */
const REVIEW_EVENTS = new Set(['APPROVE', 'REQUEST_CHANGES', 'COMMENT']);

/**
 * One of the reviewer's comments, in the shape the reviews endpoint takes — a *line* on
 * a file, not a byte offset into the diff. `side` is always the new version of the file:
 * nothing here reviews a deletion, and GitHub defaults to `RIGHT` regardless, so leaving
 * it implicit would be leaving out the one field that says so.
 *
 * `null` for anything GitHub would refuse outright — no path, no positive integer line,
 * no body — so a submission drops a broken comment rather than finding out from a 422
 * after every other comment in the same review has already gone out with it.
 */
const inlineComment = (c) => {
  const path = String(c?.path || '').trim();
  const line = Number(c?.line);
  const body = String(c?.body || '').trim();
  if (!path || !Number.isInteger(line) || line <= 0 || !body) return null;
  return { path, line, side: 'RIGHT', body };
};

/**
 * Submit a review carrying inline comments — one per file and line — rather than a bare
 * approve or a single body on its own. `approve()` above is the narrower case Adam asked
 * for by name: an approval, always disclosed, always as the reviewer. This is the general
 * shape underneath it, for the ordinary case where the ReviewAdvocate has objections to
 * raise on specific lines, whether or not the review also approves.
 *
 * Goes through the same reviews endpoint `approve()` does, and for the same reason it is
 * the *only* endpoint here that can carry them at all — `gh pr review` has no flag for an
 * inline comment. Sent as a JSON body over `--input -` rather than as `-f` fields, because
 * `comments` is an array and `gh api`'s field flags each set one scalar value; there is no
 * field syntax for a nested list. `approve()` keeps its `-f` fields, which carry no array
 * and have their own pinned test already.
 *
 * Runs under the reviewer identity, exactly like `approve()` and for the same reason:
 * GitHub refuses a review — inline comments included — from the pull request's own
 * author, which is the account able to write to this checkout on this Mac. A one-login
 * Mac returns `{ submitted: false, reason }` rather than throwing: there is no second
 * identity to submit as, and the caller's fallback is the same one `approve()`'s is.
 *
 * `dropped` counts comments that did not survive `inlineComment` above, so a caller that
 * gets back `submitted: true, dropped: 2` knows the review went out lighter than it was
 * asked to. A submission with nothing left at all — no body, no comment survived — is
 * refused before `gh` is reached, the same direction `approve()`'s empty-body refusal is.
 */
export async function submitReview(dir, number, { event = 'COMMENT', body = '', comments = [] } = {}) {
  const blank = { submitted: false, reviewer: '', permission: '', url: '', at: '', reason: '', dropped: 0 };

  const ev = REVIEW_EVENTS.has(String(event || '').trim().toUpperCase()) ? String(event).trim().toUpperCase() : 'COMMENT';
  const text = String(body || '').trim();
  const wanted = Array.isArray(comments) ? comments : [];
  const inline = wanted.map(inlineComment).filter(Boolean);
  const dropped = wanted.length - inline.length;

  if (!text && !inline.length) {
    return { ...blank, dropped, reason: 'nothing to submit — no body and no comment survived validation' };
  }
  ...
  if (!ctx) {
    return {
      ...blank,
      dropped,
      reason: 'there is no second GitHub account here, and an account may not review a pull request it opened itself',
    };
  }

  const { reviewer, env } = ctx;
  const found = { ...blank, dropped, reviewer: reviewer.login, permission: reviewer.permission || '' };

  const payload = { event: ev };
  if (text) payload.body = text;
  if (inline.length) payload.comments = inline;

  let review = null;
  try {
    const out = await gh(
      ['api', '--method', 'POST', `repos/{owner}/{repo}/pulls/${Number(number)}/reviews`, '--input', '-'],
      { cwd: dir, env, input: JSON.stringify(payload) }
    );
    review = JSON.parse(out);
  } catch (err) {
    return { ...found, reason: err.message };
...
/**
 * The GraphQL query that lists a pull request's review threads — REST has no idea threads
 * are resolvable at all, so this is the one place in the file that speaks GraphQL rather
 * than REST. `databaseId` on each comment is the same number the REST comments endpoint
 * would give it; carried because that is the id a review block's comments are meant to be
 * anchored to, never GraphQL's own opaque node id, which no caller here has a use for.
 *
 * A hundred threads and fifty comments a thread is far past anything one review of this
 * repo produces — `MAX_REVIEW_COMMENTS` in lib/mergebead.js caps a round at twenty — so
 * there is no pagination: a pull request with more than that has bigger problems than a
 * missing page.
 */
...
/**
 * A dir+number pair whose most recent `reviewThreads` came back null because GitHub
 * itself never answered, rather than because the pull request genuinely has no threads
 * or nobody here can read them — the same distinction `probeTransient` draws for
 * `slugFor`, kept separately because a repo can resolve fine while its own thread read
 * still 503s (bc-36xx.26). Keyed by pull request, not just `dir`, since one checkout
 * answers for every pull request against that repo.
 *
 * Separate from `transientDirs` but never disagreeing with it: `reviewThreads` folds
 * `probeTransient(dir)` in on every early return, so an outage that took out the repo
 * probe before the thread query was ever reached is flagged here too.
 */
const threadsTransientKeys = new Set();
const threadsKey = (dir, number) => `${dir}::${number}`;

/** Whether `dir`'s pull request `number` most recently failed to read threads because GitHub never answered. */
export function threadsTransient(dir, number) {
  return threadsTransientKeys.has(threadsKey(dir, number));
}

/**
 * `gh api graphql`, retried the same way `seenBy` retries `repo view` — reusing
 * `isTransientErr`/`PROBE_RETRIES`/`PROBE_BACKOFF_MS` rather than a second classifier
 * for the same shape of failure on the same API (bc-36xx.26). A non-transient error —
 * a genuine refusal, a bad query — is thrown straight through on the first try, exactly
 * as before this existed.
 */
async function ghGraphql(args, opts) {
  for (let attempt = 0; ; attempt += 1) {
    try {
      return await gh(args, opts);
    } catch (err) {
      if (isTransientErr(err) && attempt < PROBE_RETRIES) {
        await wait(PROBE_BACKOFF_MS * (attempt + 1));
        continue;
...
/**
 * A pull request's review threads, each with its GitHub-assigned id, whether *GitHub*
 * considers it resolved, and the comments inside it.
 *
 * This is the read `resolveThread` below is checked against, and the reason both exist:
 * a beadcause review block can say a comment is `resolved` because the reviewer answered
 * it, but "unresolved" on the pull request page is GitHub's own state, set only by
 * `resolveReviewThread` — the two must never be read as the same fact, or an id anchored
 * here would be inferred rather than checked. `null` when the repo cannot be resolved or
 * the call fails outright, exactly as `baseChecks` above: an ordinary answer every caller
 * must handle rather than an error — but a null caused by GitHub never answering is told
 * apart from a null that means "genuinely no threads" by `threadsTransient` above, so a
 * caller does not read an outage as "nothing to resolve" (bc-36xx.26).
 *
 * Runs under the reviewer identity, the same as `submitReview` and `resolveThread` below
 * it — one identity for the whole of a review's traffic is one fewer account a caller has
 * to keep straight, and a one-login Mac's `null` here is the same "nothing to anchor to
 * yet" as everywhere else in this file.
 */
export async function reviewThreads(dir, number) {
  // Cleared before every fresh read, the same way `resolve()` clears `transientDirs` —
  // so a flag a caller reads right after this call reflects THIS read, never a leftover
  // from an outage that has since cleared. Above the bails below rather than beneath
  // them, because a read that never gets as far as GraphQL is still a read.
  const key = threadsKey(dir, number);
  threadsTransientKeys.delete(key);

  // Every null below is "GitHub never answered" whenever the repo probe underneath it
  // says so. `slugFor` -> `resolve` -> `seenBy` is `gh repo view`, which is GraphQL too:
  // the outage this bead was filed for (2026-08-17) is the one where *that* call 503'd,
  // on roughly four requests in five, so the likelier way for this function to meet an
  // outage is to never reach its own query at all. Flagging only the query's own failure
  // left the flag reading "this pull request genuinely has no threads" in exactly the
  // case it exists to name (measured, round 1 of #655's review). `probeTransient` is the
  // authority rather than a second guess at the same classification, which is the whole
  // point of reusing `isTransientErr` in the first place.
  const unanswered = () => {
    if (probeTransient(dir)) threadsTransientKeys.add(key);
    return null;
  };

  const slug = await slugFor(dir);
  if (!slug) return unanswered();
  const [owner, repo] = slug.split('/');
  if (!owner || !repo) return unanswered();

  // A null here is ordinarily "there is no second account on this Mac" — but
  // `reviewerFor` reaches it through `resolve`/`seenBy` as well, so an outage can land
  // on this bail rather than the one above, and `probeTransient` tells them apart. A
  // cached null never probes at all, which is why it correctly stays "no reviewer".
  const ctx = await reviewerContext(dir);
  if (!ctx) return unanswered();

  let raw;
  try {
    raw = await ghGraphql(
      ['api', 'graphql', '-f', `query=${REVIEW_THREADS_QUERY}`, '-f', `owner=${owner}`, '-f', `repo=${repo}`, '-F', `number=${Number(number)}`],
      { cwd: dir, env: ctx.env }
    );
  } catch (err) {
    if (isTransientErr(err)) threadsTransientKeys.add(key);
    return null;
  }
  ...
  return nodes
    .map((t) => ({
      ...
      comments: (Array.isArray(t?.comments?.nodes) ? t.comments.nodes : []).map((c) => ({
...
/**
 * The GraphQL mutation that resolves one thread. There is no REST equivalent at all —
 * thread resolution only exists on this API — which is `reviewThreads` above's reason for
 * speaking GraphQL too.
 */
...
/**
 * Resolve one named thread — the one write in this whole file that only the reviewer may
 * make, in Adam's own words: *"only the reviewer marks a thread resolved."* Nothing else
 * here calls this and nothing else should: a worker's `changed` or `declined` answers a
 * comment, it does not close the thread, and the review block's own `resolved` field
 * (lib/mergebead.js) is commented "only the reviewer writes this" for the same reason.
 *
 * `threadId` is GitHub's own id from `reviewThreads` above — never the review block's own
 * comment id, which is beadcause's bookkeeping and means nothing to this mutation.
 *
 * Runs under the reviewer identity for the reason `approve()` and `submitReview()` do:
 * this is outward-facing GitHub state, and the account that opened the pull request may
 * not be the one that settles an argument on it. Returns `{ resolved: false, reason }` on
 * a one-login Mac rather than throwing — there is no reviewer to resolve as, so nothing is
 * attempted — and the same shape for any other failure, GitHub's own sentence included.
 * A 5xx or network error is retried the way `seenBy` retries `repo view` (bc-36xx.26); if
 * it still fails, the answer carries `transient: true` so a caller can tell "GitHub never
 * answered" apart from a genuine refusal — "no such thread", a permission error — which
 * come back the same shape without it.
 */
export async function resolveThread(dir, threadId) {
  const id = String(threadId || '').trim();
  if (!id) return { resolved: false, reason: 'no thread id given' };

  const ctx = await reviewerContext(dir);
  if (!ctx) {
    return {
      resolved: false,
      reason: 'there is no second GitHub account here, and resolving a thread belongs to the reviewer login',
    };
  }

  let raw;
  try {
    raw = await ghGraphql(['api', 'graphql', '-f', `query=${RESOLVE_THREAD_MUTATION}`, '-f', `threadId=${id}`], { cwd: dir, env: ctx.env });
  } catch (err) {
    return isTransientErr(err)
      ? { resolved: false, reason: err.message, transient: true }
      : { resolved: false, reason: err.message };
  }
  ...
  try {
    ...
    return { resolved: false, reason: 'gh returned something that is not JSON' };
  }
  const thread = parsed?.data?.resolveReviewThread?.thread;
  if (!thread?.isResolved) return { resolved: false, reason: 'GitHub did not report the thread as resolved' };
...
function rollup(checks) {
  ...
  let newest = null;
  for (const c of list) {
    // See `at` below. Newest wins, and an unparseable stamp is skipped rather than
    // becoming `Invalid Date` — which compares false against everything and would make a
    // stale run look current.
    for (const stamp of [c.completedAt, c.completed_at, c.startedAt, c.started_at, c.createdAt, c.created_at]) {
      if (!stamp) continue;
    ...
    if (['SUCCESS', 'NEUTRAL', 'SKIPPED'].includes(state)) passing++;
      ...
      // Where the failure can be *read*, kept beside the name because the name is the one
      // thing that cannot say what went wrong — see `checkFailure`. Both shapes again:
      // CheckRun has `detailsUrl`, StatusContext `targetUrl`, and either can be missing.
      if (c.detailsUrl || c.details_url || c.targetUrl || c.target_url) {
        failedUrls.push(String(c.detailsUrl || c.details_url || c.targetUrl || c.target_url));
      }
    }
  }
  return {
    ...
    /**
     * The links behind the red ones, in the order they came and to the same bound.
     *
     * Not aligned index-for-index with `failed`, and deliberately not promised to be: a
     * check run with no name and one with no URL are both ordinary, and pairing them up
     * would mean inventing an entry for whichever half is missing. What this is for is
     * `checkFailure`, which wants *a* place to read the failure from and does not care
     * which of the red checks it came from when there is only ever one here.
     */
    failedUrls: failedUrls.slice(0, 6),
    state: failing ? 'failing' : pending ? 'pending' : list.length ? 'passing' : 'none',
    /**
     * When these checks last had anything to say — the newest timestamp across them.
     *
     * Carried because a rollup on its own cannot answer the question bc-91srt is about:
     * *what base was this measured against*. GitHub builds `refs/pull/N/merge` for a
     * pull request, so a run is a verdict on the branch **merged with the base as it
     * stood when the run fired**. While the base is broken every open pull request goes
     * red, and those runs stay red afterwards because nothing re-runs them — so a gate
     * comparing names against a base that has since been repaired reads every one of
     * them as a failure the branch introduced. `gateVerdict` uses this to tell a verdict
     * about the branch from a verdict about a base that is no longer there.
     *
     * Null when nothing carries a time, which is an ordinary answer: a caller must treat
     * "no timestamp" as "cannot tell", never as "recent". Both shapes are read because
     * `statusCheckRollup` mixes them — CheckRun has `completedAt`/`startedAt`, the REST
     * check-runs endpoint the snake_case pair, and a StatusContext only `createdAt`.
     */
...
/**
 * How much of a failing check's log is worth carrying — bc-xl7n.116.
 *
 * Twelve lines of at most 160 characters, and no more than 900 in total, because the
 * whole point of carrying it is that it is read on a phone. The failure this was measured
 * against (#543, 2026-08-21) came to twelve lines and 774 characters and contained the
 * file, the line, the offending source and the remedy — a log of 16,149 lines answered in
 * under a screenful. A cap that fits *that* is a cap that fits.
 */
export const FAILURE_LINES = 12;
export const FAILURE_LINE_CHARS = 160;
export const FAILURE_CHARS = 900;

/**
 * Colour, as a CI log is full of it and a bead's notes are not a terminal.
 *
 * The `[...m` half is optional so that an escape character with nothing recognisable after
 * it is dropped too. A stray one is invisible in every reader and survives a naive strip,
 * and what it leaves behind is a control byte in a tracker field — which is the one thing
 * `test/filter.mjs` fails this repo for.
 */
const ANSI = /\u001b(?:\[[0-9;]*[A-Za-z])?/g;

/**
 * One line of `gh run view --log-failed`, with everything that is about the *log* rather
 * than about the failure taken off.
 *
 * `gh` prefixes every line with the job name, the step name and the run's own timestamp,
 * tab-separated — three fields that are identical on all 16,000 lines and eat 60
 * characters of a 160-character budget. The timestamp is what anchors the strip: matching
 * on tabs alone would cut into any line whose own content contains one.
 *
 * `##[...]` is the Actions runner talking to itself. The text after `error`, `notice` and
 * `command` is the runner reporting something real, so the marker comes off and the line
 * stays; `group`, `endgroup`, `debug` and `warning` are the runner's own bookkeeping and
 * are dropped whole by `runnerNoise` below.
 */
function cleanLogLine(line) {
  return String(line || '')
    .replace(ANSI, '')
    .replace(/^(?:[^\t\n]*\t){0,2}\d{4}-\d\d-\d\dT[0-9:.]+Z ?/, '')
    .replace(/^##\[(?:error|notice|command)\]/, '')
    .replace(/\s+$/, '');
}

/**
 * A line reporting something that *passed*, which by construction is not why the check is
 * red.
 *
 * This is the whole of what makes the excerpt useful rather than merely short. A test
 * runner's last screenful is overwhelmingly ticks — on the run this was measured against,
 * the failing assertion was twenty lines above the end and every line between them said
 * `✓`. A plain tail names the suite and stops there; dropping the passes reaches the
 * assertion, the file, the line and the fix in the same twelve lines.
 *
 * Deliberately a small set of shapes rather than a parser: `✓`/`✔`/`√` is what this
 * repo's runner and most others print, `ok` is TAP, `PASS` is jest and its like, and
 * `N passed` is a tally. Anything it does not recognise is kept, which is the safe
 * direction — an unrecognised runner yields a plain tail, which is what there was before.
 */
const passingLine = (s) => /^(?:[✓✔√]|ok\b|PASS\b|\d+ passed\b)/.test(s.trim());

/** The runner's own noise, which says nothing about the branch. */
const runnerNoise = (s) => {
  const t = s.trim();
  if (!t) return true;
  if (t.startsWith('##[')) return true;
  // The one line every failed Actions job ends with, and the one that made this bead:
  // "Process completed with exit code 1" is the exact shape of a refusal that names the
  // failure without saying anything about it.
  if (/^Process completed with exit code/.test(t)) return true;
  return /^(?:Post job cleanup|Cleaning up orphan processes)/.test(t);
};

/**
 * The tail of a failed check's log, as the few lines that say what broke.
 *
 * Pure, and exported for that reason: what is hard here is the *selection*, and a test
 * that has to reach GitHub to assert on it is a test nobody runs. `checkFailure` is the
 * three lines of I/O that wrap it.
 *
 * Read backwards from the end, because a runner reports the failure and then stops. Lines
 * are collected until the budget runs out and then put back in order, so what survives a
 * tight budget is the *end* of the failure rather than the start of the log — which is
 * the right half: the summary line naming the suite is the last thing printed.
 *
 * An array rather than a string, because every caller has a different idea of how to draw
 * it and one of them is YAML in a bead's notes, where a list of strings is unambiguous
 * and a folded multi-line scalar is a round-trip to be wrong about.
 */
export function failureExcerpt(log, { lines = FAILURE_LINES, chars = FAILURE_LINE_CHARS, total = FAILURE_CHARS } = {}) {
  const raw = String(log || '').split('\n');
  const out = [];
  let spent = 0;
  for (let i = raw.length - 1; i >= 0 && out.length < lines; i--) {
    const s = cleanLogLine(raw[i]);
    if (runnerNoise(s) || passingLine(s)) continue;
    // Markers, because this ends up inside a marked block in a bead's `notes` and a log
    // line that happened to carry one would close it early.
    const kept = (s.length > chars ? `${s.slice(0, chars - 1)}…` : s).replace(/<!--|-->/g, '');
    if (spent + kept.length > total) break;
    spent += kept.length;
    out.push(kept);
  }
  return out.reverse();
 ...
 *
 * The queue's refusal names the check and stops: on this repo there is exactly one check
 * and it is called `test`, so "1 check failing (test)" is the answer for every red pull
 * request there has ever been and tells nobody anything. The fact that decides whether a
 * refusal is a two-line fix or a flake is twenty lines inside the log, and reading it has
 * cost an advocate or a human a trip to GitHub every time.
 *
 * `--log-failed` rather than the check run's **annotations**, and that is a measurement
 * rather than a preference: the annotations on the run this was written against are two
 * rows, one a Node deprecation warning and the other `Process completed with exit code 1`
 * — the same non-answer the refusal already gives. The log is where the answer is.
 *
 * Best-effort in every direction. No URL that names an Actions run, `gh` refusing, a
 * timeout, an empty log: all of them come back as an empty list, and the caller's refusal
 * is exactly the sentence it was before. Nothing here may turn a merge into a failure.
 */
export async function checkFailure(dir, urls, { timeoutMs = TIMEOUT_MS, lines, chars, total } = {}) {
  const list = (Array.isArray(urls) ? urls : [urls]).map((u) => String(u || ''));
  const run = list.map((u) => u.match(/\/actions\/runs\/(\d+)/)).find(Boolean);
  if (!run) return [];
  let log;
  try {
    log = await gh(['run', 'view', run[1], '--log-failed'], { cwd: dir, timeout: timeoutMs });
  } catch {
    return [];
  }
  return failureExcerpt(log, { lines, chars, total });
}
...
function normalize(pr) {
  ...
  return {
    ...
    /** What is at the tip of it — see `PR_FIELDS`, and lib/reviewgate.js for who asks. */
