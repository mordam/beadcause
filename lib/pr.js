 ...
 *
 * **`total` is the count; `commits` is a page of it.** GitHub's compare endpoint returns
 * at most **250** entries in `.commits` and reports the real figure separately, in
 * `.total_commits` — measured on this repo on 2026-08-23, `compare/4ea4b599...main`
 * answers `{returned: 250, total_commits: 348}`. So `commits.length` is a count only
 * while it is under 250 and silently saturates above it, which is the range every
 * interesting behind-count on a stale branch lives in (bc-4r10.1's was 593). Callers
 * that want "how far behind" must read `total`; callers that want to *classify* the
 * commits (lib/reviewgate.js) still read `commits`, and for those a truncated page is
 * a page, which is why both are returned rather than one being derived from the other.
 */
export async function commitsBetween(dir, from, to, { timeoutMs = TIMEOUT_MS } = {}) {
  ...
  try {
    raw = await gh(
      [
        ...
        '--jq',
        '{status: .status, total: .total_commits, commits: [.commits[] | {sha: .sha, parents: (.parents | length), ' +
  ...
  return {
    ...
    // The honest count, when GitHub gave one. A fixture or an older payload without
    // `total_commits` falls back to the page length, which is right for every compare
    // small enough not to have been truncated in the first place.
    total: Number.isFinite(out.total) ? out.total : (Array.isArray(out.commits) ? out.commits.length : 0),
...
/**
 * Which files a pull request's diff touches — paths only, nothing about what changed.
 *
 * `gh pr diff <number> --name-only`, asked of GitHub rather than of any checkout on
 * disk — bc-4r10.19, and the reason that distinction matters: bc-khoe.30.6 found one
 * of its three colliding pull requests came from a worktree that had already been
 * retired, so a survey of local worktrees would never have seen it. This has no such
 * blind spot, because it never looks at a worktree at all.
 */
export async function filesTouched(dir, number) {
  const out = await ghIn(dir, ['pr', 'diff', String(number), '--name-only']);
  return out
    .split('\n')
    .map((l) => l.trim())
    .filter(Boolean);
 ...
 * must handle rather than an error.
 *
...
export async function resolveThread(dir, threadId) {
  ...
  let raw;
  try {
    raw = await gh(['api', 'graphql', '-f', `query=${RESOLVE_THREAD_MUTATION}`, '-f', `threadId=${id}`], { cwd: dir, env: ctx.env });
  } catch (err) {
