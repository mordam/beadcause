...
function ownWorkspace(cfg, root) {
  ...
  for (const rec of listDeploys({ limit: 200 })) {
    ...
    if (!LIVE.has(rec.status)) {
      ...
      if (ended && now - ended < graceMs) {
        return held(rec, ended + graceMs, `a deploy of ${rec.workspace} finished ${ago(ended)} (${rec.id})`);
      }
      continue;
    }

    if (running(rec)) return held(rec, now + graceMs, `a deploy of ${rec.workspace} is in flight (${rec.id})`);
    ...
    if (started && now - started < ORPHAN_QUIET_MS) {
      return held(rec, now + graceMs, `a deploy of ${rec.workspace} restarted this daemon ${ago(started)} and is not settled yet (${rec.id})`);
    }
  }
  return null;
}
