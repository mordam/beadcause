 ...
 *
 * The pause it sets is one of six places that decide whether something may run with
 * nobody watching (the check itself is `a.paused` in lib/advocate.js's tick loop) —
...
export function createAdmin(cfg, { advocates, terminals }) {
  ...
  /**
   * Stop the ptys, keeping the conversations.
   *
   * `suspend` rather than `close` is the whole point: closing is recorded as
   * `exited` and is never offered back, which is right for the ✕ on a terminal and
   * wrong for a pause. Suspending leaves each one `resumable`, exactly as a daemon
   * restart would — so resume brings back the session you were talking to rather
   * than a new one wearing the same bead.
   */
  function pauseTerminals(names) {
    ...
    for (const t of terminals.list()) {
      ...
      // The id is the handle to resume by; the rest is what it would take to open a
      // replacement if the record is gone by then. Captured before the suspend,
      // because a reaped terminal takes its bead and its size with it.
      const record = { id: t.id, workspace: t.workspace, bead: t.bead || null, cols: t.cols, rows: t.rows, closedAt: iso() };
      terminals.suspend(t.id);
  ...
  /**
   * Bring back exactly the terminals this suspended.
   *
   * Two paths, and the difference is worth reporting rather than smoothing over.
   * Normally the record is still there and `resume` reattaches `claude` to its own
   * session id — the conversation continues. If it has gone (reaped after long
   * enough, or the record deleted), the fallback opens a fresh one in the same
   * directory on the same bead, which is a *new* conversation. `opened` and
   * `fresh` are separate lists so the screen can say which it got.
   */
  function resumeTerminals(names) {
    ...
    for (const record of [...state.owned.terminals]) {
      ...
      try {
        if (record.id && terminals.resume(record.id)) opened.push(record);
        else {
        ...
        // suspended. Keep the record — the ones that did not fit are still
        // resumable once a slot frees, and dropping them would lose the session.
        failed.push({ ...record, error: err.message });
        continue;
      }
      state.owned.terminals = state.owned.terminals.filter((r) => r !== record);
    }
    return { opened, fresh, failed };
  }
  ...
  function status() {
    ...
    return {
      // bc-4zz landed, so a resumed terminal is the same conversation rather than a
      // new one on the same bead. Kept as a field rather than deleted because the
      // installed Android build and a cached service worker both read this page's
      // endpoint and one of them still draws the warning it used to gate.
      reopenIsFresh: false,
      at: state.at,
