...
export async function materialise(dir, sessions, root) {
  ...
  for (const s of sessions) {
    ...
    out.push({
      ...
      // Whether the hour fitted the window that model came with (bc-nc6o.8). One word on
      // the row rather than the whole `tokens` record: the numbers are in the `meta.json`
      // beside it, and what the row is for is deciding which of forty sessions to open. A
      // session that spent part of itself compacting is one where "it did this by hand"
      // may be a symptom of a window rather than of a missing command.
      pressure: line(parsed.tokens?.pressure, 10),
...
export function auditPrompt({ sessions = [], library = [], filed = [], misses = [], repo = '' } = {}) {
  const rows = sessions.map(
    (s) =>
      ...
      `${s.pressure === 'over' ? ' · ran out of context' : s.pressure === 'tight' ? ' · context was tight' : ''}` +
...
/**
 * How long the oldest unread archive has been sitting there, or `null` if none is.
 *
 * `sessions` is newest-first and `audited` is not required to be a prefix of it — a run
 * can be forced out of order, or the ledger's `AUDITED_MAX` can have aged an old commit
 * out of the remembered list while it is still in range of `sessionsIn`'s own limit — so
 * this walks every row rather than trusting the first unread one it meets to be the
 * oldest.
 */
export function oldestUnreadAt(sessions, audited) {
  const seen = new Set(audited);
  let oldest = null;
  for (const s of sessions) {
    if (seen.has(s.commit)) continue;
 ...
 * the transcripts materialised outside the repo. The spawn-and-parse itself is
 * lib/agentrun.js's `runHeadless` — this and lib/jiraingest.js's runner share it now
 * (bc-dgx7.8), which is what puts both runs on `refs/beadcause/agentlogs` rather than
 * neither.
 *
 ...
 */
export async function runAgent({ dir, prompt, readDir = null, timeoutMs = TIMEOUT_MS, cfg = null, key = null, meta = {} }) {
  return runHeadless({
    dir,
    prompt,
    systemText: PROTOCOL,
    addDirs: readDir ? [readDir] : [],
    timeoutMs,
    cfg,
    key,
    meta,
    tmpPrefix: 'beadcause-audit',
  });
}
...
export function options(cfg) {
  ...
  return {
    ...
    // bc-dgx7.7 — the backstop. A checkout whose unread archives never reach `every`
    // because nothing ends a session in it again would otherwise wait forever; past this
    // age the oldest unread archive is audited *whatever the count*. A day by default —
    // long enough that an ordinary busy repo never sees it, short enough that a quiet one
    // does not sit unread for a week.
    maxAgeMs: clamp(a.sessionAuditMaxAgeMinutes, 60, 7 * 24 * 60, 24 * 60) * 60 * 1000,
    // How often the backstop itself is worth asking about — not the fast poll clock,
    // because asking costs two `git` reads per checkout and a workspace can hold forty.
    sweepMs: clamp(a.sessionAuditSweepMinutes, 5, 24 * 60, 30) * 60 * 1000,
  };
}
...
export function createAuditor({
  ...
  async function audit(dir, workspace, { force = false } = {}) {
    ...
    try {
      ...
      // bc-dgx7.7 — the count threshold is what makes a busy repo wait for a batch, and
      // it is exactly what strands a quiet one: nothing else ever asks again once the
      // session that would have pushed `fresh` over `every` stops arriving. Past
      // `maxAgeMs`, the oldest unread archive runs anyway, whatever the count — this is
      // the one place that bypass lives, so every caller (a session ending, or the sweep
      // below) gets it for free rather than reimplementing the decision.
      const oldest = fresh < o.every ? oldestUnreadAt(sessions, ledger.audited) : null;
      const stale = oldest !== null && now() - oldest >= o.maxAgeMs;
      if (fresh < o.every && !force && !stale) {
        return { ...out, why: `${fresh} unread session(s) — the threshold is ${o.every}` };
      }
      ...
      let answer = '';
      try {
        // `bc-dgx7.8` — every audit run is now a run on `refs/beadcause/agentlogs`, keyed
        // per checkout rather than per bead: an audit is not about any one bead (`bead`
        // stays null, the way an advocate survey's does), and the checkout's own basename
        // disambiguates a workspace that spans more than one repo.
        answer = await run({
          dir,
          prompt,
          readDir: root,
          cfg,
          key: `${out.workspace}/audit-${path.basename(dir)}`,
          meta: {
            workspace: out.workspace,
            bead: null,
            agent: 'session-audit',
            endorsed: true,
            endorsementNote: 'an audit is beadcause reading its own history, not work opened on a bead',
          },
        });
      } catch (err) {
      ...
      if (!out.error) {
        ...
        if (beads.length && bd) {
          // A cited session's bead is the `from`: the provenance edge, and the reason a
          // reader of a candidate can get back to the runs that evidenced it. A
          ...
          const res = await file(
            ...
            {
              from,
              // bc-khoe.48. `from` is provenance and it used to be the *parent* as well
              // — lib/homing.js hangs a discovery under the root the discovering bead
              // descends from, which is the only honest answer for a seam that files
              // one-off discoveries. It is the wrong answer for this one. Every bead
              // this file files is the same kind of bead, and which epic evidenced it is
              // an accident of which sessions ended that hour: by 2026-08-21 thirty-seven
              // open candidates were spread over twelve unrelated epics, each counted
              // against a theme it had nothing to do with, and each sitting *ahead* of
              // that theme's own work in the ready queue, because every one of them is
              // P2 and the tiebreak is `created_at` — a burst filed at midnight can never
              // fall behind the beads it outranks; it only leaves the front by closing.
              //
              // So the home is named rather than inferred, and it is named the way
              // lib/homing.js names the unsorted backlog: the open root carrying the
              // label, not an id in config.json, because config is per-Mac and this
              // graph is shared. `SKILL_LABEL` is already that label — bc-dgx7's own
              // description says the whole programme is one query on it — and it is safe
              // to reuse even though every candidate carries it too, because
              // `rootLabelled` looks only at roots and a candidate is a P2 task.
              //
              // Nothing here refuses if that epic is not there: with no root carrying
              // the label the answer falls back to the root above `from`, which is
              // exactly what this seam did before.
              homeLabel: SKILL_LABEL,
  ...
  /** On its own clock, beside `noteArchive`'s trigger and never faster than `sweepMs`. */
  let sweptAt = 0;

  /**
   * The backstop's other half — bc-dgx7.7. `noteArchive` only ever fires from inside the
   * advocate's own archive loop, which is the one place that knows which checkout a
   * session just landed in; a checkout that finishes its sessions and then goes quiet
   * gets no further look from anything, because nothing else ends a session in it. This
   * is the poll cycle's own look: every checkout of every configured workspace, asked in
   * turn whether `audit()` thinks a run is worth it.
   *
   * Deliberately not a second decision. The staleness bypass lives inside `audit()`
   * itself (next to the `every` threshold it widens), so calling `audit(dir, ws)` here
   * unforced is enough — a checkout with nothing old enough gets back the same refusal it
   * always would, at the cost of the two `git` reads `pending()` already does. Nothing
   * here ever passes `force: true`: forcing is what a caller uses to mean "now, whatever
   * this run would normally cost," and a periodic sweep asking "is anything worth it
   * yet?" is exactly the opposite of that.
   *
   * **Never awaited past the dispatch**, for the reason `noteArchive` already gives: a
   * run that actually starts is a `claude -p` that can take up to `TIMEOUT_MS`, and this
   * is called from the poll cycle's slow half — awaiting it would hold up the advocate
   * tick and the merge queue behind it for however long that run takes. `running` is
   * checked synchronously at the top of `audit`, before its first `await`, so firing every
   * checkout's call in the same tick without awaiting still lets only one real run start:
   * whichever checkout's call reaches that check first sets the latch, and every other
   * call in this loop sees it set and returns its refusal immediately. `audit` never
   * rejects — every path through it resolves with an outcome, including its own failures
   * — so the `.catch` below is belt-and-suspenders rather than load-bearing.
   */
  function sweepStale() {
    const o = options(cfg);
    if (now() - sweptAt < o.sweepMs) return;
    sweptAt = now();
    for (const ws of cfg.workspaces || []) {
      for (const unit of repoUnits(cfg, ws.name)) {
        ...
        try {
          ...
          // The ordinary state for a workspace not checked out on this Mac — nothing to
          // audit here, and not a failure worth a log line every sweep.
          continue;
        }
        if (!dir) continue;
        audit(dir, ws).catch((err) => warn(`[audit] the audit failed — ${String(err?.message || err).split('\n')[0]}`));
      }
    }
  }
