...
export function classifySuite(source) {
  ...
  // The marker is the lifting, not the mention. A suite that merely reads public/app.js as
  // text is a static read and wants a lib preamble; widening the test to `public/app.js`
  // doubles the group with suites that share none of its shape, and `lift` — the one helper
  // this kind cannot do without — then loses its majority to them.
  if (/\blift\(\s*APP\s*,/.test(code)) return 'app';
