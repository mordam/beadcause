...
export const NOT_EGRESS = Object.freeze([
  ...
  },
  {
    ...
    kind: 'command',
    why:
      'Three callers pipe `git archive <sha>` straight into `tar -x -C <dest>` to materialise a tree on local disk: ' +
      '`b7e-head --tree` (lib/head.js, bc-dgx7.37), `--tree` in lib/premerge.js (bc-dgx7.52), and lib/prtree.js ' +
      "(bc-dgx7.38), which unpacks a pull request's tree for a reviewer to run. All three read only what `git archive` " +
      'hands them on stdin and write only under os.tmpdir() — lib/prtree.js additionally asserts the directory it ' +
      "writes into is contained there (see lib/evidence.js's entry for it). No socket, no network option is ever " +
      'passed, and nothing about extracting an archive onto local disk can reach a third party.',
  },
  {
    what: 'python3',
    kind: 'command',
    why:
      ...
      'into. lib/plate.js is the second call site and the narrower one: `python3 -c <script>` with the Pillow ' +
      'compositing script this repo holds as a string constant, its job handed over on stdin as JSON, and nothing in ' +
      'that script opens a socket — it reads the image files it is given and writes the one sheet it was asked for.',
  },
  {
    what: 'sips',
    kind: 'command',
    why:
      "macOS's own image tool, and lib/plate.js runs it for one reading rather than any writing: " +
      '`sips -g pixelWidth -g pixelHeight -g format <files>` prints the header of each source image so the sheet can ' +
      "report the source's native dimensions in a single process instead of one per file (`nativeDimensions`), plus " +
      '`sips --help` to ask whether the tool is there at all (`toolsPresent`). Every argument is a `-g` query flag or ' +
      'a local path this repo has already resolved off disk, so nothing beadcause constructs can name a URL for it ' +
      'to fetch, and reading an image header cannot reach a third party. It is found by bare name rather than by ' +
      "path — /usr/bin/sips would be the operating system structurally — so it is named here instead.",
  },
]);
...
export function egressProblems(root, register = REGISTER, exempt = NOT_EGRESS) {
  ...
  for (const [command, where] of [...commands].sort()) {
    ...
    // This arm never consulted `exemptCommands` until bc-dgx7.37 and bc-dgx7.52 found the
    // gap independently on the same day, even though the sentence below has always told a
    // reader to put an exemption in NOT_EGRESS — only the reverse check further down (an
    // exemption that matches nothing) ever read a command-kind entry. Nothing had claimed a
    // command through NOT_EGRESS before, so it never fired; `tar` is the first. Matched by
    // name rather than by which supplier's account it runs under — `claimsCommand` already
    // strips a directory prefix for this reason, and a NOT_EGRESS command exemption is that
    // same lookup with no supplier behind it, so it has to be the same match.
