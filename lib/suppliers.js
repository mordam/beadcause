...
export const NOT_EGRESS = Object.freeze([
  ...
  },
  {
    ...
    kind: 'command',
    why:
      'Two callers pipe `git archive <sha>` straight into `tar -x -C <dest>` to materialise a tree into a scratch ' +
      'directory: `b7e-head --tree` (lib/head.js, bc-dgx7.37) and `--tree` in lib/premerge.js (bc-dgx7.52). It only ' +
      'ever reads its own stdin and writes files under os.tmpdir() — the same local-extraction shape `git`\'s own ' +
      'object store writes are, never a network call.',
  },
]);
...
export function egressProblems(root, register = REGISTER, exempt = NOT_EGRESS) {
  ...
  for (const [command, where] of [...commands].sort()) {
    ...
    // bc-dgx7.37: this arm never consulted `exemptCommands` until now, even though the
    // sentence below has always told a reader to put an exemption in NOT_EGRESS — only
    // the reverse check further down (an exemption that matches nothing) ever read a
    // command-kind entry. Nothing had claimed a command through NOT_EGRESS before, so
    // the gap never fired; `tar` (the `git archive | tar -x` in lib/head.js and
    // lib/premerge.js, sending nothing anywhere) is the first. bc-dgx7.52 found the same
    // gap independently on the same day and fixed it the same way.
    if (exemptCommands.some((x) => x.what === command)) continue;
