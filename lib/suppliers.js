...
export const NOT_EGRESS = Object.freeze([
  ...
  },
  {
    ...
    kind: 'command',
    why:
      "lib/episode.js asks it three questions and never gives it anything to fetch: `ffprobe -version` to find out " +
      'whether the tool is installed at all (`toolsAvailable`), and two read-only queries — ' +
      '`-v error -show_entries format=duration -of csv=p=0 <file>` and ' +
      '`-v error -select_streams v:0 -show_entries stream=width,height -of csv=p=0:s=x <file>` — that print one ' +
      "media header line to stdout. It is the one exemption here that is genuinely network-capable: ffprobe speaks " +
      'http and rtmp, so the argument cannot be "the tool cannot reach out", it has to be about what it is handed. ' +
      "Every `<file>` is `path.join(dir, f)` where `f` came out of `fs.readdirSync(dir)` and `dir` is a directory " +
      'this process already resolved and stat-ed on local disk, so an argv value never reaches ffprobe as an input ' +
      'path and a URL can never be one — `path.join` on a local directory would only ever mangle one. stderr is ' +
      "silenced and stdin is ignored, so the only channel out of the process is the dimension or duration number " +
      'this repo then prints. See test/episode.mjs, which drives both readers against real files.',
  },
]);
