...
export const NOT_EGRESS = Object.freeze([
  ...
  },
  {
    ...
    kind: 'command',
    why:
      "lib/prtree.js pipes `git archive`'s own stdout into it to unpack a pull request's tree on disk. It reads " +
      "only what `git archive` hands it on stdin and writes only under the tree lib/prtree.js already resolved " +
      "and asserted contained (see lib/evidence.js's entry for the same file) — no network, no host, no third party.",
  },
]);
