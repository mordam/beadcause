...
export const NOT_EGRESS = Object.freeze([
  ...
  },
  {
    ...
    kind: 'command',
    why:
      'lib/head.js pipes `git archive <sha>` straight into `tar -x -C <dest>` to materialise a pull request\'s head ' +
      'into a scratch directory (`b7e-head --tree`, bc-dgx7.37). It only ever reads its own stdin and writes files ' +
      'under os.tmpdir() — the same local-extraction shape `git`\'s own object store writes are, never a network call.',
  },
]);
...
export function egressProblems(root, register = REGISTER, exempt = NOT_EGRESS) {
  ...
  for (const [command, where] of [...commands].sort()) {
    ...
    // the gap never fired; `tar` (lib/head.js's `git archive | tar -x`, sending nothing
    // anywhere) is the first.
