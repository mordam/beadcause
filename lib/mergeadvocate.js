...
export function gateVerdict({
  ...
  const no = (refused, extra = {}) => ({
    ...
    stale: false,
    baseline: [],
    // The checks this branch broke, when that is what refused it — see the `fresh` branch
    // below. Empty everywhere else, so a caller may read it unconditionally.
    broke: [],
    ...extra,
  });
  ...
  if (fresh.length) {
    ...
    const inherited = known && already.length
      ...
      : '';
    /**
     * The flake reading, offered only where it is still available — bc-xl7n.116.
     *
     * "If it is a flake, that is your call" was written unconditionally, one line below
     * `newlyFailing`, which is the code that has *just decided this is not one*: `fresh`
     * is the names the branch is red on that the base is green on. So on the ordinary
     * refusal the card handed Adam a judgement the queue had already made, and withheld
     * the only fact that could have settled it — bc-n21dy on 2026-08-21, where accepting
     * the flake reading would have landed a line that turns `main` red for everybody.
     *
     * `baseHasChecks === true` rather than `!== false`, and the strictness is the point:
     * a base nothing has ever run reports no failures either, so `already.length === 0`
     * on its own is "the base said nothing", not "the base is green". Where the caller
     * could not ask — a null baseline, a base with no CI — the flake reading is genuinely
     * still open and the sentence is left exactly as it was. Saying less than it knows is
     * the cost of never saying more.
     */
    const baseGreenOnThese = known && baseHasChecks === true;
    // `inherited` has already said the base is green on these when it had something else
    // to name, so the sentence is not written twice.
    const green = baseGreenOnThese && !inherited
      ? ` The base is green on ${fresh.length === 1 ? 'it' : 'them'}, so ${fresh.length === 1 ? 'this failure' : 'these failures'} ` +
        `came in with this branch.`
      : '';
    const verdict = baseGreenOnThese
      ? ` A merge queue will not merge over a check the branch broke.`
      : ` A merge queue will not merge over a check the branch broke — if it is a flake, that is your call.`;
    return no(
      `${fresh.length} check${fresh.length === 1 ? '' : 's'} failing${named}.${inherited}${green}${verdict}`,
      // The names the branch is red on and the base is not, handed back so the caller can
      // go and read what they actually said. It is the caller that has GitHub; this is
      // pure and always will be. Empty on every other refusal, which is what makes it the
      // test for "this is a refusal about a check" rather than a second flag beside it.
      { broke: fresh }
    );
  }
  ...
  return { merge: true, refused: '', awaitingApproval: false, conflicted: false, stale: false, baseline: merged, broke: [] };
}
...
/**
 * What the red check said, drawn — bc-xl7n.116.
 *
 * `baselineNote`'s reason exactly, and it is the same shape of mistake being guarded
 * against: this is written on the merge-bead's comment thread and on the card, and two
 * copies of it is how one of them stops saying it. Empty for the ordinary case, so a
 * caller can concatenate it unconditionally.
 *
 * An **indented** code block rather than a fenced one, and not for taste. This ends up in
 * a bead description beside a ```decision fence that a phone parses into buttons
 * (lib/decision.js), and a log line that happened to begin with three backticks would
 * close a fence it never opened — which is a card whose options stop rendering. Four
 * spaces cannot be closed by anything the log contains.
 *
 * No leading blank line, because `deliveryBody` trims what it is handed. Separation is
 * the caller's, which is the only thing either caller has an opinion about.
 */
export function failureNote(failure) {
  const lines = (failure || []).map((s) => String(s)).filter((s) => s.trim());
  if (!lines.length) return '';
  return `What the check said, from the end of its log:\n\n${lines.map((s) => `    ${s}`).join('\n')}`;
}
 ...
 *
 * ## And a reviewer's veto is such a decision — bc-uxrix, answered by Adam 2026-08-24
 *
 * The rule above was written about the *queue's* sentence and it is the whole selection
 * test, so a card that carries both — a reviewer who will not approve this pull request,
 * and the queue's own stale "waiting on a review" line sitting underneath it in the queue
 * block — passes it. That is an accident of two blocks in one field, and it is the one
 * case where waking this function up does harm: `gateVerdict` does not consult the review
 * gate at all, so a green, clean, vetoed pull request would be taken back and merged over
 * the reviewer's refusal, unattended, as a side effect of a bug fix. Measured on #655 on
 * 2026-08-24, which was exactly that.
 *
 * So `reviewEscalation` is the carve-out, and it is the same predicate `lib/mergeraise.js`
 * escalates on: a `refused` verdict, or a disagreement that has used up
 * `MAX_REVIEW_ROUNDS`. **The reclaim is for cards the queue gave up on, not cards a
 * reviewer gave up on.** Neither of those lapses — a reviewer does not change their mind
 * because a check went green — which is the rule above applied rather than an exception
 * to it.
 *
 * A `changes` verdict mid-loop is deliberately **not** carved out: that is a round in
 * progress, the queue's own hold is what is recorded, and nothing about it says this pull
...
export function cardedFor(beads, { assignee = MERGE_ASSIGNEE, label = MERGE_LABEL, cardLabel = 'human' } = {}) {
  ...
  for (const b of rows) {
    ...
    // A reviewer gave up on it, not the queue — see the header. The queue's stale sentence
    // is still sitting underneath, which is the only reason this row got this far.
    if (reviewEscalation(reviewState(b))) continue;
 ...
 * real ones.
 *
 * **That was the intent and not the behaviour until bc-uxrix, and the gap was in the
 * input.** This function was right; what reached it was `bd.listAgent`, which runs
 * `--exclude-label human`, so a card was filtered out one layer above and contributed no
 * cover — and every carded pull request came back here as a strand. Measured 2026-08-26:
 * 22 live cards, each reported once a tick. `sweepMergeQueue` reads them separately now
 * and hands both halves over; **a caller that passes only the queued half gets the old
 * answer**, which is why the sweep suppresses the report entirely when the card read fails
 * rather than running it on what it has.
 */
