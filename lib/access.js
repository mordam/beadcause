 ...
 * each agent kind a row that reads like a user and you have described six accounts that
 ...
 * seventh kind cannot come into existence without this file having heard of it. What
 ...
 *
 * No `CC6.1`, no `A.8.3`, no 42001 control id appears in this file, and that is a seam
 * rather than an omission. bc-4r10.1 builds one closed control corpus across the three
 * frameworks with crosswalk edges on the control, and an id written here first would be
 * a second vocabulary — exactly the "same control implemented three times" that the
 * corpus exists to prevent. The register is the *evidence*; the corpus cites it. The
 * prose in the README says which criteria it answers, because prose cannot be mistaken
 * for a vocabulary.
 *
...
export const AGENT_ACCESS = {
  ...
  'epic-advocate': {
    reaches: 'One owned P0 and the beads it files underneath it, in one approved repo.',
...
export const CREDENTIALS = [
  {
    ...
    where: `${CONFIG_DIR}/config.json — \`auth.token\``,
    ...
    revoke: 'Rotate: replace `auth.token`, restart, re-pair each caller from the QR.',
  },
  {
    id: 'session-key',
  ...
  {
    id: 'github-cli',
    what: 'The `gh` login every push, pull request and merge rides on.',
    where: "The `gh` CLI's own credential store, in the daemon's user account.",
    holder: 'The merge queue and every worker delivery.',
    scope: 'The repos that account can see on GitHub — wider than beadcause, and worth naming for that reason.',
    revoke: '`gh auth logout`, and revoke the token on GitHub.',
  },
  {
    ...
    holder: 'Every agent kind. There is one of these, not six.',
 ...
 *
 * Throws on a kind with no `AGENT_ACCESS` row. A register that answered "six of the
 * seven" without saying so is the failure this whole file is against.
 */
...
export function humanPrincipals(cfg = {}, state = {}, now = new Date()) {
  ...
  if (cfg.auth?.token) {
    out.push({
      ...
      revoke: 'Rotate `auth.token` and re-pair every caller.',
