 ...
 *   - **How long it waited on a sweep that was already running.** The layer coalesces:
 *     a second caller on a key whose producer is in flight is handed that producer's
 *     promise (lib/cache.js), and is charged nothing for the children it did not spawn
 *     — deliberately, because moving that charge would count one sweep twice. So a
 *     request that queued behind somebody else's board sweep spawned nothing, had an
 *     idle loop, and came out `no subprocess, all ours`: the one reading the figure
 *     above exists to leave meaning *this handler really did run for a minute*. Measured
 *     2026-08-21 on the two worst samples in the log — a `/api/prs` at 62.6s and a
 *     `/api/queues` at 64.3s, both finishing twelve seconds before the 73.7s sweep with
 *     102 children that they had joined, and both filed as though the handler itself had
 *     been busy for a minute. `joined` is the fourth number and it is *reporting only*:
 *     who pays for the sweep is unchanged (bc-1kwl.33).
 *
...
/**
 * And how much of it has to have gone on somebody else's sweep before `joined` names it.
 *
 * Half, for the same reason and with the same claim behind it: a route over budget that
 * spent *most* of its wall clock waiting on a producer it did not start is not a route
 * with a slow handler, and calling it one is what this figure exists to stop. Below half
 * the wait is real but it is not the explanation, and a list of suspects is a list that
 * stops being read.
 */
const JOINED_SHARE = 0.5;
...
const bucket = () => ({ n: 0, ms: 0, max: 0, sub: 0, wall: 0, join: 0, joins: 0, ours: 0, loop: 0, calls: 0, recent: [], statuses: {} });
...
export function begin(key) {
  const rec = {
    ...
    spans: [],
    // The same, for every wait on a producer this request did not start. Kept apart from
    // `spans` rather than added to it: it is not this request's fan-out. See `joining`.
    joinSpans: [],
    joins: 0,
    cache: null,
    ended: false,
...
/**
 * Charge a wait on a producer that was **already in flight** to the request in scope.
 *
 *     const waited = timing.joining();
 *     try { return await already.promise; } finally { waited(); }
 *
 * Returns the function that closes the interval rather than taking a start stamp the way
 * `spend` does, because the only caller is a `finally` that has one already — see the
 * cold path in lib/cache.js, where `running.has(key)` is what says a sweep is under way
 * before this request awaits it. Off the request path it charges nothing: a background
 * refresh joining another background refresh is the daemon waiting on itself, and there
 * is nobody to bill.
 *
 * **Its own kind, deliberately not folded into `children`.** The request spawned
 * nothing, so counting the wait there would claim a fan-out it never had — and the ratio
 * of summed child work to the union of the intervals *is* how wide the fan-out was, read
 * as `×` on the table. Nor is it `background`: that column is work no request is waiting
 * for, and this is exactly the opposite — a request waiting on work it did not start.
 *
 * An interval rather than a duration, for the same reason `spend` takes one: one request
 * can be queued behind several sweeps at once, and an overlap counted twice would push
 * `ours` below zero. See `union`.
 */
export function joining() {
  ...
  return () => {
    if (!rec) return;
    const from = Math.max(0, Number(startedAt - rec.t0) / 1e6);
    const to = Math.max(from, Number(process.hrtime.bigint() - rec.t0) / 1e6);
    if (!Number.isFinite(from) || !Number.isFinite(to)) return;
    rec.joinSpans.push([from, to]);
    rec.joins += 1;
  };
}
...
export function header(rec) {
  ...
  // Beside `children` rather than inside it, and absent when there was no such wait — an
  // empty field on every ordinary request would be a column of zeros nobody reads.
  if (rec.joinSpans?.length) parts.push(`joined;dur=${union(rec.joinSpans).toFixed(1)}`);
...
export function end(rec, status = 0) {
  ...
  // The wall time this request spent queued behind a producer it did not start. Overlaps
  // `wall` freely — a fan-out can pay for children on one key while waiting on somebody
  // else's sweep for another — and is never more than the request itself. See `joining`.
  const joined = Math.min(total, union(rec.joinSpans || []));
  // What is left for us once both kinds of waiting are taken off. The union of the two
  // sets rather than two subtractions, because where they overlap subtracting each in
  // turn takes the overlap off twice and `ours` goes negative — the exact failure the
  // note on `union` is about, one level up.
  const ours = Math.max(0, total - Math.min(total, union([...rec.spans, ...(rec.joinSpans || [])])));
  ...
  if (slowMs > 0 && total >= slowMs && !parked) {
    const split = Object.entries(rec.byKind)
      ...
      .join(' + ');
    // Where the wall clock went, in as many clauses as there were kinds of waiting —
    // both are possible at once, since a request reading thirty keys can pay for
    // children on one and queue behind somebody else's sweep on another. With neither,
    // the line is the one it always was: `all ours` now means it, which is the whole
    // point of the second clause existing (bc-1kwl.33).
    const waits = [];
    if (rec.calls) waits.push(`${Math.round(wall)}ms of it waiting on ${rec.calls} child process(es) (${split} of work)`);
    if (joined > 0) waits.push(`${Math.round(joined)}ms of it waiting on a sweep already running`);
    write(
      `[beadcause] slow ${rec.key} ${Math.round(total)}ms ${temp}` +
        (waits.length ? ` — ${waits.join(' and ')}, ours ${Math.round(ours)}ms` : ' — no subprocess, all ours') +
  ...
  return { key: rec.key, ms: total, sub: rec.sub, wallMs: wall, joinedMs: joined, oursMs: ours, loopMs: loop, temperature: temp, parked };
}
...
function stats(b) {
  ...
  return {
    ...
    // Summed per request rather than derived as `ms - wall`, because a request can have
    // been waiting on a sweep it did not start as well as on children of its own, and
    // those two intervals overlap. See `end`.
    oursMs: round(b.ours / b.n),
    // And how much of the average request went on a producer already in flight. A route
    // over budget whose `joined` is most of its total did not run for a minute; it queued
    // behind something that did, and `ours` no longer absorbs that.
    joinedMs: round(b.join / b.n),
    joinedShare: b.ms > 0 ? Math.round((b.join / b.ms) * 100) / 100 : 0,
...
export function snapshot({ budgetMs = BUDGET_MS } = {}) {
  ...
  for (const route of routes.values()) {
    ...
    // Same reasoning for the joined wait: a request queues behind another request's sweep
    // whatever temperature its own keys were going to be.
    const everyJoin = route.cold.join + route.stale.join + route.warm.join;
    rows.push({
      route: route.key,
      parked: route.parked,
      cold,
      ...
      joinedShare: everyMs > 0 ? Math.round((everyJoin / everyMs) * 100) / 100 : 0,
      calls: route.cold.calls + route.stale.calls + route.warm.calls,
      joins: route.cold.joins + route.stale.joins + route.warm.joins,
  ...
  return {
    ...
    // `joins === 0` is there for the same reason and arrived with the column beside it
    // (bc-1kwl.33): a request that queued behind somebody else's sweep also has an
    // ordinary explanation available, and it is a *quiet* loop that gives it away — the
    // two 62s samples on 2026-08-21 sat at a loop share of 0.15. Without this the list
    // would be right about them by luck rather than by rule.
    starved: rows
      .filter((r) => !r.parked && r.worstMs > budgetMs && r.calls === 0 && r.joins === 0 && r.loopShare >= STARVED_SHARE)
      .map((r) => r.route),
    // The routes that missed the budget mostly waiting on a producer another request had
    // already started. Not a caching problem either — the caching is what they were
    // waiting for, and it was working — and not a slow handler, which is what `all ours`
    // used to imply of them. Decided here rather than by whoever prints it, the same rule
    // as `overBudget` and `starved` (bc-fg37).
    joined: rows
      .filter((r) => !r.parked && r.worstMs > budgetMs && r.joins > 0 && r.joinedShare >= JOINED_SHARE)
      .map((r) => r.route),
