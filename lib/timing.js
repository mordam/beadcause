 ...
 *   - **Warm against cold.** The five hand-rolled route caches in lib/server.js mean
 *     the *same* route has two completely different costs, and averaging them together
 *     hides both: the average of a 30ms cache hit and a 3s sweep is a number that has
 *     never happened. So every route keeps two sets of figures, and the cold one is
 *     the one a budget is about.
 *
...
/**
 * Say outright whether this request was served warm or cold, overriding the derivation.
 *
 * For the shared cache layer: a stale-while-revalidate hit answers from memory *and*
 * spawns a refresh behind it, so "it spawned something" would file the fastest kind of
 * request there is under `cold` and make the cache look like it had done nothing.
 */
export function cache(state) {
  const rec = store.getStore();
  if (rec && (state === 'warm' || state === 'cold')) rec.cache = state;
}
...
export function end(rec, status = 0) {
  ...
  const route = routes.get(key) || { key, parked: key === OTHER ? false : parked, cold: bucket(), warm: bucket() };
...
/**
 * Every route with its two sets of figures, worst first, plus the ones over budget.
 *
 * Sorted by the cold p95 because that is the figure a page load is spent against —
 * ordering by average would put a route that is fast a hundred times and catastrophic
 * once below one that is mediocre throughout, and it is the catastrophic one somebody
 * is complaining about. `overBudget` names them outright rather than leaving it to be
 * read off the table, because "which routes miss the second" is the question this whole
...
export function snapshot({ budgetMs = BUDGET_MS } = {}) {
  ...
  for (const route of routes.values()) {
    ...
    rows.push({
      ...
      cold,
      warm,
      worstMs: Math.max(cold?.p95Ms || 0, warm?.p95Ms || 0),
    });
  }
  ...
  return {
    ...
    budgetMs,
    slowMs,
    requests: rows.reduce((n, r) => n + (r.cold?.n || 0) + (r.warm?.n || 0), 0),
    routes: rows,
