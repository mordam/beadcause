 ...
 * had no trigger for is the merge itself, and there are three doors into `main`:
 *
 ...
 *
 * Four doors and not three, because the bead was written from the epic's sentence
 * ("from a card, from `beadcause-deliver` or from github.com") and the board's Merge
 * button is a fourth way a thumb lands one. A door that does not sweep is a feature
 * that looks broken exactly when Adam happens to merge that way, which is the failure
 * this exists to prevent, so it gets the same call as the other three.
 *
 ...
 * merge's.
 */
export async function sweepMerged(bd, cfg, { sweep = sweepConflicts, now = Date.now() } = {}) {
  const taken = takeSweepRequests();
  if (!taken.length) return [];
  const out = [];
  for (const rec of taken) {
    if (cfg?.pr?.enabled === false) {
      out.push({ ...rec, status: 'off', why: 'pull requests are disabled in config' });
      continue;
    }
    const age = rec.at ? now - new Date(rec.at).getTime() : 0;
    if (Number.isFinite(age) && age > STALE_MS) {
      out.push({ ...rec, status: 'stale', why: `it was written ${Math.round(age / 3600000)}h ago` });
      continue;
    }
    const where = locate(cfg || {}, rec);
    if (!where.ws) {
      out.push({ ...rec, status: 'gone', why: where.why });
      continue;
    }
    let result;
    try {
    ...
    out.push({ ...rec, status: 'swept', result });
  }
  return out;
}
...
export function describeSweepOutcome(o) {
  ...
  if (o.status === 'stale' || o.status === 'gone') return `dropped the sweep of ${where} — ${o.why}`;
