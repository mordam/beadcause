 ...
 * - **the merge queue** — `afterMerge` in lib/server.js, which is where most merges now
 *   happen and where the PR board's button sends its own (bc-02ldo: that button used to
 *   be a fifth door of its own, merging on the spot with nothing in the tracker to say
 *   so, and it is an admission to this queue now);
 * - **a worker's own merge** — `bin/deliver.js`, which is how work lands in a space with
 ...
 * sentence ("from a card, from `beadcause-deliver` or from github.com"), and a thumb on
 * the PR board was a fourth way to land one. A door that does not sweep is not a gap
...
export function describeSweepOutcome(o) {
  ...
  // A fold is the exception, and for the same reason (bc-xl7n.36): what it says is that a
  // card did *not* appear, which is invisible from the inbox and would otherwise read in
  // the log as a sweep that quietly filed nothing.
  if (o.card?.folded) {
    const rec = o.card.record || {};
    const merges = Array.isArray(rec.merges) ? rec.merges.length : 1;
    return `swept ${where} — folded into the open card ${o.card.card}, now ${(rec.prs || []).length} pull request${
      (rec.prs || []).length === 1 ? '' : 's'
    } across ${merges} merge${merges === 1 ? '' : 's'}`;
  }
  return '';
}
