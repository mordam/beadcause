 ...
 *
 * **That paragraph is history now, and only half of it stopped being true.** bc-jvt0.4 gave
 * `heldByChildren` a fourth check: an *owned* childless epic is held out of the queue until
 * its advocate has recorded whether it is one job or several. So the fall-through it
 * describes no longer reaches an owned standing root — which is a second belt and not a
 * replacement for this file, for two reasons. It is `owner:`-conditional, and a container
 * with no owner has always been possible; and it *clears*, because the whole point of it is
 * that a decision releases the epic, so a standing root whose advocate concluded "one job"
 * would be dispatched again with a label saying so. Being furniture is permanent and being
 * undecided is not, which is exactly why the two are different marks.
 *
 ...
 *
 * ## Four decisions, none of them inherited
 *
 ...
 *
 * **The phone's own door is not gated either**, for lib/endorse.js's reason rather than
 * this one: `POST /api/session` opens a *discuss* session, which is you present and
 * choosing to talk about a bead. Talking about the shelf is how something gets filed
 * under it, and that window's brief is a conversation rather than a delivery — there is
 * no `bd close` at the end of it. This gate is about the windows nobody is watching.
 *
 * **A container still draws on the epic board, and no code was needed for that.**
 * `rootBoard` (lib/server.js) is built from `bd.graph` and not from `bd.ready`, so a bead
 ...
 *
 * **Nothing here is loud, which is a departure from lib/underroot.js.** That file's rule is
 ...
 * lib/underroot.js needs read. The refusal at the door is loud, because that one is a
 ...
 * lib/superseded.js, lib/stillopen.js, lib/shipbead.js and lib/underroot.js field for
 ...
 * being one means.
 *
 * The sentence says what the bead is *for* rather than only what it is not. Whoever reads
 * it — a log line under a refused launch, a 409 on a tap that reached this door — is
 * holding a bead they thought was work, and what they need next is where the work actually
 * goes, not a second sentence saying this is not it.
 */
