...
export const DEFAULT_TOOL_LIST = [
  ...
  'Bash(b7e-stillred:*)',
  // Read-only in the same construction sense as b7e-siblings/b7e-prior above: it spawns
  // `bd show`, then only `git`/`git status` reads (`git worktree list`, `git log`, `git
  // status --porcelain` in a worktree) plus `~/.claude/sessions/*.json` and a `ps` read —
  // no push, no merge, no write of any kind. bc-7qo.24 is the argument — three sessions on
  // three different beads each hand-built a partial version of "who else is live on this
  // bead" before their first edit, one of them silently resuming a live peer's worktree.
  // See bin/b7e-onbead and lib/onbead.js.
  'Bash(b7e-onbead:*)',
