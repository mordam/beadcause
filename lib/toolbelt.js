...
export const DEFAULT_TOOL_LIST = [
  ...
  'Bash(b7e-stillred:*)',
  // Read-only in the same construction sense as b7e-def/b7e-owes/b7e-affected above: it
  // only ever reads source under bin/, lib/, public/, scripts/, test/ (and, for `--imports`,
  // reuses lib/affected.js's own import graph) and prints an answer — no `bd`, no `git`,
  // no subprocess. bc-36xx.24 is the argument — seven sessions each hand-built "who calls
  // this" differently, and got it wrong at least twice in ways that looked like success: a
  // zsh glob silently expanding before grep saw it, and a real call site written with
  // optional chaining (`beadcause?.views?.mark?.(...)`) that a literal-text search never
  // matched. See bin/b7e-callers and lib/callers.js.
  'Bash(b7e-callers:*)',
