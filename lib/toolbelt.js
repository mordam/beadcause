...
export const DEFAULT_TOOL_LIST = [
  ...
  // See bin/b7e-owes.js.
  'Bash(b7e-owes:*)',
  // Read-only in the same sense as b7e-def/b7e-owes above: every path through it is a
  // `git fetch`/`diff`/`status`/`merge-base` read, and it never pushes, commits or talks
  // to `gh`. bc-khoe.27.10 is the argument — five sessions each asked "what does my
  // branch change against main, and will bin/deliver.js refuse it" with a different git
  // incantation, two of them only finding out it would refuse *after* writing the whole
  // delivery summary. See bin/b7e-shipcheck and lib/shipcheck.js.
  'Bash(b7e-shipcheck:*)',
