...
export const DEFAULT_TOOL_LIST = [
  ...
  // bin/b7e-call.
  // b7e-landed (bc-khoe.47) is deliberately NOT on this list, for the b7e-worktree/
  // b7e-eyeball reason rather than a write: it does not touch the tracked tree, but its
  // whole occasion is a worker session deciding, at the start of a window, whether the
  // bead in front of it already landed — a worktree branch to check, a `--base` to
  // compare against, a bead id already in hand. `dispatch` has none of that: no branch
  // of its own, `ownsRepo: false`, and one phone comment to answer with one `bd
  // comment`. It also asks GitHub (`gh pr list`), which lib/grants.js does not classify
  // as free the way a filesystem read is; a worker session already has the unrestricted
  // CLI default and loses nothing by this staying off the narrower list. See
  // lib/landedcheck.js and bin/b7e-landed.
];
