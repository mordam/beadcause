...
export const DEFAULT_TOOL_LIST = [
  ...
  'Bash(b7e-ws:*)',
  // Read-only in the same sense as b7e-readme/b7e-ws above: it reads a workspace's own
  // relay config plus the charter and role files that config names, and writes nothing —
  // not even a claim. bc-dgx7.77 is the argument — five sessions each sliced
  // `docs/STUDIO_CHARTER.md` (726 lines) and a role file by hand at every role change,
  // one of them reading a rule off trunk that a later ruling had already superseded
  // because its hand-cut slice happened to stop just short of the correction. See
  // bin/b7e-role and lib/rolecontract.js.
  'Bash(b7e-role:*)',
