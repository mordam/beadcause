...
export const DEFAULT_TOOL_LIST = [
  ...
  // b7e-verified (bc-36xx.27) is deliberately NOT on this list either, for the exact
  // b7e-blame/b7e-watch reason: any red suite it is told about (from a log, or from its
  // own auto-discovered gaterun.js record) it hands straight to `lib/blame.js`'s
  // `runBlame`, which re-runs that suite here and, if still red, again against a real
  // `origin/main` worktree — `Bash(npm test:*)`'s own shape, a write held by
  // merge-advocate alone. Its whole occasion is a worker or a human turning a finished
  // gate run into a `--tests` sentence for a delivery; `dispatch`, the one agent this
  // list governs, has no gate run of its own, no branch, and nothing to deliver past its
  // own single `bd comment`. Checked against lib/grants.js first, per
  // [[b7e-command-write-shaped-check-grants-before-default-tool-list]] — the answer for
  // this shape is "not on this list at all," matching every sibling above, not "add it
  // and classify it read." See lib/verified.js and bin/b7e-verified.
];
