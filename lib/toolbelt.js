...
export const DEFAULT_TOOL_LIST = [
  ...
  'Bash(b7e-known:*)',
  // Read-only: everything it touches in the sophab checkout is an `import`
  // (`sys.path.insert` then `import fea_report`/`webapp.engine`/`tools.planset_sweep`/
  // `costing`) and a `python3` subprocess, never a write to that checkout or this one.
  // bc-dgx7.15 — five sophab sessions each hand-built "what does the model say for this
  // size" from scratch. See bin/b7e-model and lib/b7e_model.js.
  'Bash(b7e-model:*)',
