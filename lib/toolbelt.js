...
export const DEFAULT_TOOL_LIST = [
  ...
  'Bash(b7e-stillred:*)',
  // Read-only in the same construction sense as b7e-prior just above, and asked for the
  // same reason: every path through it is `git`/`gh` reads (`git grep`, `git cat-file`,
  // `git diff`, `gh pr list`/`gh pr view`) against refs already fetched, and nothing it
  // does ever touches a working tree or a branch of its own. bc-68ou.15 is the argument —
  // four sessions each read a symbol, a file or a hunk out of an unmerged branch or pull
  // request by hand, one of them losing a call to a shell gluing a ref and a path
  // together wrong. See bin/b7e-unlanded and lib/unlanded.js.
  'Bash(b7e-unlanded:*)',
