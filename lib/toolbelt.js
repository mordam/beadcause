...
export const DEFAULT_TOOL_LIST = [
  ...
  // b7e-suite (bc-khoe.30.17) is deliberately NOT on this list either, for the same
  // reason as b7e-gate above rather than a read-only one: it runs every suite it is
  // given, concurrently, through the same lib/gate.js pool b7e-gate itself uses — which
  // is `Bash(npm test:*)`'s own shape, already a write held by merge-advocate alone on
  // "nothing about run the tests is a read". Its whole occasion is a worker session
  // wanting a handful of suites after an edit, without the worktree guard refusing the
  // shell loop that used to be the only way to ask for more than one by name;
  // `dispatch`, the one agent this list governs, has no branch and no suite of its own
  // to re-run. See lib/suite.js and bin/b7e-suite.
