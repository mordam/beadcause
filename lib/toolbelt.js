...
export const DEFAULT_TOOL_LIST = [
  ...
  'Bash(b7e-answered:*)',
  // Read-only in the same construction sense as b7e-already/b7e-answered just above
  // (bc-dgx7.85): it parses one suite with acorn — comments included, the same reason
  // b7e-already parses rather than greps — and prints what it finds, never spawning a
  // process or touching `bd`. Its whole occasion is a session about to add a check to a
  // suite that already exists, which five sessions each paid for by reading several
  // hundred lines of one by hand: what the suite imports from the module under test,
  // what its fixture/loader helpers are called, and which named check to anchor after.
  // See bin/b7e-suitemap and lib/suitemap.js.
  'Bash(b7e-suitemap:*)',
