...
export const DEFAULT_TOOL_LIST = [
  ...
  'Bash(b7e-show:*)',
  // b7e-register is deliberately NOT on this list. It writes to whatever checkout it is run in:
  // package.json, package-lock.json, lib/toolbelt.js and lib/grants.js, four tracked files, with
  // no branch of its own and no path to commit or review the result — the same argument the
  // apply-command and the sw-cache-bump command above are withheld for. Its whole occasion is a
  // worker session that has just written a new command and is about to deliver it; the one agent
  // this list governs answers a single phone comment with a single bd comment and has no bin/
  // file of its own to register. See bin/b7e-register.
];
