...
export const BASE_TOOL_LIST = [
  ...
  // never shells out to a system `grep` at all — it walks `lib/`, `bin/`, `test/`,
  // `scripts/`, `public/`, `android/` and README.md with `fs` and matches with a JS
  // `RegExp`, and writes nothing anywhere. bc-4r10.21 is the argument — six sessions
  // (bc-4r10.9, bc-eqn1.2, bc-4r10.1 twice, bc-ka5y.15.5, bc-7wwbb, bc-4r10.4) each
  // hand-wrote `grep -rn <pat> --include=*.js ...` and each hit the same zsh footgun, an
  // unquoted glob in a *flag value* killing the whole line before grep ever ran. See
  // bin/b7e-grep and lib/repogrep.js.
  'Bash(b7e-grep:*)',
