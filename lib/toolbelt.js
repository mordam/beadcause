...
export const DEFAULT_TOOL_LIST = [
  ...
  // lib/strays.js.
  // b7e-commit (bc-xl7n.119) is deliberately NOT on this list, for the b7e-say reason
  // rather than a new one: it stages the whole tree (`git add -A`) and writes a commit,
  // which is a write to the checkout no reading of "the read-only surface every reply
  // agent gets" can cover. Its whole occasion is the last step of a worker's own run —
