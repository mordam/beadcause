...
export const DEFAULT_TOOL_LIST = [
  ...
  'Bash(b7e-stillred:*)',
  // Read-only in the same construction sense as b7e-def/b7e-owes/b7e-affected above,
  // despite spawning `node --check` per file: that flag only parses, never runs, the
  // module — it never binds a port, never touches the tracker, and the one write it
  // could plausibly own (fixing what it finds) is left to whoever reads its report.
  // bc-khoe.30.18 is the session-audit argument — three sessions (bc-4r10.3, bc-4r10.4,
  // bc-4r10.13) each hand-checked a large, prose-heavy module they had just written for
  // exactly this shape of damage, one of them losing ten minutes to a SyntaxError stack
  // a one-second `node --check` would have named immediately. See bin/b7e-lint and
  // lib/lint.js.
  'Bash(b7e-lint:*)',
