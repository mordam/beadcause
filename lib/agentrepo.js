 ...
 * agent and per arm, how many runs touched the repo at all, how many read, and how many
 * read *before* they wrote. A run is counted whether or not the agent ever touches the
 ...
 *
 * The three are not one population and the report must never add them up. A worker edits
 * files with somebody at the keyboard; a P0 advocate does one turn of thinking about a
 * queue and exits; a repo advocate argues about work nobody has agreed to. They differ in
 * run length, in how often they run and in whether anyone is watching — so a pooled
 * `readFirst` mixes three behaviours and reports whichever of them ran most as if it were
 * all of them. Hence `summary` buckets by agent first and arm second, and `report` says
 * *out loud* when an arm has no data rather than printing a zero, because a starved arm
 * and a well-behaved one both read as `readFirst: 0`.
 *
 ...
 * under the `index` arm.
 *
 * **Bucketed by agent and never pooled across them**, which this used to claim while
 * doing the opposite — it keyed on the arm alone, and the claim held only by accident,
 * because for four days exactly one agent kind had ever run. See the header: three kinds
 * own a repo now and they behave nothing alike, so a pooled `readFirst` reports whichever
 * of them runs most often under all three names.
 *
 * Returns `{ [agent]: { blind, index } }` — an object keyed by agent id, empty when
 * nothing has ever run. An agent that has never run has no key at all, which is why
 * `report` takes the set that *should* be there separately: a missing row is the exact
 * failure this bead was filed for, and it cannot be seen in data that is not there.
 */
export function summary(opts = {}) {
  const out = {};
  for (const r of runs(opts)) {
    const arm = ARMS.includes(r.arm) ? r.arm : 'blind';
    // An id off a log line, so it is validated the way a directory name is rather than
    // trusted: a torn or hand-edited row must not be able to invent an agent.
    const agent = NAME.test(String(r.agent || '')) ? String(r.agent) : 'unknown';
    if (!out[agent]) out[agent] = blankArms();
    const a = out[agent][arm];
    a.runs += 1;
