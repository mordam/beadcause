 ...
 *
 * ## Who is in it, and why the numbers are never pooled across them
 *
 * Three agent kinds own a repo: the repo advocate, the P0 advocate and the worker. That
 * is bc-goo.12 and it is the correction to a version of this that ran on the advocate
 * alone — the grant sat on the one agent whose runs are the rarest event in the daemon
 * (a survey, behind a twelve-hour cooldown and an unanswered-proposal gate), so in four
 * days the experiment recorded **one** run, in one arm, and the comparison it exists for
 * could not be computed at all. An experiment hosted on an event that does not happen is
 * indistinguishable from one that is running.
 *
 * The three are not one population and nothing that reports on them may add them up. A
 * worker edits files with somebody at the keyboard; a P0 advocate does one turn of
 * thinking about a queue and exits; a repo advocate argues about work nobody has agreed
 * to. They differ in run length, in how often they run and in whether anyone is watching
 * — so a pooled `readFirst` mixes three behaviours and reports whichever of them ran most
 * as if it were all of them. `summary` therefore takes an `agent`, `summaryByAgent` is
 * every agent's arms side by side, and `report` says *out loud* when an arm has no data
 * rather than printing a zero, because a starved arm and a well-behaved one both read as
 * `readFirst: 0`.
 *
 * Workspaces *are* pooled inside an agent, and that is deliberate rather than an
 * oversight: the population under test is the agent kind, a workspace is a sample of it,
 * and `armFor` alternates per workspace *and* agent — so every workspace contributes to
 * both arms in step and pooling them cannot tilt the comparison.
 *
...
/** One arm's counters, before anything has run. */
const blank = () => ({ runs: 0, touched: 0, read: 0, wrote: 0, readFirst: 0, commands: 0 });

/** Both arms for one agent, before anything has run. */
const blankArms = () => ({ blind: blank(), index: blank() });

/**
 * The comparison the whole thing exists for: per agent, per arm, what it actually did.
 *
 ...
 * promises never to.
 *
 * Called bare it is still the pooled number, which is right for "what has this log got
 * in it at all" and wrong for anything else. `summaryByAgent` below is the shape to
 * reach for when the question is about the experiment rather than about one agent, and
 * `report` is what turns it into something a person can be wrong-footed by less easily.
 */
...
/**
 * Every agent's arms at once, keyed by agent — the whole-experiment view, built out of
 * the narrowing `summary` already does rather than out of a second traversal with its
 * own rules.
 *
 * Two sources for the list of agents and both are needed. The log's own ids are what an
 * agent that ran under a name nothing ships any more shows up as; `expect` (from
 * lib/foundation.js, at the call site — this module must not import it) is what makes an
 * agent that has **never** run appear at all. A run that did not happen leaves no row, so
 * no amount of reading the log can produce that name, and that absence was exactly the
 * failure bc-goo.12 was filed for.
 */
export function summaryByAgent({ expect = [], ...opts } = {}) {
  const seen = runs(opts)
    .map((r) => String(r.agent || ''))
    .filter((a) => NAME.test(a));
  const out = {};
  for (const agent of new Set([...expect, ...seen])) out[agent] = summary({ ...opts, agent });
  return out;
}

/**
 * The same numbers, written out for a person — and the part of this bead that is not
 * arithmetic.
 *
 * bc-goo.12 was filed because *a starved experiment and a running one look identical*.
 * `summary()` on a log holding one run of one agent returns a perfectly well-formed
 * object full of zeroes, and every one of those zeroes reads as a measurement: `readFirst
 * 0` under `index` is the headline result the prediction expects, and it is also what an
 * arm that has never run says. Four days of that went unnoticed. So the rule here is that
 * **an arm with no runs never prints a number**. It prints the sentence.
 *
 * `expect` is the set of agents that own a repo, passed in rather than looked up — this
 * module must not import lib/foundation.js (the fence in `grantsFor` takes a foundation
 * as an argument for the same reason). It is what makes an agent that has *never* run
 * visible at all: no runs means no rows in the log, and a row that is not there cannot
 * be reported by anything reading the log alone. That was the actual failure — the
 * beadcause advocate had never reached the code once, and nothing said so.
 *
 * **And there is a second way for both arms to be full and the comparison to be empty**,
 * which is what the first read of this report at bc-goo.6 actually found: no agent has
 * ever written to its repo, so the `index` brief has only ever had "It is empty" to
 * list. The arms then differ by one sentence and by no information at all, and a reader
 * who has both arms' numbers in front of them will read `readFirst 0` under `index` as
 * the prediction confirmed. It is not — it is a treatment that was never applied. So an
 * agent that has never written says that out loud too, in the same voice and for exactly
 * the same reason as `NO DATA` above.
 *
 * The Memory tab (lib/agentview.js) answers the same question one agent at a time, from
 * the same numbers. This is the other half of it: the whole experiment on one screen,
 * for the reader who has not thought to go and look at an agent that might not be there.
 */
export function report(byAgent = {}, { expect = [] } = {}) {
  const names = [...new Set([...expect, ...Object.keys(byAgent)])].sort();
  const lines = ['Tier 3 — what each agent did with a repo of its own.', ''];
  if (!names.length) {
    lines.push('No agent owns a repo on this install, so there is no experiment to report.');
    return lines.join('\n') + '\n';
  }
  const starved = [];
  const undifferentiated = [];
  for (const agent of names) {
    // Defaulted rather than trusted: this is the one function whose whole job is to be
    // read when something has gone wrong, so a half-shaped bucket must not be able to
    // throw where it should print "nothing has run".
    const arms = { ...blankArms(), ...(byAgent[agent] || {}) };
    lines.push(agent);
    if (!arms.blind?.runs && !arms.index?.runs) {
      lines.push('  NOTHING HAS EVER RUN — this agent owns a repo and the log holds no run of it.');
      lines.push('');
      starved.push(agent);
      continue;
    }
    for (const arm of ARMS) {
      const a = { ...blank(), ...(arms[arm] || {}) };
      lines.push(
        a.runs
          ? `  ${arm.padEnd(6)} runs ${a.runs}  touched ${a.touched}  read ${a.read}  wrote ${a.wrote}  ` +
            `readFirst ${a.readFirst}  commands ${a.commands}`
          : `  ${arm.padEnd(6)} NO DATA — nothing has run in this arm, so nothing here is a zero.`
      );
    }
    if (!arms.blind?.runs || !arms.index?.runs) starved.push(agent);
    // The same failure as the one above, one level up, and the one this report could not
    // see until both arms had runs: `index` is *told what is in the repo*, so an agent
    // that has never written to it has been told "it is empty" on every run of that arm.
    // Two arms carrying the same information are one arm counted twice, and `readFirst 0`
    // under `index` then says what a treatment that was never applied says rather than
    // anything about the prediction. Reported from `wrote` across both arms because that
    // is what decides whether any listing has ever existed to be shown — an agent that
    // wrote in `blind` alone has still put content in the repo the next `index` run sees.
    else if (!(arms.blind?.wrote || 0) && !(arms.index?.wrote || 0)) undifferentiated.push(agent);
    lines.push('');
  }
  if (undifferentiated.length) {
    lines.push(
      `The two arms have not yet differed for: ${undifferentiated.join(', ')} — nothing has ever ` +
        'been written, so every `index` brief said "It is empty" and carried no listing. ' +
        'A `readFirst` of 0 there is a treatment that was never applied, not a null result.'
    );
  }
  lines.push(
    starved.length
      ? `The comparison cannot be computed for: ${starved.join(', ')} — an arm with no runs is ` +
        'not a result, and the two arms are the finding rather than either one of them.'
      : undifferentiated.length
        ? 'Every agent has run in both arms, so the runs are there — what is not there is a ' +
          'difference between the arms to measure.'
        : 'Both arms carry runs for every agent, so the comparison is computable for all of them.'
  );
  return lines.join('\n') + '\n';
}
...
/**
 * What `advocates.agentRepo` is when nobody has set it, in one place.
 *
 * It lived in lib/advocate.js's `DEFAULTS` while the advocate was the only agent that
 * could reach it. Three agents read it now and two of them are launched from
 * lib/session.js, which cannot import lib/advocate.js — that module imports session.js —
 * so a second copy of the string is a second thing to forget. The *documentation* of the
 * setting stays where the rest of the advocate options are documented; this is only the
 * value.
 */
export const ARM_SETTING = 'alternate';

/** The setting on this install. Config still owns it; this is just where it is read. */
export const armSetting = (cfg) => cfg?.advocates?.agentRepo ?? ARM_SETTING;

/**
 * Everything one run of one agent needs: the directory, the grant, the arm, the line in
 * the denominator and the brief.
 *
 * One function because there are three callers now — lib/advocate.js's survey and both
 * doors in lib/session.js — and the order inside it is not obvious enough to be worth
 * writing out three times. In particular `record` happens *before* the agent starts and
 * whether or not it ever touches the repo, because the denominator is the half the
 * prediction turns on; a caller that recorded on first use instead would silently drop
 * every run where the agent ignored the affordance, which is the result being measured.
 *
 * Returns `null` for an agent that does not own a repo and for `agentRepo: off`, and the
 * caller is expected to carry on: this is an experiment riding along with the real job,
 * so nothing here may be a reason a survey or a work session does not happen. Failures
 * throw and every caller wraps it — provisioning touches the disk, and a diary that
 * would not initialise is not worth a window.
 */
export async function startRun(foundation, workspace, { setting = ARM_SETTING, owner = 'the owner' } = {}) {
  if (!foundation?.ownsRepo) return null;
  const arm = armFor(workspace, foundation.id, setting);
  if (!arm) return null;
  await ensureAgentRepo(workspace, foundation.id);
  const grant = grantsFor(foundation, workspace, { arm });
  if (!grant) return null;
  record({ workspace, agent: foundation.id, run: grant.run, arm, verb: 'session', kind: 'meta' });
  const index = arm === 'index' ? await indexOf(workspace, foundation.id) : null;
  return { grant, arm, brief: repoBrief(index, { arm, owner }) };
}
