...
export function createEpicFiler({ bd = null } = {}) {
  ...
  async function fileOne(cfg, workspace, ticket, rows) {
    ...
    if (found && found.how === 'ref') {
      remember(name, ref, found.row.id);
      return null;
    }
  ...
  async function fileFor(cfg, workspace, tickets, now) {
    ...
    const filed = [];
    const failed = [];
    const rows0 = tickets.filter((t) => t?.key);
    ...
    const missing = rows0.filter((t) => !seen(name).has(refFor(t.key)));
    ...
    try {
      ...
      const rows = due.map((t) => ({ workspace: name, key: t.key, error: why }));
      return { workspace: name, filed, failed: rows, read: false, held: missing.length - due.length };
    }
    ...
    for (const [ref, row] of refIndex(rows)) remember(name, ref, row.id);
