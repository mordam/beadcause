 ...
 *
 * **What is not fine is the same message with nobody on the other end of it.** A git
 * that dies without dropping its lock leaves a file that fails every commit from then
 * on, identically and forever, and the drop-and-log above turns that into six days of
 * silence (bc-xl7n.79). So `commit()` tells the two apart before it gives up:
 * `clearAbandonedLocks` removes a lock that is old *and* open in no process, says so,
 * and tries once more. A live race still loses its commit, which is the design.
 */
 ...
 *
 * Every git command this module runs is a subprocess a few milliseconds long, capped
 * at the 30s timeout in lib/gitref.js, so nothing here can legitimately hold a lock
 * for ten minutes. What can is a *person*: `git commit` with no `-m`, run by hand in
 * that directory, holds `index.lock` open for as long as the editor is. That is why
 * age is not the only test below — it is the cheap half, and `heldBy` is the half
 ...
 * arrives here as the first line of stderr through `git()`. The `File exists` form is
 * what six days of `[beadcause] common repo snapshot failed: git commit: fatal: cannot
 * lock ref 'HEAD'` actually said.
 */
const LOCK_FAILURE = /cannot lock ref|Unable to create '[^']*\.lock'|\.lock': File exists|index\.lock/i;

/**
 * Every `*.lock` git could be holding here, as paths relative to the git directory.
 *
 * The top of the git directory (`index.lock`, `HEAD.lock`, `config.lock`,
 * `packed-refs.lock`) and everything under `refs/`, which is where a ref update takes
 * its lock. Deliberately *not* `objects/` — the locks in there belong to `gc` and to
 * packing, which have their own recovery and are nothing to do with a snapshot that
 * cannot commit.
 */
 ...
 *
 * bc-xl7n.109. `lsof` was asked for by name, so `execFile` resolved it from `PATH`, and
 * on macOS the binary is at **`/usr/sbin/lsof`** — there is no `/usr/bin/lsof`. The
 * daemon's `PATH` comes from its launchd plist and is `…/node/bin:/opt/homebrew/bin:
 * /usr/local/bin:/usr/bin:/bin`, with no `/usr/sbin` in it. So in the daemon every call
 * threw `ENOENT`, `heldBy` returned "I could not tell", and `clearAbandonedLocks` left
 * every lock alone — for the whole of the six days bc-xl7n.79 was written to end, and
 * again for 37 hours afterwards, with `common repo: removed` never once printed.
 *
 * It was invisible because an *interactive* shell does have `/usr/sbin`: anybody testing
 * the fix by hand watched it work. The daemon was the only place it ran and the only
 * place it failed. So the search is by absolute path first and `PATH` only as a
 * fallback, which is the one form that does not depend on who started the process.
 *
 * `BEADCAUSE_LSOF` names one binary and skips the search, for a machine that keeps it
 * somewhere else — and it is what test/commonrepo.mjs points at nothing to reach the
 ...
 * is visible to `lsof` and a dead one is not. Nothing else here can tell them apart: the
 * file records no pid, and `HEAD.lock` is zero bytes for most of its life anyway.
 *
 * Fails *closed*. `lsof` is not on every machine and is slow enough to be worth a
 * timeout, and either way what comes back is "I could not tell" — which has to mean
 * "leave it alone", because the alternative is deleting a lock out from under a live
 * `git commit` on the strength of a missing binary.
 *
 * **"No lsof" and "lsof says nobody" are two answers and used to be one silence.** They
 * still return the same fail-closed `null`, because they have to — but the missing case
 * now says so out loud, once. That single line is what six days of bc-xl7n.79 and then
 * 37 hours of bc-xl7n.109 were missing: the log said the snapshot failed, thousands of
 * times, and never that the thing meant to rescue it could not run at all.
 *
 * Asynchronous, and not for tidiness: `lsof` walks every process on the machine and is
 * the one call here that can take a noticeable moment, so the synchronous form would
 * stop the daemon's event loop for as long as it took — up to the timeout, on the day
 * something has it wedged.
 */
async function heldBy(file) {
  ...
  if (!saidNoLsof) {
    saidNoLsof = true;
    console.error(
      `[beadcause] common repo: cannot tell whether a git lock still has an owner — no lsof at ` +
        `${candidates.join(', ')}. Abandoned locks will be left in place and every snapshot will ` +
        `keep failing until one is removed by hand.`
    );
  }
  return null;
}
...
/**
 * Remove the locks behind a failed commit that no longer have an owner, and name them.
 *
 * The debounced snapshot is *allowed* to lose a commit — the module note at the top of
 * this file says so, and means it: two processes writing here collide on `index.lock`,
 * one loses, the state file is safe on disk either way and the next write picks up both
 * changes. That is a race, it resolves in milliseconds, and nothing should touch it.
 *
 * An *abandoned* lock is the same message and the opposite situation. bc-xl7n.79 is what
 * it costs: a zero-byte `HEAD.lock` from a git that died on 11 August, no holder, and
 * every snapshot for the next six days logging one line as it dropped the history it was
 * asked to keep. Nothing was broken and nothing was recoverable without a person, which
 * is the worst shape a failure can have.
 *
 * So the two are told apart rather than treated alike, and both tests have to pass: older
 * than `STALE_LOCK_MS`, and open in no process. A young lock is the race and is left; a
 * lock `lsof` cannot speak for is left too. Only the failure path asks — a healthy
 * snapshot never runs any of this.
 */
async function clearAbandonedLocks(d, err) {
  if (!LOCK_FAILURE.test(String(err?.message || ''))) return [];
  const gitDir = (await ok(git(d, ['rev-parse', '--absolute-git-dir'])))?.trim() || path.join(d, '.git');
 ...
 *
 * One retry, and only ever after clearing a lock nothing was holding — see
 * `clearAbandonedLocks`. Every other failure here is a refusal about a secret, and a
 * refusal retried is a refusal twice.
 */
export async function commit(reason = 'state') {
  try {
    return await commitOnce(reason);
  } catch (err) {
    const cleared = await clearAbandonedLocks(dir(), err);
    if (!cleared.length) throw err;
    console.error(
      `[beadcause] common repo: removed ${cleared.join(', ')} — abandoned, no process holding it, ` +
        `untouched for over ${Math.round(STALE_LOCK_MS / 60000)} minutes; taking the snapshot again`
    );
    return await commitOnce(reason);
  }
}

async function commitOnce(reason) {
