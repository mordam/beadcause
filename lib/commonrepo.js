...
import { git, gitInput, ok, clearAbandonedLocks, STALE_LOCK_MS } from './gitref.js';
...
/**
 * `clearAbandonedLocks` and `STALE_LOCK_MS` used to live here — bc-xl7n.79's own fix,
 * one file, one lock (`HEAD`). bc-xl7n.93 found the identical failure one door along,
 * on `refs/beadcause/<topic>` inside the same repo but written through
 * `commitToRef` in lib/gitref.js rather than through `commit()` below, and the module
 * note at the top of gitref.js says why the recovery moved there instead of being
 * copied: two writers can each abandon a lock, and it should have exactly one
 * implementation to be wrong in. `commit()` below calls the shared one exactly as it
 * always called its own.
 */
