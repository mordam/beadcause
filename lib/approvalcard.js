 ...
 *
 * ## The two string literals this shares with lib/approval.js
 *
 ...
 * depend on a chip. The two labels are spelled twice on purpose, the way a prefix is; if
 * that ever stops being cheap the fix is a third file holding the vocabulary, not an
 * import in either direction.
 */

/** The packet is awaiting Adam's ruling. Always paired with `human`; see `problem`. */
const NEEDS_APPROVAL = 'needs-approval';
/** What actually puts a bead on the phone. Its absence beside the above is a bug. */
const HUMAN = 'human';
/** beadcause's own — you commented and did not answer. Never read on its own here. */
const REPLIED = 'human-replied';
/** An agent is working; nothing is expected of anybody. */
const DRAFT = 'draft';
/** `revision:2` — the Nth pass on a deliverable, carried by the child that does it. */
const REVISION_PREFIX = 'revision:';
/** The bare label means *"I am a gate"*. The namespaced one below means the opposite. */
const GATE = 'gate';
/** `gate:G2` — *"I count towards G2"*, which is what tally's readiness query counts. */
