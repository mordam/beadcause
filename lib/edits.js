 ...
 *
 * Exported because lib/editwork.js decides from the same list which brief a worker on one
 * of these beads gets — a fourth gesture added here and not there would file beads this
 * app could not then say anything useful about.
 */
export const KINDS = new Set(['retype', 'describe', 'point']);
...
export async function rootFor(bd, ws, cfg, { actor = null } = {}) {
  ...
  if (marked.length) {
    ...
    // already under it is the one every earlier pass was filed against. created_at is
    // millisecond-resolution — two concurrent passes racing to file the first root
    // before either sees one can tie on it — so id breaks the tie deterministically
    // rather than letting accidental array order decide which becomes "the" root.
    const oldest = [...marked].sort(
