...
/**
 * Is this row the **parent** of a round of findings, rather than one of the findings?
 *
 * The plain `review-followup` label goes on both — provenance is provenance — so it cannot
 * tell them apart. The keyed one is written to the parent alone (`followUpPlan`), which
 * makes it the only mark in the tracker that says *this bead's content is its children*
 * about a bead that is not an epic. lib/finishedepic.js reads it to offer the one-tap close
 * an epic in the same state already gets (bc-xl7n.137).
 *
 * The prefix and not the whole key, because the reader has no pull request in hand and does
 * not care which one it was — and here rather than in the reader, so the shape of the key
 * stays a fact this file owns.
 */
export const isFollowUpParent = (row) =>
  (Array.isArray(row?.labels) ? row.labels : []).some((l) => String(l || '').trim().startsWith(FOLLOWUP_KEY_PREFIX));
