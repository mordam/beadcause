 ...
 *
 * ## Not lib/authority.js's family either, and it is worth saying so once
 *
 * This looks like a "may this proceed" check from a distance and it answers a different
 * question — who else is editing this file right now, with no notion of endorsement, a
 * human tap or an unattended window anywhere in it. lib/authority.js's map names it and
 * excludes it explicitly, so a future reader does not re-file it as a seventh member.
 *
...
export function collisions(now = Date.now(), { alive = onDisk } = {}) {
  ...
  for (const bucket of paths.values()) {
    ...
    out.push({
      repo: held[0].repo,
      file: held[0].file,
      // `since` is millisecond-resolution — two sessions racing to claim the same
      // file, exactly the scenario this collision detector exists to catch, can tie
      // on it. Untied, which one reports as "mine" (sessions[0], see regionsForCollision)
      // vs "holder" would flip by accidental bucket order. session breaks the tie.
      sessions: held.sort((a, b) => a.since.localeCompare(b.since) || String(a.session).localeCompare(String(b.session))),
