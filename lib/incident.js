 ...
 * daemon — a P0 lands on the inbox the phone is already polling. And the interesting
 * half of a communication commitment is the one that does not exist yet: if the system
 * this is ever attested about has user entities under contract, *they* have to be told,
 * within whatever window their contract says, and that window is not knowable from here
 * (bc-228x has not settled whose boundary this is). A commitment stated on the bead and
 * measured is worth having now; a mail-out to a customer list nobody has agreed on is
 * not.
 */
...
export function register(rows = [], { now = Date.now(), config = null } = {}) {
  ...
  const clocks = all
    .filter((r) => !(r.labels || []).includes(REVIEW_LABEL))
    .map((r) => {
