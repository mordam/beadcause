...
function defaults() {
  ...
  return {
    ...
    advocates: {
      ...
      // Close a work session's window once its bead is closed. `claude` is
      ...
      // Only ever a *closed* bead, only ever an idle process, and only ever one whose
      // pid Claude Code still reports under that bead's name. See lib/reap.js.
      closeFinishedSessions: true,
      // Between the bead closing and the first signal: the tail of a delivery — the
