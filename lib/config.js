...
function defaults() {
  ...
  return {
    ...
    // How finished work gets in: a branch, a pull request, and a question whose
    // answer is the merge. See lib/delivery.js and bin/deliver.js.
    //
...
/**
 * Everything state.json holds, and what a reader gets when the file doesn't hold it.
 *
 * One object rather than a default per call site, because the call sites are where
 * this went wrong: the fallback used to be `{ notified: [] }` alone, so every reader
 * of `commentCounts` had to remember its own `|| {}` and exactly one of them did.
 * A key added here is defaulted everywhere at once.
 *
 *   notified      keys already pushed, so a question buzzes the phone once
 *   commentCounts key → comments seen, so an agent's reply is detected
 *   filter        the inbox's space/workspace chips — see /api/filter in lib/server.js
 */
const STATE_DEFAULTS = { notified: [], commentCounts: {}, filter: null };

export function loadState() {
  let saved = null;
  try {
    saved = JSON.parse(fs.readFileSync(STATE_PATH, 'utf8'));
  } catch {
    /* absent, torn or hand-mangled — all three mean "nothing saved yet" */
  }
  return { ...STATE_DEFAULTS, ...(saved && typeof saved === 'object' ? saved : null) };
}

/**
 * Merge `patch` into the state file. Read-modify-write, not a wholesale replace.
 *
 * Several owners write this file and each knows only its own keys: the poller saves
 * `{ notified, commentCounts }` from four separate places, and the inbox filter is
 * written by a request that can land between any two of them. A plain write from
 * either side drops the other's key — silently, only under concurrency, which is the
 * shape of bug that lives happily under a passing test suite.
 *
 * Shallow on purpose. Every key here is owned whole by one writer, so a caller
 * handing over `commentCounts` means *these are the counts now*, not "add these".
 *
 * No lock, and none needed: the read and the atomic rename are both synchronous, so
 * node cannot run another handler in between.
 */
