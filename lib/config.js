...
// And once more, in the other direction only: lib/accounts.js imports lib/addressee.js,
// which imports nothing at all, so this one cannot start a cycle whatever else grows in
// either file. `accountHandles` is called from `loadConfig`, long after both have
// evaluated.
import { accountHandles, accountList } from './accounts.js';
// Discovery itself lives in its own module, and lib/reposcan.js is why: it needs the
// container/in-repo distinction and `test/reposcan.mjs` asserts that it never imports
// this file. Both settings that steer it live over there — `workspaceRoots` says where
// to look, `workspaceDirs` names one workspace outright — and both are re-exported here
// because they are config settings to every caller but that one, which expects to read
// them off lib/config.js. (`expandHome` went with them; a second copy of it is how two
// settings quietly disagree about what `~` means.)
import {
  discoverWorkspaces,
...
export { workspaceRoots, containerRoots, isRepoRoot, namedWorkspaces } from './workspaceroots.js';
 ...
 *
 * Checked at each act site individually, not through one function — there is no single
 * choke point every act in this daemon passes through. One of six places that decide
 * whether something may run with nobody watching; lib/authority.js is the map of all
 * of them.
 */
 ...
 *
 * `BEADCAUSE_TAILSCALE_IP` answers instead of the CLI when it is set, and that exists for
 * one reason: without it this function is the one place a test cannot get away from *the
 * tailnet identity of the machine it is running on*. bc-rcrt found `test/tlsadmin.mjs`
 * asserting the literal `100.96.105.106` — this laptop's own Tailscale address — as the
 * origin the daemon falls back to when HTTPS is switched off. It passed here for exactly
 * the reason it could never pass anywhere else, and the first CI runner to see the suite
 * was the first machine to say so.
 *
 * Deliberately **not** folded into `BEADCAUSE_TAILSCALE`, which overrides the *binary*:
 * `test/certrenew.mjs` points that at a fake shell script and asserts the exact list of
 * subcommands it was handed, so teaching this function to run that fake would put an
 * `ip -4` in a list three assertions there count.
 */
export function tailscaleIp() {
  const pinned = process.env.BEADCAUSE_TAILSCALE_IP;
  if (pinned) return /^\d+\.\d+\.\d+\.\d+$/.test(pinned) ? pinned : null;
...
function defaults() {
  ...
  return {
    ...
    // How long bin/router.js lets a retired backend finish requests it already had
    // before killing it out from under them — bc-xl7n.134. A parked long poll runs up
    // to 55s and a board sweep has been measured past 65s, so this is a starting point
    // rather than a ceiling worth raising to chase them: `/api/poll` and `/api/queues`
    // both answer a forced kill without treating it as the daemon failing (see
    // `bin/router.js`'s `retire`), so a phone mid-request across a swap gets told to
    // retry rather than filing a P0 about a backend that was working fine. Lower it
    // only in a test that needs the drain-and-kill path reachable on demand.
    drainMs: 60000,
    ...
    /**
     * The **base** of the byline every write this daemon makes carries, and it is a
     * base rather than the whole thing since bc-lx3k.
     *
     * What lands on the bead is `bylineFor(cfg)`: this string on its own, or
     * `beadcause (carol@example.com)` once `me` below says who this Mac is. On a
     * tracker six people share, an identical `beadcause` in `created_by` on all six
     * machines meant nothing recorded which engineer filed anything — see
     * lib/byline.js, and note that a byline is display only, because `--actor` is a
     * field an agent can write whatever it likes into.
     *
     * A signed-in browser overrides it per call with the address it signed in as; that
     * is a person speaking rather than a byline, and it is not wrapped. See lib/bd.js.
     */
    actor: 'beadcause',
    ...
    owner: detectOwner(),
    /**
     * Who this Mac's person *is*, in the tracker — and it is not `owner`.
     *
     * `owner` is what agents call you in prose ("Adam is not at the keyboard"). This is
     * the handle a question is addressed to, on a graph other machines can also see:
     * `"adam@example.com"`, or a list when one person answers to two addresses.
     *
     * **Null by default, and null means this daemon is everybody.** Every question in
     * the graph rings this phone, which is what a single-person install has always done
     * and what it must keep doing — the silence is not a default that happens to be
     * quiet, it is a branch `addressedElsewhere` cannot enter. Set this only on a
     * tracker more than one person reads, and set it on each machine to *that* person.
     *
     * With it set, two things happen and nothing else does: every question this machine
     * files carries a `for:<handle>` label, and a question labelled for somebody else
     * arrives quietly here — still filed, still counted, still on the screen, exactly
     * as a muted space or a filtered workspace is. Deliberately not asked by `npm run
     * configure`: on the one-Mac install that command is written for, the honest answer
     * is that there is nobody to distinguish you from. See lib/addressee.js.
     */
    me: null,
    ...
    // NOT the way to put a workspace on a server. This sets BEADS_DOLT_SHARED_SERVER,
    // which selects bd's *shared* (global) server; a per-project `dolt sql-server` is
    // dolt_mode="server" in that workspace's own .beads/metadata.json and needs nothing
    // here. And this flag is fleet-wide, so turning it on forces every workspace into
    // shared mode and every bd call then fails against a server that isn't listening.
    // Left off, embedded workspaces retry through the Dolt lock and server-mode ones
    // are already fine. See lib/bd.js.
    sharedServer: false,
    ...
    //
    // `readSpaces` is the *other* direction and is a separate decision: the spaces an
    // unattended agent may read a page out of, empty until you name one. It is empty
    // rather than "whatever you publish into" because the token that publishes can
    // read the whole site, so inheriting would quietly mean every agent this Mac
    // dispatches may read the company wiki. See lib/confluence.js.
    confluence: {
      site: null,
      email: null,
      space: null,
    ...
    // Where to look for trackers. `~/beads` is the shape almost every install has —
    // one subdirectory per workspace — and a root that has its own `.beads` **is** a
    // workspace, which is what a tracker living inside the repo it tracks looks like.
    // Add the repo, not its parent:
    //
    //   "workspaceRoots": ["~/beads", "~/climative.dev/architecture"]
    //
    // Discovery runs on every start and writes the result into `workspaces` below, so a
    // root added here needs no second edit, and a workspace *created* under it later
    // arrives on its own. That is the difference between this and `workspaceDirs`
    // beneath it, which pins one name: use a root when the answer is "and look here
    // too", and a name when the tracker is somewhere nothing sensible could be a root —
    // or when the point is `null`, which no amount of looking can express. Both are
    // re-applied on every load, and a name pinned below wins over anything a root turned
    // up under it. See `discoverWorkspaces` in lib/workspaceroots.js.
    workspaceRoots: [defaultWorkspaceRoot()],
    // Workspaces the roots above cannot answer for, keyed by name, and empty on every
    // install that has never needed one. A **directory** puts a workspace into the list
    // from wherever it actually lives; **`null`** takes one out and keeps it out.
    //
    //   "workspaceDirs": {
    ...
    //   }
    //
    // What a root cannot express is a tracker that lives **inside a checkout** you would
    // not want read as a root, and that is not a hypothetical shape: a team's shared
    // graph belongs in the repo the team already clones, because that is what makes
    // `bd bootstrap` a clone of something rather than an empty database (lib/team.js,
    // lib/sync.js). Moving one there used to make it invisible — Climative's `cl-`
    // graph, the one wired to JIRA and the only workspace on that Mac with a Dolt
    // remote, was moved into the `architecture` checkout on 2026-08-12 and simply
    // stopped being swept. Nothing said so. Its questions stopped reaching the phone,
    // its ready beads stopped being queued, and a workspace that is not polled looks
    // exactly like a workspace with nothing in it.
    //
    // **`null` is the other half and it is not symmetry for its own sake.** The copy
    // left behind by that move was still under `~/beads`, so it was still swept — and a
    // retired tracker swept as though it were live draws stale questions as current ones
    // and lets them be answered, which is worse than not seeing them at all. The only
    // way to stop that was to move the directory out of `~/beads` entirely, which is a
    // person remembering a rule about this program while doing something else. Written
    // here it survives, it is reviewable, and it says which name was set aside — and,
    // in the name, when. Nothing a root can say does that, which is why both keys exist.
    //
    // Both halves are re-applied on every load rather than baked into `workspaces` once,
    // which is the difference between this and hand-editing `workspaces` directly — what
    // people were doing until this key existed. An entry there is a fact about the
    // moment it was typed, and there is nothing to distinguish it from one discovery put
    // there: it is kept only for as long as its directory happens to exist, and a
    // checkout that is briefly away — moved, re-cloned, on a volume that was not mounted
    // — drops it for good, silently, because reconciliation writes the shorter list back.
    // An entry here is a rule, and a rule comes back. See `reconcileWorkspaces`.
    workspaceDirs: {},
    ...
    // (a `chpwd` hook mapping `<projectRoot>/<repo>` → the `<repo>` workspace). When
    // set, a session opens in the matching checkout and beadcause verifies a shell there
    // really would resolve back to the same workspace. Left null — the default —
    // sessions open in the workspace’s own directory, the parent of its `.beads`, where
    // `bd` finds the tracker by walking up from the cwd.
    projectRoot: null,
    // Workspace a shell OUTSIDE projectRoot resolves to, if yours has one.
    fallbackWorkspace: null,
    /**
     * Where Save sends a pass made with edit mode on (lib/edits.js).
     *
     * Both null, and both nulls do something rather than nothing. The **workspace** is
     * the one whose sessions open in this checkout — an edit typed into this screen is a
     * change to *this app* whichever tracker happens to be on it, and filing it onto
     * `workspaces[0]` would put it on somebody else's board (lib/crash.js makes the same
     * argument for the daemon's own crashes, out of the same function). The **root** is
     * found by its `edit-root` marker or created on the first Save, so an install that
     * has never configured anything still files a pass under a P0 that exists — without
     * which nothing under it would be workable at all (bc-rfnr.7).
     *
     * Name a root here only to override that: a second standing container, or a graph
     * where the marker is on the wrong bead. A root named here and since closed is
     * ignored rather than used, because a pass filed under a closed bead is a pass
     * nothing can reach.
     */
    edits: {
      workspace: null,
    ...
    /**
     * Where the requirements corpus is — bc-fvmx.
     *
     * Climative records acceptance criteria as requirements in `resources/reqs/**` in the
     * architecture repo, and that vocabulary is what every requirement id in the tracker
     * is validated against (lib/requirements.js). Null means "look in the repos you
     * already know about": the corpus lives inside a checkout that is normally one of the
     * approved repos for a workspace, so the path is derived rather than configured, and
     * an install with no such checkout simply has no requirements — which is every
     * personal one, and is not a fault.
     *
     * Name a directory here only when that derivation cannot work: an architecture
     * checkout that is not one of the repos this daemon works in. Either the checkout root
     * or its `resources/reqs` is accepted, because both are what somebody has to hand.
     */
    requirements: {
      corpus: null,
    },
    ...
    autoEndorse: false,
    // …and per *repo*, which outranks both. A space is the wrong unit for this one
    // setting: the reason to stop holding — nobody but me reads this tracker, so the tap
    // is not a review — is true of one checkout and not of the five sitting beside it in
    // the same space. `{ "beadcause": true, "sophab": false }`, keyed by workspace name
    // like `jira` and `advocates.perWorkspace` and for the same reason — the `workspaces`
    // array is rediscovered on every start, so a field written onto an entry there is
    // gone at the next restart. Absent means inherit; set it from the repo row on the
    // space details screen.
    autoEndorsePerWorkspace: {},
    // …and the same shape for the four answers that decide whether finished work lands
    // and goes live without a tap: does a worker merge its own pull request, must Adam
    // approve it first, must an agent have reviewed it first, and does the merge then
    // deploy itself. Each outranks the space, absent means inherit, and `PER_WORKSPACE`
    // in lib/spaces.js is the table that ties the five together.
    //
    // The same argument as above, applied to the setting that most needed it: only one of
    // the six repos in the Personal space has a deploy this Mac can run at all, so saying
    // "beadcause ships its own merges" through the space meant arming five repos nobody
    // had asked about. `{ "beadcause": true }` says it about the one.
    //
    // None of the four loosens a gate. `bin/deliver.js` still waits for the pull
    // request's checks and refuses over a red one; `autoShipPerWorkspace` only ever
    // reaches a merge that is already on `origin/main`. They move who presses the button,
    // not what has to be true before it can be pressed.
    autoMergePerWorkspace: {},
    requireApprovalPerWorkspace: {},
    reviewRequiredPerWorkspace: {},
    autoShipPerWorkspace: {},
    // Empty, including for beadcause, and that is a correction rather than caution.
    //
    // This was shipped as `{ beadcause: true }` on the reasoning that bc-8479e had made
    // a downmerge here almost always trivial. bc-xl7n.121 landed the same afternoon and
    // showed the premise underneath it is false: the arm this setting sits in tests
    // `mergeStateStatus === 'BEHIND'`, and that value **is only ever reported under a
    // branch-protection rule requiring branches to be up to date**, which
    // `mordam/beadcause` does not have (`gh api repos/mordam/beadcause/branches/main/
    // protection` is a 404). So on this repo that arm has never fired, and turning this
    // on here would have promised a saving on a path nothing reaches.
    //
    // It is left in with nothing in it rather than removed, because the arm is real on a
    // repo that *is* protected — which is where "the base moved, so run the whole gate
    // again before you may merge" is a rule somebody else's GitHub is enforcing. Turn it
    // on there, having read `GLOBAL_DEFAULT` in lib/spaces.js first.
    ...
    pr: {
      ...
      base: 'main',
      /**
       * The workspaces whose integration branch is not that one — `{ "deluvia":
       * "atlas/public-launch" }`, keyed by workspace name like `autoMergePerWorkspace`
       * and `jira` above.
       *
       * `base` is one string for the whole install and it was right for as long as
       * every workspace merged into `main`. `deluvia` does not: its work lands on
       * `atlas/public-launch`, so an install-wide `main` would open every deluvia pull
       * request against a branch that repo never merges into — and moving the global
       * would have made `main` wrong for the other nine.
       *
       * Resolved in `configuredBase` in lib/prbase.js, which is the one place any of
       * this is read, and it stays *underneath* the per-repo answer a multi-repo
       * workspace gets from GitHub: one string cannot be the right base for forty
       * repos. `--base` on `bin/deliver.js` still wins over both.
       */
      ...
      //
      // **The default was asked about once a reviewer existed, and it stays `false`.**
      // bc-36xx.9, and it is recorded here so the next reader does not reopen it. The
      // argument for flipping it was that the paragraph above stops being true the moment
      // a ReviewAdvocate reads every pull request: "nobody is going to review it" is no
      // longer the case, so why is the flag still off? Because the reviewer that now
      // exists is an *agent* and this flag is about the *human*. Adam's answer to bc-0cop
      // is that where a space asks for his approval the agent's is necessary and not
      // sufficient — he still admits those himself — and everywhere else the agent's
      // approval alone releases the pull request. So the agent review is a separate gate
      // in series with this one (`reviewRequired` below, lib/reviewgate.js) rather than a
      // replacement for it, and turning this on by default would start demanding his tap
      // on every delivery in the repo, which is the opposite of what that epic is for.
      requireApproval: false,
      // Must an **agent** have reviewed it before the merge queue merges it? bc-36xx.4,
      // and lib/reviewgate.js is the gate.
      //
      // A different question from `requireApproval` above and in series with it, not
      // instead of it: that one is Adam's own tap, and Adam's answer to bc-0cop is that
      // where a space asks for it the ReviewAdvocate's approval is necessary and not
      // sufficient. Everywhere else the agent's approval alone releases the pull request.
      //
      // **Off, and this is the one default here that is dangerous the other way round.**
      // The gate holds a pull request until a verdict appears, so turning it on in a
      // workspace where no ReviewAdvocate window opens does not slow the queue, it stops
      // it — every branch at once, quietly, with each merge-bead saying only that nothing
      // has reviewed it. Turn it on per repo, in `reviewRequiredPerWorkspace`, once
      // something is actually reviewing there.
      reviewRequired: false,
      // May the merge queue judge a clean downmerge on the checks the pull request
      // already had, instead of waiting for them to run again? — bc-kbvhg.
      //
      // Off, and off is exactly today's behaviour: outcome 2 of six in
      // lib/mergequeue.js downmerges and ends the tick, because "the checks are now
      // re-running against what is actually going to land". `MAX_DOWNMERGES` is 3 and
      // main takes about forty merges a day here, so a branch that sits in the queue
      // for two days can pay for three full gate runs over a diff that never changed.
      //
      // On, a downmerge that GitHub put in **without a conflict**, on a branch whose
      // checks were already passing, falls straight through to the verdict and merges
      // on the same tick. Conflicts are untouched: those still open a resolver, whose
      // brief still runs the repo's gate.
      //
      // The reason it is a switch rather than a change of behaviour is bc-rcrt's — two
      // branches that each pass alone can still break main together, and the downmerge
      // is where that gets caught. `GLOBAL_DEFAULT` in lib/spaces.js carries the whole
      // argument for when the trade is worth taking, and the one condition that makes
      // it safe: the `test` job also runs on push to main, so a bad pairing reddens the
      // base within minutes and lib/redbase.js holds the queue on it. A repo whose CI
      // does not run on its base should leave this off.
      trustChecksAcrossDownmerge: false,
      ...
      // a check that has not reported in a quarter of an hour is a fact about CI, not a
      // licence to merge over it. Then it asks.
      //
      // **Fifteen minutes because this repo now has a check, and it was measured**
      // (bc-rcrt). `.github/workflows/test.yml` runs all 159 suites on a macOS runner in
      // about five minutes, plus queueing — so the five minutes this used to be would
      // have turned roughly every delivery into "checks were still running, so it
      // stopped waiting", a card for Adam over a pull request that was about to go
      // green on its own. The job's own `timeout-minutes` is 20, so a wait longer than
      // this one would be waiting on a run GitHub has already killed.
      mergeWaitMs: 900000,
    ...
    release: {
      ...
      seconds: 300,
      // And how often while a settle window is open, because the window is only ever
      // looked at when the sweep runs — a one-minute window on a five-minute sweep is a
      // five-minute window wearing the wrong number. This is the expensive cadence and it
      // is paid for only during the few minutes an hour a workspace is armed. See
      // `sweepEvery` in lib/server.js.
      armedSeconds: 30,
      // The settle window: how long an armed workspace waits, with nothing new arriving,
      // before its auto-ship fires — and the ceiling on that however much keeps arriving.
      // Every merge inside the window pushes the deadline out by `settleSeconds` again,
      // so what is being waited for is the merging stopping; `settleMaxSeconds` is what
      // stops a repo that is merged into every fifty seconds from never deploying at all.
      // The arguments for both numbers are in lib/release.js, where they are also the
      // defaults — these are here to be found, not because the values differ.
      settleSeconds: 60,
      settleMaxSeconds: 300,
    },
    // What we said we would do about an error, before one happened. See lib/incident.js.
    //
    // An error already files itself as a P0 the moment it happens — detection and
    // recording are the same act here. These are the numbers that turn that into a
    // control: how long each severity has to be acknowledged and resolved in, both in
    // MINUTES, measured against bd's own `created_at`, `started_at` and `closed_at`. The
    // defaults are in lib/incident.js and are repeated nowhere, so a fresh install has a
    // stated commitment without having configured anything; a value here overrides one
    // number and leaves the rest, because a half-configured commitment must not quietly
    // become no commitment at all.
    incidents: {
      // sev1 the daemon is down · sev2 degraded · sev3 one surface · sev4 handled
      sev1: { acknowledge: 15, resolve: 240 },
      sev2: { acknowledge: 60, resolve: 1440 },
      sev3: { acknowledge: 1440, resolve: 10_080 },
      sev4: { acknowledge: 10_080, resolve: 43_200 },
      // Occurrences of one fingerprint before it escalates a level. It can never reach
      // sev1: that one means the process died, which is a fact and not a volume.
      escalateAt: 10,
      // The severity at and above which a resolved incident owes a post-incident review.
      reviewFrom: 'sev2',
      // And the remediation SLA for a dependency advisory, in days, by npm's own severity
      // word — the other half of CC7.1. See lib/vulnscan.js.
      vulnerabilityDays: { critical: 7, high: 30, moderate: 90, low: 180 },
    },
    ...
    advocates: {
      ...
      globalMaxWorkers: 20,
      // And the other half of that total, because a resolver is the same kind of thing:
      // an unattended session in an iTerm window running the repo's own gate, opened by
      // the pull-request sweep rather than by an advocate. It used to be a constant in
      // lib/resolvers.js that nobody could turn down, so the two caps added up to a
      // number written nowhere; since bc-29b3 they are one budget — `globalMaxWorkers`
      // is the total, and this is the most of it a sweep may take. Clamped to 6.
      maxResolvers: 2,
      perWorkspace: {},
      ...
      // Read those archives back, looking for work the agents did by hand that a command
      // should have done, and file each repeated shape as a held candidate bead —
      // lib/sessionaudit.js. Triggered by a session *ending* rather than by a bead going
      // ready, which is what makes it the one agent here that reads finished work.
      sessionAudit: true,
      // How many unread archives have to pile up before a run is worth its `claude -p`.
      // Also what makes a run's input interesting: a batch is a pattern's worth of work,
      // and one session at a time can only ever show a one-off.
      sessionAuditEvery: 5,
      // The floor between runs whatever the arrivals, so a burst of finished sessions
      // cannot turn into a burst of agents.
      sessionAuditCooldownMinutes: 60,
      // How many archived sessions one run reads. Clamped to 3..40.
      sessionAuditMax: 12,
      // bc-dgx7.7. The backstop: past this age, the oldest unread archive is audited
      // whatever `sessionAuditEvery` says, because a checkout that finishes a few
      // sessions and then goes quiet never sees another one land to push the count over
      // it. Clamped to 60..10080 (an hour to a week).
      sessionAuditMaxAgeMinutes: 24 * 60,
      // How often the poll cycle asks every checkout whether the backstop above applies.
      // Not the fast poll clock — asking costs two `git` reads per checkout and a
      // workspace can hold forty of them. Clamped to 5..1440.
      sessionAuditSweepMinutes: 30,
      ...
      // `QUEUED-`, `DONE-` or `done-`, which is a thing only a session writes about
      // itself at the end of its work — or, for `DONE-`, what the merge queue writes
      // over the first of those when the branch lands (lib/retitle.js) — and the bead
      // named in that name must be closed. A window of your
      ...
      // The windows that are no longer sessions at all: an iTerm frame whose last tab
      // went away, which iTerm occasionally leaves behind when a session ends. There is
      // nothing in one — no process, no scrollback, and no way in, since ⌘W is Close
      // Session and there is no session — so closing it needs none of the guards above,
      // and nothing else ever will: the only close in this daemon is the shell's own
      // `exit`, and that shell is what is missing. See bc-30ve.
      closeEmptyWindows: true,
      // The third sweep, and the widest: a window this daemon opened that has gone quiet
      // is parked — its conversation written down by session id — and then closed, so an
      // answer can bring the same agent back rather than briefing a new one.
      //
      // Its own switch, like the other two, and for a sharper version of the same reason.
      // This one closes windows whose work is *not* provably anywhere else — no closed
      // bead, no merged pull request, just an agent that stopped talking — and the only
      // thing that makes that safe is the park being written first. Somebody who does not
      // trust the resume must be able to turn off the close in one place. Off also when
      // `closeFinishedSessions` is false, which stays the whole feature's off switch.
      parkIdleWindows: true,
      // How long quiet is long enough. The argument for the number is beside the value,
      // in `PARK_DEFAULTS` in lib/parked.js.
      parkIdleMinutes: 10,
      // And the two bounds that stop "wait" meaning "forever". `parkWorkerIdleMinutes` is
      // how long this sweep yields to `reconcile` over a window whose worker slot the
      // advocate still holds, before parking it with the plain sentence rather than
      // leaving it on screen until `workerTimeoutMinutes`; `parkStuckMinutes` is how long
      // a status that is neither `idle` nor `busy` may stand before it is read as stale.
      // Both argued beside their values in `PARK_DEFAULTS`.
      parkWorkerIdleMinutes: 20,
      parkStuckMinutes: 60,
      // How many times one conversation may be carried over into a new window after its
      // own **disappeared** — the loop guard on the `gone` ending, argued beside its value
      // in `PARK_DEFAULTS`. The first disappearance is an accident and is repaired for
      // free; a resumed window that also disappears is a pattern, and gets the fresh brief.
      maxResumes: 1,
    },
    ...
    spaces: [],
    // Which of your lives the app is about — the level above a space. See lib/accounts.js.
    //
    //   "accounts": [
    ...
    //   ]
    //
    // **Empty is off, and off is what every install that has never touched the picker
    // stays.** With no accounts every workspace is in scope everywhere, which is what
    // this program has always done; with one, the only thing that changes is the address
    // in the top bar and the handle a filing is stamped with. The second account is what
    // separates anything — after it, a screen shows one account's workspaces and gives no
    // sign the others exist.
    //
    // A space groups workspaces by *when they may interrupt you*; an account groups them
    // by *whose day this is*. They nest, coarsest first, and the second does not replace
    // the first: "don't buzz me about work at the weekend" is still a quiet-hours answer,
    // and it is still worth having inside a work account you spend Tuesday in.
    //
    // Written from the phone — `POST /api/accounts` — because a split you cannot change
    // from the device you are holding is one you make wrong once and live with. Unlike
    // `workspaces` above, nothing here is discovered or reconciled: an account is a rule
    // a person wrote, and a workspace it names that is not on this Mac is left in place
    // and simply not offered (`accountWorkspaces`).
    accounts: [],
    // Department relays, per workspace — see lib/relay.js. A bead whose **assignee** names
    // a role here is not dispatched to a generic worker: one window carries it through
    // that department's whole check chain, adopting each role's standing profile in turn,
    // and stops at a review packet for you rather than at a merge.
    //
    // Keyed by workspace name, like `jira` and `sessionDirs`, which is what makes the
    // `deluvia` entry below inert on an install that has no such workspace. Empty it —
    // `"relays": {}` — to turn the mechanism off everywhere; there is no other switch,
    // because a relay nothing is assigned to costs nothing.
    //
    // A department is stated as its members and its checks, never as a hand-written
    // ordering: the chain is derived from `docs/STUDIO_CHARTER.md` §6 — draft, check,
    // revise, file — so a department that gains a checker gains it in every chain at once.
    relays: {
      deluvia: {
        // Where a role's standing profile lives, relative to the checkout. `{role}` is
        // filled in; the brief tells the window to read it, so nothing is inlined here.
        profile: 'ai-context/agents/{role}/{role}.md',
        // The one role that is not under that path: herald is wired as a real Claude Code
        // subagent, so its profile is where subagents live (charter §10). A path in a
        // brief that resolves to nothing is worse than none — the session opens it, gets
        // nothing, and has to work out on its own whether the role is real.
        profiles: { herald: '.claude/agents/herald.md' },
        // Read once, by the first role, before it does anything. The charter is who works
        // here and what they may decide; the pipeline is how a draft reaches the phone.
        docs: ['docs/STUDIO_CHARTER.md', 'docs/APPROVAL_PIPELINE.md'],
        // The approval steward, and the only route to the phone. Runs last in every chain.
        filer: 'ward',
        // What the filer puts on the review packet. `needs-approval` without `human` is
        // always a bug — the pair is what makes a bead answerable from a lock screen.
        packet: ['needs-approval', 'human'],
        // Executive produces process, not reviewable deliverables, so a bead assigned to
        // one of these gets no relay and dispatches as an ordinary worker.
    ...
    //
    // The default is the default workspace root and **not** `workspaceRoots`, which the
    // wizard unions in instead. Discovery and file-read reach are two different
    // permissions: adding a root so a tracker is found must not, by itself, hand every
    // agent on this Mac read access to the repo that tracker lives in.
    assetRoots: [defaultWorkspaceRoot()],
    // JIRA, per workspace. Empty — the default — is every workspace on *auto*: on exactly
    // when that workspace's bd already points at a site (`bd config get jira.url`), and
    // off, at one memoised `bd` spawn, when it does not (bc-6s383). `enabled: true` is on
    // whatever bd says; `enabled: false` is off and costs nothing at all.
    //
    //   "jira": { "climative": { "enabled": true, "email": "you@company.com" } }
    //   "jira": { "sophab": { "enabled": false } }
    //
    ...
    jira: {},
    /**
     * How often the daemon *sweeps* — one `bd human list` per workspace, plus a
     * `bd comments` per conversation you are waiting on. This is a cost, not a
     * latency: `detectSeconds` below is what decides how quickly a change is noticed,
     * and this is now the backstop under it.
     */
    pollSeconds: 30,
    /**
     * How often the daemon asks JIRA what is assigned to you — one HTTP call per
     * workspace whose `jira` block above is switched on, and nothing at all for the rest.
     * See lib/jirapoll.js.
     *
     * Beside `pollSeconds` rather than inside `jira` because that block is keyed by
     * *workspace name*, and a number sitting in it would be a setting for a workspace
     * called "seconds". A minute, because an assigned ticket is not something anybody
     * watches a second hand for, and the floor under it is fifteen seconds rather than
     * the five `pollSeconds` gets: a faster read of the same queue buys nothing but
     * traffic against a rate limit shared with everything else on that site.
     */
    jiraSeconds: 60,
    /**
     * A request that took this long is named in the log, with where its time went.
     *
     * One second because that is the page-load budget bc-1kwl set, so a line in the log
     * means "this missed the budget" rather than "this was slower than its neighbours" —
     * a threshold picked for tidiness would make the log a thing to be tuned instead of
     * read. `0` turns the log off and nothing else: the per-route figures behind
     * `/api/timings` are always collected, because instrumentation you have to switch on
     * is off for every complaint nobody anticipated. See lib/timing.js.
     */
    slowRequestMs: 1000,
    /**
     * How long an archived agent run's *body* is kept, in months — see lib/agentarchive.js.
     *
     * Twenty-four, and the number is not ours to pick freely: it is `RETENTION_FLOOR_MONTHS`
     * in lib/evidence.js, which sets it from the report rather than from the disk — a Type
     * II observation window is twelve months, and the report is relied on for about twelve
     * months after issuance, during which somebody can come back to the population the
     * sample was drawn from.
     *
     * **Raising it is a setting; lowering it is not.** A number under the floor is ignored,
     * because an install shortening this is not making a retention decision, it is making a
     * disk decision and writing a retention rule beside it. The chained record of each run —
     * which bead, which agent, which model, which foundation revision, and the digest of the
     * body — is permanent either way, so what this governs is how long the *text* survives.
     */
    agentLogRetentionMonths: 24,
    /**
     * How old a stranded headless Chrome or scratch directory must be before the daemon
     * reaps it, in hours — see lib/strays.js.
     *
     * **Twenty-four, and the number is the whole safety margin.** A browser check another
     * session started thirty seconds ago looks exactly like one that was abandoned thirty
     * seconds ago; the only thing that tells them apart is waiting, and three such
     * processes were watched finishing normally between two measurements of the pile this
     * exists for. No real run lasts a day. Every orphan does.
     *
     * **Raising it is a setting; lowering it below `FLOOR_HOURS` is not.** An hour is the
     * floor and a smaller number is raised to it, for the reason `agentLogRetentionMonths`
     * has a floor: a number under it is a disk decision written where a safety decision
     * lives. `0` switches the sweep off entirely, which is the honest way to disable it —
     * there is deliberately no boolean beside it, because a boolean would be a second
     * thing to get wrong about the same question.
     */
    strayHours: 24,
    /**
     * How often it *asks whether anything moved*, which is a ~150-byte read per
     * workspace and spawns nothing — see lib/detect.js.
     *
     * Five, because the latency budget says a change reaches the phone within five
     * seconds. Setting this equal to `pollSeconds` turns the whole mechanism off and
     * restores the single-clock cycle that shipped before it, which is the honest way
     * to disable it: there is deliberately no boolean, because a boolean would be a
     * second thing to get wrong about the same question.
     */
    detectSeconds: 5,
    /**
     * Keeping a shared tracker shared — `bd dolt pull` then `bd dolt push`, on a clock.
     * See lib/sync.js.
     *
     * **On, and there is deliberately no list of which workspaces.** A workspace with a
     * Dolt remote is one that syncs; a workspace without one is silent and costs a
     * single cheap `bd dolt remote list` per interval to establish that there is
     * nothing to do. That is not laziness about the config — it is the one shape that
     * cannot be configured wrong. A list is a second place to be right, and both ways
     * of being wrong are bad in a way that is hard to see: a shared workspace missing
     * from the list never syncs and says nothing about it, and a solo workspace named
     * in the list reports a failure every interval about a tracker that is working
     * exactly as intended.
     *
     * It follows that turning this off is not how you keep a graph private — **not
     * having a remote is.** beadcause never adds one. Where a tracker is published is a
     * decision with consequences no daemon should make on your behalf, and it is not a
     * reversible one: a push to a public repo is on the internet whatever you do next.
     *
     * `seconds` is the width of the window in which two machines can act on stale
     * information, which is why it is a setting and not a constant — see
     * `syncEveryMs` in lib/sync.js for the argument for two minutes, and why the floor
     * under it is thirty rather than the five `pollSeconds` gets.
     */
    sync: {
      enabled: true,
      seconds: 120,
    ...
    /**
     * Which files `b7e-dossier` reads, and in what order — see lib/dossier.js.
     *
     * Ordered globs rather than a directory list, and config rather than a constant,
     * because the order *is* the answer: the first block in a dossier is what canon says
     * and a later one is what some draft says, and bc-dgx7.101's whole finding — a
     * chapter making a character 43 where `reference/` says 173 — is only a finding
     * because one of those two sources outranks the other. A workspace nobody has named
     * here still works; it gets `sources` below, which puts canon-shaped directories
     * first and everything else after them.
     */
    dossier: {
      // The fallback set, for a workspace with no entry of its own.
      sources: ['reference/**/*.md', 'docs/**/*.md', 'compendium/**/*.md', '**/*.md'],
      /**
       * Per workspace, keyed by name like `pr.basePerWorkspace` and `jira` above — either
       * an ordered glob list, or an object keyed by `--kind` with a `default` beside the
       * named ones, for a corpus where "what says something about a species" and "what
       * says something about a place" are different shelves.
       *
       * deluvia ships one because it is the corpus the command was built against and its
       * shelves have names nothing could guess: `reference/` is canon, `compendium/` and
       * `docs/` are derived summaries, `novel/` and `graphic-novel/` are drafts that may
       * legitimately disagree with all three until somebody propagates a change.
       */
      sourcesPerWorkspace: {
        deluvia: {
          default: [
          ...
          species: ['reference/SPECIES_*.md', 'reference/**/*.md', 'compendium/species/**/*.md', 'docs/**/*.md', 'CHANGE_LOG.md'],
          place: ['reference/WORLD_GEOGRAPHY.md', 'reference/regions/**/*.md', 'reference/**/*.md', 'docs/**/*.md', 'CHANGE_LOG.md'],
          character: [
            'reference/CHARACTER_*.md',
            'reference/SECONDARY_CHARACTERS.md',
            'reference/**/*.md',
            'compendium/**/*.md',
            'docs/**/*.md',
            'CHANGE_LOG.md',
    ...
    /**
     * How often the daemon publishes what this install can say about itself — see
     * lib/publishsweep.js.
     *
     * It does nothing at all on an install with the management system off, which is every
     * install by default, so this number costs an ordinary user one `git cat-file` an hour
     * that finds nothing. On an install with the layer on it is the longest an amendment,
     * an election or a management transition can sit unpublished, because a governed ref
     * that moved is published on the first sweep after it moved.
     *
     * **There is deliberately no `enabled` beside it.** A cadence is a setting; whether an
     * install with the management system on publishes at all is not, and a key that could
     * stop the chain growing would be a way to open a gap in the record by editing a
     * settings file — which is the whole thing lib/management.js exists to make impossible.
     * The switch is the transition on `refs/beadcause/management`, and it has an author and
     * a reason on it.
     */
    publication: {
      seconds: 3600,
...
/**
 * Turn a hand-added workspace into a rule, once — the migration `workspaceDirs` owes the
 * installs that were already working around its absence.
 *
 * `workspaces` is discovered, but it is *persisted*, and a persisted list invites being
 * edited. That is how the Climative tracker was being served from the `architecture`
 * checkout before this key existed: somebody added the entry by hand, it worked, and
 * nothing said what it was resting on. What it was resting on is `reconcileWorkspaces`
 * finding the directory on every start — so the entry survived exactly as long as the
 * checkout stayed put, and the first start after it moved, was re-cloned, or sat on a
 * volume that had not mounted yet dropped it and wrote the shorter list back. A rule
 * comes back from that; a fact does not.
 *
 * So any saved entry that **cannot have come from discovery** — no configured root
 * turns up that name at that directory, which for a default install means it is not
 * `~/beads/<its name>/.beads` — is copied into `workspaceDirs`, where it means the same
 * thing durably. Asked of the roots rather than of `~/beads` alone since bc-x9u5: an
 * entry a root already reaches must be left as discovery finds it, because pinning it
 * would freeze the directory and turn a later rename into a drop rather than a move. Nothing changes about what is served today: the entry is already in the
 * list and already points at that directory, and this only writes down why.
 *
 * Bounded three ways rather than by a flag, because unlike `moveSquashDefault` this one
 * can read its own answer off the config. A name already in `workspaceDirs` is never
 * touched, so an explicit `null` cannot be undone by it. A directory that is not there is
 * left alone, since pinning a dead path as a rule would be a warning on every start
 * forever — reconciliation drops that entry, and that is the right outcome. And after it
 * has run there is nothing left for it to find, which is why it needs no flag: it is a
 * no-op on every start after the first, on every machine, forever.
 *
 * The file is edited rather than rewritten, for the reason `moveSquashDefault` gives at
 * length: by the time this runs `cfg` carries every default, and `saveConfig(cfg)` would
 * turn somebody's nine-line config into a dump of every setting this repo has.
 *
 * **It says nothing when it works**, which is the one place this departs from the
 * migrations above it, and the reason is that `loadConfig` is not the daemon's — it is
 * every `bin/*.js` in the repo, and a CLI's two streams are both contracts. stdout is
 * parsed (`bin/file.js` prints an id and callers `.trim()` it) and stderr is asserted
 * empty on the happy path (`test/park.mjs`: "nothing was reported, because nothing went
 * wrong"). Both were measured, both went red. A normalisation nothing went wrong in is
 * not news, and it leaves its own record: the key it writes is in the config file, under
 * the name the README explains. A write that *fails* is news, and is returned.
 *
 * Returns `{ adopted, error }` — the names taken on, and the sentence to warn with.
 */
export function adoptHandAddedWorkspaces(cfg) {
  const stray = (cfg.workspaces || []).filter(
    (w) => w?.name && w?.dir && !isDiscoverable(cfg, w) && !(w.name in (cfg.workspaceDirs || {})) && isDirectory(w.dir),
  );
  if (!stray.length) return { adopted: [], error: null };

  cfg.workspaceDirs = { ...(cfg.workspaceDirs || {}) };
  for (const w of stray) cfg.workspaceDirs[w.name] = w.dir;

  try {
    if (fs.existsSync(CONFIG_PATH)) {
      const raw = JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf8'));
      raw.workspaceDirs = raw.workspaceDirs && typeof raw.workspaceDirs === 'object' ? raw.workspaceDirs : {};
      for (const w of stray) if (!(w.name in raw.workspaceDirs)) raw.workspaceDirs[w.name] = w.dir;
      writeJsonAtomic(CONFIG_PATH, raw);
      snapshot('config');
    }
  } catch (err) {
    // The in-memory adoption still stands for this process, and the next load tries the
    // file again — the same choice `moveSquashDefault` makes, for the same reason.
    return { adopted: [], error: `workspaceDirs: could not write the adopted entries — ${err.message}` };
  }

  return { adopted: stray.map((w) => w.name), error: null };
}
...
export function loadConfig() {
  ...
  if (fs.existsSync(CONFIG_PATH)) {
    ...
    // And the incident commitments, for the same reason and with one extra: every reader
    // in lib/incident.js takes `config?.incidents?.…` with a fallback to the defaults in
    // that file, so the absent block is already correct — the merge is here so that a
    // config somebody opens can be *read*, and a commitment nobody can see is one that
    // can be argued about after the fact.
    cfg.incidents = { ...defaults().incidents, ...(cfg.incidents || {}) };
    // And where a pass from edit mode lands. Every reader takes it with `?.`, so the
    // absent block is already the default — the merge is here for the config somebody
    // hand-edits to name a root, which would otherwise arrive with no `workspace` key
    // at all and make "unset means this checkout's own workspace" true by accident.
    cfg.edits = { ...defaults().edits, ...(cfg.edits || {}) };
  ...
  // Before reconciliation rather than after: it reads the *saved* list, which is the
  // only place a hand-added entry is distinguishable from a discovered one.
  const adopted = adoptHandAddedWorkspaces(cfg);
  if (adopted.error) console.warn(`[beadcause] ${adopted.error}`);
  cfg.workspaces = reconcileWorkspaces(cfg.workspaces, cfg);
  reconcileBaseUrl(cfg, { persist: true });
  /**
   * Identity follows the account — see `accountHandles` in lib/accounts.js.
   *
   * A **reorder** of a list somebody typed, never an edit of it: the address of the
   * account in force is moved to the front, and every place that stamps a write already
   * takes the front of this list. So the byline on a comment, the addressee on a
   * question, the owner on a P0 and the handle on a claim all follow the account without
   * one of those four call sites knowing accounts exist.
   *
   * Here rather than at each call site because the short-lived writers are processes,
   * not requests: `bin/file.js` and `bin/plan.js` load this config, file one bead and
   * exit, and doing it at load is what makes them stamp the account you are actually in.
   * The daemon is the long-lived one, so it does the same again when the account is
   * switched under it (`POST /api/account`).
   *
   * A no-op on every install that has configured no accounts, which is what keeps the
   * order somebody typed meaning exactly what it did before.
   */
  // Only when there is an account to put in front. Without the guard an install that has
  // never set either would gain `me: []` where it had no `me` at all — behaviourally the
  // same (`meHandles` reads both as "this Mac does not know who it is") and still a field
  // written into a config file by a program nobody asked to write one. `test/onboard.mjs`
  // is right to refuse it: what may write `me` is a person, not a load.
  if (accountList(cfg).length) cfg.me = accountHandles(cfg, loadState());
  return cfg;
}
 ...
 *
 * **`workspaceDirs` is applied here rather than only in discovery**, and that is what
 * makes it a rule instead of a one-time edit. Three things follow from it, and each is
 * something a saved list on its own could not do:
 *
 * - a name it excludes is dropped even though the directory is still there, which is
 *   the retired-tracker case — the thing being said is "not this one", and a directory
 *   that exists is exactly the state it is being said about;
 * - a name it points somewhere else is dropped from wherever the saved list had it, so
 *   moving a tracker into a checkout cannot leave two entries with one name pointing at
 *   two graphs;
 * - a named workspace comes **back** after an absence, because discovery answers for it
 *   every load. A hand-added entry never did: it survived only while its directory did,
 *   and one boot with the checkout moved away wrote the shorter list to disk for good.
 *   That is the same failure the paragraph above is about, from the other side.
 *
 * Exported for `npm run configure`, which changes `workspaceRoots` mid-run and needs the
 * workspace list to catch up before the questions that are keyed by workspace name. It
 * passes `persist: false` there — the wizard writes once, at the end, and a reconcile
 * that saved on its way past would put half an answer on disk. `cfg` is still required,
 * because it is what says where to look.
 */
export function reconcileWorkspaces(saved, cfg, { persist = true } = {}) {
  const { named, excluded, problems } = namedWorkspaces(cfg);
  for (const p of problems) console.warn(`[beadcause] ${p}`);
  const pinned = new Map(named.map((w) => [w.name, w.dir]));
  const discovered = discoverWorkspaces(cfg);

  const why = (w) => {
    if (!w?.dir) return 'it names no directory';
    if (excluded.has(w.name)) return `workspaceDirs.${w.name} is null`;
    if (pinned.has(w.name) && pinned.get(w.name) !== w.dir) return `workspaceDirs.${w.name} names ${pinned.get(w.name)} instead`;
    if (!fs.existsSync(w.dir)) return `${w.dir} no longer exists`;
    return null;
  };

  const reasons = new Map((saved || []).map((w) => [w, why(w)]));
  const live = (saved || []).filter((w) => !reasons.get(w));
  const gone = (saved || []).filter((w) => reasons.get(w));
  const added = discovered.filter((d) => !live.some((w) => w.dir === d.dir));

  // Both on stderr, and that is not tidiness — it is the argument `reconcileBaseUrl`
  // makes below, hit for real. Every `bin/*.js` here loads the config, and several of
  // them exist to print one value that something else parses: `bin/file.js` prints the
  // id it filed and `test/autoendorse.mjs` reads it with `stdout.trim()`. A workspace
  // notice on stdout does not appear beside that id, it *becomes part of it* — the
  // filing lands, the id read back is a sentence, and the failure says nothing landed.
  for (const w of gone) console.warn(`[beadcause] dropping workspace ${w.name} — ${reasons.get(w)}`);
  for (const w of added) console.error(`[beadcause] adding workspace ${w.name} — ${w.dir}`);

  const merged = [...live, ...added].sort((a, b) => a.name.localeCompare(b.name));
  if ((gone.length || added.length) && cfg && persist) {
...
// caused and has not cancelled, so a bead leaving the inbox can have its row on the
// phone cancelled with it. See lib/ringing.js.
...
const stateDefaults = () => ({
  ...
  // Which account is in force — an email address, or null on every install that has
  // configured none, which is what every predicate over it reads as "everything". The
  // level above `filter`, and stored beside it for the same reason that one is not in
  // localStorage: the push path reads it from inside the poll with no client in the
  // loop, and a phone and a laptop should not disagree about which life you are in.
  // See lib/accounts.js.
  account: null,
  ringing: {},
  answered: {},
  ...
  slack: {},
  // Keyed `workspace/TICKET-KEY`, and the one record here that is **never pruned**: the
  // JIRA tickets you have cancelled. Everything else in this file is dropped when the
  // thing it is about leaves the inbox, which for this one would mean un-cancelling it
  // on the very next sweep — the ticket is still assigned to you and comes back every
  // minute. See lib/jiracancel.js.
  jiraCancelled: {},
  ...
  // The signed-in browsers, keyed by the session id stamped into each one's cookie —
  // the list that makes revoking one device possible without ending the rest. It lives
  // here rather than in memory because bin/router.js replaces the backend several
  // times an hour and every backend reads this file. See lib/devices.js.
  devices: {},
  ...
  // The conversations that are waiting on you, keyed `workspace/id` like most of the
  // rest of this file — a session id, the directory it ran in, and one sentence about
  // what has to happen before it comes back. It is on disk rather than in memory for a
  // harder reason than the others: the window it describes has already been **closed**
  // on the strength of this record existing, so losing it loses the only route back to
  // an hour of an agent's work. See lib/parked.js, which is also where the argument for
  // why closing that window was safe at all is written down.
  parked: {},
  // And the windows that are open *now*, keyed by session id — the register `launch`
  // writes to and the park sweep reads. Two stores rather than one because a window
  // moves between them exactly once and in one direction, and a row in the wrong one is
  // a different kind of wrong: an open window listed as parked is a conversation nothing
  // will resume, and a parked one listed as open is a window the sweep will try to
  // signal forever. See lib/parked.js.
  opened: {},
  // Keyed by workspace name: the last outcome lib/sync.js's `createSyncer` recorded for
  // it, restamped on every tick. Since bc-y3qk.7 — before that this was deliberately
  // never written, and a daemon restart forgot an ongoing outage and re-announced it as
  // freshly broken (85 of 97 logged sync lines followed a restart). Read back into
  // `createSyncer`'s `initial` at boot so a restart mid-outage stays quiet, the same way
  // the running process already would have. See lib/sync.js's header for the argument.
  sync: {},
});
...
export function loadState() {
  ...
  return {
    ...
    // The account, if it is a string and nothing else. Anything odd reads as "none
    // selected", which `activeAccount` resolves to the first configured account — never
    // to "everything", because answering *which of my lives* by showing all of them is
    // the state accounts exist to end.
    account: typeof raw.account === 'string' && raw.account ? raw.account : base.account,
    ...
    // And the same again for the record of what is ringing. The failure to avoid here
    // is the opposite one — a wrong shape must read as "nothing is ringing", so a junk
    // file costs a row left on the phone rather than a cancel aimed at a bead this
    ...
    // And once more for the signed-in devices. The failure to keep out here is the
    // opposite of the others': a wrong shape must read as *no* devices, because every
    // reader of this treats "not in the list" as "that session is over" — so a junk
    // file signs browsers out rather than letting one through that cannot be revoked.
    ...
    // And for the cancelled tickets. The direction of failure is the same as every
    // other one here and it is the deliberate one: a junk field reads as "nothing is
    // cancelled", which costs a ticket coming back onto a screen where you can cancel
    // it again — never a ticket that silently never appears and cannot be got back.
    jiraCancelled:
      raw.jiraCancelled && typeof raw.jiraCancelled === 'object' && !Array.isArray(raw.jiraCancelled)
        ? raw.jiraCancelled
        : {},
    // And for the parked conversations. The direction of failure here is the one that
    // matters most in this list: a junk field must read as *nothing is parked*, so a
    // half-written file costs a window that stays open — which is the state before this
    // feature existed and is survivable — rather than a window closed on the strength of
    // a record that will not resume. `parkable` in lib/parked.js re-checks every record
    // one at a time for the same reason; this only guarantees the shape around them.
    parked: raw.parked && typeof raw.parked === 'object' && !Array.isArray(raw.parked) ? raw.parked : {},
    // And the open windows. A junk field reads as *no windows are open*, which costs the
    // sweep forgetting about windows it opened — they stay on screen, which is the old
    // behaviour — rather than the sweep acting on rows that describe nothing.
    opened: raw.opened && typeof raw.opened === 'object' && !Array.isArray(raw.opened) ? raw.opened : {},
    // And the last sync outcome per workspace. Same direction as `ringing`/`quiet`: a
    // junk field reads as *no workspace has ever synced*, which costs one re-announced
    // `broke` on the next tick — exactly the pre-bc-y3qk.7 behaviour — never a card
    // asserting an outage that is not real.
    sync: raw.sync && typeof raw.sync === 'object' && !Array.isArray(raw.sync) ? raw.sync : {},
  };
}
