...
// this file. Re-exported here because `workspaceRoots` is a config setting to every
// caller but that one, and reading it out of lib/config.js is what they all expect.
import { discoverWorkspaces, defaultWorkspaceRoot } from './workspaceroots.js';
export { workspaceRoots, containerRoots, isRepoRoot } from './workspaceroots.js';
...
function defaults() {
  ...
  return {
    ...
    // root added here needs no second edit — and a workspace named by hand in
    // `workspaces` whose root is not listed survives only until its directory moves,
    // which is the rot `reconcileWorkspaces` exists to stop. See `discoverWorkspaces`.
