...
function defaults() {
  ...
  return {
    ...
    // …and the same shape for the three answers that decide whether finished work lands
    // and goes live without a tap: does a worker merge its own pull request, must an
    // approving review come first, and does the merge then deploy itself. Each outranks
    // the space, absent means inherit, and `PER_WORKSPACE` in lib/spaces.js is the table
    // that ties the four together.
    //
    ...
    //
    // None of the three loosens a gate. `bin/deliver.js` still waits for the pull
