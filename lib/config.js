...
function defaults() {
  ...
  return {
    ...
    // Shipped with an answer in it, like `pr.basePerWorkspace` below and for the same
    // reason: this is the repo the argument was measured in. bc-8479e union-merged the
    // four registries every branch here appends to, which took 13 conflicts out of 34
    // open branches down to 4 — so a beadcause downmerge is now almost always git
    // keeping both sides of one appended line, and a full gate run to re-prove that is
    // the cost bc-kbvhg removes. Nothing is presumed about the other nine workspaces.
