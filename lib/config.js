...
function discoverWorkspaces(cfg) {
  ...
  const byName = new Map(
    names.map((name) => ({ name, dir: path.join(root, name, '.beads') })).filter((w) => fs.existsSync(w.dir)).map((w) => [w.name, w]),
  );
 ...
 *
 * Returns the sentence to print, or '' when there was nothing to do.
 */
export function adoptHandAddedWorkspaces(cfg) {
  ...
  try {
    ...
    return `workspaceDirs: could not write the adopted entries — ${err.message}`;
  }

  const which = stray.map((w) => `${w.name} → ${w.dir}`).join(', ');
  return `workspaceDirs: adopted ${which} — hand-added workspaces are now rules, so they come back if the directory ever goes away`;
}
...
export function loadConfig() {
  ...
  if (adopted) console.error(`[beadcause] ${adopted}`);
