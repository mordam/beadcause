...
// One pure function, and lib/repos.js imports nothing of ours, so there is no cycle to
// worry about here. `~/climative.dev/architecture` is a path a person types into a
// hand-edited config, and expanding it in two places with two slightly different
// answers is how one of them ends up serving a directory called `~`.
 ...
 * workspaces that live somewhere other than `~/beads`; `excluded` is the set of names
 * never to serve, however they were found; `problems` is one sentence per entry that
 * could not be read, for the caller to log.
 *
 ...
 * dropped and discovery still answers for that name if `~/beads` has one. A typo must
 * be loud — a workspace silently not served is the exact failure this key exists to
 * end — but it must not also take out a workspace that was working.
 */
...
/**
 * Every workspace this install serves, in alphabetical order: `~/beads/<name>/.beads`,
 * plus whatever `workspaceDirs` names, minus whatever it excludes.
 *
 * `~/beads` stays the rule and the named entries are the exception, deliberately — an
 * install that has never configured anything still finds its workspaces, which is what
 * makes the first run work at all. A named directory **wins** over a `~/beads` entry of
 * the same name: naming one is a more specific statement than a directory happening to
 * be there, and two entries called `climative` pointing at different graphs is the one
 * outcome nothing downstream could make sense of.
 */
function discoverWorkspaces(cfg) {
...
function defaults() {
  ...
  return {
    ...
    // Workspaces `~/beads` cannot answer for, keyed by name, and empty on every install
    // that has never needed one. A **directory** puts a workspace into the list from
    // wherever it actually lives; **`null`** takes one out and keeps it out.
    //
    //   "workspaceDirs": {
    ...
    //   }
    //
    // `~/beads/*/.beads` is still the rule — `discoverWorkspaces()` above reads that
    // directory and takes what is in it, and on almost every install that is the whole
    // answer. What it cannot express is a tracker that lives **inside a checkout**, and
    // that is not a hypothetical shape: a team's shared graph belongs in the repo the
    // team already clones, because that is what makes `bd bootstrap` a clone of
    // something rather than an empty database (lib/team.js, lib/sync.js). Moving one
    // there used to make it invisible — Climative's `cl-` graph, the one wired to JIRA
    // and the only workspace on that Mac with a Dolt remote, was moved into the
    // `architecture` checkout on 2026-08-12 and simply stopped being swept. Nothing said
    // so. Its questions stopped reaching the phone, its ready beads stopped being
    // queued, and a workspace that is not polled looks exactly like a workspace with
    // nothing in it.
    //
 ...
 *
 * So any saved entry that **cannot have come from discovery** — its directory is not
 * `~/beads/<its name>/.beads` — is copied into `workspaceDirs`, where it means the same
 * thing durably. Nothing changes about what is served today: the entry is already in the
...
export function adoptHandAddedWorkspaces(cfg) {
  const home = path.join(os.homedir(), 'beads');
  const discoverable = (w) => path.resolve(w.dir) === path.join(home, w.name, '.beads');
  const stray = (cfg.workspaces || []).filter(
    (w) => w?.name && w?.dir && !discoverable(w) && !(w.name in (cfg.workspaceDirs || {})) && isDirectory(w.dir),
  );
