...
// the opening one — otherwise a fenced block *inside* a bead's own description, indented
// by the YAML round-trip, terminates this block early and drops every field after it.
// See bc-ka5y.23; lib/beadfiles.js and lib/decision.js carry the same fix.
const BLOCK_RE = /(^|\n)(?:```|~~~)[ \t]*beadproposal[ \t]*\r?\n([\s\S]*?)(?<=^|\n)(?:```|~~~)[ \t]*(?=\r?\n|$)/;
...
export function normalizeBead(raw, i = 0) {
  ...
  return {
    ...
    deps: (Array.isArray(raw.deps) ? raw.deps : raw.deps ? [raw.deps] : [])
      .map((d) => String(d).trim())
      .filter(Boolean),
    // What this bead expects to touch (bc-42ow). Not a field `bd create` takes either —
    // `beadToIssue` writes it *into* the description as the block lib/beadfiles.js reads,
    // because a description is the only text field a `bd list --json` row carries for
    // free — but it is a field on this shape, so the card can show it and an edit can
    // change it while it is still cheap. Read under four names for the same reason
    // `complexity` is read under two: the YAML is generated, and the cost of an
    // unrecognised key is a surface that silently is not there.
    files: normalizeSurface(raw.files ?? raw.touches ?? raw.paths ?? raw.surface),
...
export function proposalBody(workspace, beads, { context = '', intro = '' } = {}) {
  ...
  beads.forEach((b, i) => {
    ...
    // Above the rationale, because it is part of the bead rather than an argument for
    // it — and because it is the one line on this card that says something about the
    // *other* rows: two of them naming one file is a proposal that wants splitting, and
    // that is a judgement to make before pressing the button rather than after.
    if (b.files?.length) lines.push('', `**Expects to touch:** ${b.files.join(', ')}`);
...
function strip(b) {
  ...
  // Carried for the same reason `duplicate` below is: this block is what the phone
  // re-parses to draw the row and what the server files from, so a surface that did not
  // survive the round trip would be a declaration nobody ever wrote to the bead.
  if (b.files?.length) out.files = b.files;
