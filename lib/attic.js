 ...
 * puts the flag *after* the path, and `grep` on this laptop is ugrep, which only honours
 * `--exclude-dir` before it, so the warning went to `/dev/null`, `archive/` was searched
 * anyway, and every *spent* handoff protected its attic entry forever.
 *
...
import { expireRetired, parseWorktrees, retiredAt, ATTIC_DAYS } from './tidy.js';
