 ...
 *
 * A leaf, like `lib/evidence.js`, `lib/access.js` and `lib/documents.js`: it imports the
 * registers it cites and nothing else, reads no state and writes none, so a check, a report
 * and a screen can each hold it.
 */
...
export const REGISTER = Object.freeze([
  ...
  },
  {
    ...
    purpose:
      ...
      'most heavily, and `lib/memoryuse.js` records which notes a run was actually handed.',
    adequacy:
  ...
  },
  {
    ...
    purpose:
      ...
      'opposite direction and is beadcause\'s: which notes reached a brief, which is what makes a memory\'s influence on a ' +
      'run traceable at all.',
    adequacy:
