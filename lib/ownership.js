/**
 * Who owns a root — the one label that answers "what am I answerable for".
 *
 ...
 * `ownerName(cfg)` is the service owner; `ownerOf(issue)` is who took this epic.
 *
 ...
 * `bd update --claim` wrote there. Nothing on the phone says which epic is yours, so the
 ...
 * addresses a question to you is the same string that says you own an epic. Two spellings
 ...
 *
 * **An unowned root is a state, not an error.** Nothing here throws, nothing refuses, and
 * no queue is filtered on it. A root with no owner draws as unowned on the card and is
...
/**
 * The urgent priority. Named rather than inlined because several files ask about it —
 * the stamp in lib/bd.js, the inbox's board, and the gate in lib/underroot.js.
 *
 * A number rather than a string because that is what a `bd --json` row carries. Three
 * literal zeroes is three places for the answer to drift apart.
 */
export const P0 = 0;
...
/**
 * Is this bead an epic? `issue_type`, the field bd itself keys the type off.
 *
 * `type` as well as `issue_type` because the two spellings both reach this: an export
 * row and `Bd.graph`'s index carry `issue_type`, while lib/proposal.js, lib/draft.js and
 * the console's drafts carry `type`. One predicate that reads either is the alternative
 * to every caller remembering which shape it is holding, and the day one of them
 * remembers wrong the bug is an epic that silently is not one.
 */
export const isEpic = (issue) =>
  String(issue?.issue_type ?? issue?.type ?? '').trim().toLowerCase() === 'epic';

/**
 * Is this bead something somebody decided — the predicate the whole of lib/underroot.js,
 * the inbox board and the Epic Advocate are keyed off.
 *
 * **A P0, or an epic at any priority, and the `or` is the point.** Until bc-htoy this was
 * `isP0` alone, and the rule it enforced was that only a P0 could root work, take an
 * `owner:` label, or be given an advocate. That was a mistake, and the shape of the
 * mistake is worth keeping written down: an epic *is* a decision — that is what filing
 * one means — and priority is how urgent the decision is, not whether it was made. Tying
 * the two together meant the only way to have an advocate manage a piece of work was to
 * claim it was the most urgent thing on the tracker, so either the board filled with P0s
 * that were not urgent or real epics went unmanaged. Both directions lose the thing the
 * board exists to say.
 *
 * **Widened rather than replaced, and that is not tidiness.** lib/errors.js files every
 * daemon crash at P0 by construction and those beads are `type=task`. If a root had
 * become "an epic" rather than "an epic or a P0", the first thing to stop being workable
 * would have been the stack trace the app filed on itself — invisible on the board and
 * refused at the door, which is precisely the failure bc-rfnr.4 exists to prevent,
 * arriving through the mechanism meant to fix it.
 */
export const isRoot = (issue) => isP0(issue) || isEpic(issue);
 ...
 * inbox with an empty board rather than one full of other people's work.
 */
...
/**
 * The owner label a root filed *on this machine* should carry, or `[]`.
 *
 ...
 * graph cannot tell whose session filed a bead from `created_by`. It is a *byline* —
 * `beadcause`, and only since bc-lx3k `beadcause (carol@example.com)` on a machine that
 * has said who it is (lib/byline.js) — and a byline is display, not routing: it is bare
 * on every install that has not set `me`, bare on every bead filed before it existed,
 * and a field an agent can write anything it likes into. So ownership is not derived at
 * read time. The machine doing the writing knows, so it says so at write time, as a
 * label, and it rides the sync to the other five.
 *
 * Only the first handle, for `ownAddresseeLabels`' reason: `me` is a list because one
 * person answers to two addresses, and a root owned by both of them is no more owned and
 ...
 * how a root filed against the wrong person gets handed back to the triage that assigns it.
 */
