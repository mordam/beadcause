 ...
 * tick takes the top slice, and what is left over is reported on `skipped` and taken by
 * the next tick. That distinction is the whole of the bug this exists to fix — a cap that
 * silently dropped the tail is how twelve board rows came to decide, for good, which ship
 * beads could ever close.
 *
 ...
 *
 * - **Never asked, or the interval has elapsed.** The ordinary one.
 * - **The last ask left a backlog.** A burst that strands more than `OFF_BOARD_MAX` at
...
function offBoardDue(entry, now) {
  if (entry.offBoardMore === true) return true;
...
export async function sweepReleases(
  ...
  for (const card of board?.repos || []) {
    ...
    if (lookup && leftover.length && offBoardDue(entry, now)) {
      // Newest first, and capped — one `gh pr view` apiece on a clock. A tail longer than
      // this is reported rather than silently trimmed, and the next tick takes the next
      // slice: nothing is lost, it just takes a few ticks to drain a backlog of hundreds.
      const asking = leftover.slice(0, OFF_BOARD_MAX);
      const more = leftover.length > asking.length;
      if (more) {
        result.skipped.push(
          `${whereOn(card)}: ${leftover.length} ship beads are for pull requests off the board — asked about ${asking.length} this tick, the rest on the next`
        );
      }
      // Stamped before the call, not after, and `more` with it — see `offBoardDue`. A
      // lookup that throws has still spent the tick's ask, and one that left a backlog
      // must not have to wait out the interval to take the next slice.
      entry.offBoardAt = new Date(now).toISOString();
      entry.offBoardMore = more;
      dirty = true;
      try {
