...
async function settle(entry, { ready, ledger, cfg, ws, card, wsDeploys, now, ship, result }) {
  ...
  try {
    ...
    const rec = await ship(ws, queue, { numbers: ready.map((r) => r.number), key: keyOfCard(card), card });
    result.shipped.push({
      workspace: ws.name,
      key: keyOfCard(card),
      where: whereOn(card),
...
export async function sweepReleases(
  ...
  for (const card of board?.repos || []) {
    ...
    for (const row of merged) {
      ...
      result.filed.push({ workspace: ws.name, key: keyOfCard(card), where: whereOn(card), number: row.number, bead: id });
      dirty = true;
