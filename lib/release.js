 ...
 * lib/evidence.js's `deployment-record` entry names this number in its `disposal` prose,
 * and test/evidence.mjs fails if the two drift apart, so this is exported for that reason
 * and not because anything else here needs to see it.
 */
export const KEEP_DAYS = 45;

/**
 * How many off-board pull requests one repo is asked about in one tick — see the leftover
 * pass in `sweepReleases`, and bc-xl7n.108 for why there is one at all.
 *
 * A cap on the *rate*, never on the answer: the leftovers are sorted newest first, this
 * tick takes a slice, and what is left over is reported on `skipped` and taken by the
 * next tick — which starts *below* the one this tick ended on, because the slice moves
 * (`offBoardFrom`, and `offBoardCycle` for what happens when it runs off the end). That
 * distinction is the whole of the bug this exists to fix — a cap that silently dropped
 * the tail is how twelve board rows came to decide, for good, which ship beads could
 * ever close, and a cap that took the same top slice every tick forever would be the
 * same bug with a `skipped` line over it.
 *
 * Twenty because each one is a `gh pr view`: a burst big enough to strand a batch is a
 * dozen or two, so the ordinary backlog drains in one tick, and a tracker carrying years
 * of them costs a bounded handful of subprocesses per five minutes rather than hundreds
 * in one breath.
 */
const OFF_BOARD_MAX = 20;

/** And how often one repo may make that ask at all — see `offBoardDue`. */
const OFF_BOARD_EVERY_MS = 30 * 60000;
...
/**
 * The one key in the ledger that is not a workspace: when a sweep last got all the way
 * through. `$` because a workspace name comes from a config the user writes and is a
 * directory-ish word, so this cannot collide with one; `entryFor` never creates it and
 * `anyArmed` reads `armedAt` off it and finds none.
 *
 * It exists because of bc-68ou.8. The release sweep went roughly three hours filing
 * nothing while eight pull requests merged, and the only thing anyone could look at
 * afterwards was this file — which said the same thing a genuinely quiet morning says.
 * A watermark records what was *found*; nothing recorded that anybody had looked. So the
 * heartbeat is written on **every** completed pass and not only on a pass that changed
 * something, which is also what makes the file's own mtime mean "the sweep is alive".
 *
 * Durable rather than a variable in the poller on purpose: the gap this is for spanned
 * six daemon restarts, and a timestamp that resets at boot cannot see across one.
 */
export const SWEEP_KEY = '$sweep';

/** When a sweep last completed, in ms, or null if none ever has. */
export const lastSweptAt = (ledger = {}) => {
  const at = Date.parse(ledger?.[SWEEP_KEY]?.at || '');
  return Number.isFinite(at) ? at : null;
};

/* --------------------------------------------------- what the sweep says about itself */

/** How long a run of skipped sweeps goes before it says so again. */
export const SWEEP_NAG_MS = 30 * 60 * 1000;

/** How many intervals a gap has to be before it is worth a line of its own. */
export const SWEEP_LATE_FACTOR = 3;

/** A duration in the words a log line can use — "45 seconds", "12 minutes", "3.1 hours". */
...
/**
 * The release sweep's account of its own silence — the other half of bc-68ou.8.
 *
 * `sweepRelease` in lib/server.js used to `return` on `board.unavailable` and say
 * nothing at all, so a board that would not collect for three hours read, in the log,
 * exactly like three quiet hours. Every other outcome of a sweep gets a line; the two
 * that mean *no sweep happened* got none, which is backwards — those are the ones
 * nothing else in the system will ever mention.
 *
 * Three lines, and the shape is a storm rather than a tick, because `gh` being absent is
 * a legitimate permanent state on a machine that never installed it and a line every
 * five minutes forever would be the same silence by other means:
 *
 * - the **first** skip of a run, which is the news;
 * - a **nag** every `nagMs` while it goes on, so a long outage is still on the screen
 *   somebody scrolls to;
 * - the **recovery**, naming how long it was out and how many sweeps it cost, so the
 *   catch-up burst that follows reads as a catch-up rather than as a surprise.
 *
 * And on a sweep that did run, a line when the gap since the last completed one is more
 * than `lateFactor` intervals — which is the case a skip run cannot cover, because the
 * sweep can also be late for reasons this function never sees: a cycle ahead of it that
 * ran long, a daemon that restarted before its first interval elapsed, a Mac asleep.
 *
 * Stateful, so it is a factory rather than a function, and pure otherwise: it takes the
 * clock and hands back strings. That is the whole reason it lives here and not inside
 * `startPoller`, which is a thousand-line closure no test can reach into.
 */
export function sweepVoice({ nagMs = SWEEP_NAG_MS, lateFactor = SWEEP_LATE_FACTOR } = {}) {
  let outSince = 0;
  let outSweeps = 0;
  let naggedAt = 0;

  const oneLine = (v) => String(v ?? 'no reason given').split('\n')[0].trim() || 'no reason given';

  return {
    /** A sweep that could not collect the board. */
    skipped(reason, { at = Date.now() } = {}) {
      outSweeps += 1;
      if (!outSince) {
        outSince = at;
        naggedAt = at;
        return [
          `sweep skipped — the pull request board is unavailable (${oneLine(reason)}). ` +
            'Nothing will be filed, armed or shipped until it comes back.',
        ];
      }
      if (at - naggedAt < nagMs) return [];
      naggedAt = at;
      return [
        `sweep still skipping after ${sinceWords(at - outSince)} and ${outSweeps} attempts — ${oneLine(reason)}`,
      ];
    },

    /**
     * A sweep that ran. `since` is the ledger's gap in ms — null when no sweep has ever
     * completed, which is a first run and not news.
     */
    ran({ at = Date.now(), since = null, every = 0 } = {}) {
      if (outSince) {
        const lines = [
          `the pull request board is readable again after ${sinceWords(at - outSince)} — ` +
            `${outSweeps} sweep${outSweeps === 1 ? '' : 's'} skipped, and whatever merged in that time is being caught up now`,
        ];
        outSince = 0;
        outSweeps = 0;
        naggedAt = 0;
        // Deliberately instead of the lateness line below and not as well as it: the run
        // of skips just said how long the gap was and why, and a second sentence saying
        // the same number would read as two separate faults.
        return lines;
      }
      if (!Number.isFinite(since) || !(every > 0) || since <= every * lateFactor) return [];
      return [
        `${sinceWords(since)} since the last completed sweep, on a ${sinceWords(every)} cadence — ` +
          'anything that merged in that window did not deploy itself when it landed',
      ];
    },
  };
}
...
/**
 * May this repo ask GitHub about its off-board ship beads on this tick?
 *
 * The leftover pass is not free and its answer is not fast-moving. A ship bead that is
 * open because its merge is genuinely not live yet is a bead in the state it exists to
 * record, and it will be in that state on the next tick too — so asking about it every
 * five minutes for as long as it lasts is a `gh pr view` per merge per tick, forever, for
 * an answer that changes when somebody deploys.
 *
 * Hence a floor, and it is a floor on *asking*, never on closing: half an hour is inside
 * the "bounded, stated delay is fine, silence is not" that bc-xl7n.108 asked for, and it
 * is nothing beside the four days that bug actually cost. Two ways to be due:
 *
 * - **Never asked, or the interval has elapsed.** The ordinary one, and it starts a
 *   *cycle* — one pass over every leftover this repo has, `OFF_BOARD_MAX` at a time.
 * - **A cycle is part-way through.** A burst that strands more than `OFF_BOARD_MAX` at
 *   once should drain on consecutive ticks rather than twenty per half hour — the whole
 *   point of the cap is that it costs time and never a bead, and half an hour a slice
 *   would make that a much emptier promise than it sounds.
 *
 * The second one is bounded by the **cycle**, and that is the word that matters. A flag
 * meaning "the last ask left a backlog" would read the same and would not be the same
 * thing: only the *answers* could ever clear it, and a ship bead that is open is usually
 * one whose merge is not live yet — so the head of the list not closing is the ordinary
 * state, and a flag with no cursor under it would pin itself on and re-ask the same
 * twenty numbers every five minutes forever while the tail behind them was never asked
 * about at all. `offBoardCycle` is on until the cursor (`offBoardSlice`) runs off the
 * end of the list instead, so a cycle finishes whether or not anything closed, and the
 * floor starts applying again the moment it does.
 */
function offBoardDue(entry, now) {
  if (entry.offBoardCycle === true) return true;
  const asked = Date.parse(entry.offBoardAt || '');
  return !Number.isFinite(asked) || now - asked >= OFF_BOARD_EVERY_MS;
}

/**
 * Which slice of this repo's leftovers to ask about now — the moving window bc-xl7n.108's
 * review round found missing, and the whole of what makes "the rest on the next" true.
 *
 * `leftover` is newest-first. `entry.offBoardFrom` is the lowest number the last tick
 * asked about, so this one starts at the first number strictly below it; a cursor that
 * names nothing left in the list (everything under it closed, or the tracker moved) is
 * a cycle that has run out, and a cycle that has run out starts again from the top —
 * which is also where a cycle with no cursor starts.
 *
 * Returns `{ asking, more, from }`: the numbers, whether the cycle has further to go,
 * and the cursor to write for the next tick. A number that appears *above* the cursor
 * mid-cycle waits for the wrap, which is bounded by one cycle rather than by nothing.
 */
function offBoardSlice(entry, leftover) {
  const cursor = Number.isFinite(entry.offBoardFrom) ? entry.offBoardFrom : null;
  const at = cursor === null ? -1 : leftover.findIndex((n) => n < cursor);
  const start = at < 0 ? 0 : at;
  const asking = leftover.slice(start, start + OFF_BOARD_MAX);
  const more = start + asking.length < leftover.length;
  return { asking, more, from: more ? asking[asking.length - 1] : null };
}
 ...
 * has never heard of, and lib/underroot.js would draw a `nothing decided above this` pill on it for
...
/**
 * The label a *work* bead wears once the merge that carried it is live — bc-68ou.6.
 *
 * **Not `SHIP_LABEL`, and the distance between the two is the whole point.** `ship` marks
 * a bead this file *invented*, about a pull request, which closes itself and which
 * lib/bd.js excludes from every ready list because nobody should ever open a session on
 * one. `shipped` marks a bead somebody *wrote the code for*, and it is excluded from
 * nothing: a bead can carry it and still be open, still be in `bd ready`, still be work.
 * Filing them under one constant would make `--label ship` answer the wrong question and
 * `--exclude-label ship` hide real work, so they are two words on purpose.
 *
 * It lives here rather than in lib/shipbead.js because this is the only thing that writes
 * it, and it is exported because the tests read it and a reader grepping for the string
 * should land on this paragraph.
 */
export const SHIPPED_LABEL = 'shipped';

/**
 * Label every bead this merge was *for* — the answer to "is this bead's code live?".
 *
 * **The resolver is `row.beads`, and there is deliberately no new one here** (bc-68ou.4).
 * lib/beadref.js already answers "which bead is this pull request for" in tiers, verified
 * against the tracker, and lib/prboard.js has already asked it on the way to the board; a
 * second implementation is exactly what that file's header refuses, because then the
 * screen and the sweep would disagree about which bead a merge belongs to. So this costs
 * no tracker read at all — the row arrived carrying the answer.
 *
 * **Every bead in the winning tier, not just the first.** `beadOf` takes `beads[0]`
 * because a ship bead needs one parent and a bead can only have one; a label is a separate
 * fact about each bead, and the commit convention on this repo puts several ids before the
 * colon — `bc-68ou.2, bc-68ou.3: … (#293)` is one merge that shipped two beads. Labelling
 * only the first would leave the second reading as unshipped forever.
 *
 * **The label never touches status** (bc-68ou.5). lib/landed.js already closes the work
 * bead when the pull request merges, under three guards written out in its header, and a
 * deploy is weaker evidence than the merge that closer already acted on: it says the code
 * is live and says nothing about whether the work is right. So an open bead carrying
 * `shipped` is not a contradiction — it is live code over work somebody has not finished,
 * which is a state this board had no way to say before.
 *
 * Returns the ids it labelled, `[]` for a pull request that named no bead — which is
 * ordinary, and is a merge that labels nothing rather than a merge that guesses.
 */
async function markShipped(bd, ws, row) {
  const ids = [...new Set((row?.beads || []).map((b) => String(b?.id || '').trim()).filter(Boolean))];
  // One `--add-label` per bead. Adding a label a bead already carries is a no-op in bd, so
  // correctness never rests on it — but the caller still gates on the ledger, because a
  // no-op is still a Dolt write on a single-writer database, once per merge per sweep,
  // forever.
  for (const id of ids) await bd.update(ws, id, { addLabels: [SHIPPED_LABEL] });
  return ids;
}
 ...
 *
 * `lookup` is injected for the same reason and works the same way: it is
 * `offBoardRows(bd, ws, card, numbers, deploys)` in lib/prboard.js, which this file
 * cannot import because that file imports this one, and **a caller that passes no
 * `lookup` never asks about a pull request that is not on the board** — which is
 * exactly what every existing test does. See `OFF_BOARD_MAX` for what it is for.
 *
...
export async function sweepReleases(
  ...
  cfg,
  board,
  { owner = 'you', deploys = [], now = Date.now(), ship = null, lookup = null } = {}
) {
  // `since` is how long it had been since a sweep last completed — filled in at the end,
  // null on every path that returns before the ledger is stamped, which is a sweep that
  // did not happen and so has no gap to report.
  const result = { filed: [], closed: [], marked: [], watermarked: [], armed: [], extended: [], shipped: [], skipped: [], since: null, error: null };
  ...
  for (const card of board?.repos || []) {
    ...
    /**
     * And the ship beads whose pull request is *not* on the board — bc-xl7n.108.
     *
     * `open` was read from the tracker from the first day, but until now it was only ever
     * consulted by number, from inside the loop over the board's rows. That made the board
     * the work list for closing as well as for filing, and the board is trimmed to twelve
     * settled rows per repo for a screen (`RECENT_MAX`, lib/prboard.js). A merge pushed off
     * those twelve by later merges was never visited again — not on that tick and not on
     * any tick after it — and nothing said so. Nine of them sat open for four days after
     * 2026-08-17, when thirteen further merges landed inside the three minutes behind them.
     *
     * The leftovers are the join the other way round, and they are what makes the close
     * half tracker-driven: what is open here that the board did not account for.
     */
    const onBoard = new Set(merged.map((r) => r.number));
    const leftover = [...open.keys()].filter((n) => !onBoard.has(n)).sort((a, b) => b - a);
    let extra = [];
    if (lookup && leftover.length && offBoardDue(entry, now)) {
      // Newest first, capped, and taken from where the last tick stopped — one
      // `gh pr view` apiece on a clock. A tail longer than the cap is reported rather
      // than silently trimmed, and the next tick takes the numbers *below* these: nothing
      // is lost and nothing starves, it just takes a few ticks to walk a long backlog.
      const { asking, more, from } = offBoardSlice(entry, leftover);
      if (more) {
        result.skipped.push(
          `${whereOn(card)}: ${leftover.length} ship beads are for pull requests off the board — asked about #${asking[0]} down to #${asking[asking.length - 1]} this tick, the rest below them on the next`
        );
      }
      // Stamped before the call, not after, and the cursor with it — see `offBoardDue`.
      // A lookup that throws has still spent the tick's ask *and still moves the window*:
      // a slice this Mac could not get an answer for must not be the slice it retries
      // forever, which is the shape of the bug this whole pass exists to fix.
      entry.offBoardAt = new Date(now).toISOString();
      entry.offBoardCycle = more;
      entry.offBoardFrom = from;
      dirty = true;
      let answered = false;
      try {
        // `|| []` because the next line spreads it: an injector that hands back nothing
        // would otherwise throw a `TypeError` out of a function whose docstring promises
        // it throws none, past the `saveLedger` at the end — losing the heartbeat and
        // every entry this tick touched, for all six workspaces.
        extra = (await lookup(bd, ws, card, asking, wsDeploys)) || [];
        answered = true;
      } catch (err) {
        result.skipped.push(`${whereOn(card)}: could not ask GitHub about ${asking.length} off-board pull request(s) — ${String(err.message).split('\n')[0]}`);
      }
      // Numbers the lookup came back without a row for — not a pull request, closed
      // unmerged, a repository that moved. Each one's ship bead stays open, so it stays a
      // leftover and costs a slot on every cycle from here on; saying so once a cycle is
      // the difference between that and the four-day silence this bead is about. Cheap by
      // construction: the ordinary answer is that every number asked about came back.
      if (answered) {
        const got = new Set(extra.map((r) => r.number));
        const silent = asking.filter((n) => !got.has(n));
        if (silent.length) {
          result.skipped.push(
            `${whereOn(card)}: no merged pull request answers for ${silent.map((n) => `#${n}`).join(', ')} — their ship beads stay open`
          );
        }
      }
    }
    ...
    for (const row of [...merged, ...extra]) {
      ...
      if (state === true) {
        if (bead) {
          try {
            ...
            // `offBoard` rides along for the log and for nothing else. The failure this
            // pass fixes was silent — a ship bead that stopped being reachable and never
            // cost a line — so a close that only the leftover pass could have made is
            // worth saying out loud, on the day it happens rather than in a count later.
            result.closed.push({
              workspace: ws.name,
              key: keyOfCard(card),
              where: whereOn(card),
        ...
        /**
         * And the bead the work was *for* is labelled `shipped` — see `markShipped`.
         *
         * Here rather than beside the auto-ship in `settle`, because this is the moment a
         * merge is *live* and that is the moment the label is about. `settle` only knows
         * that a deploy was started, and only for the batches this process fired itself —
         * a queue drained by the Ship button, or by a deploy somebody ran by hand, would
         * never pass through it. `shippedState` is what both of those come back through.
         *
         * **Gated on the ledger, so a re-run over the same deploy writes nothing.** The
         * label is idempotent in effect but not in cost, and this sweep runs on a clock:
         * without the gate every merge still on the board would pay a Dolt write per tick
         * for the rest of its life on it. `shippedAt` is already the per-merge "settled"
         * mark and it is stamped just below, so first-time-live and every-time-after are
         * the same question this file already answers.
         *
         * **Forward-only, no backfill** — a merge that predates the watermark labels
         * nothing. The three weeks of history on a board at first sight all shipped long
         * ago, and stamping today's label across them would make `--label shipped` a
         * record of when beadcause started watching rather than of what is live.
         */
        const since = Date.parse(entry.since || '');
        const mergedAt = Date.parse(row.mergedAt || '');
        const fresh = Number.isFinite(since) && Number.isFinite(mergedAt) && mergedAt >= since;
        if (fresh && !rec?.shippedAt) {
          try {
            const marked = await markShipped(bd, ws, row);
            if (marked.length) {
              result.marked.push({ workspace: ws.name, key: keyOfCard(card), where: whereOn(card), number: row.number, beads: marked });
            }
          } catch (err) {
            // Retried for free, like the close above: nothing is stamped, so the next tick
            // finds this merge live and unmarked and asks again. A tracker mid-write must
            // not cost a bead its label permanently.
            result.skipped.push(
              `${whereOn(card)}: could not label the work behind #${row.number} \`${SHIPPED_LABEL}\` — ${String(err.message).split('\n')[0]}`
            );
            continue;
          }
        }
        ...
        if (rec && !rec.shippedAt) {
          ...
          // Merged and deployed between two sweeps, so nothing here ever filed a ship bead
          // for it and there is no record to stamp. Written now for one reason: without it
          // this row is unmarked on every future tick and pays for the label again each
          // time. `filedAt: null` is the honest shape — there was no bead to file by the
          // time anything saw it.
          entry.handled[key] = {
            bead: null,
            sha: row.mergeCommit || null,
            mergedAt: row.mergedAt || null,
            ...
            note: 'live before this sweep saw it merge',
          };
          dirty = true;
        }
        continue;
       ...
       * can never re-arm.
       *
       * And **never for a row that is not on the board**. A row off the board reached this
       * loop to have its ship bead closed and for nothing else: the Ship button's queue is
       * `owedFor`, which counts board rows, so a batch that quietly carried a merge the
       * queue never counted would deploy a number the screen it came from does not show.
       * The merge is not stranded by that — it ships with the next batch like anything
       * else, and this closes it when it does. (Filing needs no such guard: every row here
       * off the board has an open ship bead by construction, which the `rec || bead` test
       * below already refuses to file over.)
       */
      const verdict =
        canAutoShip && !row.offBoard && !rec?.autoShipAt
          ? await resolveAutoShip(bd, cfg, ws, row, { seen: opinions })
          : null;
  ...
  // How long it had been since a sweep last got this far — read *before* the heartbeat
  // is stamped over it, and handed back rather than logged, because this file has no
  // opinion about the cadence it should be measured against. See `sweepVoice`.
  const before = lastSweptAt(ledger);
  result.since = before === null ? null : Math.max(0, now - before);
  ledger[SWEEP_KEY] = { at: new Date(now).toISOString() };

  // Unconditional now, where it used to be `if (dirty)`. A pass that found nothing still
  // has one thing to record — that it happened — and that is the whole of bc-68ou.8: the
  // file a three-hour gap was diagnosed from could not distinguish a sweep that found
  // nothing from a sweep that never ran. The write is one small atomic JSON per sweep,
  // which is per five minutes at the ordinary cadence.
  try {
    saveLedger(ledger);
  } catch (err) {
    ...
    result.error = dirty
      ? `filed, but ${LEDGER_PATH} could not be written — ${err.message}`
      : `${LEDGER_PATH} could not be written — ${err.message}`;
  }
  return result;
}
