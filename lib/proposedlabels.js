 ...
 * - **`filed-while:<bead>`** (lib/filing.js, bc-xl7n.76.1) — the filer stamp: which bead
 *   was being worked when an agent filed this one, unattended. A card carries a person
 *   at the other end of it, not an agent mid-session, so one arriving already stamped
 *   would misattribute it to whichever epic the value happened to name — exactly the
 *   thing this feature exists to stop doing by accident.
 *
...
import { FILED_LABEL, FILED_WHILE_PREFIX } from './filing.js';
...
import { CTX_PREFIX, isCtxLabel } from './sessiontokens.js';
...
export const DAEMON_ONLY = [
  ...
  },
  {
    label: `${CTX_PREFIX}…`,
    test: (l) => isCtxLabel(l),
    why: 'that records whether a finished session fitted the context window it was routed into, and this bead has never run — set complexity: instead, which is the tier that picks the window',
  },
  {
  ...
  {
    label: `${FILED_WHILE_PREFIX}…`,
    test: (l) => hasPrefix(l, FILED_WHILE_PREFIX),
    why: 'that records which bead an agent was working when it filed this one unattended, and a card is a person filing it — the advocate console would credit an epic that never produced this',
  },
];
