 ...
 * kind, and the whole graph contained **two** see-also edges — one of them drawn by hand
 * days earlier, the other minutes before this was started. "The same defect as bc-767a", "see also bc-rcrt", "sits in
 * bc-42ow's neighbourhood" are all real relationships, and none of them was reachable
 * by `bd show`, `bd dep tree`, the graph page or a dispatch brief. So every session
 * rebuilt the neighbourhood by reading descriptions, and mostly did not bother.
 *
 ...
 * because the single edge that existed before this was created by hand years earlier
 * with the other spelling. Left alone, the first sweep would have drawn 841 edges that
 * the phone rendered as "Waits on", which is the one thing a loose see-also must never
 * say. Both spellings are accepted everywhere; `RELATED_EDGE` is the one that is
 * written.
 */
