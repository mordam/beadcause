...
/**
 * The turn a conversation wakes up to when its window disappeared.
 *
 ...
 * a window was closed by hand, killed, or lost with its terminal — nothing was decided,
 * nothing is waiting, and the only news is that time passed. Handing this agent
 * `resumePrompt` would open with "Adam answered", and it would then go looking for a
 ...
 *    again on a new one.
 */
export function interruptedPrompt({ owner = 'Adam', bead = null, parkedAt = null } = {}) {
  const who = owner || 'Adam';
  const ago = parkedAt ? agoPhrase(parkedAt) : '';
  return [
