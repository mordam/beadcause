...
import { isHeld as isUnendorsed } from './endorse.js';
import { isAssigned, isCrash, isPaused } from './epicadvocate.js';
import { isEpic, isRoot, ownedByMe, ownerLabel, ownersOn } from './ownership.js';
...
/* ------------------------------------------------------------------ whose beads they are */

/**
 * The beads a person is asked about the moment a bead-space is added or linked — bc-9i62w.
 *
 * A bead-space arriving on this Mac is the one moment its roots are all new to you at once,
 * and until this existed nothing asked the question that makes them *yours*: ownership is
 * the `owner:<handle>` label (lib/ownership.js), a workspace nobody has labelled draws an
 * empty board (bc-njfui), and an Epic Advocate will not open on a root nobody owns
 * (`wantsAdvocate`). So a freshly linked tracker full of epics was a tracker the app could
 * show you and could do nothing with until somebody found the sheet on each one.
 *
 * **The roots, and only the roots** — an epic at any priority, or a P0, open, and not a
 * crash the app filed on itself. That is `rootsOf` less the crash beads, and the crash beads
 * are left out for `wantsAdvocate`'s reason: a stack trace is not an epic, and a list you
 * are picking your week from should not offer one.
 *
 * Each row says what it already is (`mine`, `advocate`) so the dialog draws those as done
 * rather than as choices, and `noAdvocate` carries the sentence for the three states an
 * advocate would be refused on that the labels already show. Owning one of those is still
 * fine — being answerable for a paused epic is ordinary — so it is the advocate half alone
 * that is withheld, and withheld with the reason rather than drawn and then refused.
 *
 * Yours first, then by priority, then by id numerically: the same first term `/api/roots`
 * sorts on, and a stable order so the list does not reshuffle under a thumb between taps.
 */
export function ownershipOffer(beads, cfg = {}) {
  const rows = beads instanceof Map ? [...beads.values()] : Array.isArray(beads) ? beads : [];
  return rows
    .filter((b) => b?.id && isRoot(b) && String(b.status || '').trim().toLowerCase() !== 'closed' && !isCrash(b))
    .map((b) => ({
      id: String(b.id),
      ...
      advocate: isAssigned(b),
      noAdvocate: isUnendorsed(b)
        ? `${b.id} is unendorsed — endorse it before an advocate plans it`
        : isSuperseded(b)
          ? `${b.id} is superseded — the work is somewhere else`
          : isPaused(b)
            ? `${b.id}'s advocate is paused — resume it from its card on the console`
            : null,
    }))
    .sort(
      (x, y) =>
        Number(y.mine) - Number(x.mine) ||
        Number(x.priority ?? 9) - Number(y.priority ?? 9) ||
        x.id.localeCompare(y.id, 'en', { numeric: true })
    );
}

/**
 * What the dialog asked for → the writes to make, or a sentence saying why none will be.
 *
 * `{problem, steps}`, and a problem means **nothing** is written: the picks are checked
 * whole before the first label goes on, because a half-applied answer to one question is a
 * dialog that reports success and failure about the same press.
 *
 * - **Zero picks is an answer**, and the ordinary one. It is `steps: []` with no problem,
 *   never a refusal — the question is "which of these", and "none" is on the list.
 * - **An advocate implies ownership.** `wantsAdvocate` refuses a root nobody owns, and an
 *   advocate answers to somebody; a pick of the advocate alone would otherwise be a window
 *   refused at the door a moment after the dialog said yes. So an advocate pick is also an
 *   own pick, rather than a refusal the person has to read and re-press.
 * - **A Mac that does not know who it is writes nothing** — lib/ownership.js's guarantee,
 *   kept here as a sentence rather than as a silent no-op, because a dialog that said it
 *   took three epics and labelled none of them would be the worst version of this.
 * - **Every id must be in the offer.** The offer is what was drawn, re-read on the write,
 *   so an id that is not in it is a root that closed, was superseded or never was one
 *   since the list came up — and taking ownership of that is not what anybody pressed.
 * - **Already true is skipped**, not re-done: a root already yours gets no label write, and
 *   one already carrying `advocate-assigned` does not get a second window. One advocate
 *   per epic, never two, is `POST /api/bead/advocate`'s rule and it is this one's too.
 */
export function ownershipPicks(offer = [], { own = [], advocate = [] } = {}, cfg = {}) {
  const ids = (list) => [...new Set((Array.isArray(list) ? list : []).map((x) => String(x ?? '').trim()).filter(Boolean))];
  const wantAdvocate = ids(advocate);
  const wantOwn = [...new Set([...ids(own), ...wantAdvocate])];
  if (!wantOwn.length) return { problem: null, steps: [] };

  if (!ownerLabel(meHandles(cfg)[0])) {
    return {
      problem: 'this Mac does not know who you are — set `me` on the config screen, or ownership would be written to nobody',
      steps: [],
    };
  }
  const byId = new Map(offer.map((row) => [row.id, row]));
  const gone = wantOwn.filter((id) => !byId.has(id));
  if (gone.length) {
    return {
      problem: `${gone.join(', ')} ${gone.length === 1 ? 'is not an open epic or P0' : 'are not open epics or P0s'} in this bead-space any more`,
      steps: [],
    };
  }
  const withheld = wantAdvocate.map((id) => byId.get(id).noAdvocate).filter(Boolean);
  if (withheld.length) return { problem: withheld.join('; '), steps: [] };

  const steps = wantOwn
    .map((id) => {
      const row = byId.get(id);
      return { id, own: !row.mine, advocate: wantAdvocate.includes(id) && !row.advocate };
    })
    .filter((step) => step.own || step.advocate);
  return { problem: null, steps };
}
