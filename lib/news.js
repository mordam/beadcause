 ...
 * - `released` — a deploy succeeded, or ended `unconfirmed`, which is the ordinary
 *                ending of one that restarts this daemon. See [deployVoice].
 * - `stuck`    — a deploy failed or was lost, or a tracker is not syncing. Carries
 *                `state: 'stuck' | 'clear'`, because this one is a *state* rather than
 *                an arrival: the card that says it goes away when the state does, and
 *                nothing else here is ever taken back.
...
// The table that says which of the five classes each of these speaks in — and, for the
// one that is a judgement rather than a name, which side a deploy status falls on.
import { deployVoice, STUCK } from './voices.js';
...
/**
 * How many beads lib/landed.js's sweep may close in one tick before they stop being
 * one-line news and become a list.
 *
 * Below this, each landing is still readable as its own arrival — a merge and the bead
 * it closed, one blip each, which is exactly what `landedEvent` above is for. At or
 * above it, that stops being true: a Mac that reconnects after a week away can close a
 * dozen beads in the same sweep (lib/landed.js's own header names twenty), and a dozen
 * identical-looking "#N landed" cards stacked in the shade is not twelve pieces of
 * news, it is one piece of news — *the sweep caught up* — read twelve times. bc-
 * ka5y.15.7's own acceptance says so directly: "a sweep that finds many at once does
 * not post one card each."
 */
const LANDED_MANY_AT = 3;

/**
 * The events lib/landed.js's sweep should put on the bus for one tick's worth of
 * closes — one `landedEvent` each below [LANDED_MANY_AT], or a single summary at or
 * above it.
 *
 * Deliberately still `type: 'landed'` for the summary rather than a fifth type: the
 * phone already knows how to draw and sound a `landed` card (Notifications.news,
 * voiceFor), and a sweep that caught up on a dozen merges is not a different *kind* of
 * news, it is a bigger helping of the same kind. Every row is expected to carry a
 * `workspace` of its own, unlike the single-bead shape `landedEvent` takes above — the
 * caller in lib/advocate.js attaches one before handing the sweep's `closed` rows here,
 * because lib/landed.js's own rows do not carry it.
 */
export function landedNewsEvents(closed, { quiet = false } = {}) {
  const rows = (closed || []).filter(Boolean);
  if (!rows.length) return [];
  if (rows.length < LANDED_MANY_AT) return rows.map((r) => landedEvent(r, { quiet }));
  const workspace = rows[0].workspace || null;
  const numbers = rows.map((r) => Number(r.number)).filter((n) => Number.isInteger(n) && n > 0);
  const highest = numbers.length ? Math.max(...numbers) : 0;
  const lines = rows.map((r) => `#${r.number} ${r.title || r.bead || ''}`.trim().slice(0, 200));
  return [
    {
      type: 'landed',
      // Keyed on the highest PR number in the batch rather than on any one bead, for
      // the reason the singular key's own comment gives: a card still in the shade as
      // a delivery card must not collide with this. A PR number the tracker has not
      // seen before is exactly what makes this batch new, so it also keeps two
      // sweeps' summaries from colliding with each other.
      key: `news/landed/${workspace}/sweep-${highest}`,
      workspace,
      id: null,
      number: null,
      title: `${rows.length} pull requests merged on GitHub`,
      text: lines.join('\n').slice(0, 2000),
      quiet,
    },
  ];
 ...
 *
 * **Which side `unconfirmed` falls on is [deployVoice]'s to say, and it changed.** It
 * counted as stuck until bc-ka5y.15.5, on the argument that "we ran it and nothing
 * outlived it to check" is not a release you can rely on. That sentence is still what the
 * card says; what was wrong was the sound. `sweepDeploys` writes that word only for a
 * deploy that restarts beadcause, so it is the ordinary ending of every deploy this repo
 * makes of itself — and class 2 meant the commonest release here was also the loudest
 * noise the phone can make. The whole argument, and the corroboration from lib/queues.js,
 * is in lib/voices.js.
 */
export function deployEvent(rec, { quiet = false } = {}) {
  const voice = deployVoice(rec.status);
  const news = voice !== STUCK;
  const word = { ok: 'deployed', failed: 'deploy failed', lost: 'deploy lost', unconfirmed: 'deploy unconfirmed' }[rec.status] || `deploy ${rec.status}`;
  return {
    type: news ? 'released' : 'stuck',
    key: news ? `news/released/${rec.workspace || ''}/${rec.id || ''}` : deployStuckKey(rec),
    workspace: rec.workspace || null,
    id: rec.id || null,
    ...(news ? {} : { state: 'stuck', source: 'deploy' }),
 ...
 */
/**
 * A tracker that keeps breaking and coming back — bc-y3qk.4.
 *
 * Its own event rather than a third title inside `syncStuckEvent`, and the reason is
 * what it is *for*. The other two are told about an incident. This one is told **instead
 * of** the incidents: it is the last thing said about this workspace until it holds one
 * way for an hour, so it has to carry what a person needs to decide whether to go and
 * look — the count, the window, and the fact that the silence after it is deliberate.
 *
 * `key: 'stuck/sync'` like the other two, which is the load-bearing detail. It replaces
 * the row already on the card rather than stacking a third beside it, and it means the
 * ordinary `syncClearEvent` still takes it away when the workspace settles — one card
 * for the tracker, three things it can say, one way to make it go.
 *
 * `state: 'stuck'` because the card must stay up. A flapping tracker is not in sync in
 * any sense a person can use: half the time it is failing outright, and the other half
 * it is a minute away from failing again. Filing it as clear would put the app back to
 * drawing a calm screen about a graph nobody can trust, which is the one thing this
 * whole area exists to stop.
 */
export function syncFlappingEvent(rows) {
  const names = rows.map((r) => r.workspace).join(', ');
  const plural = rows.length === 1 ? 'it' : 'they';
  return {
    type: 'stuck',
    key: 'stuck/sync',
    ...
    state: 'stuck',
    source: 'sync',
    title: `tracker FLAPPING · ${names}`,
    text: [
      `Not one outage and not a working sync: ${names} has been breaking and coming back all hour, so neither sentence is true for long enough to be worth sending. Beads written on either machine may be minutes out of date in either direction.`,
      '',
      ...rows.map((r) => `${r.workspace} — ${r.flaps} transitions in the last hour${r.error ? `, latest: ${r.error}` : ''}`),
      '',
      `This is the last thing you will hear about ${plural} until ${plural} hold${rows.length === 1 ? 's' : ''} one way for an hour — or until the failure stops being the kind that retries, which is said separately and is worth reading.`,
    ].join('\n'),
    quiet: false,
  };
}
