 ...
 */
export async function gatherMerges(bd, cfg, { refresh = false, ceilingMs = cache.CEILING_MS, waitMs = ceilingMs } = {}) {
  let got;
  try {
    // `ceilingMs` is the slot's own ceiling — how long the sweep may hold the
    // single-flight entry — and the suite shrinks it; the daemon never passes it, so a
    // genuinely cold key still gives the sweep the full 150s to land in.
    //
    // `waitMs` is how long THIS call is willing to wait on it — bc-19vt.1. `/api/queues`
    // (lib/server.js) passes `cache.WAIT_MS`: a phone gains nothing from inheriting the
    // slot's whole ceiling, since `errors[]` below already has somewhere honest to say
    // "not yet" and the abandoned sweep keeps running toward `ceilingMs` regardless.
    got = await cache.read(MERGES_KEY, () => sweepMerges(bd, cfg), { freshMs: MERGES_MS, refresh, ceilingMs, waitMs });
  } catch (err) {
    ...
    const why =
      `the merge queue did not answer within ${Math.round(err.waitMs / 1000)}s — ` +
