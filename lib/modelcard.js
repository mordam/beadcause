...
import { pressuresSeen } from './sessiontokens.js';
 ...
 *   - `pressures` — whether the finished sessions fitted the window their tier routed them
 *     into, off `ctx:<verdict>` labels (bc-nc6o.8): `['over']` for a run that had to
 *     compact, `['tight']` for one that fitted with little to spare, `['fit']` for a
 *     comfortable one, `[]` for the beads nothing has measured, which is nearly all of
 *     them. Two entries is a bead worked twice under different verdicts, and both are kept
 *     for the reason both `ran:` models are: "it overflowed" and "it fitted once and
 *     overflowed later" are different sentences, and only the second one says the tier has
 *     gone stale rather than having always been wrong.
 *
 *     **Only the sheet draws this.** The chip has room for the model and the tier and
 *     nothing else, and a fourth token on it would push the two facts it exists for off
 *     the card; the sheet is the surface with room to explain itself. Derived here anyway,
 *     rather than in the sheet, because it is the same rule twice over: one derivation, so
 *     the two surfaces cannot come to disagree about a bead if the chip ever grows room.
 */
