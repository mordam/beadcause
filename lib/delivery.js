...
/**
 * Which of these open cards are already asking about this pull request?
 *
 ...
 *
 * So a delivery now finds the cards already open for its own pull request and closes
 * them before it files or merges anything. This is the finding half: given the rows
 * `bd list --label pr-delivery` returned, hand back the ones whose `beadpr` block
 * names this same request.
 *
 * Matched on the pull request number **and** the repo, because a number alone is only
 * unique within a repo and a workspace is not guaranteed to be one repo forever. A
 * card that names no repo at all — hand-written, or from before the field existed —
 * matches on the number, which is the older behaviour and the best that can be done
 * with what it carries.
 */
export function cardsForRequest(rows, { repo = '', number } = {}) {
  ...
  return (rows || [])
    ...
    .filter(({ d }) => d && !d.error && Number(d.number) === want)
    .filter(({ d }) => {
      const slug = slugOf(d);
      return !slug || !wantSlug || slug === wantSlug;
    })
    .map(({ row, d }) => ({ id: row.id, bead: d.bead || null, title: row.title || '' }));
}
