 ...
 * for two of the three it is the entire content of the answer: for `CHANGES:` it is
 ...
 */
export function deliveryBody(d, { context = '', refused = '', asked = false } = {}) {
  const parts = [];

  const opening = refused
    ? `**${d.repo || d.workspace}** — the ${d.workspace} worker finished **${d.bead}**, opened a pull request and ` +
      `tried to merge it. **It could not:** ${refused} So it is yours: answering **Merge** ${d.method}-merges it ` +
    ...
    : asked
      ? `**${d.repo || d.workspace}** — the ${d.workspace} worker finished **${d.bead}** and opened a pull request. ` +
        ...
        `below. Answering **Merge** ${d.method}-merges it and closes ${d.bead}, **Request changes** sends your note ` +
