 ...
 * outrank a worker's own hesitation — and `edit` outranks `asked` because with the hold
 * on there was no hesitation to credit, the worker could not have merged this either
 * way. `review` sits above `refused` in the chain below rather than beneath it, and that
 * is not a change of mind about the precedence: a review escalation arrives *carrying* a
 * refusal sentence — the reviewer's, in `refused` — so the test for it has to come first
 * or its own sentence could never be reached.
 */
export function deliveryBody(
  d,
  { context = '', refused = '', approval = false, review = false, edit = false, asked = false, ship = '' } = {}
) {
  const parts = [];
