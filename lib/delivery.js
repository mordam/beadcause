 ...
 *
 * The first paragraph is built from *why this card exists*, and there are four
 ...
 * know when one of these arrives is which of the four happened:
 *
 ...
 * - **`review`** — the ReviewAdvocate read the diff and it did not end in an approval:
 *   it refused outright, or it and the worker did not agree inside `MAX_REVIEW_ROUNDS`
 *   (lib/mergebead.js). Nothing about the *merge* went wrong and the checks may well be
 *   green, so it must not borrow the refusal wording — what is being handed over is a
 *   disagreement about the code, and the sentence has to say whose and about what, or the
 *   obvious answer is to press Merge over the objection nobody read.
 * - **`approval`** — this workspace's space asks for an approving review before
 *   anything merges (`prPolicyFor` in lib/spaces.js), and the pull request has not
 *   got one. Nothing went wrong and nothing refused: the checks are green and the
 *   worker stopped on purpose, which is exactly why it needs a sentence of its own.
 *   Without one it reads as the fourth case below — *auto-merge is off* — which is a
 *   fact about the config rather than about this pull request, and would leave you
 *   hunting for a switch that is already set the way you want it.
 * - **`edit`** — this bead was typed into the running app with edit mode on, and one of
 *   those is merged by the person who asked for it whatever the space says (bc-p49x.4,
 *   and `fromEditMode` in lib/editwork.js). Its own sentence for the same reason
 *   `approval` has one: without it the card reads as *auto-merge is off here*, which is
 *   a fact about the config and sends you looking for a switch — where this is a fact
 *   about the bead, and the answer to it is the review nobody else can do.
 * - **`gate`** — this bead is labelled `gate` or `needs-approval` (lib/approval.js), and
 *   since bc-bmry.8 the worker did not put it on the merge queue and could not have
 *   merged it either way, whatever `--review` said. Its own sentence for the reason
 *   `edit` has one: without it this reads as `asked` — a worker's own hesitation —
 *   where there was none to have, or as a refusal, where nothing was attempted.
 ...
 *   checks.
 * - **None of them** — auto-merge is off for this space, so every delivery is a
 *   question and this one is not special. The original wording, kept verbatim.
 *
 * The order above is the precedence, and it is the order of how much each one tells
 * you: a refusal that happened outranks a policy that stopped one happening, all three
 * outrank a worker's own hesitation — and `edit` and `gate` outrank `asked` because with
 * either hold on there was no hesitation to credit, the worker could not have merged this
 * either way. `review` sits above `refused` in the chain below rather than beneath it, and
 * that is not a change of mind about the precedence: a review escalation arrives
 * *carrying* a refusal sentence — the reviewer's, in `refused` — so the test for it has to
 * come first or its own sentence could never be reached.
 */
export function deliveryBody(
  d,
  { context = '', refused = '', approval = false, review = false, edit = false, gate = '', asked = false, ship = '' } = {}
) {
  const parts = [];

  const opening = review
    ? `**${d.repo || d.workspace}** — the ${d.workspace} worker finished **${d.bead}** and opened a pull request, ` +
      `and the reviewer read it. **It did not end in an approval:** ${refused} The review loop stops here rather ` +
      `than asking again, so it is yours: answering **Merge** ${mergesIt(d.method)} and closes ${d.bead} over the ` +
      `objection, **Request changes** sends your note back to a session on the same branch.`
    : refused
    ...
    : approval
      ? `**${d.repo || d.workspace}** — the ${d.workspace} worker finished **${d.bead}**, opened a pull request and ` +
        `its checks are green. **It is waiting on an approving review:** this space asks for one before anything ` +
        `merges, so the worker stopped rather than land it unseen. Your **Merge** is that review — it ` +
        `${mergesIt(d.method)} and closes ${d.bead}, **Request changes** sends your note back to a session on the ` +
        `same branch.`
      : edit
        ? `**${d.repo || d.workspace}** — the ${d.workspace} worker finished **${d.bead}**, which is something you ` +
          `typed into the app itself with edit mode on. **An in-app edit is merged by the person who asked for it:** ` +
          `you said this to a screen, and looking at what came back is the whole of the review. Answering **Merge** ` +
          `${mergesIt(d.method)} and closes ${d.bead}, **Request changes** sends your note back to a session on the ` +
          `same branch.`
        : gate
          ? `**${d.repo || d.workspace}** — the ${d.workspace} worker finished **${d.bead}**, which is labelled ` +
            `\`${gate}\`. **${
              gate === 'needs-approval'
                ? 'It is waiting on an approval, and a green pull request is not that approval'
                : 'A gate closes when it is met, and a pull request merging is no evidence about that'
            }:** the worker did not put this on the merge queue and could not have merged it either way. Answering ` +
            `**Merge** ${mergesIt(d.method)} and closes ${d.bead}, **Request changes** sends your note back to a ` +
          ...
          : asked
            ...
            // openings the wording fix missed, and now the one it matters most on: with
            // auto-merge off, every delivery in that space comes through here.
  ...
  parts.push(['_What merging would act on, in the form the server reads it:_', '', deliveryBlock(d)].join('\n'));

  return parts.join('\n\n');
}

/**
 * The `beadpr` block on its own, for anything that needs the same fact in a body of its
 * own shape.
 *
 * Exported since bc-r941.1, and exported rather than copied for the reason the block
 * exists at all: a merge-bead (lib/mergebead.js) carries exactly this, is parsed by
 * exactly `parseDelivery`, and turns into exactly the card above when the merge is
 * refused. Two serialisers for one block is two places for the field list to drift, and
 * the drift would show up as a card whose Merge button acts on a pull request the bead
 * no longer names.
 */
export function deliveryBlock(d) {
  return ['```beadpr', YAML.stringify(strip(d)).trimEnd(), '```'].join('\n');
}
...
/**
 * The repo a card is about, from its own field or out of the pull request URL.
 *
 * Exported since bc-l853.6, because it is now the only thing on a card that says *which
 * checkout on this Mac* the merge has to run in: a workspace of forty repos has no single
 * directory, and `gh pr merge` in the wrong one merges a different repo's pull request of
 * the same number. Lower-cased, as the matcher below needs it — a GitHub slug is
 * case-insensitive and a card written by hand may not match the remote's capitalisation.
 */
export const slugOf = (d) =>
  String(d?.repo || (String(d?.url || '').match(/github\.com\/([^/]+\/[^/]+)\/pull\//) || [])[1] || '')
    .trim()
    .toLowerCase();

/**
 * Which of these open cards is this delivery about to ask again?
 *
 * One card per attempt is deliberate — a delivery question closes on all four of its
 * answers and the next push files a fresh one, so the inbox carries one card per
 * attempt rather than one card that quietly changes meaning under you. What that
 * assumed is that every attempt follows an *answer*. A re-delivery nobody answered
 * first breaks it: bc-ec6 was delivered three times in twenty minutes and filed three
 * cards, two of which were open together. The inbox carried the same question twice,
 * with identical titles and identical bodies, and it was answered twice a minute
 * apart — both answers claiming to have closed a work bead that neither could close,
 * because each card was a blocker on the other's close.
 *
 * So a delivery now finds the cards already open for the work it is delivering and
 * closes them before it files or merges anything. This is the finding half: given the
 * rows `bd list --label pr-delivery` returned, hand back the ones this delivery
 * supersedes.
 *
 * **Two ways to be the same question, and both are needed.**
 *
 * The **pull request** — number *and* repo, because a number alone is only unique
 * within a repo and a workspace is not guaranteed to be one repo forever. A card that
 * names no repo at all — hand-written, or from before the field existed — matches on
 * the number, which is the older behaviour and the best that can be done with what it
 * carries. This is the re-delivery of an unanswered card, and it is the common one.
 *
 * The **work bead**, which the pull request alone misses. A card is not really asking
 * "should #25 merge", it is asking "should the work for bc-ec6 land", and a session
 * that abandoned its branch and delivered the same bead on a new one leaves the first
 * card open beside the second — two cards, one bead, and the older one pointing at a
 * pull request nobody is going to merge. That is the same pile in a shape matching on
 * the number cannot see, so the bead is matched too, and the caller's close reason
 * names the older request rather than pretending the two cards were about one.
 *
 * Ids are compared case-insensitively and nothing is matched on wording, because the
 * wording is exactly what a re-delivery changes.
 */
export function cardsForDelivery(rows, { repo = '', number, bead = null } = {}) {
  ...
  // Neither half known is not "match everything" — it is a caller with nothing to
  // compare, and closing every open card on that basis is the worst thing here could do.
  ...
  return (rows || [])
    .filter((r) => r && r.id && r.status !== 'closed')
    .map((r) => ({ row: r, d: parseDelivery([r.description, r.design, r.notes].filter(Boolean).join('\n\n')) }))
    .filter(({ d }) => d && !d.error && (sameRequest(d) || sameBead(d)))
    // `number` comes back with the id because a card matched on its bead may be about
    // a *different* pull request, and saying which is the difference between a close
    // reason that explains itself and one that looks like a mistake.
