 ...
 *
 * The three are not a scale, they are three different asks — and since bc-9ntye only the
 * first of them holds anything:
 *
 * - `blocking` — this branch must not merge as it stands. It is a promise rather than an
 *   emphasis, it is the *only* thing that holds a merge, and the brief spends a paragraph
 *   on what may and may not be one, because a reviewer that blocks on taste jams a queue
 *   dozens of pull requests deep and buys nothing.
 * - `suggestion` — worth doing, and never worth holding a merge for. It does not come back
 *   to the worker as a round either: the branch merges and the suggestion is filed as work
 *   of its own, to be scheduled like anything else.
 ...
 *   favour of a wrong `blocking`. It is filed the same way a suggestion is.
 *
 * **Which is why the brief's wording is load-bearing rather than cosmetic.** The two ways
 * this rule fails are opposite: a reviewer that goes on calling taste `blocking` jams the
 * queue exactly as before, and one that stops reaching for it on a real bug ships the bug,
 * because nothing further down this path reads the diff.
 */
 ...
 * review comment is here, and which kind of it actually holds the branch**. An agent asked
 * to review a diff will find something to say about every hunk of it — that is what it is
 * good at. Under bc-9ntye only a `blocking` comment holds a merge and everything else is
 * filed as work, so the brief has to carry both halves of that at once: the veto is narrow,
 * and it is still a veto. A reviewer told only the first half stops using `blocking` on
 * real bugs; one told only the second goes on holding branches for taste.
 *
 * The severity vocabulary is the mechanism; the paragraphs below are what makes it mean
 * something.
 */
export function reviewAdvocatePrompt(workspace, issue, spec, state = {}, extra = {}) {
  ...
  if (last) {
    lines.push(
      ...
      'There is a second round at all because a `blocking` comment stood — that is the only thing that sends',
      'a branch back here. Whatever else you raised is already filed as work and is not yours to re-argue.',
      'Take each answer on its merits:',
      '',
      ...answered.map((c) => {
        /**
         * Two comment shapes reach this line and only one of them was in view when it was
         * written — bc-36xx.5, which is the bead that first handed this brief real state.
         *
         * `checkVerdict` above normalises a *verdict's* comments to `{file, what, severity}`.
         * What a later round is handed is the **block on the merge-bead** (`reviewState`,
         * lib/mergebead.js), where the same comment is `{path, body, answer, note}` — and
         * `answer` there is one of `changed`/`clarify`/`declined` with the worker's prose
         * beside it in `note`, not the prose itself. Rendered from `c.what` alone, every
         * second-round brief read `- **c1** (undefined) — you said:` with nothing after it:
         * a reviewer asked to weigh answers it had not been shown, which is exactly the
         * round this branch exists to make worth having.
         *
         * Read from either rather than migrating one onto the other, because both are real:
         * the verdict shape is what this module writes and parses, and the block shape is
         * what survives between two windows. Severity is optional for the same reason — the
         * block does not carry one yet (bc-36xx.18), and a brief that printed `undefined`
         * for it was how that showed up.
         */
        const file = clean(c.file) || clean(c.path);
        const at = file ? ` \`${file}${c.line ? `:${c.line}` : ''}\`` : '';
        const severity = clean(c.severity);
        const what = clean(c.what) || clean(c.body) || 'the comment itself is no longer on the bead';
        const answer = [clean(c.answer), clean(c.note)].filter(Boolean).join(' — ') || 'no answer recorded';
        return `- **${c.id}**${severity ? ` (${severity})` : ''}${at} — you said: ${what}\n  - The worker: ${answer}`;
      }),
      '',
      ...
      '- **Nothing you write this round holds the branch unless it is `blocking`.** A `suggestion` you add',
      '  here is filed as work when the branch merges, exactly as one raised in round one is. It is a real',
      '  thing to say and it is not a way to keep the argument going.'
    );
  } else {
    lines.push(
      '',
      '**Your finding step is `/code-review`.** Run it over this diff before you write anything — it hunts',
      'bugs, reuse and simplification issues the way a reviewer here should, and it is not a thin wrapper',
      'you skip: it is the thing that finds most of what you will raise. Treat what it returns as findings',
      'to triage, not as the review itself — it has no notion of `bd show` or what this bead was asked for,',
      'so its findings become comments in the vocabulary below rather than going straight into your verdict.',
      '',
      '**Only a `blocking` comment holds this branch. Everything else you write becomes work rather than',
      'a round.** That is the rule here and it is probably not the one you arrived with: a verdict carrying',
      'nothing but suggestions and questions merges on the sweep that reads it, and those comments are filed',
      'as a follow-up bead — one child per comment, carrying your file, line, severity and words — which gets',
      'scheduled like any other work. Nothing you say is lost by not holding the branch for it.',
      '',
      '**So say what you mean in the severity, because that is the only place it counts.** A suggestion is',
      'not a soft `blocking` and withholding your approval is not one either; the one sentence that stops',
      'this merge is a comment you marked `blocking`. Both ways of getting that wrong are expensive and they',
      'are opposite. Reaching for `blocking` to be sure of being heard holds a branch that dozens of others',
      'are queued behind, and you no longer need to — the point survives you either way, as a bead. Declining',
      'to use it on something real because it feels heavy ships that thing: you are the only party in this',
      'path that reads the diff.',
      '',
      '**What a good review comment is here.** You did not write this code and you are not its author’s',
      'editor. A `blocking` comment costs this pull request a round and everything behind it a review slot;',
      'every other comment costs somebody a session to work. Neither is free, so the bar is not “could this',
      'be better” — for `blocking` it is “would I hold this branch for it”, and for the rest it is “is this',
      'worth somebody’s next session”.',
      '',
      ...
      '- **It is about this diff.** Something wrong in the code the change landed next to is not this pull',
      '  request’s problem — say so in one line at most and move on. A review that widens into the file',
      '  around it is how a two-line fix acquires a week, and now also how it acquires a bead nobody asked',
      '  for.',
      '- **`blocking` is a promise, not an emphasis.** It means this branch must not merge as it stands, and',
      '  it is the whole of your veto. Keep it for correctness, data loss, a security hole, a broken contract',
      '  with a caller, or a test that does not test what it claims. Style, naming, structure and taste are',
      '  `suggestion` — this repo has no formatter and no style gate, and a reviewer that blocks on taste',
      '  burns a worker’s round and a merge slot for nothing.',
      '- **If you cannot say what the fix would look like, it is a `question`.** Not understanding something',
      '  is a real and cheap thing to say, and it is filed and answered like anything else. Dressing it as a',
      '  `blocking` comment is how a reviewer holds a branch hostage to its own confusion.',
      '- **Few and real beats many and plausible.** Three comments the worker acts on is a good review.',
      '  Eleven guesses is a follow-up bead nobody can triage — and if you marked any of them `blocking`, a',
      '  round spent teaching a worker to ignore you.'
    );
  }

  lines.push(
    '',
    `**And separately from any of that — judge whether this diff delivers ${spec.bead}.** Neither /code-review`,
    `nor the answers above will tell you: read \`bd show ${spec.bead}\`'s acceptance criteria and decide, on`,
    'your own, whether what this pull request does actually satisfies them. That is a different question',
    'from "is this clean code" — a diff can be well-written and still not be the thing the bead asked for,',
    'and that is a `blocking` comment of its own, not something an approval can wave through.'
  );
  ...
  lines.push(
    ...
    '',
    '**Approving is `approved: true` with no blocking comment standing, and that is still what to write',
    'when none does.** Suggestions and questions may stand under an approval — they are not what an',
    'approval is about, and they do not hold a merge. A verdict that approves while carrying a blocking',
    'comment is refused, because the gate would merge the branch while your own comment said it must not.',
    '',
    '**And `approved: false` is not a veto on its own.** What holds this branch is a standing `blocking`',
    'comment and nothing else, so a refusal carrying only suggestions merges anyway rather than waiting on',
    'a round. Do not use the flag as a quiet block: put it in the severity, and use `why` for what you would',
    'still have liked changed.'
  );
  ...
  if (maxRounds) {
    lines.push(
      ...
      `**There are ${maxRounds} rounds, and then this becomes a card for ${owner}.**` +
        ...
        ' A round only ever happens over a `blocking` comment — nothing else sends this branch back — so' +
        ' spending one is you saying, in as many words, that it must not merge as it stands.' +
