 ...
 * a bead may make a noise, and answers with *which* of the reasons it may not:
 * `'addressed'`, `'account'`, `'filtered'` or `'muted'`. That answer reaches the server log and the
 * Android logcat line and stopped there. On screen they are indistinguishable, and so is
 * the case where none of them applies — a bead that rang normally and you missed the
 * buzz. You widen the
 ...
 * value from then and not a live read. `for` is who the question was addressed to,
 * kept only for `'addressed'` and for the same reason again: the label can come off the
 * bead, and a card that had gone quiet for a person it can no longer name would read as
 * the app having done it for no reason.
 *
 ...
 *
 * **Named `hushed` rather than `quiet` on purpose.** "Quiet" in this codebase already
 * means the clock in lib/spaces.js — `isQuiet`, `quietHours`, `quietUntil`, and
 * `test/quiet.mjs` which covers them. This file is the other thing entirely: not when
 * a space is allowed to interrupt, but which beads were already denied the chance, and
 * why. It is the sibling of lib/ringing.js, one file per outcome of the same decision.
 */

import { describeFilter } from './spaces.js';

/** The four answers `quietReasonFor` can give. Anything else is not a kind of quiet. */
const REASONS = new Set(['addressed', 'account', 'filtered', 'muted']);
 ...
 */
export function quietArrival(reason, q, filter, now = new Date(), account = null) {
  return {
    reason,
    at: now.toISOString(),
    space: q?.space || null,
    ...
    // The account this bead's workspace belongs to, and only for the account half —
    // *this is in your work account* is the sentence that card needs, and the switcher
    // in the top bar is the lever it points at. Kept from the arrival like everything
    // else here: the split can be re-drawn afterwards, and a card that had gone quiet
    // for an account it can no longer name would read as the app having done it for no
    // reason. Written by the caller, which is the only side that holds the roster.
    account: reason === 'account' ? account || null : null,
    // And only for the addressed one, for the same reason and with more to say: *Bob
    // was asked this* is the whole explanation, and it is the one of the three that
    // names a person rather than a setting. Kept from the arrival like everything else
    // here — the label can be taken off the bead afterwards, and the card would then
    // have arrived quietly for no reason anybody could see.
    for: reason === 'addressed' ? (q?.addressees || []).slice(0, 4) : null,
  };
}
...
export function arrivedQuiet(quiet, key) {
  ...
  return {
    ...
    // Absent on every record written before accounts existed, which reads as a card that
    // says "this arrived quietly" and names no account — one sentence shorter, and true.
    account: typeof rec.account === 'string' ? rec.account : null,
    // Null rather than `[]` for a record written before this existed, so the card can
    // tell "addressed to nobody I can name" from "addressed to these people" and say
    // the shorter sentence instead of a longer one with a hole in it.
    for: Array.isArray(rec.for) && rec.for.length ? rec.for.filter((h) => typeof h === 'string') : null,
  };
}
