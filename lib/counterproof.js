 ...
 * idea than either racing itself. Like `bin/b7e-gate`'s own use of this lock, this calls
 * `lock.release()` directly on every path, normal or signalled, as well as the
 * `onExit`-returned disarm — the same two-call shape `scripts/helpers/chrome.mjs`'s
 * `teardown` already uses. The disarm alone does not release anything: it marks the job
 * done so the exit handler will not run it, and never runs it itself. `bin/b7e-gate` did
 * call only the disarm, and so left its lock file behind after every clean run — harmless
 * only because the next `acquireLock` read a dead pid and reclaimed it as stale — which is
 * what bc-dgx7.40 fixed, by writing there the shape this file already had.
 */
