 ...
 *
 * **The roster arrives as an argument rather than being read from `cfg` here**, and that
 * is not a taste: lib/agents.js and lib/foundation.js are a cycle — foundation reads
 * `DEFAULT_TOOL_LIST` at module scope and agents imports `mark` back — so whichever is
 * evaluated first decides whether the pair loads at all. An `import` of agents.js from
 * this file is early enough in lib/server.js's import list to reach agents first, and the
 * daemon then dies at boot on `Cannot access 'DEFAULT_TOOL_LIST' before initialization`.
 * The caller already has the roster in hand (`rosterNow()`), so asking for it costs
 * nothing and this file needs to know nothing about config. See bc-u4na for the cycle.
 *
