 ...
 * heading; a `card` (lib/card.js, bc-7qo.9) is a question the daemon raised whose entire
 * life is a report and a tap — a merge sweep card, an advocate's proposal. Put any of them
 * on a UAT list and the release agent goes looking for a feature that does not exist.
 *
 ...
 *
 * **Nor is `human`, for the same reason `unendorsed` is not: it is pinned by a test.**
 * test/promotework.mjs asserts `whyNotWork({labels:['human','p0']}) === ''` — a bead that
 * went to Adam mid-flight is still work, because answering it did not close it, building
 * did. `card` is the narrower, correct claim: not "Adam looked at this" but "closing this
 * was the whole of what it was for", and only the filer can say that truthfully.
 */
export const NOT_WORK = [SHIP_LABEL, PROMOTE_LABEL, CONTAINER, CARD_LABEL];
