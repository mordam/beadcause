 ...
 */
export function ranToken(model) {
  const id = String(model ?? '').trim().toLowerCase();
  if (!id || NOT_A_MODEL.has(id)) return '';
  return modelFamily(id) || label(id);
}

/** `ran:opus`. `''` for a model this cannot make a label out of — never `ran:`. */
export function ranLabel(model) {
  const token = ranToken(model);
