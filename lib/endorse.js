 ...
 *
 * The one exception is you. Tapping "work on this" on the phone **endorses the bead and
 * then opens it** (`POST /api/session`, lib/server.js): you are present and choosing,
 * and a refusal there would send you to another screen to press a button and come back.
 * That is what `endorse` below is for — nothing else in the daemon takes the marker off.
 *
 ...
 * nobody's yet, and a ship bead is nobody's *ever* — only a deploy closes one, and a deploy
 * is a tap (lib/shipbead.js). Passed at the call site in lib/advocate.js so the survey reads
 * truthfully, *and* forced on by `Bd.ready` so a caller cannot ask for work it may not have.
 *
 * `ship` is here rather than in lib/shipbead.js's own list because this is the list every
 * queue already consults, and a second one would be a second place to forget.
 */
export const QUEUE_EXCLUDED = ['human', UNENDORSED, SHIP_LABEL];
