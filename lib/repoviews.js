...
/**
 * What the daemon puts on the 502 it answers when a generator failed with nothing held.
 *
 * The status is right — the failure is in a script this daemon spawned on another repo's
 * behalf, not in what was asked of it — but public/report.js files a sev2 P0 for every
 * 5xx that does not say otherwise, and "deluvia's board script timed out" is not "a
 * function of the daemon has stopped working". So the response says which it is, the way
 * bin/router.js's swap-drain 503 does. The browser reads the header, not this constant:
 * public/ has no import bridge to lib/, so the literal is repeated there on purpose and
 * test/repoviews.mjs is what keeps the two in step. bc-3wf1r; bc-xl7n.134 is the precedent.
 */
export const GENERATOR_HEADER = 'x-beadcause-view-generator';
export const GENERATOR_CODE = 'view-generator';
 ...
 *
 * A third chokepoint for lib/timing.js, beside `Bd.run` and the `gh` call in lib/pr.js: it
 * charges this child's wall time to whichever request is in scope, so a slow generator is
 * counted as the subprocess it is rather than as event-loop time nobody spent. Without it,
 * `/api/views/<ws>/<id>/data` always derived `warm` — see the note on `temperature` in
 * lib/timing.js — and a 30-second generator logged as this handler having run that long.
 */
