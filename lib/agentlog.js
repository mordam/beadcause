...
/**
 * A key as a filename. A key is `workspace/id`, and `/` is not a filename.
 *
 * Exported because lib/agentarchive.js names an archived run after the same key and the
 * two names have to agree: a second copy of this expression would put a run's archive
 * under one spelling and its live log under another the first time either changed.
 */
export const slug = (key) => String(key).replace(/[^A-Za-z0-9._-]/g, '_');

/** One file per bead. */
export function logPath(key) {
  return path.join(LOG_DIR, `${slug(key)}.log`);
}
 ...
 *
 * **Call `archiveAndReset` in lib/agentarchive.js instead, and only ever that.** This
 * function is the destructive primitive it is built on: it removes the previous run
 * outright, which is right for the pane and is exactly why the record of what an agent did
 * on a bead used to survive only until the next dispatch at that bead (bc-eqn1.7). The
 * archive copies the body and chains its provenance *before* this runs, so a second call
 * site here is a run destroyed with nobody noticing. `test/agentarchive.mjs` reads lib/
 * and fails on one.
 */
...
/** One line, bounded. Exported because lib/transcript.js renders into this format too. */
export const clip = (s, n = 300) => {
  ...
  if (event.type === 'assistant' || event.type === 'user') {
    ...
    for (const part of content) {
      if (part.type === 'text' && part.text?.trim()) out.push(clip(part.text, 600));
        ...
        // A stream carries the reasoning; a stored transcript keeps only its
        // signature, so the text is usually absent. Either way the fact that it is
        // thinking is the useful part — it is the difference between working and hung.
        const said = clip(part.thinking, 400);
        out.push(said ? `✻ ${said}` : '✻ thinking');
