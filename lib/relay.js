...
/**
 * `dept:` — the prefix a department key carries, beside `REPO_PREFIX` (lib/repos.js) and
 * `SUPERSEDE_PREFIX` (lib/superseded.js).
 *
 * **Nothing enforces it on `cfg.relays`, and nothing should.** `departmentOf` below reads
 * no prefix at all: it matches *any* bead label against the department keys, and the
 * shipped `deluvia` keys are literally the strings `dept:story`, `dept:design`. That is
 * fine while the keys are written in the config file on this Mac — they are yours.
 *
 * It stops being fine the moment a *branch* can write them, which is what
 * `lib/relaydefs.js` makes possible: a repo declaring `departments: { agent-filed: … }`
 * would capture every `agent-filed` bead in that checkout into a chain of its own
 * choosing. `bd ready` already excludes `human`, `ship`, `container` and `unendorsed`;
 * nothing else is excluded. So a repo-defined key must start with this, `cfg` keys stay
 * unvalidated, and that asymmetry is the whole authority line.
 */
export const DEPT_PREFIX = 'dept:';

/**
 * What the filer puts on a review packet — a **beadcause** constant, not part of a
 * definition and not a per-workspace choice. lib/config.js already says why:
 * *"`needs-approval` without `human` is always a bug — the pair is what makes a bead
 * answerable from a lock screen."*
 *
 * It lives here rather than in the definition because of what the alternative costs.
 * `relayBrief` (lib/session.js) renders the `bd label add <the-new-id> …` lines only
 * under `chain.packet.length ?`, so a definition that states no packet produces a chain
 * whose filer files a review packet with **no** `needs-approval`/`human` pair on it — an
 * approval that never reaches the phone, with nothing raised anywhere. That is survivable
 * while every definition is one you wrote; it is not survivable once a definition can
 * come from a repo file that may simply not mention it, under whole-definition
 * replacement. So the default is not the definition's to omit.
 */
export const PACKET = ['needs-approval', 'human'];

/**
 * The packet for a workspace: `cfg.relays[<ws>].packet` where the config states one,
 * `PACKET` otherwise. The one override, and it is deliberately *only* in `cfg` — a repo
 * file that names `packet:` is refused by lib/relaydefs.js rather than honoured.
 *
 * A stated `packet: []` is kept as stated. Emptying it is a thing the config on this Mac
 * is allowed to say; not mentioning it is not the same sentence.
 */
export function packetFor(cfg = {}, workspaceName = '') {
  const entry = relayOptions(cfg)[String(workspaceName || '').trim()];
  const stated = entry && typeof entry === 'object' ? entry.packet : undefined;
  return stated === undefined || stated === null ? [...PACKET] : list(stated);
}
 ...
 *
 * A **label that is itself a department key** wins where the bead carries one, because
 * that is the routing label `APPROVAL_PIPELINE.md` defines and the thing a human actually
 * set. Note the shape of that sentence: this reads no prefix and never has. The key *is*
 * the label — deluvia's keys are the strings `dept:story`, `dept:design` — and any label
 * matching any key routes the bead. `DEPT_PREFIX` is the convention that keeps that safe,
 * enforced on repo-defined keys and not on yours; see the constant.
 *
 * Failing that, the department that *staffs* this role — `members`, not `check`, so clio
 * checking a Design deliverable is still a Story agent and a Design bead assigned to clio
 * is still Design. Null when neither answers, which is every executive role and every
 * unstaffed one.
 */
...
/**
 * The relay this bead would run **under a definition you already hold**, or null.
 *
 * The body of `chainFor` with the lookup taken off the front, so that a definition can
 * come from somewhere other than `cfg`. lib/relaydefs.js reads one out of a checkout;
 * this is what turns it into a chain, and it is deliberately the *same* function that
 * turns a `cfg` entry into one — a second derivation would be a second §6 loop to get
 * wrong, and the two disagreeing is the silently-wrong-chain failure this file's header
 * argues against.
 *
 * `packet` is an option rather than a field of `def` because it is not the definition's
 * to state — see `PACKET`. Omitted, it is the constant, which is what a repo-defined
 * definition always gets; `chainFor` passes what `cfg` says for the workspace.
 *
 * Null is the ordinary answer and means *dispatch this bead exactly as before*: no
 * definition, an assignee that is not a role, a role with no department, or an executive
 * role. Every one of those has to leave the brief unchanged to a character, because they
 * are all of deluvia and all four other workspaces.
 */
export function chainIn(def, workspaceName = '', bead = {}, { packet = PACKET } = {}) {
  if (!def) return null;
  ...
  return {
    ...
    profiles:
      def.profiles && typeof def.profiles === 'object'
        ? Object.fromEntries(Object.entries(def.profiles).map(([k, v]) => [clean(k), String(v || '')]))
        : {},
...
/**
 * The relay this bead would run under the **config on this Mac**, or null. Unchanged in
 * signature and in answer: every existing expectation in test/relay.mjs holds over it,
 * which is the cheapest available proof that lifting the body out changed nothing.
 */
export function chainFor(cfg = {}, workspaceName = '', bead = {}) {
  return chainIn(relayFor(cfg, workspaceName), workspaceName, bead, { packet: packetFor(cfg, workspaceName) });
}
 ...
 *
 * `profiles` overrides the template for one role, and it exists because the pattern is
 * already not universal: eighteen of deluvia's nineteen agents live under
 * `ai-context/agents/<name>/<name>.md` and herald — the one wired as a real Claude Code
 * subagent — lives at `.claude/agents/herald.md` instead (charter §10). A path in a brief
 * that resolves to nothing is worse than no path at all: the session opens it, gets
 * nothing, and has to decide on its own whether the role is real.
 */
export function profilePath(relay, role) {
  const who = clean(role);
  if (!who) return '';
  const override = relay?.profiles ? relay.profiles[who] : '';
