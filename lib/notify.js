...
export async function pushSyncTrouble(cfg, rows) {
  ...
  // A conflict and a stuck sync both need a person; a plain failure does not. Ordered
  // so the louder word wins the title when a tick manages to produce both.
  const needsHands = conflicted.length || stuck.length;
    ...
    message: minimal
      ...
      : [
          conflicted.length
            ...
            : stuck.length
              ? // Says the opposite of the failure message on purpose. The whole reason
                // this state exists is that the sentence below it — "it retries on every
                // interval" — was true-sounding and wrong for 73 ticks running.
          ...
          //
          // A stuck row gets a *different* command, because the obvious one is the one
          // that has already failed every two minutes for a week. `bd dolt commit` has
          // been tried by then (see lib/sync.js), so what is left to suggest is the half
          // a daemon may not do on its own: the working set differs from HEAD in a way
          // `dolt status` cannot show you, and resetting the table Dolt names in the
          // error is what clears it.
          ...rows
            .filter((r) => r.dir)
            .map((r) =>
              r.stuck
