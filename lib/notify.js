...
export async function pushQuestion(cfg, q) {
  const n = cfg.ntfy || {};
  if (!n.enabled || !n.topic) return { skipped: true };
 ...
 *
 * - **It says what kind of decision it is before you read a word of it.** "⚖️ console
 *   asks to change what it is" and "climative · cl-abc" are different enough on a
...
export async function pushFoundationRequest(cfg, q) {
  ...
  const link = `${cfg.baseUrl}/#${encodeURIComponent(q.key)}`;
  const minimal = ntfyDetailFor(cfg, q.workspace) === 'minimal';
  const agent = q.amendment?.agent || 'an agent';
