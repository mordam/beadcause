...
export async function pushQuestion(cfg, q) {
  ...
  // mean nothing.
  //
  // bc-7qo.20: the body used to carry `response` and no `option`, so `answerShape` in
  // lib/server.js had no id to look up, `picked` came back null, and the answer took
  // the ordinary closing path whatever the option said about itself — a `closes:
  // false` commission closed anyway, and a `defers: true` "not yet" lost the card
  // outright, which is the one thing it exists to prevent. Both fields go now, the
  // same shape lib/slack.js already sends (`response: option.response, option:
  // option.id`) — the id is not new trust: the server re-reads the bead and resolves
  // the option's meaning off it (`optionById` in lib/decision.js), so a stale or
  // forged id in the body decides nothing on its own.
  ...
  if (n.actionButtons && !minimal) {
    for (const o of opts.slice(0, 3)) {
      actions.push({
        ...
        body: JSON.stringify({ workspace: q.workspace, id: q.id, response: o.response, option: o.id }),
        clear: true,
      });
    }
  }
