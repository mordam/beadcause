...
export function prunePublished(published, max = PUBLISHED_MAX) {
  ...
  // `at` is millisecond-resolution; a publish burst can tie. Untied, a losing
  // just-published record would be evicted as if it were the stale one — the exact
  // "second page" this cap exists to prevent. Tie-break on the record's own key
  // (the document path) so eviction is deterministic, not accidental object order.
  rows.sort((a, b) => String(b[1]?.at || '').localeCompare(String(a[1]?.at || '')) || String(a[0]).localeCompare(String(b[0])));
