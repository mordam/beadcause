 ...
 * `google-client-secret.key` beside it, which is named on the same reasoning.
 */
export const DEFAULT_TOKEN_FILE = 'confluence.key';

/** The file the API token is read from — yours if you named one, ours if you did not. */
export const apiTokenFile = (cfg = {}) =>
  cfg.confluence?.apiTokenFile || path.join(CONFIG_DIR, DEFAULT_TOKEN_FILE);
 ...
 */
export function apiToken(cfg = {}, { read = (f) => fs.readFileSync(f, 'utf8') } = {}) {
  const fromEnv = process.env.BEADCAUSE_CONFLUENCE_TOKEN?.trim();
  if (fromEnv) return fromEnv;
  try {
    return read(apiTokenFile(cfg)).trim() || null;
  } catch {
    return null;
  }
}
 ...
 *
 * The same hole `secretFileWarning` covers in lib/auth.js: `apiTokenFile` is yours to
 * point anywhere, and pointing it at `~/.config/beadcause/confluence-token.txt` puts a
 * credential in a file that directory commits after every write. Not refused — that
 * would turn a working integration off over a filename — said out loud instead.
 */
export function tokenFileWarning(cfg = {}) {
  if (!cfg.confluence) return null;
  const file = apiTokenFile(cfg);
  const rel = path.relative(CONFIG_DIR, file);
  if (!rel || rel.startsWith('..') || path.isAbsolute(rel)) return null;
  if (/\.(key|secret)$/.test(rel)) return null;
  return `${file} is inside the config repo and not on its denylist, so the token in it WILL be committed — give it a name ending .key or .secret, or move it out of ${CONFIG_DIR}`;
}
...
/** Basic auth, which is what Atlassian Cloud wants for an account API token. */
 ...
 * went wrong.
 */
async function call(conf, method, endpoint, body, { fetchImpl = fetch } = {}) {
  ...
  try {
    res = await fetchImpl(url, {
      method,
      headers: {
        authorization: authHeader(conf.email, conf.token),
    ...
    throw new ConfluenceError(`could not reach ${conf.site} — ${err.message}`, 502);
  }
  ...
  try {
    ...
    /* left null: an HTML error page is not a body worth quoting back */
  }
  if (!res.ok) {
    const said = parsed?.errors?.map((e) => e.title || e.detail).filter(Boolean).join('; ');
