 ...
 * bead applies `human`, and `human` is what `bd.listAgent` excludes — which is the read
 * `queueFor` is fed, and the only one that decides whether anything merges
 * (lib/mergequeue.js). So one card here does not annoy one bead, it takes that pull request
 * out of the queue's sight for ever, in silence: an empty sweep describes as the empty
 * string and logs nothing at all. (bc-uxrix added a second read for the queue's *own*
 * cards, which is why such a bead is at least counted as cover by the strand report now —
 * but that read changes nothing here: this flag leaves `merge-queue` on, and both the
 * queue and the reclaim select against that label, so the bead is still unmergeable and
 * still unreclaimable.) Measured 2026-08-16
