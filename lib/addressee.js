 ...
 * bead — `created_by` is `cfg.actor`, the literal string `beadcause` on every machine —
 * so the addressee cannot be derived at read time. The machine doing the *asking*
 * knows, so it says so at write time, and the label rides the sync to the other five.
 *
