...
something refused and somebody has to read it.`,
    nodes: [
      ...
      },
      {
        ...
        label: 'the reviewer',
        detail:
          'Reads the diff nobody else reads. Its verdict — the comments it raised, whether it approved, and if not why — is a marked block on the merge-bead, so the worker can answer it and the next round can read the answers back. **The queue now waits for it** where a workspace asks for one (`reviewRequired`): no verdict is a wait rather than a refusal, comments open the worker’s window again, and a refusal or two rounds without agreement becomes a card. **What is still missing is the window** — nothing opens the reviewer on a delivered pull request yet, which is why the switch is off in every workspace: on, with nothing reviewing, is a queue that stops rather than a queue that waits.',
        agent: 'review-advocate',
        source: ['lib/reviewadvocate.js', 'lib/reviewgate.js'],
      },
      {
        id: 'mergequeue',
        kind: 'code',
    ...
    edges: [
      ...
      { from: 'mergebead', to: 'review', label: 'where a workspace asks for one' },
      { from: 'review', to: 'mergequeue', label: 'approved' },
      { from: 'review', to: 'blocked', label: 'refused / two rounds' },
...
built. Everything is loaded once and kept: a tab tap is a repaint, not a fetch.`,
    nodes: [
      ...
      },
      {
        ...
        kind: 'device',
        label: '📜 Ledger',
        detail: 'Every bead the space has ever had, open and closed, most recently touched first — with the door back into the conversation that decided it.',
        source: ['public/history.js'],
      },
      {
        id: 'admin',
        kind: 'device',
