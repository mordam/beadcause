 ...
 *
 * beadcause is 90-odd modules, 83 routes and five agent kinds, and the README describes
 ...
 * task, and naming one of the five there would be a lie that reads as precision — the
...
edited it, and pressed the button.`,
    nodes: [
      ...
      },
      {
        ...
        kind: 'code',
        label: 'bd create ×N',
        detail: 'Filed with their dependencies, under the P0 they belong to, labelled for the repo they are about.',
...
the queue is genuinely finished.`,
    nodes: [
      ...
      },
      {
        ...
        detail: `In order: no P0 above it · priority floor · names no checkout this workspace can work in ·
...
over one would go on to propose *more* work beside it.`,
  ...
  {
    id: 'epic',
    title: 'A P0 you own — planned, not worked',
    icon: '🧭',
    trigger: 'You own a P0. The advocate opens its supervisor rather than a worker.',
...
is arguing about a queue nobody agreed to, and this one is \`writes: true\` because a P0
    ...
    nodes: [
      {
        ...
        kind: 'human',
        label: 'you own a P0',
        detail: 'Priority and ownership are yours. No agent may raise a P0, own one, or change who owns one.',
      ...
      },
      {
        ...
        detail: 'Everything it files goes under the P0 with `--parent`. A non-P0 with no P0 above it is not workable, by design.',
        source: ['lib/underp0.js'],
      },
      {
        id: 'plan',
        kind: 'store',
      ...
      },
      {
        id: 'waiting',
        kind: 'store',
        label: 'what the P0 is waiting on\n— one sentence, in notes',
        detail:
          'Not decoration. A P0 that has not moved in a week and a P0 quietly progressing are identical from outside, and the inbox card has a slot for exactly this.',
        source: ['lib/epicadvocate.js'],
      },
      {
        id: 'promote',
...
it. So the worker merges its own pull request through GitHub — GitHub serialises the
merges — and the card on your phone is now the *exception*: something stopped this landing
on its own.`,
    nodes: [
      {
        id: 'deliver',
    ...
    edges: [
      ...
      { from: 'checks', to: 'blocked', label: 'red / never reported / --review' },
...
export function mermaidFor(flowId, { direction = 'TD' } = {}) {
  ...
  const shape = (n) => {
    const t = JSON.stringify(String(n.label).replace(/\n/g, '<br/>'));
    if (n.gate) return `{{${t}}}`;
    switch (n.kind) {
      case 'agent':
        return `[/${t}\\]`;
