 ...
 * beadcause and not a claim about segregation of duties: `bc-eqn1.1` is where the legal
 * entity and the top-management role are settled (`bc-jlpj` asks which entity the
 * certificate names), and when it lands the names here gain the roles that signed. What
 * this file refuses to do in the meantime is invent an approver, because an approval
 * nobody gave is worse than an approval nobody has recorded.
 *
...
export const REGISTER = Object.freeze([
  ...
  },
  {
    ...
    reviewedOn: '2026-08-15',
    reviewMonths: 12,
    serves: 'ISO/IEC 42001 Clause 4.3 — the scope of the management system, as the thing that is actually enforced.',
    why:
      'It is the nearest thing this repo has to a scope statement, and scope is the first document an auditor reads. ' +
      'bc-eqn1.1 replaces it with a formal one; until then the section that decides enforcement is the one that must not drift.',
  },
  {
    id: 'policy-set',
