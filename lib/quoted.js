 ...
 * tool like this get switched off. So the classifiers below refuse rather than guess: a
 * slashed token with no file extension is neither a path nor a branch until the tree has
 * been asked which it is (`settle`), a `refs/…` or `https://…` token is never either, and
 * a bare number is a count only in one of two named shapes.
 */
