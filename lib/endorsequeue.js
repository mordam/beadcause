...
/**
 * The queue's key on the shared layer — one instance, so no scope after the colon.
 *
 * Every request sweeps every workspace and the space picker narrows the list in the
 * browser (see `/api/unendorsed` in lib/server.js), so there is exactly one queue on
 * this daemon and keying it per space would be a cache that misses every time the
 * picker moves. `<what>:<scope>` with an empty scope is lib/cache.js's own spelling
 * for that case; it is also what makes `queue:` safe to drop by name.
 */
const KEY = 'queue:';
