 ...
 *
 * ## The other deadlock — a hold bead that grows a child, bc-beleq.2
 *
 * A hold bead has no parent, so it *is* its own root (`isRoot` in lib/ownership.js), and
 * a worker following its own brief's "found more work? file the bead" instruction while
 * working the hold lands the new bead under the hold by default — lib/homing.js homes an
 * agent-filed bead under the root the discovering bead descends from, and a root
 * descends from itself. bd refuses to close any bead with open children, so `close`
 * below (called with `overClaim`, which lifts the claim guard and not the children
 * guard) is refused the moment that happens, and the hold that would otherwise have
 * lifted the instant the base went green now never lifts at all. `rootOver` in
 * lib/homing.js skips a hold bead as a home for exactly this reason; this file's part is
 * only `stuckReason` below, so the refusal is said on the bead itself rather than only in
 * a log line nobody is tailing.
 */
...
/**
 * What is said on the bead itself when the base came back but `bd` would not let the
 * hold close — bc-beleq.2's second half.
 *
 * The base-green log line already exists and is exactly the failure this is for: a hold
 * that will not lift is the whole repo stopped, and a console line nobody is tailing is
 * not "said" in any sense that reaches a person. This is the same fact, written where a
 * person actually looks — the bead's own history — so a card that still reads "holding"
 * an hour after the base went green carries the reason rather than only a green check on
 * GitHub that contradicts it. Names the refusal verbatim, because "blocked by open
 * issues" is the one detail that turns this into a five-second fix rather than a puzzle.
 */
export function stuckReason(base = 'main', at = '', err) {
  return (
    `\`${base}\` is green again${at ? ` as of ${at}` : ''}, but the merge queue could not close this hold — ` +
    `the close was refused: ${line(err)}. The hold is staying on until this bead can close; if it has open ` +
    `children (most often a bead someone filed \`--from\` it while it was open — bc-beleq.2), close or ` +
    `reparent them and the next tick clears it on its own.`
  );
}
 ...
 * - `comment(id, text)` — optional. Said on the bead when `close` is refused, for
 *   `stuckReason`'s reason: the log line the caller also gets is real but nobody tails
 *   it, and a hold that will not lift is the whole repo stopped. Never called on any
 *   other act, and its own failure is swallowed — a comment that could not be posted
 *   must not turn a stuck hold into an unhandled rejection on top of it.
...
export async function sweepBase({ key, base = 'main', last = null } = {}, {
  ...
  if (verdict.act === 'clear') {
    try {
      ...
      // not lift is exactly the thing nobody would think to go looking for — on the daemon
      // log for whoever is tailing it, and on the bead itself (`stuckReason`) for whoever
      // is not.
      log(`${base} is green again in ${key} but ${open_.id} would not close — ${line(err)}`);
      if (comment) await comment(open_.id, stuckReason(base, at, err)).catch(() => {});
