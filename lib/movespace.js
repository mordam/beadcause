...
/** A refusal, in the shape lib/superseded.js and lib/onewindow.js use. */
const refuse = (sentence, extra = {}) => Object.assign(new Error(sentence), { status: 409, ...extra });
...
export async function planMove(
  ...
  for (let salt = 0; salt < MINT_TRIES; salt += 1) {
    const candidate = mintId(targetPrefix, plan.from, bead, salt);
    if (!(await bd.exists(to, candidate))) {
      rootId = candidate;
      break;
    }
  }
  ...
  for (const moving of plan.moving) {
    ...
    // A descendant keeps its position under the new root, which is what makes the map
    // readable: `bc-ka5y.42.1` under `sp-a1b2c` is `sp-a1b2c.1`, and no second hash is
    // needed because the root's uniqueness carries the whole subtree.
    map.set(moving, `${rootId}${moving.slice(bead.length)}`);
  }
  ...
  for (const moving of plan.moving) {
    const hits = await liveProcessesNaming(plan.from, moving, ps ? { ps } : {}).catch(() => []);
    for (const hit of hits) {
      plan.blockers.push({ id: moving, why: `a window is working it right now (pid ${hit.pid})` });
    }
  }
...
export async function applyMove(bd, plan, { from, to, actor = null } = {}) {
  ...
  // The shape of the source graph has changed under everything that caches it.
  await bd.graph(from, { refresh: true }).catch(() => {});
...
/** The refusal a route throws, so a caller gets a 409 with the sentence on it. */
export const moveRefusal = (plan) => refuse(plan.refused, { blockers: plan.blockers || [] });
