 ...
 *
 * **And the same four facts are carried as fields, not only as that paragraph.** The
 * deploy record's `error` is the message, and the two things that read it — the deploy
 * strip and the ntfy push — used to render it as one blob, the push truncating it at
 * 300 characters. What a truncation eats first is the tail, and the tail is the fix. So
 * `label`, `program`, `plist` and `fix`/`fixCommand` are their own keys: a screen can
 * put each under a heading, a push can send three short lines that all survive, and
 * neither has to parse prose to find the command. The paragraph stays for the log,
 * which is the one reader a paragraph suits.
 */
function verdict(problem, label, dir) {
  ...
  return {
    ...
    label,
    program,
    // Where the plist is (or, when there is none, where launchd looked first) — the
    // file you open next, and not something `program` can stand in for.
    plist: problem.installed?.path || null,
    fix: problem.fix || null,
    fixCommand: problem.fixCommand || null,
    message: [
    ...
    return verdict(
      {
        ...
        // No command: this label is not ours, and how it is installed is the deploy's
        // own business. Naming an action we cannot run is still better than naming none.
        fix: 'install the LaunchAgent for this label, or set `launchAgent: false` on this deploy',
        fixCommand: null,
        installed: svc,
      },
      label,
    ...
    return verdict(
      {
        ...
        fix: `read ${svc.path} and say what it runs — a binary plist reads with \`plutil -p\``,
        fixCommand: `plutil -p ${svc.path}`,
        installed: svc,
      },
      label,
      root
  ...
  return verdict(
    {
      ...
      fix: want
        ? `rewrite ${svc.path} to run ${want}, then boot the job out and back in`
        : 'reinstall the service from this checkout, or declare the program with `launchAgent`',
      fixCommand: null,
      installed: svc,
    },
