 ...
 * sentence is not.
 *
 * **Counted in memory, and that is knowingly weaker than it looks here.** This daemon
 * restarted 505 times across the log this was measured on, and 85 of the 97 sync lines
 * in it follow a restart — so a streak almost never gets the chance to accumulate, and
 * the header's argument for not persisting state (a failure that did not survive the
 * restart is not a failure any more) keeps it that way on purpose. That is exactly why
 * `isStuck` above classifies on the *shape* of the error instead of on how many times
 * it has happened: the shapes that will never clear are recognisable on the first tick,
 * with no history at all, which is the only kind of recognition that survives a restart.
 */
 ...
 */
export async function syncOnce(bd, workspace, { recover = true } = {}) {
 ...
 */
export function createSyncer({ bd = null, now = () => Date.now() } = {}) {
