 ...
 *
 * ## And quiet again when it will not stop changing its mind
 *
 * The paragraph above assumes a tracker has incidents. One that does not — that breaks,
 * recovers, breaks and recovers on a two-minute clock — satisfies the transition rule
 * perfectly and defeats its purpose completely: every one of those is a real transition
 * and so every one of them is a push, which on the log this was measured from is
 * nineteen notifications about a single workspace in a single day. That is the
 * swipe-it-away notification arriving by the other door, and worse than the every-tick
 * version, because half of them are recoveries and a recovery carries no urgency at all.
 *
 * So the transitions are counted as well as reported: four inside an hour and the
 * workspace is **flapping**, which is said once and is then the last thing said about it
 * until it holds one way for a clear hour. See `FLAP_AFTER` for why this counts
 * transitions rather than ticks, which is the part that matters and the part that keeps
 * it from ever turning into silence.
 */
...
/**
 * How many times a workspace may break and come back inside one window before beadcause
 * stops telling the phone about each one — bc-y3qk.4.
 *
 * `STUCK_AFTER` above is about a tracker that will not move. This is the opposite
 * complaint and it arrives through the same door. The transition rule is honest about a
 * *sustained* outage — one push when it breaks, one when it returns, silence in between
 * — but it has nothing at all to say about a tracker that does not sustain anything.
 * The log this was filed against carried 23 `[sync]` lines for one workspace, 9
 * recoveries against 10 failures, alternating almost perfectly: **nineteen transitions,
 * nineteen pushes**, about a workspace whose real state all day was "intermittent". That
 * is the same notification-you-learn-to-swipe-away the transition rule exists to
 * prevent, and it is worse than the every-tick version it replaced, because half of
 * those pushes are a recovery and a recovery carries no urgency at all.
 *
 * Four, because the ordinary incident must be untouched. One break and one return is two
 * transitions, and two incidents in an hour is still a Wednesday. Four is the second
 * incident's *recovery*, which is the first moment "it broke" and "it came back" have
 * both stopped being true for long enough to be worth interrupting somebody over — and
 * the moment there is a better sentence available, which is that it is flapping.
 *
 * ## Why this counts transitions and not ticks, which is the whole design
 *
 * The obvious damping is a threshold on consecutive ticks: require N ticks in the new
 * state before it counts as a transition at all. That shape is wrong here and bc-y3qk.7
 * is why. The previous outcome lives in memory and is deliberately not persisted, this
 * daemon restarts constantly (505 times across the log this was measured on, against 97
 * sync lines), and every restart puts `before` back to null. A consecutive-tick counter
 * is reset by each of those, so it would spend most of its life at 1 and **almost never
 * reach its threshold** — which for a tick threshold means the transition is never
 * announced. It fails *silent*, and silence is the failure this file's header spends
 * eighty lines arguing against.
 *
 * A transition counter has the same weakness and the opposite failure. A restart resets
 * it too, so under restart churn it declares nothing and every transition speaks —
 * exactly today's behaviour, which is the state this bead is an improvement on rather
 * than a regression from. It cannot silence anything it has not first counted four
 * noises from, and a sustained outage produces one. So the two beads compose rather than
 * fight: bc-y3qk.7 stops restarts from manufacturing transitions, and each one it
 * removes makes this counter *more* accurate, never less. Neither is waiting on the
 * other, and neither can turn the other into silence.
 */
export const FLAP_AFTER = 4;

/**
 * How long a transition counts against the flap total, and so how long a workspace has
 * to hold one way before it is trusted again.
 *
 * An hour rather than a handful of ticks. The unit that matters is a person's attention
 * — the complaint is nineteen buzzes in a day, not nineteen ticks — so the window is in
 * the same units as the annoyance, and it does not move if somebody changes
 * `sync.seconds`. At the two-minute default it takes thirty quiet ticks to age a
 * transition out, which is the point: a tracker that has been solid for an hour has
 * earned the right to interrupt you again.
 *
 * It doubles as the settle rule, and deliberately has no separate constant. A workspace
 * stops being flapping when its transitions age out, which is one clear hour after the
 * last one — so the card comes down on the same evidence it went up on, rather than on a
 * second threshold that could disagree with the first.
 */
export const FLAP_WINDOW_MS = 60 * 60 * 1000;
...
/**
 * Which one of the tracker's two competing claims a tick owes the shared card,
 * decided from `trouble()` *after* the sweep rather than from what changed on this
 * tick — bc-y3qk.11.
 *
 * `syncStuckEvent`, `syncFlappingEvent` and `syncClearEvent` all name the same key, so
 * whichever is emitted last is the one left on the phone. `broke`/`clearing` only know
 * what changed on one workspace this tick, and the collision this bead is about needs
 * a *second* one: workspace A is stuck — whether it broke on this very tick or was
 * already broken — while some unrelated workspace B recovers, or its flapping quiet
 * hour merely elapses. Neither of those makes A's problem go away, so `trouble()` —
 * read fresh, after the sweep — is the only thing allowed to decide whether the card
 * may clear.
 *
 * `null` means say nothing: nothing this tick changed that the card does not already
 * show, which keeps the "exactly once per episode" rule sweepSync's log already
 * follows true of the phone's card too.
 *
 * ## `flapping`, which `trouble()` cannot answer for — bc-y3qk.15
 *
 * `trouble()` names workspaces whose *state* is failed, conflict or stuck, and a
 * flapping one alternates through `ok`. Worse, the tick it is *declared* flapping on is
 * as likely to be a recovery as a failure — the marks cross `FLAP_AFTER` whichever
 * direction the transition went — so at the moment the flapping card goes up the
 * workspace holding it up can be perfectly `ok` and therefore absent from `trouble()`
 * entirely. That is the same last-emit-wins collision one step over: workspace A's
 * FLAPPING card, whose whole claim is *"the last thing you will hear about this until
 * it holds an hour"*, wiped by an unrelated workspace B's recovery on the same tick or
 * on any tick before A holds.
 *
 * So the clear has to ask two questions, not one, and this is the second: is the
 * flapping card still true of anybody? Passed as the syncer's own currently-flapping
 * rows rather than as the tick's crossings, because the claim outlives the tick that
 * raised it — a workspace stays flapping until its marks age out, and every tick in
 * between is one where somebody else's good news must not speak for it. A flapping
 * workspace clears its own card when it settles: by then it is no longer flapping, and
 * it is itself in `clearing`.
 */
export function syncCardVerdict({ broke, clearing, trouble, flapping = [] }) {
  if (trouble.length) return broke.length || clearing.length ? 'stuck' : null;
  // Nothing further to say: the flapping card is up, it is still true, and the tick's
  // good news is about some other workspace. Saying `clear` here is the bug.
  if (flapping.length) return null;
  return clearing.length ? 'clear' : null;
}
 ...
 */
export function createSyncer({ bd = null, now = () => Date.now(), initial = null, save = null } = {}) {
  ...
  /**
   * workspace name → when each of its recent transitions happened, newest last.
   *
   * Timestamps rather than a count, because what has to be answered is "how many in the
   * last hour" and a bare integer cannot forget. Pruned on every tick and not only on a
   * transition, which is what makes an hour of quiet actually clear it: a workspace
   * sitting `ok` is still being recorded, and that is the tick the ageing has to happen
   * on. Kept beside `last` rather than on the outcome for the same reason `last` exists
   * at all — an outcome describes one tick and this is a fact about several.
   */
  const flaps = new Map();
  /**
   ...
   *
   * And four more fields, for the tracker that transitions too often to be worth one
   * push each — see `FLAP_AFTER`. `flaps` is how many transitions this workspace has
   * made inside the window and `flapping` is whether that has passed the line;
   * `flapped` is true on the single tick it crosses and `settled` on the single tick it
   * comes back under, so a caller can say each of those once without holding any state
   * of its own. `damped` is the one a notifier acts on: this transition really happened,
   * it is on the log and on the monitor, and it is not worth waking anybody for.
   */
  function record(raw) {
    ...
    //
    // Compared against what the *previous tick's `syncOnce` said*, not against the state
    // this function then promoted it to — and that distinction is the whole of bc-y3qk.4's
    // second half. `state` below turns the fifth identical `failed` into `stuck`; if the
    // sixth tick's `failed` is then compared against that stored `stuck`, the two differ,
    // the streak resets to 1, the word falls back to `failed` — and `changed` is true,
    // because the word moved. A workspace failing identically all afternoon therefore
    // oscillated `failed`→`stuck`→`failed` on a five-tick cycle and pushed the phone
    // **every ten minutes, for ever**, while un-declaring itself stuck each time round.
    // That is the same complaint this bead was filed for arriving through the escalation
    // path instead of the transition one, and no damping catches it: those are word
    // changes, and a word change is the one thing that must always speak.
    const same = Boolean(before && (before.rawState ?? before.state) === raw.state && (before.error || '') === (raw.error || ''));
    ...
    // `rawState` is kept beside it so the next tick can ask the question above. It is
    // what `syncOnce` decided from one tick's evidence, and `state` is what this function
    // decided with the history in front of it; nothing downstream reads `rawState`, and
    // nothing downstream should — a screen wants the answer, not the working.
    const out = { ...raw, state, rawState: raw.state, streak };
    ...
    // How often this workspace has changed its mind lately. Pruned first and on every
    // tick — an `ok` tick is what ages the last incident out, and a workspace that only
    // pruned when something happened would stay flagged as flapping until the day it
    // next flapped.
    const at = now();
    const marks = (flaps.get(out.workspace) || []).filter((t) => at - t < FLAP_WINDOW_MS);
    if (transition) marks.push(at);
    flaps.set(out.workspace, marks);

    // Latched by the window rather than by a flag of its own: the marks age out an hour
    // after the last transition and this goes false on its own. One rule, one clock, and
    // no flag that a path could forget to clear. `wasFlapping` is read back off the
    // outcome `last` is holding, which is why the two fields below are written onto it.
    const wasFlapping = Boolean(before?.flapping);
    const isFlapping = marks.length >= FLAP_AFTER;
    const crossed = isFlapping && !wasFlapping;
    const settled = wasFlapping && !isFlapping;
    out.flapping = isFlapping;
    out.flaps = marks.length;

    // **Nothing that has stopped retrying is ever damped, whatever else is true.** A
    // workspace in `conflict` or `stuck` has had the promise of a retry withdrawn, and a
    // tracker that will not settle is exactly where that most needs hearing — a flapping
    // series whose failures quietly turn into a merge conflict must not go out under the
    // sentence "you will not hear about this again". Written as a state test rather than
    // relying on `transition === null`: the word usually moves under a workspace that
    // was already failing, but a flapping one alternates through `ok`, so the same
    // escalation arrives as an ordinary `broke` and would otherwise be swallowed.
    const insists = out.state === 'conflict' || out.state === 'stuck';
    // The tick that crosses the line is the one that gets to name the pattern — unless
    // there is something louder to say on it, in which case that is said instead and the
    // word "flapping" is simply skipped for this episode. `crossed` still latches, so
    // the damping is in force either way.
    const flapped = crossed && !insists;
    const damped = Boolean(transition) && isFlapping && !crossed && !insists;
    ...
    trouble() {
      return [...last.values()]
        ...
        .map((o) => ({
          ...
          stuck: o.state === 'stuck',
          // Not the same claim as either of those two and it does not replace them: a
          // flapping workspace is *currently* failing (it is in this list) and the thing
          // worth knowing about it is that it will probably be fine in two minutes and
          // broken again in four. See `FLAP_AFTER`.
          flapping: Boolean(o.flapping),
          flaps: o.flaps || 0,
          streak: o.streak || 1,
