...
import { containerRoots } from './workspaceroots.js';
 ...
 *
 * Only ever a *guess offered as a default* — the same move question 5 of the wizard makes
 ...
 * there is nothing to approve about it. A **container** workspace root — `~/beads`, and
 * anything else `workspaceRoots` names that holds workspaces rather than being one — is
 * excluded outright: it is the tracker's own tree, `<root>/<workspace>` is named after
 * the workspace by construction, and nothing in it is a repo anybody works in.
 *
 * A root that **is** a workspace is deliberately not excluded, and that is the whole
 * difference `containerRoots` draws. `~/climative.dev/architecture` holds a tracker *and*
 * is a checkout somebody works in every day; excluding it would take the repo out of the
 * search on exactly the install this shape exists for.
 */
export function candidateRoot(cfg = {}, workspaceName = '') {
  const ws = String(workspaceName || '').trim();
  if (!ws) return null;
  const home = os.homedir();
  const containers = containerRoots(cfg);
  const searchIn = [home, cfg.projectRoot, ...(cfg.assetRoots || [])]
    .map((d) => (d ? expandHome(d) : ''))
    .filter(Boolean)
    .filter((d) => !containers.some((beads) => d === beads || d.startsWith(`${beads}/`)));
