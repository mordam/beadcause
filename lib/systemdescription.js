 ...
 *
 * Four sections have no source in this release at all, and they get a seam rather than a
 ...
 *
 * See the header. `unavailable` is not an error state — four of the fourteen sections are
 * unavailable in this release, and that is the honest description of a programme whose
 * commitments, control-environment narrative, changes and incidents nothing has recorded
 * yet. It becomes an error only where something signs.
 */
...
export const SECTIONS = Object.freeze([
  ...
  },
  {
    ...
    heldElsewhere:
      'the service organisation — how it actually communicates its objectives, assesses its risks and evaluates ' +
      'its own internal control. That is narrative, and no register here holds it. What can be said about it is ' +
      'said where it belongs: CC1 to CC5 are stated in full, with what claims to be the documented answer to ' +
      'each, under the criteria section, and repeating them here would be the same seventeen definitions twice.',
  },
  {
    id: 'criteria',
...
/**
 * The four sections nothing in this release derives, and the seam by which they arrive.
 *
 ...
 */
export const SUPPLIABLE = Object.freeze(['commitments', 'environment', 'changes', 'incidents']);
...
/**
 * The criteria section: every criterion in scope, and what claims to meet each.
 *
 * `ENVIRONMENT_CRITERIA` — CC1 to CC5, the restatement of four of the five COSO components
 * — deliberately does *not* get a second pass through here. They are common criteria and so
 * are always in this list already; giving the control-environment section its own copy
 * printed the same seventeen definitions twice, which is why that section names this one
 * instead.
 */
function criteriaSection(ids, register) {
...
const GENERATORS = {
  ...
  environment: (record, { supplied }) => suppliedSection('environment', supplied),
...
export function descriptionProblems(description) {
  ...
  // The common criteria are not a category anybody elects: CC1 to CC9 are in every SOC 2
  // report, and CC1 to CC5 are what the control-environment section is the description of.
  // A scope that omits one is a legitimate thing to hold mid-gap-assessment and is not a
  // legitimate SOC 2 description, which is the difference this says out loud.
  const missing = ENVIRONMENT_CRITERIA.filter((id) => !(description.criteria?.elected || []).includes(id));
  if (missing.length) {
    problems.push(
      `${missing.length} common criteri${missing.length === 1 ? 'on is' : 'a are'} outside the stated scope ` +
        `(${missing.slice(0, 4).join(', ')}${missing.length > 4 ? ', …' : ''}) — CC1 to CC5 are the description of the ` +
        'control environment, risk assessment, monitoring and communication, and every SOC 2 report contains them'
    );
  }
  return problems;
}
