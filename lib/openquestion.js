...
/**
 * A `human` **epic** is not a question, and this is the one exclusion the join makes.
 *
 * Measured against the live beadcause tracker on 2026-08-23, before this line existed: 17
 * open `human` beads, four of which were P0/P2 epics, and those four produced *every*
 * false positive — three unrelated held beads flagged because `bc-9d37`'s and
 * `bc-rfnr.9`'s notes happen to mention them. That is not a coincidence and it is not a
 * tuning problem. A `human` epic is a **standing board card**: the epic *is* the work, the
 * label puts it on the P0 board, and its notes are an advocate's running log that names
 * every bead it has touched this week. A `human` task, bug or decision is the other thing
 * entirely — somebody asking about something, once, and naming what they mean.
 *
 * With the line, the same measurement leaves one flag, and it is the true one: bc-xl7n.77.1
 * ("Do the three Climative network beads belong in this tracker?") on bc-xl7n.77.2.
 *
 * This is the difference between a warning and a row you learn to scroll past, which would
 * cost the feature everything it is for.
 */
const isEpic = (issue) => String(issue?.issue_type ?? '').trim().toLowerCase() === 'epic';
